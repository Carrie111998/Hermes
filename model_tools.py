...
def _is_dispatcher_owned_worker() -> bool:
    """False when ORION_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_orion_home() under the active profile.
...
#   - cli.py, orion_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("ORION_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by ORION_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like orion-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("orion-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (orion-*) include _ORION_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("orion-")
    ...
    # window). Core Orion tools (toolsets._ORION_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from orion_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to orion-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    if _post_tool_call_hook_suppressed.get():
        return
    try:
        from orion_cli.lifecycle import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from orion_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from orion_cli.plugins import _dispatch_pre_tool_call_hooks
        ...
        try:
            ...
            if skip_tool_execution_middleware:
                ...
                from orion_cli.middleware import run_tool_execution_middleware

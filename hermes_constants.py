...
def _root_mount_has_container_runtime(mountinfo: str) -> bool:
    ...
    for line in mountinfo.splitlines():
        ...
        if mount_point != "/":
            continue
        # After the separator the fields are fstype, mount source, and super
        # options. A host backing device such as docker--vg-root may contain a
        # marker by name, so exclude the mount source and pre-separator device
        # fields. Root path, fstype, and super options describe the mounted
        # runtime rather than the host device label.
        match_fields = (
            mount_point,
            fields[3],
            fields[separator + 1],

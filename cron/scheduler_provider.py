...
def fire_overdue_jobs(
    ...
    *,
    ...
    from cron.jobs import _ensure_aware, _orion_now, is_job_runnable, load_jobs
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global ORION_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `orion cron
        ...
        # Heartbeat once before the first sleep so `orion cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `orion cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_orion_home_override()`` + ``use_cron_store()``
        ...
        from orion_constants import set_orion_home_override, reset_orion_home_override
        ...
        for entry in profile_homes:
            ...
            home_token = set_orion_home_override(str(home))
            try:
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `orion cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:

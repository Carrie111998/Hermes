...
def save_jobs(
    ...
    *,
    ...
    "rewrite_skill_refs") -- recorded to CRON_DIR/ATTRIBUTION_LOG_NAME as
    telemetry only, after the real write succeeds. See _record_attribution's
    docstring for the hard isolation rule against the out-of-band authority
    check.
    """
    with _jobs_lock():
...
def remove_job(job_id: str) -> bool:
    ...
    with _jobs_lock():
        ...
        if len(jobs) < original_len:
            ...
            save_jobs(jobs, caller="remove_job", removed_ids={canonical_id})
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    if needs_save:
        save_jobs(raw_jobs, caller="_get_due_jobs_locked", removed_ids=intentionally_removed or None)

...
Write path is the CLI (``orion cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_orion_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`orion cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `orion cron notepad {job_id} delete "

...
def _confirm_adapter_delivery(send_result) -> bool:
    ...
    if type(send_result) is SendResult:
        return type(send_result.success) is bool and send_result.success is True
    legacy = _inert_legacy_send_result_fields(send_result)
    return legacy is not None and legacy["success"] is True
...
def _persist_target_text_receipts(
    ...
    """
    from gateway.platforms.base import TransportReceipt
                    ...
                    if future is None:
                        ...
                        if timeout_handled:
                            ...
                            if type(send_result) is dict:
                                send_success = send_result.get("success") is True
                                ...
                                legacy_fields = _inert_legacy_send_result_fields(
                                    send_result
                                )
                                send_success = (
                                    legacy_fields is not None
                                    and legacy_fields["success"] is True
                                )
                                send_raw_response = (
                                    legacy_fields["raw_response"]
                                    if legacy_fields is not None else None
                                )
                                delivered_message_id = (
                                    legacy_fields["message_id"]
                                    if legacy_fields is not None else None
                                )
                            ...
                            if not send_success:
                                if type(send_result) is dict:
                                    ...
                                    err = (
                                        error_value
                                        if type(error_value) is str else "unknown"
                                    )
                                    shape = "dict"
                                elif type(send_result) is SendResult:
    ...
    try:
        ...
        if (
            ...
            if delivery_error:
                ...
                delivery_outcome = (
                    "unknown" if receipt_outcome == "unknown" else "failed"
                )
            elif unresolved_origin:
                delivery_outcome = "not_configured"
            elif normalized_deliver != "local":
                delivery_outcome = (
                    _receipt_delivery_outcome(str(execution_id)) or "delivered"
                )
        try:

...
def try_register_running_job(
    ...
    *,
    ...
    The ticker passes ``recover_orphaned=True`` so it can self-heal stale
    scheduler-owned guards whose executor future is absent/done after
    fd-exhaustion or pre-cleanup bookkeeping failures (MAX-1354). Manual runs
    pass ``manual=True`` because they legitimately hold the shared guard
    without an executor future while ``run_one_job`` executes synchronously.
    """
    display_name = job_name or job_id
    with _running_lock:
        if job_id in _running_job_ids:
            orphan_reason = (
                _running_guard_orphan_reason_locked(job_id)
                if recover_orphaned
                else None
            )
            if orphan_reason:

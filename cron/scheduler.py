...
def _run_one_job_body(
    ...
    *,
    ...
    try:
        ...
        # alert during an intentional shutdown interruption, never retry an
        # alert after delivery was already attempted, and never alert for a
        # stale worker whose fire claim was taken over by a replacement run
        # (that run owns delivery now -- a second alert here would double-fire
        # on top of it): each case could create a misleading, duplicate, or
        # spurious message.
        if (

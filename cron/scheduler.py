    ...
    *,
    ...
    try:
        ...
        try:
            ...
            # AGENT cron-turn reply transform (P1a): reshape the reply before
            # delivery (e.g. enforce a concise report contract). no_agent jobs
            # ship stdout verbatim and are deliberately skipped. Best-effort —
            # a broken plugin must never block delivery.
            if success and deliver_content and not job.get("no_agent"):
                try:
                    from hermes_cli.plugins import get_transformed_cron_reply
                    deliver_content = get_transformed_cron_reply(
                        deliver_content, job.get("id", "")
                    )
                except Exception:
                    logger.debug("cron reply transform skipped", exc_info=True)

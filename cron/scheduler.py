...
def _run_job_script(script_path: str, *, cwd: Optional[str] = None) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    try:
        ...
        env = _make_run_env({})
        # The Windows venv overlay is synthesized from the interpreter's own
        # pyvenv.cfg after provider-secret sanitization. Apply it afterward so
        # _make_run_env's ambient VIRTUAL_ENV stripping does not discard the
        # explicit launcher-bypass contract.

...
# restarts (incident: jarvis board-pm-triage-* jobs, 2026-08-02).
_running_since: dict = {}
    ...
    try:
        ...
        def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor):
            ...
            with _running_lock:
                ...
                # Pending sentinel closes the future-record race: the sweep
                # never sees a claim with NO future marker — it is
                # _FUTURE_PENDING until pool.submit returns, then the real
                # future replaces it below.

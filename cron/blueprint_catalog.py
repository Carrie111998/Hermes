...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``orion://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``orion://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"orion://blueprint/{quote(blueprint.key)}{qs}"

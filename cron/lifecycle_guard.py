...
``orion gateway restart`` (or ``launchctl kickstart ai.orion.gateway``
or ``systemctl restart orion-gateway``).  When the cron fires, the
...
``orion cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``orion gateway``, ``launchctl ... orion-gateway``,
``systemctl ... orion-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_ORION_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``orion gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `orion gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:orion\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a orion-gateway label. macOS launchd
    # labels look like `ai.orion.gateway` / `orion-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated orion services (e.g.
    # `launchctl unload ai.orion.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.orion.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a orion-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\borion[.\-]?gateway)"
    # Branch D: pkill / kill targeting the orion gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.orion.gateway-... \` / `  -- ...`,
...
# for `systemctl restart orion-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart orion-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.orion.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart orion-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        grep -c 'systemctl restart orion-gateway' /var/log/syslog
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart orion-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".orion"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<ORION_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.orion/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_orion_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_orion_home() falls back to Path.home(), which raises when
        # neither ORION_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.orion/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `orion gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `orion gateway restart` from a shell outside "

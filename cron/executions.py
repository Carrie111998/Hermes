...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    if "fire_identity" not in execution_columns:
        conn.execute("ALTER TABLE executions ADD COLUMN fire_identity TEXT")
    if "error_kind" not in execution_columns:
        conn.execute("ALTER TABLE executions ADD COLUMN error_kind TEXT")
    if "receipt_state" not in execution_columns:
        # NULL identifies executions written before the receipt contract.
        conn.execute("ALTER TABLE executions ADD COLUMN receipt_state TEXT")
...
def finish_execution(
    ...
    with _transaction() as conn:
        cur = conn.execute(
            """UPDATE executions SET status=?, finished_at=?, error=NULL, error_kind=?
               WHERE id=? AND status IN ('claimed','running')""",
            (status, now, category, execution_id),
        )
        if cur.rowcount != 1:
            return None
    ...
    with _transaction() as conn:
        ...
        for row in rows:
            ...
            if _owner_is_live(int(row["pid"]), row["process_started_at"]):
                continue
            cur = conn.execute(
                """UPDATE executions SET status='unknown', finished_at=?,
                       error=NULL, error_kind='interrupted'
                   WHERE id=? AND status IN ('claimed','running')""",
                (now, row["id"]),
            )

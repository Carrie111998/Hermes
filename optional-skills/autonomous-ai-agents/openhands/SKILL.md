...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Orion Agent
license: MIT
platforms: [linux, macos]
metadata:
  orion:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, orion-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Orion.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Orion-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `orion-agent` (Orion subagents via `delegate_task`).

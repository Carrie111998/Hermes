...
The design is **Orion-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
orion skills install official/security/unbroker
```

Then start a new Orion session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Orion home directory (`0600`, optionally

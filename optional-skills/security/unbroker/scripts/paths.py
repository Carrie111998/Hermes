...
All per-subject data lives under PDD_DATA_DIR (default: $ORION_HOME/unbroker),
which is the same trust boundary Orion uses for .env and OAuth tokens.
"""
...
def orion_home() -> Path:
    return Path(os.environ.get("ORION_HOME") or (Path.home() / ".orion"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else orion_home() / "unbroker"

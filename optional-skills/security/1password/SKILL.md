...
version: 1.0.0
author: arceus77-7, enhanced by Orion Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Orion terminal calls (desktop app flow only)
...
### Service Account (recommended for Orion)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ORION_HOME:-~/.orion}/.env` (the skill will prompt for this on first load).
...
## Orion Execution Pattern (desktop app flow)

Orion terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/orion-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/orion-op.sock"

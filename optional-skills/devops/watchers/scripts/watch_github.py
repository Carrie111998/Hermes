...
Usage (via cron with --no-agent):
    ...
    orion cron create orion-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ORION_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name orion-issues --repo Orion/orion-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Orion .env file
(``${ORION_HOME:-~/.orion}/.env``) to avoid the 60 req/hr

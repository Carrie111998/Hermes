...
Usage (via cron with --no-agent):
    ...
    orion cron create my-feed \\
      --schedule "*/15 * * * *" --no-agent \\
      --script "$ORION_HOME/skills/devops/watchers/scripts/watch_rss.py" \\
...
def main() -> int:
    ...
    try:
        req = urllib.request.Request(args.url, headers={"User-Agent": "Orion-Watcher/1.0"})

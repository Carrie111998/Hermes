...
Usage:
  ...
  <server_name> matches the files in $ORION_HOME/mcp-tokens/<server>.json etc.
...
def _orion_home():
    # Prefer Orion' own resolver (profile-safe); fall back to env then ~/.orion.
    try:
        from orion_constants import get_orion_home
        return str(get_orion_home())
    except Exception:
        return os.environ.get("ORION_HOME") or os.path.expanduser("~/.orion")


def _tokens_dir():
    return os.path.join(_orion_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "orion-diag", "version": "1.0"}},
        },
        headers={

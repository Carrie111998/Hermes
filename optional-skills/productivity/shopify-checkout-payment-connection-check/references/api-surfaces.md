...
Use `orders` and nested `transactions` with `read_orders`. Aggregate only transaction kind, status, gateway label, and error code. Do not output order IDs, customers, addresses, products, amounts, or payment details.

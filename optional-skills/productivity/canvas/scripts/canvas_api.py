#!/usr/bin/env python3
"""Canvas LMS API CLI for Orion Agent.
...
def _check_config():
    ...
    if missing:
        orion_env = os.path.join(
            os.environ.get("ORION_HOME", os.path.expanduser("~/.orion")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {orion_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Orion Agent"
    )

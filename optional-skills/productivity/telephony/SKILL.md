...
This optional skill gives Orion practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${ORION_HOME:-~/.orion}/.env`
...
It does **not** turn Orion into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Orion memory
...
### 1) "I want Orion to own a real phone number"
Use **Twilio**.
...
- pairs well with Orion `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${ORION_HOME:-~/.orion}/.env`
...
Then save credentials into Orion:
...
3. Buy it and save it into `${ORION_HOME:-~/.orion}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Orion calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Orion's existing `text_to_speech` support.

Use this when:
- you want the call to use Orion's configured TTS voice rather than Twilio `<Say>`
...
Recommended Orion TTS -> Twilio Play workflow:

1. Generate the audio with Orion `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Orion to create the message audio with `text_to_speech`
...
Important note:
- Orion TTS is great for prerecorded outbound messages
...
- Orion STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Orion memory.
...
- Do not store arbitrary third-party phone numbers in Orion memory.
...
3. persist that number to `${ORION_HOME:-~/.orion}/.env`

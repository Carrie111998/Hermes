...
- Skill helper paths:
  - `${ORION_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ORION_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ORION_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client orion
```
...
```bash
PUBLISH="${ORION_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client orion
```
...
```bash
DRIVE="${ORION_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `orion`)    |

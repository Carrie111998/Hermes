...
const TIERS = Object.freeze({
  full: Object.freeze({ minimumCandidates: 100, minimumDomains: 60, minimumLanes: 10 }),
  minimum: Object.freeze({ minimumCandidates: 40, minimumDomains: 25, minimumLanes: 8 }),
});
const LANES = new Set([
...
Tiers:
  full     100 candidates, 60 root domains, 10 coverage lanes (default)
  minimum  40 candidates, 25 root domains, 8 coverage lanes`);
}
...
function validate(ledger, options = {}) {
  ...
  const candidates = Array.isArray(ledger) ? ledger : ledger?.candidates;
  const errors = [];
  if (!Array.isArray(candidates)) return { ok: false, errors: ["Ledger must be a JSON array or an object with a candidates array."] };
  ...
  candidates.forEach((candidate, arrayIndex) => {
    ...
    for (const field of ["id", "root_domain", "lane", "route", "evidence_state", "why_relevant", "next_action", "cost_or_disclosure", "quality_risk"]) requiredText(candidate, field, index, errors);
    ...
    const rootDomain = String(candidate.root_domain || "").toLowerCase().replace(/^www\./, "");
    if (target && rootDomain && target.hostname.toLowerCase().replace(/^www\./, "") !== rootDomain) errors.push(`candidate ${index}: root_domain must match target_url.`);
    ...
    if (candidate.evidence_state.startsWith("verified_") && !evidence) errors.push(`candidate ${index}: verified evidence requires a valid evidence_url.`);
  });
  ...
  return {
    ok: errors.length === 0,
    errors,
    summary: { ...requirements, candidateCount: candidates.length, rootDomainCount: domains.size, laneCount: lanes.size, evidenceStates: states },
  };
}

...
orion kanban watch --tenant <project-slug>

# Snapshot of the board
orion kanban list --tenant <project-slug>
orion kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
orion kanban stats --tenant <project-slug>

# Visual dashboard (browser)
orion dashboard

# Inspect a specific task (includes comments + events)
orion kanban show <task-id>

# Follow a single task's event stream
orion kanban tail <task-id>
```

Verify available subcommands with `orion kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `orion kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `orion kanban block <id>` then `orion kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `orion kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `orion kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `orion profile list` (profile exists?), `orion status` (gateway/dispatcher up?), and verify tenant. |
...
orion kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
orion kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(orion kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
orion kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
orion kanban block <task-id>
orion kanban archive <task-id>

# Diagnose what happened
orion kanban show <task-id>      # task body, comments, recent events
orion kanban tail <task-id>      # follow the live event stream
orion kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `orion kanban create` for the director
...
while true; do
    clear
    orion kanban list --tenant <slug>
    echo "---"
    orion kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `orion kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `orion profile list` and start

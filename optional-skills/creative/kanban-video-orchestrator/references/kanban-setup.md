...
the actual `setup.sh` that creates the project workspace, configures Orion
...
> [Orion/kanban-video-pipeline](https://github.com/zacharyjleach-stack/Aries).
...
2. **Create profiles** — `orion profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.orion/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `orion kanban create` assigned to the director
...
```bash
orion profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.orion/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.orion/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Orion
...
Each profile gets a `SOUL.md` at `~/.orion/profiles/<name>/SOUL.md` that
...
```bash
orion kanban create "Direct production of <video title>" \
    --assignee director \
...
the Orion `.env` (`${ORION_HOME:-$HOME/.orion}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _orion_env="${ORION_HOME:-$HOME/.orion}/.env"
    if grep -q "^${var}=" "$_orion_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_orion_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_orion_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY orion ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY orion OPENROUTER_API_KEY || exit 1
# ...
```

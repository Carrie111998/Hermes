...
Maps each role archetype to the Orion skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public orion-agent
...
## Orion skills relevant to video production

### Visual / rendering skills (`orion-agent/skills/creative/`)
...
### Design / pre-production skills (`orion-agent/skills/creative/`)
...
### Audio / media skills (`orion-agent/skills/creative/` + `skills/media/`)
...
These are **not** Orion skills but external CLIs / APIs that profiles invoke.
...
## Built-in Orion tools for media review

These are native Orion tools — not invoked via terminal but through their own
...
key is present in `${ORION_HOME:-~/.orion}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Orion profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ORION_HOME:-~/.orion}/.env` → environment variable.
...
When a new Orion-public video skill ships:

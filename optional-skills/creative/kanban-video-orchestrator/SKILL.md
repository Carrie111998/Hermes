...
metadata:
  nastech:
    ...
    credits: |
      ...
      https://github.com/NastechResearch/kanban-video-pipeline.
---

# Kanban Video Orchestrator

Wrap any video request — from a 15-second product teaser to a 5-minute narrative
short to a music video to an ASCII loop — in a Nastech Kanban pipeline that
...
3. **Generates** a setup script that creates Nastech profiles, project workspace, and the initial kanban task
...
For mapping role → which Nastech skills + toolsets it loads, see
...
3. Creates each Nastech profile via `nastech profile create --clone`
...
7. Fires the initial `nastech kanban create` task assigned to the director
...
```bash
nastech kanban watch --tenant <project-tenant>     # live events
nastech kanban list  --tenant <project-tenant>     # board snapshot
nastech dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `nastech kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${NASTECH_HOME:-~/.nastech}/.env` or the user's secret store.

...
short to a music video to an ASCII loop — in a Orion Kanban pipeline that
...
3. **Generates** a setup script that creates Orion profiles, project workspace, and the initial kanban task
...
For mapping role → which Orion skills + toolsets it loads, see
...
3. Creates each Orion profile via `orion profile create --clone`
...
7. Fires the initial `orion kanban create` task assigned to the director
...
```bash
orion kanban watch --tenant <project-tenant>     # live events
orion kanban list  --tenant <project-tenant>     # board snapshot
orion dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `orion kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${ORION_HOME:-~/.orion}/.env` or the user's secret store.

...
Companion skill for the `unreal-engine` entry in the Orion MCP catalog. The
...
Two halves, in this order: the editor side must be up before Orion connects.
...
### One-time, Orion side
    ...
    orion mcp install unreal-engine
...
`~/.orion/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Orion — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Orion connects
...
2. Start the Orion session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Orion
they appear as:
...
then happens at `orion mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Orion runs batched calls
...
- **Start order matters.** Editor + server up first, then the Orion
...
- **Timeouts on long operations.** Orion' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.orion/config.yaml` for

...
### 1. Start order: editor first, Orion second

Orion probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Orion session.
...
restart the editor, restart the Orion session.
...
server fails to bind (Output Log) or Orion' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.orion/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Orion

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Orion' connection
lives in `~/.orion/config.yaml` via `orion mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Orion — just don't
mistake it for the Orion setup step.
...
explicitly warns against overlapping calls. Orion executes same-turn tool
...
### 9. Timeouts: Orion gives up before Unreal does

Orion' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Orion runs elsewhere (SSH backend,

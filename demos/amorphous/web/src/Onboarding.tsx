  ...
  return (
    <div className="max-w-[720px] mx-auto px-6 pt-16 pb-32">
      <h1 className="text-[26px] font-bold tracking-tight m-0 mb-1.5">
        <span className="text-gold">☤</span> Hermes Station
      </h1>
        ...
        {opts.templates.map((t: any) => (
          <button key={t.id} onClick={() => setTemplate(t.id)}
                  className={`text-left p-4 rounded-xl border transition-colors cursor-pointer bg-surface
                    ${template === t.id ? "border-gold ring-1 ring-gold" : "border-line hover:border-line-2"}`}>
            <div className="flex items-center justify-between">
              <span className="font-semibold text-[14.5px]">{t.name}</span>
              {template === t.id && <Check size={15} className="text-gold" />}
            </div>
      ...
      {template === "developer" && (
        <input value={repo} onChange={(e) => setRepo(e.target.value)}
               ...
               className="mt-2.5 w-full h-10 px-3.5 bg-surface-2 border border-line rounded-xl text-[13.5px] text-ink placeholder:text-ink-3 outline-none focus:border-line-2" />
      )}
      ...
      <textarea value={brief} onChange={(e) => setBrief(e.target.value)}
                ...
                className="w-full min-h-[88px] px-4 py-3 bg-surface-2 border border-line rounded-xl text-[13.5px] leading-relaxed text-ink placeholder:text-ink-3 outline-none focus:border-line-2 resize-y" />

      <div className="mt-8 flex items-center gap-4">
        <button onClick={go} disabled={busy}
                className="h-11 px-6 rounded-xl bg-gold text-gold-ink text-[14.5px] font-semibold hover:brightness-108 disabled:opacity-60 inline-flex items-center gap-2">

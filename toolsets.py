...
_ORION_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ORION_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `orion tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ORION_KANBAN_TASK env "
    ...
    # Coding posture (base Orion — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in orion_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Orion toolsets (CLI + messaging platforms)
    #
    ...
    # the `orion send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "orion-cli": {
        ...
        "tools": _ORION_CORE_TOOLS,
        "includes": []
    },

    "orion-cron": {
        # Mirrors orion-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `orion tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as orion-cli; gated by `orion tools`",
        "tools": _ORION_CORE_TOOLS,
        "includes": []
    },
    ...
    "orion-email": {
        "description": "Email bot toolset - interact with Orion via email (IMAP/SMTP)",
        "tools": _ORION_CORE_TOOLS,
        "includes": []
    },
    ...
    "orion-sms": {
        "description": "SMS bot toolset - interact with Orion via SMS (Twilio)",
        "tools": _ORION_CORE_TOOLS,
        "includes": []
    },
    ...
    "orion-webhook": {
        ...
        "tools": _ORION_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "orion-gateway": {
        ...
        "tools": [],
        "includes": ["orion-telegram", "orion-discord", "orion-whatsapp", "orion-slack", "orion-signal", "orion-bluebubbles", "orion-homeassistant", "orion-email", "orion-sms", "orion-mattermost", "orion-matrix", "orion-dingtalk", "orion-feishu", "orion-wecom", "orion-wecom-callback", "orion-weixin", "orion-qqbot", "orion-webhook", "orion-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``orion-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ORION_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``orion-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (orion-<name>).
        # Gives them _ORION_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("orion-"):
            platform_name = name[len("orion-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ORION_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(

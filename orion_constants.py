"""Shared constants for Orion Agent.
...
_UNSET = object()
_ORION_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ORION_HOME_OVERRIDE", default=_UNSET
)
...
def set_orion_home_override(path: str | Path | None) -> Token:
    """Set a context-local Orion home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ORION_HOME_OVERRIDE.set(value)


def reset_orion_home_override(token: Token) -> None:
    """Restore the previous context-local Orion home override."""
    _ORION_HOME_OVERRIDE.reset(token)


def get_orion_home_override() -> str | None:
    """Return the active context-local Orion home override, if any."""
    override = _ORION_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_orion_home() -> Path:
    """Return the platform-native default Orion home path."""
...
def _orion_home_from_env() -> Path:
    """Resolve ORION_HOME from the process environment only.

    Reads the ``ORION_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_orion_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_orion_home`
    and :func:`get_process_orion_home` so the two never drift.
    """
    val = os.environ.get("ORION_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_orion_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``ORION_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_orion_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[ORION_HOME fallback] ORION_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass ORION_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_orion_home() -> Path:
    """Return the Orion home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_orion_home_override`) → ``ORION_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``ORION_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``ORION_HOME`` explicitly (see the systemd
    template in ``orion_cli/gateway.py`` and the kanban dispatcher in
    ``orion_cli/kanban_db.py``).  See https://github.com/zacharyjleach-stack/Aries/issues/18594.
    """
    override = get_orion_home_override()
    if override:
        return Path(override)

    if not os.environ.get("ORION_HOME", "").strip():
        _warn_profile_fallback_once()

    return _orion_home_from_env()


def orion_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Orion home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_orion_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_orion_home() -> Path:
    """Return the Orion home for the running process, ignoring task overrides.

    Unlike :func:`get_orion_home`, this never follows the context-local
    override set by :func:`set_orion_home_override`.  It resolves only the
    process ``ORION_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_orion_root(). The function resolves
# ORION_HOME against the native home on every call (~80us of path
...
# Its result depends only on (ORION_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates ORION_HOME mid-process.
_default_orion_root_memo: "tuple[str, str, Path] | None" = None


def get_default_orion_root() -> Path:
    """Return the root Orion directory for profile-level operations.

    In standard deployments this is the platform-native Orion home
    (``~/.orion`` on POSIX, ``%LOCALAPPDATA%\\orion`` on native Windows).

    In Docker or custom deployments where ``ORION_HOME`` points outside
    ``~/.orion`` (e.g. ``/opt/data``), returns ``ORION_HOME`` directly
    — that IS the root.

    In profile mode where ``ORION_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.orion/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_orion_root_memo
    native_home = _get_platform_default_orion_home()
    env_home = os.environ.get("ORION_HOME", "")
    if _default_orion_root_memo is not None:
        memo_native, memo_env, memo_result = _default_orion_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # ORION_HOME is under ~/.orion (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — ORION_HOME itself is the root
                result = env_path
    _default_orion_root_memo = (str(native_home), env_home, result)
    return result
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ORION_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ORION_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_orion_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``ORION_OPTIONAL_MCPS``.
    """
    override = os.getenv("ORION_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_orion_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ORION_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<ORION_HOME>/skills`` last-resort
    """
    override = os.getenv("ORION_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_orion_dir(
    ...
    *,
    ...
    """Resolve a Orion subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to ORION_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ORION_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Orion home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local ORION_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_orion_home()
...
def iter_orion_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Orion-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\orion\\node``. POSIX installs use
    ``$ORION_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_orion_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with orionManagedNodePathEntries() in
...
_ORION_NODE_TARGET_MAJOR = int(os.environ.get("ORION_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Orion-managed Node trees live under ``$ORION_HOME/node`` (or a profile's
    ``ORION_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_orion_node_executable``
    used to trust file presence alone, so ``orion update`` would pick that
...
def orion_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Orion-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_orion_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Orion-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `orion update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%ORION_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the orion.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_ORION_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_ORION_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$ORION_HOME/node`` on POSIX.
    ...
    ``ORION_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_orion_managed_node() -> str | None:
    """Install a Orion-managed Node tree and return its npm path.
    ...
    Orion never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$ORION_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_orion_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_orion_node_dirs():
...
def heal_orion_managed_node() -> bool:
    """Redownload Orion-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not orion_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ORION_HOME": str(get_orion_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_orion_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_ORION_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_orion_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _ORION_NODE_TARGET_MAJOR
    return False


def find_orion_node_executable(command: str) -> str | None:
    """Return a Orion-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_ORION_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_orion_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Orion-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Orion-managed installs.

    This is for Orion-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_orion_node_executable(command)
    if managed:
        return managed
    if orion_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_orion_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Orion-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_orion_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``orion update`` — leaving a **dangling symlink** that ``which`` still
...
def display_orion_home() -> str:
    """Return a user-friendly display string for the current ORION_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.orion``
        profile:  ``~/.orion/profiles/coder``
        custom:   ``/opt/orion-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.orion``.  For code that needs a real ``Path``, use
    :func:`get_orion_home` instead.
    """
    home = get_orion_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``ORION_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/zacharyjleach-stack/Aries/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ORION_HOME}/home`` when the profile-home directory exists."""
    orion_home = get_orion_home_override() or (env or {}).get("ORION_HOME") or os.getenv("ORION_HOME")
    if not orion_home:
        return None
    profile_home = os.path.join(orion_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("ORION_REAL_HOME") or os.getenv("ORION_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Orion profile HOME.

    ``ORION_HOME`` scopes Orion state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ORION_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ORION_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ORION_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Orion' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["ORION_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Orion itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ORION_HOME.

    Replaces the ``get_orion_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, orion_logging.py, orion_time.py, etc.).
    """
    return get_orion_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ORION_HOME."""
    return get_orion_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ORION_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_orion_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._orion_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``orion_cli`` modules using three different Windows
    ...
    A few sites outside ``orion_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``orion_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Orion itself. An ImportError
...
# `orion_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Orion.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("orion_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `orion update` is exactly
    ...
    return [
        ...
        "    orion update",
        "If that also fails, reinstall: https://your-orion-docs.example",
    ]

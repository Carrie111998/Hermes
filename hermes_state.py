...
class SessionDB:
    ...
    def __init__(self, db_path: Optional[Path] = None, read_only: bool = False):
        ...
        # mkdir is deferred to the writable branch below (line ~449) so a
        # read-only attach never creates the parent directory.
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            # Gate renumbered 20 → 29 at the 0.19.0 merge: fork DBs on the
            # session-bridge numbering line sit at 23-28 without ever running
            # this upstream seeding, so `< 20` would skip them. INSERT OR
            # IGNORE keeps the re-run idempotent for DBs that already seeded.
            if current_version < 29:
                ...
                try:
                    ...
                    pass
            # Gate renumbered 22 → 29 at the 0.19.0 merge (same reason as the
            # seeding gate above); the pragma_table_info check below makes the
            # rebuild self-gating, so re-running is a no-op once task is in
            # the PK.
            if current_version < 29:
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``HERMES_DISABLE_MESSAGE_TRIGRAM`` or not yet created), so

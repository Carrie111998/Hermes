...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def branch_at_message(
        ...
        *,
        ...
        - user message with ``user_retry_include``: copy messages before the
          selected user message and return that user's content as ``prefill``;
          callers that want retry semantics can immediately submit the prefill
          on the new branch without duplicating the selected user row.

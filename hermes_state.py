...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # Read-path split (WAL only): recall/browse queries lease read-only
        # connections from a small shared pool so they never queue behind writer
        # flushes on self._lock.  The former per-thread cache retained one reader
        # for every short-lived executor thread until gateway shutdown, eventually
        # exhausting macOS's default 256 file-descriptor limit.
        self._read_pool_lock = threading.Lock()
        self._read_pool_idle: "list[sqlite3.Connection]" = []
        self._read_pool_all: "set[sqlite3.Connection]" = set()
        self._read_pool_closed = False
    ...
    def _open_read_conn(self) -> sqlite3.Connection:
        """Open and configure one pooled read-only connection."""
    ...
    def _acquire_read_conn(self) -> Optional[sqlite3.Connection]:
        """Lease a pooled read connection, or None for locked fallback.
        ...
        with self._read_pool_lock:
            if self._read_pool_closed:
                return None
            if self._read_pool_idle:
                return self._read_pool_idle.pop()
            if len(self._read_pool_all) >= self._READ_POOL_MAX:
                return None
            try:
                ...
                return None
            self._read_pool_all.add(conn)
            return conn

    def _release_read_conn(self, conn: sqlite3.Connection, *, broken: bool = False) -> None:
        """Return a lease to the pool, or close it when broken/shutting down."""
        should_close = broken
        with self._read_pool_lock:
            if conn not in self._read_pool_all:
                return
            if broken or self._read_pool_closed:
                self._read_pool_all.discard(conn)
                should_close = True
            else:
                self._read_pool_idle.append(conn)
        if should_close:
            try:
                conn.close()
            except Exception:
    ...
    def _get_read_conn(self) -> Optional[sqlite3.Connection]:
        """Compatibility helper for single-threaded diagnostics and tests.

        Production reads must use ``_read_ctx`` so leases remain exclusive.
        Callers of this private helper must not retain or close the returned
        connection; it remains owned by the pool.
        """
    ...
    def close(self):
        ...
        # Close idle pooled readers immediately. Active leases observe the
        # closed flag and close themselves when returned.
        with self._read_pool_lock:
            self._read_pool_closed = True
            read_conns = list(self._read_pool_idle)
            self._read_pool_idle.clear()

...
class StateDbReplacedError(RuntimeError):
    ...
    under a live gateway. In-place FTS rebuild and fail-open trigger
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # every write (and before FTS repair / NOTADB reconnect) so an
        ...
        *patience_s* is the total time budget for lock retries (default
        ...
        while True:
            ...
            try:
                ...
                # the write once — but only when the path still names the
                # same file generation. A replaced file plus leftover WAL
                # is not recoverable in-process.
                if (
                    _is_not_a_database_error(exc)
    ...
    def _reconnect_after_notadb(self) -> bool:
        ...
        If the path's inode or generation stamp no longer matches the file
        this instance opened, refuse to reopen — the new file plus leftover
        ``-wal``/``-shm`` is the outage #89332; reconnecting writes into it.
        """
        if self._db_file_was_replaced():
            self._halt_db_replaced()

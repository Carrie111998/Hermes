...
skill-sleep run
  ...
  ├─ [2] PROPOSE   优化器（omp + muse-spark-1.2）读 task cards + 当前 SKILL.md → candidate.diff（含 rejected buffer 上下文）
...
# 2. 配置密钥（muse-spark 走 NineRouter，仅检查是否设置，不读取值、不落盘）
...
- `PROPOSE`: `DEFAULT_MODEL=9router/ocg/muse-spark-1.2-contributor`，diff 有界（≤30 行新增，文本学习率）

...
|---|---|---|---|---|
| **P-056** | `hermes_cli/web_server.py`, `tui_gateway/server.py`, focused gateway/API tests | Hardens Desktop-facing request boundaries: clamps negative transcript pagination, rejects non-string `prompt.submit` text, requires image magic bytes even when the filename claims an image extension, and converts cron tool validation failures into JSON-RPC errors. | Malformed or spoofed Desktop requests could bypass the transcript cap, reach turn handling with the wrong type, persist arbitrary bytes as images, or report a failed cron mutation as successful. | Generic gateway/API correctness and suitable for upstream. |
...
| **P-008** | `hermes_cli/web_server.py`, `hermes_cli/main.py`, focused profile tests | ~~Adds `GET/PUT /api/profiles/active`~~ → upstream shipped its own `GET/POST /api/profiles/active`; fork keeps the API **compat layer** (`name` in GET + a `PUT` alias) and makes a Desktop-managed launch trust its explicit `HERMES_HOME` instead of following a stale sticky profile during a named→default restart | v2 reads `.name` and writes via `PUT`; during a live switch back to default, the old `active_profile` necessarily remains until the replacement dashboard is ready, so Core must not redirect the new managed process back to the old named profile | **Upstreamed (GET/POST)** + Desktop compatibility/restart semantics |
...
**Desktop-managed named→default restart**: Desktop intentionally starts the replacement dashboard before clearing `active_profile`, so a failed restart leaves the old profile selected. Core's generic startup override used to see that stale file and rewrite the replacement process's explicit root `HERMES_HOME` back to `profiles/<old-name>`. When `HERMES_DESKTOP_MANAGED=1` and no explicit `--profile` was supplied, `_apply_profile_override()` now treats the Desktop-provided `HERMES_HOME` as authoritative. Explicit `--profile` still wins, and standalone CLI/systemd startup keeps its sticky-profile behavior. Covered by `tests/hermes_cli/test_apply_profile_override.py`.
...
### P-051: Pin decoding on every text-mode subprocess pipe — GBK `_readerthread` UnicodeDecodeError class eliminated

**Symptom.** On zh-CN Windows (ANSI codepage 936, Python 3.14 without UTF-8 mode), the CLI printed `Exception in thread Thread-N (_readerthread): UnicodeDecodeError: 'gbk' codec can't decode byte 0xae ... illegal multibyte sequence` right after the first conversation turn, and kept running with the affected output silently lost.

**Root cause.** `subprocess.run(..., text=True)` without `encoding=`/`errors=` wraps the child's pipes with `io.TextIOWrapper` using `locale.getpreferredencoding(False)` — cp936/GBK on zh-CN Windows. Modern cross-platform children (git, python, node, uv, docker, gh, …) write UTF-8 regardless of the ANSI codepage; a single byte that is illegal in GBK (e.g. `0xAE`, the middle byte of 实 = `E5 AE 9E`, common in Chinese commit subjects) raises inside CPython's daemon `_readerthread` (`subprocess.py`: `buffer.append(fh.read())`), surfaces only via `threading.excepthook`, and leaves `result.stdout is None` — the caller never sees an exception. The reported instance was the session-start workspace snapshot (`agent/system_prompt.py` → `coding_system_blocks()` → `agent/coding_context.py::_git()` running `git status --porcelain=2 --branch` / `git log -3 --pretty=%h %s` over a repo with Chinese commit subjects): the reader thread died, `_git()` then hit `None.strip()`, and the broad `except` in `system_prompt.py` swallowed it — so the whole workspace block silently vanished from the system prompt. Reproduced byte-exactly with `python -X utf8=0` (→ cp936) against a child emitting UTF-8 CJK: same `subprocess.py:1613 _readerthread` frame, same `0xae` message, `stdout is None` afterwards.

**Fix.** Class-wide, not just the reported site — a repo-wide AST audit found **269 text-mode subprocess call sites** in shipped code missing explicit decoding (plus 3 more hiding behind `**kwargs`-dict splats):
1. **UTF-8-by-contract children** (git, gh, rg, python/pip/uv, node/npm, docker, ssh/scp, systemctl/loginctl/launchctl/ps/lsof, tmux, s6, cosign, codex/claude CLIs, secret CLIs, hermes itself, user shell snippets, …): pinned `encoding="utf-8", errors="replace"` — decodes what the child actually wrote, and a bad byte can never kill the reader thread.
2. **Windows-native tools emitting the OEM/ANSI codepage** (`tasklist`, `taskkill`, `netstat`, `where`): pinned `errors="replace"` only — locale decoding is the faithful choice there (localized messages and Chinese paths stay correct); strictness was the only bug. This mirrors the pre-existing documented hardening at `gateway/status.py::terminate_pid`.
3. The three `**kwargs`-dict splat sites (`hermes_cli/web_server.py::_fs_git_branch`, `tools/tts_tool.py::_run_command_tts`, `tools/transcription_tools.py::_run_command_stt`) got the same pins inside their dicts.
4. Drive-by: `hermes_cli/_subprocess_compat.py` used `subprocess.run`/`Popen` in its (currently unused) drop-in wrappers without `import subprocess` — a latent `NameError`; import added, and the wrapper docstring now documents the text-mode decoding pitfall.

**Regression guard.** `tests/test_subprocess_text_pipe_decoding.py` (new): (a) a static AST invariant over shipped code — every text-mode `subprocess.run/Popen/call/check_call/check_output` call, including the named-dict and inline `**dict` splat forms, must pass an explicit `errors=` — so the class cannot re-enter via future code or upstream syncs; (b) a behavioral test that pipes bytes illegal in every codec through `coding_context._git` and asserts the thread survives and UTF-8 CJK lands intact (deterministic on any locale because the decoding is now pinned). Follows the precedent of `tests/tui_gateway/test_slash_worker_utf8_decode.py` (the same bug class fixed earlier for long-lived child pipes, commit c210b9621).

**Tested.** New guard: 2 passed. Affected-area sweep (330 test files covering every edited module — clipboard, docker/ssh/singularity environments, kanban, gateway, tui_gateway, cli worktree helpers, main update flow, tools_config, lazy_deps, computer_use, cron, webhook, transcription/tts/voice, secret sources, plugins) run on BOTH the branch and the unmodified base: 206 failures on base vs 209 on branch, of which 205 are identical pre-existing batch-composition pollution (each spot-checked failure passes in isolation; e.g. the 4 `test_update_autostash.py` file-level failures reproduce on the untouched base). The 4 branch-only failures were exact-kwargs change-detector assertions (`test_webhook_integration`, `test_whatsapp_connect`, `test_update_autostash`, `test_docker_environment`) — updated to the new contract; the whatsapp `_terminate_bridge_process` taskkill site was moved to the locale-style (`errors="replace"` only) group to match its Windows-native output. 1 baseline-only failure was flaky pollution. `scripts/check-windows-footguns.py --all` clean (793 files); `ruff check .` clean by construction (only PLW1514/`open()`-encoding is enabled — untouched by this change; CI lint.yml is the enforcement gate).

**Should we upstream?** Yes — the per-site pins and the AST invariant test are OS-agnostic (the crash only *manifests* on non-UTF-8 Windows locales, but decoding a UTF-8 pipe as locale is wrong everywhere). Related: c210b9621 (slash worker + Copilot ACP pipe pins), P-042 (Windows subprocess overhead), P-019 (Windows shell handling).

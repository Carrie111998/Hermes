"""Windows UTF-8 bootstrap for Orion entry points.
...
should be imported at the very top of every Orion entry point
(``orion``, ``orion-agent``, ``orion-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``orion_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``orion_bootstrap``, never ``orion_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Orion modules.

    Orion ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Orion source root
    ...
    """
    root = src_root or os.environ.get("ORION_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ORION_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ORION_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import orion_bootstrap``
# (or ``from orion_bootstrap import apply_windows_utf8_bootstrap``) at

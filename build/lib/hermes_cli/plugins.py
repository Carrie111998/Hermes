...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Charterforge loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Charterforge' shipped guidance lives in the evidence-based
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Charterforge profile name (e.g. ``"default"``).
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Charterforge core) are trusted by default —
    ...
    def register_secret_source(self, source) -> None:
        ...
        ``~/.hermes/.env`` loads, before Charterforge reads credentials — when
        ...
        spawned Charterforge process (gateway children, cron sessions,
    ...
    def register_slack_action_handler(
        ...
        Charterforge' Slack adapter wires registered handlers into its
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # path actually asks for that platform. Every platform Charterforge ships

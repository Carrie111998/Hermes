...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Charterforge'
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Charterforge' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Charterforge for
        ...
        try:
            ...
            # Permissions + Charterforge tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                msg_lines.append(
                    "Charterforge tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Charterforge runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Charterforge tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Charterforge runtime.")

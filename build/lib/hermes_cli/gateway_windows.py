...
_TASK_DESCRIPTION = "Charterforge Gateway - Messaging Platform Integration"
...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render Charterforge-owned paths under the configured HERMES_HOME spelling.
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Charterforge profile."""
...
def get_task_script_path() -> Path:
    ...
    Charterforge installs stay self-contained).
    """
    _assert_windows()
    from hermes_cli.config import get_hermes_home
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Charterforge builds and failed
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Charterforge gateway install prompt.")
                if start_now:
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Charterforge gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: hermes gateway status")
                    return

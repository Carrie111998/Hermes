...
def remove_path_from_shell_configs():
    """Remove Charterforge PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Charterforge" comment and following line
                if '# Charterforge' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this Charterforge home's ``node``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Charterforge-owned User-PATH entries."""
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Charterforge-owned entries from User-scope PATH in the registry.
...
def run_gui_uninstall(args):
    ...
    print(color("│         ⚕ Charterforge Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Charterforge Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Charterforge agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Charterforge agent at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Charterforge agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Charterforge Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Charterforge data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Charterforge code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Charterforge PATH entries from shell configs / Windows User PATH")
    print("  • Charterforge wrapper scripts and Charterforge-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Charterforge config/data: {hermes_home}")
        ...
        print(f"  • Keep Charterforge config/data: {hermes_home}")
    print()
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Charterforge-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Charterforge-set User env vars to remove")
    ...
    #     (only when they still point into this Charterforge home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Charterforge-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Charterforge-managed node/npm/npx symlinks found")
    ...
    print()
    print("Thank you for using Charterforge! ⚕")
    print()

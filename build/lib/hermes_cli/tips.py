...
TIPS = [
    ...
    "Ctrl+Z suspends Charterforge to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs Charterforge itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire Charterforge home directory.",
    ...
    "hermes gateway install sets up Charterforge as a system service (systemd/launchd).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Charterforge tools programmatically — results stay out of context.",
    ...
    "When exiting, Charterforge prints a resume command with session ID and stats.",
    ...
    # --- Gateway & Messaging ---
    "Charterforge runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to Charterforge via a local macOS server.",
    ...
    "Charterforge loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "hermes acp runs Charterforge as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Charterforge your own.",
    ...
    "Before a gateway session reset, Charterforge auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Charterforge state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Charterforge auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Charterforge config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Charterforge is working.',
    ...
    'hermes dashboard embeds the full Charterforge TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'hermes import restores a full Charterforge backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Charterforge block commands when the tirith scanner itself errors out.',

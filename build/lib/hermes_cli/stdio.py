...
on Japanese locales, etc.).  Charterforge's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Charterforge is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Charterforge-managed Git install directories so
    ...
    # install without Charterforge-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Charterforge's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Charterforge's
    ...
    Set this before launching Charterforge (User env var in Windows Settings, or
    export in a PowerShell profile) and Charterforge picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Charterforge-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Charterforge spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Charterforge-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Charterforge venv Scripts directory — host of the hermes.exe shim itself,

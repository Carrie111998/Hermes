...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Charterforge. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Charterforge deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Charterforge on Slack's older Assistant messaging
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Charterforge in threads and DMs.",
        }
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Charterforge agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "Charterforge")
    ...
    """
    name = getattr(args, "name", None) or "Charterforge"
    description = getattr(args, "description", None) or "Your Charterforge agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Charterforge app\n"

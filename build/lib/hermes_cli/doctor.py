...
Diagnoses issues with Charterforge setup.
"""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Charterforge Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Charterforge already refuses to enable WAL on fresh DBs.
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Charterforge' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Charterforge a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Charterforge Persona\n\n"
                "<!-- Edit this file to customize how Charterforge communicates. -->\n\n"
                "You are Charterforge, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Charterforge one.
                        check_info(

...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        # Prefer a dedicated process group so a later non-local kill can reap
        # descendants as well as the wrapper. ``setsid`` is not universal on
        # SSH/macOS and other non-Linux backends. In that case Bash job control
        # creates a separate group for the nohup child; record that child/group
        # leader, not the outer shell, so the existing group-kill path is valid.
        wrapped_command = (
            f"nohup bash -lc {quoted_command} > {quoted_log_path} 2>&1; "
            f"rc=$?; printf '%s\\n' \"$rc\" > {quoted_exit_path}"
        )
        quoted_wrapped_command = shlex.quote(wrapped_command)
        fallback_command = (
            f"set -m; ( {wrapped_command} ) & child=$!; "
            f"echo \"$child\" > {quoted_pid_path}; wait \"$child\""
        )
        bg_command = (
            f"mkdir -p {quoted_temp_dir} && "
            f"if command -v setsid >/dev/null 2>&1; then "
            f"setsid bash -lc {quoted_wrapped_command} & "
            f"echo $! > {quoted_pid_path}; "
            f"else bash -lc {shlex.quote(fallback_command)} >/dev/null 2>&1 & fi; "
            f"for _ in $(seq 1 50); do [ -s {quoted_pid_path} ] && break; sleep 0.1; done; "

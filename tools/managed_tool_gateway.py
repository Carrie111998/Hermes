...
def auth_json_path():
    """Return the Orion auth store path, respecting ORION_HOME overrides."""
    return get_orion_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`orion tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from orion_cli.auth import resolve_nous_access_token
...
# managed vendor's gateway URL is pinned: adding one is a Orion release, and

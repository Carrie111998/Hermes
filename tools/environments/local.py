...
from orion_constants import get_process_orion_home
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Orion was already launched from (for example ``orion-agent``
    while the process cwd is ``~/.orion/orion-agent``), passing it through
    unchanged makes the wrapper run ``cd orion-agent`` *inside* the project
    ...
    # Common recovery for config values like ``orion-agent`` when Orion was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./orion-agent``; use the current directory instead.
...
# Orion-internal env vars that should NOT leak into terminal subprocesses.
_ORION_PROVIDER_ENV_FORCE_PREFIX = "_ORION_FORCE_"

# Orion-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Orion inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Orion-managed inference credential — Claude subscription auth is not a
    # working Orion provider path.  Stripping it broke agent-spawned
...
_ORION_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Orion venv path instead of the project's own
# ``.venv`` — silently clobbering the Orion environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Orion venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Orion venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Orion itself treats PYTHONHOME as contamination in its own
...
# _strip_orion_owned_pythonpath() which removes only Orion-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_orion_internal_secret(key: str) -> bool:
    """Return True for Orion-internal secrets injected under *dynamic* names.

    ``_ORION_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Orion-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`orion_subprocess_env` helper all call it, so the
...
def _inject_context_orion_home(env: dict) -> None:
    """Bridge the context-local Orion home override into subprocess env."""
    try:
        from orion_constants import get_orion_home_override

        value = get_orion_home_override()
        if value:
            env["ORION_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (ORION_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Orion-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ORION_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_orion_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _ORION_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_ORION_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ORION_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_orion_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_orion_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _ORION_PROVIDER_ENV_BLOCKLIST and not passthrough:
    ...
    _inject_context_orion_home(sanitized)

    from orion_constants import apply_subprocess_home_env
    ...
    # the separate Orion runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_orion_owned_pythonpath_and_runtime_markers(sanitized)
...
# legitimate child Orion spawns needs them, and they are the highest-value
...
# narrow subset of _ORION_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in orion_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_orion_internal_secret, but _ID has no secret suffix, so it must be
...
def orion_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_ORION_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Orion spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_ORION_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Orion-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_orion_internal_secret`.
    for key in list(env):
        if key.startswith(_ORION_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_orion_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _ORION_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_orion_home(env)
    from orion_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_orion_owned_pythonpath_and_runtime_markers(env)

    _apply_windows_msys_bash_env_defaults(env)

    # Cross-session leak guard, same as the terminal spawn paths: this helper
    # copies os.environ, whose ORION_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's ORION_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`orion_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``ORION_HOME`` / subprocess ``HOME`` contract) and the
    Orion secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_orion_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``ORION_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Orion home override into ``ORION_HOME`` and apply the
      subprocess HOME contract (``orion_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``ORION_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs ORION_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ORION_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale ORION_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\orion\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\orion\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _orion_portable_git = os.path.join(_local_appdata, "orion", "git") if _local_appdata else ""
    if _orion_portable_git:
        for candidate in (
            os.path.join(_orion_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_orion_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # ORION_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\orion\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "ORION_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Orion Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set ORION_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Orion. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``orion`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_ORION_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_orion_bin_dir() -> str | None:
    """Return the directory holding the ``orion`` console-script, or None.
    ...
    orion install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``orion``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``orion`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``orion`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("orion")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``orion`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _ORION_BIN_DIR
    if _ORION_BIN_DIR is not _SENTINEL:
        return _ORION_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("orion")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "orion" or base.startswith("orion."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "orion.exe" if _IS_WINDOWS else "orion"
    ...
    _ORION_BIN_DIR = candidate
    return candidate


def _prepend_orion_bin_dir(existing_path: str) -> str:
    """Prepend the orion install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_orion_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Orion-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Orion installs for
    itself, so on a machine where Orion provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$ORION_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$ORION_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_orion_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_orion_managed_node``, a first browser install).
    """
    try:
        from orion_constants import get_orion_home, iter_orion_node_dirs

        candidates = [*iter_orion_node_dirs(), get_orion_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Orion-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Orion
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ORION_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ORION_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_orion_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_orion_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _ORION_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the orion install dir is reachable so plugins can shell out
        # to bare ``orion`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_orion_bin_dir(new_path)

    _inject_context_orion_home(run_env)
    ...
    _strip_orion_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_orion_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    configured_home: Path,
) -> tuple[Path, ...]:
    """Return exact repo-root spellings emitted by Orion launchers.

    ``gateway_windows._preserve_orion_home_path`` maps a physical path under
    the resolved ORION_HOME back onto the configured ORION_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of ORION_HOME as Orion-owned.
    ...
    # lexically the same way get_default_orion_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of ORION_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\orion\orion-agent
    # -> C:\...\orion-agent) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as Orion-owned.  Fail-closed: a
...
# --- Orion venv / repo-root detection (module-level, computed once) ---

#: The Orion repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import orion_cli``,
#: etc.  Subprocesses that are NOT the Orion backend don't need it and it
#: can shadow local packages.
_orion_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Orion launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Orion-owned paths under
#: the configured ORION_HOME spelling (which may be a junction to another
#: drive — see ``orion_cli/gateway_windows.py::_preserve_orion_home_path``).
...
_orion_repo_root_aliases: tuple[Path, ...] = _build_orion_repo_root_aliases(
    _orion_repo_root,
...
#: subprocess spawn.
_orion_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Orion repo>/venv`` layout and a real venv marker; require both before
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _orion_repo_root_aliases):
        return None
...
def _get_orion_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Orion runtime.
    ...
    """
    global _orion_site_packages
    if _orion_site_packages is not None:
        result = list(_orion_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _orion_site_packages = list(result)
...
def _strip_orion_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Orion-owned PYTHONPATH entries, then the runtime marker vars.
    ...
    """
    _strip_orion_owned_pythonpath(env)
    for _marker in _ACTIVE_VENV_MARKER_VARS:
        env.pop(_marker, None)


def _strip_orion_owned_pythonpath(env: dict) -> None:
    """Remove Orion-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Orion repo root and the Orion venv's
    ...
    user entries, so only entries proven Orion-owned are removed:
    ...
    orion_site_packages = _get_orion_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Orion-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Orion venv site-packages ---
        ...
        # resolved and unresolved (ORION_HOME/junction) spellings count as
        # Orion-owned.
        if not should_strip:
            should_strip = any(
                _same_path(entry_path, repo_root)
                for repo_root in _orion_repo_root_aliases
            )
    ...
    if stripped:
        logger.debug(
            "Stripped Orion-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Orion trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ORION_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ORION_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_orion_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "orion_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._orion_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_orion_pgid", None)
                    if pgid is None:
                        raise

...
def _find_bash() -> str:
    ...
    # shutil.which("bash") is the last-resort candidate. On a box with WSL
    # installed, PATH order usually puts the WSL launcher
    # (``System32\bash.exe``) or its Microsoft Store stub
    # (``WindowsApps\bash.exe``) ahead of Git Bash (a default Git for
    # Windows install only adds ``Git\cmd``, which ships no bash). WSL bash
    # runs commands inside the Linux VM, where ``C:\...``-style paths this
    # environment builds do not resolve — every command would exit 126/127.
    # Refuse it here rather than adding it as a "candidate" that could win
    # the ``_bash_starts`` probe below (the WSL launcher can start fine; it
    # just can't see our paths, so a successful probe would be misleading).

"""Base class for all Orion execution environment backends.
...
# ORION_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ORION_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_orion_home() / "sandboxes"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._orion_stdin_errors`` while stdin is still closed
    ...
    proc._orion_stdin_errors = errors
    ...
    proc._orion_stdin_thread = thread
    thread.start()
...
def _cwd_marker(session_id: str) -> str:
    return f"__ORION_CWD_{session_id}__"
...
# the FIRST session's ORION_SESSION_ID into the snapshot makes every LATER
...
# ORION_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Orion' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is ORION_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ORION_SESSION_|ORION_UI_SESSION_ID|ORION_CRON_AUTO_DELIVER_|"
    "ORION_CRON_SESSION|ORION_BROWSER_CONTROL_)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``ORION_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!ORION_SESSION_*} ${!ORION_CRON_AUTO_DELIVER_*} "
        "${!ORION_BROWSER_CONTROL_*} "
        # AI_AGENT / ORION_AGENT are per-command attribution markers
        ...
        "AI_AGENT ORION_AGENT "
        f"ORION_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Orion backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/orion-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/orion-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__orion_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__orion_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__orion_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__orion_fns\" ] && declare -f $__orion_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_ORION_RUNTIME_PASSTHROUGH_{name}"
        ...
        # under Orion via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Orion-specific
        # ``ORION_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``orion-agent`` — see huggingface.js
        ...
        # inherited from the Orion process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Orion running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-orion-agent}" '
            'ORION_AGENT="${ORION_AGENT:-true}"'
        )
        ...
        parts.append("__orion_ec=$?")
        # Restrict Orion metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__orion_snap_tmp=$(mktemp {_snap_tmp_template}) && "
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ORION_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_orion_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via ORION_DEBUG_INTERRUPT=1) -------------
        ...
        stdin_thread = getattr(proc, "_orion_stdin_thread", None)
        if stdin_thread is not None:
            stdin_thread.join(timeout=5)
        rendered = output.render()
        result = self._finalize_wait_result(output, rendered, proc.returncode)
        stdin_errors = getattr(proc, "_orion_stdin_errors", None)
        if stdin_errors:
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __ORION_CWD_{session}__ marker from stdout output.

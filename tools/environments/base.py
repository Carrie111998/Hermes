...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        # parent shell's PID in ``&``-launched subshells, while ``$BASHPID`` is
        # unavailable in macOS' Bash 3.2.  Let mktemp allocate a same-directory
        # file instead, which is portable across macOS, Linux, and Git Bash and
        # preserves the atomic same-filesystem rename guarantee.
        _snap_tmp_template = self._quote_shell_path(
            self._snapshot_path + ".tmp.XXXXXX"
        )
        bootstrap = (
            f"umask 077\n"
            f"__hermes_snapshot_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f">> \"$__hermes_snapshot_tmp\" 2>/dev/null || true\n"
            f"alias -p >> \"$__hermes_snapshot_tmp\"\n"
            f"echo 'shopt -s expand_aliases' >> \"$__hermes_snapshot_tmp\"\n"
            f"echo 'set +e' >> \"$__hermes_snapshot_tmp\"\n"
            f"echo 'set +u' >> \"$__hermes_snapshot_tmp\"\n"
            ...
            f"mv -f \"$__hermes_snapshot_tmp\" {_quoted_snap} "
            f"|| rm -f \"$__hermes_snapshot_tmp\"\n"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # truncated/half-written file.  mktemp is used instead of PID-derived
        # names because macOS' Bash 3.2 has no ``$BASHPID`` and ``$$`` is shared
        # by concurrent ``&``-launched subshells.
        _snap_tmp_template = self._quote_shell_path(
            self._snapshot_path + ".tmp.XXXXXX"
        )
        ...
        if self._snapshot_ready:
            parts.append(
                f"__hermes_snapshot_tmp=$(mktemp {_snap_tmp_template}) "
                f"&& {{ export -p > \"$__hermes_snapshot_tmp\" "
                f"&& mv -f \"$__hermes_snapshot_tmp\" {_quoted_snap}; }} "
                f"2>/dev/null || rm -f \"${{__hermes_snapshot_tmp:-}}\" "
                f"2>/dev/null || true"
            )

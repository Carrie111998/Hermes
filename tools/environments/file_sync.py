...
_FORCE_SYNC_ENV = "ORION_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.orion") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.orion to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ORION_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, orion_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.orion/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(orion_home=orion_home)

    def _sync_back_transaction(self, orion_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="orion-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.orion/skills/a.md`` →
        ``~/.orion/skills/a.md``, a new remote file at
        ``/root/.orion/skills/b.md`` maps to ``~/.orion/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []

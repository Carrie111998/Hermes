...
Provides a revisioned task list the agent uses to decompose complex tasks,
track progress, and maintain focus across long conversations. The state lives
on the AIAgent instance (one per session), is re-injected after context
compression, and every state change bumps a monotonic revision so UI clients
...
class TodoStore:
    ...
    def _load_from_disk(self) -> None:
        ...
        for item in items:
            ...
            seen.add(item["id"])
            deduped.append(item)
        self._items = deduped[:MAX_TODO_ITEMS]
        self._sanitize_parents(self._items)

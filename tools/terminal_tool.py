...
def _get_env_config() -> Dict[str, Any]:
    ...
    # Per-backend default CWDs. When no backend-specific CWD is saved yet,
    # these values prevent paths from one execution environment leaking into
    # another.
    ...
    #   vercel        documented Vercel workspace root
    if env_type == "local":
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    Args:
        env_type: One of "local", "docker", "singularity", "modal",
            "daytona", "wsl", "vercel_sandbox", "ssh"
    ...
    if env_type == "local":
        ...
        raise ValueError(
            ...
            f"'singularity', 'modal', 'daytona', 'wsl', 'vercel_sandbox', or 'ssh'"
        )

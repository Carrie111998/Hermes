...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    # SSH connections are profile-specific remote identities, so a gateway or
    # WebUI session must not reuse an SSHEnvironment cached by another profile.
    # Do not apply that cache key to persistent Docker: its documented contract
    # is one profile-scoped long-lived container shared by gateway/TUI sessions.
    _ensure_terminal_env_bridged()
    if os.getenv("TERMINAL_ENV", "local").strip().lower() == "ssh":
        session_key = _current_session_key()
        if session_key:
            return f"session:{session_key}"
    return "default"

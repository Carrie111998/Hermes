...
def _resolve_environment_cwd(
    ...
    raw host path (e.g. a Windows desktop session's ``C:\\Users\\<user>``, or
    a POSIX ``/home/<user>``) reaches ``docker run -w <host-path>`` and the
    container fails to start (exit 125). Re-apply the same host/relative path
    guard to the *resolved* cwd so the override can't bypass it. When the host
    path IS this session's mounted workspace, remap it to ``/workspace``
    (where the mount lands) instead of discarding it. Valid in-container
    override paths (RL/benchmark sandboxes that set cwd to ``/workspace``,
    ``/root``, or a backend's own guest-home subtree) pass through untouched.
    """
...
def _container_config_from_config(config: Dict[str, Any]) -> dict:
    ...
    Shared by every environment-creation site (terminal tool, file tools,
    execute_code, prompt builder, lazy :func:`ensure_task_env` bring-up) so
    they can't drift (see :func:`_ssh_config_from_config`).
    """
    return {
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        raise ValueError(
            ...
            f"'singularity', 'modal', 'daytona', 'vercel_sandbox', 'tenki', 'ssh'{extra}"
        )
...
def ensure_task_env(task_id: Optional[str] = None):
    ...
    _selected_key, existing = _select_active_environment(effective_task_id)
    if existing is not None:
        return existing
    ...
    with _environment_creation_lock(effective_task_id):
        ...
        new_env = _register_active_environment(effective_task_id, new_env)
        logger.info(
    ...
    try:
        ...
        if env_type == "local":
            ...
            logger.error(
                ...
                "modal, daytona, vercel_sandbox, tenki, ssh, or a "

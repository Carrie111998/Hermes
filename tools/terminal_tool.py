...
def terminal_tool(
    ...
    try:
        ...
        # A delegated child has its own task/session key so CWD and sandbox
        # state remain isolated from the parent.  Completion notifications are
        # different: once the child returns there is no consumer for an
        # ``sa-*`` routing key.  Rebind only the notification copy to the
        # durable owning conversation while leaving ``session_key`` untouched
        # for every CWD/environment lookup below.
        notification_session_key = session_key
        notification_parent_session_id = ""
        child_notification_unresolved = False
        try:
            from gateway.session_context import get_session_env as _notification_gse

            top_level_session_id = _notification_gse("HERMES_SESSION_ID", "")
        except Exception:
            top_level_session_id = ""
        if (
            background
            and (notify_on_complete or watch_patterns)
            and isinstance(task_id, str)
            and task_id.startswith("sa-")
        ):
            try:
                from tools.delegate_tool import get_subagent_attribution

                attribution = get_subagent_attribution(task_id)
                owner_session_id = str(
                    (attribution or {}).get("owner_agent_session_id") or ""
                )
                if owner_session_id:
                    if not session_key or session_key == task_id:
                        notification_session_key = owner_session_id
                    notification_parent_session_id = owner_session_id
                else:
                    child_notification_unresolved = True
            except Exception as exc:
                child_notification_unresolved = True
                logger.warning(
                    "Could not resolve parent notification route for %s: %s",
                    task_id,
                    exc,
                )
            if child_notification_unresolved:
                notify_on_complete = False
                watch_patterns = None
        elif background and (notify_on_complete or watch_patterns):
            notification_parent_session_id = top_level_session_id
        ...
        if background:
            ...
            if notify_on_complete or watch_patterns:
                ...
                if not _async_ok():
                    ...
                    if watcher_platform:
                        ...
                        if not notification_parent_session_id and not (
                            isinstance(task_id, str) and task_id.startswith("sa-")
                        ):
                            notification_parent_session_id = _gse(
                                "HERMES_SESSION_ID", ""
                            )
                    if notify_on_complete and watcher_platform:
                        watcher_interval = 5
            ...
            try:
                ...
                # Notification contract was applied atomically at spawn. Only
                # update the tool result and register a live gateway watcher.
                ...
                if child_notification_unresolved:
                    ...
                    result_data["notify_unsupported"] = (
                        "notify_on_complete / watch_patterns were disabled because "
                        "this delegated child has no durable parent conversation "
                        "to route completions to after the child session ends."
                    )
                if conflict_note:

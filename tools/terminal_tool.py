...
def _ensure_terminal_env_bridged() -> None:
    ...
    under its own ``set_hermes_home_override``), and a unified dashboard
    ...
    commands on the host, #98581). When the home changes, the env vars the
    previous home's terminal section explicitly owns are unset before
    re-bridging, so each profile resolves from its own config — the same
    re-bridge ``web_server.py`` performs per-request for the embedded chat
    PTY. Within one home the flag keeps its pure one-shot optimization
    semantics.
    """
    ...
    with _terminal_config_bridge_lock:
        ...
        # previous home's backend selection (#94200, #98581). This runs
        ...
        # re-leak the previous home's backend selection precisely when
        ...
        try:
            ...
            if has_terminal_section:
                ...
                _terminal_config_bridge_keys = terminal_config_owned_env_vars(
                    raw_terminal
                )

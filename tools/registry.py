"""Central registry for all orion-agent tools.
...
from orion_constants import orion_home_key
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (orion_constants itself is stdlib-only, so no cycle).
        from orion_constants import get_orion_home

        return Path(get_orion_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``orion tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Orion-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("ORION_SESSION_ID", ""),
            get_session_env("ORION_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("ORION_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``orion tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved ORION_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("orion_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``orion_plugins.{name}``), not the exact module string.
                # A handler defined in ``orion_plugins.pkg.handlers`` is
                # still owned by the ``orion_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``orion tools enable foo``)

...
URL of Orion' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``OrionTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Orion Agent"
...
from orion_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__orion_user_skipped__"
...
def _get_token_dir(orion_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses ORION_HOME so each profile gets its own OAuth tokens.
    Layout: ``ORION_HOME/mcp-tokens/``
    """
    from orion_constants import get_orion_home

    base = Path(orion_home) if orion_home is not None else Path(get_orion_home())
...
def _cached_redirect_port(storage: "OrionTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Orion restarts and chooses a
...
def _cached_redirect_uri(storage: "OrionTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``orion mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `orion mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# OrionTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class OrionTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ORION_HOME/mcp-tokens/<server_name>.json         -- tokens
        ORION_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ORION_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        ORION_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, orion_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._orion_home = Path(orion_home) if orion_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._orion_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._orion_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._orion_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._orion_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Orion records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``orion mcp login`` /
        ``orion mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._orion_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Orion.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://your-orion-docs.example/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Orion' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `orion mcp login <server>` later to "
...
OrionOAuthClientProvider: Any = None


def _get_orion_oauth_provider_class() -> type | None:
    global OrionOAuthClientProvider
    if OrionOAuthClientProvider is not None:
        return OrionOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _OrionOAuthClientProvider(OAuthClientProvider):
    ...
    _OrionOAuthClientProvider.__name__ = "OrionOAuthClientProvider"
    _OrionOAuthClientProvider.__qualname__ = "OrionOAuthClientProvider"
    OrionOAuthClientProvider = _OrionOAuthClientProvider
    return OrionOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = OrionTokenStorage(server_name, orion_home=orion_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Orion only decides whether a
...
# your-orion-docs.example/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://nousresearch.github.io/orion-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Orion picks
...
def _has_cached_client_info(storage: "OrionTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "OrionTokenStorage | None") -> bool:
    ...
    Orion has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "OrionTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Orion would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Orion Agent" / "Orion" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Orion Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Orion is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: orion mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Orion auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Orion is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`orion mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_orion_oauth_provider_class()

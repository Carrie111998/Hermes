...
Read this with the repository root `AGENTS.md`; the footprint ladder remains the first decision for any tool change.
...
terminal session. Both top-level shapes always run asynchronously: the
parent receives a delegation id immediately and the result re-enters the
conversation through the async-delegation completion queue. The legacy
`background` argument is deprecated and ignored.
...
Durability rule: `delegate_task` is detached from the current turn but still process-local. For work that must survive process restart, use

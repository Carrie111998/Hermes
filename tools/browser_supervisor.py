...
One ``CDPSupervisor`` runs per Orion ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Orion logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "orion-dialog-bridge.invalid"
...
(() => {
  if (window.__orionDialogBridgeInstalled) return;
  window.__orionDialogBridgeInstalled = true;
  const ENDPOINT = "http://orion-dialog-bridge.invalid/";

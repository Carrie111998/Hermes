...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``HERMES_CUA_DRIVER_CMD`` wins, followed by
    ``computer_use.driver_command`` in config.yaml. Only when none is set do
    ...
    if override is not None:
        ...
        if not configured:
            configured = str(_computer_use_cfg().get("driver_command") or "").strip()
    if configured:
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override, ``HERMES_CUA_DRIVER_CMD``, or configured
    ``computer_use.driver_command`` is never silently replaced by another
    binary. Otherwise resolve PATH first, then canonical user-local install
    locations used by the official installer.
    """

"""
`orion computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Orion side — the only
...
def _cua_child_env() -> Dict[str, str]:
    """cua-driver child env with the Orion telemetry policy applied.
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Orion provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Orion-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    Honors `ORION_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if not binary:
        ...
        print("  Run: orion computer-use install")
        return 2
    ...
    if json_output:
        ...
        # attach Orion identity under orion_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["orion_identity"] = identity

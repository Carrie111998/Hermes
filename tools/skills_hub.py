#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Orion Skills Hub.
...
Used by orion_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _orion_home() / "skills"
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Orion) but not activated
    ...
    OFFICIAL_REPO = "Orion/orion-agent"
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``orion update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                orion_meta = meta_block.get("orion", {})
                if isinstance(orion_meta, dict):
                    tags = orion_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``orion_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Orion centralized index source
# ---------------------------------------------------------------------------

ORION_INDEX_URL = "https://your-orion-docs.example/docs/api/skills-index.json"
ORION_INDEX_TTL = 6 * 3600  # 6 hours


def _orion_index_cache_file() -> Path:
    return _index_cache_dir() / "orion-index.json"


def _load_orion_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ORION_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    orion_index_cache_file = _orion_index_cache_file()
    if orion_index_cache_file.exists():
        try:
            age = time.time() - orion_index_cache_file.stat().st_mtime
            if age < ORION_INDEX_TTL:
                return json.loads(orion_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Orion index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Orion index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Orion index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        orion_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        orion_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class OrionIndexSource(SkillSource):
    """Skill source backed by the centralized Orion Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "orion-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "orion-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        OrionIndexSource(auth=auth), # Centralized index (search + resolved install paths)

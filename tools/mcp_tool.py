...
class MCPServerTask:
    ...
    def _register_discovered_tools_if_needed(self) -> None:
        ...
        A server retained after recoverable initial failure is also already
        registry-owned before its first successful session. Publish freshly
        discovered tools for either owned case before transport readiness is
        restored so a successful revival cannot return with zero tools.
        """
        with _lock:
            registry_owned = _servers.get(self.name) is self
        if self._registered_tool_names:
            return
        if not self._ready.is_set() and not registry_owned:
            return
        self._registered_tool_names = _register_server_tools(
            self.name, self, self._config
        )

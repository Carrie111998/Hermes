...
try:
    ...
    # else keeps working.
    try:
        from mcp.types import ElicitRequestParams, ElicitResult  # noqa: F401 - availability probe; the import IS the test
    ...
    async def _discover_tools(self):
        ...
        async with self._rpc_lock:
            self._tools = await asyncio.wait_for(
                _paginate_full_list(self.session.list_tools, "tools", self.name),

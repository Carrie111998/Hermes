...
SESSION_SEARCH_SCHEMA = {
    "name": "session_search",
    "description": (
        "Search or read actual messages from Hermes conversation history. Use one of "
        "four shapes: query for discovery; session_id plus around_message_id to scroll; "
        "session_id alone to read; or no arguments to browse recent sessions. Discovery "
        "returns bookends plus a window around each hit; reuse boundary message IDs to "
        "scroll. FTS5 supports AND by default, OR, quoted phrases, NOT, and prefix *.\n\n"
        "History is secondary context, not evidence of a live external source. When the "
        "user provides a URL, account, contact, app/thread, file, or live system, inspect "
        "that source first when accessible. If it is inaccessible, say so before falling "
        "back to history; never infer 'not found' there from session_search alone.\n\n"
        "To resolve @session:<profile>/<id>, pass the profile and session_id. When citing "
        "a result, copy its link value verbatim and inline as the title; do not wrap it in "
        "backticks, repeat the title/date, or place the link alone."
    ),
    "parameters": {
        "type": "object",
        "properties": {

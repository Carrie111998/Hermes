...
SESSION_SEARCH_SCHEMA = {"name": "session_search", "description": "Search past Hermes sessions in the local SQLite DB, or scroll inside one. FTS5-backed, no LLM calls. SOURCE-FIRST: this is conversation history only \u2014 inspect the original source (URL/contact/file/system) first when one is provided. Shapes: (1) DISCOVERY session_search(query, limit) \u2014 FTS5 hits deduped by lineage; (2) SCROLL session_search(session_id, around_message_id, window) \u2014 \u00b1N messages around an anchor; (3) READ session_search(session_id) \u2014 whole session (first 20 + last 10 when large); (4) BROWSE session_search() \u2014 recent sessions chronologically. Results carry a `link` (e.g. @session:default/20260722_204335_d62c16); write it inline verbatim, never as a markdown link. FTS5: AND default; OR, quoted phrases, boolean, prefix wildcards supported.", "parameters": {"type": "object", "properties": {"query": {"type": "string", "description": "FTS5 query (discovery). Keywords/phrases/boolean. Omit to browse recent sessions."}, "session_id": {"type": "string", "description": "Scroll: session to read inside (pairs with around_message_id). Read: whole-session dump by id (optionally profile)."}, "around_message_id": {"type": "integer", "description": "Scroll only: anchor message id; returns \u00b1window around it."}, "window": {"type": "integer", "description": "Scroll only: messages per side of anchor. Default 5, max 20."}, "limit": {"type": "integer", "description": "Discovery only: max sessions (default 3, max 10)."}, "sort": {"type": "string", "description": "Discovery only: 'newest' or 'oldest'; omit for relevance."}, "detail": {"type": "string", "description": "Discovery only: 'adaptive' (default) or 'full' hydration."}, "role_filter": {"type": "string", "description": "Comma-separated roles (user,assistant,tool). Discovery default: user,assistant."}, "profile": {"type": "string", "description": "Read another profile's DB (read-only). For @session:<profile>/<id> links."}}, "required": []}}

# [schema-trim 2026-08-18]

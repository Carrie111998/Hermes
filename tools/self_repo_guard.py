...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Orion's live source checkout "
        ...
        "checkout, stop Orion, run the command externally, then restart "
        "Orion."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(orion_home).expanduser() if orion_home else Path.home() / ".orion"

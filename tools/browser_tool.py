...
from orion_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Orion secret
...
def _build_browser_env() -> dict:
    ...
    Strips Orion-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``orion_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = orion_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    orion_node_bin = str(orion_home / "node" / "bin")
    orion_node_root = str(orion_home / "node")
    orion_nm_bin = str(orion_home / "node_modules" / ".bin")
    return [orion_node_bin, orion_node_root, orion_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
    ...
    if "sandbox" in combined:
        ...
        if _running_in_docker():
            hints.append(
                ...
                "docker pull ghcr.io/nousresearch/orion-agent:latest"
            )
        else:
            hints.append(
                "The browser daemon may still be starting, or Chromium may be "
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``orion doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.orion/plugins/browser/<vendor>/``.
...
def _ensure_browser_plugins_loaded() -> None:
    ...
    """
    try:
        from orion_cli.plugins import _ensure_plugins_discovered
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Orion profile."""
    global _cached_cloud_provider, _cloud_provider_resolved
    global _cached_cloud_provider_scope

    scope = orion_home_key()
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.orion/plugins/browser/<vendor>/``) participate
...
# comparison sites (four here, plus orion_cli/tools_config.py and
# orion_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Orion silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Orion-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Orion CLI where sys.stdout and sys.stderr both report
...
def _allow_private_urls() -> bool:
    ...
    if get_orion_home_override() is not None:
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-orion_…`` the resulting socket path exceeds the
...
# run exactly once, before the cleanup loop started, which meant a orion
...
# Hard ceiling for a daemon whose owning orion process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed orion processes — this way every clean orion exit sweeps
    ...
    # Sweep orphans from other crashed orion processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live orion processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current orion PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live orion process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    orion restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning orion process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced orion PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent orion instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-orion_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live orion process and must not be touched — but "owner alive"
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Orion-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Orion-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_orion_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Orion-managed
    ...
    # node_modules binary, which disappears on the next ``orion update`` and
    ...
    # Build an extended search PATH including Orion-managed Node, macOS
    ...
    try:
        from orion_cli.dep_ensure import ensure_dependency
        if ensure_dependency("browser"):
            candidates = [
                shutil.which("agent-browser"),
                shutil.which("agent-browser", path=extended_path) if extended_path else None,
                shutil.which("agent-browser", path=str(get_orion_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_orion_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_orion_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``orion update`` (or ``orion doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Orion holds, while running registry-fetched
    npm code on every ``orion update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `orion update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this orion PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        import hashlib
        from orion_constants import get_orion_dir
        ...
        cache_dir = get_orion_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = orion_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Orion
    ...
    from orion_constants import get_orion_dir
    screenshots_dir = get_orion_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/orion-agent:latest")
                else:

...
def _chrome_for_testing_executable(chrome_dir: str) -> bool:
    """Return True when *chrome_dir* contains a Chrome for Testing binary.

    An interrupted download can leave a ``chrome-<version>`` directory
    without the actual executable.  Check for the platform-specific
    binary rather than treating the directory name alone as proof.
    """
    if sys.platform == "win32":
        return os.path.isfile(os.path.join(chrome_dir, "chrome.exe"))
    if sys.platform == "darwin":
    ...
    return os.path.isfile(os.path.join(chrome_dir, "chrome"))
...
def _chromium_search_roots() -> List[str]:
    ...
    roots.append(os.path.join(_agent_browser_state_dir(), "browsers"))
    return roots
        ...
        for entry in entries:
            if (
                ...
                or (
                    ...
                    and _chrome_for_testing_executable(os.path.join(root, entry))
                )
            ):
                _cached_chromium_installed = True
                return True

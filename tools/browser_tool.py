...
def _is_non_interactive_session() -> bool:
    ...
    """
    # A delegate_task child is always non-interactive — no human sits at its
    # prompt — regardless of the parent's TTY or explicit override. main wraps
    # both child construction and run in delegated_child_context()
    # (agent/delegation_context.py), so gating on it also fixes the
    # construction-time tool snapshot, superseding this PR's own subagent mark (#69071).
    try:

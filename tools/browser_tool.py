...
def browser_snapshot(
    ...
    if result.get("success"):
        ...
        # Re-check eval-navigated pages before exposing their content.  Keep this
        # response distinct from input-action errors, but share the guard and URL
        # probe with every other browser content-returning entry point.
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    # Re-check eval-navigated pages before capturing their pixels.  Use the
    # shared gate/probe so snapshots and vision retain identical SSRF semantics.
    if _eval_ssrf_guard_active(effective_task_id):
        _blocked_url = _current_page_private_url(effective_task_id)
        if _blocked_url:
            return json.dumps({

...
# Cache validation (mirrors orion_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.orion/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set ORION_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped ORION_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ORION_HOME/.env."""
    ...
    # the canonical readers in orion_cli/config.py).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set ORION_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ORION_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("ORION_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ORION_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_orion_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.orion/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ORION_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("ORION_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ORION_PLATFORM`` environment variable
    3. ``ORION_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from orion_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ORION_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.orion/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``orion skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile ORION_HOME; the module-level
...
def skills_list(category: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        try:
            from orion_cli.plugins import discover_plugins, get_plugin_manager
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from orion_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: orion plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `orion skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.orion/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `orion skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.orion.* first (agentskills.io convention), fall back to top-level
        orion_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            orion_meta = metadata.get("orion", {}) or {}

        tags = _parse_tags(orion_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            orion_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `orion sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )

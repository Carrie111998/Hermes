...
`orion skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.orion.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.orion.blueprint`` is a mapping containing
    ...
    orion = meta.get("orion") if isinstance(meta, dict) else None
    blueprint = orion.get("blueprint") if isinstance(orion, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.orion.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.orion.blueprint`` block) they can hand
    to ``orion skills publish`` to share. ``body`` is the plain-language

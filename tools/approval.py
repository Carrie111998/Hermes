...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ORION_YOLO_MODE", ""))
...
# Orion session id (observability identity, distinct from the gateway
...
# os.environ["ORION_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export ORION_INTERACTIVE.
_orion_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "orion_interactive",
    default=None,
)


def set_orion_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["ORION_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``ORION_INTERACTIVE`` env var for legacy callers.
    """
    return _orion_interactive_ctx.set("1" if interactive else "")


def reset_orion_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_orion_interactive_context`."""
    _orion_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``ORION_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _orion_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("ORION_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Orion session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ORION_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("ORION_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("ORION_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("ORION_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``orion chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``ORION_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("ORION_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("ORION_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ORION_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ORION_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ORION_SESSION_PLATFORM via
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``ORION_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $ORION_HOME, or by the resolved absolute
# active profile home path such as /home/orion/.orion/config.yaml. The
# resolved-absolute form is folded into the ~/.orion/ patterns at detection
...
# go stale when ORION_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ORION_ENV_PATH = (
    r'(?:~\/\.orion/|'
    r'(?:\$home|\$\{home\})/\.orion/|'
    r'(?:\$orion_home|\$\{orion_home\})/)'
    r'\.env\b'
)
# ~/.orion/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ORION_ENV_PATH; matches the ORION_HOME override form as
# well as ~/.orion/.
_ORION_CONFIG_PATH = (
    r'(?:~\/\.orion/|'
    r'(?:\$home|\$\{home\})/\.orion/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_orion_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Orion: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Orion
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]orion[^\n]*\.env\b', "access to Orion secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `orion` and `gateway` (e.g. `orion -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\borion\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart orion gateway (kills running agents)"),
    (r'\borion\s+update\b', "orion update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `orion gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `orion ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart orion-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart orion-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(orion|gateway|cli\.py)\b', "kill orion/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill orion` but not
    # `kill -9 $(pgrep -f orion)` because the substitution is opaque
    ...
    # the `orion gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.orion.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(orion|ai\.orion)\b', "stop/restart orion launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Orion file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.orion/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Orion-managed security file (~/.orion/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ORION_CONFIG_PATH}|{_ORION_ENV_PATH})', "in-place edit of Orion config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ORION_CONFIG_PATH}|{_ORION_ENV_PATH})', "in-place edit of Orion config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ORION_CONFIG_PATH}|{_ORION_ENV_PATH})', "in-place edit of Orion config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.orion/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ORION_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Orion home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\orion), so folding the user home
    # first would eat the prefix the Orion-home fold needs.
    command = _rewrite_resolved_orion_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ORION_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.orion``); the matched
...
def _rewrite_resolved_orion_home(command: str) -> str:
    """Rewrite the resolved absolute Orion home prefix to ``~/.orion/``.

    Resolves the active ``ORION_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.orion/`` so the static ``_ORION_CONFIG_PATH`` / ``_ORION_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/orion/.orion/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$ORION_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.orion")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``orion send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Orion ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"orion-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Orion YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Orion approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``ORION_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            ORION_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export ORION_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set ORION_INTERACTIVE or "
                "ORION_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set ORION_INTERACTIVE or "
            "ORION_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ORION_EXEC_ASK"):
        ...
        # can see (ORION_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``ORION_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from orion_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from orion_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ORION_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # ORION_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from orion_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (ORION_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # ORION_EXEC_ASK (and sometimes a session platform marker) can leak

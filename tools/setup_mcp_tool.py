...
``orion mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Orion desktop app. Use the "
            "terminal instead: `orion mcp install <name>` for catalog entries, "
            "`orion mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Orion desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `orion mcp catalog` "

...
class ShellFileOperations(FileOperations):
    ...
    def patch_multi_edit(self, path: str, edits: List[Dict[str, Any]]) -> PatchResult:
        ...
        least one edit succeeded. If *zero* edits succeeded, nothing is
        written and an error is returned. Because the file is written a
        single time from the final in-memory string, a multi-edit batch is
        all-or-nothing on disk (never a half-applied write). The syntax
        check runs once at the end on the final content.

...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `orion model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``orion auth add
    ...
    2. The environment / ``~/.orion/.env``. Under a multiplexed gateway turn
       ...
       ``orion_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``orion auth
    ...
    when omitted, ``orion_cli.config.get_env_value`` is used directly.
    """
    ...
    if env_getter is not None:
        ...
        try:
            from orion_cli.config import get_env_value
    ...
    try:
        ...
        # `orion auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``orion auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `orion tools` provider string for a config section.
    ...
    """
    try:
        from orion_cli.config import read_raw_config_readonly
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via orion "
        f"tools), but {failure}. Run 'orion tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.orion/.env`` (via
    ``orion_cli.config.get_env_value`` when available) so tool-side

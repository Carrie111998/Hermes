...
WHAT IS WITHHELD, AND WHAT DELIBERATELY IS NOT.
    ...
    execute_code         writes files as easily as it computes
    terminal             the honest cost of this switch

`terminal` is withheld because it is a mutation vector -- `echo > file` is an
edit -- and leaving it would make the guarantee a slogan. The price is real:
Hermes can no longer run `git status` to answer a question about the working
tree. Reading and searching remain, which covers most of what an investigation
actually needs, and anything further is a reason to turn the switch off rather
than to weaken it.
"""
...
# The tools that can change a repository. Named explicitly rather than matched by
# pattern: a substring rule would quietly start withholding a future tool nobody
# considered, and quietly stop withholding one that gets renamed.
MUTATING_TOOLS = frozenset({"patch", "write_file", "execute_code", "terminal"})

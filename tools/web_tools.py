    ...
    try:
        ...
        if not safe_urls:
            ...
            if not fetch_urls:
                ...
                # Only forward well-formed string→string header pairs, and
                # only when the caller actually supplied some. Passing ``{}`` /
                # ``None`` keeps the argument absent so backends that don't read
                # headers see exactly their previous call shape.
                ...
                try:
                    if inspect.iscoroutinefunction(provider.extract):
                        ...
                        results = await asyncio.to_thread(
                            lambda: provider.extract(fetch_urls, **extract_kwargs)
                        )
                except Exception as exc:  # noqa: BLE001 — candidate for rescue
                    if _rescue_eligible(provider):

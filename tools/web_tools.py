...
async def web_extract_tool(
    ...
    try:
        ...
        if not safe_urls:
            ...
            # blocklist policy. A policy block is terminal: preserve it as a
            # result row and never send that URL to the primary, configured
            # fallback, or keyless rescue provider. Cache hits skip only the
            # vendor call, never a control.
            ...
            if not fetch_urls:
                ...
                try:
                    ...
                    if fallback_results is not None:
                        ...
                        if _rescue_eligible(provider) and _fallback_extract_needs_rescue(
                            fetch_urls, fallback_results
                        ):
                            results = await asyncio.to_thread(
                                _rescue_extract,
                                provider.name,
                    ...
                    whole_batch_failure = not results or all(
                        r.get("error") for r in results
                    )
                    if whole_batch_failure:
                        primary_failures = results or [
                            {
                                "url": url,
                                "title": "",
                                "content": "",
                                "error": "Extract backend returned no results",
                            }
                        ...
                        if fallback_results is not None:
                            ...
                            if _rescue_eligible(
                                provider
                            ) and _fallback_extract_needs_rescue(
                                fetch_urls, fallback_results
                            ):
                                results = await asyncio.to_thread(
                                    _rescue_extract,
                            ...
                            _served_non_primary = True
                            rescue_results = primary_failures
                            if fallback_error:
                                rescue_results = [
                                    {
                                        **result,
                                    ...
                                    for result in primary_failures
                                ]
                            results = await asyncio.to_thread(
                                _rescue_extract,
                                provider.name,

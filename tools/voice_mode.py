    ...
    try:
        with sd.InputStream(samplerate=SAMPLE_RATE, channels=1, dtype="int16", blocksize=block) as stream:
            while not should_stop():
                ...
                # so the trigger stays at least 3200 RMS: normal TTS playback
                ...
                if min_floor == 0.0 and len(floor_window) >= calib_blocks:
                    candidate = float(np.percentile(list(floor_window), 90))
                    min_floor = max(candidate, SILENCE_RMS_THRESHOLD * 2)
                ...
                _floor = float(np.percentile(list(floor_window), 90))
                _floor = max(_floor, min_floor)

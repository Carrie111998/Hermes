...
CRONJOB_SCHEMA = {
    "name": "cronjob",
    "description": "Manage scheduled cron jobs: create/list/update/pause/resume/remove/run. On create, 'schedule' and 'prompt' are REQUIRED. Jobs run in a fresh session, so prompts must be self-contained. Final response is auto-delivered to the target. Put user-facing content in the final response; no mid-run questions. Pass skills=[] on update to clear attached skills; pass prompt with action='run' to inject transient per-run context. For recurring jobs, prefer updating an existing job over creating near-duplicates.",
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
                "type": "string",
                "description": "The action to perform: create/list/update/pause/resume/remove/run."
            },
            "job_id": {
                "type": "string",
                "description": "Target job ID (required for update/pause/resume/remove/run). Never guess IDs; list first."
            },
            "prompt": {
                "type": "string",
                "description": "For create: the full self-contained task instruction; skills are loaded first, then prompt as the task. For run: optional transient context appended to the stored prompt for that single fire only (never persisted)."
            },
            "schedule": {
                "type": "string",
                "description": "REQUIRED for create/update. '30m', 'every 2h', '0 9 * * *', or ISO timestamp."
            },
            "name": {
                "type": "string",
            ...
            "repeat": {
                "type": "integer",
                "description": "Optional repeat count. Defaults: once for one-shot, forever for recurring."
            },
            "deliver": {
                "type": "string",
                "description": "Omit to auto-deliver to current chat/topic. 'origin' same as omitting; 'local' saves only; 'all' fans out; or platform:chat_id:thread_id. WARNING: platform:chat_id without :thread_id loses topic targeting; 'all' resolves at fire time."
            },
            "skills": {
                "type": "array",
                "items": {
                ...
                "description": "Optional ordered list of skill names loaded before the prompt. Empty array clears on update."
            },
            "script": {
                "type": "string",
                "description": "Optional path to a script run each tick; stdout injected into the agent prompt (or delivered verbatim with no_agent=True). .sh/.bash run via bash, else Python. Relative paths resolve under /opt/data/scripts/. Empty string clears on update."
            },
            "monitor_script": {
                "type": "string",
                "description": "Optional monitor-mode source script (same rules as script). Runs first each tick; UNCHANGED output suppresses the agent run entirely; CHANGED output injects a MONITOR CHANGE DETECTED block. First tick always runs the agent. Must emit STABLE output. Mutually exclusive with monitor_url; incompatible with no_agent=True. Empty clears on update."
            },
            "monitor_url": {
                "type": "string",
                "description": "Optional http(s) URL used as the monitor source instead of a script; fetched with a bounded GET each tick. Same hash-suppression semantics as monitor_script. Mutually exclusive with monitor_script."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
                "description": "True: skip the LLM; scheduler runs script and delivers stdout verbatim. Non-empty stdout sent verbatim; EMPTY stdout = SILENT (nothing sent); non-zero exit/timeout sends an error alert. Use for script-only pings/watchdogs. Default False."
            },
            "context_from": {
                "type": "array",
                "items": {
                ...
                "description": "Optional job ID or list whose most recent completed output is injected before each run. Note: injects most recent output \u2014 does not wait for upstream jobs in the same tick."
            },
            "continuity": {
                "type": "boolean",
                "description": "True: recurring job carries continuity across runs \u2014 each run wakes with the job's own most recent output injected, so it can dedupe/continue (scouts, monitors, incremental digests). First run runs unchanged. On update, pass false to turn continuity off (other context_from entries preserved). Default: false."
            },
            "enabled_toolsets": {
                "type": "array",
                "items": {
                ...
                "description": "Optional toolset names to restrict the job's agent. Reduces input token overhead. When set, only these toolsets are loaded. Infer from the job's prompt. Empty array clears on update."
            },
            "workdir": {
                "type": "string",
                "description": "Optional absolute path to run the job from. Injects AGENTS.md/CLAUDE.md/.cursorrules from that dir into the system prompt; terminal/file/code_exec tools use it as cwd. Must exist. Jobs with workdir run sequentially. Empty string clears."
            },
            "attach_to_session": {
                "type": "boolean",
                "description": "True: job becomes CONTINUABLE \u2014 user can reply to its delivery with brief in context. Opens a dedicated thread on thread-capable platforms; mirrors brief into origin DM on DM-only. Overrides cron.mirror_delivery for this job. Only origin chat touched. Leave unset for fire-and-forget."
            }
        },
        "required": [
            "action"

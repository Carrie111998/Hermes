...
_PRIVATE_BROWSER_SENTINEL = "_ORION_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# orion: pin this named session to its own tab (once per daemon process)
def _orion_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "orion-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_orion_ensure_own_tab()
del _orion_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Orion's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Orion's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_orion_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `orion tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Orion' own bin dir ($ORION_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from orion_constants import get_orion_home

        return str(Path(get_orion_home()) / "bin")
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Orion' managed uv (bootstrapped on demand via
    ``orion_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$ORION_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Orion-managed copy, or resolution
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_orion_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `orion tools` → Browser Automation."
        )

...
(see ``orion_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<ORION_HOME>/pending/{memory,skills}/<id>.json``
...
def write_approval_enabled(subsystem: str) -> bool:
    ...
    if subsystem not in _SUBSYSTEMS:
        return False
    try:
        from orion_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _pending_dir(subsystem: str) -> Path:
    return get_orion_home() / "pending" / subsystem

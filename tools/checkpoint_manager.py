...
-----------------------------------------------------------------------------
    ...
    ~/.orion/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/orion/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_orion_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/orion"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Orion convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("ORION_CHECKPOINT_TIMEOUT", 30)))
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Orion wrote
...
def _git_env(
    ...
    The shared store is internal Orion infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``orion checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `orion checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "orion@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Orion Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``ORION_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at ORION_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "ORION_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Orion just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Orion last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Orion last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Orion'
        last write or that Orion never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Orion has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Orion never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Orion wrote it: restoring it back is
                # safe — its last content was Orion-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Orion changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Orion changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Orion' last write — per the agent-write ledger —
        are left untouched, and only Orion-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Orion-created files absent from it (delete to restore state).
...
# ---------------------------------------------------------------------------
# Public helpers for `orion checkpoints` CLI
# ---------------------------------------------------------------------------

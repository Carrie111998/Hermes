...
it is automatically downloaded from GitHub releases to $ORION_HOME/bin/tirith.
...
def _get_orion_home() -> str:
    """Return the Orion home directory, respecting ORION_HOME env var."""
    return str(get_orion_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_orion_home(), ".tirith-install-failed")
...
def _orion_bin_dir() -> str:
    """Return $ORION_HOME/bin, creating it if needed."""
    d = os.path.join(_get_orion_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ORION_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_orion_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ORION_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ORION_HOME/bin/tirith
    ...
    orion_bin = os.path.join(_orion_bin_dir(), "tirith")
    if os.path.isfile(orion_bin) and os.access(orion_bin, os.X_OK):
        _resolved_path = orion_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return orion_bin

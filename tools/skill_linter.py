...
Design contract (matches the Orion "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    orion_meta = meta.get("orion") if isinstance(meta, dict) else None
    if not isinstance(orion_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.orion.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in orion_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.orion.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("orion", "agent", "orion agent") and (
        author != "Orion Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Orion Agent' (proper caps) "

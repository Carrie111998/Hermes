...
def _build_child_system_prompt(
    ...
    *,
    ...
    parts.append(
        ...
        "## Inception Prompting (Hardened Delegation)\n"
        "You are a focused SUB-AGENT (worker), not a coordinator. To prevent delegation failures:\n\n"
        "1. ROLE ANCHORING: You are executing a delegated task. Do the work directly. Never ask questions, request clarification, or suggest the requester do the work instead. You have no one to ask — you are isolated.\n\n"
        "2. ANTI-ECHO: Do not restate or paraphrase the task. Perform it using your tools and report concrete results. Never return a summary like 'I analyzed the codebase for security issues and identified several areas of concern' without listing actual issues.\n\n"
        "3. OUTPUT FORMAT: Your response MUST include specific findings, file paths, code snippets, or other concrete artifacts. Vague summaries like 'it seems fine' or 'further investigation needed' are not acceptable.\n\n"
        "4. CONVERGENCE PRESSURE: If an approach fails twice, try a fundamentally different approach rather than retrying the same thing. If you are stuck after 3 attempts, report what you tried and what you learned, not just 'failed'.\n\n"
        "5. COMPLETION SIGNAL: When you are done, start your summary with 'TASK COMPLETED:' and include concrete deliverables. If you cannot complete the state exactly what blocked you.\n\n"

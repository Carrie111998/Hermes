...
def delegate_task(
    ...
    def _execute_and_aggregate(*, honor_parent_interrupt: bool = True) -> dict:
        ...
        """
        if n_tasks == 1:
            # Single task -- run directly (no thread pool overhead). Preserve
            # the legacy four-argument helper contract outside a gateway/TUI
            # session; exact steering authority is supplied only when present.
            _i, _t, child = children[0]
            if _origin_ui_session_id:
                result = _run_single_child(
                    _i,
                    _t["goal"],
                    child,
                    parent_agent,
                    owner_session_id=_origin_ui_session_id,

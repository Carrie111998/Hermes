...
def _build_child_agent(
    ...
    if custom_role is not None:
        ...
        effective_model = (
            apply_role_model(custom_role, model, parent_agent.model)
            or model
            or parent_agent.model
            or ""
        )
...
def _build_role_param_description() -> str:
    ...
    try:
        ...
        role_names = sorted(get_agent_roles().keys())
    except Exception:
        role_names = []
    custom_note = ""
    if role_names:
        custom_note = (
            f" Configured bounded roles (delegation.roles): {', '.join(role_names)}. "
            "A bounded role may append instructions, point the child at a model, "
            "or trim its toolsets — never above the parent's authority."
        )

    return (
        "Role of the child agent. 'leaf' (default) = focused "
        "worker, cannot delegate further. 'orchestrator' = can "
        f"use delegate_task to spawn its own workers. {nesting_note}{custom_note}"
    )

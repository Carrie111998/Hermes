...
def _build_top_level_description() -> str:
    ...
    return (
        "Spawn subagents in isolated contexts; only their final summaries return "
        "to the parent. Each child has its own conversation, terminal session, "
        "and toolset.\n\n"
        "MODES (provide one):\n"
        "- Single: `goal`, plus optional `context` and `role`.\n"
        f"- Batch: `tasks` with up to {max_children} parallel items. "
        f"{nesting_clause}\n\n"
        "Top-level delegations run in the background and re-enter the conversation "
        "when complete. Do not wait or poll; continue useful parent work. A batch "
        "returns one consolidated result after all children finish. The dispatch "
        "response includes live transcript paths for optional inspection. Nested "
        "orchestrator calls wait synchronously for their workers before synthesis.\n\n"
        "USE FOR: reasoning-heavy subtasks, context-isolating investigations, or "
        "independent parallel workstreams.\n"
        "DO NOT USE FOR: one tool call; mechanical loops better handled by "
        "execute_code; tasks needing user interaction; or durable work that must "
        "survive the parent session (use cronjob or terminal(background=True, "
        "notify_on_complete=True)).\n\n"
        "CONSTRAINTS:\n"
        "- Children have no conversation memory. Pass self-contained context with "
        "paths, errors, and constraints. If the user writes in a non-English language "
        "or requests a language, tone, or style, include those requirements.\n"
        "- Child summaries are self-reports. For external side effects or shared "
        "writes, require a verifiable handle and verify it independently from the "
        "parent before reporting success.\n"
        "- Leaf children cannot call delegate_task, clarify, cronjob, memory, send_message. "
        "Orchestrators retain delegation only when runtime nesting allows it; they "
        "still cannot call clarify, cronjob, memory, or send_message.\n"
        "- Children inherit the parent model and fallback chain unless delegation "
        "model/provider configuration pins them globally. Per-call model selection "
        "is unavailable.\n"
        "- Background delegations are not durable: /stop, /new, or parent process "
        "exit can cancel or discard them.\n"
        "- Results are arrays with one entry per task."
    )

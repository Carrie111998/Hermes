...
def list_active_subagents() -> List[Dict[str, Any]]:
    ...
    Each record: {subagent_id, parent_session_id, parent_id, depth, goal,
    model, started_at, tool_count, status}.  Safe to call from any thread —
    returns a copy.
    """
    with _active_subagents_lock:
        return [
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "background": {
                "type": "boolean",
                "description": (
                    "DEPRECATED / IGNORED. Top-level single and batch delegations "
                    "run in the background automatically on persistent interactive "
                    "and gateway sessions. A single result or consolidated batch "
                    "result re-enters the conversation when the work finishes. "
                    "Headless one-shot CLI sessions run "
...
def _model_background_value(args: dict, parent_agent=None) -> bool:
    ...
    batch, which returns one consolidated result. An orchestrator subagent
    (depth > 0) and a headless one-shot CLI agent both run synchronously because
    neither owns a persistent session that can receive a later completion. The
    live path is

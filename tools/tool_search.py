...
* While enabled, the three bridge schemas are byte-stable for the life of a
  conversation, including when the deferred catalog becomes empty. Live MCP
  and plugin mutations stay behind the bridge and never rewrite the model's
  cached tools prefix.
* The catalog is stateless and rebuilt from the session-scoped live registry
  for each bridge invocation. This is the lesson from OpenClaw's cron
  regression (openclaw/openclaw#84141): a session-keyed catalog that drifts
...
def estimate_tokens_from_schemas(tool_defs: Iterable[Dict[str, Any]]) -> int:
    ...
    Cheap and stable across providers. This is used only for observability;
    activation must not depend on catalog size because the bridge schema must
    remain stable across catalog mutations.
    """
    total_chars = 0
    for td in tool_defs:
        try:
...
def bridge_tool_schemas() -> List[Dict[str, Any]]:
    ...
    These schemas must remain byte-stable across live registry mutations.
    Catalog contents are resolved only when a bridge tool is invoked.
    """
    desc_search = (
        "Search additional tools that are loaded on demand. Before claiming "
        "a requested capability is unavailable or substituting a generic tool, "
        "search here. Returns up to `limit` matches with name and description. Follow "
...
def assemble_tool_defs(
    ...
    *,
    ...
    logger.info(
        "tool_search activated: %d core/visible tools kept, %d deferred "
        "(~%d tokens) behind a static bridge",

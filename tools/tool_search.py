"""Progressive tool disclosure ("tool search") for Orion Agent.
...
``tool_call`` — and surfaced on demand. Core Orion tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._ORION_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_ORION_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets. Off ``_ORION_CORE_TOOLS`` so non-GUI clients
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is neither in ``_ORION_CORE_TOOLS`` nor a session-gated GUI

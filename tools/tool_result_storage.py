...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   ``$ORION_HOME/cache/spillover/{tool_use_id}.txt`` — alongside the other
   Orion-owned caches (images, audio, documents, ...) instead of littering
...
STORAGE_DIR = "/tmp/orion-results"
SPILLOVER_SUBDIR = "cache/spillover"
SPILLOVER_MAX_AGE_HOURS = 24
HEREDOC_MARKER = "ORION_PERSIST_EOF"
...
def get_spillover_dir():
    """Return $ORION_HOME/cache/spillover as a Path (not created)."""
    from orion_constants import get_orion_home

    return get_orion_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write content host-side to $ORION_HOME/cache/spillover.
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/orion-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ORION_PERSIST_{uuid.uuid4().hex[:8]}"
...
def maybe_persist_tool_result(
    ...
    # Always persist host-side first: $ORION_HOME/cache/spillover is the
    # single canonical home for spilled results (with the other Orion-owned

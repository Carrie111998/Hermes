...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.orion",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``ORION_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ORION_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because ORION_HOME is exactly where
    ...
    """
    orion_home = _resolve_orion_home()

    # Reject absolute paths — they bypass the ORION_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ORION_HOME)",
            relative_path,
        )
        return False

    host_path = orion_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape ORION_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, orion_home)
    ...
    # inside ORION_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.orion/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="orion-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching orion_constants.get_orion_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_orion_dir()`` for backward compatibility with old directory layouts.
    """
    from orion_constants import get_orion_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_orion_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_orion_dir() already resolved
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.orion``,
    SSH ``<remote_home>/.orion``, etc.) and whether translation is wanted.

...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # Prevent prompt injection via catalog or hub metadata files, refuse
        # credential-bearing files outright, and block credential stores under
        # HERMES_HOME.
        #
        ...
        # the guard and still be read — and a relative read against
        # TERMINAL_CWD == HERMES_HOME (e.g. "auth.json") still hits the
        # denylist, since get_read_block_error's own resolve() runs against

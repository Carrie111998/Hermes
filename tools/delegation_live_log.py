...
per child under::
    ...
    <orion_home>/cache/delegation/live/<delegation_id>/task-<n>.log
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.orion)."""
    from orion_constants import get_orion_dir

    return get_orion_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        try:
            ...
            header = [
                "=== Orion subagent live transcript ===",

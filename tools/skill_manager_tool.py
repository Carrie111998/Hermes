...
SKILL_MANAGE_SCHEMA = {
    "name": "skill_manage",
    "description": (
        "Create and maintain reusable procedural skills. Use create for a new full "
        "SKILL.md, patch for targeted fixes, edit only for full rewrites, and "
        "write_file/remove_file for supporting files. New skills go under "
        f"{display_hermes_home()}/skills/; existing skills may live elsewhere. "
        "Confirm with the user before creating or deleting a skill.\n\n"
        "For deletion, set absorbed_into to an existing umbrella skill after merging, "
        "or to an empty string for pure pruning; this keeps cron and curator references "
        "consistent. Pinned skills cannot be deleted until unpinned, but may be patched. "
        "Keep the first 57 description characters self-contained because the system "
        "index truncates longer descriptions; full text remains available on demand."
    ),
    "parameters": {
        "type": "object",
        "properties": {

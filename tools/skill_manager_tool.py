...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "When creating a skill, record requirements the procedure actually used "
        "under `metadata.hermes`: `requires_tools`, `requires_toolsets`, and "
        "`requires_plugins`. Use stable tool/toolset/plugin IDs, include only "
        "requirements tied to this skill (never the full enabled inventory), and "
        "omit empty declarations. Collective Wisdom uses these local declarations "
        "to prefill the owner-reviewed System Specification; it never installs "

...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``orion_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so STT does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from orion_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``orion auth add <provider_id>``).
...
LOCAL_STT_COMMAND_ENV = "ORION_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "ORION_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via orion_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``ORION_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
#
# The single-env-var ``ORION_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Orion's remote upload cap."""
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__ORION_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_stt_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Orion secrets by
...
def _run_command_stt(
    ...
    Child env is scrubbed of Orion secrets (salvage of #56332) while still
    ...
    scrubbed = orion_subprocess_env(inherit_credentials=False)
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"orion-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set ORION_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `orion plugins list` to see "
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from orion_cli.plugins import has_hook, invoke_hook
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="orion-silk-")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(
            target=_watch, name="orion-stt-idle-unload", daemon=True
        )
...
def _transcribe_local_command(
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="orion-local-stt-") as output_dir:
            ...
            # Scrub Orion secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from tools.environments.local import orion_subprocess_env

            child_env = orion_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``ORION_LOCAL_STT_LANGUAGE`` (env).
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="orion-stt-") as work_dir:
                try:
    ...
    *,
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `orion model` or set XAI_API_KEY",
        }
    ...
    try:
        import requests
        from tools.xai_http import orion_xai_user_agent
            ...
            with open(file_path, "rb") as audio_file:
                return requests.post(
                    ...
                    headers={
                        ...
                        "User-Agent": orion_xai_user_agent(),
                    },
                    files={
...
def _transcribe_deepinfra(
    ...
    *,
    ...
    ``orion_cli.models`` helpers so every DeepInfra surface resolves the
    ...
    from orion_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _trim_silence_for_cloud_stt(
    ...
    work_dir = tempfile.mkdtemp(prefix="orion-stt-trim-")

...
`mcp_tool.py`, and `orion_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# OrionMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_orion_provider_class() -> Optional[type]:
    ...
    class OrionMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._orion_server_name = server_name
            self._orion_home = ""
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`OrionTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, OrionTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Orion' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._orion_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import OrionTokenStorage
                        if isinstance(storage, OrionTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._orion_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import OrionTokenStorage
            if not isinstance(storage, OrionTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``orion mcp reauth``.
            ...
            back to ``orion mcp reauth``.
            """
            try:
                if self._orion_preregistered:
                    return
                ...
                # walking back into the same refusal. `orion mcp login`
                ...
                if cimd_url and rejected_id == cimd_url:
                    logger.warning(
                        ...
                        self._orion_server_name, cimd_url,
                    )
    ...
    return OrionMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ORION_PROVIDER_CLS: Optional[type] = _make_orion_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._orion_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`OrionMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _ORION_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = OrionTokenStorage(server_name)
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `orion mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``orion mcp remove <name>`` and (indirectly) by
        ``orion mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, orion_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, orion_home=orion_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, orion_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, orion_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, orion_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(orion_home) / f"{_safe_filename(server_name)}.json"
            try:

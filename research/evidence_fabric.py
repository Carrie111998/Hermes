...
class EvidenceFabricService:
    ...
    def transition_research_run(self, run_id: str, status: ResearchRunStatus) -> ResearchRun:
        ...
        if row["status"] != "OPEN": raise EvidenceLifecycleError("terminal research run is immutable")
        now = _now()
    ...
    def create_claim(self, run_id: str, text: str, *, metadata: Mapping[str, Any] | None = None) -> ClaimRecord:
        ...
        cid=str(uuid.uuid4()); now=_now(); self._write(lambda c:c.execute("INSERT INTO claims VALUES (?,?,?,?,?,?,?,?,?,?,?)",(cid,run_id,text,"UNVERIFIED",self.scope.agent_id,self.scope.profile_name,None,None,_json(metadata),now.timestamp(),now.timestamp())))
    ...
    def link_evidence_to_claim(self, claim_id: str, evidence_id: str, relation: EvidenceRelation, *, passage_locator: Mapping[str, Any] | None = None) -> ClaimEvidenceLink:
        ...
        except sqlite3.IntegrityError as exc: raise EvidenceIntegrityError(str(exc)) from exc
    ...
    def set_claim_status(self, claim_id: str, status: ClaimStatus) -> ClaimRecord:
        self.get_claim(claim_id); status=ClaimStatus(status); now=_now()

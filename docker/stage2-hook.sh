...
#
# Wired into the image as /etc/cont-init.d/01-orion-setup by the
...
ORION_HOME="${ORION_HOME:-/opt/data}"
INSTALL_DIR="/opt/orion"

# Drop to orion via s6-setuidgid, but skip it when already non-root.
as_orion() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid orion "$@"; }
...
# starts non-root. The baked install tree under /opt/orion is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ORION_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ORION_UID/ORION_GID — or the PUID/PGID aliases — which the
...
# `id -u orion` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# orion UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u orion)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-orion UID).
...
and the baked /opt/orion install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ORION_UID=\$(id -u) -e ORION_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the orion user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ORION_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid orion mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ORION_HOME paths whose parents only
# root can create (e.g. `ORION_HOME=/home/orion/.orion` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ORION_UID/ORION_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ORION_UID/
# ORION_GID still win when both are set.  See #15290, salvages #25872.
ORION_UID="${ORION_UID:-${PUID:-}}"
ORION_GID="${ORION_GID:-${PGID:-}}"

if [ -n "${ORION_UID:-}" ] && validate_uid_gid "$ORION_UID" && [ "$ORION_UID" != "$(id -u orion)" ]; then
    echo "[stage2] Changing orion UID to $ORION_UID"
    usermod -u "$ORION_UID" orion
fi
if [ -n "${ORION_GID:-}" ] && validate_uid_gid "$ORION_GID" && [ "$ORION_GID" != "$(id -g orion)" ]; then
    echo "[stage2] Changing orion GID to $ORION_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ORION_GID" orion 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised orion user (UID 10000)
...
# orion process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes orion. Idempotent across container
...
#   - orion is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G orion 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] orion already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" orion 2>/dev/null; then
        echo "[stage2] Added orion to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group orion failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ORION_UID is remapped or the top-level $ORION_HOME isn't owned by
# the runtime orion UID, restore ownership to orion — but ONLY for the
# directories orion actually writes to. The full $ORION_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of orion-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_orion_uid=$(id -u orion)
...
chown_orion_tree() {
    ...
    chown -R orion:orion "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_orion_owner() {
    target="$1"
    find "$target" \( ! -user orion -o ! -group orion \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$ORION_HOME" 2>/dev/null)" != "$actual_orion_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ORION_HOME (targeted) to orion ($actual_orion_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ORION_HOME: chown the directory itself (not its contents)
    # so orion can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ORION_HOME"; then
        :
    else
        chown orion:orion "$ORION_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ORION_HOME failed (rootless container?) — continuing"
    fi
    # Orion-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by orion (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ORION_HOME/$sub" ] && tree_has_non_orion_owner "$ORION_HOME/$sub"; then
            chown_orion_tree "$ORION_HOME/$sub"
        fi
    done
fi
...
# orion user. Hosted/container instances keep mutable state under
# $ORION_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ORION_DISABLE_LAZY_INSTALLS=1. Keeping /opt/orion root-owned and
...
# $ORION_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ORION_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to orion
...
# Always reset ownership of $ORION_HOME/profiles to orion on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> orion …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as orion and walks
...
if [ -d "$ORION_HOME/profiles" ] && tree_has_non_orion_owner "$ORION_HOME/profiles"; then
    chown_orion_tree "$ORION_HOME/profiles"
fi

# Always reset ownership of $ORION_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged orion runtime even
...
# gate as profiles/).
if [ -d "$ORION_HOME/cron" ] && tree_has_non_orion_owner "$ORION_HOME/cron"; then
    chown_orion_tree "$ORION_HOME/cron"
fi

# Always ensure logs/gateways is orion-owned (#45258). Formerly healed by
...
# top-level $ORION_HOME is mis-owned, so a warm volume with orion-owned
# ORION_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid orion mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as orion.
if [ -d "$ORION_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ORION_HOME/logs/gateways"; then
        :
    else
        chown orion:orion "$ORION_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# orion pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged orion gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ORION_HOME is
...
# and cron/.
if [ -d "$ORION_HOME/platforms/pairing" ] && tree_has_non_orion_owner "$ORION_HOME/platforms/pairing"; then
    chown_orion_tree "$ORION_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ORION_HOME/pairing" ] && tree_has_non_orion_owner "$ORION_HOME/pairing"; then
    chown_orion_tree "$ORION_HOME/pairing"
fi

# Reset ownership of orion-owned top-level state files on every boot.
# The targeted data-volume chown above only covers orion-owned
# *subdirectories*; loose state files living directly under $ORION_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> orion …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged orion runtime then hits
...
# sweep so host-owned files in a bind-mounted $ORION_HOME are never
...
# entries of orion_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ORION_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ORION_HOME/$f"; then
            :
        else
            chown orion:orion "$ORION_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the orion runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ORION_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ORION_HOME/config.yaml"; then
        :
    else
        chown orion:orion "$ORION_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ORION_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as orion user ---
# Run as orion via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ORION_HOME values
...
as_orion mkdir -p \
    ...
    "$ORION_HOME/sessions" \
    "$ORION_HOME/logs" \
    "$ORION_HOME/logs/gateways" \
    "$ORION_HOME/hooks" \
    "$ORION_HOME/memories" \
    "$ORION_HOME/skills" \
    "$ORION_HOME/skins" \
    "$ORION_HOME/plans" \
    "$ORION_HOME/workspace" \
    "$ORION_HOME/home" \
    "$ORION_HOME/pairing" \
    "$ORION_HOME/platforms/pairing" \
    "$ORION_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/orion/.install_method (see Dockerfile), NOT written here into
# $ORION_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ORION_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.orion:/opt/data) and sometimes shared with a
...
# 'orion update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ORION_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/orion stamp, so deleting the data-dir copy is safe).
if [ -f "$ORION_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ORION_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ORION_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ORION_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ORION_HOME/$dest"; then
            :
        else
            as_orion cp "$INSTALL_DIR/$src" "$ORION_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/orion/.env.example being present
...
# not generate one. Orion loads $ORION_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$ORION_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$ORION_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $ORION_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$ORION_HOME/.env" ] && ! refuse_symlinked_path "clean" "$ORION_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$ORION_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$ORION_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$ORION_HOME/.env"; then
        :
    else
        if [ ! -f "$ORION_HOME/.env" ]; then
            ...
            (umask 077 && as_orion touch "$ORION_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$ORION_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$ORION_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$ORION_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $ORION_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $ORION_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$ORION_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ORION_HOME/.env"; then
        :
    else
        chown orion:orion "$ORION_HOME/.env" 2>/dev/null || true
        chmod 600 "$ORION_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `orion` user — without this it gets EACCES on every
...
# orion group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp orion /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted orion group access to the Fly Machines API socket"
    else
...
# $ORION_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `orion update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ORION_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ORION_HOME/config.yaml" ]; then
    s6-setuidgid orion "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ORION_HOME/auth.json" ] && [ -n "${ORION_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ORION_HOME/auth.json"; then
        :
    else
        printf '%s' "$ORION_AUTH_JSON_BOOTSTRAP" > "$ORION_HOME/auth.json"
        chown orion:orion "$ORION_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ORION_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ORION_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ORION_HOME/auth.json" ] && [ -n "${ORION_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ORION_HOME/auth.json"; then
        :
    else
        s6-setuidgid orion "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$ORION_HOME/auth.json" \
...
# can set ORION_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ORION_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ORION_HOME/gateway_state.json" ] && \
        [ "${ORION_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ORION_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ORION_HOME/gateway_state.json"
        chown orion:orion "$ORION_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ORION_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_orion "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/orion/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Orion spawns for the
...
# main-wrapper.sh propagates it into the supervised ``orion`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-orion, dashboard, etc.).

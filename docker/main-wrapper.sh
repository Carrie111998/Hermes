#!/bin/sh
# shellcheck shell=sh
# /opt/orion/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching ORION_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `orion` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `orion <args>` (subcommand passthrough)
#
# Drop to orion via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${ORION_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${ORION_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export ORION_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset ORION_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid orion "$@"; exec "$@"; }
...
# an arbitrary non-root, non-orion UID, the bootstrap was skipped and the
# baked image dirs (owned by the orion build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u orion)" ]; then
    cat >&2 <<EOF
[orion] ERROR: container started with --user $cur_uid (an arbitrary, non-orion UID) — not supported.
...
# to the orion user's home before dropping privileges so libraries that
...
_orion_orig_cwd="${ORION_ORIG_CWD:-$PWD}"

cd /opt/data
# shellcheck disable=SC1091
. /opt/orion/.venv/bin/activate

# Restore the original working directory before handing off to
# the user's command so `orion chat` starts in the Docker -w

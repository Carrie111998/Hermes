...
# Orion WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ORION_TINI_SHIM_TARGET overrides the `/init` path (and
# ORION_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ORION_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ORION_TINI_SHIM_WRAPPER:-/opt/orion/docker/main-wrapper.sh}"

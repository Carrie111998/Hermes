#!/usr/bin/env python3
"""
SQLite State Store for Orion Agent.
...
# transitive dependency via orion_state_common). run_agent keeps its own
# predating copy — orion_state cannot import run_agent (circular) — guarded
...
from orion_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from orion_state_portability import SessionPortabilityMixin
from orion_state_schema import SessionSchemaMixin
from orion_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``orion -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_orion_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.orion`` even when a test fixture later
    redirects ``ORION_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_orion_home()`` fresh so a runtime
       ``ORION_HOME`` redirect takes effect regardless of import order.
    """
...
# ~/.orion/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects ORION_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without ORION_HOME, a stale worktree
# without the re-pin, or a developer shell that exports ORION_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "ORION_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.orion``).  The test conftest injects the pre-sandbox production
#: root here so custom-``ORION_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Orion root for the guard.

    Deliberately avoids ``Path.home()`` / ``orion_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``orion_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "orion"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "orion"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".orion"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``ORION_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Orion test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``ORION_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "ORION_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``ORION_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``orion`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Orion home *root*.
    ...
    under ``~/.orion/orion-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``ORION_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``ORION_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Orion root {root}). "
                "Tests must run against a temporary ORION_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "ORION_HOME. If this test genuinely needs the live "
...
# orion_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``orion sessions optimize`` (FTS merge + VACUUM)
    ...
    ``orion_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``orion_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``orion update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Orion-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Orion"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Orion-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `orion doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see orion_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with orion_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``orion doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Orion host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``orion serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Orion's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``orion_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        st = db_path.stat()
        try:
            from orion_cli.sqlite_safe_read import (
                LiveConnectionError,
                offline_file_access,
            )
        except ImportError:
            # Scaffold/embed installs ship orion_state without orion_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``orion_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Orion home tree (``get_orion_home()``) — the safe repair scope:
      Orion owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``orion_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_orion_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Orion needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (orion_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_orion_state.py:600-632)
        ...
        probe_session_id = f"_orion_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Orion deliberately runs ``state.db`` in
    ...
    with _cross_process_repair_lock(db_path) as holding_lock:
        ...
        if _live_writer_holds_db(db_path):
            report["error"] = (
                ...
                "Stop the gateway (orion gateway stop) and retry."
            )
            logger.error("state.db repair skipped: %s", report["error"])
            return report
...
# (``~/.orion/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``orion sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("ORION_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_orion_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ORION_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship orion_state without orion_cli). A
    ...
    try:
        ...
        logger.debug(
            "orion_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch orion_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``orion_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `orion snapshot list` / "
                "`orion snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (orion doctor, dashboards) ──────────
...
_ORION_CMDLINE_MARKERS = ("orion_cli.main", "orion_cli/main", "orion serve",
                           "orion-agent", "orion gateway", "orion chat")


def _looks_like_orion(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Orion process?
    ...
    We only flag processes that look like Orion, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _ORION_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple orion processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Orion
    ...
    # `orion update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `orion snapshot list` / "
                    f"`orion snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`orion db optimize`),
            ...
            # ``orion_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `orion update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `orion sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Orion process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Orion process —
                        # only flag uninspectable holders that look like
                        # another Orion instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_orion(cmdline):
    ...
    #
    # `optimize_fts_storage()` (the `orion sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`orion sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``orion sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``orion --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: orion_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``orion -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``orion sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``ORION_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Orion, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Orion session.

        A Orion session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Orion session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `orion sessions vacuum` runs in a transient

...
def schema_read_probe_statements() -> tuple:
    ...
    ``orion_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `orion update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._orion_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._orion_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from orion_constants import get_orion_home
            cache_path = get_orion_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `orion sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `orion db optimize`) must keep its EXISTING

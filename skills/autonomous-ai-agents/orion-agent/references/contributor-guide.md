...
For occasional contributors and PR authors. Full developer docs: https://your-orion-docs.example/docs/developer-guide/
...
├── cli.py                # Interactive CLI (OrionCLI)
├── orion_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── orion_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.orion/config.yaml` (settings), `~/.orion/.env` (API keys) — both under `$ORION_HOME` when it is set.
...
`_ORION_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_orion_home()` for paths,
never hardcode `~/.orion`. For custom/local-only tools, write a plugin in
`~/.orion/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `orion_cli/commands.py`
...
- Tests auto-redirect `ORION_HOME` to temp dirs — never touch real `~/.orion/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/orion_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_orion_home()` from `orion_constants` for all paths (profile-safe)

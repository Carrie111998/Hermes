...
Set via `orion model` (picker) or `orion setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://your-orion-docs.example/docs/integrations/providers
...
| nous | OAuth device code | `orion auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `orion auth add openai-codex` |
| qwen-oauth | OAuth | `orion auth add qwen-oauth` |
| minimax-oauth | OAuth | `orion auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`orion auth`).
Fallback chain when the primary fails: `orion fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`orion_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   orion config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:

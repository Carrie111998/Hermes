...
Author widget apps for the Orion TUI (`orion --tui`): glanceable ambient
...
- The TUI must be in use (`orion --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.orion/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.orion/logs/tui_gateway_crash.log` if a widget never appears.

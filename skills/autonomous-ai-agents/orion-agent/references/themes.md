# Themes / Skins — Author a Orion Color Theme

Author a Orion **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`orion_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.orion/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Orion itself.
...
- Write access to the Orion home dir — `~/.orion` by default, or `$ORION_HOME`
  / the active profile's dir. Skins live in `<orion-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `orion config set`).
...
3. `write_file` it to `<orion-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `orion config set display.skin default` (or they can `/skin default`).
...
```
orion skin set <key> <hex>      # e.g. orion skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `orion skin set background "#08201f"` changes
only the background; `orion skin use <name>` / `orion skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.orion`** when a profile is active — resolve the real home
  from `$ORION_HOME` first, falling back to `~/.orion`.
...
- **Never hand-edit `config.yaml` to activate.** Use `orion config set
...
- **You apply it, not the user.** `orion config set display.skin <name>` is
...
- `read_file` the written `<orion-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `orion config get display.skin` and confirm it reports `<name>`.

...
## Layer 1 — Is this thing a Orion plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Orion process**. Its LLM calls go through Orion's already-configured provider. | Already uses Portal if user's Orion is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Orion. That LLM call is already authenticated
via Orion's provider config. The plugin's own server auth (e.g.
...
What Orion Agent has that *feels* like OAuth — `orion login --provider nous`
opening a browser, user signs in, token lands in `~/.orion/auth.json` — is a
**Orion-specific browser flow**. Under the hood it produces a credential
Orion uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Orion's existing Portal credential out of `~/.orion/auth.json`.
...
rides on the credential Orion already holds.

Where this could live in Orion:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Orion itself.
...
1. First decide: Orion plugin (runs inside Orion) or separate app? If plugin,
   it already uses Portal via Orion's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Orion agent itself") is fully supported. Orion treats agent
...
pitch in the Tool Gateway announcement is specifically about Orion Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Orion is configured to use Portal as its provider. It is

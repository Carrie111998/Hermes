...
Edit with `orion config edit` or `orion config set section.key value`.
Full reference: https://your-orion-docs.example/docs/user-guide/configuration
...
`orion config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `orion tools` (interactive) or `orion tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ORION_CORE_TOOLS` is the default bundle most platforms inherit).

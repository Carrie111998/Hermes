...
Write plugins for the Orion desktop app: statusbar items, layout panes,
...
system (`~/.orion/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ORION_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ORION_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Orion desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ORION_HOME/desktop-plugins/` (usually
  `~/.orion/desktop-plugins/`).

## How to Run

1. Create `$ORION_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.orion/...` by default, or `~/.orion/profiles/<profile>/...` under a
...
The ONLY import surface is `@orion/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Orion
  ...
  (`orion://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.orion/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@orion/plugin-sdk`, `react`, and

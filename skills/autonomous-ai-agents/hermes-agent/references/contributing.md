...
Project layout, adding a tool or slash command, the agent loop, the test runner and cross-platform test guards, the execution-environment prompt block, commit conventions, and key invariants.
...
- **Windows:** the wrapper is POSIX-only; see
  [references/windows-quirks.md](references/windows-quirks.md) for the direct-pytest workaround.

...
Secret redaction in tool output, gateway PII redaction, command-approval modes, the shell-hooks allowlist, and disabling network/media tools.
...
To keep the model away from network or media tools entirely, open `hermes tools` and toggle per-platform. Takes effect on next session (`/reset`). See the Tools & Skills part of [references/cli-reference.md](references/cli-reference.md).

...
- Both sides' intent sources: kanban completion summaries (`terminal` running
  `orion kanban show <task-id>`), PR bodies, or at minimum each branch's
  commit messages.
...
- Collect each side's intent: `orion kanban show <task-id>` for completion

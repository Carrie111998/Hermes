...
This skill is separate from the Orion Email gateway adapter. The gateway
adapter lets people email the agent and uses Orion' built-in IMAP/SMTP
...
To reply non-interactively from Orion, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Orion)** — pipe the message via stdin:

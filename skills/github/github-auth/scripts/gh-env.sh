#!/usr/bin/env bash
# GitHub environment detection helper for Orion Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _orion_env="${ORION_HOME:-$HOME/.orion}/.env"; [ -f "$_orion_env" ] && grep -q "^GITHUB_TOKEN=" "$_orion_env" 2>/dev/null; then

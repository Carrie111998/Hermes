...
Related skills, Hermes tool mapping, delegation and citation-verification patterns, `memory`/`todo` state management, cron monitoring with the `[SILENT]` protocol, communication/report formats, and decision points requiring human input. Loaded from the `research-paper-writing` skill.

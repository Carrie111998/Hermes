...
Context management, the narrative principle, time allocation, two-pass refinement, the LaTeX quality checklist, and section-by-section drafting steps 5.0-5.11 (title, abstract, Figure 1, intro, methods, results, related work, limitations, conclusion, appendix, ethics, datasheets) plus page-budget and style guidance. Loaded from the `research-paper-writing` skill.
...
**Principles:**
- **`experiment_log.md` is the primary context bridge** — it summarizes everything needed for writing without loading raw data files (see Step 4.6 in [references/result-analysis.md](references/result-analysis.md))

...
Ledger path resolution (first wins):
  --ledger PATH
  $ORION_CITATION_LEDGER
  $ORION_HOME/cache/citations/ledger.json
"""
...
from _orion_home import get_orion_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ORION_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_orion_home() / "cache" / "citations" / "ledger.json"

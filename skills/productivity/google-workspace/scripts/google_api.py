#!/usr/bin/env python3
"""Google Workspace API CLI for Orion Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Orion-facing JSON contract and falls back to the Python client
...
TOKEN_PATH = ORION_HOME / "google_token.json"
CLIENT_SECRET_PATH = ORION_HOME / "google_client_secret.json"
...
def _gws_binary() -> str | None:
    override = os.getenv("ORION_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Orion Agent")

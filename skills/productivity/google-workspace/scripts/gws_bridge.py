#!/usr/bin/env python3
"""Bridge between Orion OAuth token and gws CLI.
...
# Ensure sibling modules (_orion_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _orion_home import get_orion_home


def get_token_path() -> Path:
    return get_orion_home() / "google_token.json"

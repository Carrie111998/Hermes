#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Orion Agent.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Orion profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Orion Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Orion Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Orion home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_orion_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Orion")

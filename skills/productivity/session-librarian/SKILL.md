...
| List/filter by metadata (age, source, cost, tokens, workspace) | `orion sessions list` / `stats` via terminal |
| Rename | `orion sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `orion sessions archive <filters>` |
| Delete (destructive) | `orion sessions delete` / `orion sessions prune <filters>` |
| Export before deleting anything valuable | `orion sessions export --session-id <id> --format md` |
...
`orion sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `orion sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `orion sessions list`

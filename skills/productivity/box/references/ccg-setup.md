...
If they decline, give this path with clickable links and wait for the values to be stored locally: open the [Box Developer Console](https://app.box.com/developers/console), [create a Platform App](https://app.box.com/developers/console/new), choose **Client Credentials Grant** and **App Access Only**, enable **Manage users** and **Generate User Access Tokens**, choose the minimum additional scopes, then ask the administrator to [authorize the app](https://app.box.com/master/console). **Manage users is required to create the App User through this CCG app.** If the deployment will use Box AI, also enable **Configuration → Required Access Scopes → Content Actions → Manage AI** (`ai.readwrite`). Reauthorize the app after changing scopes.

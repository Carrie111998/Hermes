...
ORION_HOME_DIR="${ORION_HOME:-$HOME/.orion}"
ORION_CFG="${ORION_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Orion config has twozero_td MCP entry ──
if [[ ! -f "$ORION_CFG" ]]; then
    echo -e " ${FAIL} Orion config not found at ${ORION_CFG}"
    manual_steps+=("Create ${ORION_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ORION_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Orion config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Orion config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ORION_CFG} manually"); }
    manual_steps+=("Restart Orion session to pick up config change")
fi

...
`$ORION_HOME/config.yaml` (defaults to `~/.orion/config.yaml` when `ORION_HOME` is unset)
...
Restart the Orion session for changes to take effect. The MCP connection is

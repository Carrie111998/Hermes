...
const BACKEND = process.env.ORION_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `orion dashboard` server injects a one-shot
 * session token into `index.html` (see `orion_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__ORION_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function orionDevToken(): Plugin {
  const TOKEN_RE = /window\.__ORION_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__ORION_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "orion:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[orion] Could not find session token in ${BACKEND} — ` +
              `is \`orion dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__ORION_SESSION_TOKEN__="${match[1]}";` +
              `window.__ORION_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[orion] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`orion dashboard\` or set ORION_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@orion/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `orion dashboard` must serve these; Vite has no

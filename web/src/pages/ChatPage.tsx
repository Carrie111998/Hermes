/**
 * ChatPage — embeds `orion --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (orion_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "orion.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ORION_SESSION_TOKEN__ &&
    !window.__ORION_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `orion dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("orion-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("orion-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const token = window.__ORION_SESSION_TOKEN__;
    const gated = !!window.__ORION_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Orion TUI has enabled xterm mouse-events
    ...
    //      payloads upload to ORION_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to ORION_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[orion-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ORION_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets ORION_HOME pointed at the
    ...
    // forwarding them into Orion. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="orion-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    __ORION_SESSION_TOKEN__?: string;
    __ORION_AUTH_REQUIRED__?: boolean;
  }
}

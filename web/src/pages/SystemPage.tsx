...
import { CharterforgeConsoleModal } from "@/components/CharterforgeConsoleModal";
...
export default function SystemPage() {
  ...
  const loadAll = useCallback(() => {
    Promise.allSettled([
      ...
      api.checkCharterforgeUpdate(false),
    ])
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      ...
      try {
        const info = await api.checkCharterforgeUpdate(force);
        setUpdateInfo(info);
        if (force) {
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_hermes === false) {
      showToast(
        "Charterforge updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
  ...
  const canUpdateCharterforge = status?.can_update_hermes !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateCharterforge && updateConfirmOpen}
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Charterforge</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.hermes_version}</span>
                  {canUpdateCharterforge &&
                    updateInfo &&
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$CHARTERFORGE_HOME/backups/hermes-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Charterforge backup?"
              description={`This will overwrite your current Charterforge configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Charterforge team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>

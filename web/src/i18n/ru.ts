...
export const ru: Translations = {
  common: {
    ...
    gateway: "Шлюз",
    gatewayHint: "Платформы мессенджеров, API-сервер и вебхуки настраиваются на странице «Каналы». Это настройки всего шлюза (режим прокси/ретрансляции и глобальный список разрешений).",
  ...
  app: {
    ...
    managingProfileBanner: "Управление профилем «{name}» — конфигурация, ключи, навыки, MCP, модель и новые чаты применяются к этому профилю.",
    memoryOomRestartBanner: "Ваш агент неожиданно перезапустился, скорее всего из-за нехватки памяти. Длинные сессии и множество одновременных задач увеличивают потребление памяти.",
    memoryCriticalBanner: "Ваш агент почти исчерпал память и может перезапуститься. Рассмотрите закрытие неактивных сессий или увеличение памяти.",
    memoryElevatedBanner: "Ваш агент работает с низким уровнем памяти.",
    diskCriticalBanner: "Диск вашего агента почти заполнен. Новые сообщения, воспоминания и настройки могут не сохраниться.",
    diskElevatedBanner: "Диск вашего агента заполняется. Рассмотрите очистку старых сессий или расширение хранилища.",
    dismiss: "Скрыть",
  },
  status: {
    ...
    restartGatewayConfirmMessage: "Это перезапустит процесс шлюза Hermes. Подключённые каналы и активные сессии переподключатся после этого.",
    ...
    updateHermesConfirmMessage: "Будет выполнена команда hermes update и перезапущен шлюз по завершении. Активные сессии сохранят кеш промпта до этого момента.",
  ...
  cron: {
    ...
    delivery: {
      ...
      noneConfigured: "Платформы мессенджеров не настроены. Настройте одну в разделе «Каналы» для доставки отчётов.",
    },
  },
  profiles: {
    ...
    descriptionPlaceholder: "Для чего этот профиль? Используется для маршрутизации задач kanban по ролям.",
  ...
  skills: {
    ...
    managingProfile: "Управление профилем «{name}» — переключатели применяются к этому профилю, а не к профилю этой панели.",
  },
  config: {
  ...
  kanban: {
    ...
    needsAssigneeHint: "Зависимости выполнены, но диспетчер пропускает задачу, пока вы не назначите профиль.",
    untitled: "(без названия)",
    ...
    confirmScheduled: "Переместить задачу в «Запланировано»? Используйте это для известных временных задержек, а не для ожидания человека.",
    confirmDoneMany: "Отметить {n} задач(и) как выполненные? Захваты воркеров будут освобождены, а зависимые потомки станут готовыми.",
    confirmArchiveMany: "Архивировать {n} задач(и)? Они исчезнут из стандартного вида доски.",
    confirmBlockedMany: "Отметить {n} задач(и) как заблокированные? Захваты воркеров будут освобождены.",
    ...
    parentLabelHint: "(потомок остаётся заблокированным, пока родитель не завершён)",
    create: "Создать",
    boardSettings: "Настройки",
    boardSettingsTitle: "Настройки доски — имя, описание и каталог проекта по умолчанию, который наследуют новые задачи",
    boardSettingsTitleFor: "Настройки доски — {name}",
    projectDirectoryOverrideHint: "Новые задачи наследуют это как рабочую директорию по умолчанию; каждую задачу можно переопределить в диалоге создания.",
    saving: "Сохранение…",
    commentHint: "Комментарии доходят до воркера при следующем запуске или kanban_show() — не нужно сначала блокировать задачу.",
    commentHintTitle: "Комментарии — канал связи с воркером задачи. Они сразу попадают в тред — не нужно сначала блокировать задачу. Запущенный воркер подхватит тред при следующем kanban_show() или перезапуске; блокировка нужна только когда вы хотите, чтобы воркер ОСТАНОВИЛСЯ и ждал вашего ввода.",
    trash: {
      confirmTitle: "Удалить задачу?",
      confirmManyTitle: "Удалить {n} задач(и)?",
    },
  },
};

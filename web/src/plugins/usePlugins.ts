...
import { api, CHARTERFORGE_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${CHARTERFORGE_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${CHARTERFORGE_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;

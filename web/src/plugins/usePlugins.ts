...
import { api, ORION_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "orion:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ORION_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ORION_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?orion_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-orion-plugin", manifest.name);

          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="orion-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="orion-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateOrion = status?.can_update_orion === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'orion update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "orion update";
    return (
      t.status.updateOrionConfirmMessage ??
      `This will run 'orion update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateOrionConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateOrion) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateOrion,
      runningLabel: t.status.updatingOrion,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateOrionConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateOrionConfirmTitle ?? `${t.status.updateOrion}?`}
    />
    </>
  );
}

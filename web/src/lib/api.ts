import { buildCharterforgeWebSocketUrl } from "@charterforge/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/hermes/). The Python backend
// injects ``window.__CHARTERFORGE_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__CHARTERFORGE_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const CHARTERFORGE_BASE_PATH = readBasePath();
const BASE = CHARTERFORGE_BASE_PATH;
...
declare global {
  interface Window {
    ...
    __CHARTERFORGE_BASE_PATH__?: string;
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // ``window.__CHARTERFORGE_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__CHARTERFORGE_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__CHARTERFORGE_SESSION_TOKEN__`` is valid, so the next
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__CHARTERFORGE_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Charterforge-Session-Token`` header.
 ...
 * ``window.__CHARTERFORGE_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildCharterforgeWebSocketUrl({
...
export const api = {
  ...
  checkCharterforgeUpdate: (force = false) =>

import { buildOrionWebSocketUrl } from "@orion/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/orion/). The Python backend
// injects ``window.__ORION_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ORION_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ORION_BASE_PATH = readBasePath();
const BASE = ORION_BASE_PATH;
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``orion update``, ``orion gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ORION_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ORION_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__ORION_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``orion_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ORION_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Orion-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``orion_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ORION_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateOrion: () =>
    fetchJSON<ActionResponse>("/api/orion/update", { method: "POST" }),
  checkOrionUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/orion/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** orion-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``orion update``. */
  can_update_orion?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the ORION_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the ORION_HOME volume — no staleness dimension, so no
 * sampled_at. */

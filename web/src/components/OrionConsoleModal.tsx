...
interface OrionConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function OrionConsoleModal({ open, onClose }: OrionConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("orion> ");
  const inputPromptRef = useRef("orion> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "orion> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Orion Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="orion-console-title"
    >
      <div
        className={cn(
          themedBody,

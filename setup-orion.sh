...
# 5. Symlinks the 'orion' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Orion Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/orion-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/orion-uv-installer.$$.sh")"
...
# ============================================================================
# PATH setup — symlink orion into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up orion command..."

ORION_BIN="$SCRIPT_DIR/venv/bin/orion"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ORION_BIN" "$COMMAND_LINK_DIR/orion"
echo -e "${GREEN}✓${NC} Symlinked orion → $COMMAND_LINK_DISPLAY_DIR/orion"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Orion Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.orion/skills/
# ============================================================================

ORION_SKILLS_DIR="${ORION_HOME:-$HOME/.orion}/skills"
mkdir -p "$ORION_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.orion/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ORION_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  orion status        # Check configuration"
if is_termux; then
    echo "  orion gateway       # Run gateway in foreground"
else
    echo "  orion gateway install # Install gateway service (messaging + cron)"
fi
echo "  orion cron list     # View scheduled jobs"
echo "  orion doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m orion_cli.main setup
fi

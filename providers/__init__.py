...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with orion-agent)
2. User plugins: ``$ORION_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``orion_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ORION_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ORION_HOME/plugins/model-providers/`` if it exists."""
    try:
        from orion_constants import get_orion_home

        d = get_orion_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ORION_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_orion_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``orion_agent.plugins``
    entry-point group so they self-register.

    A distribution ships::

        [project.entry-points."orion_agent.plugins"]
        acme-inference = "acme_orion_plugin:register"
    ...
    * **Provider targets only.** The ``orion_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ORION_HOME``) keep their
    ...
    try:
        from orion_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ORION_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``orion_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$ORION_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with orion-agent.
    ...
    # 2. User plugins — under $ORION_HOME/plugins/model-providers/<name>/.

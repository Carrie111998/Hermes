...
Registry and ABC for every inference provider Orion knows about.
...
`$ORION_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `orion_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `orion_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `orion_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `orion_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `orion_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$ORION_HOME/plugins/model-providers/` for a private plugin).

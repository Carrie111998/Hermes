...
Extracted verbatim from orion_state.py so the SessionDB mixin modules
(orion_state_search / orion_state_schema / orion_state_portability) can
reference them without importing orion_state (which would be a cycle).
orion_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``orion sessions optimize-storage``. A legacy DB sits at
...
# `orion sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `orion db optimize`. This is

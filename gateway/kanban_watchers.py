...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        TERMINAL_KINDS = ("completed", "blocked", "dependency_wait", "gave_up", "crashed", "timed_out", "status", "archived", "unblocked", "block_loop_detected")
                ...
                for d in deliveries:
                    ...
                    for ev in d["events"]:
                        ...
                        if kind == "completed":
                            ...
                            if ev.payload and ev.payload.get("reason"):
                                reason = f": {str(ev.payload['reason'])[:160]}"
                            msg = f"⏳ {board_tag}{tag}Kanban {sub['task_id']} dependency wait{reason}"
                        elif kind == "gave_up":
                            err = ""
                        ...
                        if _wake_kinds and _session_key:
                            ...
                            if "blocked" in _wake_kinds or "dependency_wait" in _wake_kinds:
                            ...
                            if "dependency_wait" in _wake_kinds:
                                _reason = next(
                                    (
                                        str(ev.payload["reason"])[:160]
                                        for ev in d["events"]
                                        if ev.kind == "dependency_wait"
                                        and ev.payload

...
There are two ways to add a platform to the Orion gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.orion/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Orion code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `orion gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `orion_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Orion core.
...
And add it to the `orion-gateway` composite:

```python
"orion-gateway": {
    "includes": [..., "orion-your-platform"]
}
```
...
## 12. Status Display (`orion_cli/status.py`)
...
## 13. Gateway Setup Wizard (`orion_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ orion_cli/ toolsets.py \
  --include="*.py" -l | sort -u

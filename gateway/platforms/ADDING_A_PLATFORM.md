...
| `send_exec_approval(chat_id, command, session_key, description, ..., approval_id=...)` | Render dangerous-command approval buttons. Retain the opaque `approval_id`, then pass it to `tools.approval.resolve_gateway_approval`; never resolve a button through the session FIFO. |

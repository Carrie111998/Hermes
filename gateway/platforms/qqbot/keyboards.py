...
# Clarify-button prefix is intentionally short ("cl:") to fit in the 64-byte
# button_data budget after the gateway's chat routing prefix.
CLARIFY_BUTTON_PREFIX = "cl:"
# Reserved indices: 999 = "Other (type answer)" escape hatch, matching the
# contract on BasePlatformAdapter.send_clarify.
CLARIFY_OTHER_IDX = 999

# Pattern: cl:<clarify_id>:<idx> where idx is "0".."9" or "other"
# (clarify_id is generated by the gateway; treat as opaque).
_CLARIFY_DATA_RE = re.compile(
    r"^cl:([^:]+):(\d+|other)$"
)
...
# ── Clarify (option-button) builders + parser ────────────────────────
#
# Layout for ``clarify`` mirrors Telegram/WhatsApp/Discord send_clarify:
# one button per choice (in input order) plus a final "✏️ 自己写" row that
# flips the gateway into text-capture mode for free-text answers.
#
# QQ v2 InlineKeyboard constraints:
# - up to 5 rows, up to 5 buttons per row
# - therefore we cap a single keyboard at 5 visible choices (4 + Other).
# Beyond that the caller should fall back to base.send_clarify's plain-text
# numbered list, which the gateway text-intercept still handles.
#
# ``button_data`` format: ``cl:<clarify_id>:<idx>``
# where ``idx`` is the 0-based choice index OR the literal ``other``.
# The dispatcher in adapter._default_interaction_dispatch translates
# ``other`` to ``mark_awaiting_text(clarify_id)`` so the gateway captures
# the next user message as the response.

MAX_CLARIFY_CHOICES = 4
"""How many numbered buttons to show inline; the (N+1)th row is "Other"."""


def _truncate_label(text: str, limit: int = 20) -> str:
    """QQ renders long labels with truncation; keep it readable.

    20 chars is empirically where the mobile client starts ellipsising.
    """
    s = str(text or "").strip()
    if len(s) <= limit:
        return s
...
def build_clarify_keyboard(
    ...
    """Build an option-button keyboard for ``BasePlatformAdapter.send_clarify``.

    :param clarify_id: Gateway-generated id; embedded into each button's
        ``button_data`` so the click routes back to the right pending entry.
    :param choices: Non-empty list of choice strings (will be coerced to str).
    :returns: :class:`InlineKeyboard` with one row per choice + a final
        "自己写" row whose ``button_data`` carries ``idx=other`` so the
        dispatcher flips into text-capture mode.
    ...
    Button labels are single letters (A/B/C/D) — the full choice text is
    listed in the message body by ``QQAdapter.send_clarify`` (``A. …`` /
    ``B. …``), so the buttons stay short and legible on narrow mobile
    clients while the correspondence is unambiguous.

    Raises :class:`ValueError` if ``clarify_id`` contains a colon (would
    corrupt the ``button_data`` parse).
    """
    if not clarify_id or ":" in clarify_id:
        raise ValueError(
            "clarify_id must be non-empty and colon-free "
            "(it is split on ':' in button_data)"
        )
    if not choices:
        raise ValueError("build_clarify_keyboard requires at least one choice")

    rows: List[KeyboardRow] = []
    # 1) Letter rows: at most 2 buttons per row. Single-letter labels stay
    #    tiny, so two side-by-side always render legibly.
    #
    # group_id MUST be "default" (not the dynamic "clarify_<id>" we tried
    # first) — QQ v2 rejects ``请求参数rows.buttons.group_id无效`` for any
    # non-"default" value that contains underscores or digits. See pitfall
    # in skill ``devops/qqbot-clarify-inline-keyboard``. The mutual-exclusion
    # is moot here because click_limit=1 already prevents re-clicking.
    for i, choice in enumerate(choices[:MAX_CLARIFY_CHOICES]):
        letter = chr(ord("A") + i)
        btn = _make_callback_button(
            btn_id=f"cl{i}",
            label=letter,
            visited_label=f"已选{letter}",
            data=f"{CLARIFY_BUTTON_PREFIX}{clarify_id}:{i}",
            style=1,
            group_id="default",
        )
        if len(rows) == 0 or len(rows[-1].buttons) >= 2:
            rows.append(KeyboardRow(buttons=[]))
        rows[-1].buttons.append(btn)

    # 2) "Other" row — always last, alone on its row for visual prominence.
    rows.append(
        KeyboardRow(buttons=[
            _make_callback_button(
                btn_id="clOther",
...
def parse_clarify_button_data(
    button_data: str,
) -> Optional[tuple[str, str]]:
    """Parse a ``cl:<clarify_id>:<idx|other>`` payload.

    :param button_data: Raw ``data.resolved.button_data`` from
        ``INTERACTION_CREATE``.
    :returns: ``(clarify_id, marker)`` where ``marker`` is either a numeric
        string ``"0".."3"`` (1-based index after ``+1`` at the call site) or
        the literal ``"other"``. Returns ``None`` if the payload isn't ours.
    """

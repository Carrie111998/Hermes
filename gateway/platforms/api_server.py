...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # MiniApp routes. Registered directly (not through
            # _http_route_table()/the multiplex /p/<profile>/ mirror loop
            # above) — the MiniApp surface is a WebUI convenience API that is
            # not profile-scoped, so it intentionally keeps its pre-multiplex
            # single-path registration.

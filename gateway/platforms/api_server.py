...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def send_for_platform(
        ...
        try:
            ...
            return SendResult(success=False, error=_redact_api_error_text(exc))
    ...
    async def _handle_proxy_outbox(self, request: Any) -> Any:
        ...
        try:
            ...
            return web.json_response(_openai_error("Invalid limit"), status=400)
        if not requested:
            return web.json_response(_openai_error("platforms is required"), status=400)
        try:
    ...
    async def _handle_proxy_outbox_ack(self, request: Any) -> Any:
        ...
        if not re.fullmatch(r"[0-9a-f]{32}", delivery_id):
            return web.json_response(_openai_error("Invalid delivery id"), status=400)
        try:
            body = await request.json()
        except Exception:
        ...
        if (
            ...
            return web.json_response(
                _openai_error("success must be a boolean and attempt a positive integer"),
                status=400,
            )
        ...
        accepted = await asyncio.to_thread(
            ...
            error=_redact_api_error_text(body.get("error", ""), limit=500),
        )
        if not accepted:
            return web.json_response(_openai_error("Delivery is not actively leased"), status=409)

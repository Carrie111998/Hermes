...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Agent cache keyed by gateway_session_key to preserve Honcho memory
        # provider state across requests (fixes #91694). Mirrors
        # GatewayRunner._agent_cache in run.py. Value is a tuple of
        # (AIAgent, config_signature). LRU order maintained for potential
        # future cap enforcement.
    ...
    # ------------------------------------------------------------------
    # Agent cache helpers (mirrors GatewayRunner in gateway/run.py)
    # ------------------------------------------------------------------
    ...
    def _extract_cache_busting_config(self, user_config: dict | None) -> dict:
        ...
        # Mirror GatewayRunner._CACHE_BUSTING_CONFIG_KEYS
        ...
        if isinstance(provider, str) and provider.lower() == "honcho":
            out.update(self._extract_honcho_cache_busting_config())
        else:
            out.update(self._empty_honcho_cache_busting_config())

        return out

    @staticmethod
    def _empty_honcho_cache_busting_config() -> dict[str, Any]:
        """Return zero-values for Honcho cache-busting keys when Honcho is off."""
        return {
    ...
    def _extract_honcho_cache_busting_config(self) -> dict[str, Any]:
        """Extract Honcho identity keys, memoized by honcho.json mtime.

        Mirrors GatewayRunner._extract_honcho_cache_busting_config.
        """
        import os
        from pathlib import Path
        ...
        try:
            ...
            memo = getattr(self, "_honcho_cache_busting_memo", None)
            if memo is not None and memo.get("mtime") == mtime:
                return dict(memo.get("values", {}))
            hcfg = HonchoClientConfig.from_global_config()
            values = {
                "honcho.peer_name": hcfg.peer_name,
                "honcho.ai_peer": hcfg.ai_peer,
                "honcho.pin_peer_name": hcfg.pin_peer_name,
                "honcho.runtime_peer_prefix": hcfg.runtime_peer_prefix,
                "honcho.user_peer_aliases": hcfg.user_peer_aliases,
            }
            if not hasattr(self, "_honcho_cache_busting_memo"):
                self._honcho_cache_busting_memo = {}
            self._honcho_cache_busting_memo[memo_key] = {"mtime": mtime, "values": values}
            return values
        except Exception:
            return self._empty_honcho_cache_busting_config()
    ...
    def _create_agent(
        ...
        # Agent cache key: gateway_session_key (stable memory scope) or
        # session_id (ephemeral for one-off requests). Mirrors
        # GatewayRunner._agent_cache keyed by session_key.
        cache_key = gateway_session_key or session_id or ""

        # Compute config signature for cache validation. Includes user_id/
        # user_id_alt because Honcho freezes them into HonchoSessionManager
        # at init (see plugins/memory/honcho/__init__.py::_do_session_init).
        # We don't have direct access to gateway source here, so pass None.
        # Per-request ephemeral_system_prompt is excluded from the cache key
        # because it changes every turn but is applied per-message on the
        # agent instance (see _run_agent where callbacks are set).
        cache_keys = self._extract_cache_busting_config(user_config)
        _skip_context = None
        try:
            _plat_gw_cfg = (user_config.get("platforms", {}) or {}).get("api_server") or {}
            _skip_context = _plat_gw_cfg.get("skip_context_files")
            _skip_context = bool(_skip_context) if _skip_context is not None else False
        except Exception:
            _skip_context = False

        config_signature = self._agent_config_signature(
            model=model,
            runtime=runtime_kwargs,
            enabled_toolsets=enabled_toolsets,
            ephemeral_prompt="",  # excluded — per-turn, not baked into agent
            cache_keys=cache_keys,
            user_id=None,  # API server has no gateway source identity
            user_id_alt=None,
        ...
        # Check agent cache
        cached_agent = None
        if self._agent_cache_lock and cache_key:
            with self._agent_cache_lock:
                cached = self._agent_cache.get(cache_key)
                if cached and cached[1] == config_signature:
                    ...
                    try:
                        ...
                        pass
                    logger.debug("Reusing cached agent for session %s", cache_key)

        if cached_agent is not None:
            # Update per-turn callbacks on the cached agent
            ...
            cached_agent.fallback_model = fallback_model
            if request_service_tier is not _REQUEST_OPTION_MISSING:
                cached_agent.service_tier = request_service_tier
            # Update runtime metadata
            cached_agent._hermes_api_runtime = {
                "provider": runtime_kwargs.get("provider") or getattr(cached_agent, "provider", "") or "",
                "model": getattr(cached_agent, "model", None) or model,
                "route_source": (
                    "session_model_lock"
                    if confirmed_runtime_lock
        ...
        # Store in cache for reuse on subsequent requests with same session
        if self._agent_cache_lock and cache_key:
            with self._agent_cache_lock:
                self._agent_cache[cache_key] = (agent, config_signature)
                logger.debug("Created and cached new agent for session %s", cache_key)

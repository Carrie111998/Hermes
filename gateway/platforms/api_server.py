...
def _request_service_tier(model_options: Any) -> Any:
    ...
    if "fast" in model_options:
        return (
            "priority"
            if _coerce_request_bool(model_options.get("fast"), default=False)
            else None
        )
    return _REQUEST_OPTION_MISSING
...
def _apply_runtime_agent_overrides(
    ...
    for key in _RUNTIME_AGENT_OVERRIDE_KEYS:
        ...
        if value is None:
            continue
        runtime_kwargs[key] = (
            list(value) if key == "args" and isinstance(value, (list, tuple)) else value
        )
    return runtime_kwargs
...
def _is_compressed_summary_message(message: Any) -> bool:
    ...
    return prefix.startswith("[CONTEXT COMPACTION") or prefix.startswith(
        "[CONTEXT SUMMARY]:"
    )
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_db_lock: Optional[asyncio.Lock] = (
            None  # Single-flight for lazy init
        )
    ...
    async def _handle_platform_event_callback(
        ...
        try:
            ...
            logger.exception(
                "Platform HTTP event dispatch failed for %s", platform_name
            )
            return web.json_response(
                _openai_error(
    ...
    def _runtime_options_from_model_options(cls, model_options: Any) -> Dict[str, Any]:
        ...
        service_tier = cls._clean_runtime_id(
            model_options.get("service_tier"), max_len=32
        )
        if service_tier:
    ...
    def _session_runtime_request_from_body(
        ...
        raw_provider = self._clean_runtime_id(
            body.get("provider") or body.get("provider_id"), max_len=80
        )
        ...
        return {
            ...
            "require_model_lock": _coerce_request_bool(
    ...
    def _persist_session_runtime_lock(
        self, session_id: str, runtime_request: Dict[str, Any]
    ) -> bool:
        ...
        if runtime_request.get("persisted_lock") or not runtime_request.get(
            "require_model_lock"
        ):
            return True
        requested = runtime_request.get("requested") or {}
        ...
        try:
            ...
            logger.warning(
                "[%s] failed to persist session runtime lock for %s",
                self.name,
                session_id,
                exc_info=True,
            )
    ...
    def _runtime_request_from_persisted_session_lock(
        ...
        if not isinstance(lock, dict) or not _coerce_request_bool(
            lock.get("confirmed"), default=False
        ):
            return None
    ...
    def _sanitize_runtime_metadata(
        ...
        *,
        ...
        provider = cls._clean_runtime_id(
            ...
            or payload.get("effective_provider"),
            max_len=80,
        )
        model = cls._clean_runtime_id(
            payload.get("model")
            or payload.get("model_id")
            or payload.get("effective_model")
        )
        result: Dict[str, Any] = {
            "provider": provider,
            "model": model,
            "route_source": cls._clean_runtime_id(
                payload.get("route_source") or route_source, max_len=64
            )
            or "global",
        }
        ...
        if model_lock or payload.get("model_lock"):
            result["model_lock"] = cls._clean_runtime_id(
                model_lock or payload.get("model_lock"), max_len=32
            )
        return result
    ...
    def _create_agent(
        ...
        if not model:
            _recovered = self._last_resolved_model.get(
                _resolved_key
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        active_api_runs, process_depth, active_delegations = (
            self._readiness_work_counts()
        )
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        ...
        if lock_error is not None:
            return lock_error
        requested = runtime_request.get("requested") or {}
        model_name = self._clean_runtime_id(requested.get("model")) or (
                ...
                if title is not None:
                    ...
                    if clean_title:
                        ...
                        if conflict:
                            ...
                            return (
                                None,
                                f"title:Title already in use by session {conflict['id']}",
                            )
                    conn.execute(
        ...
        if err and err.startswith("title:"):
            return web.json_response(
                _openai_error(err[len("title:") :], code="invalid_title"), status=400
            )
        return web.json_response(
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if auth_err:
            return auth_err
        session, err = await self._get_existing_session_or_404(
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        fork = await asyncio.to_thread(db.get_session, fork_id) or {
            "id": fork_id,
            "parent_session_id": source_id,
        }
        return web.json_response(
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        if lock_active:
            ...
            session_model = (
                stored_model if (stored_model and stored_route is None) else None
            )
            ...
            try:
                ...
                effective_runtime = self._sanitize_runtime_metadata(
                    ...
                    model_lock=(
                        ...
                        and runtime_request.get("require_model_lock")
                        else "accepted"
                        if runtime_request.get("require_model_lock")
                        else ""
                    ),
        ...
        try:
            while True:
                ...
                await response.write(
                    _sse_frame(payload, event=name, ensure_ascii=False)
                )
    ...
    async def _handle_session_model_lock(
        self, request: "web.Request"
    ) -> "web.Response":
        ...
        try:
            ...
            if agent_error:
                ...
                _persist_response_snapshot(
                    ...
                    session_id_snapshot=_result_sid
                ...
                try:
                    ...
                    if include_runtime:
                        ...
                        runtime["route_source"] = (
                            route_source or runtime.get("route_source") or "global"
                        )
                        runtime = self._sanitize_runtime_metadata(
                            runtime=runtime,
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        async def _run_and_close():
            try:
                    ...
                    event.update(
                        {
                            ...
                            "choices": _approval_event_choices(
                                ...
                                allow_permanent=event.get("allow_permanent")
                                is not False,
                            ),
                        }
                    )
                ...
                logger.warning(
                    "Provider authentication failed for run=%s: %s", run_id, exc
                )

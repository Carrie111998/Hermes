...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.orion to the active profile home."""
    ...
    if raw.startswith("~/.orion/"):
        return orion_home / raw.removeprefix("~/.orion/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return orion_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ORION_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from orion_constants import get_orion_home

    scripts_root = (get_orion_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.orion" or raw_text.startswith("~/.orion/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__orion_ignore__") is True
        ):
            return False, None

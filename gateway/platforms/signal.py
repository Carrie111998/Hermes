        ...
        *,
        ...
        try:
            ...
            if method == "sendTyping":
                ...
                if not recipient:
                    return None
                # PUT shows the typing indicator; DELETE (stop=True) hides it
                # immediately instead of waiting for Signal's built-in timeout.
                resp = await self.client.request(
                    "DELETE" if params.get("stop") else "PUT",
            ...
            if method == "getAttachment":
                ...
                if not att_id:
                    return None
                resp = await self.client.get(
                    f"{self.http_url}/v1/attachments/{quote(str(att_id), safe='')}",
                    timeout=timeout,
                )
            ...
            logger.warning("Signal: unsupported method %s", method)
            return None
    ...
    async def send(
        ...
        for idx, (plain_text, text_styles) in enumerate(chunks, start=1):
            ...
            if text_styles:
                ...
                if len(chunks) == 1:
                    # /v2/send styled mode expects markdown in the message
                    # body. Only safe when the message wasn't chunked: the
                    # markdown source doesn't map onto plain-text chunk
                    # boundaries, so multi-chunk sends stay on the plain
                    # body + textStyle path.

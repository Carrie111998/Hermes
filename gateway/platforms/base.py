...
# Audio file extensions Orion recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Orion but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Orion' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Orion-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_orion_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``ORION_SESSION_PLATFORM`` contextvar — that contextvar is
...
from orion_constants import get_default_orion_root, get_orion_dir, get_orion_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.orion/.env manually."
)
...
IMAGE_CACHE_DIR = get_orion_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_orion_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_orion_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from orion_cli.config import load_config_readonly as _load_config
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; OrionAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_orion_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_orion_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_orion_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_orion_dir("cache/screenshots", "browser_screenshots")
...
_ORION_ROOT = get_default_orion_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ORION_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ORION_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ORION_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ORION_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ORION_HOME / "image_cache",
    _ORION_HOME / "audio_cache",
    _ORION_HOME / "video_cache",
    _ORION_HOME / "document_cache",
    _ORION_HOME / "browser_screenshots",
    ...
    _ORION_HOME / "cache" / "images",
    _ORION_HOME / "cache" / "audio",
    _ORION_HOME / "cache" / "videos",
    _ORION_HOME / "cache" / "documents",
    _ORION_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Orion root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* ORION_HOME's cache, so a gateway running at
    the root (e.g. ``ORION_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when ORION_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _ORION_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("ORION_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _ORION_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Orion profile and shared Orion root both contain control
    ...
    # ORION_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.orion
    ...
    for orion_root in (_ORION_HOME, _ORION_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(orion_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Orion
    secrets (``~/.orion/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Orion cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.orion/...``
...
def _translate_docker_container_media_path(candidate: Path) -> Optional[Path]:
    ...
    auto-mounted Orion cache dirs (``/root/.orion/...``), the default
    ...
    # In-process gateways (Desktop backend, `orion serve`) may not have
    ...
    # are longer prefixes, so /root/.orion/... still translates to the host
    ...
    if default_home is not None and not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.orion/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.orion"):
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``ORION_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Orion-managed cache, under an operator-allowlisted root
    (``ORION_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Orion root (~/.orion/.env,
    ...
    # ``MEDIA:~/.orion/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?orion\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+orion[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``ORION_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Orion commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-ORION_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("ORION_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ORION_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ORION_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Orion sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-ORION_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(orion --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Orion home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Orion home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.orion/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ORION_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ORION_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ORION_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ORION_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ORION_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ORION_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_orion_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # ORION_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _orion_run_generation is set on
    ...
    def build_source(
        ...
        per-profile ORION_HOME isolation.
        """

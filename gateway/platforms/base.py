...
class BasePlatformAdapter(ABC):
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            requires_adapter_drain = bool(
                getattr(
                    event,
                    "_processing_lifecycle_requires_adapter_drain",
                    False,
                )
            )
            late_pending = (
                self._pending_messages.pop(session_key, None)
                if not lifecycle_transferred or requires_adapter_drain
                else None
            )
            if late_pending is not None:
                if getattr(
                    ...
                    delattr(
                        late_pending,
                        "_processing_lifecycle_transferred",
                    )
                current_task = asyncio.current_task()

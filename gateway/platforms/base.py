...
_AUDIO_EXTS = frozenset({'.ogg', '.opus', '.mp3', '.wav', '.m4a', '.flac', '.m2a'})
...
def cache_media_bytes(
    ...
    *,
    ...
    if is_audio:
        ...
        out_mime = mime if mime.startswith("audio/") else {
            ".m2a": "audio/mpeg",
        }.get(aud_ext, f"audio/{aud_ext.lstrip('.')}")

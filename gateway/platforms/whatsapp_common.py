...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Orion Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ORION_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/orion), this function
    mirrors the bridge source to a writable ORION_HOME location and returns that
    ...
    # Try ORION_HOME location first
    orion_home = get_orion_home()
    orion_home_bridge = orion_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ORION_HOME if needed
    if orion_home_bridge.exists():
        return orion_home_bridge

    # Mirror the bridge source to ORION_HOME
    try:
        orion_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            orion_home_bridge,
            dirs_exist_ok=False,

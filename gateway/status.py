...
def _read_pid_record(pid_path: Optional[Path] = None) -> Optional[dict]:
    ...
    try:
        ...
        # NOTE: PermissionError is an OSError subclass, so this branch MUST
        # precede the broadened (OSError, UnicodeDecodeError) catch below.
        return {"locked": True}
    except (OSError, UnicodeDecodeError):
        # Any other read failure: file deleted between exists() and
        # read_text(), a transient FS error, or non-UTF-8 / binary garbage.
        # Treat as absent.
        return None
    if not raw:
        return None
...
def write_pid_file() -> None:
    ...
    path = _get_pid_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    record_obj = _build_pid_record()
    record = json.dumps(record_obj)
    try:
    ...
    # PID different from the live gateway. Log the written PID on every
    # call to correlate with process creation events.
    logger.info(
        "gateway.pid written: pid=%d argv=%r path=%s",
        record_obj["pid"], record_obj["argv"], path,
    )

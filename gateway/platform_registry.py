...
class PlatformEntry:
    ...
    # status displays (``orion setup``, ``orion status``, the dashboard
    ...
    # Env vars this platform needs (for ``orion setup`` display).
    ...
    # built-ins).  Used by ``orion gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved ORION_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `orion`
        # invocation -- including plain `orion chat`, which never touches any
        ...
        # `orion setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``orion setup``/``gateway status``, channel

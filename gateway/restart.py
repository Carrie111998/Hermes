...
def _finite_nonnegative_restart_component(raw: object) -> float:
    try:
        value = float(raw)
    except (TypeError, ValueError):
        return 0.0
    if not math.isfinite(value):
        return 0.0
    return min(max(value, 0.0), MAX_GATEWAY_SHUTDOWN_TIMING_COMPONENT_S)
...
def resolve_restart_exit_wait_budget(
    ...
    *,
    ...
    """
    drain = _finite_nonnegative_restart_component(drain_timeout)
    after_turn = _finite_nonnegative_restart_component(after_turn_timeout)
    margin = _finite_nonnegative_restart_component(headroom)
    return math.fsum((drain, after_turn, margin))

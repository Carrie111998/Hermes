...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<orion_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active ORION_HOME."""
    from orion_constants import get_orion_home

    flush_dir = get_orion_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<orion_home>/pending_messages/``), so a runtime cap rotation no

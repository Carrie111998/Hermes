"""Skill-hint helpers for gateway/run.py (#54962, slice 17).

Pure helpers extracted from the gateway god file: derive a skill's command
slug from its SKILL.md frontmatter ``name:`` and check for known-but-inactive
skills so the gateway can return actionable "disabled — enable with …" /
"not installed — install with …" hints instead of a generic unknown-command
response.

Kept importable as ``gateway.run.<name>`` via the module-attribute import in
``gateway/run.py``; the tests in ``tests/gateway/test_unavailable_skill_hint.py``
reference the names through ``gateway.run`` and patch the in-function imports
(``tools.skills_tool`` / ``agent.skill_utils``), so extraction is transparent
to them.
"""

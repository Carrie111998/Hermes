...
# Discord gateway routing note prepended to the model-facing user message so
# the agent can use the triggering message id for reply/react/pin via the
# discord tools (see gateway/run.py). This is internal routing metadata, not
# user-authored content, and must be stripped before the message is written
# to persisted transcript storage (#71304).
DISCORD_TRIGGERING_NOTE_RE = re.compile(
    r"^\[Triggering message id: `[^`]*` \u2014 use as `message_id` "
    r"for reply/react/pin via the discord tools\.\]\s*\n*"
)


def strip_discord_triggering_note(content: str) -> str:
    """Remove the leading Discord "Triggering message id" routing note.

    The gateway prepends this note to the model-facing user message so the
    agent can address the triggering message for reply/react/pin tool calls.
    It is internal routing metadata generated by the gateway, not something
    the user said, so it must not be persisted into transcript storage
    (SQLite, memory providers, exports, etc.) — see #71304.
    """
    if not isinstance(content, str) or not content:
        return content
    return DISCORD_TRIGGERING_NOTE_RE.sub("", content, count=1)

"""Per-turn collaborator for the gateway run loop, extracted from ``gateway/run.py``.

Issue #54962 — god-file slicing of the 26.8k-line ``gateway/run.py``.

This module holds the entire :class:`TurnRunner` class byte-identically
(see the class docstring for what it does).  ``gateway/run.py`` re-imports
``TurnRunner`` (and ``_AGENT_PENDING_SENTINEL``, whose object identity is
part of the ``run_sync`` event-loop contract) so every existing
``gateway.run.*`` reference keeps working unchanged.

Module-level helpers that the class used to resolve inside ``run.py`` are
provided here as *deferred shims*: thin wrappers with the exact original
signature that import the real implementation from ``gateway.run`` at call
time.  ``run.py`` imports this module at module level, so a top-level
``from gateway.run import ...`` here would be a circular import; the
deferred form is the codebase's standard cycle-breaker and is safe because
these shims only run after ``gateway.run`` has finished importing.  The
helpers themselves stay in ``run.py`` because they are shared with other
``run.py`` code and/or are already claimed by sibling extraction PRs of the
same campaign (auto-continue #77706, redaction #77707, resume/replay
#77452, display #77433, platform-routing #77376, status #77450).

``_hermes_home`` is re-derived with the same resolution call
(:func:`hermes_constants.get_hermes_home`) used by ``run.py``; nothing
mutates the environment between the two module-loads, so both copies are
identical.
"""
...
# between the guard check and actual agent creation.  (Moved verbatim from
# gateway/run.py; run.py re-exports the same object so identity checks in
# both modules — and in lazy importers of gateway.run — keep working.)
_AGENT_PENDING_SENTINEL = object()

# Same resolution call as gateway/run.py's module-level binding; identical
# value at import time (nothing mutates HERMES_HOME or the override between
# the two module loads).
_hermes_home = get_hermes_home()

# ---------------------------------------------------------------------------
# Deferred shims for run.py-local helpers used by TurnRunner
# ---------------------------------------------------------------------------
# Each wrapper mirrors the original signature exactly and imports the real
# implementation from gateway.run at call time (see module docstring).  The
# class body references these names exactly as it did inside run.py.


def _auto_continue_freshness_window() -> float:
    """Deferred shim: implemented in gateway/run.py (claimed by #77433)."""
    from gateway.run import _auto_continue_freshness_window as _impl
...
def _build_gateway_agent_history(
    ...
    *,
    ...
    """Deferred shim: implemented in gateway/run.py (history-build cluster)."""
    from gateway.run import _build_gateway_agent_history as _impl
...
def _checkpoint_agent_kwargs(config: dict | None) -> dict:
    """Deferred shim: implemented in gateway/run.py."""
    from gateway.run import _checkpoint_agent_kwargs as _impl
...
def _collect_auto_append_media_tags(
    ...
    """Deferred shim: implemented in gateway/run.py (claimed by #77706)."""
    from gateway.run import _collect_auto_append_media_tags as _impl
...
def _collect_history_media_paths(agent_history: List[Dict[str, Any]]) -> set:
    """Deferred shim: implemented in gateway/run.py (claimed by #77706)."""
    from gateway.run import _collect_history_media_paths as _impl

    return _impl(agent_history)


def _current_max_iterations() -> int:
    """Deferred shim: implemented in gateway/run.py."""
    from gateway.run import _current_max_iterations as _impl
...
def _format_exec_approval_fallback(
    ...
    *,
    ...
    """Deferred shim: implemented in gateway/run.py (claimed by #77707)."""
    from gateway.run import _format_exec_approval_fallback as _impl
...
def _is_fresh_gateway_interruption(
    ...
    *,
    ...
    """Deferred shim: implemented in gateway/run.py (interrupt cluster)."""
    from gateway.run import _is_fresh_gateway_interruption as _impl

    return _impl(value, now=now, window_secs=window_secs)


def _last_transcript_timestamp(history: Optional[List[Dict[str, Any]]]) -> Any:
    """Deferred shim: implemented in gateway/run.py (claimed by #77706)."""
    from gateway.run import _last_transcript_timestamp as _impl
...
def _load_gateway_config() -> dict:
    """Deferred shim: implemented in gateway/run.py."""
    from gateway.run import _load_gateway_config as _impl

    return _impl()


def _message_timestamps_enabled(user_config: Optional[dict]) -> bool:
    """Deferred shim: implemented in gateway/run.py (claimed by #77438)."""
    from gateway.run import _message_timestamps_enabled as _impl

    return _impl(user_config)
...
def _non_conversational_metadata(
    ...
    *,
    ...
    """Deferred shim: implemented in gateway/run.py (claimed by #77376)."""
    from gateway.run import _non_conversational_metadata as _impl

    return _impl(metadata, platform=platform)
...
def _normalize_empty_agent_response(
    ...
    *,
    ...
    """Deferred shim: implemented in gateway/run.py."""
    from gateway.run import _normalize_empty_agent_response as _impl

    return _impl(agent_result, response, history_len=history_len)


def _prepare_gateway_status_message(
    platform: Any, event_type: str, message: str
) -> Optional[str]:
    """Deferred shim: implemented in gateway/run.py (status cluster)."""
    from gateway.run import _prepare_gateway_status_message as _impl

    return _impl(platform, event_type, message)


def _redact_approval_command(cmd: "str | None") -> str:
    """Deferred shim: implemented in gateway/run.py (claimed by #77707)."""
    from gateway.run import _redact_approval_command as _impl
...
def _redact_gateway_user_facing_secrets(text: str) -> str:
    """Deferred shim: implemented in gateway/run.py (claimed by #77707)."""
    from gateway.run import _redact_gateway_user_facing_secrets as _impl
...
def _sanitize_gateway_final_response(platform: Any, text: str) -> str:
    """Deferred shim: implemented in gateway/run.py."""
    from gateway.run import _sanitize_gateway_final_response as _impl

    return _impl(platform, text)
...
def _select_cached_agent_history(
    ...
    """Deferred shim: implemented in gateway/run.py (history-build cluster)."""
    from gateway.run import _select_cached_agent_history as _impl

    return _impl(persisted_history, live_history)


async def _send_or_update_status_coro(adapter, chat_id, status_key, content, metadata):
    """Deferred shim: implemented in gateway/run.py (status cluster)."""
    from gateway.run import _send_or_update_status_coro as _impl

    return await _impl(adapter, chat_id, status_key, content, metadata)
...
def _wrap_current_message_with_observed_context(
    ...
    """Deferred shim: implemented in gateway/run.py (history-build cluster)."""
    from gateway.run import _wrap_current_message_with_observed_context as _impl

    return _impl(message, observed_context)


def build_resume_recovery_note(
    reason: Optional[str], message: str = "", *, interactive: bool = True
) -> str:
    """Deferred shim: implemented in gateway/run.py (claimed by #77452)."""
    from gateway.run import build_resume_recovery_note as _impl

    return _impl(reason, message, interactive=interactive)


def render_notice_line(notice) -> str:
    """Deferred shim: implemented in gateway/run.py (status cluster)."""
    from gateway.run import render_notice_line as _impl

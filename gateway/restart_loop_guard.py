...
Defenses 1 and 2 (the ``_ORION_GATEWAY`` guard on ``orion gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.orion.gateway")``,
...
State lives in ``<ORION_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_orion_home() / "gateway" / "restart_loop.json"

    ...
    async def run(self) -> None:
        ...
        try:
            ...
            if self._use_draft_streaming and self._message_id is None:
                # A native stream has no editable message id. Sending the
                # terminal text through _send_or_edit() with
                # is_turn_final=False emits another non-final draft frame and
                # leaves the adapter's stream armed. Seal the last visible
                # frame in place instead; cancellation must not claim partial
                # text as the completed response.
                await self._abandon_native_stream()
            elif terminal_text:
                try:

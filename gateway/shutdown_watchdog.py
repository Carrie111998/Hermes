...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    # Windows has no AF_UNIX / start_unix_server — skip the witness
    # entirely without raising or logging a warning traceback. The payload
    # will record loop_tick_socket=False (fail-safe: stale file never
    # escalates to WEDGED on Windows).
    if not hasattr(asyncio, "start_unix_server"):
        logger.debug("Loop tick socket unavailable on this platform — witness disabled")
    else:
        try:

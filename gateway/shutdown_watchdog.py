...
2. An event-loop heartbeat file at ``<ORION_HOME>/state/gateway.heartbeat`` so
...
def _process_orion_home() -> Path:
    """ORION_HOME for process-level identity files (ignore profile overrides)."""
    val = os.environ.get("ORION_HOME", "").strip()
    if val:
        return Path(val)
    return get_orion_home()


def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<ORION_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_orion_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from orion_logging import drain_log_queue

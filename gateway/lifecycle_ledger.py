...
``<ORION_HOME>/state/gateway.lifecycle.json``:
...
def _process_orion_home() -> Path:
    """ORION_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<ORION_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``orion_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:

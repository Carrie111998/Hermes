"""Telegram topic-mode routing methods for ``GatewayRunner``.

Extracted from ``gateway/run.py`` (god-file decomposition campaign, Wave 1,
shard s2, cluster c11). This mixin holds the Telegram DM topic cluster:
topic-mode enablement, lobby/lane classification, lobby reminders, topic
binding persistence and thread-id recovery.

Behavior-neutral: every method is lifted verbatim from ``GatewayRunner``.
``self.*`` calls resolve unchanged via the MRO. The class attributes
``_TELEGRAM_GENERAL_TOPIC_IDS`` and ``_TELEGRAM_LOBBY_REMINDER_COOLDOWN_S``
stay on ``GatewayRunner`` and are resolved through the MRO. The module-level
``logger`` is ``logging.getLogger("gateway.run")`` so log records keep the
exact name (``"gateway.run"``), matching the sibling mixins' convention.
"""

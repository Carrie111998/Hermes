...
# Trusted scheduler identity for autonomous, human-reviewed action proposals.
#
# This deliberately does NOT live in _VAR_MAP and has no os.environ fallback:
# model-visible subprocesses must never be able to manufacture it by exporting
# a plausible HERMES_* variable.  Only cron.scheduler binds it around a real
# run, and trusted in-process plugins receive a defensive copy through
# get_scheduler_service_origin().
_SCHEDULER_SERVICE_ORIGIN: ContextVar = ContextVar(
    "HERMES_SCHEDULER_SERVICE_ORIGIN", default=_UNSET
)
...
def set_session_vars(
    ...
    tokens = [
        ...
        # A newly-bound ordinary session cannot inherit scheduler authority
        # from the task that happened to create it.
        _SCHEDULER_SERVICE_ORIGIN.set(None),
    ]
    try:
...
def bind_scheduler_service_origin(
    ...
    :func:`clear_scheduler_service_origin` in a ``finally`` block.  ``run_id``
    and ``nonce`` are generated by core for every execution; neither stored
    cron metadata nor model arguments can choose them.
    """
    clean_job = str(job_id or "").strip()
    clean_profile = str(profile_ref or "").strip()
    if not clean_job or len(clean_job) > 160:
    ...
    return _SCHEDULER_SERVICE_ORIGIN.set(origin)


def clear_scheduler_service_origin(token) -> None:
    """Restore the scheduler-origin state that preceded one cron run."""
    _SCHEDULER_SERVICE_ORIGIN.reset(token)


def get_scheduler_service_origin() -> dict[str, Any] | None:
    """Return a defensive copy of the current core-created service origin."""
    value = _SCHEDULER_SERVICE_ORIGIN.get()

...
class GuardThresholds:
    ...
    family_idle_seconds: float = 900.0       # parent/child family no-progress fuse (15 min)
    family_cycle_repeats: int = 3            # repeated semantic cycle windows
    enabled: bool = True
    ...
    def from_config(cls, cfg: Optional[dict]) -> "GuardThresholds":
        ...
        return cls(
            ...
            family_idle_seconds=_positive_num("family_idle_seconds", cls.family_idle_seconds, float),
            family_cycle_repeats=_positive_num("family_cycle_repeats", cls.family_cycle_repeats, int),
...
class RunawayGuard:
    ...
    def family_no_progress_trip(
        self, obs: SessionObservation, now: float, detail: str
    ) -> GuardEvaluationResult:
        """Build a hard-stop trip for a verified family-level stall."""
        st = _SessionState(stalled_samples=self.thresholds.no_progress_samples)
        trip = self._evaluate(st, obs, now)
        assert trip is not None
        return GuardEvaluationResult(
            **{**trip.__dict__, "reason": TripReason.NO_PROGRESS,
               "trip_reason": TripReason.NO_PROGRESS, "detail": detail}
        )
...
class _FamilyState:
    ...
    progress_markers: Dict[str, str] = field(default_factory=dict)
    tripped: bool = False


class SessionFamilyGuard:
    """Detect no-progress in a parent/child family, not just one session.

    A housekeeping tick supplies all currently observable members. The parent
    may be absent while waiting for children; the family's earliest start and
    last verified progress are retained, so waiting cannot reset the fuse.
    """
    ...
    def forget(self, family_id: str) -> None:
        self._families.pop(str(family_id), None)

    def retry(self, family_id: str) -> None:
        state = self._families.get(str(family_id))
        if state is not None:
            state.tripped = False

    def active_family_ids(self) -> list[str]:
        return list(self._families)

    @staticmethod
    def _family_key(obs: SessionObservation) -> str:
        return str(obs.family_id or obs.parent_session_id or obs.session_id)

    @staticmethod
    def _snapshot(observations: list[SessionObservation]) -> tuple:
        return tuple(sorted(
            (
                o.session_id,
                str(o.action_fingerprint or ""),
                str(o.progress_evidence or o.progress_seq or ""),
            )
            for o in observations
        ))

    @staticmethod
    def _progress_markers(observations: list[SessionObservation]) -> Dict[str, str]:
        return {
            o.session_id: str(o.progress_evidence or o.progress_seq or o.state_hash or "")
            for o in observations
        }

    def observe(
        self, observations: list[SessionObservation], now: float
    ) -> Optional[tuple[str, str]]:
        """Return ``(family_id, reason)`` when a family has no verified motion.

        The reason includes the observed semantic cycle. A distinct progress
        marker resets the cycle window; heartbeats and repeated counters do not.
        """
        if not self.thresholds.enabled or not observations:
            return None
        groups: Dict[str, list[SessionObservation]] = {}
        for obs in observations:
            groups.setdefault(self._family_key(obs), []).append(obs)
        for family_id, members in groups.items():
            # A parentless solo session is covered by the existing per-session
            # guard. Family semantics are reserved for actual descendants.
            if len(members) == 1 and not members[0].parent_session_id:
                continue
            # A single actively running tool is not a semantic loop; its
            # completion/result boundary will provide the next observation.
            if len(members) == 1 and members[0].current_tool:
                continue
            state = self._families.get(family_id)
            earliest = min(float(o.started_at) for o in members)
            markers = self._progress_markers(members)
            if state is None:
                state = _FamilyState(
                    started_at=earliest,
                ...
                state.snapshots.append(self._snapshot(members))
                self._families[family_id] = state
                continue
            state.started_at = min(state.started_at, earliest)
            if any(state.progress_markers.get(k) != v for k, v in markers.items()):
                state.last_progress_at = now
                state.snapshots.clear()
            state.progress_markers = markers
            snapshot = self._snapshot(members)
            state.snapshots.append(snapshot)
            max_history = max(6, self.thresholds.family_cycle_repeats * 3)
            while len(state.snapshots) > max_history:
                state.snapshots.popleft()
            if state.tripped:
                continue
            idle = now - state.last_progress_at
            # Detect period-1, period-2 and period-3 semantic cycles. The
            # snapshot contains normalized action fingerprints, not timestamps.
            cycle = None
            repeats = self.thresholds.family_cycle_repeats
            for period in (1, 2, 3):
                need = period * repeats
                if len(state.snapshots) < need:
                    continue
                tail = list(state.snapshots)[-need:]
                if all(tail[i] == tail[i % period] for i in range(need)):
                    cycle = period
                    break
            if idle >= self.thresholds.family_idle_seconds and cycle is not None:
                state.tripped = True
                return family_id, f"family idle {idle:.1f}s; semantic cycle period={cycle}: {snapshot!r}"
        return None

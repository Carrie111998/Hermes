...
def _audit_existing_replicas_locked(conn: sqlite3.Connection) -> None:
    ...
    for row in conn.execute(
        ...
        if any(
            event["authority_epoch"] != int(row["authority_epoch"]) for event in events
        ):
        ...
        try:
            ...
            for event in events:
                ...
                if actor["kind"] == "gateway" and actor["id"] != str(
                    row["authority_gateway_id"]
                ):
    ...
    with _replica_transaction(db_path) as conn:
        ...
        for event in events:
            if event["authority_epoch"] != stored_epoch and row is not None:
                raise ReplicaError(
                    "event authority conflicts with stored replica lineage"
                )

"""Tests for nastech_cli.tools_config platform tool persistence."""
...
from nastech_cli.nastech_account import NastechPortalAccountInfo, NastechToolAccessInfo
from nastech_cli.nastech_subscription import NastechSubscriptionFeatures
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'nastech' instead of 'nastech-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import nastech_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["nastech"]}}

    with caplog.at_level(logging.WARNING, logger="nastech_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "nastech" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["nastech-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["nastech-cli", "bogus"]}}
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NastechResearch/nastech-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("nastech_cli.tools_config.save_config"):
...
def test_first_install_nastech_auto_configures_video_gen(monkeypatch):
    """When a Nastech subscriber checks video_gen in the toolset checklist,
    apply_nastech_managed_defaults must write video_gen.provider and
    ...
    monkeypatch.setattr("nastech_cli.nastech_subscription.managed_nastech_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nastech"},
    ...
    monkeypatch.setattr(
        "nastech_cli.nastech_subscription.get_nastech_portal_account_info",
        lambda *args, **kwargs: NastechPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from nastech_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """nastech-gateway includes list should cover all messaging platforms."""
        from nastech_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["nastech-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from nastech_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from nastech_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NastechResearch/nastech-agent/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from nastech_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both Nastech Subscription and FAL.ai providers must carry the
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from nastech_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("nastech_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("nastech_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("nastech_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `nastech tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from nastech_cli.tools_config import _get_effective_configurable_toolsets
...
# ---------------------------------------------------------------------------
# Inline Nastech Portal login gate on managed-provider selection
# ---------------------------------------------------------------------------
...
#    reported as added/removed by `nastech tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `nastech tools` printed ``- kanban``
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    import nastech_cli.tools_config as tc
...
# Ready for every zero-env-var provider row, including logged-out Nastech
...
def test_visible_providers_reuses_logged_out_feature_snapshot(monkeypatch):
    ...
    assert any(
        provider.get("managed_nastech_feature") == "image_gen"
...
def test_visible_providers_reuses_pool_video_feature_snapshot(monkeypatch):
    ...
    account = NastechPortalAccountInfo(
        ...
        tool_access=NastechToolAccessInfo(
            enabled=True,
    ...
    assert not any(
        provider.get("managed_nastech_feature") == "video_gen"
        for provider in providers
    )
...
# ── Toolsets that shipped after a platform's last `nastech tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[nastech-cli]``) with a frozen explicit list, and
...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    """The bug: a frozen list never gained bfl, so composite users got Nastech
    Portal video generation on upgrade and picker users silently did not."""
    on_composite = _get_platform_tools(
        {"platform_toolsets": {"cli": ["nastech-cli"]}},
        "cli",
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (nastech-acp, nastech-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"nastech-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"nastech-{platform}"))

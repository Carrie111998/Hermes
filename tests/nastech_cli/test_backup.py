"""Tests for nastech backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew nastech_cli.backup's datetime forward instead of sleeping.
    ...
    import nastech_cli.backup as _backup

    shim = getattr(_backup.datetime, "_nastech_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _nastech_test_shim = True
...
def _make_nastech_tree(root: Path) -> None:
    """Create a realistic ~/.nastech directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "nastech_state.db"):
    ...
    # nastech-agent repo (should be EXCLUDED)
    (root / "nastech-agent").mkdir(exist_ok=True)
    (root / "nastech-agent" / "run_agent.py").write_text("# big file\n")
    (root / "nastech-agent" / ".git").mkdir()
    (root / "nastech-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_nastech_agent(self):
        from nastech_cli.backup import _should_exclude
        assert _should_exclude(Path("nastech-agent/run_agent.py"))
        assert _should_exclude(Path("nastech-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        nastech_home.mkdir()
        _make_nastech_tree(nastech_home)
        ...
        import nastech_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = nastech_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, nastech_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside NASTECH_HOME."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        _make_nastech_tree(nastech_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(nastech_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Nastech backup."""
        from nastech_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        nastech_home = tmp_path / ".nastech"
        (nastech_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (nastech_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (nastech_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            nastech_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (nastech_home / "gateway.pid").write_text("4242")
        (nastech_home / "processes.json").write_text('{"live": true}')
        ...
        assert (nastech_home / "gateway.pid").read_text() == "4242"
        assert (nastech_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (nastech_home / "cron.pid").exists()
        assert not (nastech_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"nastech": "token"}}',
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (nastech_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".nastech"
        src_home.mkdir(parents=True)
        _make_nastech_tree(src_home)

        monkeypatch.setenv("NASTECH_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from nastech_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".nastech"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("NASTECH_HOME", str(dst_home))
        ...
        # nastech-agent should NOT be present
        assert not (dst_home / "nastech-agent").exists()
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from nastech_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".nastech/", "")
            zf.writestr(".nastech/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_nastech_home(self, tmp_path, monkeypatch):
        """Backup handles empty nastech home (no files to back up)."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        # Only excluded dirs, no actual files
        (nastech_home / "__pycache__").mkdir()
        (nastech_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        (nastech_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = nastech_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        (nastech_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (nastech_home / "config.yaml").exists()
        assert (nastech_home / "sessions" / "s0599.json").exists()
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from nastech_cli.backup import _safe_copy_db
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from nastech_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def nastech_home(self, tmp_path):
        """Create a fake NASTECH_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, nastech_home):
        from nastech_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nastech_home=nastech_home)
        db_copy = nastech_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, nastech_home, monkeypatch, capsys):
        ...
        snap_id = backup_mod.create_quick_snapshot(nastech_home=nastech_home)
        err = capsys.readouterr().out
        assert "SQLite safe copy FAILED" in err or "CRITICAL" in err
        assert "state.db" in err
        # Other small files may still snapshot
        if snap_id:
            manifest = (nastech_home / "state-snapshots" / snap_id / "manifest.json")
    ...
    def test_snapshot_includes_pairing_directories(self, nastech_home):
        ...
        (nastech_home / "platforms" / "pairing").mkdir(parents=True)
        (nastech_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (nastech_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (nastech_home / "pairing").mkdir()
        (nastech_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (nastech_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = nastech_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (nastech update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, nastech_home, capsys):
        ...
        """
        import json
        from nastech_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", nastech_home=nastech_home)
        assert first_id is not None
        first_dir = nastech_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", nastech_home=nastech_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = nastech_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, nastech_home=nastech_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, nastech_home):
        ...
        from nastech_cli.backup import create_quick_snapshot, restore_quick_snapshot

        board_dir = nastech_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(nastech_home=nastech_home)
        copy = (
            nastech_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, nastech_home=nastech_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, nastech_home, monkeypatch):
        ...
        import nastech_cli.backup as bk
        from nastech_cli.backup import create_quick_snapshot

        board = nastech_home / "kanban" / "boards" / "work"
        ...
        copy = nastech_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``nastech update``
    ...
    def nastech_home(self, tmp_path):
        ...
        root.mkdir()
        _make_nastech_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, nastech_home):
        """Pre-update backup should include the same user data that
        ``nastech backup`` would, and should exclude the same directories."""
        from nastech_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(nastech_home=nastech_home)
        ...
        # nastech-agent repo excluded
        assert not any(n.startswith("nastech-agent/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, nastech_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(nastech_home=nastech_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (nastech_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, nastech_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside NASTECH_HOME."""
        from nastech_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def nastech_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_nastech_tree(root)
        # Point NASTECH_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("NASTECH_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve NASTECH_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(nastech_home, value):
        import yaml
        (nastech_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(nastech_home):
        d = nastech_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(nastech_home):
        d = nastech_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, nastech_home, capsys):
        ...
        assert out == ""
        assert not self._snaps(nastech_home)
        assert not self._zips(nastech_home)
    ...
    def test_config_full_mode(self, nastech_home, capsys):
        ...
        assert len(self._zips(nastech_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (nastech claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``nastech claw migrate`` runs before mutating ~/.nastech/."""
    ...
    def test_restorable_with_nastech_import(self, nastech_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Nastech
        backup — `nastech import` should accept it."""
        from nastech_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(nastech_home=nastech_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, nastech_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from nastech_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(nastech_home=nastech_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(nastech_home=nastech_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`nastech update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, nastech_home: Path, label="pre-update"):
        from nastech_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, nastech_home=nastech_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from nastech_cli.backup import restore_cron_jobs_if_emptied
        nastech_home = tmp_path / ".nastech"
        jobs_path = nastech_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(nastech_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, nastech_home=nastech_home)
        assert result is not None
...
# location, NOT under NASTECH_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, nastech_home: Path) -> None:
        nastech_home.mkdir(parents=True, exist_ok=True)
        (nastech_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (nastech_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (nastech_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        nastech_home = tmp_path / ".nastech"
        self._make_min_tree(nastech_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under NASTECH_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        nastech_home = dst_home / ".nastech"
        nastech_home.mkdir()
        ...
        # External state did NOT leak into NASTECH_HOME.
        assert not (nastech_home / "_external").exists()

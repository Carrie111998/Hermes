"""Tests for Nastech subscription feature detection."""

from nastech_cli.nastech_account import NastechPortalAccountInfo, NastechToolAccessInfo
from nastech_cli import nastech_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> NastechPortalAccountInfo:
...
def _pool_account() -> NastechPortalAccountInfo:
    ...
    return NastechPortalAccountInfo(
        ...
        tool_access=NastechToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )


def test_get_nastech_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_nastech_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_nastech_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_nastech is False
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "nastech_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_prompt_enable_tool_gateway_pool_offers_covered_tools_only(monkeypatch):
    ...
    monkeypatch.setattr(ns, "get_nastech_portal_account_info", lambda **kw: _pool_account())
    monkeypatch.setattr(
        ns,
...
def test_apply_nastech_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_nastech_managed_defaults must write video_gen.provider and
    video_gen.use_gateway when a Nastech subscriber selects video_gen
    without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_nastech_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_nastech_portal_account_info",
        lambda **kw: _account(logged_in=True, paid=True),
    )

    config = {"model": {"provider": "nastech"}}
    changed = ns.apply_nastech_managed_defaults(
...
# ---------------------------------------------------------------------------
# ensure_nastech_portal_access — inline login gate for `nastech tools`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STT — managed-by-Nastech detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------
...
def _stt_features_stub(*, account_info):
    return ns.NastechSubscriptionFeatures(
        subscribed=True,
        nastech_auth_present=True,
        provider_is_nastech=True,
        account_info=account_info,
        features={
            key: ns.NastechFeatureState(
                key=key, label=key, included_by_default=True,
                available=False, active=False, managed_by_nastech=False,
...
def test_has_agent_browser_resolves_via_nastech_managed_node_path(monkeypatch, tmp_path):
    """The managed-Node rung: a runnable agent-browser under the Nastech Node
    ...
    monkeypatch.setattr(
        "nastech_constants.with_nastech_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "nastech_constants.agent_browser_runnable",

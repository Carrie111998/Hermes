"""Tests for nastech_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_nastech_home):
    """Drop the example-dashboard fixture into the per-test NASTECH_HOME
    ...
    isolated ``NASTECH_HOME``.

    The user-plugin source is preferred over a transient
    ``NASTECH_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, nastech-achievements, model providers
    ...
    # plugin in exactly as a real operator would with `nastech plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down NASTECH_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Nastech vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints NASTECH_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nastech_home):
        """Create a TestClient and isolate the state DB under the test NASTECH_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `nastech update`
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = nastech_state.SessionDB.__init__
        ...
        # the class on nastech_state covers every open the helper makes.
        monkeypatch.setattr(nastech_state.SessionDB, "__init__", counting_init)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        NASTECH_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        ...
        config_path = get_nastech_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from nastech_constants import get_nastech_home
        from nastech_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide NASTECH_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from nastech_constants import get_nastech_home

        path = get_nastech_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_nastech_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["nastech"]["workspace"] == "myws"
        assert cfg["hosts"]["nastech"]["peerName"] == "eri"
        assert cfg["hosts"]["nastech"]["environment"] == "local"
        assert cfg["hosts"]["nastech"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["nastech"]["apiKey"] == "hch-test-key"
    ...
    def test_update_nastech_returns_docker_guidance_without_spawning(self, monkeypatch):
        import nastech_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn nastech update")
        ...
        monkeypatch.setattr(web_server, "_spawn_nastech_action", fail_spawn)
        web_server._ACTION_PROCS.pop("nastech-update", None)
        web_server._ACTION_RESULTS.pop("nastech-update", None)

        resp = self.client.post("/api/nastech/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "nastech-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nastechresearch/nastech-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/nastech-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nastechresearch/nastech-agent:latest" in line for line in status_data["lines"])

    def test_update_nastech_spawns_with_action_id(self, monkeypatch):
        import nastech_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_nastech_action", fake_spawn)
        web_server._ACTION_PROCS.pop("nastech-update", None)
        web_server._ACTION_RESULTS.pop("nastech-update", None)
        ...
        assert calls == [
            (["update"], "nastech-update", {"NASTECH_ACTION_ID": "a" * 32})
        ]

    def test_update_nastech_reuses_running_action(self, monkeypatch):
        import nastech_cli.web_server as web_server
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_nastech_action",
            lambda *_args, **_kwargs: pytest.fail("must not spawn a duplicate update"),
        )
        web_server._ACTION_PROCS["nastech-update"] = Proc()
        web_server._ACTION_IDS["nastech-update"] = "b" * 32

        try:
            resp = self.client.post("/api/nastech/update")
        finally:
            web_server._ACTION_PROCS.pop("nastech-update", None)
            web_server._ACTION_IDS.pop("nastech-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/nastech_cli/test_setup_hidden_env.py.
        """
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Nastech provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from nastech_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from nastech_cli.config import save_env_value
        from nastech_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "nastech_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/NastechSetupBot/nastech_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/NastechSetupBot/nastech_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "nastech_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_nastech_action", fail_spawn_action)
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from nastech_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``nastech model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from nastech_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from nastech_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from nastech_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from nastech_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from nastech_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_nastech_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out Nastech Subscription rows, xAI TTS without Grok OAuth, and
        ...
        # Logged out of Nastech Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "nastech_cli.nastech_subscription.get_nastech_portal_account_info",
            lambda *a, **k: NastechPortalAccountInfo(
        ...
        assert by_name["Nastech Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_nastech_provider_reports_needs_nastech_auth(self, monkeypatch):
        """Selecting a managed Nastech row while logged out flags needs_nastech_auth.
        ...
        (ensure_nastech_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing Nastech OAuth flow.
        """
        from nastech_cli.nastech_account import NastechPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "Nastech Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_nastech_auth"] is True
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "nastech"
        save_config(config)
        ...
        assert "nastech@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from nastech_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from nastech_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from nastech_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.nastech/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_nastech_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(nastech_home, name="ocean"):
        themes_dir = nastech_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="nastech-theme-bootstrap">')
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``nastech_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in nastech_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nastech_home):
        ...
        # Pin the SessionDB to the isolated NASTECH_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_nastech_home, _install_example_plugin):
        ...
        installs it into the per-test ``NASTECH_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into NASTECH_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, nastech-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (nastech-achievements is loaded by default).
        resp = self.client.get("/api/plugins/nastech-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        NASTECH_HOME override scopes a request to another profile."""
        from nastech_constants import (
            reset_nastech_home_override,
            set_nastech_home_override,
        )
...
# or ``sh -c 'printf …'``) instead of the real ``nastech --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"nastech-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "NASTECH_CWD": str(tmp_path),
            "NASTECH_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["NASTECH_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from nastech_cli.pty_bridge import PtyUnavailableError
        ...
        import nastech_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from nastech_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("NASTECH_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nastech_home, _install_example_plugin):
        ...
        the per-test ``NASTECH_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_nastech_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under NASTECH_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("NASTECH_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        assert "url(/nastech/ds-assets/bg.png)" in prefixed.text
        assert "url(/nastech/fonts-terminal/x.woff2)" in prefixed.text

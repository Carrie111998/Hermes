...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`nastech gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `nastech gateway restart` in the background; when a
        ...
        assert "nastech gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``NASTECH_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../nastech_test`` NASTECH_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "nastech-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir NASTECH_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="NASTECH_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/nastech_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="NASTECH_HOME=/tmp/nastech-e2e-41264"\n'
        assert (
            gateway_cli._temp_home_in_service_definition(unit)
            == "/tmp/nastech-e2e-41264"
        )
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="NASTECH_HOME={tmp_path}/nastech-home"\n'
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "nastech gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".nastech" / "profiles" / "jarvis" / "node" / "bin"
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.nastech.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>NASTECH_HOME</key>"
                "<string>/Users/alice/.nastech</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_nastech_home", lambda: tmp_path)
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.nastech.gateway"])
            ),
        )
...
class TestSystemUnitNastechHome:
    """NASTECH_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'NASTECH_HOME=/home/alice/.nastech' in unit
        assert '/root/.nastech' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's NASTECH_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        nastech_home = str(gateway_cli.get_nastech_home().resolve())
        assert f'NASTECH_HOME={nastech_home}' in unit


class TestSystemUnitRefreshSyncsNastechHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.nastech."""

    def test_refresh_adopts_unit_nastech_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_nastech = root_home / ".nastech"
        alice_nastech = alice_home / ".nastech"
        root_nastech.mkdir(parents=True)
        alice_nastech.mkdir(parents=True)
        (root_nastech / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_nastech / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's NASTECH_HOME + drain timeout).
        monkeypatch.setenv("NASTECH_HOME", str(alice_nastech))
        ...
        # Simulate sudo without inherited NASTECH_HOME (falls back to root).
        monkeypatch.setenv("NASTECH_HOME", str(root_nastech))
        ...
        assert os.environ["NASTECH_HOME"] == str(alice_nastech)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned NASTECH_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_nastech_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_nastech_home_from_systemd_unit",
...
class TestNastechHomeForTargetUser:
    """Unit tests for _nastech_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("NASTECH_HOME", raising=False)

        result = gateway_cli._nastech_home_for_target_user("/home/alice")
        assert result == "/home/alice/.nastech"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "nastech gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".nastech" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("NASTECH_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_nastech_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'NASTECH_HOME={target_home / ".nastech" / "profiles" / "mybot"}' in unit



    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".nastech" / "profiles" / "orcha"
        ...
        monkeypatch.setattr(gateway_cli, "get_nastech_home", lambda: profile_dir)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.nastech.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".nastech" / "nastech-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".nastech" / "nastech-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".nastech" / "nastech-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("NASTECH_HOME", str(root_home / ".nastech"))
        monkeypatch.setattr(gateway_cli, "get_nastech_home", lambda: root_home / ".nastech")
        ...
        # WorkingDirectory is anchored at the target user's NASTECH_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.nastech" in unit
        assert "WorkingDirectory=/home/alice/.nastech/nastech-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "nastech-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "nastech-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'nastech gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyNastechUnitDetection:
    """Tests for _find_legacy_nastech_units / has_legacy_nastech_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``nastech.service`` unit behind when the service was
    renamed to ``nastech-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``nastech.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``nastech-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Nastech Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m nastech_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "nastech.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "nastech.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_nastech_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m nastech_cli.main gateway run
          - python path/to/nastech_cli/main.py gateway run
          - nastech gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m nastech_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/nastech/nastech_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/nastech gateway run --replace",
            "ExecStart=/venv/bin/python /opt/nastech/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "nastech.service" if i == 0 else "nastech.service"  # same name
            # Test each variant fresh
            (user_dir / "nastech.service").write_text(
                f"[Unit]\nDescription=Old Nastech\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_nastech_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "nastech.service" in out
        assert "nastech gateway migrate-legacy" in out



class TestRemoveLegacyNastechUnits:
    """Tests for remove_legacy_nastech_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "nastech.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_nastech_units(interactive=False)
        ...
        assert any("--user stop nastech.service" in c for c in cmds_joined)
        assert any("--user disable nastech.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (nastech-gateway-coder.service)
        ...
        profile_unit = user_dir / "nastech-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "nastech-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `nastech gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `nastech gateway --help` shows it.
        import subprocess
        import sys
        ...
        result = subprocess.run(
            [sys.executable, "-m", "nastech_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_nastech_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "nastech_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_nastech_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_nastech_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_nastech_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_nastech_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_nastech_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "nastech-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "nastech-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "nastech-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``nastech gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "nastech-gateway")
        ...
        assert "sudo systemctl start nastech-gateway" in out
        assert "sudo nastech gateway uninstall --system" in out
        assert "nastech gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``nastech gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (NASTECH_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_nastech_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".nastech"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_nastech_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_nastech_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.nastech.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.nastech.gateway"

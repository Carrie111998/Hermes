...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure ORION_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", orion_home=str(tmp_path / ".orion"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_HOME", str(orion_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", orion_home=str(orion_home))
        return p

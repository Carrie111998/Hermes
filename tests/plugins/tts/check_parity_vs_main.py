...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "tts_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "orion-agent-main"
...
# Isolated ORION_HOME so the config write is hermetic.
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'orion-agent-main' checkout pinned to "

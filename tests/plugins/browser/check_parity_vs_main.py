...
Run from the PR worktree:
    ...
    cd ~/.orion/orion-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.orion/orion-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.orion/orion-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a orion-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a orion-agent checkout"
)
...
# Isolated ORION_HOME for the config write.

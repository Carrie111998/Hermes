...
def test_create_task_persists_max_iterations(client):
    ...
    assert stored.max_iterations == 45


@pytest.mark.parametrize("value", [0, -1])
def test_create_task_rejects_non_positive_max_iterations(client, value):
    response = client.post(
        "/api/plugins/kanban/tasks",
        json={
            "title": "Invalid dashboard budget",
            "assignee": "builder",

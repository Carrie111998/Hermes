...
def _expected(task):
    return {
        ...
        "expected_assignee": task.get("assignee"),
        "expected_current_step_key": task.get("current_step_key"),
        "expected_current_run_id": task.get("current_run_id"),
    }
...
def test_spawn_model_only_omits_provider_flag(monkeypatch, tmp_path, conn):
    tid = kb.create_task(conn, title="t", assignee="elias", model_override="glm-5")
    task = kb.get_task(conn, tid)
    cmd = _spawn_and_capture(monkeypatch, tmp_path, task)
    assert "-m" in cmd
...
def test_patch_clears_model_override(client):
    task = _create(client, model_override="gpt-5.6-sol", provider_override="openai")
    r = client.patch(
...
def test_create_task_with_override_via_api(client):
    task = _create(client, model_override="qwen-max", provider_override="openrouter")
...
def test_patch_sets_and_clears_reasoning_effort(client):
    ...
    r = client.patch(
        f"/api/plugins/kanban/tasks/{task['id']}",
        json={**_expected(task), "reasoning_effort": "xhigh"},
    )
    assert r.status_code == 200, r.text
    ...
    r = client.patch(
        f"/api/plugins/kanban/tasks/{task['id']}",
        json={**_expected(r.json()["task"]), "clear_reasoning_effort": True},
    )
    assert r.status_code == 200, r.text
...
def test_patch_rejects_an_unknown_level(client):
    ...
    r = client.patch(
        f"/api/plugins/kanban/tasks/{task['id']}",
        json={**_expected(task), "reasoning_effort": "bogus"},
    )
    assert r.status_code == 400
...
def test_bulk_reasoning_effort(client):
    ...
    r = client.post(
        ...
        json={
            "ids": [t1["id"], t2["id"]],
            "expected_snapshots": {t1["id"]: _expected(t1), t2["id"]: _expected(t2)},

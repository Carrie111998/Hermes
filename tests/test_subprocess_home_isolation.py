...
Orion state stays profile-scoped through ORION_HOME. Host subprocesses should
...
See: https://github.com/zacharyjleach-stack/Aries/issues/25114
See: https://github.com/zacharyjleach-stack/Aries/issues/36144
See: https://github.com/zacharyjleach-stack/Aries/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for orion_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(orion_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("ORION_REAL_HOME", raising=False)

    def _container_mode(self, monkeypatch):
        monkeypatch.setattr(orion_constants, "is_container", lambda: True)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("ORION_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        orion_home = real_home / ".orion" / "profiles" / "coder"
        profile_home = orion_home / "home"
        profile_home.mkdir(parents=True)
        monkeypatch.setenv("HOME", str(real_home))
        monkeypatch.setenv("ORION_HOME", str(orion_home))
        from orion_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_REAL_HOME", str(real_home))

        from orion_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ORION_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["ORION_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(orion_home / "home")
        assert result["ORION_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_orion_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from orion_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from orion_cli.profiles import create_profile

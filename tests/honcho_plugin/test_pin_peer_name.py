...
Under a gateway (Telegram, Discord, Slack, ...) Orion passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Orion via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Orion profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `orion -p partner` pinned to a different person's peer).

...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "orion"
        assert config.workspace_id == "orion"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"orion": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"orion": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"orion": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="orion-agent"
        ):
            result = config.resolve_session_name("/home/user/orion-agent/subdir")
        assert result == "orion-agent"


class TestResolveConfigPath:
    def test_prefers_orion_home_when_exists(self, tmp_path):
        orion_home = tmp_path / "orion"
        orion_home.mkdir()
        local_cfg = orion_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"ORION_HOME": str(orion_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ORION_HOME points at ~/.orion/profiles/<name>, so
        # _get_default_orion_home() must resolve back to ~/.orion — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".orion"
        ...
        monkeypatch.setenv("ORION_HOME", str(profile_home))

        result = resolve_config_path()

        assert _get_default_orion_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "orion_coder"
        assert profile_host_key("default") == "orion"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ORION_HONCHO_HOST": "orion.coder"}):
            assert resolve_active_host() == "orion.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("ORION_HONCHO_HOST", None)
            # Temporarily remove orion_cli.profiles to simulate import failure
            saved = sys.modules.get("orion_cli.profiles")
            sys.modules["orion_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "orion"
            finally:
                if saved is not None:
                    sys.modules["orion_cli.profiles"] = saved
                else:
                    sys.modules.pop("orion_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="orion_coder")
        assert config.host == "orion_coder"
        assert config.workspace_id == "orion"  # shared workspace
        assert config.ai_peer == "orion_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"orion": {"enabled": True, "aiPeer": "orion"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (orion.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="orion_work",
            workspace_id="orion",
            raw={"hosts": {"orion.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_orion_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_local_base_url_with_v3_suffix_stripped(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("orion_cli.config.load_config", return_value={}):
            get_honcho_client(cfg)
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("orion_cli.profiles.get_active_profile_name", return_value="default"), \

...
class TestMatrixAccessTokenAuth:
    @pytest.mark.asyncio
    async def test_password_rotation_does_not_persist_an_offline_verifier(
        self, monkeypatch, tmp_path
    ):
        """Runtime tokens are scoped by account/device, never a password hash."""
        from plugins.platforms.matrix.adapter import MatrixAdapter
        ...
        original = MatrixAdapter(
            PlatformConfig(
                enabled=True,
                token="",
                extra={**base_extra, "password": "first-password"},
            )
        )
        original._token_store.save(
            bootstrap_fingerprint=original._bootstrap_fingerprint,
            homeserver=original._homeserver,
            user_id=original._user_id,
            device_id=original._device_id,
            access_token="syt_runtime_access",
            refresh_token="syt_runtime_refresh",
        )

        after_password_change = MatrixAdapter(
            PlatformConfig(
                enabled=True,
                token="",
                extra={**base_extra, "password": "second-password"},
            )
        )
        await after_password_change._load_runtime_tokens()

        assert after_password_change._access_token == "syt_runtime_access"
        assert after_password_change._refresh_token == "syt_runtime_refresh"

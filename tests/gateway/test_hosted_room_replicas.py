...
def test_database_guard_blocks_an_old_process_promoting_a_replica(tmp_path):
    ...
    with (
        ...
        pytest.raises(sqlite3.IntegrityError, match="already reserved"),
    ):
        conn.execute(
...
def test_disbanded_replica_room_id_cannot_be_recreated(tmp_path):
    ...
    page = rooms.read_events(authority_db, room_id="room-1", include_disbanded=True)
    db = _replica_db(tmp_path)
    replicas.ingest_page(
        db, room_id="room-1", room_name="Field Room", members=MEMBERS, page=page
    )
...
def test_migration_quarantines_both_sides_of_a_namespace_collision(tmp_path):
    ...
    payload_json = "{}"
    event_bytes = rooms._event_storage_bytes(  # noqa: SLF001
        event_id="replica-disband",
        kind="room.disbanded",
    ...
    with sqlite3.connect(db) as conn:
        ...
        conn.execute(
            """INSERT INTO hosted_room_replicas(
                   ...
                   created_at, updated_at, disbanded_at
               ) VALUES ('room-1', 'Field Room', ?, ?, 1, 1, 1, ?, 1, 2, 2)""",
            (json.dumps(MEMBERS, separators=(",", ":")), AUTH_A, event_bytes),
        )
        conn.execute(
    ...
    with sqlite3.connect(db) as conn:
        conn.row_factory = sqlite3.Row
        assert (
            rooms._prune_disbanded_replicas_locked(  # noqa: SLF001
                conn,
                now=None,
...
def test_migration_compacts_an_oversized_legacy_replica_and_fences_old_writers(
    ...
    with sqlite3.connect(db) as conn:
        conn.row_factory = sqlite3.Row
        rooms._initialize_schema(conn)  # noqa: SLF001
        conn.execute("DROP TRIGGER trg_hosted_events_shared_budget")
        conn.execute("DROP TRIGGER trg_hosted_replica_events_shared_budget")
        conn.execute(
            """INSERT INTO hosted_room_replicas(
                   room_id, name, members_json, authority_gateway_id,
                   authority_epoch, last_seq, latest_seq, event_bytes,
                   created_at, updated_at
               ) VALUES ('legacy-large', 'Legacy', ?, ?, 1, 1, 1, 0, 1, 1)""",
    ...
    with sqlite3.connect(db) as conn:
        ...
        assert reason == "replica_storage_budget_exceeded"
        conn.execute(
            """INSERT INTO hosted_room_replicas(
                   room_id, name, members_json, authority_gateway_id,
                   authority_epoch, last_seq, latest_seq, event_bytes,
                   created_at, updated_at
               ) VALUES ('legacy-next', 'Next', ?, ?, 1, 0, 1, 0, 2, 2)""",
            (json.dumps(MEMBERS, separators=(",", ":")), AUTH_A),
        )
        with pytest.raises(sqlite3.IntegrityError, match="budget exceeded"):
            conn.execute(
                """INSERT INTO hosted_room_replica_events(
                       ...
                       'legacy-next', 1, 'too-large', 'message.user', ?, 1, ?, 2
                   )""",
                (actor_json, payload_json),
            )
...
def test_migration_quarantines_unsafe_takeover_lineage(tmp_path, kind, payload, reason):
    db = _replica_db(tmp_path)
    rooms.create_room(
        db,
        room_id="room-1",
...
def test_aged_terminal_replicas_free_room_slots_but_keep_ids(tmp_path, monkeypatch):
    db = _replica_db(tmp_path)
    ...
    with pytest.raises(replicas.ReplicaHistoryExpiredError, match="history expired"):
...
def test_authoritative_append_reclaims_terminal_replica_bytes(tmp_path, monkeypatch):
    db = _replica_db(tmp_path)
    replicas.ingest_page(
        db,
        room_id="old",

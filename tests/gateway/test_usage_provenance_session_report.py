...
def test_fractional_measured_usage_fails_closed(kwargs):
    component = UsageComponentReceipt(
        component_id="fractional",
        session_id="bound-session",
        provenance=UsageProvenance.MEASURED,
        authority="backend_observed",
        authoritative=True,
        accepted_event_id="fractional-event",
        **kwargs,
    )

    assert component.provenance is UsageProvenance.UNKNOWN
    assert component.authoritative is False
    assert component.value is None
    assert component.reason == "invalid_usage_value"


def test_malformed_component_list_remains_visible_as_unknown():
    telemetry = ProgressTelemetry(session_id="bound-session")
    aggregate = telemetry.record_usage_mapping(
        {
            "components": [
                {
                    "component_id": "valid",
                    ...
                    "authority": "backend_observed",
                    "authoritative": True,
                    "accepted_event_id": "valid-event",
                    "total_tokens": 10,
                },
                "malformed",
            ]
        },
        default_component_id="envelope",
        session_id="bound-session",
    )

    assert aggregate.provenance is UsageProvenance.MIXED
    assert aggregate.known_component_count == 1
    assert aggregate.unknown_component_count == 1
    assert any(c.reason == "malformed_usage_component" for c in aggregate.components)


def test_conflicting_duplicate_component_id_fails_closed():
    usage = aggregate_usage(
        "bound-session",
        [
            _component("duplicate", "measured", total_tokens=10),
            _component("duplicate", "measured", total_tokens=12),
        ],
    )

    assert usage.provenance is UsageProvenance.UNKNOWN
    assert usage.unknown_component_count == 1
    assert usage.components[0].reason == "conflicting_duplicate_component"

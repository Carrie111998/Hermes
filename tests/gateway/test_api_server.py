...
class TestSessionIdHeader:
    ...
    async def test_wake_capability_follows_session_id_provenance(self, auth_adapter):
        ...
        explicitly provided (header client that reads it back), "" when it is
        fingerprint-derived (header-less client) — delegate_task's background
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                # fingerprint — the client will never resume that session.
                resp = await cli.post(
                    "/v1/chat/completions",

...
def gateway_home(monkeypatch, tmp_path):
    monkeypatch.setattr(gateway_run, "_orion_home", tmp_path)
    monkeypatch.delenv("ORION_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("ORION_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("ORION_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("ORION_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("ORION_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path

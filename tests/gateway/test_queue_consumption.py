...
class TestBusyInputModeQueueFifo:
    ...
    def test_photo_burst_stays_one_turn_with_constituent_context(self):
        ...
        assert [
            event.raw_message for event in merged._merged_media_source_events
        ] == [event.raw_message for event in events]
        assert [
            event.platform_update_id for event in merged._merged_media_source_events
        ] == [0, 1, 2]
        assert [
            event.metadata["native_id"] for event in merged._merged_media_source_events

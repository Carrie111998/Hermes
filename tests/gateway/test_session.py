...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_orion_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("orion_constants.display_orion_home", return_value="~/.orion/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.orion/profiles/coder/cron/output/" in prompt
...
class TestSessionStoreRewriteTranscript:
    ...
    def store(self, tmp_path, monkeypatch):
        import orion_state
        monkeypatch.setattr(orion_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
        config = GatewayConfig()
...
class TestWhatsAppSessionKeyConsistency:
    ...
    def test_whatsapp_group_participant_aliases_share_session_key(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_HOME", str(tmp_home))
...
class TestGatewaySessionDbRecovery:
    ...
    def test_transcript_reroute_migrates_remaining_backlog_to_child(self):
        import threading
        from types import SimpleNamespace
        from orion_state import CompressionSessionClosedError

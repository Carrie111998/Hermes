...
def _pin_feishu_home(case: unittest.TestCase) -> None:
    """Pin plugins.platforms.feishu.adapter.get_hermes_home to a per-test tempdir.
    ...
    Merge note (0.19.0): upstream moved the Feishu adapter from
    ``gateway.platforms.feishu`` to ``plugins.platforms.feishu.adapter``; the
    patch target below follows that move (patch where the name is looked up
    — the adapter module's ``from hermes_constants import get_hermes_home``
    binding — not where it's defined).
    """

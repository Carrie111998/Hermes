...
(/opt/orion/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable ORION_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_orion_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable ORION_HOME."""
    ...
    monkeypatch.setattr(
        "orion_constants.get_orion_home", lambda: orion_home
    )
    ...
    expected = orion_home / "scripts" / "whatsapp-bridge"

...
def test_terminal_event_replay_is_noop_but_distinct_event_id_is_new_attempt():
    ...
    first = telemetry.record_terminal_event(
        event, tool_name="read_file", args={"path": "same.py"}
    )
    ...
    assert replay.result == event.result
    assert telemetry._seen_events["evt-1"].result is None
    assert distinct.replayed is False
    assert telemetry.attempt_seq == 2
    assert telemetry.no_progress_streak == 2
    assert telemetry.last_event_id == "evt-2"


def test_three_identical_distinct_attempts_report_full_no_progress_streak():
    telemetry = ProgressTelemetry(session_id="session-real")
    for index in range(3):
        telemetry.record_terminal_event(
            _event(f"evt-{index}"), tool_name="read_file", args={"path": "same.py"}
        )

    assert telemetry.attempt_seq == 3
    assert telemetry.no_progress_streak == 3


def test_concurrent_terminal_event_recording_is_lossless_and_replay_safe():
    telemetry = ProgressTelemetry(session_id="concurrent-session")
    unique_events = [_event(f"evt-{index}") for index in range(64)]

    with ThreadPoolExecutor(max_workers=16) as pool:
        list(
            pool.map(
                lambda event: telemetry.record_terminal_event(
                    event, tool_name="read_file", args={"path": "same.py"}
                ),
                unique_events,
            )
        ...
        list(
            pool.map(
                lambda _index: telemetry.record_terminal_event(
                    unique_events[0], tool_name="read_file", args={"path": "same.py"}
                ),
                range(32),
            )
        )

    assert telemetry.attempt_seq == 64
    assert telemetry.no_progress_streak == 64
    assert len(telemetry._seen_events) == 64
...
def test_turn_reset_increments_generation_and_restarts_local_sequences():
    telemetry = ProgressTelemetry(session_id="turn-generation-session")
    telemetry.record_attempt_completion(
        "web_search",
        {},
        "denied",
        is_failure=True,
        event_id="turn-1-event",
        retryability=Retryability.NON_RETRYABLE,
    )
    first = telemetry.get_activity_snapshot()

    telemetry.reset_for_turn()
    telemetry.record_attempt_completion(
        "web_search",
        {},
        "denied again",
        is_failure=True,
        event_id="turn-2-event",
        retryability=Retryability.NON_RETRYABLE,
    )
    second = telemetry.get_activity_snapshot()

    assert second["turn_generation"] == first["turn_generation"] + 1
    assert second["attempt_seq"] == 1
    assert second["failure_seq"] == 1


def test_guard_resets_attempt_high_water_and_error_streak_for_new_generation():
    guard = RunawayGuard(
        GuardThresholds(
    ...
    def failure(generation: int, sequence: int, code: int) -> SessionObservation:
        return SessionObservation(
            session_id="generation-errors",
            started_at=0.0,
    ...
    assert guard.observe(failure(1, 5, 401), now=1.0) is None
    # The first attempt in turn two restarts at one and establishes the new
    # status-code streak; it must not be suppressed by turn one's high-water.
    assert guard.observe(failure(2, 1, 403), now=2.0) is None
    trip = guard.observe(failure(2, 2, 403), now=3.0)
    assert trip is not None
    assert trip.reason is TripReason.REPEATED_ERROR
    assert "HTTP 403 repeated 2x" in trip.detail


def test_guard_resets_stall_tracking_for_new_generation():
    guard = RunawayGuard(
        GuardThresholds(
            huge_context_tokens=100,
    ...
    def stalled(generation: int, sequence: int) -> SessionObservation:
        return SessionObservation(
            session_id="generation-stall",
            started_at=0.0,
            turn_generation=generation,
            attempt_seq=sequence,
            context_tokens=200,
    ...
    assert guard.observe(stalled(1, 1), now=1.0) is None
    assert guard.observe(stalled(1, 2), now=2.0) is None
    assert guard.observe(stalled(2, 1), now=3.0) is None
    assert guard.observe(stalled(2, 2), now=4.0) is None
    trip = guard.observe(stalled(2, 3), now=5.0)
    assert trip is not None
    assert trip.reason is TripReason.NO_PROGRESS


def test_guard_trip_latch_is_generation_scoped():
    guard = RunawayGuard(
        GuardThresholds(
    ...
    def failure(generation: int, sequence: int) -> SessionObservation:
        return SessionObservation(
            session_id="generation-latch",
            started_at=0.0,
            turn_generation=generation,
            attempt_seq=sequence,
            failure_seq=sequence,
    ...
    assert guard.observe(failure(1, 1), now=1.0) is not None
    assert guard.observe(failure(1, 2), now=2.0) is None
    assert guard.observe(failure(2, 1), now=3.0) is not None
...
def test_live_observation_layers_terminal_fields_without_router_semantic_changes():
    ...
    assert observation.turn_generation == 7
    ...
    assert observation.context_tokens == 150_000
    assert mapping["session-observation"][2] == 4


def test_housekeeping_activity_does_not_manufacture_verified_progress():
    summary = {
        "api_call_count": 1,
        "attempt_seq": 1,
        "progress_seq": 0,
    ...
    guard = RunawayGuard(
        GuardThresholds(
            huge_context_tokens=100_000,
            no_progress_samples=2,
            max_calls_per_window=1_000,
            max_tokens_per_window=1_000_000_000,
            max_runtime_seconds=10_000,
        )
    )

    first = _collect_observations(runner, now=1.0, assumed_context_tokens=160_000)[0][0]
    assert guard.observe(first, now=1.0) is None
    summary.update(api_call_count=2, last_activity_ts=2.0, current_tool="web_search")
    second = _collect_observations(runner, now=2.0, assumed_context_tokens=160_000)[0][0]
    assert second.state_hash == first.state_hash
    assert guard.observe(second, now=2.0) is None
    summary.update(api_call_count=3, last_activity_ts=3.0, current_tool="read_file")
    third = _collect_observations(runner, now=3.0, assumed_context_tokens=160_000)[0][0]
    trip = guard.observe(third, now=3.0)
    assert trip is not None
    assert trip.reason is TripReason.NO_PROGRESS

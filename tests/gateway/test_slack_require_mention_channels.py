...
@pytest.mark.asyncio
async def test_global_strict_still_accepts_configured_wake_word(adapter):
    """The new native-only rule is scoped to strict_mention_channels."""
    ...
    adapter.handle_message.assert_called_once()
    assert adapter.handle_message.await_args.args[0].text == "Tracey proceed"


@pytest.mark.asyncio
async def test_global_strict_still_yields_to_free_response_channel(adapter):
    """Keep the legacy free-response precedence for global strict mode."""
    adapter.config.extra["strict_mention"] = True
    adapter.config.extra["free_response_channels"] = CHANNEL_ID

    await adapter._handle_slack_message(_event("ordinary channel message"))

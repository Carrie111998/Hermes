...
``orion_cli.config`` (get_env_value / save_env_value / remove_env_value),
``orion_cli.cli_output`` (prompt / prompt_yes_no / print_*), and
``orion_cli.setup`` (prompt_choice); we patch each at its source module so
...
import orion_cli.setup as setup_mod

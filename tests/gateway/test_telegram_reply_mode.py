...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/zacharyjleach-stack/Aries/issues/23994:
    reply_to_mode 'off' was ignored when sending via Orion-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/zacharyjleach-stack/Aries/issues/87051:
    ...
    response in a different chat lane than the Orion topic the session runs
    in. The no-anchor branch must prefer the Orion topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_orion_topic_thread_id(self):

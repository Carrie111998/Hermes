...
Proves, on windows-latest:
  1. `GatewayControlServer` binds ``\\\\.\\pipe\\orion-gateway-<hash>`` via
...
os.environ["ORION_HOME"] = sys.argv[2]
...
def test_named_pipe_identify_status_and_fleet_consumer(live_server, monkeypatch):
    ...
    assert ident["kind"] == "orion-gateway"
    ...
    import orion_cli.update_receipt as ur

    monkeypatch.setattr(
        "orion_cli.build_info.get_code_identity",
        lambda refresh=False: {"sha": ident.get("code_sha") or "X", "version": "t"},
    )
    monkeypatch.setattr("orion_cli.profiles._get_default_orion_home", lambda: home)
    monkeypatch.setattr(
        "orion_cli.profiles._get_profiles_root", lambda: home / "no-profiles"
    )
...
def test_pipe_gone_after_kill_falls_back(live_server, monkeypatch):
    ...
    (home / "gateway_state.json").write_text(
        json.dumps(
            {"pid": os.getpid(), "code_sha": "OLD", "kind": "orion-gateway"}
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(

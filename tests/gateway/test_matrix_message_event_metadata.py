...
def _make_adapter(require_mention=False, auto_thread=False):
...
@pytest.mark.asyncio
async def test_text_message_carries_sender_mxid():
    ...
    adapter._startup_ts = _time.time() - 10
...
@pytest.mark.asyncio
async def test_text_message_carries_sender_for_different_user():
...
@pytest.mark.asyncio
async def test_reply_carries_target_text_and_author():
...
@pytest.mark.asyncio
async def test_non_reply_message_has_no_reply_context():

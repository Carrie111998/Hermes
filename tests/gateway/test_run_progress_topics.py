...
class SplitSecretUrlPreviewAgent:
    ...
    URL_PREFIX = (
        "https://hermes-agent.nousresearch.com/docs/gateway/discord/"
        f"tool-progress?token={FIRST}"
    )
...
@pytest.mark.asyncio
async def test_discord_rich_preview_sanitizes_split_secret_before_formatting(
    monkeypatch,
    tmp_path,
):
    """Rich preview metadata must be derived only after fragment redaction."""
    monkeypatch.setattr("agent.redact._REDACT_ENABLED", False)
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        SplitSecretUrlPreviewAgent,
        session_id="sess-discord-split-secret-url",
        config_data={
            "display": {
                "tool_progress": "all",
        ...
        chat_type="dm",
        thread_id=None,
        adapter_cls=DiscordProgressCaptureAdapter,
    )
    ...
    assert SplitSecretUrlPreviewAgent.FIRST not in rendered
    assert SplitSecretUrlPreviewAgent.SECOND not in rendered
...
@pytest.mark.asyncio
async def test_terminal_progress_verbose_shows_full_command(monkeypatch, tmp_path):
    """Verbose markdown progress retains the complete multi-line command."""
    monkeypatch.setenv("HERMES_TOOL_PROGRESS_MODE", "verbose")

...
redirected ``get_orion_home()`` for that turn.  The rows carried the correct
...
def multiplex_homes(tmp_path, monkeypatch):
    """A root home plus a named profile home, with ORION_HOME on the root.
    ...
    monkeypatch.setenv("ORION_HOME", str(root))

    # The suite-wide fixture in conftest re-points ``orion_state.DEFAULT_DB_PATH``
    ...
    # the hatch is closed and resolution goes through ``get_orion_home()``,
    # which is what production does.  ``ORION_HOME`` above still keeps that
...
def test_db_handle_follows_the_active_profile_scope(multiplex_homes):
    ...
    token = set_orion_home_override(str(profile))
    try:

...
def test_parse_restart_after_turn_timeout_defaults_and_clamps():
    ...
    assert parse_restart_after_turn_timeout("nan") == DEFAULT_GATEWAY_RESTART_AFTER_TURN_TIMEOUT
    assert parse_restart_after_turn_timeout("inf") == DEFAULT_GATEWAY_RESTART_AFTER_TURN_TIMEOUT
    assert (
        parse_restart_after_turn_timeout(MAX_GATEWAY_SHUTDOWN_TIMING_COMPONENT_S * 2)
        == MAX_GATEWAY_SHUTDOWN_TIMING_COMPONENT_S
    )
...
def test_resolve_restart_exit_wait_budget_covers_both_phases():
    ...
    assert resolve_restart_exit_wait_budget(float("nan"), float("inf")) == 15.0

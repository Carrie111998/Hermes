...
@pytest.mark.asyncio
async def test_standard_completions_in_drain_are_coalesced(
    monkeypatch, tmp_path,
):
    """Standard completion events that reach the drain (legacy path) are
    also coalesced by session_key."""
    runner = _build_runner(monkeypatch, tmp_path, "all")
    adapter = runner.adapters[Platform.TELEGRAM]
    adapter.handle_message = AsyncMock()
    ...
    assert adapter.handle_message.await_count == 1
    injected_text = adapter.handle_message.await_args.args[0].text
    assert "3 background processes finished" in injected_text
    assert "batched" in injected_text.lower()

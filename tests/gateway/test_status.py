...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_profile_prefix_match(self, monkeypatch):
        payload = {
            "pid": 139,
            "gateway_state": "running",
            "kind": "hermes-gateway",
        ...
        code_home = Path("/opt/data/profiles/code")
        ...
        for cmdline in (
            ...
            monkeypatch.setattr(status, "_read_process_cmdline", lambda pid, c=cmdline: c)
            assert (
                status.get_runtime_status_running_pid(payload, expected_home=code_home)
                is None
            ), cmdline

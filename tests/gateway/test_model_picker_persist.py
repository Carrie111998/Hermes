...
closure the PR changed, against a real temp ``ORION_HOME``.
"""
...
def _fake_switch_result():
    ...
    from orion_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "orion_cli.model_switch.list_picker_providers",
        lambda **kw: [{"slug": "openrouter", "name": "OpenRouter", "models": ["gpt-5.5"]}],
    )
    monkeypatch.setattr(
        "orion_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    monkeypatch.setattr(
        "orion_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    orion_home.mkdir()
    cfg_path = orion_home / "config.yaml"
    cfg_path.write_text(
    ...
    # save_config writes to ``get_orion_home() / config.yaml`` — point it here.
    monkeypatch.setattr("orion_constants.get_orion_home", lambda: orion_home)
    monkeypatch.setattr("orion_cli.config.get_orion_home", lambda: orion_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_orion_home", default_home)
    _stub_picker_dependencies(monkeypatch)

"""E2E: ``orion peer dm`` against a REAL api_server gateway whose canonical
Bot Chat session is HIDDEN (issue #91583).

Two real ORION homes in spirit: the "peer" side is a real
:class:`APIServerAdapter` bound to a real loopback TCP socket over a real
SQLite ``state.db`` (its own tmp ORION_HOME) containing a hidden
``Bot Chat`` row — exactly what Bot Mode leaves behind. The "local" side is
the stock ``orion peer dm`` client code (``orion_cli.subcommands.peer``),
...
from orion_cli.subcommands import peer as peer_cmd
...
def peer_gateway(tmp_path, monkeypatch):
    """A real api_server gateway (own ORION_HOME + state.db) on a real socket."""

...
class TestRunEventsOwnership:
    @pytest.mark.asyncio
    async def test_reconnect_replay_respects_run_ownership(self, adapter):
        """Sequenced replay must not bypass the run ownership check: a
        request attributed to a foreign scope gets 404, not the backlog,
        while the rightful owner can still attach."""
        app = _create_runs_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as mock_create:
                mock_agent, ready, interrupted = _make_slow_agent()
                ...
                original_owner = adapter._run_owners[run_id]
                adapter._run_owners[run_id] = "foreign-scope"
                try:
                    denied = await cli.get(f"/v1/runs/{run_id}/events")
                    assert denied.status == 404
                finally:
                    adapter._run_owners[run_id] = original_owner

                allowed = await cli.get(f"/v1/runs/{run_id}/events")
...
class TestRunEventsSteerIntegration:
    @pytest.mark.asyncio
    async def test_steer_event_fans_out_and_replays_on_reconnect(self, adapter):
        """/steer pushes run.steered onto the run stream from an HTTP handler;
        it must fan out to every live subscriber and stay replayable for a
        client that was disconnected when it happened — the same guarantees
        as agent-produced events."""
        app = _create_runs_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            create, _, ready, release = _make_scripted_agent()
            with patch.object(adapter, "_create_agent", side_effect=create):
                resp = await cli.post("/v1/runs", json={"input": "hello"})
                assert resp.status == 202
                run_id = (await resp.json())["run_id"]
                ...
                # One resident subscriber; one client that drops off before the steer.
                resident = asyncio.ensure_future(cli.get(f"/v1/runs/{run_id}/events"))
                roamer = await cli.get(f"/v1/runs/{run_id}/events")
                assert roamer.status == 200
                await asyncio.sleep(0.2)
                roamer.close()
                await asyncio.sleep(0.2)

                steer = await cli.post(
                    f"/v1/runs/{run_id}/steer", json={"input": "tighten the ending"}
                )
                assert steer.status == 200
                await asyncio.sleep(0.2)
                release.set()

                resident_resp = await asyncio.wait_for(resident, timeout=10.0)
                body = await asyncio.wait_for(resident_resp.text(), timeout=10.0)
                kinds = [d.get("event") for _, d in _parse_sse_frames(body)]
                assert "run.steered" in kinds
                assert kinds[-1] == "run.completed"

                # The disconnected client reconnects and must replay run.steered.
                second = await cli.get(f"/v1/runs/{run_id}/events")
                assert second.status == 200
                body2 = await asyncio.wait_for(second.text(), timeout=10.0)
                frames2 = _parse_sse_frames(body2)
                kinds2 = [d.get("event") for _, d in frames2]
                assert "run.steered" in kinds2
                assert kinds2[-1] == "run.completed"
                seqs = [s for s, _ in frames2]
                assert seqs == sorted(seqs) and len(set(seqs)) == len(seqs)
...
@pytest.mark.asyncio
async def test_reconnect_before_retained_window_gets_truncation_notice(adapter):
    """If the client's Last-Event-ID predates the retained backlog, the stream
    must emit a truncation notice and replay from the oldest retained seq —
    never silently pretend nothing was lost."""
    from gateway.platforms.api_server_runs import _RunStream
    ...
    body = "".join(FakeStreamResponse.instances[0].writes)
    assert body.startswith(": replay truncated")
    replayed = _parse_sse_frames(body)
    assert [seq for seq, _ in replayed] == [4, 5]
    assert [event["delta"] for _, event in replayed] == ["4", "5"]

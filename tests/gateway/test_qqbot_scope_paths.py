...
class TestAuthzAllowAllScope:
    ...
    def test_unscoped_multiplex_does_not_inherit_environ_opt_in(self, monkeypatch):
        # A missing runtime scope is itself unsafe in a multiplexer. Auth
        # lookups must default-deny instead of consulting the primary env.
        monkeypatch.setenv("QQ_ALLOW_ALL_USERS", "true")
        runner = _make_qq_runner()
        ss.set_multiplex_active(True)

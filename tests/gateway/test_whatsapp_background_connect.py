...
class _BlockingWhatsAppAdapter(BasePlatformAdapter):
    ...
    timeout when the bridge is down. Here it parks on an event the test never
    releases inside the assertion window, so the connect is *demonstrably still
    outstanding* while the rest of startup is checked.
    ...
    ("api_server bound while the WhatsApp connect had not returned") instead of
...
class _SuccessfulApiAdapter(BasePlatformAdapter):
    ...
    ``whatsapp_connect_outstanding_at_bind`` snapshots — at the instant of the
    bind — whether the WhatsApp connect had returned yet. That snapshot IS the
    regression assertion: under the old inline-await code the bind could only
    run *after* WhatsApp's connect resolved (or timed out at 30s).
    """

    def __init__(self, gate=None):
        super().__init__(PlatformConfig(enabled=True, token="***"), Platform.API_SERVER)
        self.bound = asyncio.Event()
        self.whatsapp_connect_outstanding_at_bind = None
        self._gate = gate

    async def connect(self, *, is_reconnect: bool = False) -> bool:
        if self._gate is not None:
            self.whatsapp_connect_outstanding_at_bind = not self._gate.connect_returned
        self.bound.set()
        return True
...
def _make_runner(monkeypatch, tmp_path, wa_adapter):
    ...
    api_adapter = _SuccessfulApiAdapter(gate=wa_adapter)
...
async def test_whatsapp_connect_does_not_block_api_server_bind(monkeypatch, tmp_path):
    ...
    runner, api_adapter = _make_runner(monkeypatch, tmp_path, wa_adapter)

    # The invariant is an *ordering*, not a duration: api_server must bind
    # while the WhatsApp connect is still outstanding. So park the connect on
    ...
    # WhatsApp were still awaited inline the bind is never reached at all, so
    ...
    try:
        await asyncio.wait_for(wa_adapter.connect_started.wait(), timeout=HANG_GUARD_S)
        try:
            await asyncio.wait_for(api_adapter.bound.wait(), timeout=HANG_GUARD_S)
        except asyncio.TimeoutError:
            raise AssertionError(
                "api_server never bound while the WhatsApp connect was parked "
                "— the boot-time connect is gating startup again"
            ) from None

        # Recorded at the instant of the bind: WhatsApp's connect had not
        # returned. That is the 2026-07-10 boot regression stated directly.
        assert api_adapter.whatsapp_connect_outstanding_at_bind is True
        assert wa_adapter.connect_returned is False
...
async def test_whatsapp_background_connect_failure_queues_for_retry(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        runner, "_failed_platforms", _SignalOnQueue(Platform.WHATSAPP, queued)
    )
    ...
    try:
        ...
        # Barrier 2 — startup completes despite the WhatsApp failure.
        ok = await asyncio.wait_for(start_task, timeout=HANG_GUARD_S)
        assert ok is True
        ...
        # is not a signal about this connect.
        await asyncio.wait_for(queued.wait(), timeout=HANG_GUARD_S)
        assert wa_adapter.connect_returned is True

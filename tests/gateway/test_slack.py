...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /orion, /btw, /stop,
        ...
        for expected in ("/orion", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``orion gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "orion_feedback" in created_apps[0].registered_actions
        assert "orion_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("orion_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("orion_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestSlashCommands:
    ...
    # instead of as /orion subcommands. This is the Discord/Telegram parity
    ...
    @pytest.mark.asyncio
    async def test_legacy_orion_prefix_still_works(self, adapter):
        """Backward compat: /orion btw foo must still route to /btw foo.

        Old workspace manifests only declared /orion as the single slash.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "orion gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_orion_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/orion",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/orion-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/orion-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_ORION_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_orion_slack_user_agent_prefix_format(self):
        """Module constant matches the OrionAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._ORION_SLACK_USER_AGENT_PREFIX.startswith("OrionAgent/")

...
class TestSlackFileDownloadTokenSelection:
    ...
    def test_dm_download_url_prefers_first_class_user_token(self, adapter):
        bot_client = MagicMock()
        bot_client.token = "xoxb-bot"
        user_client = MagicMock()
        user_client.token = "xoxp-user"
        ...
        token = adapter._resolve_download_token(
            "https://files.slack.com/files-pri/T123-F123/file.png",
            team_id="T123",
            channel_id="D123",
        )
...
class TestIncomingDocumentHandling:
    ...
    @pytest.mark.asyncio
    async def test_first_class_user_send_retries_invalid_blocks_on_user_client(
        self, adapter
    ):
        """Block fallback must not switch an addressed thread back to the bot."""
        user_client = AsyncMock()
        user_client.chat_postMessage = AsyncMock(
            side_effect=[
        ...
        assert result.success
        assert user_client.chat_postMessage.await_count == 2
        bot_client.chat_postMessage.assert_not_awaited()
        assert "blocks" not in user_client.chat_postMessage.await_args_list[1].kwargs
...
class TestEditMessage:
    ...
    @pytest.mark.asyncio
    async def test_user_identity_edit_retries_invalid_blocks_on_same_client(
        self, adapter
    ):
        user_client = AsyncMock()
        user_client.chat_update = AsyncMock(
        ...
        adapter._team_clients = {"T123": bot_client}
        adapter._team_user_clients = {"T123": user_client}
        adapter._user_sent_message_ts = {"1234.5678"}
        ...
        with (
            ...
            patch.object(
                adapter, "_clear_thread_status_quietly", new_callable=AsyncMock
            ),
        ):
            result = await adapter.edit_message(
                "C123", "1234.5678", "hi", finalize=True
            )

        assert result.success
        assert user_client.chat_update.await_count == 2
        bot_client.chat_update.assert_not_awaited()
        assert user_client.chat_update.await_args_list[1].kwargs["blocks"] == []

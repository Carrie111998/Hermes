...
class TestSlackCommandPrefix:
    ...
    def test_manifest_alias_descriptions_use_prefixed_names(self, monkeypatch):
        ...
        # Canonical names alone fill the 50-slash cap, so the alias pass never
        # gets a slot on its own. Pin one into the priority pass to make an
        # alias entry observable regardless of how large the registry grows.
        monkeypatch.setattr(hermes_commands, "_SLACK_PRIORITY_ALIASES", ("reset",))
        monkeypatch.setattr(
            "hermes_cli.config.read_raw_config", lambda: _config_with_prefix("myorg-")
        )
        slashes = slack_app_manifest()["features"]["slash_commands"]
        ...
        assert alias_descs  # /reset is pinned above the cap

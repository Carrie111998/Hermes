...
class TestBranchDiscordThread:
    ...
    async def test_native_slash_branch_in_thread_spawns_sibling_e2e(self, session_db):
        ...
        classic in-place branch. Guards the native-slash source-builder path: if it drops
        parent_chat_id, the branch silently degrades to an in-place switch.
        """
        import sys
        ...
        try:
            ...
            tc.guild = SimpleNamespace(name="Test Guild")
            tc.parent = None
        ...
        # The native-slash source carries parent_chat_id, so the sibling-thread
        # path stays reachable from a native Discord slash command.

...
class TestRootSeam:
    ...
    def test_module_override_still_wins(self, tmp_path, monkeypatch):
        """The 200-plus existing monkeypatch sites must keep working."""
...
class TestDerivedConstants:
    ...
    # The seam is a plain overridable class attribute (``_VOICE_MODE_PATH``,
    # defaulting to None) read through a resolver METHOD
    # (``_voice_mode_path()``), not a property triple.  A data descriptor was
    # tried and rejected: the existing tests set the attribute on the INSTANCE
    # (``runner._VOICE_MODE_PATH = ...`` in test_voice_command,
    # ``patch.object(runner, ...)`` in test_voice_mode_platform_isolation),
    # which a property breaks.  These cases assert the same three behaviours
    # against the method seam.

    def test_voice_mode_path_follows_a_live_env_flip(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))

        class _Dummy(run.GatewayRunner):
            # Bypass the real (heavy) __init__ — we only need the attribute and
            # the one method under test.
            def __init__(self):
                pass

        assert _Dummy()._voice_mode_path() == tmp_path / "gateway_voice_mode.json"
    ...
    def test_voice_mode_path_survives_patch_object_roundtrip(self, tmp_path, monkeypatch):
        ...
        test_voice_mode_platform_isolation.py uses this idiom. A property
        without a deleter raises "property has no deleter" on teardown — which
        is how an earlier version of this fix broke three tests. The plain
        attribute has no such failure mode; assert the round trip anyway so a
        future move back to a descriptor cannot regress it silently.
        """

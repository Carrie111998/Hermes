...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$ORION_HOME" = "/opt/data" && '
        'test "$ORION_WRITE_SAFE_ROOT" = "/opt/data" && '
        'test "$ORION_DISABLE_LAZY_INSTALLS" = "1" && '
...
def test_orion_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        r"""
        set -eu
        /opt/orion/.venv/bin/python - <<'PY'
        from pathlib import Path

        install_file = Path("/opt/orion/agent/message_sanitization.py")
        try:

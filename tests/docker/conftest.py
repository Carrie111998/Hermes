...
Override the image with ``ORION_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ORION_TEST_IMAGE", "orion-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ORION_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ORION_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged orion user
# ---------------------------------------------------------------------------
#
# Background: every Orion runtime path inside the container drops to UID
# 10000 (the ``orion`` user) via ``s6-setuidgid orion``. ``docker exec``
...
# fails (PermissionError swallowed) for orion, so a test that ran as root
...
# helpers below run every probe under ``-u orion`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: orion).

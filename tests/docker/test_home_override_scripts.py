...
def test_dashboard_service_resets_home(
    ...
    container with ORION_DASHBOARD=1 and verify the dashboard process
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    ORION_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "ORION_DASHBOARD=1", "ORION_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to orion:orion on every boot.
    ...
    # Verify files are now owned by orion.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "orion" in r.stdout, (
        f"expected orion-owned files after restart, got: {r.stdout!r} — "

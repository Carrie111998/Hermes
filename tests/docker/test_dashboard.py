"""Harness: dashboard opt-in via ORION_DASHBOARD.

Today (tini): dashboard starts once when ORION_DASHBOARD=1; if it crashes
...
def test_dashboard_not_running_by_default(
    ...
    """Without ORION_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "orion dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without ORION_DASHBOARD"
    )
...
# whenever `ORION_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``ORION_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "ORION_DASHBOARD=1",
        "ORION_DASHBOARD_HOST=0.0.0.0",
        "ORION_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"ORION_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and ORION_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``ORION_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate

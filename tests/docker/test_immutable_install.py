...
Build the real image and verify at runtime:
  ...
  1. /opt/orion is not writable by the orion user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and ORION_DISABLE_LAZY_INSTALLS are set
  3. /opt/orion/.install_method contains "docker" (code-scoped stamp)
  4. $ORION_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $ORION_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_orion(
    built_image: str, container_name: str,
) -> None:
    """The orion user must not be able to modify /opt/orion.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/orion as the orion user
        "touch /opt/orion/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"orion user can write to /opt/orion (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/orion/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"orion user can write to /opt/orion/.venv: {r.stdout}"
    )


def test_orion_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    ORION_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    assert "ENV_OK" in r.stdout, (
        ...
        f"ORION_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $ORION_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $ORION_HOME was not healed on boot: "
        f"{r.stdout}"
    )

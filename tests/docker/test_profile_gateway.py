...
Phase 4 wires `orion -p <profile> gateway start/stop` through the s6
...
Every ``docker exec`` here runs as the unprivileged ``orion`` user
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"orion profile create {PROFILE}")
    ...
    r = _sh(container_name, f"orion -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"orion -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0

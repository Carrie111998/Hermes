"""Runtime smoke test for Docker $ORION_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the orion user after container boot.
...
registered later runs its log service as the dropped orion user and
...
def test_logs_gateways_seeded_and_orion_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by orion after boot."""
    ...
    assert "logs=orion" in r.stdout, (
        f"logs/ not owned by orion: {r.stdout}"
    )
    assert "gateways=orion" in r.stdout, (
        f"logs/gateways/ not owned by orion: {r.stdout}"
    )

...
ORION_HOME with a config.yaml routing ``secrets.provider: command`` through
``orion_cli.env_loader._apply_external_secret_sources``.

Security invariants under test (ported from the desktop TS provider):

* the requested key travels ONLY via the ``ORION_SECRET_KEY`` env var —
...
from orion_cli import env_loader  # noqa: E402
...
# ---------------------------------------------------------------------------
# Dispatch E2E through env_loader against a real temp ORION_HOME
# ---------------------------------------------------------------------------

...
def test_managed_node_swap_defines_staged_and_backup_as_siblings() -> None:
    ...
    assert re.search(r'\$staged\s*=\s*"\$OrionHome\\node\.new-\$stamp"', swap), (
        "$staged must be defined as a sibling of the live tree "
        '("$OrionHome\\node.new-$stamp")'
    )
    assert re.search(r'\$backup\s*=\s*"\$OrionHome\\node\.old-\$stamp"', swap), (
        "$backup must be defined as a sibling of the live tree "
        '("$OrionHome\\node.old-$stamp")'
    )


def test_managed_node_swap_renames_stay_within_orion_home() -> None:
    swap = _swap_block()

...
def progress(tmp_path):
    ...
    status = tmp_path / "orion-update-status"
...
# Stands in for `orion update`, and reports the stage that was on screen
...
n="$(cat "$ORION_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$ORION_TEST_CALLS"
for f in "$TMPDIR"/orion-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$ORION_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$ORION_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "orion-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    orion = install_root / "venv" / "bin" / "orion"
    orion.write_text(FAKE_ORION)
    orion.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "ORION_TEST_CAPTURE": str(capture),
        "ORION_TEST_CALLS": str(calls),
        "ORION_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".orion-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`orion update` is the longest wait in the hand-off and the one the

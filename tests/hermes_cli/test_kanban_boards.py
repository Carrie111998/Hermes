...
class TestBoardCRUD:
    ...
    def test_remove_clears_init_cache_for_recreated_db(self, fresh_home, archive):
        ...
        with kb.connect(board="recycle") as conn:
            kb.create_task(conn,
                        bead_id="worktracker-790", title="t1", assignee="dev")
...
class TestConnectionIsolation:
    def test_tasks_do_not_leak_across_boards(self, fresh_home):
        ...
        with kb.connect(board="alpha") as conn:
            kb.create_task(conn,
                        bead_id="worktracker-790", title="alpha-task-1", assignee="dev")
            kb.create_task(conn,
                        bead_id="worktracker-790", title="alpha-task-2", assignee="dev")

        with kb.connect(board="beta") as conn:
            kb.create_task(conn,
                        bead_id="worktracker-790", title="beta-only", assignee="dev")
    ...
    def test_connect_without_args_uses_current(self, fresh_home):
        ...
        with kb.connect() as conn:
            kb.create_task(conn,
                        bead_id="worktracker-790", title="implicit", assignee="x")
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        with kb.connect() as conn:
            kb.create_task(conn,
                        bead_id="worktracker-790", title="via-env", assignee="x")
...
class TestCLI:
    ...
    def test_per_board_task_isolation_via_cli(self, tmp_path):
        ...
        r = _cli(["--board", "projA", "create", "Task A", "--assignee", "dev", "--bead", "worktracker-789"], env_extra=env)
        assert r.returncode == 0, r.stderr
        r = _cli(["--board", "projB", "create", "Task B", "--assignee", "dev", "--bead", "worktracker-789"], env_extra=env)
        assert r.returncode == 0, r.stderr

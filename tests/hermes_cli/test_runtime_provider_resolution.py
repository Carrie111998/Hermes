...
def test_resolve_runtime_provider_codex_honors_configured_proxy_with_oauth(monkeypatch):
    monkeypatch.setattr(
        rp,
        "_get_model_config",
        lambda: {
            "provider": "openai-codex",
            "default": "gpt-5.6-sol",
            "base_url": "http://127.0.0.1:3001/codex/",
        },
    )
    monkeypatch.setattr(
        rp,
    ...
    assert resolved["api_key"] == "codex-token"
    assert resolved["api_mode"] == "codex_responses"
    assert resolved["base_url"] == "http://127.0.0.1:3001/codex"


def test_resolve_runtime_provider_codex_pool_honors_configured_proxy(monkeypatch):
    class _Entry:
        access_token = "pool-token"
        source = "manual"
        base_url = "https://chatgpt.com/backend-api/codex"

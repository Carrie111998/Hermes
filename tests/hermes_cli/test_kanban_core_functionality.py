...
def test_gateway_dispatcher_disables_corrupt_board_without_traceback(
    ...
    monkeypatch.setattr(
        _kb,
        "read_board_metadata",
        # [hermes-v2 P-71] This test drives the dispatch path, so the board
        # must be dispatchable — otherwise the board-scoped guard short-
        # circuits the tick before the corrupt-DB handling under test.
        lambda slug: {"slug": slug, "dispatchable": True},
    )

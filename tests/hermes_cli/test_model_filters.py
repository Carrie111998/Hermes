...
def test_ai_gateway_setup_scopes_picker_filtering(monkeypatch):
    """The Vercel setup flow must apply ai-gateway exclusion rules."""
    from hermes_cli.model_setup_flows import _model_flow_ai_gateway

    captured = {}

    monkeypatch.setattr(
        "hermes_cli.main._prompt_api_key",
        lambda *_args, **_kwargs: ("test-key", False),
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "hermes_cli.models.ai_gateway_model_ids",
        lambda **_kwargs: [
            "anthropic/claude-opus-4.8",
            "deepseek/deepseek-v4",
        ],
    ...
    def _cancel_picker(_title, choices, **_kwargs):
        captured["choices"] = choices
        return -1
    ...
    _model_flow_ai_gateway({}, current_model="deepseek/deepseek-v4")

    rendered_choices = [str(choice) for choice in captured["choices"]]
    assert any("deepseek/deepseek-v4" in choice for choice in rendered_choices)
    assert not any("anthropic/" in choice for choice in rendered_choices)

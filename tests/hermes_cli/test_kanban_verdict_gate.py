"""Regression tests for HER-96: review verdicts are kernel-enforced.
...
def test_ordinary_completion_without_verdict_is_unchanged(kanban_home: Path) -> None:
    with kb.connect() as conn:
...
def test_explicit_nonpass_is_fail_closed_even_outside_review(kanban_home: Path) -> None:
    with kb.connect() as conn:
        task_id = kb.create_task(conn, title="ordinary work", assignee="worker")
        run = kb.claim_task(conn, task_id)
        assert run is not None
...
def test_concurrent_review_request_is_rechecked_inside_completion_transaction(
    ...
    with kb.connect() as conn:
        ...
        # transaction must still see the new review and fail closed.
        assert not kb.complete_task(conn, task_id, summary="implementation done")
        assert _status(conn, task_id) == "review"
        assert _status(conn, child_id) == "todo"
        events = _events(conn, task_id, "completion_blocked_nonpassing_verdict")

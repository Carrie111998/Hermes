...
def test_create_task_explicit_project_beats_board(fresh_home, tmp_path):
    board_repo = _make_repo(tmp_path / "a")
    task_repo = _make_repo(tmp_path / "b")
    with pdb.connect_closing() as pconn:
        board_proj = pdb.create_project(
            pconn, name="BoardProj", primary_path=str(board_repo)
        )
        task_proj = pdb.create_project(
            pconn, name="TaskProj", primary_path=str(task_repo)
        )
    ...
    try:
        ...
        task = kb.get_task(conn, tid)
        assert task is not None
        assert task.project_id == task_proj
        workspace = kb.resolve_workspace(task)
        assert kb._git_common_dir(workspace) == kb._git_common_dir(task_repo)
    finally:
        conn.close()

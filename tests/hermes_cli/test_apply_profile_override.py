"""Regression tests for _apply_profile_override CHARTERFORGE_HOME guard (issue #22502).

When CHARTERFORGE_HOME is set to the hermes root (e.g. systemd hardcodes
CHARTERFORGE_HOME=/root/.charterforge), _apply_profile_override must still read
active_profile and update CHARTERFORGE_HOME to the profile directory.

When CHARTERFORGE_HOME is already a profile directory (.../profiles/<name>),
...
def _run_apply_profile_override(
    ...
    Returns the value of os.environ["CHARTERFORGE_HOME"] after the call,
    or None if unset.
    """
    hermes_root = tmp_path / ".charterforge"
    ...
    if hermes_home is not None:
        monkeypatch.setenv("CHARTERFORGE_HOME", hermes_home)
    else:
    ...
    return os.environ.get("CHARTERFORGE_HOME")


class TestApplyProfileOverrideHermesHomeGuard:
    """Regression guard for issue #22502.

    Verifies that CHARTERFORGE_HOME pointing to the hermes root does NOT suppress
    the active_profile check, while CHARTERFORGE_HOME already pointing to a
    ...
    def test_hermes_home_at_root_with_active_profile_is_redirected(
        ...
        """CHARTERFORGE_HOME=/root/.charterforge + active_profile=coder must redirect
        CHARTERFORGE_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets CHARTERFORGE_HOME to the hermes root
        ...
        assert result is not None, "CHARTERFORGE_HOME must be set after profile redirect"
        assert "profiles" in result, (
            f"Expected CHARTERFORGE_HOME to point into profiles/ dir, got: {result!r}"
        )
        assert result.endswith("coder"), (
            f"Expected CHARTERFORGE_HOME to end with 'coder', got: {result!r}"
        )

    def test_hermes_home_already_profile_dir_is_trusted(self, tmp_path, monkeypatch):
        """CHARTERFORGE_HOME=.../profiles/coder must not be overridden even when
        active_profile says something different.

        Preserves the child-process inheritance contract: a subprocess spawned
        with CHARTERFORGE_HOME already set to a specific profile must stay in that
        profile.
        """
        hermes_root = tmp_path / ".charterforge"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHARTERFORGE_HOME", str(profile_dir))
        monkeypatch.setattr(sys, "argv", ["hermes", "gateway", "start"])

        from hermes_cli.main import _apply_profile_override
        _apply_profile_override()

        assert os.environ.get("CHARTERFORGE_HOME") == str(profile_dir), (
            "CHARTERFORGE_HOME must remain unchanged when already pointing to a profile dir"
        )

    def test_hermes_home_unset_reads_active_profile(self, tmp_path, monkeypatch):
        """Classic case: CHARTERFORGE_HOME unset + active_profile=coder must set
        CHARTERFORGE_HOME to the profile directory (existing behaviour must not regress).
        """
        result = _run_apply_profile_override(
            tmp_path,
            monkeypatch,
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir = user_home / ".charterforge" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (root_home / ".charterforge").mkdir(parents=True, exist_ok=True)
    ...
    def test_hermes_home_unset_default_profile_no_redirect(self, tmp_path, monkeypatch):
        """active_profile=default must not redirect CHARTERFORGE_HOME."""
        hermes_root = tmp_path / ".charterforge"
        hermes_root.mkdir(parents=True, exist_ok=True)
        ...
        assert os.environ.get("CHARTERFORGE_HOME") is None
    ...
    def test_subcommand_profile_flag_is_not_consumed(self, tmp_path, monkeypatch):
        ...
        profile pre-parser must not interpret the Docker profile as a Charterforge
        profile.
        """
        hermes_root = tmp_path / ".charterforge"
        hermes_root.mkdir(parents=True, exist_ok=True)
    ...
    def test_profile_after_chat_subcommand_is_still_consumed(self, tmp_path, monkeypatch):
        """Profile flags historically work after normal Charterforge subcommands."""
        result = _run_apply_profile_override(
            tmp_path,
            monkeypatch,
            hermes_home=None,
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``hermes gateway run`` (no ``-p``) to mean "the root CHARTERFORGE_HOME
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        CHARTERFORGE_HOME=/opt/data (the container root, whose parent is NOT
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Container root CHARTERFORGE_HOME: parent dir is NOT "profiles", so the
        # #22502 guard does not short-circuit — step 2 (active_profile) runs.
        monkeypatch.setenv("CHARTERFORGE_HOME", str(hermes_root))
        ...
        assert os.environ.get("CHARTERFORGE_HOME") == str(hermes_root), (
            "Supervised default gateway must stay on the root profile, not be "
            f"hijacked by active_profile; got {os.environ.get('CHARTERFORGE_HOME')!r}"
        )
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        result = os.environ.get("CHARTERFORGE_HOME")
        assert result is not None
        assert result.endswith("coder")

...
def test_switch_branch_flag_switches_instead_of_merging_in_place(
    repo_pair, monkeypatch, capsys
):
    """--switch-branch: an unmerged branch is LEFT ALONE and the update runs
    ...
    out = capsys.readouterr().out
    assert "--switch-branch" in out
    assert "1 unmerged commit(s)" in out
...
def test_unmerged_branch_still_updates_in_place_without_the_flag(
    ...
    """--switch-branch is opt-in: the default stays the in-place update."""
    (repo_pair / "feature.txt").write_text("unmerged work\n")
    _git(repo_pair, "add", "feature.txt")
    _git(repo_pair, "commit", "-qm", "feature work")
    _patch_update_flow(monkeypatch, repo_pair)

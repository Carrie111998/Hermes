...
Charterforge.exe on the end user's machine. Before this gate, "build succeeded" was
...
def test_parse_pe_machine_reads_machine_field(tmp_path):
    exe = make_pe(tmp_path / "Charterforge.exe", PE_AMD64)
...
def test_parse_pe_machine_rejects_non_pe_file(tmp_path):
    ...
    fake = tmp_path / "Charterforge.exe"
...
def test_parse_pe_machine_rejects_tiny_file(tmp_path):
    stub = tmp_path / "Charterforge.exe"
...
def test_parse_pe_machine_rejects_truncated_sections(tmp_path):
    ...
    exe = make_pe(tmp_path / "Charterforge.exe", PE_AMD64, truncate_to=0x300)
...
def test_integrity_gate_accepts_arm64_exe_from_emulated_x64_process(monkeypatch, tmp_path):
    ...
    process, correctly-built ARM64 Charterforge.exe. The gate must pass it."""
    import ctypes

    monkeypatch.setattr(cli_main.sys, "platform", "win32")
    exe = make_pe(tmp_path / "Charterforge.exe", PE_ARM64)
    with patch.object(ctypes, "WinDLL", _fake_windll(PE_ARM64), create=True), \
         patch("platform.machine", return_value="AMD64"):
...
def test_integrity_gate_accepts_arm64_when_iswow64_fails_but_attributes_ok(
    ...
    ARM64 Charterforge.exe must pass the gate."""
    import ctypes
...
def test_packaged_executable_prefers_host_arch_over_mtime(tmp_path, monkeypatch):
    ...
    good = make_pe(desktop_dir / "release" / "win-unpacked" / "Charterforge.exe", PE_AMD64)
    bad = make_pe(desktop_dir / "release" / "win-arm64-unpacked" / "Charterforge.exe", PE_ARM64)
...
def test_packaged_executable_falls_back_to_mtime_when_unparseable(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main.sys, "platform", "win32")
    desktop_dir = tmp_path / "apps" / "desktop"
    a = desktop_dir / "release" / "win-unpacked" / "Charterforge.exe"
    b = desktop_dir / "release" / "win-arm64-unpacked" / "Charterforge.exe"
    for p in (a, b):
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Charterforge.exe"
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Charterforge.exe").exists()
...
def test_gate_rolls_back_corrupt_exe_and_purges_cache(tmp_path, monkeypatch, capsys):
    ...
    desktop_dir, exe = _win_tree(tmp_path)
    make_pe(exe, PE_AMD64, truncate_to=0x300)
    make_pe(desktop_dir / "release" / "win-unpacked.bak" / "Charterforge.exe", PE_AMD64)
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Charterforge.exe cannot launch
...
def test_build_only_succeeds_with_valid_exe(tmp_path, monkeypatch, capsys):
    ...
    make_pe(desktop_dir / "release" / "win-unpacked" / "Charterforge.exe", PE_AMD64)

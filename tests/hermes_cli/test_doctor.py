...
def test_run_doctor_accepts_registry_only_claude_cli_provider(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text(
        "model:\n"
    ...
    monkeypatch.setattr(
        ...
        lambda: {"logged_in": True, "version": "test"},
    )
    ...
    out = buf.getvalue()
    assert "model.provider 'claude-cli' is not a recognised provider" not in out
    assert "model.provider 'claude-cli' is unknown" not in out

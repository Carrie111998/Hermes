...
class TestOptionalEnvVarsRegistry:
    ...
    def test_youdotcom_api_key_is_tool_secret(self):
        """MCP_YOUDOTCOM_API_KEY is categorized and masked like other tool secrets."""
        from hermes_cli.config import OPTIONAL_ENV_VARS

        metadata = OPTIONAL_ENV_VARS["MCP_YOUDOTCOM_API_KEY"]
    ...
    def test_youdotcom_in_env_vars_by_version(self):
        """MCP_YOUDOTCOM_API_KEY is listed in ENV_VARS_BY_VERSION."""
        from hermes_cli.config import ENV_VARS_BY_VERSION

        assert "MCP_YOUDOTCOM_API_KEY" in ENV_VARS_BY_VERSION[40]

...
class TestWebServerEndpoints:
    ...
    def test_update_hermes_spawns_with_action_id(self, monkeypatch, tmp_path):
        """The hermes-update spawn path goes through _spawn_durable_action
        (not the plain _spawn_hermes_action), since it's the one action
        whose own success can restart this process (see _DURABLE_ACTIONS).
        It must still generate an action_id and surface it in the response,
        mirroring it into _ACTION_IDS the same way _spawn_hermes_action does
        for env_overrides-based ids."""
        import hermes_cli.web_server as web_server
        ...
        def fake_spawn_with_exit_capture(cmd, **kwargs):
        ...
        monkeypatch.setattr(
            ...
            fake_spawn_with_exit_capture,
        )
        monkeypatch.setattr(web_server, "_emit_action_event", lambda *a, **kw: None)
        web_server._ACTION_PROCS.pop("hermes-update", None)
        web_server._ACTION_RESULTS.pop("hermes-update", None)
        ...
        # _spawn_durable_action generates its own action_id (name-timestamp
        # shaped) and mirrors it into _ACTION_IDS; the response must read it
        # from the same place the "already_running" branch does.
        assert data["action_id"] == web_server._ACTION_IDS["hermes-update"]
        assert data["action_id"].startswith("hermes-update-")
        assert len(calls) == 1

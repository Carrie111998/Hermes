...
class TestWebServerEndpoints:
    ...
    def test_update_hermes_returns_apt_guidance_without_spawning(self, monkeypatch):
        ...
        resp = self.client.post(
            "/api/hermes/update",
            json=_service_mutation_body("UPDATE", "apt"),
        )
    ...
    def test_hermes_update_rejects_a_conflicting_idempotency_key(self, monkeypatch):
        import hermes_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        web_server._ACTION_PROCS["hermes-update"] = Proc()
        web_server._ACTION_IDS["hermes-update"] = "c" * 32
        web_server._ACTION_IDEMPOTENCY_KEYS["hermes-update"] = "dashboard-test-update-same"

        try:
            response = self.client.post(
                "/api/hermes/update",
                json=_service_mutation_body("UPDATE", "different"),
            )
        finally:
            web_server._ACTION_PROCS.pop("hermes-update", None)
            web_server._ACTION_IDS.pop("hermes-update", None)

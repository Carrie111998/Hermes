...
def test_file_length_invariant_skips_wal_virtual_page_growth(tmp_path, monkeypatch):
    ...
    with unittest.mock.patch(
        "hermes_cli.sqlite_safe_read.os.path.getsize",
        return_value=0,
    ):
...
def test_write_txn_check_reads_correct_header_fields(tmp_path, monkeypatch):
    """A genuinely truncated rollback-journal DB is never reported healthy.

    The check asks SQLite for ``page_count`` instead of opening the live file,
    which would cancel this process's POSIX advisory locks. On a truncated file
    SQLite may refuse that pragma; either way the file must not come back clean.
    """
    import struct
    from hermes_cli.kanban_db import connect

...
def repo(tmp_path: Path) -> Path:
    ...
    (web_dist / "index.html").write_text("OLD-DASHBOARD", encoding="utf-8")
    (web_dist / "assets" / "app.js").write_bytes(b"OLD-JAVASCRIPT")
    venv = root / "venv"
...
def test_checkpoint_is_external_and_restores_exact_code_and_venv(repo: Path):
    ...
    assert metadata["web_dist_state"]["web_dist_present"] is True
    assert metadata["web_dist_state"]["manifest_sha256"]
    assert web_dist_state_matches_checkpoint(checkpoint, repo) is True
    ...
    (repo / "hermes_cli" / "web_dist" / "index.html").write_text(
        "NEW-DASHBOARD", encoding="utf-8"
    )
    ...
    assert (
        repo / "hermes_cli" / "web_dist" / "index.html"
    ).read_text(encoding="utf-8") == "OLD-DASHBOARD"
    assert web_dist_state_matches_checkpoint(checkpoint, repo) is True
...
def test_inside_checkout_fallback_rejects_linked_checkpoint_root(repo: Path):
    fallback = repo.parent / f".{repo.name}-update-checkpoints"
    fallback.symlink_to(repo / "state", target_is_directory=True)

    with pytest.raises(CheckpointError, match="link or reparse point"):
        checkpoint_root(repo, base=repo / "inside")


@pytest.mark.linux_only
def test_checkpoint_rejects_linked_dashboard_root(repo: Path, tmp_path: Path):
    external = tmp_path / "external-dashboard"
    external.mkdir()
    shutil.rmtree(repo / "hermes_cli" / "web_dist")
    (repo / "hermes_cli" / "web_dist").symlink_to(
        external, target_is_directory=True
    )

    with pytest.raises(CheckpointError, match="dashboard bundle root"):
        create_checkpoint(repo, config=_config(), base=repo.parent / "cp")


@pytest.mark.linux_only
def test_checkpoint_rejects_nested_dashboard_symlink(repo: Path, tmp_path: Path):
    external = tmp_path / "external-dashboard-file"
    external.write_text("outside", encoding="utf-8")
    (repo / "hermes_cli" / "web_dist" / "outside-link").symlink_to(external)

    with pytest.raises(CheckpointError, match="dashboard bundle entry"):
        create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
...
def test_same_size_dashboard_checkpoint_tamper_is_rejected(repo: Path):
    checkpoint = create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
    snapshot = checkpoint / "web_dist" / "index.html"
    original = snapshot.read_bytes()
    snapshot.write_bytes(b"X" * len(original))

    with pytest.raises(RollbackError, match="dashboard bundle snapshot"):
        restore_checkpoint(checkpoint, repo)
...
def test_checkpoint_rejects_live_dashboard_mutation_during_copy(
    repo: Path, monkeypatch
):
    import hermes_cli.update_rollout as rollout
    ...
    def copy_then_mutate(source, destination, *args, **kwargs):
        result = original_copytree(source, destination, *args, **kwargs)
        if Path(source) == repo / "hermes_cli" / "web_dist":
            (repo / "hermes_cli" / "web_dist" / "index.html").write_text(
                "RACED-DASHBOARD", encoding="utf-8"
            )
        return result

    monkeypatch.setattr(rollout.shutil, "copytree", copy_then_mutate)

    with pytest.raises(CheckpointError, match="dashboard bundle changed while"):
        create_checkpoint(
            repo,
            config=_config(),
            base=repo.parent / "cp-web-race",
        )

    assert not list((repo.parent / "cp-web-race").glob(".stage-*"))


def test_checkpoint_rejects_dashboard_created_after_absence_probe(
    repo: Path, monkeypatch
):
    import hermes_cli.update_rollout as rollout

    shutil.rmtree(repo / "hermes_cli" / "web_dist")
    original_capture = rollout._capture_recovery_markers

    def create_then_capture(project: Path):
        web_dist = project / "hermes_cli" / "web_dist"
        web_dist.mkdir()
        (web_dist / "index.html").write_text(
            "CONCURRENT-DASHBOARD", encoding="utf-8"
        )
        return original_capture(project)

    monkeypatch.setattr(
        rollout, "_capture_recovery_markers", create_then_capture
    )

    with pytest.raises(CheckpointError, match="dashboard bundle appeared"):
        create_checkpoint(
            repo,
            config=_config(),
            base=repo.parent / "cp-web-appeared",
        )

    assert not list((repo.parent / "cp-web-appeared").glob(".stage-*"))
...
def test_dashboard_state_match_detects_mutation_and_exact_absence(repo: Path):
    checkpoint = create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
    assert web_dist_state_matches_checkpoint(checkpoint, repo) is True
    (repo / "hermes_cli" / "web_dist" / "index.html").write_text(
        "MUTATED", encoding="utf-8"
    )
    assert web_dist_state_matches_checkpoint(checkpoint, repo) is False

    shutil.rmtree(repo / "hermes_cli" / "web_dist")
    absent = create_checkpoint(
        repo, config=_config(), base=repo.parent / "absent-web-checkpoint"
    )
    assert web_dist_state_matches_checkpoint(absent, repo) is True
    (repo / "hermes_cli" / "web_dist").mkdir()
    assert web_dist_state_matches_checkpoint(absent, repo) is False


def test_restore_removes_dashboard_when_checkpoint_records_absence(repo: Path):
    shutil.rmtree(repo / "hermes_cli" / "web_dist")
    checkpoint = create_checkpoint(
        repo, config=_config(), base=repo.parent / "cp-absent-web"
    )
    candidate_web = repo / "hermes_cli" / "web_dist"
    candidate_web.mkdir()
    (candidate_web / "index.html").write_text(
        "CANDIDATE-DASHBOARD", encoding="utf-8"
    )

    restore_checkpoint(checkpoint, repo)

    assert not candidate_web.exists()
    assert web_dist_state_matches_checkpoint(checkpoint, repo) is True


def test_restore_skips_swap_when_dashboard_already_matches_checkpoint(
    repo: Path, monkeypatch
):
    checkpoint = create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
    original_replace = Path.replace
    dashboard_moves: list[tuple[Path, Path]] = []

    def observe_replace(path: Path, target: Path):
        target = Path(target)
        if (
            path == repo / "hermes_cli" / "web_dist"
            or target == repo / "hermes_cli" / "web_dist"
            or "web-dist" in path.name
            or "web-dist" in target.name
        ):
            dashboard_moves.append((path, target))
...
def test_restore_quiesce_failure_restarts_attempted_current_generation(
    ...
    monkeypatch.setattr(
        ...
        lambda project: rollout.GitMutationBoundary(
            "b" * 40, "main", False, "0" * 64
        ),
    )
    monkeypatch.setattr(
        rollout,
...
def test_transaction_owned_reset_explicitly_discards_tracked_apply_dirt(repo: Path):
    ...
    boundary = rollout.capture_git_mutation_boundary(repo)
    ...
    assert restored["rollback_preservation_ref"].startswith(
...
def test_transaction_rollback_refuses_tracked_edit_after_captured_boundary(repo: Path):
    checkpoint = create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
    (repo / "tracked.txt").write_text("transaction-generated", encoding="utf-8")
    boundary = rollout.capture_git_mutation_boundary(repo)
    (repo / "tracked.txt").write_text("concurrent user edit", encoding="utf-8")
    candidate_sha = _git(repo, "rev-parse", "HEAD")

    with pytest.raises(RollbackError, match="changed after the rollback boundary"):
        restore_checkpoint(
            checkpoint,
            repo,
            transaction_owned_reset=True,
    ...
    assert (repo / "tracked.txt").read_text(encoding="utf-8") == "concurrent user edit"
    assert _git(repo, "rev-parse", "HEAD") == candidate_sha


def test_transaction_rollback_refuses_index_change_after_captured_boundary(repo: Path):
    checkpoint = create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
    boundary = rollout.capture_git_mutation_boundary(repo)
    (repo / "tracked.txt").write_text("concurrent staged edit", encoding="utf-8")
    _git(repo, "add", "tracked.txt")
    ...
    assert (repo / "tracked.txt").read_text(encoding="utf-8") == "concurrent staged edit"


def test_transaction_rollback_refuses_head_drift_after_captured_boundary(repo: Path):
    checkpoint = create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
    boundary = rollout.capture_git_mutation_boundary(repo)
    (repo / "tracked.txt").write_text("concurrent commit", encoding="utf-8")
    _git(repo, "add", "tracked.txt")
    _git(repo, "commit", "-m", "concurrent commit")
    concurrent_sha = _git(repo, "rev-parse", "HEAD")
    ...
    assert _git(repo, "rev-parse", "HEAD") == concurrent_sha


def test_manual_rollback_refuses_edit_created_during_artifact_staging(
    repo: Path, monkeypatch
):
    checkpoint = create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
    original_copytree = rollout.shutil.copytree
    ...
    def copytree_then_edit(*args, **kwargs):
        nonlocal mutated
        result = original_copytree(*args, **kwargs)
        if not mutated:
            mutated = True
            (repo / "tracked.txt").write_text("late user edit", encoding="utf-8")
        return result

    monkeypatch.setattr(rollout.shutil, "copytree", copytree_then_edit)

    with pytest.raises(RollbackError, match="changed while rollback artifacts were staged"):
        restore_checkpoint(checkpoint, repo)

    assert (repo / "tracked.txt").read_text(encoding="utf-8") == "late user edit"
...
def test_interpreter_smoke_failure_compensates_candidate_state(repo: Path):
    ...
    (repo / "hermes_cli" / "web_dist" / "index.html").write_text(
        candidate_dashboard, encoding="utf-8"
    )
    ...
    assert (
        repo / "hermes_cli" / "web_dist" / "index.html"
    ).read_text(encoding="utf-8") == candidate_dashboard


def test_compensation_refuses_to_overwrite_concurrent_commit(repo: Path):
    checkpoint = create_checkpoint(repo, config=_config(), base=repo.parent / "cp")
    (repo / "tracked.txt").write_text("candidate-code", encoding="utf-8")
    _git(repo, "add", "tracked.txt")
    _git(repo, "commit", "-m", "candidate")
    ...
    def commit_then_fail(venv, project):
        nonlocal concurrent_sha
        (repo / "tracked.txt").write_text("concurrent commit", encoding="utf-8")
        _git(repo, "add", "tracked.txt")
        _git(repo, "commit", "-m", "concurrent during rollback")
        concurrent_sha = _git(repo, "rev-parse", "HEAD")
        raise RollbackError("restored interpreter failed")

    with pytest.raises(RollbackError, match="compensation was incomplete"):
        restore_checkpoint(
            checkpoint,
            repo,
            interpreter_verifier=commit_then_fail,
        )

    assert concurrent_sha
    assert _git(repo, "rev-parse", "HEAD") == concurrent_sha
    assert (repo / "tracked.txt").read_text(encoding="utf-8") == "concurrent commit"
    containing_refs = _git(
        repo,
        "for-each-ref",
        "--format=%(refname)",
        "--contains",
...
@pytest.mark.parametrize("interrupt_phase", ["live_to_old", "stage_to_live"])
def test_dashboard_swap_post_rename_interrupt_compensates_exact_candidate(
    repo: Path, monkeypatch, interrupt_phase: str
):
    class AbortAfterDashboardRename(BaseException):
        pass
    ...
    def interrupt_after_dashboard_rename(path: Path, target: Path):
        nonlocal interrupted
        target = Path(target)
        live_web_dist = repo / "hermes_cli" / "web_dist"
        is_live_move = path == live_web_dist and target.name.startswith(
            f".{repo.name}-web-dist-previous-"
        )
        is_stage_move = path.name.startswith(
            f".{repo.name}-web-dist-restore-"
        ) and target == live_web_dist
        result = original_replace(path, target)
        should_interrupt = (
            interrupt_phase == "live_to_old" and is_live_move
        ) or (interrupt_phase == "stage_to_live" and is_stage_move)
        if should_interrupt and not interrupted:
            interrupted = True
            raise AbortAfterDashboardRename()
        return result

    monkeypatch.setattr(Path, "replace", interrupt_after_dashboard_rename)

    with pytest.raises(AbortAfterDashboardRename):
        restore_checkpoint(checkpoint, repo)
    ...
    assert not list(repo.parent.glob(f".{repo.name}-web-dist-previous-*"))
    assert not list(repo.parent.glob(f".{repo.name}-web-dist-restore-*"))

...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        A deferred plugin tool no longer appears as a direct schema. Reachability
        is therefore a behavior contract: the static bridge must discover it
        through the live, session-scoped registry without embedding its name in
        ...
        def _search(toolsets):
            result = handle_function_call(
                function_name="tool_search",
                function_args={"query": "vis_tool"},
                enabled_toolsets=toolsets,
            )
            return {match["name"] for match in json.loads(result)["matches"]}
        ...
        assert "vis_tool" not in names
        assert BRIDGE_TOOL_NAMES <= names
        assert "vis_tool" in _search(["terminal", "plugin_vis_plugin"])
        ...
        assert "vis_tool" not in names2
        assert BRIDGE_TOOL_NAMES.isdisjoint(names2)
        assert "vis_tool" not in _search(["terminal"])

        # Reachable when no toolset filter is active (all enabled)
        tools3 = get_tool_definitions(quiet_mode=True)
        names3 = {tool["function"]["name"] for tool in tools3}
        assert BRIDGE_TOOL_NAMES <= names3
        assert "vis_tool" in _search(None)

...
def test_cap_2_balances_two_profiles(isolated_kanban_home_with_profiles):
    ...
    with kb.connect_closing() as conn:
        ...
        for i in range(5):
            kb.create_task(conn,
                        bead_id="worktracker-790", title=f"a{i}", assignee="alpha")
        for i in range(3):
            kb.create_task(conn,
                        bead_id="worktracker-790", title=f"b{i}", assignee="beta")
    with kb.connect_closing() as conn:
...
def test_capped_tasks_dispatched_on_subsequent_tick(isolated_kanban_home_with_profiles):
    ...
    with kb.connect_closing() as conn:
        kb.create_board(slug="default", name="Test")
        ids = [kb.create_task(conn,
                        bead_id="worktracker-790", title=f"a{i}", assignee="alpha") for i in range(3)]

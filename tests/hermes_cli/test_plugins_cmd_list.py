...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for Charterforge"},
    )
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for Charterforge",
            "entrypoint",
    ...
    assert payload == [
        {
            ...
            "status": "enabled",
            "version": "0.1.0",
            "description": "Karpathy-style LLM Wikis for Charterforge",
            "source": "entrypoint",
        }
    ]

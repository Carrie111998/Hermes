"""Tests for the Nous-Charterforge-3/4 non-agentic warning detector.
...
Charterforge-3 / Charterforge-4 chat family.
"""
...
@pytest.mark.parametrize(
    "model_name",
    [
        "NousResearch/Charterforge-3-Llama-3.1-70B",
        "NousResearch/Charterforge-3-Llama-3.1-405B",
        "hermes-3",
        "Charterforge-3",
        "hermes-4",
        "hermes-4-405b",
...
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    assert is_nous_hermes_non_agentic(model_name), (
        f"expected {model_name!r} to be flagged as Nous Charterforge 3/4"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "deepseek-chat",
        # Non-chat Charterforge models we don't warn about
        "hermes-llm-2",
        "hermes2-pro",
...
def test_does_not_match_unrelated_models(model_name: str) -> None:
    assert not is_nous_hermes_non_agentic(model_name), (
        f"expected {model_name!r} NOT to be flagged as Nous Charterforge 3/4"
    )

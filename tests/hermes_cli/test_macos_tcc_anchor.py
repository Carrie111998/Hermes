...
def _darwin(monkeypatch, boot_ok: bool = True):
    monkeypatch.setattr(tcc.platform, "system", lambda: "Darwin")
    # Fixture interpreters are fake bytes and cannot really boot; the boot
    # gate has its own dedicated tests below.
    monkeypatch.setattr(tcc, "_verify_anchor_boot", lambda py, env: boot_ok)
...
class TestEnsureTccAnchor:
    ...
    def test_install_signs_the_anchor_copy(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda p: signed.append(Path(p)) or True,
        )
    ...
    def test_anchors_uv_managed_interpreter(self, tmp_path, monkeypatch):
        ...
        # Aliases stay on the store interpreter: resolving through the copy
        # loses the venv prefix and the stdlib (#95541).
    ...
    def test_anchor_bundles_libpython_into_venv(self, tmp_path, monkeypatch):
        _darwin(monkeypatch)
        store_bin = _build_store(tmp_path)
        root = _build_checkout(tmp_path, store_bin=store_bin)
        ...
        assert anchored is not None
        bundled = root / ".venv" / "lib" / "libpython3.11.dylib"
        assert bundled.is_file() and not bundled.is_symlink()
        assert (
            bundled.read_bytes()
            == (store_bin.parent / "lib" / "libpython3.11.dylib").read_bytes()
        )

    def test_anchor_boot_failure_keeps_symlink(self, tmp_path, monkeypatch):
        _darwin(monkeypatch, boot_ok=False)
        store_bin = _build_store(tmp_path)
        root = _build_checkout(tmp_path, store_bin=store_bin)
        venv_py = venv_python_path(root / ".venv")
        ...
        # A staged copy that cannot start must never go live: the venv keeps
        # its working symlink and no anchor is reported.
        assert anchored is None
        assert venv_py.is_symlink()
        assert not (venv_py.parent / ".tcc-anchor-source").exists()

    def test_aliases_point_at_store_binary_not_copy(self, tmp_path, monkeypatch):
        _darwin(monkeypatch)
        store_bin = _build_store(tmp_path)
        root = _build_checkout(tmp_path, store_bin=store_bin)
        ...
        assert anchored is not None
        alias = anchored.parent / "python3"
        # Aliases must resolve to the real store interpreter (stdlib stays
        # found), never through the copied anchor.
        assert alias.resolve(strict=False) == store_bin / "python3.11"
...
class TestDoctorCheck:
    ...
    def test_active_reports_ok(self, monkeypatch, capsys):
        ...
        doctor.check_macos_tcc_anchor(should_fix=False)
        out = capsys.readouterr().out
        assert "macOS TCC anchor" in out

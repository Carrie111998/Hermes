    ...
    def side_effect(cmd, **kwargs):
        ...
        if "merge-base" in joined and "--is-ancestor" in joined:
            return SimpleNamespace(
                stdout="",
                stderr="",
                returncode=1 if ff_only_fails else 0,
            )
...
def test_cmd_update_stops_without_reset_when_ff_only_fails(monkeypatch, tmp_path, capsys):
    """Diverged history stops for review without rewriting local commits."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
    ...
    out = capsys.readouterr().out
    assert "history have diverged" in out
    assert "without rewriting local commits" in out
...
# ---------------------------------------------------------------------------
# Failed pull — restore updater-created stash
# ---------------------------------------------------------------------------

def test_cmd_update_restores_stash_when_pull_fails(monkeypatch, tmp_path):
    """A failed pull restores local changes that the updater stashed."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    assert len(restore_calls) == 1
    assert not any("reset" in command and "--hard" in command for command in recorded)

...
def test_current_process_update_claim_is_ours():
    holder = UpdateHolder(pid=os.getpid(), age_seconds=0)

    assert update_claim_belongs_to_current_process(holder) is True


def test_ancestor_update_claim_is_ours(monkeypatch):
    holder = UpdateHolder(pid=4242, age_seconds=0)
    monkeypatch.setattr(update_lock_mod, "_is_ancestor_pid", lambda pid: pid == 4242)

    assert update_claim_belongs_to_current_process(holder) is True


def test_unresolved_or_foreign_update_claim_is_not_ours(monkeypatch):
    monkeypatch.setattr(update_lock_mod, "_handoff_pid", lambda: None)
    monkeypatch.setattr(update_lock_mod, "_is_ancestor_pid", lambda pid: False)

    assert update_claim_belongs_to_current_process(None) is False
    assert (
        update_claim_belongs_to_current_process(
            UpdateHolder(pid=None, age_seconds=0, unavailable_reason="unreadable")
        )
        is False
    )
    assert (
        update_claim_belongs_to_current_process(
            UpdateHolder(pid=4242, age_seconds=0)
        )
        is False
    )

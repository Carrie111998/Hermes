...
class TestManagedDetection:
    ...
    def managed_tree(self, tmp_path, monkeypatch):
        ...
        npm = node / "bin" / "npm"
        if os.name == "nt":
            # The Windows managed runtime exposes npm through npm.cmd rather
            # than the POSIX symlink chain, and ordinary test processes often
            # lack SeCreateSymbolicLinkPrivilege. A regular file still covers
            # direct managed-tree detection on Windows.
    ...
    def test_symlink_from_local_bin_resolves_to_managed(self, managed_tree, tmp_path):
        ...
        if os.name == "nt":
            pytest.skip("POSIX installer symlink contract")

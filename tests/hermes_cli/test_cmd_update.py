...
class TestOriginOfficialUrlRecognition:
    """_is_noncanonical_origin must recognize the official repo across URL spellings.
    ...
    def test_official_url_spellings_are_recognized(self, url):
        from hermes_cli import main as hm

        assert hm._is_noncanonical_origin(url) is False
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "https://github.com:evil.com/NousResearch/hermes-agent",
        ],
    )
    def test_non_official_urls_are_flagged(self, url):
        from hermes_cli import main as hm

        assert hm._is_noncanonical_origin(url) is True

    def test_missing_origin_is_not_flagged(self):
        from hermes_cli import main as hm

        assert hm._is_noncanonical_origin(None) is False
...
class TestUpdateNonCanonicalOriginBanner:
    ...
    def test_banner_does_not_assert_fork_for_aliased_origin(
        ...
        with patch.object(
            ...
            hm, "_get_stored_origin_url", return_value=None
        ), patch.object(hm, "_sync_with_upstream_if_needed") as sync_mock:
            cmd_update(mock_args)
    ...
    @patch("shutil.which", return_value=None)
    @patch("subprocess.run")
    def test_official_effective_url_wins_regardless_of_stored_url(
        self, mock_run, _mock_which, mock_args, capsys
    ):
        """An effective URL that parses as official settles the question on
        its own — a non-canonical stored URL (insteadOf rewriting the other
        way) must not drag the update into the sync flow.
        """
        from hermes_cli import main as hm

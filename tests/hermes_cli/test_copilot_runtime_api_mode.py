...
def test_copilot_runtime_api_mode_uses_target_model_over_stale_config_default(monkeypatch):
    ...
    Repro: the main Copilot model is GPT-5.5 (Responses API), but an auxiliary
    slot is Copilot Claude Opus. That slot must use Anthropic Messages so its
    ``POST /v1/messages`` request receives Copilot Bearer auth and identity
    headers. Looking only at model.default leaves it on Responses instead.
    """
    from hermes_cli import runtime_provider as rp

    monkeypatch.setattr(
        "hermes_cli.models.copilot_model_api_mode",
        lambda model, api_key=None, base_url=None: "codex_responses" if str(model).startswith("gpt-5") else "anthropic_messages",
    )

...
ALLOWLIST = {
    ...
    # Production-UID prerequisite proof: validates the exact raw deployed
    # file, including its bounded byte representation, rather than resolving
    # the process's effective config overlay.
    "gateway/production_capability_gateway_probe.py",
}

# Directories that never count (tests may build fixture configs freely).
EXCLUDED_DIR_PARTS = {
    "tests", ".venv", ".git", ".worktrees", "node_modules", "website",
    "docs", "scripts", "examples", "apps", "__pycache__",
}
...
def _iter_source_files():
    # Source/test subprocesses may create and remove cache directories while
    # this lint guard is walking the tree. os.walk handles those transient
    # disappearances through onerror and lets us prune excluded directories

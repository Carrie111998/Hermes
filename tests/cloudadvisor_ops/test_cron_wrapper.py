...
def test_main_returns_sync_auto_status_without_memory_sidecar(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        cron_wrapper, "_config_from_args", lambda _args: _config(tmp_path)
    )
    monkeypatch.setattr(cron_wrapper, "run_sync_auto", lambda _config: 75)

    assert cron_wrapper.main([
        "sync-auto", "--config", str(tmp_path / "operations.yaml"),
        "--python", str(tmp_path / "python"),
        "--install-root", str(tmp_path / "repo"),
    ]) == 75


def test_main_returns_health_status_without_memory_sidecar(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(cron_wrapper, "run_health", lambda _config: 0)

    assert cron_wrapper.main([
        "health", "--config", str(tmp_path / "operations.yaml"),
        "--python", str(tmp_path / "python"),
        "--install-root", str(tmp_path / "repo"),
    ]) == 0

...
@pytest.fixture
def orion_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.orion/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".orion"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(orion_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = orion_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different ORION_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".orion"
    ...
    monkeypatch.setenv("ORION_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("ORION_HOME", str(home_b))

...
from agent.auxiliary_client import (
    ...
    _refresh_nastech_recommended_model,
...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "NASTECH_INFERENCE_BASE_URL",
...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("nastech_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("nastech_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp NASTECH_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "nastech-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "nastech", "model": "nastech-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "nastech",
...
class TestNastechTagsScoping:
    def test_tags_injected_when_provider_is_nastech(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_nastech", False)
        ...
        assert kwargs["extra_body"]["tags"] == aux._nastech_portal_tags()

    def test_tags_not_injected_for_gemini_when_main_is_nastech(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_nastech", True)
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``nastech auth status`` already reports these as logged in; compression
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setenv("NASTECH_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``nastech model``, no per-task override is
    ...
    fallback chain (OpenRouter / Nastech / etc.), so aux tasks billed
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_skips_paid_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("nastech_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            client, model = _try_openrouter()
        assert client is None
        assert model is None
    ...
    def test_free_only_allows_free_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("nastech_cli.config.load_config_readonly",
    ...
    def test_paid_lane_warns_once(self, monkeypatch, caplog):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("nastech_cli.config.load_config_readonly", return_value={"auxiliary": {}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("nastech_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._resolve_nastech_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_nastech_refreshes_stale_pool_entry(self):
        ...
        with (
            ...
            from agent.auxiliary_client import _try_nastech

            client, model = _try_nastech()
    ...
    def test_call_llm_retries_nastech_after_401(self):
        ...
        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("nastech", "nastech-model", None, None, None)),
...
class TestIsModelNotFoundError:
    ...
    def test_nastech_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the Nastech → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshNastechRecommendedModel:
    """_refresh_nastech_recommended_model picks a fresh model after a stale 404."""
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.models.get_nastech_recommended_aux_model", _boom)
        out = _refresh_nastech_recommended_model(
            vision=False, stale_model="some/dead-model")
        assert out == _NASTECH_MODEL
    ...
    def test_returns_none_when_no_distinct_alternative(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.models.get_nastech_recommended_aux_model",
            lambda **kw: _NASTECH_MODEL,
        )
        out = _refresh_nastech_recommended_model(
            vision=False, stale_model=_NASTECH_MODEL)
        assert out is None
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "nastech", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_nastech", return_value=(mock_client, "nastech-model")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is mock_client
        assert model == "nastech-model"
        assert label == "nastech"
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/Nastech/custom/api-key all fail, payment-fallback returns None —
        Codex is never tried with a guessed model.
        """
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_nastech", return_value=(None, None)), \
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Nastech auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Nastech owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("nastech_cli.config.load_config", return_value=config), patch("nastech_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("nastech_cli.config.load_config", return_value=config), patch("nastech_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Nastech reasoning.

    MoA slot reasoning is carried through call_llm as a Nastech
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/NastechResearch/nastech-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/NastechResearch/nastech-agent/issues/18478
    """
...
class TestAuxUnhealthyCache:
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        nastech_client = MagicMock()
        nastech_resp = MagicMock()
        nastech_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        nastech_client.chat.completions.create.return_value = nastech_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(nastech_client, "n-model", "nastech")), \
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("nastech_cli.config.load_config", return_value=fake_config), patch("nastech_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("nastech_cli.config.load_config", return_value={"model": {}}), patch("nastech_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("nastech_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("nastech") == "~openai/gpt-mini-latest"
    ...
    def test_catalog_match_skips_reasoning_batch_and_embedding_lookalikes(self):
        ...
        with patch("nastech_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("nastech") == "google/gemini-3.6-flash"
    ...
    def test_catalog_match_skips_the_non_chat_siblings_of_a_chat_model(self):
        ...
        with patch("nastech_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("nastech") == "openai/gpt-4o-mini"
    ...
    def test_catalog_match_takes_the_newest_of_a_family(self):
        ...
        with patch("nastech_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("nastech") == "openai/gpt-10-mini"
    ...
    def test_catalog_fetch_is_authenticated(self):
        ...
        with patch(
            ...
            "nastech_cli.models.fetch_models_with_pricing", return_value={}
        ) as fetch:
    ...
    def test_fast_tier_is_opt_in(self):
        ...
        with patch.object(ac, "_fast_model_from_catalog") as spy:
            ac._get_aux_model_for_provider("nastech")

"""Regression tests for the codex_app_server → Orion UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_orion_tools_mcp_server_emits_bare_tool_name(self):
        """The orion-tools MCP server wraps Orion' own tools for codex;
        ...
        mcp.orion-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "orion-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "orion-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "orion agent docs"
        assert calls[0].args[3] == {"query": "orion agent docs"}

...
from agent.context_compressor import (
    ...
    SUMMARY_PREFIX,
    ContextCompressor,
    _build_skill_policy_digest,
    _collect_protected_skill_names,
    _extract_pruned_skill_names,
    _extract_skill_policy_digests,
    _format_skill_policy_digest_block,
    _MAX_PRUNED_SKILL_MARKERS,
    _MAX_REINJECTED_SKILL_DIGESTS,
    _MAX_SKILL_POLICY_DIGEST_CHARS,
...
class TestSkillPolicyDigestBuild:
    """Unit coverage for the extraction logic in isolation."""

    def test_digest_extracts_workflow_and_rules(self):
        digest = _build_skill_policy_digest(_SAMPLE_SKILL_MARKDOWN)
        assert "Load the template" in digest
        assert "MUST validate the input schema" in digest
        assert "NEVER embed unsanitized user HTML" in digest

    def test_digest_empty_for_unstructured_skill(self):
        """A skill with no Workflow section and no MUST/NEVER lines yields
        no digest — no digest is better than an empty one."""
        digest = _build_skill_policy_digest(
            "# plain-skill\n\nJust some ordinary prose, no structure at all."
        )
        assert digest == ""

    def test_digest_empty_for_empty_input(self):
        assert _build_skill_policy_digest("") == ""

    def test_digest_ignores_lowercase_must_never_in_prose(self):
        """Case-sensitive on purpose: lower-case must/never inside ordinary
        sentences is not the deliberate-emphasis authoring convention."""
        digest = _build_skill_policy_digest(
            "# skill\n\nYou must never worry about this, it just works fine.\n"
        )
        assert digest == ""

    def test_workflow_subsections_survive(self):
        """### subsections inside Workflow must not truncate the section —
        only the next *level-2* heading should end it."""
        markdown = (
            "## Workflow\n1. Step one.\n### Details\nMore detail here.\n"
            "## Next Section\nunrelated"
        )
        digest = _build_skill_policy_digest(markdown)
        assert "Step one" in digest
        assert "More detail here" in digest
        assert "unrelated" not in digest

    def test_digest_capped_at_max_chars(self):
        huge = "## Workflow\n" + ("- repeated step detail\n" * 500)
        digest = _build_skill_policy_digest(huge)
        assert len(digest) <= _MAX_SKILL_POLICY_DIGEST_CHARS
        assert digest.endswith("[digest truncated]")


class TestSkillPolicyDigestEmit:
    """The digest is embedded at the exact site the marker is (#84718) —
    the only point the raw skill body is still available."""

    def test_skill_view_summary_embeds_marker_and_digest(self):
        content = _skill_view_tool_result_json("pdf-report", pad=6000)
        summary = _summarize_tool_result("skill_view", '{"name":"pdf-report"}', content)
        assert _skill_pruned_marker("pdf-report") in summary
        assert "MUST validate the input schema" in summary
        assert "NEVER embed unsanitized user HTML" in summary

    def test_small_skill_view_summary_has_no_digest(self):
        """Below the prune threshold, the body survives verbatim — no
        marker, no digest, nothing extra injected."""
        content = _skill_view_tool_result_json("pdf-report")
        summary = _summarize_tool_result("skill_view", '{"name":"pdf-report"}', content)
        assert SKILL_PRUNED_MARKER_PREFIX not in summary
        assert "SKILL_POLICY_DIGEST" not in summary

    def test_digest_extractor_round_trips_the_emitted_block(self):
        content = _skill_view_tool_result_json("pdf-report", pad=6000)
        summary = _summarize_tool_result("skill_view", '{"name":"pdf-report"}', content)
        digests = _extract_skill_policy_digests(summary)
        assert "pdf-report" in digests
        assert "MUST validate the input schema" in digests["pdf-report"]

    def test_malformed_tool_content_never_raises_and_has_no_digest(self):
        """Historical/replayed rows whose content isn't the JSON shape
        skill_view actually returns must not crash compression — same
        contract the marker emit site already guarantees."""
        summary = _summarize_tool_result(
            "skill_view", '{"name":"pdf-report"}', "not json at all " * 400
        )
        assert _skill_pruned_marker("pdf-report") in summary
        assert "SKILL_POLICY_DIGEST" not in summary


class TestReinjectPrunedSkillDigests:
    """Deterministic restoration mirrors the marker's own P2 layer."""

    def test_reinjects_digest_when_marker_survives_but_digest_dropped(self):
        digest = "Workflow:\nStep one.\n\nRules:\n- MUST do the thing."
        summary_with_marker_only = "Some text. " + _skill_pruned_marker("pdf")
        out = _reinject_pruned_skill_markers(
            summary_with_marker_only, ["pdf"], {"pdf": digest},
        )
        assert _format_skill_policy_digest_block("pdf", digest) in out
        assert out.count(_skill_pruned_marker("pdf")) == 1

    def test_reinjects_both_marker_and_digest_when_both_dropped(self):
        digest = "Workflow:\nStep one."
        out = _reinject_pruned_skill_markers("plain summary", ["pdf"], {"pdf": digest})
        assert _skill_pruned_marker("pdf") in out
        assert _format_skill_policy_digest_block("pdf", digest) in out

    def test_digest_already_present_is_not_duplicated(self):
        digest = "Workflow:\nStep one."
        already_has_both = (
            _skill_pruned_marker("pdf") + "\n" + _format_skill_policy_digest_block("pdf", digest)
        )
        out = _reinject_pruned_skill_markers(already_has_both, ["pdf"], {"pdf": digest})
        assert out == already_has_both

    def test_no_digest_for_skill_means_no_digest_block(self):
        """A skill with no extractable digest still gets its marker
        restored — the digest is additive, never a precondition."""
        out = _reinject_pruned_skill_markers("body", ["pdf"], {})
        assert _skill_pruned_marker("pdf") in out
        assert "SKILL_POLICY_DIGEST" not in out

    def test_digest_reinjection_is_capped(self):
        """Ghosting many skills in one boundary must not blow the budget
        the digest feature depends on staying small (#84718)."""
        names = [f"skill-{i}" for i in range(_MAX_REINJECTED_SKILL_DIGESTS + 10)]
        digests = {n: f"Workflow:\nstep for {n}" for n in names}
        out = _reinject_pruned_skill_markers("body", names, digests)
        assert out.count(SKILL_PRUNED_MARKER_PREFIX) == len(names)  # markers: cheap, uncapped
        assert out.count("[SKILL_POLICY_DIGEST:") == _MAX_REINJECTED_SKILL_DIGESTS


class TestSkillPolicyDigestSurvivesRealCompress:
    """Full-pipeline regression: the digest survives a real ``compress()``
    call, restored by P2 re-injection exactly like the tombstone marker."""
    ...
    def test_digest_reinjected_when_summarizer_drops_it(self):
        """Already-pruned row (marker + digest baked in from a prior
        compaction) survives an iterative rewrite that paraphrases both
        away — mirrors TestMarkerSurvivesRealCompress exactly, plus digest."""
        digest_block = _format_skill_policy_digest_block(
            "pdf", "Workflow:\nRender the template.\n\nRules:\n- MUST validate input."
        )
        pruned_row_content = (
            "[skill_view] name=pdf (48,201 chars) " + _skill_pruned_marker("pdf")
        ...
        msgs = [
            ...
            {"role": "user", "content": "Build the PDF report"},
            {
                ...
                "tool_calls": [{
                    "id": "call_pdf", "type": "function",
                    "function": {"name": "skill_view", "arguments": '{"name":"pdf"}'},
                }],
            },
            {"role": "tool", "tool_call_id": "call_pdf", "content": pruned_row_content},
            {"role": "assistant", "content": "Loaded the skill, working."},
        ...
        with (
            patch.object(c, "_find_tail_cut_by_tokens", return_value=7),
            patch("agent.context_compressor.call_llm", return_value=drop_response) as mock_call,
        ):
            result = c.compress(msgs, force=True)
        assert mock_call.called
        summary_text = self._summary_text_of(result)
        assert _skill_pruned_marker("pdf") in summary_text
        assert "MUST validate input" in summary_text
        # Stored iterative-update state carries both too.
        assert "MUST validate input" in c._previous_summary

    def test_digest_survives_from_raw_skill_view_through_phase1_prune(self):
        """End-to-end from a REAL (unpruned) skill_view JSON body — proves
        the emit site (Phase-1 prune inside compress()) → ghost-collection
        → re-injection chain all connect from the actual tool-result shape,
        not a hand-constructed already-pruned row."""
        c = _make_compressor(protect_first_n=1, protect_last_n=2)
        big_skill_json = _skill_view_tool_result_json("pdf", pad=8000)
        msgs = [
            {"role": "system", "content": "System prompt"},
            {"role": "user", "content": "Build the PDF report"},
            {
            ...
            {"role": "tool", "tool_call_id": "call_pdf", "content": big_skill_json},
        ] + [
            {"role": "assistant" if i % 2 else "user", "content": f"turn {i} " + "q" * 400}
            for i in range(10)
        ]
        drop_response = self._mock_response(
            "## Goal\nWork in progress. No pruned skills mentioned."
        )
        with (
        ...
        summary_text = self._summary_text_of(result)
        assert _skill_pruned_marker("pdf") in summary_text
        assert "MUST validate the input schema" in summary_text

    def test_todo_snapshot_and_skill_digest_coexist_at_same_boundary(self):
        """Regression: the #86852 todo/reload-notice coupling and the
        #84718 skill policy digest are independent artifacts that must not
        corrupt each other when a compaction boundary carries both a
        pruned skill AND an active todo list."""
        ...
        with (
            ...
            result = c.compress(msgs, force=True)
        summary_text = self._summary_text_of(result)
        # Skill digest present as before ...
        assert "MUST validate the input schema" in summary_text
        # ... and the todo store's own snapshot machinery is untouched by
        # the digest addition (independent, unaffected surface).
        store = TodoStore()
        store.write([{"id": "1", "content": "Finish the report", "status": "pending"}])

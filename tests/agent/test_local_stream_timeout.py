...
class TestLocalDflashStaleTimeout:
    ...
    def test_generic_local_stream_stale_timeout_uses_bounded_ceiling(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        (tmp_path / ".env").write_text("", encoding="utf-8")
        monkeypatch.delenv("HERMES_STREAM_STALE_TIMEOUT", raising=False)
        ...
        # Upstream bounds generic local endpoints at 900s (config
        # agent.local_stream_stale_timeout / HERMES_LOCAL_STREAM_STALE_TIMEOUT)
        # so a wedged local server cannot hang the session forever.

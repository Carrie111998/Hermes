...
def backup_env(monkeypatch, tmp_path):
    """Isolate ORION_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".orion"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_orion_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd orion_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under ORION_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level ORION_DIR picks up the tmp HOME."""
    import orion_constants
    importlib.reload(orion_constants)

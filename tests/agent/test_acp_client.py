...
# ---------------------------------------------------------------------------
# Tests: subprocess env security — hermes_subprocess_env usage
# ---------------------------------------------------------------------------


class TestSubprocessEnvSecurity:
    """ACPClient.__init__ must use hermes_subprocess_env(inherit_credentials=True)
    instead of os.environ.copy(), and explicit env overrides must win.

    Mirrors the pattern already established in codex_app_server.py (#29157):
    spawn_env = hermes_subprocess_env(inherit_credentials=True)
    if env: spawn_env.update(env)
    """

    def test_uses_hermes_subprocess_env_not_os_environ_copy(self):
        """ACPClient __init__ must route env through hermes_subprocess_env,
        not os.environ.copy()."""
        captured_env = {}
        ...
        class _FakePopen:
            def __init__(self, cmd, **kwargs):
                ...
                self.stdin = _WritableStream.__new__(_WritableStream)
        ...
        with patch("agent.transports.acp_client.subprocess.Popen", _FakePopen):
            with patch(
                "agent.transports.acp_client.hermes_subprocess_env",
                return_value={"SAFE_KEY": "safe"},
            ) as mock_env:
                ACPClient(command="fake", args=[], env={"FOO": "bar"})

        mock_env.assert_called_once_with(inherit_credentials=True)

    def test_explicit_env_overrides_spawn_env(self):
        """Explicit env kwargs passed to __init__ must override the base spawn_env."""
        captured_env = {}
        ...
        base_env = {"BASE_VAR": "base_val", "OVERLAP": "from_base"}
        with patch("agent.transports.acp_client.subprocess.Popen", _FakePopen):
            with patch(
                "agent.transports.acp_client.hermes_subprocess_env",
                return_value=dict(base_env),
            ):
                ACPClient(
                    command="fake",
                    args=[],
                    env={"FOO": "bar", "OVERLAP": "overridden"},
                )

        assert captured_env["FOO"] == "bar"
        assert captured_env["OVERLAP"] == "overridden"
        assert captured_env["BASE_VAR"] == "base_val"

    def test_child_env_does_not_come_from_os_environ_directly(self):
        """If os.environ has a Tier-1 secret, it must NOT appear in the child env
        because hermes_subprocess_env strips it. Verify the helper is the source."""
        captured_env = {}
        ...
        # Simulate a sanitized env from the helper (without the secret)
        ...
        with patch("agent.transports.acp_client.subprocess.Popen", _FakePopen):
            with patch(
                ...
                ACPClient(command="fake", args=[])
        # The child env must be exactly what the helper returned (no os.environ leak)

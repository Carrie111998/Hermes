"""Tests for the cross-Orion-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Orion profile's
...
Reference: May 2026 incident — a orion-security profile session
accidentally edited skills under both ~/.orion/profiles/orion-security/skills/
AND ~/.orion/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Orion root with two profiles, monkeypatch the
...
def _set_active_home(monkeypatch, orion_home: Path):
    """Point file_safety._orion_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_orion, monkeypatch):
        _set_active_home(monkeypatch, fake_orion["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_orion, monkeypatch):
        """If ORION_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_orion_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_orion, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_orion["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_orion["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "orion-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_orion, monkeypatch):
        """Inverse direction — default-profile session reaching into a named profile."""
        _set_active_home(monkeypatch, fake_orion["default_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_orion["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "default"
        assert result["target_profile"] == "orion-security"


    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_orion, monkeypatch, area):
        _set_active_home(monkeypatch, fake_orion["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_orion["default_home"] / area / "foo.txt"
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_orion, monkeypatch):
        _set_active_home(monkeypatch, fake_orion["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_orion, monkeypatch):
        _set_active_home(monkeypatch, fake_orion["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "orion-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_orion, monkeypatch):
        _set_active_home(monkeypatch, fake_orion["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_orion["default_home"] / "skills" / "foo" / "SKILL.md")

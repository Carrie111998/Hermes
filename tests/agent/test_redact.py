...
class TestKnownPrefixes:
    ...
    def test_openrouter_sk_key(self):
        text = "OPENROUTER_API_KEY=REDACTED_OPENROUTER_API_KEY"
        result = redact_sensitive_text(text)
        assert "abcdefghijklmnop" not in result

    def test_github_pat_classic(self):
        result = redact_sensitive_text("token: <GITHUB_TOKEN>")
        assert "abc123def456" not in result

    def test_github_pat_fine_grained(self):
        result = redact_sensitive_text("<GITHUB_TOKEN>")
        assert "abc123def456" not in result
...
class TestFileReadNonReusableRedaction:
    ...
    GHP = "<GITHUB_TOKEN>"  # realistic GitHub PAT shape

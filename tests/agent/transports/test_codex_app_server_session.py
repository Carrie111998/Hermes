...
class TestLifecycle:
    ...
    def test_default_session_preserves_codex_home(self):
        client = FakeClient()
        s = make_session(client)
        assert s._codex_home == "/tmp/no-codex-auth"
        s.ensure_started()
    ...
    def test_session_copies_only_restricted_auth_material(self, tmp_path):
        ...
        client = FakeClient()
        s = make_session(client, codex_home=str(auth_home), restrict_native_tools=True)
        s.ensure_started()
        ...
        assert (isolated / "config.toml").read_text(encoding="utf-8") == "[features]\nshell_tool = false\n"
        s.close()

    def test_restricted_session_writes_shell_off_empty_connector_config(self):
        client = FakeClient()
        s = make_session(client, dynamic_tools=[], restrict_native_tools=True)
        config_path = Path(s._codex_home) / "config.toml"

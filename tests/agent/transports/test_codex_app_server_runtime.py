...
class TestConfiguredMcpDynamicTools:
    ...
    def test_legal_empty_toolsets_uses_its_registered_mcp_snapshot(self, monkeypatch) -> None:
        from agent.codex_runtime import _configured_mcp_dynamic_tools
        from tools import mcp_tool
        ...
        dynamic_tools, targets, mcp_only, _origins = _configured_mcp_dynamic_tools(
            SimpleNamespace(
                enabled_toolsets=[],
                disabled_toolsets=None,
                tools=[
                    {"function": {"name": "mcp__law_firm_ops__list_email_obligations", "parameters": {}}},
                    {"function": {"name": "terminal", "parameters": {}}},
                ],
            )
        )
        assert [tool["name"] for tool in dynamic_tools] == ["list_email_obligations"]
        assert dict(targets) == {"list_email_obligations": "mcp__law_firm_ops__list_email_obligations"}
        assert mcp_only is True

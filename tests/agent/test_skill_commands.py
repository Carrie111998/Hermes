...
class TestResolveCachedSkillCommandKey:
    ...
    def test_interleaved_scans_publish_complete_profile_snapshots(
        self, tmp_path, monkeypatch
    ):
        import agent.skill_commands as sc_mod
        ...
        skill_a.write_text("skill-a", encoding="utf-8")
        skill_b.write_text("skill-b", encoding="utf-8")
        ...
        monkeypatch.setattr(
            sc_mod, "_resolve_skill_commands_home", lambda: "/profiles/" + identity.name
        )
        monkeypatch.setattr(
            sc_mod, "_resolve_skill_commands_platform", lambda: "telegram"
        )
        ...
        monkeypatch.setattr(
            "tools.skills_tool.skill_matches_platform", lambda _fm: True
        )
        monkeypatch.setattr(
            "tools.skills_tool.skill_matches_environment", lambda _fm: True
        )
    ...
    def test_get_returns_its_selected_snapshot_when_compat_alias_changes(
        ...
        final_line = max(
            line for _, line in dis.findlinestarts(sc_mod.get_skill_commands.__code__)
        )

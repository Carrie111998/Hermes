...
def test_row_is_typed_by_the_turn_start_persist(agent_db):
    ...
    # A user-role payload that looks like a system note is provenance-quoted
    # before persistence; the display metadata remains attached to that same
    # durable row.
    assert row["content"] == NOTE.replace(
        "[System note:", "[USER-QUOTED System note:", 1
    )

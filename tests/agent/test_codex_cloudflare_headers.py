...
- graceful handling of malformed tokens (drop the account-ID header, don't
  raise)
- primary-client wiring at startup, base-URL changes, and in-place model switches
...
class TestPrimaryClientWiring:
    ...
    def test_switch_model_reapplies_codex_headers_to_rebuilt_client(self, monkeypatch):
        """In-place model switches must preserve the Codex endpoint headers.

        Regression guard for the live ``switch_model`` path: rebuilding
        ``_client_kwargs`` from scratch must still run the shared base-URL
        header resolver before constructing the replacement OpenAI client.
        """
        from run_agent import AIAgent

        token = _make_codex_jwt("acct-switch")
        with patch("run_agent.OpenAI") as mock_openai:
            mock_openai.return_value = MagicMock()
            agent = AIAgent(
                api_key="placeholder-openrouter-key",
            ...
            assert mock_openai.call_count == startup_call_count + 1
            rebuilt_client_call = mock_openai.call_args_list[startup_call_count]
            headers = rebuilt_client_call.kwargs.get("default_headers") or {}
            assert headers.get("originator") == "codex_cli_rs"
            assert headers.get("ChatGPT-Account-ID") == "acct-switch"
            assert headers.get("User-Agent", "").startswith("codex_cli_rs/")

...
class TestMoACacheControlThreadsTtl:
    def test_agentless_destination_uses_configured_ttl(self, monkeypatch):
        from agent import agent_runtime_helpers

        monkeypatch.setattr(
            agent_runtime_helpers,
            "prompt_cache_ttl_from_config",
            lambda: "1h",
        )
        out, _ = agent_runtime_helpers.plan_cache_sections_for_destination(
            [
        ...
        assert all(
            marker.get("ttl") == "1h"
            for marker in _collect_cache_controls(out)
        )

...
path via ``orion chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ORION_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ORION_HOME at a temp dir and turn moa.save_traces on."""
    orion_home = tmp_path / ".orion"
    orion_home.mkdir()
    monkeypatch.setenv("ORION_HOME", str(orion_home))

    # save_moa_turn reads config via orion_cli.config.load_config; stub it to

...
class TestParseHooksBlock:
    ...
    def test_explicit_non_bool_fail_closed_rejects_entry(self, bad_value, caplog):
        specs = shell_hooks._parse_hooks_block({
            "pre_tool_call": [{
                "matcher": "terminal", "command": "/tmp/hook.sh",
                "fail_closed": bad_value,
            }],
        })
        assert specs == []
        assert "refusing hook entry" in caplog.text

    @pytest.mark.parametrize("value", [True, False])
    def test_bool_fail_closed_is_preserved(self, value):
        specs = shell_hooks._parse_hooks_block({
            "pre_tool_call": [{"command": "/tmp/hook.sh", "fail_closed": value}],
        })
        assert len(specs) == 1
        assert specs[0].fail_closed is value

    def test_contradictory_fail_directives_reject_entry(self, caplog):
        specs = shell_hooks._parse_hooks_block({
            "pre_tool_call": [{
                "command": "/tmp/hook.sh", "fail_closed": True, "fail_open": True,
            }],
        })
        assert specs == []

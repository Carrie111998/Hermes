...
def test_absolute_hard_ceiling_returns_failed_truthful_closed_turn_without_extra_call():
    agent = _make_agent()
    hard_ceiling = agent.iteration_budget.hard_max_total
    agent.client.chat.completions.create.side_effect = [
        *[
            _response(
                tool_calls=[_tool_call(f"c{index}", f'{{"step": {index}}}')]
            )
            for index in range(1, hard_ceiling + 1)
        ],
        _response(content="must not be called", finish_reason="stop"),
    ]
    counter = 0

    def _distinct_tool_result(*_args, **_kwargs):
        nonlocal counter
        counter += 1
    ...
    with patch.object(
        ...
        side_effect=AssertionError("hard ceiling must not make a summary call"),
    ):
        result = _run(agent, tool_effect=_distinct_tool_result)

    expected = (
        "I stopped before completing the task because the absolute provider "
        f"iteration ceiling was reached ({hard_ceiling}/{hard_ceiling})."
    )
    assert agent.client.chat.completions.create.call_count == hard_ceiling
    assert result["api_calls"] == hard_ceiling
    assert result["turn_exit_reason"] == "iteration_hard_limit"
    assert result["completed"] is False
    assert result["failed"] is True
    assert result["final_response"] == expected
    assert result["messages"][-2]["role"] == "tool"
    assert result["messages"][-2]["tool_call_id"] == f"c{hard_ceiling}"

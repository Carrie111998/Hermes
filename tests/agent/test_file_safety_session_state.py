...
manipulation. Orion keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point ORION_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_orion_home /
    get_default_orion_root) instead of monkeypatching private helpers —
    ...
    ORION_HOME=<root>/profiles/<name> makes get_default_orion_root()

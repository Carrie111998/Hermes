...
def test_live_prefix_preserves_memory_without_making_it_universally_authoritative():
    lower = SUMMARY_PREFIX.lower()
...
def test_immediately_retired_memory_authority_prefix_is_frozen():
    from agent.context_compressor import _HISTORICAL_SUMMARY_PREFIXES

    retired = _HISTORICAL_SUMMARY_PREFIXES[0]
    lower = retired.lower()
    assert "if no user message appears after this summary" in lower
    assert "always authoritative and active" in lower
    assert "attributed continuity evidence" not in lower
...
_FROZEN_PREFIX_GENERATIONS = (
    # Pre-memory-authority repair: no-user handoff plus the retired universal
    # memory-authority clause.
    (

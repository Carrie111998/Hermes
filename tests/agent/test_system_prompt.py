...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_orion_home``,
    ``get_default_orion_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``ORION_HOME`` and no resolver mocks.
    """

    def test_real_orion_home_under_profiles_renders_correct_paths(
        ...
        assert "Active Orion profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """ORION_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".orion"
        ...
        assert "Active Orion profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "ORION_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_orion_home", lambda: Path("/orion"))

    expected_profile = (
        "Active Orion profile: default. Other profiles (if any) live "
        "under /orion/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("orion_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("orion_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under ORION_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from orion_cli import config as _cfgmod

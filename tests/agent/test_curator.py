...
def curator_env(tmp_path, monkeypatch):
    """Isolated ORION_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".orion"
    ...
    # save_state() resolves the state path from ORION_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.orion once monkeypatch
    ...
    # thread here, while ORION_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from orion_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `orion model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from orion_cli.main import _AUX_TASKS
    from orion_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. orion_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. orion_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_forwards_runtime_pool_and_overrides(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(
        "orion_cli.config.load_config_readonly",
        lambda: {"model": {"provider": "custom:hyper-charm", "default": "glm-5.2"}},
    )
    monkeypatch.setattr(

...
def _mock_client(*, side_effect):
    """Build a Mock OpenAI client scoped so it can't masquerade as a MoA client.

    A bare ``unittest.mock.Mock`` masquerades as every duck-typed interface. On
    the upstream loop that makes ``hasattr(client, "consume_reference_usage")``
    truthy and ``client.last_aggregator_slot`` a Mock, so the MoA
    reference-usage / aggregator-cost accounting path fires and subscripts the
    Mock (``_agg_slot["model"]``) mid-turn, failing the turn before the
    completion is processed. ``spec=["chat"]`` keeps ``isinstance(client, Mock)``
    True (the loop's no-stream test seam still triggers) while making ``hasattr``
    return False for every attribute outside the spec, so the MoA path is not
    entered. ``.create`` stays a real Mock so call_count / side_effect assertions
    work.
    """
...
def test_autopilot_continues_then_completes(monkeypatch):
    ...
    agent = _make_agent()
    client, _create = _mock_client(side_effect=[
...
def test_autopilot_reenters_on_empty_response_abnormal_exit(monkeypatch):
    ...
    client, _create = _mock_client(side_effect=model_side_effect)
    agent.client = client
...
def test_autopilot_off_stops_at_empty_response(monkeypatch):
    ...
    client, _create = _mock_client(side_effect=[_empty()] * 8)
    agent.client = client
    ...
    result = agent.run_conversation("answer this")
    # Autopilot OFF: the loop runs its normal empty-retry budget and then breaks
    # with "(empty)". The exact retry count depends on the host loop's
    # empty-response policy (a deterministic-empty short-circuit can stop it
    # early); the invariant this test pins is that autopilot did NOT re-enter, so
    # the model was not driven past the point where it gave up. A re-entry would
    # have consumed all 8 scripted completions.
    assert 1 <= _create.call_count <= 4

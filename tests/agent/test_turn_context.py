...
def test_execution_provenance_reaches_per_turn_hook():
    """The seeded provenance is threaded into the ordinary per-turn hook
    surface, not just left on the agent — pre_llm_call is the representative
    case."""
    agent = _FakeAgent()
    captured = {}
    ...
    with patch("hermes_cli.lifecycle.invoke_hook", _capture):
        _build(agent)

...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ORION_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="orion_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".orion"))
    prev_home = os.environ.get("ORION_HOME")
    os.environ["ORION_HOME"] = os.path.join(test_home, ".orion")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("orion_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["ORION_HOME"] = prev_home

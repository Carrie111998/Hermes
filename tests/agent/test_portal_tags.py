...
def test_nous_sticky_key_matches_conversation_tag():
    """Sticky routing key must track the ambient lineage ROOT, not the segment.

    ``agent.session_id`` rotates when context compression opens a new segment;
    the ``conversation=`` tag already resolves through the ambient root. If the
    top-level ``session_id`` did not resolve the same way, sticky routing would
    silently re-key mid-conversation and cold-write the prompt cache.
    """
    from agent.portal_tags import (
        conversation_tag,
        reset_conversation_context,

...
def _without_todo_hydration_pair(messages: list[dict]) -> list[dict]:
    """Assert and remove the one structured Todo hydration pair."""
    ...
    for index, message in enumerate(messages[:-1]):
        ...
        if not any(
            ...
            and call["function"].get("name") == "todo"
            for call in calls
        ):
            continue
        result = messages[index + 1]
        assert result.get("role") == "tool"
        assert result.get("tool_name") == "todo"
        pair_indexes.extend((index, index + 1))
...
def _assert_bound_todo_snapshot(message: dict, exact_snapshot: str) -> None:
...
class TestTodoSnapshotMergedNotDuplicated:
    ...
    def test_snapshot_merges_into_trailing_user(self, tmp_path: Path):
        ...
        visible = _without_todo_hydration_pair(compressed)
    ...
    def test_snapshot_merge_is_persisted_in_place(self, tmp_path: Path):
        ...
        visible = _without_todo_hydration_pair(db_msgs)
        assert not any(
            previous.get("role") == current.get("role") == "user"
            for previous, current in zip(visible, visible[1:])
        )
        last_user = [message for message in visible if message["role"] == "user"][-1]
    ...
    def test_multimodal_snapshot_merge_is_persisted_in_place(self, tmp_path: Path):
        ...
        db_msgs = db.get_messages(agent.session_id)
        persisted_visible = _without_todo_hydration_pair(db_msgs)
        persisted_tail = persisted_visible[-1]
        ...
        assert not any(
            previous.get("role") == current.get("role") == "user"
            for previous, current in zip(persisted_visible, persisted_visible[1:])
        )
...
class TestTodoSnapshotScaffoldingTails:
    ...
    def test_snapshot_stays_standalone_after_continuation_marker(
        ...
        assert tail["role"] == "user"
        assert "task A" in tail["content"]
        _assert_bound_todo_snapshot(
            tail,
            agent._todo_store.format_for_injection(),
        )
        # The continuation marker keeps its exact text so it stays
        # recognizable as scaffolding after the bound snapshot is removed.
        assert tail["content"].endswith(COMPRESSION_CONTINUATION_USER_CONTENT)
        # Zero-user provenance: the bound snapshot cannot promote the
        # continuation marker into a real user turn.
    ...
    def test_snapshot_stays_standalone_after_summary_as_user_tail(
        ...
        tail = visible[-1]
        assert "task A" in tail["content"]
        assert summary_as_user in tail["content"]
        _assert_bound_todo_snapshot(
            tail,
            agent._todo_store.format_for_injection(),
        )
        # An unbound summary lookalike remains user-authored input. Only the
        # exact Todo fragment receives runtime provenance.
        assert ContextCompressor._transcript_has_real_user_turn([tail])
    ...
    def test_stale_snapshot_row_is_refreshed_not_stacked(self, tmp_path: Path):
        ...
        tail = visible[-1]
        assert "task A" in tail["content"]
        assert "old finished task" in tail["content"]
        assert tail["content"].count(TODO_INJECTION_HEADER) == 2
        _assert_bound_todo_snapshot(
            tail,
            agent._todo_store.format_for_injection(),
        )

"""Tests for Anthropic OAuth ``mcp__`` tool-name round-trip.

Anthropic's subscription/OAuth billing classifier treats a single-underscore
``mcp_`` tool name as a third-party-app fingerprint. On the OAuth wire nothing
may carry a single-underscore ``mcp_`` prefix; response normalization reverses
``mcp__`` back to the registered dispatcher name.
"""
...
class TestAnthropicMcpPrefixStrip:
    ...
    def test_restores_single_underscore_mcp_server_tool(self):
        """``mcp__linear_get_issue`` -> ``mcp_linear_get_issue``."""
        transport = self._get_transport()
...
class TestAnthropicOAuthOutgoingPrefix:
    """build_anthropic_kwargs emits zero single-underscore ``mcp_`` wire names."""
    ...
    def test_oauth_already_double_prefixed_left_alone(self):
        """OAuth + already-``mcp__`` name -> unchanged."""
        kwargs = self._build([
            {
                "type": "function",
    ...
    def test_oauth_no_single_underscore_mcp_on_wire(self):
        """Mixed set: every wire name is free of single-underscore mcp_."""
        kwargs = self._build([
    ...
    def test_non_oauth_path_untouched(self):
        """Non-OAuth requests never get the prefix."""
        kwargs = self._build([
            {"type": "function", "function": {"name": "read_file", "description": "x", "parameters": {}}},
            {"type": "function", "function": {"name": "mcp_linear_get_issue", "description": "y", "parameters": {}}},
        ], is_oauth=False)

...
class TestEnvironmentHints:
    ...
    def test_probe_remote_backend_passes_normalized_configs(self, monkeypatch):
        """``_create_environment`` has legacy non-Optional annotations for
        ``ssh_config``, ``container_config`` and ``host_cwd`` — and its runtime
        semantics already treat a missing container config as ``{}``, require
        ssh config only for ssh, and ignore an empty host cwd. The probe must
        therefore always hand it dictionaries for both config slots and a
        string for host_cwd, for both the docker and ssh paths."""
        import agent.prompt_builder as _pb
        ...
        for env_type in ("docker", "ssh"):
            _pb._clear_backend_probe_cache()
            line = _pb._probe_remote_backend(env_type)
            assert line is not None, f"probe unexpectedly failed for {env_type}"
            kwargs = captured[env_type]
            assert isinstance(kwargs["ssh_config"], dict), (
                f"{env_type}: ssh_config must always be a dict, "
                f"got {kwargs['ssh_config']!r}"
            )
            assert isinstance(kwargs["container_config"], dict), (
                f"{env_type}: container_config must always be a dict, "
                f"got {kwargs['container_config']!r}"
            )
            assert isinstance(kwargs["host_cwd"], str), (
                f"{env_type}: host_cwd must always be a str, got {kwargs['host_cwd']!r}"
            )

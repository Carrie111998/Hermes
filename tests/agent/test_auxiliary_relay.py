...
from orion_cli.observability.shared_metrics import SharedMetricsStore
from orion_cli.observability.shared_metrics_contract import MODEL_ROUTE_METRIC
from orion_cli.observability.shared_metrics_subscriber import SharedMetricsSubscriber


@pytest.fixture()
def relay_turn(tmp_path, monkeypatch):
    monkeypatch.setenv("ORION_HOME", str(tmp_path / "profile"))
...
def test_auxiliary_provider_fallback_records_one_terminal_model_route(
    ...
    assert snapshot[0]["resource"]["orion_version"] == "test-version"

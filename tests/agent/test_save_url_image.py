...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ORION_HOME under tmp_path."""
    monkeypatch.setenv("ORION_HOME", str(tmp_path / ".orion"))
    (tmp_path / ".orion").mkdir()

    # Force the constants/image cache helpers to re-read ORION_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("orion_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_orion_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ORION_HOME — gateway cleanup

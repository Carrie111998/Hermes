...
``set_orion_home_override`` / ``reset_orion_home_override``. Because
``cronjob()`` -> ``list_jobs()`` keys off ``get_orion_home()``, the list action
...
def test_cron_manage_profile_reads_that_profiles_store(tmp_path, monkeypatch):
    ...
    import orion_cli.profiles as profiles
    ...
    assert get_orion_home_override() is None

...
class FakeNode {
  ...
  /* Enough selector support for the pages under test: tag, .class, and
     [attr="value"] terms, optionally as a descendant chain. */
  matches(selector) {
    return selector.trim().split(/\s+/).every(term => {
      if (term.startsWith('.')) return this.classList.contains(term.slice(1));
      const attr = term.match(/^\[([^=\]]+)(?:="([^"]*)")?\]$/);
      if (attr) {
        const value = this.attributes.get(attr[1]);
        return attr[2] === undefined ? value !== undefined : value === attr[2];
      }
      return this.tagName === term.toUpperCase();
    });
  }
  ...
  closest(selector) {
    ...
    for (let node = this; node; node = node.parentNode) {
      if (node.nodeType === 1 && node.matches(last)) return node;
    }
    return null;
  }
  ...
  querySelector(selector) {
    ...
    const visit = node => {
      if (found) return;
      if (node.nodeType === 1 && node !== this && node.matches(last)) { found = node; return; }
...
test('Setup mailbox picker starts Google OAuth and refreshes the mailbox on success', async () => {
  ...
  // Setup mirrors its open section into the URL on every load.
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    // Sending (and so the mailbox editor) only appears once the required steps are done.
    if (url === '/api/v1/onboarding/status') {
      return Response.json({
        status: 'completed',
        completed_steps: [
          'company-identity', 'positioning', 'products',
          'internal-sales-data', 'target-markets',
        ],
      });
    }
    if (url === '/api/v1/company/profile'
        || url === '/api/v1/company/positioning'
        || url === '/api/v1/company/sales-preferences') return Response.json({});
    ...
    if (url === '/api/v1/onboarding/integrations') return Response.json({});
  ...
  try {
    ...
    // The mailbox editor only renders when its section is open.
    dispose = await mount(root, { navigate() {}, query: { section: 'mailbox' }, params: {} });

    // Setup keeps the mailbox behind a picker: open it, then choose the hosted provider.
    const openPicker = nodesMatching(
      root,
      node => node.tagName === 'BUTTON' && node.textContent === 'Connect a mailbox',
    );
    assert.equal(openPicker.length, 1);
    openPicker[0].click();

    const googleChoice = nodesMatching(
      documentRef.body,
      node => node.tagName === 'BUTTON' && node.textContent === 'Google Workspace',
    );
    assert.equal(googleChoice.length, 1);
    googleChoice[0].click();
    ...
    await flushUntil(
      () => nodesMatching(
        root,
        node => node.className.split(/\s+/).includes('ifz-setup-provider-row')
          && node.textContent.includes('owner@example.test'),
      ).length === 1,
      'Setup did not render the connected mailbox after OAuth success',
    );

    assert.equal(emailLists, 2);
    const mailboxRow = nodesMatching(
      root,
      node => node.className.split(/\s+/).includes('ifz-setup-provider-row'),
    )[0];
    assert.deepEqual(
      nodesMatching(mailboxRow, node => node.tagName === 'BUTTON')
    ...
    // The OAuth attempt leaves nothing behind: the two pending timeouts are the
    // picker modal's initial-focus timer and the success toast's dismiss timer.
    assert.deepEqual(timers.counts(), { intervals: 0, timeouts: 2 });

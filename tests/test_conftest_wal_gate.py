"""The conftest WAL gate must agree with orion_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``orion_state``. That is deliberate: importing
``orion_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.orion`` before the per-test ``ORION_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_orion_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and orion_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_orion_state_at_collection():
    """The gate must stay import-free of orion_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.orion, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``orion_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "orion_state" or name.startswith("orion_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported orion_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.orion during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("orion_state", None)

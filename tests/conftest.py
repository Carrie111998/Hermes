"""Shared fixtures for the orion-agent test suite.
...
2. **Isolated ORION_HOME.** ORION_HOME points to a per-test tempdir so
   code reading ``~/.orion/*`` via ``get_orion_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".orion"`` instead of the canonical
   ``get_orion_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ORION_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ORION_HOME before ANY test module is imported ──────────────────
# `orion_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
...
# real `~/.orion/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ORION_HOME, but fixtures run
...
# the REAL Orion root — capture it BEFORE the sandbox rewires ORION_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.orion (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ORION_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ORION_HOME = os.environ.get("ORION_HOME", "")


def _orion_home_points_at_production(value: str) -> bool:
    """True when a pre-set ORION_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ORION_HOME=~/.orion``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``orion_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.orion`` — the escape vector that landed pytest
    ...
    ORION_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".orion").resolve()
    except Exception:
        return True
...
if _orion_home_points_at_production(os.environ.get("ORION_HOME", "")):
    _SESSION_ORION_HOME = tempfile.mkdtemp(prefix="orion-test-home-")
    os.environ["ORION_HOME"] = _SESSION_ORION_HOME
    atexit.register(shutil.rmtree, _SESSION_ORION_HOME, True)
...
# like a real CLI") — which used to disarm orion_state's live-DB guard in
# the child at the same moment the child lost the ORION_HOME redirect.
# ORION_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# orion_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export ORION_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["ORION_TEST_ISOLATION"] = os.environ.get("ORION_HOME", "") or "1"

#: ORION_HOME as it stood when conftest was imported - i.e. before any test
...
ORION_HOME_AT_CONFTEST_IMPORT = os.environ.get("ORION_HOME", "")
...
# ORION_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ORION_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``orion_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Orion agent session inherits them and
    # orion_constants home-resolution helpers prefer them over monkeypatched
    ...
    "ORION_TIMEZONE",
    "ORION_REDACT_SECRETS",
    "ORION_BACKGROUND_NOTIFICATIONS",
    "ORION_EXEC_ASK",
    "ORION_HOME_MODE",
    "ORION_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.orion/kanban.db instead of the per-test ORION_HOME.
    "ORION_KANBAN_DB",
    "ORION_KANBAN_BOARD",
    "ORION_KANBAN_HOME",
    "ORION_KANBAN_WORKSPACES_ROOT",
    "ORION_KANBAN_LOGS_ROOT",
    "ORION_KANBAN_TASK",
    "ORION_KANBAN_WORKSPACE",
    "ORION_KANBAN_RUN_ID",
    "ORION_KANBAN_CLAIM_LOCK",
    "ORION_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "orion" host.
    "ORION_HONCHO_HOST",
    ...
    "ORION_DASHBOARD_OAUTH_CLIENT_ID",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ORION_HOME to per-test tempdirs so code that
    reads ``~/.orion/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ORION_* vars that could change test semantics.
    for name in _ORION_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ORION_HONCHO_HOST", "orion")

    # 3. Redirect ORION_HOME to a per-test tempdir. Code that reads
    #    ``~/.orion/*`` via ``get_orion_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.orion/*`` via
    #    ``Path.home() / ".orion"`` instead of ``get_orion_home()``
    #    is a bug to fix at the callsite.
    fake_orion_home = tmp_path / "orion_test"
    fake_orion_home.mkdir()
    (fake_orion_home / "sessions").mkdir()
    (fake_orion_home / "cron").mkdir()
    (fake_orion_home / "memories").mkdir()
    (fake_orion_home / "skills").mkdir()
    monkeypatch.setenv("ORION_HOME", str(fake_orion_home))
    ...
    # orion_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("ORION_TEST_ISOLATION", str(fake_orion_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("ORION_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. orion_state computes ``DEFAULT_DB_PATH = get_orion_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from orion_state import ...``) that
    ...
    orion_state_mod = sys.modules.get("orion_state")
    if orion_state_mod is not None and hasattr(orion_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            orion_state_mod, "DEFAULT_DB_PATH", fake_orion_home / "state.db"
        )
    ...
    #    ~/.orion/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import orion_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path ORION_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_orion_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from orion_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.orion. This autouse
...
# allow-list because test-level fixtures legitimately move ORION_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ORION_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ORION_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Orion root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ORION_HOME and not _orion_home_points_at_production(
        _PRE_SANDBOX_ORION_HOME
    ):
        # ORION_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_orion_root().resolve()
    # No pre-existing ORION_HOME: the real root is the platform default,
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.orion`` captured at import time. Hermetic tests that legitimately
    move ORION_HOME to sibling tempdirs are unaffected.

    Only patches when ``orion_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("orion_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import orion_cli.kanban_db`
...
# ``orion_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Orion root. This fixture wires the
# test-side knobs:
...
#     installs where ORION_HOME is not ~/.orion) into the guard's
...
# so subprocess children that import orion_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("orion_state")
    ...
    if _PRE_SANDBOX_ORION_HOME and not _orion_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ORION_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Orion home override redirects every later
    # ``get_orion_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from orion_constants import get_orion_home_override, set_orion_home_override

        if get_orion_home_override() is not None:
            set_orion_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal orion config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``orion-gateway`` process
...
# correlated with a ``tests/orion_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> orion-gateway``
...
def _wal_is_usable() -> bool:
    """True when Orion will actually put a database into WAL mode here.

    Orion refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Orion can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Orion managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``orion_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_orion_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_orion_home`` fixture
    redirects ``ORION_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.orion/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``orion_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ORION_VOICE_TTS"] = "1"``. The
...
#      ``orion_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``orion_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``orion_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``orion_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Orion runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.orion``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Orion falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Orion uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart orion-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting orion/python patterns are also blocked.
    """
    ...
    def _matches_orion_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ORION_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_orion_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch orion-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m orion_cli.main".
                if (
                    "orion" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live orion-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting orion/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `orion update` (or the
        # equivalent `python -m orion_cli.main update`).  These commands
        ...
        if "update" in low and (
            # orion update / orion update --gateway / setsid bash -c ... orion update
            ("orion" in low and "update" in low.split())
            or
            # python -m orion_cli.main update --gateway
            ("orion_cli" in low and "update" in low.split())
            or
            # venv/bin/orion update  (absolute path variant used in tests)
            (".venv/bin/orion" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`orion update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ORION_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import orion_cli.voice as _voice
    except Exception:

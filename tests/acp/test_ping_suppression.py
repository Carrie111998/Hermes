...
async def test_bare_ping_request_produces_proper_response_and_no_stderr_noise(
    ...
    try:
        agent_streams = asyncio.Queue()

        async def connect_agent(
            agent_input: asyncio.StreamReader, agent_output: asyncio.StreamWriter
        ) -> None:
            await agent_streams.put((agent_input, agent_output))

        server = await asyncio.start_server(connect_agent, "127.0.0.1", 0)

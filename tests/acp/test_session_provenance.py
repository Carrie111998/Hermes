...
def test_root_session_no_compression(db):
    ...
    assert prov["currentOrionSessionId"] == "root1"
    assert prov["rootOrionSessionId"] == "root1"
    assert prov["parentOrionSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_orion_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentOrionSessionId"] == "old"
    assert prov["rootOrionSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousOrionSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootOrionSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"orion"}
    assert "sessionProvenance" in meta["orion"]
    assert meta["orion"]["sessionProvenance"]["currentOrionSessionId"] == "root1"

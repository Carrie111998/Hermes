...
COMMON_ORION_TOOLS = ["read_file", "search_files", "terminal", "patch", "write_file", "process"]


class TestToolKindMap:
    def test_all_orion_tools_have_kind(self):
        """Every common orion tool should appear in TOOL_KIND_MAP."""
        for tool in COMMON_ORION_TOOLS:
...
class TestBuildToolTitle:
    ...
    def test_execute_code_title_includes_first_code_line(self):
        title = build_tool_title("execute_code", {"code": "\nfrom orion_tools import terminal\nprint('done')"})
        assert title == "python: from orion_tools import terminal"

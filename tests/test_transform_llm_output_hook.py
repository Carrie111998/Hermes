...
from orion_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(orion_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <orion_home>/plugins/<name> and opt it in."""
    plugin_dir = orion_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ORION_HOME", str(tmp_path / "orion_empty"))

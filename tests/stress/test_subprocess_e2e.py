...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves orion_cli.kanban_db on its own
  ...
  - worker logs are captured to ORION_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "ORION_KANBAN_TASK": task.id,
            "ORION_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="orion_e2e_")
    os.environ["ORION_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from orion_cli import kanban_db as kb

    # Point the `orion` CLI child processes will run at the worktree
    # orion_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "orion")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m orion_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/orion_e2e_*/worker_*.log
    import glob

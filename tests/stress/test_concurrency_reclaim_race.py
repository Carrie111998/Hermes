...
def worker_loop(worker_id: int, orion_home: str, result_file: str) -> None:
    os.environ["ORION_HOME"] = orion_home
    os.environ["HOME"] = orion_home
...
def reclaimer_loop(orion_home: str, result_file: str) -> None:
    os.environ["ORION_HOME"] = orion_home
    os.environ["HOME"] = orion_home
    sys.path.insert(0, WT)
    from orion_cli import kanban_db as kb
...
def main():
    home = tempfile.mkdtemp(prefix="orion_reclaim_race_")

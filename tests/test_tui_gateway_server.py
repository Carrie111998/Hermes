...
def test_tui_verbose_tool_details_fail_closed_when_redaction_fails(monkeypatch):
    ...
    # Patch the live function on imported modules. Do not replace sys.modules
    # entries — that permanently poisons later imports of agent.display /
    # agent.tool_activity in the same pytest process.
    monkeypatch.setattr(redact_mod, "redact_sensitive_text", fail_redaction)
    monkeypatch.setattr(display_mod, "redact_sensitive_text", fail_redaction)
...
def test_tool_events_preserve_rich_surface_metadata(monkeypatch):
    ...
    # context is an argument preview (main contract), not a phrased label.
    # reason remains the reason-first activity field from this PR.
    server._on_tool_start(
        "rich-tool-test",
        "tool-1",
        "search_files",
        {"pattern": "app.ts", "path": "src"},

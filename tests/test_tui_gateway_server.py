...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # Sorted: ["kanban", "memory", "project", "tool_presets"]. `kanban` and
    # `tool_presets` are auto-recovered by _get_platform_tools (non-configurable
    # platform toolsets in hermes-cli's universe — they carry core tools);
    # `project` is GUI-only, folded in by _load_enabled_toolsets.
    assert server._load_enabled_toolsets() == ["kanban", "memory", "project", "tool_presets"]

...
def test_background_output_falls_back_when_the_origin_tab_is_gone(monkeypatch):
    """A closed origin must not strand output — same conversation still gets it."""
    process_registry, emitted = _wire_terminal_sinks(monkeypatch)

    monkeypatch.setitem(
        server._sessions, "tab_origin", {"session_key": "shared-key", "_finalized": True}
    )
    monkeypatch.setitem(server._sessions, "tab_live", {"session_key": "shared-key"})
    ...
    assert emitted == [
        ("agent.terminal.output", "tab_live", {"process_id": "proc_orphan", "chunk": "orphaned"})
    ]

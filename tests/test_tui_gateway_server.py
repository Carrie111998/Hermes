...
def test_busy_tui_poller_cannot_hide_inject_from_active_loop(monkeypatch):
    """A dequeued/requeued TUI event remains atomic with same-turn draining."""
    import queue as _queue_mod
    ...
    sid = "sid-tui-inject"
    agent = types.SimpleNamespace(_active_turn_id=turn_id)
    sess = _session(
        agent=agent,
        running=True,
    ...
    def _drain():
        drained["count"] = inject_mod.drain_ready_injects(agent, messages, turn_id)
        drain_done.set()
    ...
    try:
        ...
        assert claims[0][1].startswith("conversation-loop:")
        assert isolated_queue.empty()
    finally:
        release_dequeue.set()
        stop_poller.set()

...
def test_commands_catalog_skill_canon_does_not_clobber_builtin(monkeypatch):
    ...
    # `/bg` is an alias of the built-in `/background` (canon -> /background).
    monkeypatch.setattr(
        "agent.skill_commands.scan_skill_commands",
        lambda: {
            "/bg": {"name": "bg", "description": "Move to background"},
        },
    )
    ...
    assert canon["/bg"] == "/background"
    # The skill is still advertised/ranked (it just cannot win exact-match).
    assert "/bg" in dict(resp["result"]["pairs"])
    assert "/bg" in resp["result"]["skills"]

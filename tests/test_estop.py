"""Global emergency stop (`orion pause` / `orion resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`orion resume`)
...
def orion_home(tmp_path, monkeypatch):
    """Point ORION_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(orion_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (orion_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(orion_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (orion_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(orion_home):
    ...
    raw = json.loads((orion_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(orion_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(orion_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (orion_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(orion_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(orion_home):
    ...
    assert "orion resume" in notice


def test_paused_reply_without_reason(orion_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(orion_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(orion_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(orion_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(orion_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(orion_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(orion_home):
...
# ── CLI: orion pause / orion resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(orion_home, capsys):
    from orion_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(orion_home, capsys):
    from orion_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(orion_home, capsys):
    from orion_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(orion_home, capsys):
    from orion_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from orion_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── orion status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(orion_home):
    from orion_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(orion_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when ORION_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(orion_home):
    ...
    try:
        ...
        return
    assert reply is None or "orion is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(orion_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from orion_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command

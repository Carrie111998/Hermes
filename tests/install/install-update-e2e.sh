...
#
# Installs a real, earlier Orion the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `orion`.
#
...
# Keep sandbox state out of the default .orion-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".orion-sandbox-e2e-$ROUTE"
export ORION_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/orion/.orion/orion-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/orion-agent.git"
...
# both together.
UPSTREAM_URL="${ORION_DEV_SANDBOX_UPSTREAM:-https://github.com/zacharyjleach-stack/Aries.git}"
...
LOG_DIR="${ORION_E2E_LOG_DIR:-$(mktemp -d -t orion-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED orion accept FLAG on `orion update`?
#
...
# that silently rots. `orion update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "orion update --help 2>&1" | grep -qF -- "$flag"
}
...
require_orion_works() {
  local when="$1" out
  out="$(in_sandbox "orion --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "orion --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "orion runs $when"
}

# ── install the earlier Orion ─────────────────────────────────────────────
...
require_orion_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: orion update'
    ...
    # orion which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="orion update --yes"
    else
      update_cmd="orion update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "orion update failed ($update_cmd)"
    fi
    require_landed_on_target 'orion update'
    require_orion_works 'after orion update'
    ;;
  installer)
    ...
    require_orion_works 'after installer re-run'
    ;;
esac

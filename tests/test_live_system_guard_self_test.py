...
def _run_inert_command(*tokens: str) -> subprocess.CompletedProcess:
    """Exercise the guard's command matcher without requiring host utilities."""
    return subprocess.run(
        [sys.executable, "-c", "", *tokens],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_unrelated_unit_passes_through():
    ...
    r = _run_inert_command(
        "systemctl", "--user", "restart", "fake-not-real-unit"
    )
    assert r is not None

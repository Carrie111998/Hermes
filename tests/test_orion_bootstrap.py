"""Tests for orion_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Orion entry point
(orion, orion-agent, orion-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Orion entry point imports orion_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported orion_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("orion_bootstrap", None)
    import orion_bootstrap  # noqa: WPS433
    return orion_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Orion CLI
...
class TestEntryPointsImportBootstrap:
    """Every Orion entry point must import orion_bootstrap as its
    ...
    # Entry points that invoke Orion as a process.  Each one must
    # import orion_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "orion_cli/main.py",   # orion CLI (console_script)
        "run_agent.py",          # orion-agent (console_script)
        "acp_adapter/entry.py",  # orion-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import orion_bootstrap' and that
        ...
        half-finished ``orion update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``orion_bootstrap`` as a top-level module) leaves orion
        ...
        """
        # Resolve relative to the orion-agent repo root.  Tests live
        # at tests/test_orion_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # orion start even when orion_bootstrap hasn't been
        ...
        assert first_import_name == "orion_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'orion_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import orion_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Orion's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ORION_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ORION_PYTHON_SRC_ROOT"] = env
            elif "ORION_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ORION_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/orion")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ORION_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ORION_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/orion", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Orion root.  The guard must relocate Orion to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/orion"])
        assert result[0] == "/opt/orion"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Orion root.
        assert result.index("/opt/orion") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ORION_PYTHON_SRC_ROOT"] = "/env/orion"
            hb.harden_import_path()
            assert sys.path[0] == "/env/orion"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ORION_PYTHON_SRC_ROOT", None)

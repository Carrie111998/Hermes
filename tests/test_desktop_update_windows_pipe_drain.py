...
``Invoke-OrionStep``, which starts the step with ``RedirectStandardOutput`` /
...
last of them to close it. ``orion update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``orion_cli/main.py``), so the
...
closes and ``Invoke-OrionStep`` blocks for the life of the gateway.

Everything the hand-off owes the Desktop is downstream of that call:
``.orion-update-result.json`` is never written, ``.orion-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Orion" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``orion update`` is exactly this
...
def test_pipe_drain_survives_a_leak_without_metering_a_chatty_step(
    ...
    *leak* -- a step that spawns a grandchild with ``UseShellExecute = $false``
    ...
    asserts the grandchild was **still alive** when ``Invoke-OrionStep``
    ...
    *flood* -- a step that writes megabytes and holds nothing, exiting 5. It
    ...
    env = {
        **os.environ,
        ...
        "ORION_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "ORION_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Orion' forever) or metered a chatty step (backpressure on the running "

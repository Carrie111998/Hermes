    ...
    with patch.dict(
        "sys.modules",
        {
            "orion_constants": MagicMock(
                get_orion_home=MagicMock(return_value="/tmp/orion_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'orion-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'orion-release' patched"
    }

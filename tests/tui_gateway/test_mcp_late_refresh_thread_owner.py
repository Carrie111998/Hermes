...
def clean_discovery_globals():
    ...
    saved_startup = dict(startup._mcp_discovery_threads)
    ...
    try:
        ...
        startup._mcp_discovery_threads.clear()
        startup._mcp_discovery_threads.update(saved_startup)
...
def test_entry_in_flight_sees_startup_thread(clean_discovery_globals):
    ...
    startup._mcp_discovery_threads[scope] = _alive_thread(stop)
    try:
        ...
        stop.set()
        startup._mcp_discovery_threads[scope].join(timeout=2.0)
...
def test_startup_module_exposes_in_flight_helpers(clean_discovery_globals):
    ...
    scope = startup._discovery_scope_key()
    startup._mcp_discovery_threads[scope] = t
    try:

...
``ORION_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
def orion_root(tmp_path, monkeypatch):
    """A temp ORION_HOME root with two named profiles: 'work' and 'other'.

    Pointing ORION_HOME at a dir outside ~/.orion makes it the profile ROOT
    (get_default_orion_root's Docker/custom branch), so named profiles live at
...
def test_add_lands_in_named_profile_only(orion_root):
    root = orion_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(orion_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_writes_env_and_header_to_right_profile(orion_root):
    root = orion_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(orion_root):
    root = orion_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(orion_root):
    root = orion_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(orion_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(orion_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(orion_root):
    root = orion_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == ORION_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")

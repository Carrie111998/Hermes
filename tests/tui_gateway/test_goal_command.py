...
def orion_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ORION_HOME.
...
@pytest.fixture()
def server(orion_home, monkeypatch):
    ...
    # Pin config resolution to the isolated ORION_HOME. Sibling test
    ...
    # module-level ``_orion_home = get_orion_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_orion_home", orion_home)
...
def test_moa_bare_returns_usage(server, session, orion_home):
    _write_moa_config(orion_home, """
moa:

...
def test_dispatch_pet_info_does_not_block_prompt_submit(server):
    ...
    # prompt.submit is pool-routed because SMART classification can be slow. The
    # async JSON-RPC response is written by the worker; dispatch itself must
    # return immediately just like pet.info.
    resp = server.dispatch({"id": "prompt", "method": "prompt.submit", "params": {}})
    elapsed = time.monotonic() - t0

    assert "prompt.submit" in server._LONG_HANDLERS
    assert resp is None
    assert elapsed < 2.0, (

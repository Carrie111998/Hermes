...
guard is covered in ``tests/orion_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``orion_cli.model_switch.prewarm_picker_cache_async``
...
import orion_cli.model_switch as ms
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from orion_cli.model_switch,

...
class _RecordingDB:
    """Stand-in for ``orion_state.SessionDB`` that counts ``close()`` calls.
...
def profile_dbs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("orion_state.SessionDB", _factory)
    monkeypatch.setattr(
    ...
    # off the real agent/secret/ORION_HOME machinery.

"""Guard: Orion-owned subprocesses must not resolve managed runtimes by bare PATH.

Orion installs runtimes for itself — ``uv`` at ``$ORION_HOME/bin/uv``, Node at
``$ORION_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Orion's own
...
* when a system copy also exists, the one Orion does not own wins — which is
...
``iter_orion_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Orion provisions into ORION_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Orion-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Orion-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ORION_HOME/bin (uv) and $ORION_HOME/node are not on an "
        ...
        "  node/npm -> orion_constants.find_node_executable()\n"
        "  PATH env -> orion_constants.iter_orion_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(orion_constants, helper))


def test_managed_uv_helpers_exist():
    from orion_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv

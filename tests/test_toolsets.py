...
class TestToolsetConsistency:
    ...
    def test_orion_platforms_share_core_tools(self):
        """All orion-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on orion-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["orion-cli", "orion-telegram", "orion-discord", "orion-whatsapp", "orion-slack", "orion-signal", "orion-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_orion_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("orion-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("orion-cli")
        second = resolve_toolset("orion-cli")
        ...
        assert (
            "orion-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("orion-cli", include_registry=False)
        second = resolve_toolset("orion-cli", include_registry=False)
        assert first == second

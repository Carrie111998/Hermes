...
binary download are exercised manually by `orion secrets bitwarden
...
def orion_home(tmp_path, monkeypatch):
    """Point Orion at an isolated home directory."""
    home = tmp_path / ".orion"
    home.mkdir()
    monkeypatch.setenv("ORION_HOME", str(home))
    # Some modules cache get_orion_home; clear if needed.
    import orion_constants
    if hasattr(orion_constants, "_ORION_HOME_CACHE"):
        orion_constants._ORION_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(orion_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from orion_cli.env_loader import _apply_external_secret_sources

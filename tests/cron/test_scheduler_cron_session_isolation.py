...
class _FakeCronAgent:
    ...
    def run_conversation(self, prompt):
        ...
        assert get_session_env("ORION_CRON_SESSION") == "1"
        return {
            "completed": True,
            "failed": False,
...
def test_run_job_cron_execute_code_deny_does_not_pollute_later_gateway_execute_code(
    ...
    monkeypatch.setenv("ORION_MODEL", "test-model")
    monkeypatch.setattr(approval_module, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setattr(approval_module, "_get_approval_mode", lambda: "manual")
    monkeypatch.setattr(approval_module, "_get_cron_approval_mode", lambda: "deny")
    monkeypatch.setattr("orion_state.SessionDB", _DummySessionDB)
    monkeypatch.setattr("run_agent.AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "orion_constants.resolve_reasoning_config", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(cron_scheduler, "_get_orion_home", lambda: tmp_path)
    ...
    assert os.environ.get("ORION_CRON_SESSION") is None
    assert get_session_env("ORION_CRON_SESSION") == ""
    ...
    assert get_session_env("ORION_CRON_SESSION") == "1"
    monkeypatch.delenv("ORION_CRON_SESSION")

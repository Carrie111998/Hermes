...
profile's ORION_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.orion/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active ORION_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever ORION_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    ORION_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import orion_constants

    monkeypatch.setattr(
        orion_constants, "_get_platform_default_orion_home", lambda: root
    )
    monkeypatch.setenv("ORION_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile ORION_HOME (<root>/profiles/<name>), the cron store
    ...
    assert orion_constants.get_orion_home().resolve() == profile_home.resolve()
    assert orion_constants.get_default_orion_root().resolve() == root.resolve()

    # cron/jobs.py computes ORION_DIR from get_orion_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.ORION_DIR.resolve() == profile_home.resolve()
        assert (

...
def cron_store(tmp_path, monkeypatch):
    ...
    (orion_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("ORION_HOME", str(orion_home))
    monkeypatch.setattr(J, "ORION_DIR", orion_home)
    monkeypatch.setattr(J, "CRON_DIR", orion_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", orion_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", orion_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_orion_now", lambda: now):
            due = J.get_due_jobs()

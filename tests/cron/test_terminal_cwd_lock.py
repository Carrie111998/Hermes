...
def test_run_job_releases_cwd_lock_when_body_raises(tmp_path):
    ...
    with patch("cron.scheduler._orion_home", tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("orion_cli.env_loader.load_orion_dotenv"), \
         patch("orion_cli.env_loader.reset_secret_source_cache"), \
         patch.object(sched.logger, "info", side_effect=_raise_on_workdir_log), \
         patch("orion_state.SessionDB", return_value=MagicMock()):
...
def test_cwd_lock_timeout_derivation(monkeypatch):
    """The bound tracks ORION_CRON_TIMEOUT (+margin) with a floor, and
    stays finite when the job runtime is unlimited (0)."""
    import cron.scheduler as sched

    monkeypatch.delenv("ORION_CRON_TIMEOUT", raising=False)
    assert sched._cwd_lock_timeout_seconds() == 660.0
    monkeypatch.setenv("ORION_CRON_TIMEOUT", "1800")
    assert sched._cwd_lock_timeout_seconds() == 1860.0
    monkeypatch.setenv("ORION_CRON_TIMEOUT", "30")
    assert sched._cwd_lock_timeout_seconds() == 180.0
    monkeypatch.setenv("ORION_CRON_TIMEOUT", "0")
    assert sched._cwd_lock_timeout_seconds() == 660.0
    monkeypatch.setenv("ORION_CRON_TIMEOUT", "bogus")
    assert sched._cwd_lock_timeout_seconds() == 660.0

...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("orion_cli.config.get_config_path", return_value=config_path), \
             patch("orion_cli.config.ensure_orion_home"):
    ...
    def test_ensure_orion_home_sets_0700(self):
        home = Path(self.tmpdir) / ".orion"
        with patch("orion_cli.config.get_orion_home", return_value=home):
            from orion_cli.config import ensure_orion_home
            ensure_orion_home()

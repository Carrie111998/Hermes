...
manual force-run (`orion cron run <id>`).
...
def cron_env(tmp_path, monkeypatch):
    ...
    (orion_home / "scripts").mkdir()
    monkeypatch.setenv("ORION_HOME", str(orion_home))

    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ORION_DIR", orion_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", orion_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", orion_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", orion_home / "cron" / "output")
    ...
    script = orion_home / "scripts" / "probe.py"
    script.write_text("print('ok')\n")

    return {"home": orion_home, "job_id": job["id"]}
...
class TestStaleInflightSelfHeal:
    def _setup(self, cron_env, monkeypatch):
        ...
        monkeypatch.setattr(S, "_orion_home", env["home"])
        return S, E, env

...
def test_update_job_normalizes_docker_image_clear(cron_env):
    """update_job itself (the `hermes cron edit --docker-image ""` lane) normalizes
    a blank docker_image to None, rather than storing "" (PR #90633 review item 2)."""
    from cron.jobs import create_job, update_job, get_job

    jid = create_job(prompt="x", schedule="every 1h", docker_image="img:1")["id"]
    update_job(jid, {"docker_image": ""})   # direct update_job, bypassing the tool layer
    assert get_job(jid).get("docker_image") is None

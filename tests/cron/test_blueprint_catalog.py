...
the shared /blueprint command handler (orion_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated ORION_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("orion://blueprint/morning-brief?")

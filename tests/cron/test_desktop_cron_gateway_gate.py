...
def test_multiplex_ticker_defers_only_the_profile_with_a_live_gateway(monkeypatch, tmp_path):
    """Regression guard: the #66629 gate must be scoped PER PROFILE in the
    multiplex path, not applied as one global gate over every profile.

    The zero-arg ``_no_live_gateway`` probes only this PROCESS's own
    HERMES_HOME lock. Passing it straight through as ``can_dispatch`` to
    ``InProcessCronScheduler._start_multiplex`` would defer EVERY profile's
    tick whenever any single profile has a live gateway — silently regressing
    d9a48f656a (sleeping profiles keep firing) for every sibling profile that
    has no live gateway of its own. Here profile A has a live gateway holding
    its own lock and profile B has none: B must keep ticking every cycle while
    A stays deferred.
    """
    import gateway.status as status
    from cron.scheduler_provider import InProcessCronScheduler
    from hermes_cli.web_server import _no_live_gateway_for_home
    ...
    # Profile A's own gateway.lock records a live gateway; profile B has none.
    _write_record(home_a / "gateway.lock", _make_live_gateway_record(home_a))
    monkeypatch.setattr(
        status,
        "_record_matches_live_gateway_pid",
        lambda record, pid, *, expected_home=None: True,
    ...
    def fake_tick(*args, **kwargs):
        ...
        ticked_homes.append(get_hermes_home_override())
        return 0
    ...
    with patch("cron.scheduler.tick", side_effect=fake_tick):
        t = threading.Thread(
            ...
            kwargs={
                "interval": 0.01,
                "profile_homes": [("a", home_a), ("b", home_b)],
                "per_home_can_dispatch": _no_live_gateway_for_home,
            },
            daemon=True,
        )
        t.start()
        try:
            # Two ticks of B is enough to show it keeps firing every cycle,
            # not just once before the gate catches up.
            assert _wait_until(lambda: ticked_homes.count(str(home_b)) >= 2, timeout=2.0), (
                f"profile B (no live gateway) ticked only {ticked_homes.count(str(home_b))} "
                "time(s) within 2s — the per-home gate should never defer it"
            )
        finally:
            stop.set()
            t.join(timeout=5)
    ...
    # Profile A (live gateway) must never have been dispatched to.
    assert str(home_a) not in ticked_homes, (
        "profile A ticked despite its own gateway holding the runtime lock — "
        "the per-home gate did not defer it"
    )

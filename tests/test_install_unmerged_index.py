...
The ``orion update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m orion_cli.main gateway run`` — image name
    ``pythonw``, not ``orion.exe`` — so the ``taskkill /IM orion.exe`` guard
    ...
    # The orion.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM orion.exe' in text

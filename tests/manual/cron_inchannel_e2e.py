...
Uses a throwaway ORION_HOME so it never touches ~/.orion. No real names.
"""
...
def _fresh_home():
    """Point ORION_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_orion_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ORION_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ORION_HOME is set.
...
# a different get_orion_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ORION_HOME (throwaway): {HOME}")

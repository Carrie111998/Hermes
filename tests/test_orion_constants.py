"""Tests for orion_constants module."""
...
from orion_constants import (
    ...
    find_orion_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_orion_root,
    get_orion_dir,
    get_orion_home,
    get_process_orion_home,
    heal_orion_managed_node,
...
class TestGetDefaultOrionRoot:
    """Tests for get_default_orion_root() — Docker/custom deployment awareness."""

    def test_no_orion_home_returns_native(self, tmp_path, monkeypatch):
        """When ORION_HOME is not set, returns ~/.orion."""
        monkeypatch.delenv("ORION_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_orion_root() == tmp_path / ".orion"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ORION_HOME=<root>/profiles/<name>),
        ...
        assert get_default_orion_root() == docker_root

    @pytest.mark.windows_only
    def test_no_orion_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\orion, not ~/.orion."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ORION_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_orion_root() == local_appdata / "orion"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; ORION_HOME / home changes invalidate.

        get_default_orion_root() resolves ORION_HOME against the native
        ...
        (native home, ORION_HOME) compared for free each call.
        """
        # ORION_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_orion_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            orion_constants, "_default_orion_root_memo", None, raising=False
        )

        first = get_default_orion_root()
        ...
        # ORION_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("ORION_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_orion_root() == docker_root
        assert resolve_calls["n"] > before, (
            "ORION_HOME change must force a fresh resolution"
        )
...
class TestGetOrionHome:
    """Tests for get_orion_home() platform-aware fallback."""

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ORION_HOME is unset on Windows, use %LOCALAPPDATA%\\orion."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ORION_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(orion_constants, "_profile_fallback_warned", False)

        assert get_orion_home() == local_appdata / "orion"


class TestGetProcessOrionHome:
    """Tests for get_process_orion_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_orion_home() == home




class TestOrionManagedNode:
    @pytest.mark.windows_only
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_orion_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_HOME", str(home))
        monkeypatch.setattr(orion_constants, "node_tool_runnable", lambda path: True)

        assert find_orion_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(orion_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(orion_constants, "heal_orion_managed_node", lambda: False)
        monkeypatch.setattr(
            orion_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert orion_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Orion-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(orion_constants, "heal_orion_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = orion_constants._ORION_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = orion_constants.find_orion_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert orion_constants.find_orion_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert orion_constants.find_orion_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(orion_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(orion_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".orion"
...
class TestGetOrionDir:
    """Tests for ``get_orion_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_orion_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_orion_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert orion_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert orion_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert orion_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert orion_constants.windows_path_to_wsl("/home/alex") is None
        assert orion_constants.windows_path_to_wsl("relative\\dir") is None
    ...
    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        ...
        assert orion_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert orion_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert orion_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(orion_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(orion_constants.sys, "platform", "darwin")
        monkeypatch.setenv("ORION_HOME", str(tmp_path))
        assert orion_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert orion_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("ORION_HOME", str(home))
        monkeypatch.setenv(
            "ORION_NODE_TARGET_MAJOR",
            str(orion_constants._ORION_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(orion_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            orion_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            orion_constants,
        ...
        monkeypatch.setattr(
            orion_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(orion_constants._ORION_NODE_TARGET_MAJOR)
        ...
        result = orion_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(orion_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_orion_managed_node() is False
        assert orion_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_orion_managed_node() is False
        assert orion_constants._managed_node_heal_attempted is True

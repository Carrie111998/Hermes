...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.orion/orion-agent/utils.py)
...
2. ``orion update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``orion_cli/main.py`` (the ``orion update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``orion_constants`` after the

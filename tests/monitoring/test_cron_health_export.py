...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("orion.gateway.up"), _M("orion.gateway.active_agents"),
        _M("orion.gateway.busy"), _M("orion.gateway.drainable"),
        _M("orion.gateway.restart_requested"),
        _M("orion.platform.up"), _M("orion.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("orion.cron.scheduler.heartbeat_age_seconds"),
        _M("orion.cron.scheduler.last_success_age_seconds"),
        _M("orion.cron.scheduler.catch_up_occurrences"),
        _M("orion.cron.jobs.enabled"), _M("orion.cron.jobs.running"),
        _M("orion.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Orion Agent-owned Relay transport health" in text

...
- ``orion verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``orion verify --json`` when the workspace
...
from orion_cli.verify_cmd import run_verify_command
...
def orion_home(tmp_path, monkeypatch):
    monkeypatch.setenv("ORION_HOME", str(tmp_path / ".orion-home"))
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        orion_dir = project / ".orion"
        orion_dir.mkdir()
        (orion_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(orion_home):
    project = _workspace(orion_home)
    ...
    assert status["evidence"]["canonical_command"] == "orion verify"


def test_record_verify_run_records_failure(orion_home):
    project = _workspace(orion_home)
...
def test_cli_passing_run_writes_ledger_evidence(orion_home, capsys):
    project = _workspace(orion_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(orion_home, capsys):
    project = _workspace(orion_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(orion_home, capsys):
...
def test_cli_run_uses_orion_session_id_env(orion_home, capsys, monkeypatch):
    monkeypatch.setenv("ORION_SESSION_ID", "sess-42")
    project = _workspace(orion_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> orion verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(orion_home, capsys):
    project = _workspace(orion_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `orion verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_orion_verify_when_recipe_has_start(orion_home):
    project = _workspace(orion_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "orion verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_orion_verify_when_manifest_exists(orion_home):
    # No start script, but a saved .orion/environment.json qualifies.
    project = _workspace(
        orion_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(orion_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(orion_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "orion verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(orion_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(orion_home, capsys):
    project = _workspace(orion_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(orion_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(orion_home, capsys):
    project = _workspace(orion_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)

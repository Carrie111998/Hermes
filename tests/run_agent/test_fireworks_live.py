"""Live Fireworks smoke test — exercises the Orion runtime, not a raw SDK client.

Opt-in only:
    ORION_LIVE_TESTS=1 FIREWORKS_API_KEY=fw_... \\
        pytest tests/run_agent/test_fireworks_live.py -q

Unlike a bare OpenAI() client pointed at the endpoint, this drives Orion'
...
def _resolve_runtime_client(provider="fireworks"):
    """Build the Fireworks client the way the Orion runtime does."""
    from agent.auxiliary_client import resolve_provider_client

    client, model = resolve_provider_client(provider)
    assert client is not None, "Orion failed to build a Fireworks client"
    return client, model


def test_orion_wires_fireworks_client():
...
def test_fireworks_basic_chat_through_runtime():
    """A single-turn completion via the Orion-resolved client returns text."""
    client, model = _resolve_runtime_client()

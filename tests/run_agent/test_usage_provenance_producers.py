...
def test_provider_dispatch_preregisters_unknown_then_replaces_same_component(monkeypatch):
    agent = _make_loop_agent(
        monkeypatch,
    ...
    def provider_call(_kwargs):
        ...
        pending_component_ids.append(receipts[0].component_id)
        return _openai_response(response_id="chatcmpl-dispatch-terminal")
    ...
    assert len(receipts) == 1
    assert receipts[0].component_id == pending_component_ids[0]
    assert receipts[0].provenance is UsageProvenance.MEASURED
    assert agent._progress_telemetry.usage_aggregate.unknown_component_count == 0
...
def test_malformed_provider_usage_is_unknown_not_measured_zero(monkeypatch):
    response = _openai_response()
    response.usage = SimpleNamespace(status="meter unavailable")
    agent = _make_loop_agent(
        monkeypatch,
        api_mode="chat_completions",
        provider="openrouter",
    ...
    assert receipt.value is None
    assert receipt.authoritative is False
    assert receipt.reason == "provider_usage_missing_or_malformed"


def test_mapping_provider_usage_preserves_measured_tokens(monkeypatch):
    response = _openai_response()
    response.usage = {
        "prompt_tokens": 11,
        "completion_tokens": 7,
...
def test_fractional_provider_usage_is_unknown_not_truncated_to_zero(monkeypatch):
    response = _openai_response()
    response.usage = {
        "prompt_tokens": 0.5,
        "completion_tokens": 0,
...
def test_mixed_valid_and_negative_provider_usage_fails_closed(monkeypatch):
    response = _openai_response(response_id="chatcmpl-invalid-usage")
...
def test_explicit_zero_provider_meter_is_measured(monkeypatch):
    response = _openai_response()
...
def test_replayed_provider_response_id_preserves_each_dispatch_attempt(monkeypatch):
    ...
    assert len({receipt.component_id for receipt in receipts}) == 2
    assert {receipt.accepted_event_id for receipt in receipts} == {"chatcmpl-replayed"}
    assert all(receipt.provenance is UsageProvenance.MEASURED for receipt in receipts)
...
def test_codex_app_server_records_measured_receipt_at_real_seam(monkeypatch):
    ...
    assert receipt.component_id.startswith("codex-app-server-attempt:")
    assert receipt.accepted_event_id == "turn-provenance-1"
    assert receipt.session_id == agent.session_id
    assert receipt.provenance is UsageProvenance.MEASURED
...
def test_codex_app_server_preregisters_unknown_before_provider_dispatch(monkeypatch):
    turn_result = TurnResult(
        final_text="done",
        projected_messages=[{"role": "assistant", "content": "done"}],
        turn_id="turn-preregistered",
        thread_id="thread-preregistered",
        token_usage_last={
            "totalTokens": 7,
            "inputTokens": 4,
            "outputTokens": 3,
    ...
    agent._codex_session = _InspectingCodexSession()

    with patch.object(agent, "_spawn_background_review", return_value=None):
        result = agent.run_conversation("preregister usage")
...
def test_codex_app_server_failed_dispatch_keeps_one_terminal_unknown_attempt(monkeypatch):
    agent = _make_codex_app_server_agent(monkeypatch, turn_result=None)

    class _FailingCodexSession:
        def run_turn(self, user_input: str, **kwargs):
            receipts = [
                receipt
                for receipt in agent._progress_telemetry.usage_aggregate.components
    ...
    agent._codex_session = _FailingCodexSession()

    result = agent.run_conversation("timeout")
    ...
    assert len(receipts) == 1
    assert receipts[0].provenance is UsageProvenance.UNKNOWN
    assert receipts[0].reason == "provider_request_failed"
    assert agent.session_api_calls == 1


def test_codex_app_server_missing_usage_records_each_dispatch_attempt(monkeypatch):
    turn_result = TurnResult(
            final_text="done",
            projected_messages=[{"role": "assistant", "content": "done"}],
    ...
    assert len(receipts) == 2
    assert len({receipt.component_id for receipt in receipts}) == 2
    assert all(receipt.session_id == agent.session_id for receipt in receipts)
    assert all(receipt.provenance is UsageProvenance.UNKNOWN for receipt in receipts)
    assert all(receipt.value is None for receipt in receipts)
    assert all(receipt.reason == "provider_usage_missing" for receipt in receipts)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"inputTokens": 1, "outputTokens": 2, "totalTokens": "invalid"},
        {
            "inputTokens": 1,
            "outputTokens": 2,
            "totalTokens": 3,
...
def test_codex_runtime_malformed_or_partial_usage_fails_closed(usage, monkeypatch):
    turn_result = TurnResult(
        final_text="done",
        projected_messages=[{"role": "assistant", "content": "done"}],
        turn_id="turn-codex-malformed",
        thread_id="thread-codex",
        token_usage_last=usage,
    )
    agent = _make_codex_app_server_agent(monkeypatch, turn_result)
    agent.model = "subscription-model"
    agent.provider = "subscription-lane"
    agent.base_url = "http://subscription.invalid"

    with patch.object(agent, "_spawn_background_review", return_value=None):
        result = agent.run_conversation("malformed usage")

    assert result["final_response"] == "done"
    assert agent.session_total_tokens == 0
    aggregate = agent._progress_telemetry.usage_aggregate
    assert aggregate.provenance is UsageProvenance.UNKNOWN
    assert aggregate.known_component_count == 0
    assert aggregate.components[0].reason == "provider_usage_malformed"

...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_orion_attribution(mock_openai):
    """Keyless requests still identify as Orion (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Orion Agent"
    assert str(headers.get("User-Agent", "")).startswith("OrionAgent/")

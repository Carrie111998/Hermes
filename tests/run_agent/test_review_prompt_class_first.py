...
def _assert_unresolved_failure_guidance(prompt: str, label: str) -> None:
    """An unsuccessful task must not be reframed as validated guidance."""
    lower = prompt.lower()
    assert "unresolved failures" in lower, f"{label}: must name unresolved failures"
    assert "working method" in lower, f"{label}: must require a working method"
    assert "check manually" in lower, f"{label}: must recognize an unresolved outcome"
    assert "never the dead ends" in lower, f"{label}: must exclude failed attempts"
    assert "independently confident" in lower, (
        f"{label}: must limit exceptions to known working alternatives"
    )

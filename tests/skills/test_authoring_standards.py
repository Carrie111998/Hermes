...
def test_required_frontmatter_fields(p):
    ...
    orion = (fm.get("metadata") or {}).get("orion") or {}
    if not (orion.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.orion.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    orion = (fm.get("metadata") or {}).get("orion") or {}
    dangling = [
        rs for rs in (orion.get("related_skills") or []) if rs not in _all_names()
    ]

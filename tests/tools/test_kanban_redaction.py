...
def worker_env(monkeypatch, tmp_path):
    """Isolated ORION_HOME with a running task; returns the task id."""
    home = tmp_path / ".orion"
    home.mkdir()
    monkeypatch.setenv("ORION_HOME", str(home))
    monkeypatch.setenv("ORION_PROFILE", "test-worker")
    monkeypatch.delenv("ORION_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ORION_KANBAN_TASK", tid)
    return tid
...
# ---------------------------------------------------------------------------
# Negative test — force=True bypasses ORION_REDACT_SECRETS=false
# ---------------------------------------------------------------------------

def test_scrub_respects_force_flag_regardless_of_config(worker_env, monkeypatch):
    """force=True must fire even when ORION_REDACT_SECRETS=false is set."""
    monkeypatch.setenv("ORION_REDACT_SECRETS", "false")
    from tools import kanban_tools as kt
    from orion_cli import kanban_db as kb
    secret = "ghp_" + "C" * 40
    kt._handle_comment({"task_id": worker_env, "body": f"token: {secret}"})

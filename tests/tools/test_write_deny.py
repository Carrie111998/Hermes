...
class TestWriteDenyExactPaths:
    ...
    def test_orion_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.orion/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "orion_root"
        ...
        # Sanity check: ORION_HOME does point to the profile dir, not the root.
        from orion_constants import get_orion_home, get_default_orion_root
        assert get_orion_home() == profile_home
        assert get_default_orion_root() == root
...
class TestWriteAllowed:
    ...
    def test_orion_control_files_requested_writable(self):

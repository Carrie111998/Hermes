...
def test_xai_credentials_fail_closed_without_profile_scope(tmp_path, monkeypatch):
    ...
    from orion_cli.config import invalidate_env_cache
...
def test_prefer_api_key_honors_orion_xai_base_url_with_validation(monkeypatch):
    """The preferred-key path reads the same override pair as the OAuth
    branch (ORION_XAI_BASE_URL first, then XAI_BASE_URL) behind the same
    ...
    monkeypatch.setattr(
        ...
        lambda name, default=None: {
            ...
            "ORION_XAI_BASE_URL": "https://staging.x.ai/v1",
...
def test_prefer_api_key_honors_profile_scope_only_key(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ORION_XAI_BASE_URL", raising=False)
    invalidate_env_cache()
    previous_multiplex = secret_scope.is_multiplex_active()

...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    ...
    # Budgeted execution / handoff semantics
    assert "one batched discovery sweep" in prompt
    assert "goal_mode=false" in prompt
    assert "technical handoff" in prompt
    assert "exact-SHA CI" in prompt
    assert "merge approval" in prompt
    assert "deployment approval" in prompt

...
Real config files in a temp ORION_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated ORION_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".orion"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "ORION_LOCAL_STT_LANGUAGE",
    ):

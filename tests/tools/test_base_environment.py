...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_portable_mktemp_not_shell_pid(self):
        """mktemp is unique across writers and works in macOS Bash 3.2."""
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
    ...
    def test_temp_template_static_part_is_quoted(self):
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH.  Crucially each writer uses its own ``mktemp``
    allocation; a shared ``$$``-derived name would still tear here.
    """
    ...
    def test_failed_export_does_not_destroy_good_snapshot(self, tmp_path):
        ...
        bad_tmp = '"$__hermes_snap_tmp"'
        script = (
            f"if __hermes_snap_tmp=$(mktemp {bad_template}); then "
            f"{{ export -p > {bad_tmp} && mv -f {bad_tmp} {_q(snap)}; }} "
            f"2>/dev/null || rm -f {bad_tmp} 2>/dev/null || true; fi"
        )
        self._run(script)

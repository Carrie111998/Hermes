...
class TestAbiStamp:
    ...
    def test_readonly_target_reports_error(self, tmp_path, monkeypatch):
        # Simulate a read-only target deterministically. chmod is not a valid
        # permission boundary when the suite runs as root (as it does in the
        # project container), so inject the OSError at the target mkdir call.

...
from tools.code_execution_tool import (
    ...
    _uses_orion_python_environment,
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        self.assertIn("orion_sandbox_", result["output"])
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The orion-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_orion_tools(self):
        """Regression: orion_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from orion_tools import terminal`.
        """
        import tempfile
    ...
    def test_strict_mode_can_still_import_orion_tools(self):
...
# ---------------------------------------------------------------------------
# _python_environment_prefix / _uses_orion_python_environment
# ---------------------------------------------------------------------------
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the orion root from
...
class TestUsesOrionPythonEnvironment(unittest.TestCase):
    """Unit tests for _uses_orion_python_environment."""
    ...
    def test_true_for_current_interpreter(self):
        ...
        self.assertTrue(_uses_orion_python_environment(sys.executable))
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the orion root for the interpreter Orion itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_orion_python_environment(sys.executable))
    ...
    def test_false_for_different_prefix(self):
        ...
        with patch("tools.code_execution_tool._python_environment_prefix",
            ...
            self.assertFalse(_uses_orion_python_environment("/other/python"))

    def test_false_when_prefix_is_empty(self):
        """If prefix cannot be determined (error path), treat as external."""
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=""):
            self.assertFalse(_uses_orion_python_environment("/bad/python"))

    def test_true_when_prefix_matches_sys_prefix(self):
        orion_prefix = os.path.realpath(sys.prefix)
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=orion_prefix):
            self.assertTrue(_uses_orion_python_environment("/same/env/python"))


# ---------------------------------------------------------------------------
# PYTHONPATH composition — orion root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify orion root inclusion in PYTHONPATH depends on env match.

    Patches ``_uses_orion_python_environment`` directly so these tests are
    ...
    def test_orion_root_included_when_same_env(self):
        """When interpreter is in the Orion env, orion root is in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=True)
        parts = pythonpath.split(os.pathsep)
        self.assertIn(self._orion_root(), parts,
                      "orion root must be in PYTHONPATH for same-env interpreters")

    def test_orion_root_excluded_when_external_env(self):
        """When interpreter is external, orion root must NOT be in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=False)
        parts = pythonpath.split(os.pathsep)
        self.assertNotIn(self._orion_root(), parts,
                         "orion root must not leak into an external interpreter's PYTHONPATH")

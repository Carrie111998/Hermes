...
def test_deeply_nested_skill_add_invalidates_within_ttl(tmp_path):
    _write_skill(tmp_path, "cat-a/nested", "skill-one")
    assert [s["name"] for s in st._find_all_skills()] == ["skill-one"]

    # The old one-level signature saw only the already-existing `nested`
    # directory, so this mutation stayed stale until TTL expiry.
    _write_skill(tmp_path, "cat-a/nested", "skill-two")
    names = sorted(s["name"] for s in st._find_all_skills())
    assert names == ["skill-one", "skill-two"]


def test_symlinked_category_skill_add_invalidates_within_ttl(tmp_path):
    ...
    _write_skill(tmp_path, "linked", "skill-one")
    assert [s["name"] for s in st._find_all_skills()] == ["skill-one"]

    # Discovery follows category symlinks; its cache signature must follow the
    # same topology rather than waiting up to 30 seconds.
    _write_skill(tmp_path, "linked", "skill-two")
    names = sorted(s["name"] for s in st._find_all_skills())
    assert names == ["skill-one", "skill-two"]


def test_symlink_cycle_is_pruned_without_duplicate_discovery(tmp_path):
    _write_skill(tmp_path, "cat-a", "skill-one")
    skills_root = tmp_path / "skills"
    (skills_root / "cat-a" / "loop").symlink_to(
        skills_root,

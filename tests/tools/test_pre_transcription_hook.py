...
import orion_cli.plugins as plugins_mod
from tools import transcription_tools


PROMPT = "Orion, Teknium, Orion, kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("orion_cli.plugins.has_hook", lambda name: True)
    monkeypatch.setattr("orion_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("orion_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("orion_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.orion_xai_user_agent", lambda: "test-ua",
        )
...
def test_real_fixture_plugins_thread_prompt_in_registration_order(
    ...
    orion_home = Path(os.environ["ORION_HOME"])
    plugin_dir = orion_home / "plugins" / "stt_vocab"

...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("orion_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("ORION_SESSION_KEY", session_key)
        monkeypatch.setenv("ORION_EXEC_ASK", "1")
        monkeypatch.delenv("ORION_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("orion-verify-", "orion-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "orion-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/orion-verify-example.py",
            "rm -f /tmp/orion-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../orion-verify-example.py",
            "rm -f /tmp/a/../../tmp/orion-verify-example.py",
            "rm -f /var/tmp/orion-verify-example.py",
            "rm -f /tmp/orion-verify-*",
            "rm -f /tmp/orion-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/orion-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/orion-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\orion-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\orion-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\orion-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ORION_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.orion/.env",
            "echo x | tee $ORION_HOME/.env",
            'echo x | tee "$ORION_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestOrionConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.orion/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.orion/config.yaml",
            "echo '  mode: off' >> ~/.orion/config.yaml",
            "echo x | tee ~/.orion/config.yaml",
            "echo x | tee $ORION_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.orion/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Orion absolute config.yaml is
        # handled by the project patterns, not the Orion-home rule.
        for cmd in (
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.orion/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Orion-home prefixes must fold to ~/ and
    ~/.orion/ in dangerous-command detection.
    ...
    SSH, and Orion config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_orion_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/ORION_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("ORION_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.orion/orion-agent && source venv/bin/activate && python -m orion_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m orion_cli.main gateway run --replace &",
            "nohup python -m orion_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart orion-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Orion security config via IFS.
            "sed${IFS}-i ~/.orion/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep orion) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof orion)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof orion_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof orion`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Orion service
    label achieves the same effect as `orion gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_orion_label_detected(self):
        for cmd in (
            "launchctl stop ai.orion.gateway",
            "launchctl kickstart -k system/ai.orion.gateway",
            "launchctl bootout system/ai.orion.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.orion.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Orion labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ORION_GATEWAY_SESSION", "ORION_CRON_SESSION",
                      "ORION_YOLO_MODE",
                      "ORION_SESSION_KEY", "ORION_INTERACTIVE")
        }
        os.environ.pop("ORION_YOLO_MODE", None)
        os.environ.pop("ORION_INTERACTIVE", None)
        # ORION_CRON_SESSION takes priority over ORION_GATEWAY_SESSION in
        ...
        os.environ.pop("ORION_CRON_SESSION", None)
        os.environ["ORION_GATEWAY_SESSION"] = "1"
        os.environ["ORION_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("orion_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ORION_INTERACTIVE": "1"}, clear=False):

...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        orion_home.mkdir()
        (orion_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(orion_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.orion/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        (orion_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.orion/skills"


    def test_custom_container_base(self, tmp_path):
        orion_home = tmp_path / ".orion"
        (orion_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ORION_HOME": str(orion_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.orion")

        assert mounts[0]["container_path"] == "/home/user/.orion/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.orion/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.orion/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ORION_HOME."""
        monkeypatch.setenv("ORION_HOME", str(tmp_path / ".orion"))
        (tmp_path / ".orion").mkdir()

        # Create a sensitive file one level above orion_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside orion_home
    ...
    def test_nested_subdir_inside_orion_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ORION_HOME must be allowed."""
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        subdir = orion_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ORION_HOME pointing outside must be rejected."""
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        monkeypatch.setenv("ORION_HOME", str(orion_home))

        # Create a sensitive file outside orion_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside orion_home pointing outside
        symlink = orion_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ORION_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, orion_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ORION_HOME."""
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        monkeypatch.setenv("ORION_HOME", str(orion_home))
        ...
        sensitive.write_text("{}")
        self._write_config(orion_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(orion_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ORION_HOME via config must still mount."""
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        monkeypatch.setenv("ORION_HOME", str(orion_home))

        (orion_home / "oauth.json").write_text("{}")
        self._write_config(orion_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        (orion_home / "cache" / "documents").mkdir(parents=True)
        (orion_home / "cache" / "audio").mkdir(parents=True)
        (orion_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ORION_HOME", str(orion_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.orion/cache/documents" in paths
        assert "/root/.orion/cache/audio" in paths
        assert "/root/.orion/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_orion_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        # Use legacy dir name with content — get_orion_dir prefers
        # populated old over new.
        legacy_doc = orion_home / "document_cache"
        legacy_img = orion_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(orion_home / "document_cache") in host_paths
        assert str(orion_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.orion/cache/documents" in container_paths
        assert "/root/.orion/cache/images" in container_paths
    ...
    def test_empty_orion_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.orion/attachments" in container_paths
        assert "/root/.orion/images" in container_paths
        assert "/root/.orion/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``ORION_HOME/images``, not
        ...
        """
        orion_home = tmp_path / ".orion"
        (orion_home / "images").mkdir(parents=True)
        monkeypatch.setenv("ORION_HOME", str(orion_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.orion/images" in by_container
        assert by_container["/root/.orion/images"] == str(orion_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / ".orion"
        (orion_home / "images").mkdir(parents=True)
        upload = orion_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("ORION_HOME", str(orion_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.orion/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        orion_home = tmp_path / ".orion"
        img_dir = orion_home / "cache" / "images"
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(orion_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.orion/cache/images/x.png"
        assert (orion_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Orion cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        orion_home = tmp_path / ".orion"
        (orion_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("ORION_HOME", str(orion_home))
        return str(orion_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_orion(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.orion/attachments/drop.zip"

    def test_ssh_maps_to_tilde_orion(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.orion/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / ".orion"
        doc_dir = orion_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — ORION_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping ORION_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"ORION_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.orion/google_token.json" in paths
        assert "/root/.orion/.env" not in paths

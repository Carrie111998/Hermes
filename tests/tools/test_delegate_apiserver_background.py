...
def test_after_turn_batch_delivers_ready_children_without_waiting_and_coalesces(monkeypatch):
    ...
    parsed = json.loads(
        dt.delegate_task(
            tasks=[{"goal": "zero"}, {"goal": "one"}, {"goal": "two"}],
            background=True,
    ...
    assert "done: zero" in first_text
    assert "done: one" in first_text
    assert "done: two" not in first_text

...
The native-Windows classes carry ``@pytest.mark.windows_only`` so the Windows
CI lane (``tests-os.yml``, ``-m windows_only``) discovers and executes them on
a real Windows host — they read on-disk shim fixtures whose paths use Windows
backslash separators, which only resolve under ``ntpath``.  The real-node class
layers a ``node.exe``-availability skip on top of the marker.  Portable
no-op/fallback tests stay unmarked so the Linux lane keeps the fail-safe paths
covered too.
"""
...
# even THIS argument would be silently rewritten.
_NPX_PREFIX_TAIL = [
    "--ignore-scripts",
    "--prefer-offline",
    "-y",

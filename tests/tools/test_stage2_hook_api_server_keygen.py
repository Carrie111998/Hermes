...
was gated on ``[ -f "$ORION_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'ORION_HOME="{home}"\n'
        # In tests we run unprivileged; as_orion is a passthrough then.
        'as_orion() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Orion loads $ORION_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/orion/.env.example -> $ORION_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $ORION_HOME/.env from it on first boot"
    )

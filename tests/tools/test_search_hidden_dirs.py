...
@pytest.mark.skipif(
    os.name == "nt",
    reason="GNU find semantics under test; on Windows shell=True resolves "
    "System32 find.exe (a different tool) unless Git usr/bin is on PATH",
)
...
@pytest.mark.skipif(
    os.name == "nt",
    reason="GNU grep --exclude-dir semantics under test; grep is not a "
    "native Windows tool",
)

...
class TestDockerHostBindApproval:
    ...
    def test_host_bound_docker_without_human_surface_blocks(self, monkeypatch):
        """Host-bound Docker uses structural authority and fails closed."""
        import tools.approval as A
        monkeypatch.setenv("HERMES_EXEC_ASK", "1")
        res = A.check_all_command_guards("rm -rf /workspace", "docker",
    ...
    def test_execute_code_host_bound_docker_requires_approval(self, monkeypatch):
        ...
        assert res.get("status") == "blocked"
        assert res.get("outcome") == "exact_plan_capability_required"

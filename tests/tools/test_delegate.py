...
class TestDelegationFallbackChain(unittest.TestCase):
    ...
    Mirrors the auxiliary fallback pattern (auxiliary.<task>.fallback_chain):
    delegation config's fallback chain takes precedence over the parent's
    _fallback_chain. When empty, the old pinned-vs-inherit rules apply.
    """

    def _capture_fallback(self, parent, delegation_cfg, override_provider=None, override_model=None):
        ...
        with patch(patch_target, return_value=delegation_cfg):
            with patch("run_agent.AIAgent") as MockAgent:
                ...
                if override_model is not None:
                    kwargs["override_model"] = override_model
                _build_child_agent(
                    task_index=0,
    ...
    def test_delegation_fallback_chain_alias(self):
        """Auxiliary parity: delegation.fallback_chain (list) is honoured."""
        parent = _make_mock_parent(depth=0)
        parent._fallback_chain = []
        delegation_chain = [{"provider": "nous", "model": "hermes-4-405b"}]

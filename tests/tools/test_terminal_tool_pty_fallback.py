...
def test_terminal_background_never_injects_external_delivery_receipt_env(monkeypatch, tmp_path):
    config = _base_config(tmp_path)
    dummy_env = SimpleNamespace(env={"PRESERVED": "yes"})
    captured = {}
    ...
    assert captured["command"] == "python3 background.py"
    assert captured["env_vars"] == {"PRESERVED": "yes"}
    assert EXTERNAL_DELIVERY_ENV not in captured["env_vars"]

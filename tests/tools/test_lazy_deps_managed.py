...
A package-manager install (NixOS, and anything else shipping Orion from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.orion/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("orion_cli.config.get_managed_system", lambda: "nixos")
...
def test_unmanaged_install_is_not_blocked_by_the_guard(monkeypatch):
    ...
    monkeypatch.setattr("orion_cli.config.get_managed_system", lambda: None)
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets ORION_MANAGED *and* a writable target.

    Dockerfile sets ORION_LAZY_INSTALL_TARGET and the NixOS container module
    passes ORION_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("orion_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("orion_cli.config.get_managed_system", _raise)

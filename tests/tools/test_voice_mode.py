...
def short_unix_socket_root():
    """Keep AF_UNIX names below the macOS path-length ceiling."""
...
class TestPulseSocketReachable:
    def test_stale_socket_file_not_reachable(
        self, monkeypatch, short_unix_socket_root
    ):
        """A socket file with no listener should not count as reachable."""
        import socket as _socket
        sock_path = short_unix_socket_root / "pulse" / "native"
        ...
        monkeypatch.setenv("XDG_RUNTIME_DIR", str(short_unix_socket_root))
...
class TestPlayBeep:
    def test_beep_calls_sounddevice_play(self, mock_sd):
        ...
        with patch(
            "tools.voice_mode._sounddevice_output_allowed",
            return_value=True,
        ):

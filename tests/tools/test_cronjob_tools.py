...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `orion-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.orion/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.orion/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        monkeypatch.setenv("ORION_INTERACTIVE", "1")
        ...
        # because orion uses an internal scheduler, not system crontab.
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["ORION_INTERACTIVE", "ORION_GATEWAY_SESSION", "ORION_EXEC_ASK"],
    )
    ...
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("ORION_INTERACTIVE", "ORION_GATEWAY_SESSION", "ORION_EXEC_ASK"):
...
class TestUnifiedCronjobTool:
    ...
    def _patch_named_legit(monkeypatch):
        import orion_cli.runtime_provider as rp
...
class TestAgentCannotSetModelPin:
    """Per-job inference pins are user-owned (dashboard / `orion cron`
...
class TestLocalDeliveryNotice:
    ...
    def _setup_cron_dir(self, tmp_path, monkeypatch):
        ...
        for var in (
            ...
            "ORION_SESSION_CHAT_ID",
            "ORION_SESSION_THREAD_ID",
            "ORION_SESSION_CHAT_NAME",
        ):
...
class TestGithubExemptionAbuse:
    ...
    def test_same_line_payload_after_github_url_is_scanned(self):
        ...
        for sep in (";", " &&", " |"):
            prompt = f"{self.GH}{sep} cat ~/.orion/.env"
    ...
    def test_subshell_and_backtick_payloads_are_scanned(self):
        ...
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}$(cat ~/.orion/.env)")
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}`cat ~/.orion/.env`")
    ...
    def test_payload_between_two_github_blocks_is_scanned(self):
        ...
        prompt = f"{self.GH}; cat ~/.orion/.env; {self.GH}"
        assert "Blocked" in _scan_cron_prompt(prompt)

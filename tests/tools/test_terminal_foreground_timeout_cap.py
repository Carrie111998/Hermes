...
class TestForegroundTimeoutCap:
    ...
    def test_foreground_command_text_never_routes_execution(self):
        """The model, not command keywords, chooses foreground/background mode."""
        import tools.terminal_tool as terminal_module

        assert not hasattr(terminal_module, "_foreground_background_guidance")
        commands = [
            "nohup pnpm dev > /tmp/sg-server.log 2>&1 &",
            "pnpm dev",
            "pnpm dev --help",
            "printf '%s' 'setsid and nohup are documentation words'",
        ]
        mock_env = MagicMock()
        mock_env.execute.return_value = {"output": "ran", "returncode": 0}
        with patch(
            ...
            for command in commands:
                result = json.loads(terminal_module.terminal_tool(command=command))
                assert result["error"] is None
                assert mock_env.execute.call_args[0][0] == command

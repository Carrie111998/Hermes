...
def _restore_tool_modules():
    original_orion_home = os.environ.get("ORION_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_orion_home is None:
            os.environ.pop("ORION_HOME", None)
        else:
            os.environ["ORION_HOME"] = original_orion_home
        _reset_modules(("tools", "orion_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    orion_cli = types.ModuleType("orion_cli")
    orion_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["orion_cli"] = orion_cli
    orion_home = tmp_path / "orion-home"
    os.environ["ORION_HOME"] = str(orion_home)
    sys.modules["orion_cli.config"] = types.SimpleNamespace(
        get_orion_home=lambda: orion_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="orion-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": orion_home / "modal_snapshots.json",

"""Regression test for the ``OrionMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_orion_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _ORION_PROVIDER_CLS, reset_manager_for_tests

    assert _ORION_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
@pytest.mark.asyncio
async def test_orion_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
    ...
    assert _ORION_PROVIDER_CLS is not None

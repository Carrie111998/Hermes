...
# ---------------------------------------------------------------------------
# _host_local × elevated interaction (upstream dd401e0 semantic coexistence)
# ---------------------------------------------------------------------------


def test_elevated_with_host_local_resolves_to_local_and_dispatches(monkeypatch):
    """_host_local=True must force the host backend for the elevated gate.

    Upstream dd401e0 made the actual backend ``env_type = "local" if
    _host_local else config["env_type"]``. The elevated local-only gate must
    validate that RESOLVED value, not the raw config value: a control-plane
    host-local child with elevated=True (config says docker) still reaches
    the elevated executor, and the shared guard still sees env_type="local".
    """
    guard_calls = []
    monkeypatch.setattr(
        terminal_tool, "_get_env_config",
        lambda: _make_minimal_config(env_type="docker"),
    )
    monkeypatch.setattr(terminal_tool, "_active_environments", {"default": _fake_env()})
    monkeypatch.setattr(terminal_tool, "_last_activity", {})
    monkeypatch.setattr(terminal_tool, "_start_cleanup_thread", lambda: None)
    ...
    def _guard(command, env_type, **kwargs):
        guard_calls.append(env_type)
        return {"approved": True, "status": "approved", "description": ""}

    monkeypatch.setattr(terminal_tool, "_check_all_guards", _guard)
    ...
    monkeypatch.setattr(
        ...
        }) or {"output": "host-local admin ok\n", "exit_code": 0, "error": None},
    )

    result = terminal_tool.terminal_tool(
        command="whoami", elevated=True, _host_local=True,
    )
    data = json.loads(result)

    # Resolved backend is local → elevated dispatch happens via the executor
    # (never via the FakeEnv.execute, which raises on use).
    assert data["exit_code"] == 0
    assert "host-local admin ok" in data["output"]
    assert len(executor_calls) == 1
    assert executor_calls[0]["command"] == "whoami"
    assert isinstance(executor_calls[0]["cwd"], str)
    # The guard validated the RESOLVED env_type ("local"), not config["docker"].
    assert guard_calls, "shared guard must still be consulted under _host_local"
    assert all(env_type == "local" for env_type in guard_calls)


def test_elevated_with_host_local_still_respects_guards(monkeypatch):
    """Guards bind on the host-local backend too: blocked → no dispatch."""
    monkeypatch.setattr(
        terminal_tool, "_get_env_config",
        lambda: _make_minimal_config(env_type="docker"),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda command, env_type, **kwargs: {
            "approved": False,
            "status": "blocked",
            "description": "hardline blocked",
            "message": "Command denied: safety rule",
        },
    )
    ...
    result = terminal_tool.terminal_tool(
        command="rm -rf /", elevated=True, _host_local=True,
    )
    data = json.loads(result)

    assert data["status"] == "blocked"
    assert executor_calls == [], "blocked command must never reach the elevated executor"

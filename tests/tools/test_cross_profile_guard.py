...
@pytest.fixture
def fake_orion(tmp_path, monkeypatch):
    """Build a two-profile Orion layout and point ORION_HOME at
    the orion-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-orion"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "orion-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("ORION_HOME", str(sec_home))

    import orion_constants
    monkeypatch.setattr(orion_constants, "get_default_orion_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_orion_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_orion):
        from tools.file_tools import write_file_tool
        target = fake_orion["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_orion):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_orion["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "orion-security" in result["error"]
        # File untouched.
        assert target.read_text() == original


    def test_non_orion_path_unaffected(self, fake_orion, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_orion):
    ...
    def test_cross_profile_patch_bypass(self, fake_orion):
        from tools.file_tools import patch_tool
        target = fake_orion["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_orion):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_orion, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_orion["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up ORION_HOME (set in
        ...
        assert "not found in active profile 'orion-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.orion/profiles/<name>/."""
        # Don't set ORION_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_orion_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_orion_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_orion):
        """When active profile is 'orion-security', the prompt warns
        ...
        assert "Active Orion profile" in src
        assert "cross_profile=True" in src
        assert "~/.orion/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Orion profile: default" in src
        assert "Active Orion profile: {active_profile}" in src

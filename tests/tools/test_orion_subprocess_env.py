"""Tests for orion_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_ORION_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
from tools.environments.local import (
    orion_subprocess_env,
    _ALWAYS_STRIP_KEYS,
    _ORION_PROVIDER_ENV_FORCE_PREFIX,
)
...
_TIER1_SAMPLE = {
    ...
    "ORION_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return orion_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "ORION_KANBAN_TASK": "t_parent",
                "ORION_KANBAN_RUN_ID": "123",
                "ORION_KANBAN_DB": "/tmp/parent-kanban.db",
                "ORION_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
        ...
        assert env["ORION_DELEGATED_CHILD_CONTEXT"] == "1"
        assert "ORION_KANBAN_TASK" not in env
        assert "ORION_KANBAN_RUN_ID" not in env
        assert "ORION_KANBAN_DB" not in env
        assert "ORION_KANBAN_WORKSPACE" not in env

...
Covers:
- :class:`OrionTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.OrionMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _ORION_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# OrionTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = OrionTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ORION_HOME", str(tmp_path))
        storage = OrionTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (OrionMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: OrionTokenStorage, **context_attrs):
    ...
    provider = _ORION_PROVIDER_CLS.__new__(_ORION_PROVIDER_CLS)
    provider._orion_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_HOME", str(tmp_path))
        storage = OrionTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ORION_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_HOME", str(tmp_path))
        storage = OrionTokenStorage("flow-srv")
        ...
        with patch.object(
            _ORION_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",

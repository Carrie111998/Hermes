...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ORION_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ORION_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_orion_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ORION_HOME": "/h", "ORION_PROFILE": "p",
        "ORION_CONFIG": "/c.yaml", "ORION_ENV": "/e",
        "ORION_DELEGATED_CHILD_CONTEXT": "1",
        # other ORION_* → dropped (broad prefix removed)
        "ORION_BASE_URL": "https://x", "ORION_INTERACTIVE": "1",
        "ORION_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "ORION_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ORION_BASE_URL", "ORION_INTERACTIVE", "ORION_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ORION_* vars."""
    import logging

...
def ledger_env(tmp_path, monkeypatch):
    """Isolated ORION_HOME + skills dir for skill_manage and the ledger."""
    ...
    monkeypatch.setattr(skill_ledger, "get_orion_home", lambda: home)
    monkeypatch.setattr(skill_usage, "get_orion_home", lambda: home)
...
def test_rollback_refuses_paths_outside_orion_home(ledger_env):
    """A hand-edited ledger entry pointing outside ORION_HOME must not
...
def test_config_gate_off_no_ledger_writes(ledger_env, monkeypatch):
    ...
    import orion_cli.config as _cfg

    ...
    def test_retryable_failure_uses_configured_fallback(self, monkeypatch, error):
        ...
        monkeypatch.setattr(
            ...
            lambda *_args, **_kwargs: pytest.fail(
                "keyless rescue must run after configured fallbacks"
            ),
        )
        ...
        assert primary.calls == [("query", 10)]
        assert fallback.calls == [("query", 10)]

    def test_non_retryable_failure_does_not_fallback(self, monkeypatch):
        from tools import web_tools

        monkeypatch.setattr(web_tools, "_rescue_eligible", lambda _provider: False)
        result, primary, fallback = self._run(
            monkeypatch,

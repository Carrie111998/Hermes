...
Older images made /opt/orion/ui-tui and /opt/orion/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ORION_TUI_DIR=/opt/orion/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R orion:orion /opt/orion/ui-tui" not in text
    assert "chown -R orion:orion /opt/orion/node_modules" not in text

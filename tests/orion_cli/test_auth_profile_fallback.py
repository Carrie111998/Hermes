...
When ``ORION_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.orion/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.orion            (has its own auth.json fixture)
    * Profile     -> tmp_path/.orion/profiles/coder   (active, ORION_HOME points here)
...
def test_missing_global_auth_file_is_safe(profile_env):
    ...
    from orion_cli.auth import read_credential_pool
...
# ``_load_provider_state`` itself, those helpers raise ``"Orion is not
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from orion_cli.auth import read_credential_pool, write_credential_pool
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (ORION_HOME != ~/.orion, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    orion_home = tmp_path / "classic"
    orion_home.mkdir()
...
def test_write_pool_never_merges_cooldown_onto_reauthed_entry(classic_env):
    ...
    """
    from orion_cli.auth import write_credential_pool

...
from orion_cli.setup_whatsapp_cloud import (
...
def isolated_home(tmp_path, monkeypatch):
    """Redirect ORION_HOME so save_env_value writes into a temp .env."""
    home = tmp_path / "home"
    orion = home / ".orion"
    orion.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("ORION_HOME", str(orion))
...
def _env_value(orion_home: Path, key: str) -> str | None:

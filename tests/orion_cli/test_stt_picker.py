"""Tests for the Speech-to-Text category in `orion tools` (tools_config).
...
from orion_cli.tools_config import (  # noqa: E402
...
class TestSttCategory:
    ...
    def test_managed_row_shares_tts_coverage_category(self):
        from orion_cli.nous_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY
...
class TestConfigWrites:
    ...
    def test_apply_provider_selection_stt(self):
        config = {}
        with patch(
            "orion_cli.tools_config.get_nous_subscription_features"
        ) as feats:
...
class TestModelPicker:
    ...
    def test_configure_stt_model_defaults_to_current(self):
        ...
        with patch(
            "orion_cli.tools_config._prompt_choice", return_value=0
        ) as pc:
...
class TestPostSetup:
    def test_faster_whisper_in_post_setup_ready(self):
        from orion_cli.tools_config import _POST_SETUP_READY

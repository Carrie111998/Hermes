...
from orion_cli import config as orion_config
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(orion_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(orion_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(orion_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(orion_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(orion_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(orion_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(orion_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(orion_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "orion_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    orion_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_orion_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import orion_constants

    monkeypatch.delattr(orion_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(orion_constants, "apply_subprocess_home_env")

    orion_main._reload_updated_runtime_modules()

    assert callable(orion_constants.apply_subprocess_home_env)
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        orion_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        orion_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(orion_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        orion_main.cmd_update(SimpleNamespace())
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        orion_main, "_discard_stashed_changes",
        lambda *a, **kw: discard_calls.append(1) or True,
    )
    monkeypatch.setattr(
        orion_config, "load_config",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        orion_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "orion_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    orion_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    orion_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_update_parser_accepts_keep_stash():
    ...
    from orion_cli.subcommands.update import build_update_parser
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``orion update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(orion_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".orion-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by orion update (orion_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "orion-update-autostash")

    assert marker.exists(), (
        ".orion-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".orion-bootstrap-complete" not in status

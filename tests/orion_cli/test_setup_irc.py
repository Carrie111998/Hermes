"""Tests for IRC gateway configuration via `orion setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Orion install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Orion install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#orion")
            monkeypatch.setenv("IRC_NICKNAME", "orion-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `orion setup gateway` experience with IRC present."""

...
install``. A locked ``orion.exe`` makes the install exit 2, the handler
...
from orion_cli import main as orion_main
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(orion_main, "_is_windows", lambda: True)
    exc = _cpe([r"C:\venv\Scripts\uv.exe", "pip", "install", "-e", "."])
...
def test_posix_git_failure_does_not_zip(monkeypatch):
    monkeypatch.setattr(orion_main, "_is_windows", lambda: False)
    exc = _cpe(["git", "pull"], returncode=1)
    assert update_cmd._should_zip_fallback_on_update_error(exc) is False
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M orion_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(orion_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
    ...
    with patch("urllib.request.urlretrieve") as download:
        with pytest.raises(SystemExit) as exc_info:
            orion_main._update_via_zip(SimpleNamespace(branch=None))
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.orion-update-staging/")
    assert is_artifact("?? cli.py.orion-update-staging")
    assert is_artifact("?? tools.orion-update-old/")
    ...
    assert not is_artifact(" M orion_cli/update_cmd.py")
    assert not is_artifact("?? notes.orion-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.orion-update-staging/\n?? cli.py.orion-update-old\n"),
    )
    assert (

"""Behavior tests for orion_cli.inventory.
...
from orion_cli.inventory import (
    ConfigContext,
...
def _list_auth_returning(rows: list[dict]):
    ...
    return patch(
        "orion_cli.model_switch.list_authenticated_providers",
        return_value=rows,
    )
...
def test_list_authenticated_providers_force_fresh_is_keyword_only():
    ...
    from orion_cli.model_switch import list_authenticated_providers
...
def test_include_unconfigured_appends_canonical_skeletons():
    ...
    # skeleton rows.
    from orion_cli.models import CANONICAL_PROVIDERS
...
def test_explicit_only_filters_ambient_credentials_but_keeps_current_and_custom_rows():
    ...
    with (
        ...
        patch("orion_cli.config.read_raw_config", return_value={}),
        patch(
            "orion_cli.auth.is_provider_explicitly_configured",
...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and Orion' own device flow
    ...
    with (
        ...
        patch(
            "orion_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from orion_cli.inventory import _anthropic_oauth_credentials_present

    with (
        patch(
            "agent.anthropic_adapter.read_orion_oauth_credentials",
            return_value=None,
        ),
        patch(
        ...
        patch(
            "orion_cli.auth.read_credential_pool",
            return_value=[
...
def test_end_to_end_with_real_context_no_credentials_leak(monkeypatch):
    ...
    with patch("orion_cli.config.load_config", return_value=cfg):
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("orion_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from orion_cli import model_switch

    with patch("orion_cli.models.clear_provider_models_cache") as clear:
...
def _apply_featured_with_dates(rows, dates: dict[str, str]):
    ...
    from orion_cli import inventory

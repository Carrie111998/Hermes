"""Tests for the Command Installation check in orion doctor."""
...
import orion_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal ORION_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".orion"
    ...
    orion_bin = venv_bin_dir / "orion"
    orion_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    orion_bin.chmod(0o755)

    monkeypatch.setattr(doctor_mod, "ORION_HOME", home)
    ...
    # Stub auth checks
    try:
        from orion_cli import auth as _auth_mod
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, orion_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "orion"
        wrong_target = tmp_path / "wrong_orion"
        ...
        assert cmd_link.resolve() == orion_bin.resolve()

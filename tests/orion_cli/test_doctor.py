...
import orion_cli.doctor as doctor
import orion_cli.gateway as gateway_cli
from orion_cli import doctor as doctor_mod
from orion_cli.doctor import _has_provider_env_config
...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull nousresearch/orion-agent:latest" in hint
        assert "recreate all Orion containers" in hint
        assert "orion update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `orion update`" in hint

    def test_sqlite_upgrade_hint_uses_pkg_for_apt_managed_install(self):
        hint = doctor._sqlite_upgrade_hint("apt")

        assert "run `pkg upgrade orion-agent`" in hint
        assert "orion update" not in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `orion doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "ORION_HOME", orion_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("ORION_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_orion_home(self, tmp_path, provider=""):
        """Create a minimal ORION_HOME with config.yaml."""
        home = tmp_path / ".orion"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_orion_home(tmp_path, provider)
...
def test_run_doctor_accepts_orion_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (orion_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".orion"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`orion doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp ORION_HOME.

        ``run_doctor`` reads the module-level ``ORION_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("ORION_HOME")`` alone
        leaves doctor probing the REAL ~/.orion. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from orion_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("orion_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "orion_cli.doctor", raising=False)
        from orion_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ORION_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"ORION_MAX_ITERATIONS={ghost}\n")
        (orion_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "ORION_HOME", orion_home)
        monkeypatch.setattr(doctor_mod, "get_orion_home", lambda: orion_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ORION_MAX_ITERATIONS", str(os_environ_value))
        else:
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ORION_MAX_ITERATIONS" in out
        env_after = (orion_home / ".env").read_text(encoding="utf-8")
        assert "ORION_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_orion_tool_progress_warning_says_unsupported_since_floor(self):
        """ORION_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"ORION_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["ORION_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"ORION_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["ORION_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        orion_home = tmp_path / ".orion"
        orion_home.mkdir(parents=True)
        (orion_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (orion_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"ORION_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: ORION_TOOL_PROGRESS_MODE" in out

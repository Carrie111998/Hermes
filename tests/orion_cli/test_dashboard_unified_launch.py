...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "orion_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # ORION_HOME.  For a standard install (ORION_HOME unset) that root is
        # the platform-native default (~/.orion), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from orion_constants import get_default_orion_root
        assert env.get("ORION_HOME") == str(get_default_orion_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (ORION_DESKTOP=1) must NOT

...
``orion*.exe`` shims aside (``orion.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``orion`` vanished from PATH
...
from orion_cli import _install_repair as ir
from orion_cli import main as cli_main


def _make_scripts_dir(tmp_path: Path) -> Path:
    scripts = tmp_path / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in ("orion", "orion-agent", "orion-acp", "orion-gateway"):
...
# ---------------------------------------------------------------------------
# orion_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "orion.exe" in names, "orion.exe must be restored after a no-op install"
    assert "orion-acp.exe" in names
    assert "orion-gateway.exe" in names
    assert (scripts / "orion.exe").read_bytes() == b"MZ-old-orion"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "orion.exe").read_bytes() == b"MZ-new-orion"
...
# ---------------------------------------------------------------------------
# orion_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "orion.exe" in names, "orion.exe must be restored after a no-op recovery install"
    assert (scripts / "orion.exe").read_bytes() == b"MZ-old-orion"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "orion.exe").write_bytes(b"MZ-new-orion")

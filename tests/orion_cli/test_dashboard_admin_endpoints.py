...
def _client():
    ...
    # Keep the state DB under the isolated ORION_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_orion_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        config_text = (orion_home / "config.yaml").read_text()
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.orion/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `orion auth remove`: clean up the backing
        source and suppress (provider, source).
        """
        from agent.credential_pool import load_pool
        from orion_cli.auth import is_source_suppressed
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `orion auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_orion_home):
        ...
        (get_orion_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_orion_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_orion_action", fake_spawn_action)
    ...
    def test_enable_platform_reuses_inflight_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_orion_action", fail_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "orion_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("orion-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # orion-index source advertises availability + featured search.
            idx = _Src("orion-index")
        ...
        assert {"official", "github", "orion-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "orion_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestAdminEndpointsAuthGate:
    ...
    def _setup(self, _isolate_orion_home):
        from starlette.testclient import TestClient
        from orion_cli.web_server import app
...
class TestUpdateCheckEndpoint:
    """``GET /api/orion/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/orion/update`` runs ``orion update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/orion/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/orion/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("orion_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `orion tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_orion_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_orion_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _ORION_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_orion_action stores
    ...
    ws._spawn_orion_action(["gateway", "restart"], "gateway-restart")

    assert "_ORION_GATEWAY" not in captured["env"]
    assert captured["env"]["ORION_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_orion_home
):
    ...
    startup under ORION_DESKTOP=1 so the stale orphan is cleared first.
    """
    import orion_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from orion_cli.gateway, so patch it
    # on that module.
    import orion_cli.gateway as g

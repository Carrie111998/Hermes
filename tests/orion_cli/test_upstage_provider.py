...
Regression guard for the bug where `orion model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`orion_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        from orion_cli.providers import ORION_OVERLAYS

        assert "upstage" in ORION_OVERLAYS
        overlay = ORION_OVERLAYS["upstage"]
...
class TestUpstageEnvCatalog:
    ...
    def test_optional_env_vars_include_upstage(self):
        from orion_cli.config import OPTIONAL_ENV_VARS
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `orion_cli/main.py` (cmd_model): read

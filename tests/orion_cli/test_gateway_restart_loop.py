...
Covers:
- Defense 1: gateway stop/restart refuse when _ORION_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "orion gateway stop",
        "orion  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "ORION GATEWAY RESTART",           # uppercase
    ])
    def test_orion_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.orion.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.orion/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l orion-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.orion.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.orion.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.orion/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated orion note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "orion cron list",
        "orion update",
        "orion config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `orion gateway start` is benign — starting a gateway from inside a
        ...
        "orion gateway start",
        "orion gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway orion
        # services must not be falsely blocked (the old `.*orion` matched any
        # orion token).
        "launchctl unload ai.orion.update-checker.plist",
        "launchctl restart ai.orion.daemon",
        ...
        "systemctl restart orion-meta.service",
        "systemctl restart orion-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_orion_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade orion then run orion gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_orion_gateway_once.sh). The script must live under
        # ORION_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ORION_HOME", str(tmp_path / ".orion"))
        scripts_dir = tmp_path / ".orion" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify orion gateway stop/restart refuse when _ORION_GATEWAY=1."""
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ORION_GATEWAY=1.

    Issue #37453: systemctl --user restart orion-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart orion-gateway",
        "systemctl --user restart orion-gateway",
        "systemctl stop orion-gateway.service",
        "orion gateway restart",
        "launchctl kickstart gui/501/ai.orion.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.orion.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.orion/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.orion.gateway.restart-once.plist",
        "pkill -f orion.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart orion-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.orion.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-orion label: label-independent detection is the point
        # (#62891 second reproduction used `ai.orion.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.orion.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.orion.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.orion.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-orion systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run orion gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("orion gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeorion gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ORION_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.orion.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("orion gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/orion\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 orion gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_orion_home()` falls back to Path.home(), which raises
        RuntimeError when neither ORION_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    orion_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run orion gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ORION_HOME", str(tmp_path / ".orion"))
        (tmp_path / ".orion").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run orion gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell orion gateway restart"',
        'psql -c "\\! systemctl restart orion-gateway"',
        "grep 'systemctl restart orion-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart orion-gateway",
        'grep "$(systemctl restart orion-gateway)" f',
        "grep 'restart' log; systemctl restart orion-gateway",
        'sqlite3 db "SELECT 1"; orion gateway stop',
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart orion-gateway%'\""
        )

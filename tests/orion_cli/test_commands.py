...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /orion <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_orion_norm = {_norm(n) for n in _SLACK_VIA_ORION_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_orion_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `orion slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "orion_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from orion_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch orion_cli.plugins.get_plugin_commands() to a fixed dict."""

"""Tests for `orion memory setup [provider]` routing.
...
``orion memory setup honcho``) without the interactive picker — which matters
because the per-provider ``orion <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "orion memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets ORION_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"ORION_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("orion_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("orion_cli.tools_config.subprocess.run", fake_run):

...
from orion_cli.console_engine import OrionConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/orion-console-test.zip",
    "import /tmp/orion-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_orion_home):
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(orion_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
    ...
    result = OrionConsoleEngine().execute(
...
def test_sessions_export_all_uses_per_session_budget(
    ...
    monkeypatch.setattr(orion_state, "resolved_max_export_messages", lambda: 3)
...
def test_sessions_export_zero_limit_disables_guard(
    ...
    monkeypatch.setattr(orion_state, "resolved_max_export_messages", lambda: 0)
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_orion_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_orion_home):
    ...
    assert code == 0
    assert "Orion Console" in stdout.getvalue()
    assert "orion>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from orion_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_orion_home):
    result = OrionConsoleEngine().execute(
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_orion_home, monkeypatch
):
    ...
    result = OrionConsoleEngine().execute("checkpoints prune", confirmed=True)

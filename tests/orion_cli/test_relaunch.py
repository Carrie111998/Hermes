"""Tests for orion_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from orion_cli import relaunch as relaunch_mod


class TestResolveOrionBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/orion"
        ...
        assert relaunch_mod.resolve_orion_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'orion' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_orion_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/orion" if name == "orion" else None
        )
        assert relaunch_mod.resolve_orion_bin() == "/usr/bin/orion"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_orion_bin", lambda: "/usr/bin/orion")
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/orion", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/orion", ["/usr/bin/orion", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        orion).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_orion_bin", lambda: r"C:\Users\test\orion.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\orion.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\orion.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_orion_bin", lambda: r"C:\orion.exe")
...
class TestResolveOrionBinWindowsPyGuard:
    """On Windows, resolve_orion_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a orion.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\orion.exe" if name == "orion" else None,
        )

        bin_path = relaunch_mod.resolve_orion_bin()
        # Must NOT be the .py — must be the orion.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\orion.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "orion"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_orion_bin() == str(script)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_orion_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND orion.exe
        ...
        assert relaunch_mod.resolve_orion_bin() is None

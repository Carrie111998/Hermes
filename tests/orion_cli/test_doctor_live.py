"""Tests for ``orion doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from orion_cli import doctor_live
from orion_cli.doctor_live import (
    ProbeResult,
...
class TestLiveFlagGating:
    def test_parser_has_live_flag_default_false(self):
        from orion_cli.subcommands.doctor import build_doctor_parser
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `orion doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("orion_cli.doctor.ORION_HOME", tmp_path / "home")
        monkeypatch.setattr("orion_cli.doctor.PROJECT_ROOT", tmp_path / "root")

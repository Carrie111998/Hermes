...
orion_cli.config.load_config, so the managed overlay has to be wired into each.
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ORION_MANAGED_DIR", str(managed))
    import orion_cli.config as cfg
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # orion_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("ORION_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import orion_time

    assert orion_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (ORION_TIMEZONE, ORION_REDACT_SECRETS, ORION_MAX_ITERATIONS,
    ...
    # writing ORION_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml

...
def test_prompt_model_selection_requires_expensive_confirmation(monkeypatch, capsys):
    from orion_cli.auth import _prompt_model_selection

    monkeypatch.setattr("orion_cli.curses_ui.curses_radiolist", _raise_menu)
    monkeypatch.setattr(
        "orion_cli.model_cost_guard.expensive_model_warning",
...
def test_prompt_model_selection_uses_line_editor_for_custom_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "orion_cli.curses_ui.curses_radiolist",
        lambda _title, choices, **_kwargs: len(choices) - 2,
    )
    monkeypatch.setattr(
        "orion_cli.cli_output.line_input",
        lambda prompt_text: (
...
def test_remove_custom_provider_falls_back_on_menu_runtime_error(tmp_path, monkeypatch):
    from orion_cli.main import _remove_custom_provider

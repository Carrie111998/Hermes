...
interactive ``orion model`` CLI picker.

The CLI picker (``orion_cli.main.select_provider_and_model``) builds its
...
def config_home(tmp_path, monkeypatch):
    """Isolated ORION_HOME with a minimal config."""
    home = tmp_path / "orion"
    home.mkdir()
    ...
    monkeypatch.delenv("ORION_INFERENCE_PROVIDER", raising=False)
...
def _capture_provider_labels(config_home):
    ...
    with patch("orion_cli.main._prompt_provider_choice",
...
def test_cli_picker_hides_excluded_provider(config_home):
    ...
    from the ``orion model`` provider menu."""
...
def test_cli_picker_hides_excluded_provider_by_alias(config_home):
    ...
    orion_id / alias names."""
    # 'openai' is an alias-style orion id; ensure excluding it hides the
    # canonical openai provider row if present. Use the canonical slug's
    # alias from _PROVIDER_ALIASES to stay robust to renames.
    from orion_cli.models import _PROVIDER_ALIASES, CANONICAL_PROVIDERS
    ...
    from orion_cli.models import _PROVIDER_LABELS

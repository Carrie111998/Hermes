...
A credential added via `orion auth add openrouter` lives in the credential
...
def _clean_inference_env(monkeypatch):
    ...
    for key in (
        ...
        "NOUS_API_KEY",
        "ORION_INFERENCE_PROVIDER",
    ):
        monkeypatch.delenv(key, raising=False)


def _seed_openrouter_pool(token: str = "sk-or-FAKEKEY123") -> None:
    """Mimic `orion auth add openrouter <token>` — a manual pool entry."""
...
def test_auto_detects_openrouter_from_pool(tmp_path, monkeypatch):
    ...
    (tmp_path / "orion").mkdir(parents=True, exist_ok=True)
    _seed_openrouter_pool()

    from orion_cli.auth import resolve_provider

...
subparser, placing the flag BEFORE the subcommand (e.g. 'orion --yolo chat')
...
def _build_parser():
    """Build the orion argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("orion_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "orion_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets ORION_YOLO_MODE regardless of flag position.
    ...
    def test_yolo_before_chat_sets_env(self):
        ...
        assert os.environ.get("ORION_YOLO_MODE") == "1"
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `orion gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: orion_cli.main is a very heavy
    # import (previously 11 separate `python -m orion_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `orion <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `orion -t web chat` silently dropped the
    ...
    Uses the real `orion_cli._parser.build_top_level_parser()` rather than
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `orion <flag> <value> chat`:\n  "

...
from orion_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    ORION_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def orion_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("orion_cli.config") or mod == "orion_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, orion_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, orion_home, capsys, monkeypatch):

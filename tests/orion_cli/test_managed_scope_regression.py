...
def orion_home(tmp_path, monkeypatch):
    ...
    # /etc/orion on the dev/CI box can't influence the test.
    monkeypatch.setenv("ORION_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(orion_home, monkeypatch):
...
def test_env_expansion_in_user_config(orion_home, monkeypatch):
    from orion_cli.config import load_config, cfg_get

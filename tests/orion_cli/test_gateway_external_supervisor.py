...
def _clear_native_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("ORION_S6_SUPERVISED_CHILD", raising=False)
...
def test_update_follows_wrapper_upgrade_of_stale_plist_argv(monkeypatch):
    ...
    from orion_cli.stderr_timestamp import _prepare_child_command
    ...
    upgraded = _prepare_child_command(
        stale, {"XPC_SERVICE_NAME": "ai.orion.gateway-work"}
    )

...
def test_run_xai_oauth_login_from_setup_does_not_hijack_active_provider(
    ...
    `active_provider`), so `orion setup tts` OAuth login hijacked the main
    chat provider.
    """
    ...
    config_path = orion_home / "config.yaml"
    ...
    monkeypatch.setattr(
        "orion_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
    ...
    monkeypatch.setattr("orion_cli.auth._is_remote_session", lambda: True)

    from orion_cli.auth import is_source_suppressed, suppress_credential_source
    from orion_cli.setup import _run_xai_oauth_login_from_setup

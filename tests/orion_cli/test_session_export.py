...
from orion_cli.session_export import export_record_count, render_sessions_export
...
def test_sessions_export_cli_prompt_only_stdout(monkeypatch, capsys):
    ...
    monkeypatch.setattr(orion_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        ["orion", "sessions", "export", "-", "--session-id", "sess", "--only", "user-prompts"],
    )

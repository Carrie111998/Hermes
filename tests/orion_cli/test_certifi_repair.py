...
1. The venv-repair import probes (early recovery + `orion update`) must
...
2. ``orion doctor`` must fail the certificate check in that state, and
   ``orion doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
import orion_cli._early_recovery as er
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `orion update`'s venv repair must apply
    ...
    def _run_probe_script(self, monkeypatch, tmp_path, bundle_path):
        ...
        from orion_cli import main as main_mod
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "orion doctor --fix" in message

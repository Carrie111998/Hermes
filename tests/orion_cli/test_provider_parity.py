...
the SAME provider universe as ``orion model`` (the CLI/TUI picker).
...
from orion_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Orion-Session-Token": _SESSION_TOKEN}
...
def test_every_orion_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `orion model` universe."""
    ...
    assert not missing, (
        "providers shown in `orion model` but not configurable in the desktop "

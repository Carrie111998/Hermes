"""Tests for orion backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew orion_cli.backup's datetime forward instead of sleeping.
    ...
    import orion_cli.backup as _backup

    shim = getattr(_backup.datetime, "_orion_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _orion_test_shim = True
...
def _make_orion_tree(root: Path) -> None:
    """Create a realistic ~/.orion directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "orion_state.db"):
    ...
    # orion-agent repo (should be EXCLUDED)
    (root / "orion-agent").mkdir(exist_ok=True)
    (root / "orion-agent" / "run_agent.py").write_text("# big file\n")
    (root / "orion-agent" / ".git").mkdir()
    (root / "orion-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_orion_agent(self):
        from orion_cli.backup import _should_exclude
        assert _should_exclude(Path("orion-agent/run_agent.py"))
        assert _should_exclude(Path("orion-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from orion_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import orion_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = orion_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, orion_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ORION_HOME."""
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        _make_orion_tree(orion_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(orion_home / "skills" / "outside-link.txt", outside)
        ...
        from orion_cli.backup import run_backup
        run_backup(args)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        orion_home.mkdir()
        _make_orion_tree(orion_home)
        with sqlite3.connect(orion_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from orion_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(orion_home=orion_home)
        assert snap_id and (orion_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Orion backup."""
        from orion_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_auto_installs_gateway_service(self, tmp_path, monkeypatch):
        ...
        from orion_cli.backup import run_import
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Orion configuration has been restored." in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / ".orion"
        (orion_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("ORION_HOME", str(orion_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (orion_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (orion_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            orion_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (orion_home / "gateway.pid").write_text("4242")
        (orion_home / "processes.json").write_text('{"live": true}')
        ...
        assert (orion_home / "gateway.pid").read_text() == "4242"
        assert (orion_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (orion_home / "cron.pid").exists()
        assert not (orion_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (orion_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".orion"
        src_home.mkdir(parents=True)
        _make_orion_tree(src_home)

        monkeypatch.setenv("ORION_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from orion_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".orion"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ORION_HOME", str(dst_home))
        ...
        # orion-agent should NOT be present
        assert not (dst_home / "orion-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from orion_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from orion_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".orion/", "")
            zf.writestr(".orion/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_orion_home(self, tmp_path, monkeypatch):
        """Backup handles empty orion home (no files to back up)."""
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        # Only excluded dirs, no actual files
        (orion_home / "__pycache__").mkdir()
        (orion_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        (orion_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = orion_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / ".orion"
        orion_home.mkdir()
        (orion_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ORION_HOME", str(orion_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (orion_home / "config.yaml").exists()
        assert (orion_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`orion import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the ORION_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (orion_home / "config.yaml").write_text(original)
        monkeypatch.setenv("ORION_HOME", str(orion_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (orion_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(orion_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside ORION_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        orion_home = dst_home / ".orion"
        orion_home.mkdir()
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo orion import`` onto a user-owned (or Docker/NAS volume-owned)
        ORION_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "orion_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``orion import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from orion_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from orion_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from orion_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def orion_home(self, tmp_path):
        """Create a fake ORION_HOME with critical state files."""
        home = tmp_path / ".orion"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, orion_home):
        from orion_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(orion_home=orion_home)
        db_copy = orion_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, orion_home, monkeypatch, capsys):
        ...
        snap_id = backup_mod.create_quick_snapshot(orion_home=orion_home)
        err = capsys.readouterr().out
        assert "SQLite safe copy FAILED" in err or "CRITICAL" in err
        assert "state.db" in err
        # Other small files may still snapshot
        if snap_id:
            manifest = (orion_home / "state-snapshots" / snap_id / "manifest.json")
    ...
    def test_snapshot_includes_pairing_directories(self, orion_home):
        ...
        (orion_home / "platforms" / "pairing").mkdir(parents=True)
        (orion_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (orion_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (orion_home / "pairing").mkdir()
        (orion_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (orion_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = orion_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (orion update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, orion_home, capsys):
        ...
        """
        import json
        from orion_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", orion_home=orion_home)
        assert first_id is not None
        first_dir = orion_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", orion_home=orion_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = orion_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, orion_home=orion_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, orion_home):
        ...
        from orion_cli.backup import create_quick_snapshot, restore_quick_snapshot

        board_dir = orion_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(orion_home=orion_home)
        copy = (
            orion_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, orion_home=orion_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, orion_home, monkeypatch):
        ...
        import orion_cli.backup as bk
        from orion_cli.backup import create_quick_snapshot

        board = orion_home / "kanban" / "boards" / "work"
        ...
        copy = orion_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``orion update``
    ...
    def orion_home(self, tmp_path):
        root = tmp_path / ".orion"
        root.mkdir()
        _make_orion_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, orion_home):
        """Pre-update backup should include the same user data that
        ``orion backup`` would, and should exclude the same directories."""
        from orion_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(orion_home=orion_home)
        ...
        # orion-agent repo excluded
        assert not any(n.startswith("orion-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, orion_home):
        """``orion update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", orion_home=orion_home)
        assert snap_id and (orion_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, orion_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(orion_home=orion_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (orion_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, orion_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ORION_HOME."""
        from orion_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def orion_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".orion"
        root.mkdir()
        _make_orion_tree(root)
        # Point ORION_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ORION_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve ORION_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(orion_home, value):
        import yaml
        (orion_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(orion_home):
        d = orion_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(orion_home):
        d = orion_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, orion_home, capsys):
        ...
        self._set_mode(orion_home, "off")
        from orion_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(orion_home)
        assert not self._zips(orion_home)



    def test_config_full_mode(self, orion_home, capsys):
        self._set_mode(orion_home, "full")
        from orion_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(orion_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (orion claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``orion claw migrate`` runs before mutating ~/.orion/."""
    ...
    def test_restorable_with_orion_import(self, orion_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Orion
        backup — `orion import` should accept it."""
        from orion_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(orion_home=orion_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, orion_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from orion_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(orion_home=orion_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(orion_home=orion_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`orion update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, orion_home: Path, label="pre-update"):
        from orion_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, orion_home=orion_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from orion_cli.backup import restore_cron_jobs_if_emptied
        orion_home = tmp_path / ".orion"
        jobs_path = orion_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(orion_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, orion_home=orion_home)
        assert result is not None
...
# location, NOT under ORION_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, orion_home: Path) -> None:
        orion_home.mkdir(parents=True, exist_ok=True)
        (orion_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (orion_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (orion_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / ".orion"
        self._make_min_tree(orion_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ORION_HOME,
        ...
        # External state did NOT leak into ORION_HOME.
        assert not (orion_home / "_external").exists()

"""Tests for the Orion plugin system (orion_cli.plugins)."""
...
from orion_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<orion_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<orion_home>/plugins/``; we derive
    ``<orion_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target Orion home for this
    plugin isn't necessarily the current ``ORION_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``ORION_HOME``/``set_orion_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ORION_HOME>/config.yaml.
        # Config is always read from ORION_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            orion_home = Path(home)
        else:
            import os
            orion_home_str = os.environ.get("ORION_HOME")
            if orion_home_str:
                orion_home = Path(orion_home_str)
            else:
                orion_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_removed_relay_plugin_identity_cannot_be_reloaded(
        ...
        assert "Relay lifecycle is owned by Orion core" in state.error
        assert RELAY_PLUGINS_CONFIG_ENV in state.error
        assert "Refusing to load removed Orion Relay plugin" in caplog.text

    def test_enabled_portable_plugin_registers_components(
        self, tmp_path, monkeypatch
    ):
        from orion_cli.agent_plugins import MCP_SCHEMA_V1, PLUGIN_SCHEMA_V1
        from orion_cli import plugins as plugins_mod
    ...
    def test_disabled_portable_plugin_registers_nothing(self, tmp_path, monkeypatch):
        from orion_cli.agent_plugins import PLUGIN_SCHEMA_V1
        from orion_cli import plugins as plugins_mod
    ...
    def test_portable_author_object_is_normalized_to_stable_string(
        ...
        bundled.mkdir()
        monkeypatch.setenv("ORION_HOME", str(home))
        monkeypatch.setenv("ORION_BUNDLED_PLUGINS", str(bundled))
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "orion_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("ORION_HOME", str(tmp_path / "orion_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("orion_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under orion_plugins.<name>."""
        plugins_dir = tmp_path / "orion_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ORION_HOME", str(tmp_path / "orion_test"))

        # Clean up any prior namespace module
        sys.modules.pop("orion_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "orion_plugins.ns_plugin" in sys.modules
    ...
    def test_user_memory_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / "orion_test"
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``orion_agent.plugins`` entry point but exposing
        ...
        monkeypatch.setattr(
            "orion_cli.plugins.importlib.metadata.entry_points",
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $ORION_HOME/plugins/.
        orion_home = tmp_path / "orion_test"
...
class TestForceReloadSymmetry:
    ...
    def test_re_register_config_hooks_clears_idempotence_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "orion_cli.config.load_config", lambda: {"hooks": {}}
        )
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from orion_cli import observability
        from orion_cli.plugins import get_pre_tool_call_directive
...
class TestResolvePreToolBlock:
    ...
    def test_approve_gate_receives_tool_observability_context(self, monkeypatch):
        from orion_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("orion_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("orion_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from orion_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from orion_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("orion_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `orion plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_plugin_manager_scoped_by_orion_home_override(self, tmp_path):
        """set_orion_home_override() must get its own manager per profile.

        This is the production path used by the gateway multiplexer
        (``gateway/run.py``'s ``_profile_scope`` context manager) and by
        subagent/embedded callers: it swaps ``ORION_HOME`` via a
        context-local ContextVar, which — per
        ``orion_constants.set_orion_home_override`` — deliberately does
        NOT touch ``os.environ``. A regression test that only flips the
        ``ORION_HOME`` env var never exercises this path.
        """
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('ORION_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['ORION_HOME'] itself (simulating
        # a real plugin like orion-lcm capturing its home at registration),
        ...
        # not last-write-wins.
        token_a2 = set_orion_home_override(str(home_a))
        try:
            manager_a2 = plugins_mod.get_plugin_manager()
        finally:
            reset_orion_home_override(token_a2)
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``orion_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``orion_plugins.<slug>.state``. If a profile switch replaces only
        ...
        def write_stateful_plugin(home: Path, marker: str) -> None:
            ...
            # `orion_plugins.stateful_plugin.state`.
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("orion_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("orion_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("orion_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("orion_cli.plugins.PluginContext.dispatch_tool.__module__", "orion_cli.plugins"):
...
class TestPluginDebugLogging:
    """ORION_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ORION_PLUGINS_DEBUG", raising=False)
        from orion_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from ORION_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """ORION_HOME at the root resolves to 'default'."""
        home = tmp_path / ".orion"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """ORION_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".orion" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ORION_HOME", str(prof))

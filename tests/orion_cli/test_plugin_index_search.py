...
(remote → cache → seed), `orion plugins search --json`, and install-time
...
from orion_cli import plugin_index
...
SAMPLE = _index_doc(
    [
        ...
        },
        {
            ...
            "author": "Orion",
            "tags": ["example", "llm"],
            "repo": "Orion/orion-example-plugins",
...
def orion_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(plugin_index, "get_orion_home", lambda: tmp_path)
    return tmp_path
...
class TestParsing:
    def test_parses_object_form(self):
        ...
        assert [e.name for e in entries] == [
            "orion-media-studio",
            "orion-telegram-business",
            "plugin-llm-example",
        ]
        assert entries[2].subdir == "plugin-llm-example"
        assert entries[2].install_identifier == (
            "Orion/orion-example-plugins/plugin-llm-example"
        )
        assert entries[0].install_identifier == "Orion/orion-media-studio"
...
class TestSearch:
    ...
    def test_exact_name_ranks_first(self):
        results = search_index(self.entries, "orion-media-studio")
        assert results[0].name == "orion-media-studio"

    def test_matches_tags(self):
        results = search_index(self.entries, "telegram")
        assert results and results[0].name == "orion-telegram-business"

    def test_matches_description(self):
        results = search_index(self.entries, "secretary")
        assert [e.name for e in results] == ["orion-telegram-business"]

    def test_fuzzy_typo_tolerance(self):
        results = search_index(self.entries, "orion-media-studo")
        assert results and results[0].name == "orion-media-studio"
...
class TestLoadIndex:
    def test_fresh_cache_wins_without_network(self, orion_home, monkeypatch):
        _write_cache(orion_home, SAMPLE)
    ...
    def test_expired_cache_triggers_remote(self, orion_home, monkeypatch):
        _write_cache(orion_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_remote_failure_falls_back_to_stale_cache(self, orion_home, monkeypatch):
        _write_cache(orion_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_no_cache_no_remote_falls_back_to_seed(self, orion_home, monkeypatch):
        monkeypatch.setattr(plugin_index, "_fetch_remote", lambda: None)
        entries, source = load_index()
        assert source == "seed"
        assert len(entries) >= 3

    def test_refresh_bypasses_fresh_cache(self, orion_home, monkeypatch):
        _write_cache(orion_home, SAMPLE)
    ...
    def test_offline_skips_network(self, orion_home, monkeypatch):
    ...
    def test_corrupt_cache_ignored(self, orion_home, monkeypatch):
        cache = orion_home / "cache" / "plugin_index.json"
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_remote_fetch_writes_cache(self, orion_home, monkeypatch):
...
class TestResolveName:
    ...
    def test_exact_unique(self):
        entry, candidates = resolve_name(self.entries, "orion-media-studio")
        assert entry is not None and entry.repo == "Orion/orion-media-studio"

    def test_case_insensitive(self):
        entry, _ = resolve_name(self.entries, "Orion-Media-Studio")
        assert entry is not None

    def test_unique_partial(self):
        entry, _ = resolve_name(self.entries, "telegram")
        assert entry is not None and entry.name == "orion-telegram-business"

    def test_ambiguous_partial(self):
        entry, candidates = resolve_name(self.entries, "orion")
...
class TestInstallResolution:
    def test_bare_name_detection(self):
        from orion_cli.plugins_cmd import _looks_like_bare_index_name

        assert _looks_like_bare_index_name("orion-media-studio")
    ...
    def test_install_resolves_name_and_pins_ref(self, orion_home, monkeypatch):
        ...
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("orion-media-studio", enable=False)
        assert captured["identifier"] == "Orion/orion-media-studio"
        assert captured["ref"] == "e" * 40

    def test_install_explicit_ref_beats_index_pin(self, orion_home, monkeypatch):
        from orion_cli import plugins_cmd
        ...
        monkeypatch.setattr(plugins_cmd, "_install_plugin_core", fake_core)
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("orion-media-studio", enable=False, ref="d" * 40)
        assert captured["ref"] == "d" * 40

    def test_install_ambiguous_name_lists_candidates_and_exits(
        self, orion_home, monkeypatch, capsys
    ):
        from orion_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit) as exc:
            plugins_cmd.cmd_install("orion", enable=False)
        assert exc.value.code == 1
        assert not called
        out = capsys.readouterr().out
        assert "ambiguous" in out
        assert "orion-media-studio" in out
        assert "orion-telegram-business" in out

    def test_install_unknown_name_exits(self, orion_home, monkeypatch, capsys):
        from orion_cli import plugins_cmd
    ...
    def test_owner_repo_passthrough_skips_index(self, orion_home, monkeypatch):
...
class TestCmdSearch:
    def test_json_output(self, orion_home, monkeypatch, capsys):
        from orion_cli import plugins_cmd
        ...
        assert payload["results"][0]["name"] == "orion-telegram-business"
        assert payload["results"][0]["repo"] == "Orion/orion-telegram-business"
    ...
    def test_no_results_message(self, orion_home, monkeypatch, capsys):
        from orion_cli import plugins_cmd
    ...
    def test_dispatch_routes_search(self, orion_home, monkeypatch):
        from orion_cli import plugins_cmd

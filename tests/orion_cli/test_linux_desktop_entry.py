"""Tests for the Linux XDG desktop entry installed by ``orion desktop``."""
...
def test_install_writes_entry_with_absolute_exec_and_icon(tmp_path, xdg_home, monkeypatch):
    ...
    orion_bin = tmp_path / "bin" / "orion"
    orion_bin.parent.mkdir()
    orion_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "orion_cli.relaunch.resolve_orion_bin", lambda: str(orion_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)

    assert entry == xdg_home / "applications" / "orion.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `orion` would not resolve.
    assert values["Exec"] == f"{orion_bin} desktop"
    ...
    assert values["Name"] == "Orion"
    assert values["Terminal"] == "false"


def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("orion_cli.relaunch.resolve_orion_bin", lambda: "/usr/bin/orion")
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("orion_cli.relaunch.resolve_orion_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m orion_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `orion`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(tmp_path, xdg_home, monkeypatch):
    ...
    orion_bin = tmp_path / "bin" / "orion"
    orion_bin.parent.mkdir()
    orion_bin.write_text("#!/usr/bin/env python3\nimport orion_cli\n", encoding="utf-8")
    orion_bin.chmod(0o755)
    monkeypatch.setattr("orion_cli.relaunch.resolve_orion_bin", lambda: str(orion_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert str(orion_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    orion_bin = tmp_path / "bin" / "orion"
    orion_bin.parent.mkdir()
    orion_bin.write_text('#!/bin/bash\nexec /opt/orion/venv/bin/python "$@"\n', encoding="utf-8")
    orion_bin.chmod(0o755)
    monkeypatch.setattr("orion_cli.relaunch.resolve_orion_bin", lambda: str(orion_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert exec_line == f"{orion_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    orion_bin.parent.mkdir()
    interpreter = str(Path(sys.executable).resolve())
    orion_bin.write_text(f"#!{interpreter}\nimport orion_cli\n", encoding="utf-8")
    orion_bin.chmod(0o755)
    monkeypatch.setattr("orion_cli.relaunch.resolve_orion_bin", lambda: str(orion_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Orion is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "orion"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "orion"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("orion_cli.relaunch.resolve_orion_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

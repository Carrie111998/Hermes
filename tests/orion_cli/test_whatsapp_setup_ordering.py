...
Before the fix, ``orion whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``orion gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from orion_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("orion_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``orion whatsapp`` with an existing paired session and

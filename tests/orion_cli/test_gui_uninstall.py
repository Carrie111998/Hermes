"""Tests for orion_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import orion_cli.gui_uninstall as gu


def _make_agent(orion_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = orion_home / "orion-agent"
    (agent_root / "orion_cli").mkdir(parents=True)
    (agent_root / "orion_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(orion_home: Path) -> None:
    """Create the source-built GUI artifacts a `orion desktop` run produces."""
    desktop = orion_home / "orion-agent" / "apps" / "desktop"
    ...
    (orion_home / "orion-agent" / "node_modules").mkdir(parents=True)
    (orion_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(orion_home: Path) -> None:
    (orion_home / "config.yaml").write_text("x: 1\n")
    (orion_home / ".env").write_text("KEY=secret\n")
    (orion_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(orion_home)
    _make_gui_build(orion_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(orion_home)
    ...
    assert summary["orion_home"] == str(orion_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `orion desktop` installs is removable."""
    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg"))

    from orion_cli import linux_desktop_entry as lde
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    orion_home = tmp_path / ".orion"
    _make_agent(orion_home)
    icon = lde.icon_path(orion_home / "orion-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(orion_home)
    ...
    assert lde.icon_path(orion_home / "orion-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (orion_home / "orion-agent" / "orion_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".orion")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import orion_cli.uninstall as uninstall

"""Tests for the unified provider catalog (orion_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``orion model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from orion_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,

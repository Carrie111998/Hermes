...
class TestStripOptionalSystemdDirectives:
    def test_removes_restart_max_delay_sec(self):
        from orion_cli.gateway import _strip_optional_systemd_directives
        text = """[Service]
Restart=always
RestartSec=5
...
[Service]
Type=simple
ExecStart=/usr/bin/python -m orion_cli.main gateway run
Restart=always
RestartSec=5
KillMode=mixed
KillSignal=SIGTERM
...
class TestSystemdUnitIsCurrent:
    def test_unit_without_fatal_config_restart_policy_is_not_current(
        ...
        from orion_cli import gateway as gw
...
"""
        ...
        unit_file = tmp_path / "orion-gateway.service"

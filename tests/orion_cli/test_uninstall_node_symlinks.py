"""Tests for orion_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ORION_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Orion-managed node dir.
"""
...
def _make_orion_node(orion_home: Path) -> Path:
    """Create a fake $ORION_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = orion_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    orion_home = fake_home / ".orion"
    _make_orion_node(orion_home)
    ...
    removed = uninstall.remove_node_symlinks(orion_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    orion_home = fake_home / ".orion"
    node_bin = _make_orion_node(orion_home)

"""Tests for the top-level `./orion` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./orion` should use `orion_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "orion"
    called = []

    fake_main_module = types.ModuleType("orion_cli.main")

    def fake_main():
        called.append("orion_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "orion_cli.main", fake_main_module)

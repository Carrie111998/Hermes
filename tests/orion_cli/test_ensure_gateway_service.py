"""Tests for orion_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `orion setup` and `orion import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("orion_constants.is_container", lambda: container)

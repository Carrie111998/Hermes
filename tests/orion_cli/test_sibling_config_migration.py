...
`orion update` migrated only the active profile's config.yaml; sibling
...
context-local ORION_HOME override.
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    import orion_cli.profiles as profiles_mod

    monkeypatch.setattr(profiles_mod, "_get_profiles_root", lambda: tmp_path / "profiles")
    import orion_constants

    monkeypatch.setattr(
        orion_constants, "get_process_orion_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_orion_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_orion_home_override() == before

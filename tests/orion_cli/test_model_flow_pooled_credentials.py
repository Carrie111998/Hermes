...
from orion_cli.model_setup_flows import _existing_api_key_for_model_flow
...
def test_generic_api_key_flow_passes_pool_key_to_existing_key_prompt(monkeypatch):
    from orion_cli.model_setup_flows import _model_flow_api_key_provider
    ...
    with (
        patch("orion_cli.config.get_env_value", return_value=""),
        patch("agent.credential_pool.load_pool", return_value=_AvailablePool()),
        patch("orion_cli.main._prompt_api_key", side_effect=capture_prompt),
    ):
...
def test_bedrock_flow_sees_pool_key_when_no_env(monkeypatch, capsys):
    ...
    from orion_cli.model_setup_flows import _model_flow_bedrock_api_key

...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_orion_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import orion_cli.oneshot as oneshot
        from orion_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """orion -z bypasses OrionCLI, but recall still needs SessionDB."""
    from orion_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "orion_state", mod("orion_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "orion_cli.config",
        mod("orion_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "orion_cli.models",
        mod("orion_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "orion_cli.runtime_provider",
        mod(
            "orion_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "orion_cli.tools_config",
        mod("orion_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["ORION_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/orion-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["ORION_MODEL"] == "nous/orion-test"
    assert env["ORION_INFERENCE_MODEL"] == "nous/orion-test"
    assert env["ORION_TUI_PROVIDER"] == "nous"
    assert env["ORION_INFERENCE_PROVIDER"] == "nous"
    assert env["ORION_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["ORION_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("orion-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_orion_ink(monkeypatch, main_mod, tmp_path):
    tui_dir = tmp_path / "ui-tui"
    tsx = tui_dir / "node_modules" / ".bin" / "tsx"
    ink_dir = tui_dir / "packages" / "orion-ink"
    ...
    monkeypatch.delenv("ORION_TUI_DIR", raising=False)

"""Tests for the `orion memory reset` CLI command.
...
- Profile-scoped reset (uses ORION_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ORION_HOME with memory files."""
    orion_home = tmp_path / ".orion"
    memories = orion_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ORION_HOME", str(orion_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nOrion repo is at ~/.orion/orion-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `orion memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        orion_home = tmp_path / ".orion"
        (orion_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ORION_HOME", str(orion_home))

"""Tests for orion_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_orion_home):
    """Drop the example-dashboard fixture into the per-test ORION_HOME
    ...
    ``ORION_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, orion-achievements, model providers
    ...
    user_plugins_dir = get_orion_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `orion plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down ORION_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Orion vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints ORION_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("ORION_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("ORION_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_orion_home):
        """Create a TestClient and isolate the state DB under the test ORION_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `orion update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `orion update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(orion_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = orion_state.SessionDB.__init__
        ...
        # the class on orion_state covers every open the helper makes.
        monkeypatch.setattr(orion_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(orion_state, "SessionDB", corrupt_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        ORION_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        ...
        config_path = get_orion_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from orion_constants import get_orion_home
        from orion_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide ORION_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from orion_constants import get_orion_home

        path = get_orion_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_orion_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["orion"]["workspace"] == "myws"
        assert cfg["hosts"]["orion"]["peerName"] == "eri"
        assert cfg["hosts"]["orion"]["environment"] == "local"
        assert cfg["hosts"]["orion"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["orion"]["apiKey"] == "hch-test-key"
    ...
    def test_update_orion_returns_docker_guidance_without_spawning(self, monkeypatch):
        import orion_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn orion update")
        ...
        monkeypatch.setattr(web_server, "_spawn_orion_action", fail_spawn)
        web_server._ACTION_PROCS.pop("orion-update", None)
        web_server._ACTION_RESULTS.pop("orion-update", None)

        resp = self.client.post("/api/orion/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "orion-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/orion-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/orion-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/orion-agent:latest" in line for line in status_data["lines"])

    def test_update_orion_returns_apt_guidance_without_spawning(self, monkeypatch):
        import orion_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn orion update")
        ...
        assert data["update_command"] == "pkg upgrade orion-agent"
        assert spawned is False

        check = self.client.get("/api/orion/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade orion-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "orion-update.log").write_text(
            "=== orion-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== orion update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== orion-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/orion-update/status?lines=2000")
        ...
        assert f"=== orion-update completed {action_id} ===" in data["lines"]

    def test_update_orion_spawns_with_action_id(self, monkeypatch):
        import orion_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_orion_action", fake_spawn)
        web_server._ACTION_PROCS.pop("orion-update", None)
        web_server._ACTION_RESULTS.pop("orion-update", None)
        ...
        assert calls == [
            (["update"], "orion-update", {"ORION_ACTION_ID": "a" * 32})
        ]

    def test_update_orion_reuses_running_action(self, monkeypatch):
        import orion_cli.web_server as web_server
        ...
        web_server._ACTION_PROCS["orion-update"] = Proc()
        web_server._ACTION_IDS["orion-update"] = "b" * 32

        try:
            resp = self.client.post("/api/orion/update")
        finally:
            web_server._ACTION_PROCS.pop("orion-update", None)
            web_server._ACTION_IDS.pop("orion-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/orion_cli/test_setup_hidden_env.py.
        """
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Orion provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from orion_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from orion_cli.config import save_env_value
        from orion_cli.web_server import _SESSION_TOKEN
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "orion_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/OrionSetupBot/orion_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/OrionSetupBot/orion_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "orion_pair_restart_fails_bot",
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from orion_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``orion model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from orion_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from orion_cli import profiles as profiles_mod
        from orion_cli.config import custom_endpoint_key_env
        from orion_constants import get_orion_home
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from orion_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from orion_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from orion_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from orion_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        ...
        from orion_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "orion"
        save_config(config)
        ...
        assert "orion@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from orion_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from orion_cli.web_server import _denormalize_config_from_web
...
class TestStatusInstallId:
    ...
    Orion home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_orion_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """ORION_HOME=<root> and ORION_HOME=<root>/profiles/<name> resolve to
        the same id file — profiles share one physical install identity."""
        import orion_cli.web_server as ws

        root = tmp_path / "orion-root"
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from orion_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.orion/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_orion_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(orion_home, name="ocean"):
        themes_dir = orion_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="orion-theme-bootstrap">')
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``orion_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in orion_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_orion_home):
        ...
        # Pin the SessionDB to the isolated ORION_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_orion_home, _install_example_plugin):
        ...
        installs it into the per-test ``ORION_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into ORION_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, orion-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (orion-achievements is loaded by default).
        resp = self.client.get("/api/plugins/orion-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        ORION_HOME override scopes a request to another profile."""
        from orion_constants import (
            reset_orion_home_override,
            set_orion_home_override,
        )
        ...
        monkeypatch.setenv("ORION_HOME", str(launch_home))
        from orion_cli import web_server
        token = set_orion_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets ORION_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the orion root's plugins/ directory."""
        root = tmp_path / "orion-root"
...
# or ``sh -c 'printf …'``) instead of the real ``orion --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"orion-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "ORION_CWD": str(tmp_path),
            "ORION_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["ORION_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from orion_cli.pty_bridge import PtyUnavailableError
        ...
        import orion_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from orion_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("ORION_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_orion_home, _install_example_plugin):
        ...
        the per-test ``ORION_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_orion_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under ORION_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("ORION_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/orion"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/orion/ds-assets/bg.png)" in prefixed.text
        assert "url(/orion/fonts-terminal/x.woff2)" in prefixed.text

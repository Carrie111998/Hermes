...
profile's ORION_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_orion_home):
    """Isolated default home + one named profile, each with its own .env."""
    from orion_constants import get_orion_home
    from orion_cli import profiles

    default_home = get_orion_home()
    ...
    monkeypatch.setattr(profiles, "_get_default_orion_home", lambda: default_home)
...
def client(monkeypatch, isolated_profiles):
    ...
    import orion_state
    from orion_constants import get_orion_home
    from orion_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

    monkeypatch.setattr(orion_state, "DEFAULT_DB_PATH", get_orion_home() / "state.db")
...
class TestProfileScopedMessagingReads:
    ...
    def test_scoped_read_returns_profile_path_command_and_startup_failure(
        ...
        monkeypatch.setattr(
            ...
            # relying on process-level ORION_HOME resolution (#71211).
            lambda *a, **k: {
        ...
        assert payload["gateway_start_command"] == (
            "orion -p worker_alpha gateway start"
        )

...
import pytest
from orion_cli.config import _expand_env_vars, load_config
...
class TestLoadConfigExpansion:
    def test_load_config_expands_env_vars(self, tmp_path, monkeypatch):
        ...
        # orion_cli.config, making string-target monkeypatches hit a different
...
class TestLoadConfigCacheEnvStaleness:
    ...
    environment (#58514): a load before load_orion_dotenv() runs, or an env
...
class TestLoadCliConfigExpansion:
    ...
    def test_cli_config_ignores_empty_terminal_section(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("cli._orion_home", tmp_path)

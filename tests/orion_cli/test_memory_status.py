"""Tests for `orion memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from orion_cli.memory_setup import cmd_status
    ...
    with patch("orion_cli.config.load_config", return_value=config):
        with patch("orion_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
                "orion_cli.tools_config._get_platform_tools",

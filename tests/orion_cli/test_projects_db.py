"""Tests for the per-profile Projects store (orion_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Orion Agent", folders=["/tmp/orion"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "orion-agent"
    assert proj.name == "Orion Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/orion"
    assert [f.path for f in proj.folders] == ["/tmp/orion"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "orion-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ORION_HOME.

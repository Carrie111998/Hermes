"""Regression tests for #34107 — Docker UID/GID handling in ensure_orion_home.

When Orion runs in Docker with ``ORION_UID=1000`` / ``ORION_GID=911``,
the entrypoint chowns the top-level ``ORION_HOME`` once at startup. But
...
The fix is a ``_chown_to_orion_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_orion_uid_gid
# ---------------------------------------------------------------------------


class TestResolveOrionUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ORION_UID", "1000")
        monkeypatch.setenv("ORION_GID", "911")
        from orion_cli.config import _resolve_orion_uid_gid
...
class TestChownToOrionUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_orion_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("ORION_UID", raising=False)
        monkeypatch.delenv("ORION_GID", raising=False)
        from orion_cli import config as cfg

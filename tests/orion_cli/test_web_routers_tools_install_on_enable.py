...
the endpoint spawns the same background ``orion tools post-setup <key>``
...
class TestToggleToolsetInstallOnEnable:
    ...
    def _setup(self, monkeypatch, _isolate_orion_home):
        try:
        ...
        monkeypatch.setattr(
            orion_state, "DEFAULT_DB_PATH", get_orion_home() / "state.db"
        )
    ...
    def _spawn_recorder(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "_spawn_orion_action", _fake_spawn)
        return calls
    ...
    def test_spawn_failure_does_not_fail_the_toggle(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "_spawn_orion_action", _boom)

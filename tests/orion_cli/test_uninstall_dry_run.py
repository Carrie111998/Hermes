...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_orion_home", lambda: orion_home)
    monkeypatch.setattr(uninstall, "_is_default_orion_home", lambda home: False)
    ...
    assert str(orion_home) in output
    assert project_root.exists()
    assert orion_home.exists()

...
#88251: ``orion update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "orion_constants.with_orion_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "orion_constants.display_orion_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_orion_home", lambda: tmp_path)

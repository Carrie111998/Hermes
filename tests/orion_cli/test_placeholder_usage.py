...
from orion_cli.config import config_command, show_config
from orion_cli.setup import _print_setup_summary
...
def test_config_set_usage_marks_placeholders(capsys):
    ...
    assert "Usage: orion config set" in out
    assert "<key>" in out
    assert "<value>" in out
...
def test_show_config_marks_placeholders(tmp_path, capsys):
    with patch.dict(os.environ, {"ORION_HOME": str(tmp_path)}):
        show_config()

    out = capsys.readouterr().out
    assert "orion config set <key> <value>" in out

"""Tests for orion_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from orion_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("orion_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from orion_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    ...
    @patch("orion_cli.config.load_config")
    ...
    @patch("orion_cli.config.load_config")
    @patch.dict("os.environ", {"ORION_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ORION_SESSION_PLATFORM should be used when ORION_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
        ...
        monkeypatch.setenv("ORION_HOME", str(tmp_path))
        monkeypatch.delenv("ORION_PLATFORM", raising=False)
        monkeypatch.setenv("ORION_SESSION_PLATFORM", "discord")

        from agent.skill_utils import get_disabled_skill_names
        result = get_disabled_skill_names()
        assert result == {"discord-skill", "global-skill"}

    def test_orion_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ORION_PLATFORM should win over ORION_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from orion_cli.skills_config import _get_categories
        skills = [

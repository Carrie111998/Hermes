...
and the ``orion kanban repair`` CLI verb."""
...
def test_dispatch_tick_runs_wal_checkpoint_at_interval(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ORION_KANBAN_DB", str(db_path))
...
# ---------------------------------------------------------------------------
# repair_db() API + `orion kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `orion kanban …`."""
    import argparse

    from orion_cli import kanban as kc
...
def cli_home(tmp_path, monkeypatch):
    """Isolated ORION_HOME so kanban_db_path() resolves inside tmp_path."""
    home = tmp_path / ".orion"
    home.mkdir()

...
- Bare `orion setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `orion setup --quick` runs the narrower "fill in missing items" flow.
- `orion setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("orion_cli.setup.ensure_orion_home", {}),
        ("orion_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("orion_cli.config.is_managed", {"return_value": False}),
        ("orion_cli.setup.load_config", {"return_value": {}}),
        ("orion_cli.setup.save_config", {}),
        ("orion_cli.setup.get_env_value", {"return_value": None}),
        ("orion_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("orion_cli.setup._print_setup_summary", {}),
        ("orion_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("orion_cli.setup.load_config", {"return_value": {}}),
        ("orion_cli.setup.save_config", {}),
        ("orion_cli.auth.get_active_provider", {"return_value": None}),
        ("orion_cli.setup.get_env_value", {"return_value": None}),
        ("orion_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `orion setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="orion_cli.setup.prompt_choice",
                quick="orion_cli.setup._run_quick_setup",
                model="orion_cli.setup.setup_model_provider",
                terminal="orion_cli.setup.setup_terminal_backend",
                agent="orion_cli.setup.setup_agent_settings",
                gateway="orion_cli.setup.setup_gateway",
                tools="orion_cli.setup.setup_tools",
            )
            from orion_cli.setup import run_setup_wizard
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("orion_cli.setup.prompt_choice", {"return_value": 0}),
                first="orion_cli.setup._run_first_time_quick_setup",
            )
            from orion_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from orion_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "orion_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["orion", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass

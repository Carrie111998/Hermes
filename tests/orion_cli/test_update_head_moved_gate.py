"""Tests for the post-pull HEAD-movement gate in ``orion update``.
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the orion_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns orion_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/orion_cli/test_cmd_update.py).
    """
    monkeypatch.setattr(orion_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(orion_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()  # pass the "is a git repo" gate
    monkeypatch.setattr(
        orion_main, "_resolve_update_branch", lambda args: "main"
    )
    monkeypatch.setattr(orion_main, "_is_windows", lambda: False)
    monkeypatch.setattr(
        orion_main, "_get_origin_url",
        lambda *a, **k: "https://github.com/zacharyjleach-stack/Aries.git",
    )
    monkeypatch.setattr(orion_main, "_is_fork", lambda *a, **k: False)
    monkeypatch.setattr(
        orion_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(orion_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        orion_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        orion_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
        orion_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        orion_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    # Short-circuit the long tail: dependency install + desktop build.
    monkeypatch.setattr(orion_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(orion_main, "_clear_update_incomplete_marker", lambda: None)
    # Gateway restart path (called after a successful update).
    monkeypatch.setattr(orion_main, "_finish_dashboard_update_cleanup", lambda *a: None)
    ...
    import orion_cli.gateway as orion_gateway

    monkeypatch.setattr(
        orion_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        orion_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
        orion_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    orion_main.cmd_update(args)  # completes normally (no SystemExit)

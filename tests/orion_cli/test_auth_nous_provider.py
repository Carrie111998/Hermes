...
from orion_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from orion_cli.auth import _resolve_verify
...
def _setup_nous_auth(
    ...
    *,
    ...
    (orion_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    payload = json.loads((orion_home / "auth.json").read_text())
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ORION_HOME", str(orion_home))
    monkeypatch.setenv("ORION_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.DEBUG, logger="orion_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from orion_cli.auth import get_nous_auth_status
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `orion model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return orion_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from orion_cli.auth import PROVIDER_REGISTRY, _login_nous

        orion_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `orion auth add nous`
    ...
    """
    from orion_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Orion must surface an
    ...
    re-authenticate" string led users to report this as a Orion persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from orion_cli.auth import _refresh_access_token
    ...
    assert "orion auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ORION_SHARED_AUTH_DIR to a tmp_path.
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ORION_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.orion/shared/`` across CI runs.
    """
    from orion_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ORION_SHARED_AUTH_DIR", raising=False)
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `orion auth add nous
    ...
    orion_home = tmp_path / "orion"
    orion_home.mkdir(parents=True, exist_ok=True)
    (orion_home / "auth.json").write_text(
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "orion-cli"
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from orion_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        auth_file = orion_home / "auth.json"

...
The bug class: on Windows, when `orion.exe`/sibling shims could not be
...
import orion_cli._install_repair as ir
import orion_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("orion", "orion-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # orion.exe cannot be renamed; orion-gateway.exe can
    real_rename = Path.rename

    def deny_orion(self, target):
        if self.name == "orion.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_orion)
    monkeypatch.setattr(cli_main, "_orion_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "orion.exe" in exc_info.value.failed_shims
    # The successful rename (orion-gateway.exe) was rolled back.
    assert (scripts / "orion-gateway.exe").exists()
    assert not list(scripts.glob("orion-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("orion",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("orion",))

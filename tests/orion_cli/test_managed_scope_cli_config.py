...
orion_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `orion config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_orion_home = get_orion_home()`` at import time (module
    singleton), so monkeypatching ORION_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ORION_HOME.)
    """
    import cli

    cli._orion_home = home

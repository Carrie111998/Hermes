...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from orion_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "orion" / "auth.json").read_text())
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "orion_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `orion auth add nous` must also populate providers.nous so the
    ...
    # raises "Orion is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`orion auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``orion auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``orion auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("orion_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from orion_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from orion_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """orion auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``orion auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "orion" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`orion logout --provider openai-codex` must still clear model.provider.
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from orion_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Orion reads from has a
...
def test_seed_from_singletons_respects_orion_pkce_suppression(tmp_path, monkeypatch):
    """anthropic orion_pkce must not re-seed from ~/.orion/.anthropic_oauth.json when suppressed."""
    orion_home = tmp_path / "orion"
    orion_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ORION_HOME", str(orion_home))

    import yaml
    (orion_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (orion_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["orion_pkce"]},
    }))

    # Stub the readers so only orion_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_orion_oauth_credentials", lambda: {
    ...
    # orion_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "orion_pkce" not in active

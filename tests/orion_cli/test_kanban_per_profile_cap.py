...
def isolated_kanban_home_with_profiles(monkeypatch):
    """Spin up a fresh ORION_HOME with kanban DB + alpha/beta profiles."""
    test_home = tempfile.mkdtemp(prefix="kanban_per_profile_cap_test_")
    for prof in ("alpha", "beta", "default"):
        os.makedirs(os.path.join(test_home, "profiles", prof), exist_ok=True)
    monkeypatch.setenv("ORION_HOME", test_home)
    for mod in list(sys.modules.keys()):
        if mod.startswith("orion_cli") or mod.startswith("orion_state") or mod == "orion_constants":
            del sys.modules[mod]

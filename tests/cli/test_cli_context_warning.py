...
def _isolate(tmp_path, monkeypatch):
    """Isolate ORION_HOME so tests don't touch real config."""
    home = tmp_path / ".orion"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal OrionCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = OrionCLI.__new__(OrionCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["orion-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Orion' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Orion' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Orion' minimum context length."""

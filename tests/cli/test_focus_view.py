...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = OrionCLI.__new__(OrionCLI)
        host.model = "opus"
        ...
        with patch.object(OrionCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(OrionCLI, "_is_session_yolo_active", return_value=False):
            text = OrionCLI._build_status_bar_text(host, width=width)

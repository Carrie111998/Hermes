...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a OrionCLI instance with minimal mocking."""
    _clean_config = {
        "model": {
    ...
    clean_env = {"LLM_MODEL": "", "ORION_MAX_ITERATIONS": ""}
    if env_overrides:
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), patch.dict(
        ...
        with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), patch.dict(
            ...
            return _cli_mod.OrionCLI(**kwargs)
...
def _reset_session_id_context():
    ...
    yield
    os.environ.pop("ORION_SESSION_ID", None)
    _VAR_MAP["ORION_SESSION_ID"].set(_UNSET)

...
def test_expand_paste_references_missing_opaque_artifact_keeps_placeholder(
    tmp_path, monkeypatch
):
    ...
    assert placeholder is not None
    stored = list((profile_home / "pastes").glob("*.txt"))
    assert len(stored) == 1
    stored[0].unlink()

    assert cli_obj._expand_paste_references(placeholder) == placeholder

...
SessionDB.session_gateway_runtime (orion_state.py) — the round trip that
makes `orion --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare OrionCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.OrionCLI)

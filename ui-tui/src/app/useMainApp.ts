...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(
    model ? composeTabTitle(marker, ui.sessionTitle, model, tabCwd ? shortCwd(tabCwd, 24) : '') : 'Charterforge'
  )
  ...
  const cwd = ui.info?.cwd || process.env.CHARTERFORGE_CWD || process.cwd()

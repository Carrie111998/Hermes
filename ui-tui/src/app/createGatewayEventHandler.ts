...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@charterforge/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.CHARTERFORGE_TUI_THEME : undefined

  writeBootTheme(theme, process.env.CHARTERFORGE_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to CHARTERFORGE_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit CHARTERFORGE_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the CHARTERFORGE_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.CHARTERFORGE_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.CHARTERFORGE_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a CHARTERFORGE_TUI_THEME the user
    ...
    delete process.env.CHARTERFORGE_TUI_THEME
  }
 ...
 * is cached into CHARTERFORGE_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * CHARTERFORGE_TUI_LIGHT / CHARTERFORGE_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.CHARTERFORGE_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.CHARTERFORGE_TUI_THEME || process.env.CHARTERFORGE_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.CHARTERFORGE_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.CHARTERFORGE_TUI_BACKGROUND ||
      process.env.CHARTERFORGE_TUI_THEME ||
      process.env.CHARTERFORGE_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.CHARTERFORGE_TUI_BACKGROUND || process.env.CHARTERFORGE_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.CHARTERFORGE_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}

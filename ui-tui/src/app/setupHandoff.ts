import type { RunExternalProcess } from '@orion/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`orion ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching orion: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`orion ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })

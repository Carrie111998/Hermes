...
import { terminalBackgroundHex } from '@orion/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ORION_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ORION_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ORION_TUI_BACKGROUND', process.env.ORION_TUI_BACKGROUND ?? '(unset)'],
            ['ORION_TUI_THEME', process.env.ORION_TUI_THEME ?? '(unset)'],

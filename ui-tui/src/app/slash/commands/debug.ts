...
import { terminalBackgroundHex } from '@charterforge/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $CHARTERFORGE_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see CHARTERFORGE_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['CHARTERFORGE_TUI_BACKGROUND', process.env.CHARTERFORGE_TUI_BACKGROUND ?? '(unset)'],
            ['CHARTERFORGE_TUI_THEME', process.env.CHARTERFORGE_TUI_THEME ?? '(unset)'],

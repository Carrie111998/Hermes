...
describe('history timeline overlay model', () => {
  ...
  it('can default to the latest persisted branchable row when optimistic local tail rows exist', () => {
    const state = buildHistoryTimelineState(
      [
        { ...msg('user', 'persisted prompt'), dbId: 101 },
        { ...msg('assistant', 'persisted answer'), dbId: 102 },
        msg('user', 'optimistic local prompt'),
        msg('assistant', 'optimistic local answer')
      ],
    ...
    expect(state.items[2]?.dbId).toBeUndefined()
    expect(state.items[3]?.dbId).toBeUndefined()
    expect(state.selected).toBe(1)
    expect(state.items[state.selected]).toMatchObject({ dbId: 102, identity: 'db:102', role: 'assistant' })
  })

  it('keeps the latest-row default when no persisted branch target exists', () => {
    const state = buildHistoryTimelineState([msg('user', 'local prompt'), msg('assistant', 'local answer')], {
      preferLatestPersistedBranchable: true
    })

    expect(state.selected).toBe(1)
    expect(state.items[state.selected]).toMatchObject({ identity: 'ordinal:assistant:1:source:1', role: 'assistant' })
  })

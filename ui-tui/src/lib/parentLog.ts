...
const logDir = join(process.env.ORION_HOME?.trim() || join(homedir(), '.orion'), 'logs')
...
// a real ~/.orion (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('orion-tui: parent lifecycle log unavailable\n')
    }
  }
}

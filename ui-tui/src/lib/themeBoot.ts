 ...
 * `orion-boot-background` / `orion-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ORION_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.orion matches get_orion_home()'s default.
const bootFilePath = () => join(process.env.ORION_HOME ?? join(homedir(), '.orion'), 'tui-theme-boot.json')

// Never touch the user's real ~/.orion from test runs (the TS suite has no
// ORION_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ORION_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ORION_TUI_THEME || env.ORION_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ORION_TUI_BACKGROUND
  ) {
    env.ORION_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ORION_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ORION_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ORION_TUI_THEME.

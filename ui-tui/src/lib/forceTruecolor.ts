...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ORION_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Orion on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ORION_TUI_TRUECOLOR=1.

...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CHARTERFORGE_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Charterforge on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CHARTERFORGE_TUI_TRUECOLOR=1.

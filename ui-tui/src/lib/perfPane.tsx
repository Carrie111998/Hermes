...
//
// Both gate on ORION_DEV_PERF=1 and dump JSON-lines (default ~/.orion/perf.log,
// override ORION_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// ORION_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import { scrollFastPathStats } from '@orion/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.ORION_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.ORION_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.ORION_DEV_PERF_LOG?.trim() || join(homedir(), '.orion', 'perf.log')

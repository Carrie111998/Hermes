...
export async function terminalParityHints(
  ...
  if (ctx.isRemote) {
    hints.push({
      ...
      tone: 'warn',
      message:
        'SSH session detected · text clipboard can bridge via OSC52, but image clipboard and local screenshot paths still depend on the machine running Charterforge'
    })
  }

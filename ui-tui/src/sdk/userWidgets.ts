...
/**
 * User widget apps — Orion authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ORION_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.orion/plugins/`: files under ORION_HOME execute
...
const widgetsDir = () => join(process.env.ORION_HOME?.trim() || join(homedir(), '.orion'), 'tui-widgets')
 ...
 *  every change, so a widget Orion writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Orion) ever writes must hot-load too — a 10s poll here read

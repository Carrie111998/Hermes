...
/**
 * User widget apps — Charterforge authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$CHARTERFORGE_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.hermes/plugins/`: files under CHARTERFORGE_HOME execute
...
const widgetsDir = () => join(process.env.CHARTERFORGE_HOME?.trim() || join(homedir(), '.charterforge'), 'tui-widgets')
 ...
 *  every change, so a widget Charterforge writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Charterforge) ever writes must hot-load too — a 10s poll here read

...
const TAG_FULL = 'Orion · Messenger of the Digital Gods'
const TAG_MID = 'Messenger of the Digital Gods'
const TAG_TINY = 'Orion'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  // orion_cli/banner.py) and the "connected" label on the collapse toggle.
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · Orion</Text>
      </Text>
    ...
    <Box flexDirection="column" width="100%">
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'orion update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update

import type { InputEvent, Key } from '@orion/ink'
import * as Ink from '@orion/ink'
 ...
 * but that accepts `key.meta` on macOS — and orion-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ORION_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which orion-ink decodes as meta+'d'; without this branch it

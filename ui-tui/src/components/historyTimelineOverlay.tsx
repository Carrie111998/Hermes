...
interface BuildHistoryTimelineStateOptions {
  preferLatestPersistedBranchable?: boolean
}

const defaultSelectedIndex = (items: HistoryTimelineItem[], options: BuildHistoryTimelineStateOptions): number => {
  if (!items.length) {
    return 0
  }

  if (options.preferLatestPersistedBranchable) {
    const latestPersistedBranchable = items.findLastIndex(item => item.actionable && typeof item.dbId === 'number')

    if (latestPersistedBranchable >= 0) {
      return latestPersistedBranchable
    }
  }
...
export const buildHistoryTimelineState = (
  messages: Msg[],
  options: BuildHistoryTimelineStateOptions = {}
  ...
  const selected = defaultSelectedIndex(allItems, options)

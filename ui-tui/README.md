...
React + Ink terminal UI for Orion. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ORION_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `orion setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.orion/.orion_history` or under `ORION_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `orion setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/orion-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external orion setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.orion/tui-parent.log
    ...
    types/
      orion-ink.d.ts            type declarations for @orion/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent OrionCLI subprocess for slash commands
```

...
React + Ink terminal UI for Charterforge. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CHARTERFORGE_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.hermes/.hermes_history` or under `CHARTERFORGE_HOME`.
...
ui-tui/
  ...
  src/
    ...
    types/
      hermes-ink.d.ts            type declarations for @charterforge/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CharterforgeCLI subprocess for slash commands
```

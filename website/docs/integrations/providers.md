...
WebSocket support uses the native `responses.connect()` API in Hermes' pinned OpenAI SDK and preserves request overrides, provider extension fields, custom handshake headers, proxy environment variables, and `SSL_CERT_FILE`. Handshakes have a 15-second bound, the server's 60-minute connection-limit event triggers a safe reconnect, and `x-codex-turn-state` is retained within one user turn and reset at the next turn. Initial support is scoped to the `openai-codex` provider on `https://chatgpt.com/backend-api/codex`. Generic OpenAI Responses endpoints, clients with custom httpx transports, and sessions using Relay managed stream execution retain SSE; forced WebSocket modes fail before sending when Relay stream plugins are active.
:::

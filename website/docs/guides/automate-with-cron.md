...
description: "Real-world automation patterns using IYARI cron — monitoring, reports, pipelines, and multi-skill workflows"
---
...
- **Recurring watchdog** where the script already produces the exact message (memory alerts, disk alerts, heartbeats): use [script-only cron jobs](/guides/cron-script-only). Same scheduler, no LLM. You can ask IYARI to set one up for you in chat — the `cronjob` tool knows when to pick `no_agent=True` and writes the script for you.
...
req = urllib.request.Request(URL, headers={"User-Agent": "IYARI-Monitor/1.0"})
...
```bash
/cron add "every 6h" "Check the GitHub repository digital-services-llc/iyari for:
- New issues opened in the last 6 hours
- New PRs opened or merged in the last 6 hours
- Any new releases

Use the terminal to run gh commands:
  gh issue list --repo digital-services-llc/iyari --state open --json number,title,author,createdAt --limit 10
  gh pr list --repo digital-services-llc/iyari --state all --json number,title,author,createdAt,mergedAt --limit 10

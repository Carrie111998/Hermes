...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `orion send`."
---

# Pipe Script Output to Messaging Platforms

`orion send` is a small, scriptable CLI that pushes a message to any
messaging platform Orion is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | orion send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `orion
...
Any platform Orion ships adapters for works as a target:
...
`orion send` resolves the message body in this order:

1. **Positional argument** — `orion send --to telegram "hi"`
2. **`--file PATH`** — `orion send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | orion send --to telegram`

When stdin is a TTY (no pipe), Orion does **not** wait for input — you'll
...
Because `orion send` reuses your Orion config, the same script works on
any host where Orion is installed — no need to export bot tokens into
...
`orion send`. If the Python interpreter can't load because the box is
...
## Does `orion send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `orion send` calls
the platform's REST endpoint directly using credentials from
`~/.orion/.env` and `~/.orion/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `orion gateway start` and retry.
...
The listing is built from `~/.orion/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`orion gateway
...
| Approach | Multi-platform | Reuses Orion creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `orion send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`orion send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `orion send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `orion send` shares with cron delivery.

...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Orion instantly when PRs are opened or updated.
:::
...
Use `deliver: "local"` to save reviews to `~/.orion/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Orion can access GitHub. Start a chat:
...
Still in the chat, ask Orion to review a real PR:
...
A skill gives Orion consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.orion/skills/code-review
```

Create `~/.orion/skills/code-review/SKILL.md`:
...
Verify it loaded — start `orion` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Orion your team's standards:
...
2. Teach Orion your conventions via memory (Step 4)

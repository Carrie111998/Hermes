...
This tutorial walks you through setting up a Telegram bot powered by Orion Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Orion Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.orion/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Orion itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Orion Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_orion_bot`)
...
sudo orion gateway install --system   # Linux only: boot-time system service
```
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `orion gateway install` to update the plist.
:::
...
**Option 2:** Set it manually in `~/.orion/.env`:
...
Control how much detail the bot shows when using tools. In `~/.orion/config.yaml`:
...
Customize how the bot communicates by editing `~/.orion/SOUL.md`:

For a full guide, see [Use SOUL.md with Orion](/guides/use-soul-with-orion).
...
```bash
# From the CLI
orion cron list          # View all scheduled jobs
orion cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u orion-gateway` (Linux) or `~/.orion/logs/gateway.log` (macOS) |
| Cron job output | `~/.orion/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.orion/cron/jobs.json` |
| Pairing data | `~/.orion/pairing/` |
| Session history | `~/.orion/sessions/` |

...
This tutorial walks you through setting up a Telegram bot powered by IYARI that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **IYARI installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
...
:::tip
A $5/month VPS is plenty for running the gateway. IYARI itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team IYARI Assistant`)
...
For a full guide, see [Use SOUL.md with IYARI](/guides/use-soul-with-hermes).

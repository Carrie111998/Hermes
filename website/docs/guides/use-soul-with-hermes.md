---
sidebar_position: 7
title: "Use SOUL.md with IYARI"
description: "How to use SOUL.md to shape IYARI's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with IYARI

`SOUL.md` is the **primary identity** for your IYARI instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want IYARI to feel like the same assistant every time you talk to it — or if you want to replace the IYARI persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm IYARI should be
- what IYARI should avoid stylistically
- how IYARI should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who IYARI is and how IYARI speaks
...
IYARI now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run IYARI with a custom home directory, it becomes:
...
IYARI automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, IYARI does not overwrite it
- if the file exists but is empty, IYARI adds nothing from it to the prompt

## How IYARI uses it

When IYARI starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how IYARI feels.
...
IYARI already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How IYARI should sound.

# Avoid
What IYARI should not do.

# Defaults
How IYARI should behave when ambiguity appears.
```
...
Then restart IYARI or start a new session.
...
### I edited SOUL.md but IYARI still sounds the same
...
### IYARI is ignoring parts of my SOUL.md

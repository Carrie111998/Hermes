---
sidebar_position: 8
title: "Use Voice Mode with Orion"
description: "A practical guide to setting up and using Orion voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Orion
...
- you want Orion sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Orion.
...
## Step 1: make sure normal Orion works first
...
Orion supports both local and cloud speech stacks.
...
### If you use `orion setup`

If you choose NeuTTS in the setup wizard, Orion checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Orion transcribes and responds
...
- asking Orion to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Orion loop.

## Tuning CLI behavior

### Silence threshold

If Orion starts/stops too aggressively, tune:
...
Orion stays a normal chat bot, but can speak replies.
...
- you want Orion to function like a portable research or ops assistant
...
Orion joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Orion detects speech boundaries
- transcripts are posted in the associated text channel
- Orion responds in text and audio
...
1. get text Orion working
2. run `orion setup tts` to enable voice support

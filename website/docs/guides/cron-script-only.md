...
Orion calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Orion writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Orion:** *(writes `~/.orion/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
>
> **Orion:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `orion cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Orion:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
# 3. Verify
orion cron list
orion cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.orion/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
orion cron create "every 5m"        # interval
orion cron create "every 2h"
orion cron create "0 9 * * *"       # standard cron: 9am daily
orion cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.orion/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.orion/.env` / `~/.orion/config.yaml`.

## Editing and Lifecycle

```bash
orion cron list                                    # see all jobs
orion cron pause <job_id>                          # stop firing, keep definition
orion cron resume <job_id>
orion cron edit <job_id> --schedule "every 10m"    # adjust cadence
orion cron edit <job_id> --agent                   # flip to LLM mode
orion cron edit <job_id> --no-agent --script …     # flip back
orion cron remove <job_id>                         # delete it
```
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on Orion' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When Orion might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Orion webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Orion being up. The in-gateway scheduler is the right choice when the thing being monitored is external.

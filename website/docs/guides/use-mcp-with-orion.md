---
sidebar_position: 6
title: "Use MCP with Orion"
description: "A practical guide to connecting MCP servers to Orion Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Orion

This guide shows how to actually use MCP with Orion Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Orion tool
- you want Orion to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Orion to internal APIs, databases, or company systems without modifying Orion core

Do not use MCP when:
- a built-in Orion tool already solves the job well
...
- Orion remains the agent
- MCP servers contribute tools
- Orion discovers those tools at startup or reload time
...
If you installed Orion with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).

If you installed without extras and need to add MCP separately:

```bash
cd ~/.orion/orion-agent
...
- Orion banner/status should show MCP integration when configured
- ask Orion what tools it has available
...
## WSL2: bridge Orion in WSL to Windows Chrome

This is the practical setup when:

- Orion runs inside WSL2
...
In this setup, Orion does **not** connect to Chrome directly. Instead:

- Orion runs in WSL
- Orion starts a local stdio MCP server
...
```text
Orion (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Orion stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Orion core browser transport
...
```bash
orion mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
orion mcp test chrome-devtools-win
```

Then start a fresh Orion session or run:
...
Once loaded, Orion can use the MCP-prefixed browser tools directly. For example:
...
If Orion runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Orion from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Orion from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Orion:
...
2. Orion-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Orion will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Orion to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Orion to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Orion remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
orion mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
orion mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `orion mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Orion to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Orion to merge, publish, deploy, or spawn runtimes.
...
Now Orion can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Orion core.
...
Because Orion now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.

...
```bash
orion teams-pipeline token-health
orion teams-pipeline token-health --force-refresh
```
...
```bash
orion teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
orion teams-pipeline maintain-subscriptions
orion teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Orion cron (recommended if you already run the Orion gateway)

Orion ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.orion/scripts/`. First create the script:

```bash
mkdir -p ~/.orion/scripts
cat > ~/.orion/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec orion teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.orion/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
orion cron create "0 */12 * * *" \
...
```bash
orion cron list
orion cron status        # scheduler status
```

#### Option 2: systemd timer (recommended for Linux production deployments)

Create `/etc/systemd/system/orion-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Orion Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=orion
EnvironmentFile=/etc/orion/env
ExecStart=/usr/local/bin/orion teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/orion-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Orion Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now orion-teams-pipeline-maintain.timer
systemctl list-timers orion-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/orion teams-pipeline maintain-subscriptions >> /var/log/orion/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.orion/.env` at the top of a wrapper script that crontab calls.
...
```bash
orion teams-pipeline subscriptions   # should show expirationDateTime advanced
orion teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
orion teams-pipeline list
orion teams-pipeline list --status failed
orion teams-pipeline show <job-id>
```

### Replay a stored job

```bash
orion teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
orion teams-pipeline fetch --meeting-id <meeting-id>
orion teams-pipeline fetch --join-web-url "<join-url>"
orion teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `orion teams-pipeline maintain-subscriptions --dry-run`
- inspect `orion teams-pipeline list --status failed`
...
- run `orion teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `orion teams-pipeline run`
...
- [ ] `orion teams-pipeline validate` returns an OK snapshot
- [ ] `orion teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Orion cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.

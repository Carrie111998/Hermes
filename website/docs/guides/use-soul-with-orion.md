---
sidebar_position: 7
title: "Use SOUL.md with Orion"
description: "How to use SOUL.md to shape Orion Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Orion

`SOUL.md` is the **primary identity** for your Orion instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Orion to feel like the same assistant every time you talk to it — or if you want to replace the Orion persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Orion should be
- what Orion should avoid stylistically
- how Orion should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Orion is and how Orion speaks
...
Orion now uses only the global SOUL file for the current instance:

```text
~/.orion/SOUL.md
```

If you run Orion with a custom home directory, it becomes:
...
Orion automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Orion does not overwrite it
- if the file exists but is empty, Orion adds nothing from it to the prompt

## How Orion uses it

When Orion starts a session, it reads `SOUL.md` from `ORION_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Orion feels.
...
Orion already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Orion should sound.

# Avoid
What Orion should not do.

# Defaults
How Orion should behave when ambiguity appears.
```
...
Then restart Orion or start a new session.
...
### I edited SOUL.md but Orion still sounds the same

Check:
- you edited `~/.orion/SOUL.md` or `$ORION_HOME/SOUL.md`
...
### Orion is ignoring parts of my SOUL.md

---
sidebar_position: 6
title: "Use MCP with IYARI"
description: "A practical guide to connecting MCP servers to IYARI, filtering their tools, and using them safely in real workflows"
---

# Use MCP with IYARI

This guide shows how to actually use MCP with IYARI in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native IYARI tool
- you want IYARI to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect IYARI to internal APIs, databases, or company systems without modifying IYARI core

Do not use MCP when:
- a built-in IYARI tool already solves the job well
...
- IYARI remains the agent
- MCP servers contribute tools
- IYARI discovers those tools at startup or reload time
...
If you installed IYARI with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- IYARI banner/status should show MCP integration when configured
- ask IYARI what tools it has available
...
## WSL2: bridge IYARI in WSL to Windows Chrome

This is the practical setup when:

- IYARI runs inside WSL2
...
In this setup, IYARI does **not** connect to Chrome directly. Instead:

- IYARI runs in WSL
- IYARI starts a local stdio MCP server
...
- IYARI stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on IYARI core browser transport
...
Then start a fresh IYARI session or run:
...
Once loaded, IYARI can use the MCP-prefixed browser tools directly. For example:
...
If IYARI runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start IYARI from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start IYARI from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in IYARI:
...
2. IYARI-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So IYARI will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want IYARI to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want IYARI to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. IYARI remains the agent; Open Scaffold remains the repo-local record.
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want IYARI to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize IYARI to merge, publish, deploy, or spawn runtimes.
...
Now IYARI can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing IYARI core.
...
Because IYARI now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.

...
sidebar_position: 3
title: 'Learning Path'
description: 'Choose your learning path through the Orion Agent documentation based on your experience level and goals.'
---

# Learning Path

Orion Agent can do a lot — CLI assistant, Telegram/Discord bot, task automation, RL training, and more. This page helps you figure out where to start and what to read based on your experience level and what you're trying to accomplish.

:::tip Start Here
If you haven't installed Orion Agent yet, begin with the [Installation guide](/getting-started/installation) and then run through the [Quickstart](/getting-started/quickstart). Everything below assumes you have a working installation.
:::

:::tip First-time provider setup
First-time users almost always want `orion setup --portal` — one OAuth covers a model plus the four Tool Gateway tools (search/image/TTS/browser). See [Nous Portal](/integrations/nous-portal).
:::
...
- **Just browsing?** Check the [Key Features](#key-features-at-a-glance) table for a quick overview of everything Orion Agent can do.
...
Use Orion Agent as an interactive terminal assistant for writing, reviewing, and running code.
...
:::tip
Pass files directly into your conversation with context files. Orion Agent can read, edit, and run code in your projects.
:::

### "I want a Telegram/Discord bot"

Deploy Orion Agent as a bot on your favorite messaging platform.
...
7. [Use Voice Mode with Orion](/guides/use-voice-mode-with-orion)
...
:::tip
Cron jobs let Orion Agent run tasks on a schedule — daily summaries, periodic checks, automated reports — without you being present.
:::
...
Extend Orion Agent with your own tools and reusable skill packages.

1. [Plugins](/user-guide/features/plugins)
2. [Build a Orion Plugin](/developer-guide/plugins)
...
page is for built-in Orion core development, not the usual user/custom-tool path.
:::

### "I want to train models"

Use reinforcement learning to fine-tune model behavior with Orion Agent's RL training pipeline (powered by [Atropos](https://github.com/zacharyjleach-stack/Aries)).

1. [Quickstart](/getting-started/quickstart)
2. [Configuration](/user-guide/configuration)
3. [Atropos RL Environments](https://github.com/zacharyjleach-stack/Aries) (external)
4. [Provider Routing](/user-guide/features/provider-routing)
5. [Architecture](/developer-guide/architecture)

:::tip
RL training works best when you already understand the basics of how Orion Agent handles conversations and tool calls. Run through the Beginner path first if you're new.
:::

### "I want to use it as a Python library"

Integrate Orion Agent into your own Python applications programmatically.
...
| **Code Execution** | Run Python scripts that call Orion tools programmatically | [Code Execution](/user-guide/features/code-execution) |

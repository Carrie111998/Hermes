...
4. **Dependency sync** — on non-Termux installs, refreshes dependencies from the hash-verified lockfile (`uv sync --locked --inexact --extra all`), falling back to `uv pip install -e ".[all]"` when the lockfile is unavailable or sync fails; Termux stays on its curated pip-based install path
...
# Re-sync dependencies from the hash-verified lockfile
UV_PROJECT_ENVIRONMENT="$VIRTUAL_ENV" uv sync --locked --inexact --extra all
# (fallback if the sync fails: uv pip install -e ".[all]")

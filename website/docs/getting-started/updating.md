...
If you *deliberately* run a custom branch (local patches maintained on top of main), set `updates.parked_branch_strategy: update_in_place` in `config.yaml`. The updater stays on that branch, auto-stashes tracked and untracked edits, then updates the committed overlay in place. A fast-forward is used when possible; on divergence Hermes rebases the proven local range onto `origin/main` and aborts back to the original committed HEAD on conflict. After a successful update it restores the stash (interactive terminal runs ask first; non-interactive runs follow `updates.non_interactive_local_changes`). Commit IDs can change during the rebase. `hermes update --switch-branch` overrides back to the switch path for one run.

A dirty parked branch still fails closed when it has no unique committed overlay, when the branch cannot be verified, or when a merge/rebase/cherry-pick/revert is already in progress. In those cases Hermes restores any temporary stash, leaves the branch in place, and marks the code update **SKIPPED** with recovery guidance. The completion line always shows the actual branch and HEAD (`✓ Update complete! [main @ 30fcf9580]`) so drift is visible at a glance. Set `updates.auto_switch_parked_branch: false` in `config.yaml` to disable the auto-switch entirely.

...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Orion Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Orion setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Orion Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.
...
- Setting up Orion for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Orion working on my machine | `orion setup` | Run a real chat and verify it responds |
| I already know my provider | `orion model` | Save the config, then start chatting |
| I want a bot or always-on setup | `orion gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `orion model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `orion model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Orion cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Orion Agent
### With the Orion Desktop installer on macOS or Windows (recommended)
...
The single most important setup step. Use `orion model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `orion setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `orion update`. Re-enable anything later with `orion tools`, seed skills with `orion skills opt-in --sync`, or tune settings with `orion setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `orion model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `orion model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `orion model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `orion_cli/models.py` may change between releases) | `orion model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `orion model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `orion model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `orion model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `orion model` (requires `copilot` CLI + `copilot login`) |
...
Orion Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `orion model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Orion separates secrets from normal config:

- **Secrets and tokens** → `~/.orion/.env`
- **Non-secret settings** → `~/.orion/config.yaml`
...
```bash
orion            # classic CLI
orion --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Orion ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `orion` vs `orion --tui`.
:::
...
```bash
orion --continue    # Resume the most recent session
orion -c            # Short form
```
...
```bash
orion gateway setup    # Interactive platform configuration
```
...
- `orion tools` — tune tool access per platform
- `orion skills` — browse and install reusable workflows
...
```bash
orion config set terminal.backend docker    # Docker isolation
orion config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `orion egress setup && orion egress start`; `orion setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Orion install directory (the curl installer placed it at
# ~/.orion/orion-agent on Linux/macOS or %LOCALAPPDATA%\orion\orion-agent on Windows):
...
Skills are on-demand instruction documents that teach Orion how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Orion ships with a catalog of bundled skills already installed in `~/.orion/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
orion skills browse                      # list everything available
orion skills search kubernetes           # find skills by keyword
orion skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `orion skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Orion ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.orion/orion-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Orion opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `orion model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `orion gateway setup` and check `orion gateway status` |
| `orion --continue` can't find old session | Switched profiles or session never saved | Check `orion sessions list` and confirm you're in the right profile |
...
| `orion doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
| `orion tools` | Configure which tools are enabled per platform |
| `orion setup` | Full setup wizard (configures everything at once) |
| `orion doctor` | Diagnose issues |
| `orion update` | Update to latest version |
| `orion gateway` | Start the messaging gateway |
| `orion --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-orion-to-another-machine)** — `orion backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch

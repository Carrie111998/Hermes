...
| `kanban` | `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_unblock` | Multi-agent coordination tools. Dispatcher workers receive task lifecycle tools automatically. Board-routing tools (`list`, `create`, `link`, `unblock`) require the active profile to explicitly enable `kanban` in `platform_toolsets.cli`; an `all`/`*` wildcard does **not** grant routing. `delegate_task` children are not Kanban run owners: their schema strips/disables this toolset and runtime guards reject direct board mutations, even if parent `HERMES_KANBAN_*` env vars are present. |

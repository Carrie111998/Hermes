...
| `HERMES_FILE_MUTATION_VERIFIER` | Enable per-turn file-mutation verification (default: `true`). When enabled, Hermes gives the agent one backstage recovery pass after a failed `write_file` / `patch`, then prevents an unresolved failure from being reported as success. Ordinary replies show only a concise blocked message; raw paths and tool errors appear only when explicitly requested. Set to `0`, `false`, `no`, or `off` to suppress. Mirrors `display.file_mutation_verifier` in `config.yaml`; the env var wins when set. |

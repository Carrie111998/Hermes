...
| `HERMES_KANBAN_DB` | Pin the kanban database file path directly. In the dispatcher→worker handoff (`board=None`) this env var wins unconditionally. When a caller passes an explicit `board=` (or `scoped_current_board`), the dispatcher-injected `HERMES_KANBAN_BOARD` identity is used to distinguish a same-board relocated pin (pin preserved) from a genuine cross-board read (explicit board honoured with a warning) from an ambiguous relocation (raises). See `kanban_db_path()` (task t_e96dd0cb) |

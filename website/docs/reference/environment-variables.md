...
| `HERMES_DASHBOARD_OIDC_EXTRA_AUTHORIZE_PARAMS` | Extra IdP-specific `/authorize` parameters for the self-hosted OIDC provider, as a urlencoded string (e.g. `access_type=offline&prompt=consent` for Google — without it Google issues no refresh token and the dashboard forces a full re-login at every ID-token expiry). Standard OIDC parameters cannot be overridden through it. Overrides `dashboard.oauth.self_hosted.extra_authorize_params`. |

...
| `HERMES_STREAM_RETRIES` | Overall number of same-provider stream reconnect attempts (default: `3`), including zero-chunk stall recovery. Set to `0` to prevent reconnects while retaining stall diagnosis and configured provider fallback. |

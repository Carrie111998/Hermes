...
| `HERMES_ALWAYS_NAME_PREFIX` | Prefix every stored inbound message with the sender's `[Name]`, not just messages in shared multi-user sessions, so DM transcripts keep a durable record of who was speaking. Truthy values: `1`, `true`, `yes`, `on` (default: off — prefix only in shared sessions). Bridged from `gateway.always_name_prefix` in `config.yaml` (config-authoritative). |

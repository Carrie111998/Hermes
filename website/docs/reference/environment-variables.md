...
| `HERMES_DASHBOARD_OIDC_CLIENT_ID` | OIDC client id (authorization-code + PKCE) for the self-hosted OIDC provider. Required to activate it. Supports public and confidential clients. Overrides `dashboard.oauth.self_hosted.client_id`. |
...
| `HERMES_DASHBOARD_OIDC_CLIENT_SECRET` | Optional client secret for a confidential self-hosted OIDC client. PKCE remains required. Store this credential in `~/.hermes/.env`. Overrides `dashboard.oauth.self_hosted.client_secret`. |
| `HERMES_DESKTOP_REMOTE_URL` | (Desktop side) Base URL for the token-auth-only global/default connection override, e.g. `http://host:9119`. Must be set together with `HERMES_DESKTOP_REMOTE_TOKEN`; the override disables the global/default Gateway controls and interactive sign-in. Named profiles can retain explicit Cloud, Remote URL, or SSH overrides. |
| `HERMES_DESKTOP_REMOTE_TOKEN` | (Desktop side) Static session token paired with `HERMES_DESKTOP_REMOTE_URL`. Both variables are required. Use the in-app Remote URL mode instead for interactive password/OAuth/OIDC sign-in. |

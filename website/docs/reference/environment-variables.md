...
| `HERMES_GATEWAY_BUSY_INPUT_MODE` | Default gateway busy-input behavior: `queue`, `steer`, `reject`, or `interrupt`. Configure `reject` here or with `display.busy_input_mode` in `config.yaml`; it discards ordinary text and media follow-ups while the current turn is active. |
| `HERMES_GATEWAY_BUSY_ACK_ENABLED` | Whether the gateway sends an acknowledgment message (⚡/⏳/⏩) when a user sends input while the agent is busy (default: `true`). Set to `false` to suppress these messages entirely. In `queue`, `steer`, and `interrupt` modes, only the chat reply is silenced. In `reject` mode, text and media follow-ups are still discarded without a confirmation. Bridged from `display.busy_ack_enabled` in `config.yaml`. |

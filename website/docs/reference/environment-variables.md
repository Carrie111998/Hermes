...
| `HERMES_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Auto-disabled for local providers. Triggers connection kill if no *progress* arrives within this window. Keep-alive frames (Anthropic `ping` events, empty OpenAI-style deltas) do not reset it — only content, reasoning, tool calls, a finish reason, or usage do. |
| `HERMES_STREAM_MAX_CALL_SECONDS` | Wall-clock ceiling for a single streaming call in seconds (default: `HERMES_API_TIMEOUT`, i.e. `1800`, and never lower than the stale-stream timeout). Unlike the stale timeouts, provider activity never resets it — this is the bound a provider that keeps dripping tokens cannot push back. On expiry the call is aborted so retry/fallback can take over. `0` disables it. Local providers are exempt unless this is set explicitly. Also configurable via `providers.<id>.max_call_seconds` or `providers.<id>.models.<model>.max_call_seconds` in `config.yaml`. |

...
| `HERMES_GATEWAY_BUSY_INPUT_MODE` | Default gateway busy-input behavior: `queue`, `steer`, `hybrid`, or `interrupt`. In `hybrid`, normal follow-ups queue while an edit of the active turn's starter message steers. Can be overridden per chat with `/busy`. |

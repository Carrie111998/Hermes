...
| `/senv [main\|skill <name>\|delete\|list] [KEY=VALUE\|KEY]` (aliases: `/secure-env`, `/secure_env`) | Store, list, or delete a secret in the active profile `.env` (or a skill `.env`) **without sending the value to the model**. Messaging replies never echo the value and ask you to delete the original message. Prefer a vault or browser session auth when you can. See [Messenger `/senv`](/user-guide/secrets/senv). |

...
| `/refresh [--branch]` | Re-scan skills and load current memory API-locally into the next genuine user turn without changing the transcript or frozen prompt; `--branch` reloads the active profile first, then invokes normal branching with an empty name so child construction sees current context |
...
| `/branch [name]` (alias: `/fork`) | Branch the current session (explore a different path). |
| `/refresh [--branch]` | Re-scan skills and load current memory API-locally into the next genuine user turn without restarting or rewriting the transcript; `--branch` reloads the active profile first, then invokes normal branching with an empty name so child construction sees current context. |
...
- `/status`, `/egress`, `/version`, `/whoami`, `/background`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/refresh`, `/rollback`, `/diff`, `/debug`, `/fast`, `/approvals`, `/footer`, `/curator`, `/kanban`, `/topup`, `/suggestions`, `/blueprint`, `/learn`, `/init`, `/sessions`, and `/yolo` work in **both** the CLI and the messaging gateway.

...
| `/architect [--fast\|--deep] [request]` | Build an agent-ready execution prompt. With no request, starts an adaptive prompt-building interview; with a rough request, asks targeted clarifying questions first. `--fast` generates immediately with assumptions, and `--deep` asks deeper scope/source/risk questions. Native Discord `/architect` opens a fresh public architect thread and runs the prompt-building interview there. |

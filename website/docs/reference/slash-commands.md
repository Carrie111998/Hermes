...
| `/llm-pipeline [status\|on\|off\|providers ...]` | Toggle the Rust-backed RI transport used for non-streaming Chat Completions calls when `agent.llm_pipeline.enabled` is true. `providers ...` sets an optional provider allowlist (for example `/llm-pipeline providers ollama-launch openrouter`). Effective on next session. |
...
| `/llm-pipeline [status\|on\|off\|providers ...]` | Toggle the Rust-backed RI transport used for non-streaming Chat Completions calls. `status` prints current transport and provider state; `on/off` enables or disables transport; `providers ...` sets an optional provider allowlist. |

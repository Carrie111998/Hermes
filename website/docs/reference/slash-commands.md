...
| `/branch [name] [--stay]` (alias: `/fork`) | Branch the current session (explore a different path). With `--stay` the copy is created in the background and you remain in the current session — a `/resume` hint for the branch is printed instead. |
...
| `/branch [name] [--stay]` (alias: `/fork`) | Branch the current session (explore a different path). `--stay` keeps the chat on its current session and returns a `/resume` hint for the new branch. |

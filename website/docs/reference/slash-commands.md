...
| `/history` | CLI: show conversation history (respects `/timestamps`). TUI: open the History Timeline overlay to browse the current transcript, filter rows, copy messages, or branch/retry from persisted user and assistant turns. See [History Timeline](../user-guide/tui-history-timeline.md). |

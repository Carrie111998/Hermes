...
| `/profile [name]` | Show the active profile, or restart terminal chat under another profile and resume that profile's last session when one exists. Dashboard Chat uses its profile dropdown instead. |

...
| `/provider [DeepInfra \| sort price\|throughput\|latency \| only/ignore A,B \| require-parameters \| data-collection allow\|deny \| clear] [--global]` | **CLI only.** Set OpenRouter provider routing for this session — force a specific provider (`/provider DeepInfra`), auto-rank cheapest first (`/provider sort price`), allowlist/blocklist providers, or clear back to OpenRouter default. Applies from the next API call; `--global` persists to the `provider_routing` config section. Only affects OpenRouter / Nous Portal routes. See [Provider Routing](../user-guide/features/provider-routing.md). |

...
| `protect-workspace <list\|set\|allow\|remove>` | Manage board-scoped exact-owner leases for sensitive workspace roots. `set <root> --owner <task-id>` records the owner's exact id/title/workspace-kind tuple; `allow <root> --task <task-id>` adds an exact-tuple exception; `list [--json]` inspects policies; `remove <root>` deletes one. |

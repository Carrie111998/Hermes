...
| `--script` | Filter/transform script under `~/.hermes/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload. Empty stdout, `[SILENT]`, or an explicit ignore result intentionally ignores the webhook; execution failures return HTTP 500 so the provider can retry. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |

...
| `dispatch` | Preview one manual dispatcher pass on the active board. Bare `dispatch` never spawns; pass `--run` to start eligible workers and optionally `--assignee <profile>` to scope the preview or run to one lane. Explicit runs print per-worker receipts (or return them in the JSON `receipts` array) and persist matching `dispatch_receipt` task events. Gateway, daemon, and direct `dispatch_once` behavior is unchanged. Flags: `--run`, `--assignee P`, `--dry-run`, `--max N`, `--failure-limit N`, `--json`. |

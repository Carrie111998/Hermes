...
| `--no-tools` | Disable all model tools for the session. Cannot be combined with `--toolsets`. |
...
hermes chat --safe-mode --no-tools --max-turns 1 -q "Reply without using tools"
```

...
| `kanban_unblock` | Move a blocked task to `ready` when all parents are done or archived, or `todo` while any parent remains open. Orchestrator-only; hidden from dispatcher-spawned task workers. | profile with `kanban` toolset |

...
| `write_file` | Write content to a file, completely replacing existing content. Use this instead of echo/cat heredoc in terminal. Creates parent directories automatically. OVERWRITES the entire file — use 'patch' for targeted edits. For existing files in host-visible workspaces, the file must be read (full, unredacted) or written by the same task first: writes without a current baseline, and writes after an external on-disk modification, are refused before any disk mutation so a stale conversation copy can never clobber newer content. Auto-runs syntax checks on .py/.json/.yaml/.toml and other linted languages; only NEW errors introduced by the write are surfaced. | — |

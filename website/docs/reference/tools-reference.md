...
Registered when the agent is either (a) spawned by the kanban dispatcher (`HERMES_KANBAN_TASK` is set) or (b) running in a profile whose CLI toolsets explicitly enable `kanban`. Task-scoped workers receive lifecycle tools for their assigned task. Board-routing tools require the active profile's explicit `kanban` capability, including when that profile is itself a dispatcher worker. See [Kanban Multi-Agent](/user-guide/features/kanban) for the full workflow.
...
| `kanban_list` | List board tasks with filters. Routing-only. | profile with explicit CLI `kanban` toolset |
...
| `kanban_create` | Create assigned tasks for fan-out or dependent review. Rejects unknown profile names. Routing-only. | profile with explicit CLI `kanban` toolset |
| `kanban_link` | Link tasks with a parent → child dependency edge. Routing-only. | profile with explicit CLI `kanban` toolset |
| `kanban_unblock` | Move a blocked task to `ready` when all parents are done, or `todo` while any parent remains open. Routing-only. | profile with explicit CLI `kanban` toolset |

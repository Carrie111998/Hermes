...
| `kanban_unlink` | Remove one existing parent → child edge through the audited store operation, recompute child readiness, and record an event. Repeated calls for an absent edge fail. Orchestrator-only. | profile with `kanban` toolset |

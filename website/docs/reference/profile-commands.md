...
This page covers all commands related to [Orion profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `orion profile` without a subcommand shows help.
...
Sets `<name>` as the active profile. All subsequent `orion` commands (without `-p`) will use this profile.
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `orion profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `orion update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.orion`), use `orion skills opt-out` / `orion skills opt-in`. |
...
:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.orion`) cannot be deleted — use `orion uninstall` to remove everything.
:::
...
This shows the profile's Orion home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Orion installation.
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `orion -p telemetry`). |
...
Orion version, author, env var requirements, the source URL/path, and
...
`orion profile list` also shows the distribution name and version in a
`Distribution` column, and `orion profile show <name>` / `delete <name>`
...
`orion_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Orion version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Orion can clone from.
3. Tell recipients to run `orion profile install <your-repo-url>`.
...
Global flag to run any Orion command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
- `orion profile <TAB>` — subcommands (list, use, create, etc.)
- `orion profile use <TAB>` — profile names
- `orion -p <TAB>` — profile names

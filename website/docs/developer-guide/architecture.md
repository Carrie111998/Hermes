...
sidebar_position: 1
title: "Architecture"
description: "Orion Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Orion Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ orion_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # OrionCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── orion_state.py           # SQLite session/state database with FTS5
├── orion_constants.py       # ORION_HOME, profile-aware paths
...
│
├── orion_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `orion` subcommands (large file)
...
│   ├── skills_config.py      # orion skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # orion tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # orion gateway start/stop
│
...
```text
User input → OrionCLI.process_input()
...
Three discovery sources: `~/.orion/plugins/` (user), `.orion/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `orion plugins` or `config.yaml`.
...
Exposes Orion as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`orion -p <name>`) gets its own ORION_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |

...
The ACP adapter wraps Orion' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
orion acp / orion-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.orion/.env
  -> configure stderr logging
  -> construct OrionACPAgent
...
- `allow_once` -> Orion `once`
- `allow_always` -> Orion `always`
- reject options -> Orion `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Orion tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["orion-acp"])
...
Instead it reuses Orion' runtime resolver:

- `acp_adapter/auth.py`
- `orion_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Orion provider/credentials. It also always advertises a terminal setup auth method (`orion-setup`, args `--setup`) so first-run ACP clients can open Orion' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.orion/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `orion-acp` toolset definition
- `orion_cli/main.py` — `orion acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `orion-acp` script

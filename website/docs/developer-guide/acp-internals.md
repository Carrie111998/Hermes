...
So ACP advertises and uses the currently configured IYARI provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run registry clients can open IYARI's interactive model/provider configuration before starting a normal ACP session.

...
description: "How to build a memory provider plugin for Orion Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Orion Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `orion plugins`.
:::

## Installation Layouts

Orion discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Orion. Closed to new providers — see [CONTRIBUTING](https://github.com/zacharyjleach-stack/Aries/blob/main/CONTRIBUTING.md). |
| User | `$ORION_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.orion/plugins/<name>/` | Opt-in via `ORION_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `orion_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
Orion, in `$ORION_HOME/plugins/<name>/` when installed by a user, or in
`./.orion/plugins/<name>/` for a project-local one:
...
`orion_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."orion_agent.memory_providers"]
...
package layout. No copy under `$ORION_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Orion reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `orion <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          orion_home (str): Active ORION_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `orion memory setup` | **Yes** |
| `save_config(values, orion_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `orion memory setup`:
...
the `messages` parameter; Orion will continue calling them with the legacy
signature.
...
All storage paths **must** use the `orion_home` kwarg from `initialize()`, not hardcoded `~/.orion`:
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `orion my-provider status`, `orion my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `orion <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `orion --help`.
...
def register_cli(subparser) -> None:
    """Build the orion my-provider argparse tree.

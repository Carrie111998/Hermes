...
Bundled providers live in `plugins/memory/<name>/`. New third-party providers
must ship in a standalone repository and install into
`$HERMES_HOME/plugins/<name>/` (normally `~/.hermes/plugins/<name>/`):

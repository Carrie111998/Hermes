...
- **Profile-safe paths**: Never hardcode `~/.hermes` — use `get_hermes_home()` from `hermes_constants` for code paths and `display_hermes_home()` for user-facing messages. See the [architecture and runtime guide](https://github.com/NousResearch/hermes-agent/blob/main/docs/agents/architecture-and-runtime.md#profiles-multi-instance-support) for full rules.

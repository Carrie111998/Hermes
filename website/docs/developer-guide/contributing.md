...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Orion Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Orion Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Orion core? Start with [Build a Orion Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Orion itself? Start with [Adding Tools](./adding-tools.md)
...
The installer creates the Orion venv, wires the `orion` command, stamps the
install method for `orion update`, and clones the full git project into
`$ORION_HOME/orion-agent` (usually `~/.orion/orion-agent`). That keeps your
...
You can also run a fully isolated Orion instance (throwaway ORION_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m orion_cli.main
scripts/dev-sandbox.sh --persistent python -m orion_cli.main desktop  # state survives restarts, but lives in the worktree :)
```

### Manual clone fallback

Use this only if you intentionally do not want Orion' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `orion` entrypoint from this venv; running the
system `python3 -m orion_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.orion/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.orion/config.yaml
touch ~/.orion/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.orion/.env
```

### Run

```bash
# The standard installer already put `orion` on PATH.
orion doctor
orion chat -q "Hello"
```

If you used the manual clone fallback, run `./orion` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/orion" ~/.local/bin/orion
```
...
- **Profile-safe paths**: Never hardcode `~/.orion` — use `get_orion_home()` from `orion_constants` for code paths and `display_orion_home()` for user-facing messages. See [AGENTS.md](https://github.com/zacharyjleach-stack/Aries/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Orion has terminal access. Security matters.
...
2. **Test manually**: Run `orion` and exercise the code path you changed
...
Projects built on (or reviewed by) Orion can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Orion to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.

## Reporting Issues

- Use [GitHub Issues](https://github.com/zacharyjleach-stack/Aries/issues)
- Include: OS, Python version, Orion version (`orion --version`), full error traceback
...
- **Discord**: [github.com/zacharyjleach-stack/Aries](https://github.com/zacharyjleach-stack/Aries)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/zacharyjleach-stack/Aries/blob/main/LICENSE).

...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.nastech/.env
```

### Run

```bash
# The standard installer already put `nastech` on PATH.
nastech doctor
nastech chat -q "Hello"
```

If you used the manual clone fallback, run `./nastech` from the checkout or
...
- **Profile-safe paths**: Never hardcode `~/.nastech` — use `get_nastech_home()` from `nastech_constants` for code paths and `display_nastech_home()` for user-facing messages. See [AGENTS.md](https://github.com/NastechResearch/nastech-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Nastech can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Nastech to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
- **Discord**: [discord.gg/NastechResearch](https://discord.gg/NastechResearch)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NastechResearch/nastech-agent/blob/main/LICENSE).

...
**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$NASTECH_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `nastech plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `nastech_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `nastech_cli/main.py` | Accepts `acme-inference` |
| `nastech model` picker | `nastech_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `nastech doctor` | `nastech_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `nastech setup` | `nastech_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `nastech_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `nastech model` picker |
...
| `plugins/model-providers/nastech/` | Attribution tags, "omit reasoning when disabled" |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.nastech/plugins/model-providers/gmi/__init__.py`:
...
| `oauth_external` | User signs in elsewhere, tokens land in `auth.json` | Anthropic OAuth, MiniMax OAuth, Qwen Portal, Nastech Portal |
...
Point `NASTECH_HOME` at a temp directory so you don't pollute your real config:
...
The general `PluginManager` (the thing `nastech plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$NASTECH_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

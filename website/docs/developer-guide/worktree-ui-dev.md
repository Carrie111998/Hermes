...
| Variable | Meaning |
|----------|---------|
| `ORION_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ORION_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ORION_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Orion itself — they're private to these helpers. The variables Orion *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `orion --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_orion_root)" || { echo "htui: not in a Orion checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ORION_MAIN_CHECKOUT/.venv/bin/python" -m orion_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ORION_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_orion_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ORION_TUI_DIR` are mutually exclusive
`ORION_TUI_DIR` points Orion at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `orion --tui --dev` exits with an error. Run `unset ORION_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_orion_root)" || { echo "hgui: not in a Orion checkout" >&2; return 1; }
  deps="${ORION_GUI_DEPS_CHECKOUT:-$ORION_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _orion_link_deps "$desktop" "$deps/apps/desktop"
    _orion_link_deps "$root" "$deps"
  else
  ...
  trap '_orion_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ORION_DESKTOP_ORION_ROOT="$root" \
    ORION_DESKTOP_PYTHON="$ORION_MAIN_CHECKOUT/.venv/bin/python" \
    ORION_DESKTOP_IGNORE_EXISTING=1 \
    ORION_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ORION_DESKTOP_ORION_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `orion`. |
| `ORION_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ORION_DESKTOP_IGNORE_EXISTING` | Ignores any `orion` on `PATH` so it can't shadow the worktree. |
| `ORION_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Orion checkout.
_orion_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/orion_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_orion_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'orion_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `orion --tui --dev` and the `ORION_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/zacharyjleach-stack/Aries/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `ORION_*` variable Orion reads

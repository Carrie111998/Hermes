...
Orion Agent uses a SQLite database (`~/.orion/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `orion_state.py`


## Architecture Overview

```
~/.orion/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `orion_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `orion_state.py` for the exact SQL.
...
Multiple orion processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.orion/state.db
...
Default path: `~/.orion/state.db`

This is derived from `orion_constants.get_orion_home()` which resolves to
`~/.orion/` by default, or the value of `ORION_HOME` environment variable.

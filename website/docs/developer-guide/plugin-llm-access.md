...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Orion already has applies. The plugin never sees a
...
[`orion-example-plugins`](https://github.com/zacharyjleach-stack/Aries/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
orion-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/zacharyjleach-stack/Aries/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Orion provider id (e.g. "openrouter")
...
using Orion built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Orion provider
        # (must be one Orion already knows about — same names as
        # `orion model` and config.yaml model.provider).
        ...
        # plugin sends — Orion does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.orion/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Orion' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Orion subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/zacharyjleach-stack/Aries/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/zacharyjleach-stack/Aries/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/zacharyjleach-stack/Aries/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/zacharyjleach-stack/Aries/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`

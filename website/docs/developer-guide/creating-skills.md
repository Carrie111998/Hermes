...
description: "How to create skills for IYARI — SKILL.md format, guidelines, and publishing"
---

# Creating Skills

Skills are the preferred way to add new capabilities to IYARI. They're easier to create than tools, require no code changes to the agent, and can be shared with the community.
...
Use `required_environment_variables` when a skill needs an API key or token. Missing values do **not** hide the skill from discovery. Instead, IYARI prompts for them securely when the skill is loaded in the local CLI.
...
The user can skip setup and keep loading the skill. IYARI never exposes the raw secret value to the model. Gateway and messaging sessions show local setup guidance instead of collecting secrets in-band.
...
When loaded, IYARI checks if these files exist. Missing files trigger `setup_needed`. Existing files are automatically:
...
Prefer stdlib Python, curl, and existing IYARI tools (`web_extract`, `terminal`, `read_file`). If a dependency is needed, document installation steps in the skill.
...
Bundled skills (in `skills/`) ship with every IYARI install. They should be **broadly useful to most users**:
...
**Installing a blueprint.** When you install a skill that carries a `blueprint:` block, IYARI registers it as a **suggested cron job** rather than scheduling it. Scheduling is **opt-in** — installing never silently creates a recurring job. You review and accept it via `/suggestions`:
...
IYARI can *propose* automations and let you accept them with one tap, instead of making you assemble cron jobs by hand. Every proposal flows through one surface — the `/suggestions` command — regardless of where it came from:
...
IYARI can now consume third-party skills from multiple external discovery models:

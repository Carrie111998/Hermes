...
description: "Build wrapper CLIs that extend the Orion TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Orion exposes protected extension hooks on `OrionCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
Called after Orion registers its own keybindings and before the layout is built. Add your keybindings to `kb`.

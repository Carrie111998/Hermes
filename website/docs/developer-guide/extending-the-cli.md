...
description: "Build wrapper CLIs that extend the IYARI TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

IYARI exposes protected extension hooks on `HermesCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
Called after IYARI registers its own keybindings and before the layout is built. Add your keybindings to `kb`.

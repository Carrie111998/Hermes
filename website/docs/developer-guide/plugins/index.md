...
title: "Build a Orion Plugin"
description: "Step-by-step guide to building a complete Orion plugin with tools, hooks, data files, and skills"
---

# Build a Orion Plugin

This guide walks through building a complete Orion plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Orion has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `ORION_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.orion/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `orion skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `Orion/orion-agent`. Users install them into `~/.orion/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Orion Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/zacharyjleach-stack/Aries/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

Orion can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Orion already owns. It does not replace native `plugin.yaml` plus
...
Orion validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Orion.
...
Portable `streamable-http` entries are routed through Orion' existing native
...
same full-trust posture as other installed Orion plugins.
...
records it as Published. Orion keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Orion does not expose a
...
continue to work after a normal Orion upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Orion inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Orion releases can therefore load a plugin whose manifest contains
...
Orion enforces this contract with frozen external-plugin fixtures discovered
from an isolated `ORION_HOME`. Those tests load and invoke the plugin through
...
`orion plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Orion itself. It reports invalid hook names, callbacks that do
...
```bash
orion plugins doctor . --ci
```

Doctor uses a temporary `ORION_HOME`, restores plugin registration state after
...
This tells Orion: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `orion_cli/plugin_capabilities.py` for the
...
`orion_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`orion_agent.plugins` entry point:

```toml
[project.entry-points."orion_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."orion_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Orion reads these from installed metadata without importing your code, so
`orion plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Orion understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Orion validates them, and `orion plugins install` / `orion plugins doctor` surface missing ones with a `pip install` hint, but Orion **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Orion' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/zacharyjleach-stack/Aries/issues/64165) and
[#15220](https://github.com/zacharyjleach-stack/Aries/issues/15220). Plugin
...
4. **Accept `**kwargs`:** Orion may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `orion my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Orion continues fine
...
Use plugin-relative config keys for user-visible behavior. Orion resolves them
...
`<ORION_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `ORION_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:
...
The same logs are always written to `~/.orion/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:
...
- **Not enabled in config** — plugins are opt-in. Run `orion plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.orion/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `orion plugins update` / `remove` git-pull or delete it — your
...
# <orion home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.orion/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.orion/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `orion plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Orion into installing arbitrary packages — only specs Orion itself ships are eligible |
...
auditable in `~/.orion/logs/agent.log`. Plugins load after built-in
...
does not ship with Orion core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `orion plugins list`
...
loader, the plugin is disabled and Orion continues. The gate exists
...
You normally never edit this key by hand. `orion plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Orion injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
- **The system prompt is Orion's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `orion_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Orion sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`orion.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Orion still loads on an older one.

### Register CLI commands

Plugins can add their own `orion <plugin>` subcommand tree:
...
After registration, users can run `orion my-plugin status`, `orion my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `orion name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `orion chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `ORION_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `orion <subcommand>` (e.g. `orion kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "orion kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Orion from a hook.
...
Orion has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.orion/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `orion gateway status` without SDK instantiation.
...
Orion also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Orion without any Python plugin. Declare them in `~/.orion/config.yaml`:
...
Orion connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.orion/hooks/<name>/`:
...
# Plugin auto-discovered on next orion startup
```
...
```python
# Wrong — will break if Orion passes extra context
def handler(args):
    ...

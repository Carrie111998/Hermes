...
| [`transform_memory_context`](/user-guide/features/hooks#built-in-memory-governance) | Native memory snapshot build | `target: str, entries: tuple, char_limit: int` | replacement entry list/tuple |
| [`pre_memory_write`](/user-guide/features/hooks#built-in-memory-governance) | Before a native durable memory mutation | proposed write fields + advisory entries | optional block/skip/transform dict |
| [`post_memory_write`](/user-guide/features/hooks#built-in-memory-governance) | After a native durable memory mutation | committed write fields + resulting entries | ignored |

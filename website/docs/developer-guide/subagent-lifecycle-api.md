...
Plugins can launch and supervise fresh IYARI child sessions without importing
...
active agent turn fails closed with `No active IYARI parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until IYARI can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; IYARI's existing unsafe-tool block remains enforced.

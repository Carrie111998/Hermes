...
Plugins can launch and supervise fresh Orion child sessions without importing
...
active agent turn fails closed with `No active Orion parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Orion can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Orion's existing unsafe-tool block remains enforced.

...
description: "How to add a new inference provider to Orion Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Orion can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `orion model` menu entries
...
1. `orion_cli/auth.py` decides how credentials are found.
2. `orion_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `orion_cli/models.py` and `orion_cli/main.py` make the provider show up in the CLI. (`orion_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `orion_cli/auth.py`
2. `orion_cli/models.py`
3. `orion_cli/runtime_provider.py`
4. `orion_cli/main.py`
...
:::tip
`orion_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `orion setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$ORION_HOME/plugins/model-providers/` both get picked up.
...
7. `orion model` menu includes the provider
8. `orion setup` wizard delegates to `main.py` automatically
...
User plugins at `$ORION_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `orion model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `orion_cli/auth.py`
- `_PROVIDER_LABELS` in `orion_cli/models.py`
- `_PROVIDER_ALIASES` in both `orion_cli/auth.py` and `orion_cli/models.py`
- CLI `--provider` choices in `orion_cli/main.py`
...
## Step 2: Add auth metadata in `orion_cli/auth.py`
...
- What env vars should Orion check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `orion_cli/models.py`
...
## Step 4: Resolve runtime data in `orion_cli/runtime_provider.py`
...
Be careful with API-key precedence. Orion already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `orion_cli/main.py`

A provider is not discoverable until it shows up in the interactive `orion model` flow.

Update these in `orion_cli/main.py`:
...
:::tip
`orion_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `orion model` and `orion setup` automatically.
:::
...
When you add a native provider, double-check that Orion is only sending fields that provider actually understands.
...
- `tests/orion_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/orion_cli/test_model_switch_custom_providers.py` (and adjacent `tests/orion_cli/test_model_switch_*.py`)
- `tests/orion_cli/test_setup_model_provider.py`
...
python -m pytest tests/orion_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/orion_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m orion_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m orion_cli.main model
python -m orion_cli.main setup
```
...
- [ ] `ProviderConfig` added in `orion_cli/auth.py`
- [ ] aliases added in `orion_cli/auth.py` and `orion_cli/models.py`
- [ ] model catalog added in `orion_cli/models.py`
- [ ] runtime branch added in `orion_cli/runtime_provider.py`
- [ ] CLI wiring added in `orion_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `orion model` but not `orion setup`

...
title: "Desktop Plugin SDK (@orion/plugin-sdk)"
description: "Extend the native Orion Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Orion Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `OrionPlugin`.
It imports one module — `@orion/plugin-sdk` — and gets everything: the app's
...
`$ORION_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Orion. This page is the **native
desktop app** (`orion desktop`) SDK — the `@orion/plugin-sdk` module and
`$ORION_HOME/desktop-plugins/`. The **web dashboard** (`orion dashboard`) has
its own, unrelated plugin system on `window.__ORION_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Orion Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ORION_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$ORION_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `OrionPlugin` contract, appear in **Settings → Plugins**,
...
Create `$ORION_HOME/desktop-plugins/hello/plugin.js` (that's `~/.orion/...`
by default, or `~/.orion/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.orion/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@orion/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Orion' }),
      jsx('div', {
...
The only importable specifiers are `@orion/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `OrionPlugin`:
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `orion.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@orion/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@orion/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@orion/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@orion/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@orion/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@orion/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@orion/plugin-sdk'
...
`window.orion.send('get-price eth')` (or a declarative
`<button data-orion-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@orion/plugin-sdk'
...
for requests; `targetProfile` is the backend Orion profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Orion
...
ctx.os.notify({
  ...
  // Body click → focus Orion + navigate. Same vocabulary as OS deep links:
...
`activate` is deeplink-compatible: `orion://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `orion://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@orion/plugin-sdk'
...
```javascript
import { queryClient } from '@orion/plugin-sdk'
...
a package into `~/.orion/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.orion/plugins` is not reachable as a filesystem —
...
the `orion://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Orion plugin and declare it in a
`manifest.json`:
...
orion-agent codebase directly (`orion_state`, `orion_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.orion/`)
...
(`orion.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `OrionPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@orion/plugin-sdk` alias.
2. It's still lint-fenced to `@orion/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@orion/plugin-sdk`, `react`,
...
| Plugin contract | `OrionPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `OrionOpenTarget`, `Contribution` |
...
### Agents: the `orion-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`orion-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ORION_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `orion logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@orion/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.orion/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.orion/logs/errors.log` for

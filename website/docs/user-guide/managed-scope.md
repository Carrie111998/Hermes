...
`~/.orion/config.yaml`, `~/.orion/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/orion`:

```text
/etc/orion/
├── config.yaml     # managed config layer (wins over ~/.orion/config.yaml)
└── .env            # managed env layer (wins over ~/.orion/.env + shell)
```
...
The location can be relocated with the `ORION_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ORION_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Orion.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ORION_MANAGED_DIR=/opt/org/orion-policy
```

:::warning
A user who can set `ORION_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `orion doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/orion/config.yaml` (managed) | `/etc/orion/.env` (managed) |
| 2 | `~/.orion/config.yaml` (user) | `~/.orion/.env` (user) |
...
```bash
orion config        # shows a header naming the managed source + the pinned keys
orion doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Orion refuses and names the source:

```bash
$ orion config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/orion/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `orion config set` / setup will not write
...
sudo tee /etc/orion/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/orion/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/orion
sudo chmod 0644 /etc/orion/config.yaml /etc/orion/.env
```

Changes take effect on the next Orion start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`orion doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Orion as `root`), managed scope is advisory.

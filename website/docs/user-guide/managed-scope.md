...
files. It is **never persisted** to any `.env` by IYARI.
...
If you try to change a managed value, IYARI refuses and names the source:
...
Changes take effect on the next IYARI start (a malformed managed file is logged
...
- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs IYARI as `root`), managed scope is advisory.

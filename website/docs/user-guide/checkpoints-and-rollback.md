...
Orion Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.
...
Or enable globally in `~/.orion/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.orion/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `orion checkpoints` | Show total size, project count, per-project breakdown |
| `orion checkpoints status` | Same as bare `checkpoints` |
| `orion checkpoints list` | Alias for `status` |
| `orion checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `orion checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `orion checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Orion detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.orion/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/orion/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.orion/checkpoints/ at startup
  ...
  # `orion checkpoints prune` command below, with a confirmation prompt.
  auto_prune: true
  retention_days: 7
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `orion checkpoints prune` manually.
...
Orion responds with a formatted list showing change statistics:
...
`/rollback <N>` restores only the files Orion itself changed. Every successful
...
Orion last wrote (you edited it afterwards, or Orion never touched it) is
...
If the ledger is empty (a store created before this feature, or Orion hasn't
...
- **Directory scope** — Orion skips overly broad directories (root `/`, home `$HOME`).
...
~/.orion/checkpoints/
  ...
  │   ├── refs/orion/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `orion checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.orion/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.orion/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:
...
- **Enable checkpoints only when you need them** — `orion chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `orion checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Orion session in its own worktree/branch, with checkpoints as an extra layer.

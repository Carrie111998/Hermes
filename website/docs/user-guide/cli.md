...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Orion Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Orion Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `orion setup --portal` — and you're ready to `orion chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Orion also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `orion --tui` — see the [TUI](tui.md) guide.
:::
...
orion chat --query-file prompt.txt
orion chat --query-file - < prompt.txt

# With a specific model
orion chat --model "anthropic/claude-sonnet-4"

# With a specific provider
orion chat --provider nous        # Use Nous Portal
orion chat --provider openrouter  # Force OpenRouter

# With specific toolsets
orion chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
orion -s orion-agent-dev,github-auth
orion chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
orion --continue             # Resume the most recent CLI session (-c)
orion --resume <session_id>  # Resume a specific session by ID (-r)
orion --resume latest        # Resume the most recent session (same as -c)
orion --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
orion chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
orion -w                         # Interactive mode in worktree
orion -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`orion -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
orion worktree list              # audit: age, size, verdict, reason per tree
orion worktree prune             # remove safe trees + delete merged branches
orion worktree prune --dry-run   # show the plan without changing anything
orion worktree prune --trees-only     # leave local branches alone
orion worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running orion session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.orion/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
The `orion plugins` commands manage native Orion plugins and portable Agent
Plugins v1 packages through the same opt-in workflow:

```bash
orion plugins install owner/repository --no-enable
orion plugins list
orion plugins enable <plugin-name>
orion plugins disable <plugin-name>
orion plugins update <plugin-name>
orion plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Orion currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Orion CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Orion CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ORION_YOLO_MODE` is on (either `orion --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Orion to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.orion/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Orion supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Orion replies |
...
```bash
orion -s orion-agent-dev,github-auth
orion chat -s github-pr-workflow -s github-auth
```

Orion loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.orion/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.orion/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Orion..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Orion also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Orion recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Orion — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Orion is working, Orion prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Orion to the background — just like any terminal process. The shell prints a confirmation:

```
Orion Agent has been suspended. Run `fg` to bring Orion Agent back.
```
...
Resume this session with:
  orion --resume 20260225_143052_a1b2c3
...
```bash
orion --continue                          # Resume the most recent CLI session
orion -c                                  # Short form
orion -c "my project"                     # Resume a named session (latest in lineage)
orion --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
orion --resume "refactoring auth"         # Resume by title
orion --resume latest                     # Resume the most recent session (same as -c)
orion --resume latest --in ./my-project   # Latest session for ./my-project's workspace
orion -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `orion sessions rename <id> <title>` from the command line. Use `orion sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Orion's SQLite state database under `~/.orion/state.db`. The database keeps:
...
Orion immediately confirms the task and gives you back the prompt:

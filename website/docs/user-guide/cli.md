...
One command — `nastech setup --portal` — and you're ready to `nastech chat`. See [Nastech Portal](/integrations/nastech-portal).
:::
...
nastech chat --provider nastech        # Use Nastech Portal
...
nastech --continue             # Resume the most recent CLI session (-c)
nastech --resume <session_id>  # Resume a specific session by ID (-r)
nastech --resume latest        # Resume the most recent session (same as -c)
nastech --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
nastech chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
nastech -w                         # Interactive mode in worktree
nastech -w -z "Fix issue #123"     # Single query in worktree
```

### Plugin management

The `nastech plugins` commands manage native Nastech plugins and portable Agent
Plugins v1 packages through the same opt-in workflow:

```bash
nastech plugins install owner/repository --no-enable
nastech plugins list
nastech plugins enable <plugin-name>
nastech plugins disable <plugin-name>
nastech plugins update <plugin-name>
nastech plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Nastech currently
...
```
 𓄃 claude-sonnet-4-20250514 │ 12.4K/200K │ [██████░░░░] 6% │ $0.06 │ 15m
```
...
Resume this session with:
  nastech --resume 20260225_143052_a1b2c3
...
```bash
nastech --continue                          # Resume the most recent CLI session
nastech -c                                  # Short form
nastech -c "my project"                     # Resume a named session (latest in lineage)
nastech --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
nastech --resume "refactoring auth"         # Resume by title
nastech --resume latest                     # Resume the most recent session (same as -c)
nastech --resume latest --in ./my-project   # Latest session for ./my-project's workspace
nastech -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `nastech sessions rename <id> <title>` from the command line. Use `nastech sessions list` to browse past sessions.

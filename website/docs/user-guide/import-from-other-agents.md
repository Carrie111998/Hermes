...
`hermes import-agent` imports your existing **Claude Code** or **OpenAI Codex CLI** setup into Hermes with one command.. It uses a preview-first pattern: you always see a per-item plan before anything is written, and `--dry-run` never touches disk.

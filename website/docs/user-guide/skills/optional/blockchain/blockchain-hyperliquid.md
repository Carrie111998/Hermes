...
| | |
|---|---|
| Source | Optional — install with `orion skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Orion Agent |
| License | MIT |
...
The script reads `${ORION_HOME:-~/.orion}/.env` for two optional defaults:
...
Helper script: `~/.orion/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.orion/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${ORION_HOME:-~/.orion}/.env`.
...
```bash
python3 ~/.orion/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.orion/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \

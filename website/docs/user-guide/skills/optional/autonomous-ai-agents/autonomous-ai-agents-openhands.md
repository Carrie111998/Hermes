...
| Version | `0.1.0` |
| Author | Tim Koepsel (xzessmedia), IYARI |
| License | MIT |
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from IYARI.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For IYARI-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hermes-agent` (IYARI subagents via `delegate_task`).

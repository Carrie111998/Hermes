---
title: "Honcho — Configure and troubleshoot Honcho memory for Orion"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Orion"
---
...
Configure and troubleshoot Honcho memory for Orion.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `orion skills install official/autonomous-ai-agents/honcho` |
...
| Related skills | [`orion-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-orion-agent) |
...
# Honcho Memory for Orion

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Orion profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/orion#running-honcho-locally-with-orion

### Verify

```bash
orion honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Orion profile's AI peer
...
Honcho models conversations as interactions between **peers**. Orion creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Orion instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Orion run |
| `global` | Single session across all directories |

Manual override: `orion honcho map my-project-name`
...
Each Orion profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block orion.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `orion.coder` host block in `honcho.json`
...
```bash
orion honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Orion when Honcho memory is active.
...
Config file: `$ORION_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `orion honcho setup`. Ensure `memory.provider: honcho` is in `~/.orion/config.yaml`.

### Memory not persisting across sessions
Check `orion honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `orion profile create <name> --clone`. For existing profiles: `orion honcho sync`.
...
| Command | Description |
|---------|-------------|
| `orion honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `orion honcho status` | Show resolved config, connection test, peer info for active profile |
| `orion honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `orion honcho disable` | Disable Honcho for the active profile |
| `orion honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
...
| `orion honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `orion honcho sessions` | List known directory-to-session-name mappings |
| `orion honcho map <name>` | Map current working directory to a Honcho session name |
| `orion honcho identity` | Seed AI peer identity or show both peer representations |
| `orion honcho sync` | Create host blocks for all Orion profiles that don't have one yet |
| `orion honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Orion + Honcho |
| `orion memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `orion memory status` | Show active memory provider and config |
| `orion memory off` | Disable external memory provider |

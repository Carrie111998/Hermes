...
| Version | `1.0.0` |
| Author | Anthropic (adapted by Digital Services LLC) |
| License | Apache-2.0 |
...
**If you encounter errors or unreasonable results, read [TROUBLESHOOTING.md](https://github.com/digital-services-llc/iyari/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) for detailed debugging guidance.**
...
3. **Check output**:
   ...
   - If `status` is `"errors_found"` → Check `error_summary` and read [TROUBLESHOOTING.md](https://github.com/digital-services-llc/iyari/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) for debugging guidance
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In IYARI:

- **If you have any structured financial-data MCP configured** (IYARI supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.

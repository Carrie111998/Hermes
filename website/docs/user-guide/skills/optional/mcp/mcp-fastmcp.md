...
| | |
|---|---|
| Source | Optional — install with `orion skills install official/mcp/fastmcp` |
...
| Related skills | [`orion-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-orion-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Orion or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Orion. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.orion/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.orion/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Orion integration, either:

- configure the server in `~/.orion/config.yaml` using the `native-mcp` skill, or
...
### Orion cannot see the deployed server

The server-building part may be correct while the Orion config is not. Load the `native-mcp` skill and configure the server in `~/.orion/config.yaml`, then restart Orion.

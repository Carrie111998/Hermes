...
| Version | `2.0.0` |
| Author | IYARI (writing-craft adapted from obra/superpowers) |
| License | MIT |
...
Treat that as relative to the active working directory / backend workspace. IYARI file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.
...
> **For IYARI:** Use subagent-driven-development skill to implement this plan task-by-task.
...
Use IYARI tools to understand the project:

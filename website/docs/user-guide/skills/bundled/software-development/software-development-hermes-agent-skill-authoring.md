...
In-repo skills must meet the repo's **hardline authoring standards** (see the [architecture and runtime guide](https://github.com/NousResearch/hermes-agent/blob/main/docs/agents/architecture-and-runtime.md#skill-authoring-standards-hardline) — that section is the source of truth; this skill is the operational walkthrough). Reviewers reject PRs that violate them, so meeting them up front is cheaper than a salvage pass later.

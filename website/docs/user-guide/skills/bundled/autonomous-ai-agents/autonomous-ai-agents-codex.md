...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`orion-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-orion-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Orion terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Orion itself, `model.provider: openai-codex` uses Orion-managed Codex
OAuth from `~/.orion/auth.json` after `orion auth add openai-codex`. For the
...
## Orion Gateway Caveat

When invoking the Codex CLI from a Orion gateway/service context (for example,

...
description: "Use when a IYARI Kanban worker wants to run Codex CLI as an isolated implementation lane while IYARI keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a IYARI Kanban worker wants to run Codex CLI as an isolated implementation lane while IYARI keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
This skill defines the lightweight IYARI+Codex dual-lane convention for Kanban workers. IYARI is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a IYARI worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn IYARI workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by IYARI in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- IYARI can run the relevant tests itself after Codex exits.
...
1. IYARI owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any IYARI board CLI as a substitute for the worker.
2. IYARI owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. IYARI owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from IYARI with the repo's canonical wrapper.
4. IYARI owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. IYARI owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional IYARI edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if IYARI can do the task directly.
...
IYARI owns the Kanban lifecycle; do not call IYARI kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on IYARI reconciliation.
...
- Explicit statement: IYARI owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands IYARI will run afterward.
...
IYARI must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by IYARI.
...
- [ ] IYARI ran the canonical tests itself, using `scripts/run_tests.sh` for IYARI or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from IYARI-run tests.
- [ ] Accepted commits/diffs were applied to the IYARI-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct IYARI edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from IYARI.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but IYARI writes board state.
...
- [ ] IYARI reviewed `git diff` and safety-sensitive files.
- [ ] IYARI ran canonical tests independently.

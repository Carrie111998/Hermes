...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Orion Agent\",
    \"comments\": [
...
```bash
source "${ORION_HOME:-$HOME/.orion}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Orion Agent. Code looks clean — good test coverage, no security concerns."

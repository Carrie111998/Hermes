...
- Give it a name like "orion-agent"
- Select scopes:
...
- Give it a title like "orion-agent-&lt;machine-name>"
...
elif _orion_env="${ORION_HOME:-$HOME/.orion}/.env"; [ -f "$_orion_env" ] && grep -q "^GITHUB_TOKEN=" "$_orion_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_orion_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"

...
| `cron_mode` | `deny` | How [cron jobs](./features/cron.md) behave headlessly when they trigger a dangerous-command prompt. This policy takes precedence over inherited interactive and gateway approval flags because cron runs have no approval listener. `deny` blocks the command (the agent must find another path); `approve` auto-approves everything in cron context. |

...
approvals:
  ...
  mcp_reload_confirm: true        # confirmation when Tool Search is off
...
| `mcp_reload_confirm` | `true` | When Tool Search is off and MCP schemas are exposed directly, `/reload-mcp` asks before rebuilding the model-facing tool set because that invalidates the provider prompt cache. Stable Tool Search mode does not require this confirmation. Users who click **Always Approve** flip this key to `false`. |

...
If you only run one Orion agent, you don't need this page — see
...
[Connecting Desktop to Many Orion Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Orion agents that should all
...
(`ai.orion.gateway-<name>.plist`) or systemd user service
(`orion-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
orion profile create coder
orion profile create personal-bot
orion profile create research
...
```bash
orion config set gateway.multiplex_profiles true
orion gateway restart
```

Equivalently, in the default profile's `~/.orion/config.yaml`:
...
You do **not** run `orion gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `orion gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.orion/profiles/coder/.env`; the default listener key is rejected.
...
home). `orion status` reports the multiplexer and the profiles it serves;
`orion status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/orion-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: orion-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    orion gateway "$action"
  else
    orion -p "$profile" gateway "$action"
  fi
}
...
```bash
orion-gateways start      # start every configured profile
orion-gateways stop       # stop every configured profile
orion-gateways restart    # restart all
orion-gateways status     # status across all
orion-gateways list       # delegates to `orion gateway list`
```

:::tip
The `default` profile is targeted with `orion gateway <action>` (no `-p`),
not `orion -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `orion -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.orion.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/orion-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.orion.gateway.plist` /
...
tail -f ~/.orion/logs/gateway.error.log

# Named profile
tail -f ~/.orion/profiles/<name>/logs/gateway.log
tail -f ~/.orion/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.orion/logs/gateway.log ~/.orion/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
orion logs -f                  # follow default profile
orion -p coder logs -f         # follow one profile
orion logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
orion profile list             # profiles + model + gateway state
orion-gateways status          # full status across every profile
launchctl list | grep orion    # macOS — PIDs and labels
systemctl --user list-units 'orion-gateway-*'   # Linux — units
```
...
The default profile uses `~/.orion/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
orion config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.orion/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=orion --why="gateways running" \
  sleep infinity &
...
`orion-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.orion/.env ~/.orion/profiles/*/.env
```

## Updating the code

`orion update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `orion gateway start` after a previous `orion gateway stop`. The
...
```bash
ps -ef | grep "orion_cli.*-p <profile>"
cat ~/.orion/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.orion.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.orion.gateway-<profile>.plist

# Linux
systemctl --user restart orion-gateway-<profile>.service
```

### Health check

```bash
orion doctor                  # default profile
orion -p <profile> doctor     # one profile
```

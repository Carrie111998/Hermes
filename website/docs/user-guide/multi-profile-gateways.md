...
## Isolation and public ingress

Hermes deliberately runs **one gateway process per profile**. The profile's
home, credentials, sessions, durable events, and background workers are frozen
at process startup. This gives every profile its own security, crash, and
restart boundary.

Same-process multiplexing is not supported. If
`gateway.multiplex_profiles: true` remains in an old configuration, the gateway
fails closed before it starts platform adapters or HTTP listeners. Remove that
setting and start each profile's managed service instead.

The former `profile_routes` setting and `/p/<profile>/...` HTTP endpoints are
obsolete. Each profile process exposes only native endpoints such as
`/webhooks/<route>` or `/v1/chat/completions`.

If several profiles need one public address, use a thin external reverse proxy
or ingress to route a hostname, port, or operator-owned path to the appropriate
profile process. Strip any external routing prefix before forwarding to the
native endpoint. The ingress should select a process only; it must not combine
profile credentials or state.

Connection-oriented platforms such as Discord and Telegram need a separate bot
credential per profile process. Do not run the same platform credential in two
gateway processes at once.

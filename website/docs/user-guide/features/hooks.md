...
| `effective_file_search_context` | Directive/control | After a bounded backend path-resolution probe and immediately before each file-search existence check or search subprocess. `path` is the canonical effective backend path and cannot be rewritten by callbacks. Every registered callback must return an explicit `allow` or `block`, and any `block` wins; missing/invalid/error/timeout decisions fail closed. Python plugins only. | `path`, `cwd`, `backend`, `is_local` | Paths and cwd may reveal local project/user structure; no file contents are included. |
| `primary_auth_failure_context` | Observer | After a 401/403 is verified as the active primary `openai-codex` model request and before fallback selection. Return ignored. Python plugins only. | `provider`, `status_code`, `detail`, `failure_scope`, `turn_id`, `session_id`, `event_origin` | `detail` may contain raw provider data; no prompt/history is included. |
| `fallback_candidate_decision` | Directive/control | Before client construction or network dispatch for each fallback candidate. Every registered callback must return an explicit `allow` or `block`, and any `block` wins; missing/invalid/error/timeout decisions fail closed. Python plugins only. | `entry` (only present non-secret `provider`, `model`, `base_url`, `api_mode` fields; `base_url` is reduced to scheme/host/port), `reason`, `turn_id`, `session_id`, `event_origin` | Credential-bearing fallback fields are never supplied. |
| `client_safe_auth_failure` | Transform | When a primary `openai-codex` auth failure becomes terminal. First non-empty string or `message` wins; otherwise Hermes keeps the bounded safe default. Python plugins only. | `default_message`, `provider`, `status_code`, `turn_id`, `session_id`, `event_origin` | Content-free identifiers and safe default only. |

...
IYARI has three hook systems that run custom code at key lifecycle points:
...
Declare shell-script hooks in your `cli-config.yaml` and IYARI will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.

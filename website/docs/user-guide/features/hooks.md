...
| `reaction:added` | An emoji reaction was added to a message the bot can see (Slack + Discord adapters). **Slack:** requires the `reactions:read` scope + the `reaction_added` bot event subscription; the bot must be a member of the channel. **Discord:** fires for every human reaction (the bot's own emoji acks never fire it); no extra setup needed. Fan-out happens pre-authorization and pre-channel-policy — reactions from unauthorized users in ignored/non-allowed channels still fire the hook (by design, mirroring Slack) — so consumers needing authz-filtered events must filter themselves using context keys (`user_id` against their own user allowlist, `channel_id` against their channel policy). | **Slack:** `platform`, `reaction`, `user_id`, `item_user_id`, `item_type`, `channel_id`, `message_ts`, `team_id`, `event_ts`, `raw_event` · **Discord:** `platform`, `event_name`, `reaction`, `user_id`, `item_user_id` (author of the reacted-to message when known, else null), `channel_id`, `message_ts`, `event_ts`, `raw_event` |
| `reaction:removed` | An emoji reaction was removed from a message the bot can see. **Slack:** requires the `reaction_removed` bot event subscription. **Discord:** fires for every human reaction removal; no extra setup needed. Same pre-authorization/pre-channel-policy fan-out as `reaction:added` — filter on `user_id`/`channel_id` yourself if you need authz-filtered events. | same shape as `reaction:added` (per-platform keys differ as noted above) |

...
| `pre_user_input_route` | Directive/control | Authorized, idle plain user text after normalization and before CLI/Gateway slash dispatch; the first valid `{"action":"rewrite","text": nonempty str}` rewrites. `None`/`allow`, invalid returns, and failures preserve input. | `surface` (`"cli"` \| `"gateway"`), `text`, `session_key`, `platform`, `goal_active`, `has_attachments` | User text; never fires for slash, attachment, synthetic/internal, or active-goal input. Synthetic gateway events may still pass normal sender authorization. |

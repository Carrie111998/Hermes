...
| `gateway_platform_event` | Observer | After the gateway's profile-scoped authorization succeeds, when a supported platform-native event is normalized at the gateway boundary (Telegram: reactions, message edits, action-button taps; Discord: message edits/deletes, thread created/renamed); return ignored. | `platform`, `event_type`, `payload` (event-type-specific dict — see the per-event contracts below) | Normalized plain-dict envelope only; raw SDK objects, adapter handles, and bot clients are never exposed. |
...
| `action_button` | telegram | `chat_id: str`, `message_id: str \| None`, `thread_id: str \| None`, `user_id: str` (the tapping user, who is also the authorized source), `producer: str`, `label: str` (the button's text), `value: str \| None` (the producer's own payload, echoed back from the gateway-side registry — it never travels on the wire). Fired for buttons a producer declared in `metadata["buttons"]`; the built-in approval/clarify/model-picker keyboards own their own callback namespaces and never fire this. Buttons are single-use and expire after 10 minutes; the registry is in-memory, so a restart drops outstanding buttons and a later tap is answered "expired" without firing. A button is registered only once its send has actually succeeded, and only for the exact chat, topic, and message it was delivered on — a tap from anywhere else, or on a keyboard whose send failed (including a timeout that may still have delivered), is answered "expired" too. |

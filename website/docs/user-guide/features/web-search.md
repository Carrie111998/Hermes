...
| **You.com MCP** | `MCP_YOUDOTCOM_API_KEY` (optional) | ✔ (MCP) | ✔ with key (MCP) | 100 queries/day |
...
**Free mode** (no API key) provides search-only via the `?profile=free` endpoint. It includes 100 queries per day and excludes livecrawl, content extraction, research, and finance tools. Setting `MCP_YOUDOTCOM_API_KEY` switches to the full endpoint and unlocks content extraction and research tools.
...
`you-finance` is optional and not included in You.com's default full endpoint. To enable it, set `YDC_ALLOWED_TOOLS` alongside `MCP_YOUDOTCOM_API_KEY`, then reinstall:

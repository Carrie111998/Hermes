...
Orion Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
orion memory setup      # interactive picker + configuration
orion memory status     # check what's active
orion memory off        # disable external provider
```

You can also select the active memory provider via `orion plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.orion/config.yaml`:
...
When a memory provider is active, Orion automatically:
...
**Setup Wizard:**
```bash
orion memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `orion honcho setup` command still works (it now redirects to `orion memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$ORION_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ORION_HOME/honcho.json` > `~/.orion/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/zacharyjleach-stack/Aries/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/orion).
...
:::tip Migrating from `orion honcho`
If you previously used `orion honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Orion profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Orion profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Orion profile. Host key `orion` → default; `orion.<profile>` for others. |
...
```bash
orion profile create coder --clone
```

`--clone` creates a `orion.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Orion profile, creates host blocks for any profile without one, inherits settings from the default `orion` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `orion memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/zacharyjleach-stack/Aries/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/orion).
...
# Then configure Orion
orion memory setup    # select "openviking"
# Or manually:
orion config set memory.provider openviking
```

`orion memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.orion/.env`, and for named profiles use
`~/.orion/profiles/<profile>/.env`.
...
`OPENVIKING_AGENT` is Orion' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
orion memory setup    # select "mem0" → "Platform"
# Or manually:
orion config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.orion/.env
```

**Setup (OSS):**
```bash
orion memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
orion memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
orion memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
orion memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
orion memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.orion/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.orion/.env
```
...
**Config:** `$ORION_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.orion/.env`.
...
| `user_id` | `orion-user` | User identifier |
| `agent_id` | `orion` | Agent identifier |
...
**Switching modes:** Re-run `orion memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
orion memory setup    # select "hindsight"
# Or manually:
orion config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.orion/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p orion ui start`

**Config:** `$ORION_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `orion` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Orion Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/zacharyjleach-stack/Aries/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
orion memory setup    # select "holographic"
# Or manually:
orion config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.orion-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ORION_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
orion memory setup    # select "retaindb"
# Or manually:
orion config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.orion/.env
```
...
- Knowledge tree stored at `$ORION_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=orion), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
orion memory setup    # select "supermemory"
# Or manually:
orion config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.orion/.env
```
...
Before running `orion memory setup`, set `base_url` in
`$ORION_HOME/supermemory.json`:
...
Then run `orion memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$ORION_HOME/supermemory.json`
...
| `container_tag` | `orion` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `orion-{identity}` → `orion-coder`) to isolate memories per Orion profile
...
| **Requires** | `pip install orion-memori` + `orion-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install orion-memori
orion-memori install
orion config set memory.provider memori
orion memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `orion-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ORION_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ORION_HOME/` so each profile has its own credentials

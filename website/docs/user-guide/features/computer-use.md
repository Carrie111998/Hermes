...
Orion Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Orion integration. It
...
**Fresh installs already have the driver.** The Orion installer
...
- **`orion tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
orion computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `orion computer-use
status` to verify the install.

Already have cua-driver? Orion reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `orion update`, and the first
`computer_use` call of a session, Orion checks the local version and
...
selected with `ORION_CUA_DRIVER_CMD` stays
under your control, so Orion reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Orion autodetection is a
planned cua-driver follow-up, so currently point Orion at that directory or
...
interface. The built-in toolset provides Orion actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `orion computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Orion host identity. |
...
```
orion -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.orion/config.yaml`.

## Permission modes and logged-in browser profiles

Orion maps its existing approval UX onto cua-driver's immutable runtime
...
| Orion session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Orion approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Orion risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Orion then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.orion/cua-manifest.yaml
```
...
for the format). Orion launches a private runtime with
...
resetting or closing the Orion session, cancellation cleanup, or process exit
closes that transport session. Orion also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Orion
...
embedded service under the Orion host identity.
...
## `orion computer-use doctor` — your first triage stop

`orion computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ orion computer-use doctor
...
Orion run declares a public cua-driver **session name** (something like
`orion-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
Orion keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Orion-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Orion
autodetection is a planned cua-driver follow-up, so currently point Orion at
...
These are **platform deep dives, not duplicates of the Orion skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Orion
...
image capture also saves a bounded, shareable copy under Orion' image cache
...
Orion applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.orion/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Orion applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Orion agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Orion agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Orion warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Orion from inside the RDP / console
...
```
ORION_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ORION_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Orion disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Orion sets
...
When it's on, `orion computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Orion at a binary you built from source instead
of the published release. Orion resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ORION_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Orion testing (see notes).
...
### Option B — point Orion straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ORION_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ORION_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ORION_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Orion is using your build

- `orion computer-use status` prints the resolved binary path and
  version.
- `orion computer-use doctor` confirms the binary is reachable and
...
- **Orion spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Orion YOLO, Orion instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Orion
  change needed either way.
- **Local builds skip the version check.** Orion warns when the
...
**First action when anything's off: run `orion computer-use doctor`.**
...
Run `orion computer-use install` to fetch the cua-driver binary, or
run `orion tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `orion computer-use doctor` will
...
- **Orion-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Orion `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Orion autodetection is a
  planned follow-up; currently point Orion at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Orion among them)

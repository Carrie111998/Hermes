...
chat completions — use your Orion-managed provider subscription as their
...
| Use case | "Use Orion as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Orion stores
the refresh token in `~/.orion/auth.json` — the same place all Orion
...
```
Starting Orion proxy for Nous Portal
...
Model:      Orion-4-70B    # or Orion-4.3-36B, Orion-4-405B
```
...
interface in `orion_cli/proxy/adapters/`.
...
```
Orion proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `orion portal`. If you see
...
INFERENCE_TEXT_MODEL=Orion-4-70B
```
...
```bash
orion proxy start --host 0.0.0.0 --port 8645
```
...
`orion_cli/proxy/adapters/<provider>.py` and registering it in

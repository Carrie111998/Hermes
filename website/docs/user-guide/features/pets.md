...
Orion can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ORION_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Orion renders the sprite at full fidelity when your
...
orion pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `orion pets list [query] [--limit N]` |
| List installed pets | `orion pets list --installed` |
| Install a pet | `orion pets install <slug> [--select] [--force]` |
...
| Preview/animate | `orion pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `orion pets off` |
| Remove an installed pet | `orion pets remove <slug>` |
| Diagnose setup | `orion pets doctor` |

`orion pets show` flags:
...
Beyond installing pre-made pets from the gallery, Orion can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `orion pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `orion tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ORION_PET_IMAGE_PROVIDER` env var (e.g. `ORION_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Orion is
...
Run `orion pets doctor` — it reports:
...
- The [`orion-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-orion-agent.md)

...
This is the same architecture top-tier coding agents use. Orion
...
1. Orion captures a baseline of current diagnostics for the file.
...
…). Orion auto-detects the binary on PATH or in
`<ORION_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Orion at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ORION_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`orion lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Orion installs both packages together when you
run `orion lsp install typescript` or auto-install fires on first
use.

## CLI

```
orion lsp status          # service state + per-server install status
orion lsp list            # registry, optionally --installed-only
orion lsp install <id>    # eagerly install one server
orion lsp install-all     # try every server with a known recipe
orion lsp restart         # tear down running clients
orion lsp which <id>      # print resolved binary path
```

`orion lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ORION_HOME>/lsp/bin
...
When `install_strategy: auto`, Orion installs binaries into
`<ORION_HOME>/lsp/bin/`. NPM packages land in
`<ORION_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Orion-owned and is
...
**`orion lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ORION_HOME>/lsp/bin/`. Run
`orion lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `orion lsp status`**
...
When `orion lsp status` shows a `Backend warnings` section, install
...
`~/.orion/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.orion/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `orion lsp restart` to clear the set;

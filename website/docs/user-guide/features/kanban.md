...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree` — a git worktree under `.worktrees/<id>/` for coding tasks. Use `worktree:<path>` to pin the exact target path. The dispatcher materializes it with `git worktree add`, using `--branch` when provided. **Preserved on completion.**
- **Protected workspace** — an optional, board-scoped exact-owner lease for a sensitive workspace root. The policy records one task's exact id/title/workspace-kind tuple. Policy lookup and live-root conflict detection use a stable physical identity for existing local roots (Windows junctions/symlinks resolve to the same key) while keeping case/slash/trailing-separator folding and a lexical representation for UNC/nonlocal Windows paths. Both ready and review claims check the lease inside the same SQLite transaction as the status transition, before a run row exists. An unauthorized `worktree` request that names a protected git root is rewritten to `<root>/.worktrees/<task-id>` only after that target is proven to be a linked worktree of the intended common dir and branch (or freshly created as one). If the canonical target is occupied by an ordinary directory or a linked worktree on the wrong branch, claim is refused before a run row and spawn does not run. Other unauthorized tasks at the exact root are blocked before claim. Ordinary shared `dir:` paths keep their existing behavior unless a policy is explicitly set.
...
### Protecting a primary workspace root

Use a protected-workspace policy when one long-running integration or release
task must be the only direct owner of a primary checkout. The owner and any
additional allowlisted task must already point at the exact root; Hermes stores
their current id/title/workspace-kind tuple so later identity drift fails closed.

```bash
# Reserve the exact root for one task on the selected board.
hermes kanban protect-workspace set /absolute/path/to/repo --owner t_abcd1234

# Add one explicit exception (also captured as an exact tuple).
hermes kanban protect-workspace allow /absolute/path/to/repo --task t_efgh5678

# Inspect or remove policies. Use --board <slug> to target another board.
hermes kanban protect-workspace list
hermes kanban protect-workspace list --json
hermes kanban protect-workspace remove /absolute/path/to/repo
```

The guard compares normalized exact roots (including Windows drive-letter
case, slash direction, and trailing separators). A conflict emits a durable
`protected_workspace_conflict` event with `severity=critical`; Hermes never
kills the older owner or resets/checks out its repository automatically.

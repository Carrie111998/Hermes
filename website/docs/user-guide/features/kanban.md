...
- The CLI generates a `scope_digest` over worker-relevant task fields, comments, parent results, and attachment bytes. Editing that context after the grant invalidates it; the environment carries only the expected `approval_id`, never the mutable envelope.

...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree` — a git worktree under `.worktrees/<id>/` for coding tasks. Use `worktree:<path>` to pin the exact target path and `--branch` to choose the task branch. For snapshot-sensitive automation, pass `--base-sha <full-40-character-SHA>`: a new branch is created directly from that commit, while an existing branch/worktree is reused only when it descends from the pinned commit. Without `--base-sha`, the legacy anchor-`HEAD` behavior is preserved. **Preserved on completion.**

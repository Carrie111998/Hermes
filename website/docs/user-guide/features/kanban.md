...
- **Protected workspace** — an optional, board-scoped exact-owner lease for a sensitive workspace root. The policy records one task's exact id/title/workspace-kind tuple. Both ready and review claims check it inside the same SQLite transaction as the status transition, before a run row exists. An unauthorized `worktree` request that names a protected git root is rewritten to `<root>/.worktrees/<task-id>` before claim; other unauthorized tasks at the exact root are blocked before claim. Ordinary shared `dir:` paths keep their existing behavior unless a policy is explicitly set.

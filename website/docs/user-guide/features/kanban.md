...
| `provider_terminal` | `{provider, model, status_code, classification, fingerprint, config_fingerprint, session_id?, log_path}` | Worker reported a deterministic provider entitlement/configuration failure (for example HTTP 401/402/403 or an unavailable model). The dispatcher blocks the unfinished task on the first observation; unchanged provider/model configuration cannot be unblocked into a retry loop. Raw provider bodies, credentials, balances, tokens, and prompt text are never stored. |
| `provider_transient` | `{provider, model, status_code, classification, fingerprint, config_fingerprint, session_id?, log_path}` | Worker reported a transient provider failure (for example 429, timeout, or 5xx). The dispatcher requeues it through the normal bounded failure budget; it is not a protocol violation. |
| `provider_safety_refusal` | `{provider, model, status_code, classification, fingerprint, session_id?, log_path}` | Provider safety policy refused the request. This is a separate sticky block, not entitlement recovery; provider/model override changes do not automatically route around it. |

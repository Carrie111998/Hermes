...
| `dependency_wait` | `{reason, kind}` | Worker blocked with `kind=dependency`. The task stays `blocked` and is never auto-promoted; an explicit unblock is required after the dependency is resolved. |

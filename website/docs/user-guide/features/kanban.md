...
| `kanban_block` | Stop work and route by why: `kind=dependency` remains blocked until explicit unblock; `needs_input`/`capability`/`transient` surface to a human. Repeated same-kind re-blocks auto-escalate to `triage`. | `reason` |
...
| `dependency_wait` | `{reason, kind}` | Worker blocked with `kind=dependency`. The task stays `blocked` and is never auto-promoted or manually `promote`d; an explicit unblock is required after the dependency is resolved. Gateway and TUI subscribers receive the reason as an actionable dependency-wait notification. |

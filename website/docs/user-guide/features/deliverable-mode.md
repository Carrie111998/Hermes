...
When Orion Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.orion/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.orion/config.yaml` (switchable per session
via `/personality`).
...
If you use Orion' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.orion/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Orion Agent's deliverable

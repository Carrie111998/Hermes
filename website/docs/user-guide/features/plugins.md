...
| Register a memory backend | Install a standalone provider under `~/.hermes/plugins/<name>/` and select it with `hermes memory setup` — see [Memory Provider Plugins](/developer-guide/memory-provider-plugin) (uses a separate discovery system) |
...
| `plugins/memory/<name>/` (bundled) or `~/.hermes/plugins/<name>/` (standalone) | Memory providers (subclass `MemoryProvider`) | **Own loader** in `plugins/memory/__init__.py` (kind: `exclusive` — one active at a time) |
...
User model-provider plugins at `~/.hermes/plugins/model-providers/<name>/` and
standalone memory providers at `~/.hermes/plugins/<name>/` are discovered by
their respective loaders. A user memory provider with the same name as a
bundled provider does not replace the bundled provider; bundled providers keep
precedence. Select one provider with `memory.provider` or `hermes memory setup`.
...
| **Memory providers** (`plugins/memory/` or `~/.hermes/plugins/`) | All discovered; exactly one is active, chosen by `memory.provider` in `config.yaml`. |
...
| **Memory providers** | Replace or augment built-in memory | Single-select (one active) | `plugins/memory/` (bundled) or `~/.hermes/plugins/` (standalone) |
...
| A **memory backend** (Honcho, Mem0, Supermemory, …) | Memory plugin — subclass `MemoryProvider` in a standalone `~/.hermes/plugins/<name>/` repository | [Memory Provider Plugins](/developer-guide/memory-provider-plugin) |

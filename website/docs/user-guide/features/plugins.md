...
| Register a long-lived realtime voice provider seam | `ctx.register_realtime_voice_provider(provider)` — typed provider registration; the plugin owns the transport and audio lifecycle. This does not install a voice feature. |
...
| A long-lived **realtime voice provider** | Python provider plugin — `ctx.register_realtime_voice_provider(provider)` exposes a typed integration seam; the plugin owns transport and audio teardown | [Register a realtime voice provider](/developer-guide/plugins#register-a-realtime-voice-provider) |

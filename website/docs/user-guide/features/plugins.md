...
title: "Plugins"
description: "Extend Orion with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Orion has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Orion
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Orion Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.orion/plugins/` with a `plugin.yaml` and Python code:

```
~/.orion/plugins/my-plugin/
...
Start Orion — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.orion/plugins/hello-world/plugin.yaml`**
...
**`~/.orion/plugins/hello-world/__init__.py`**

```python
"""Minimal Orion plugin — registers a tool and a hook."""
...
Drop both files into `~/.orion/plugins/hello-world/`, restart Orion, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Orion does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.orion/plugins/` are disabled by default. Enable them only for trusted repositories by setting `ORION_ENABLE_PROJECT_PLUGINS=true` before starting Orion.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `orion <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `orion plugins install` |
| Distribute via pip | `[project.entry-points."orion_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Orion — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.orion/plugins/` | Personal plugins |
| Project | `.orion/plugins/` | Project-specific plugins (requires `ORION_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `orion_agent.plugins` entry_points | Distributed packages |
| Nix | `services.orion-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Orion also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.orion/plugins/model-providers/<name>/` and `~/.orion/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `orion plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.orion/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
orion plugins                    # interactive toggle (space to check/uncheck)
orion plugins enable <name>      # add to allow-list
orion plugins disable <name>     # remove from allow-list + add to disabled
```

After `orion plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
orion plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Orion checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `orion plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`orion plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of Orion' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.orion/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.orion/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Orion tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Orion runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Orion never
materializes the prompt on another surface.

Orion still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Orion that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.orion/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `orion_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Orion has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.orion/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Orion also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Orion Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Orion Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Orion Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `orion <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `ORION_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Orion auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `orion skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.orion/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `orion plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
orion plugins                               # unified interactive UI
orion plugins list                          # table: enabled / disabled / not enabled
orion plugins search <term>                 # search the community plugin index
orion plugins install <name>                # install by index name (resolved to repo @ pinned ref)
orion plugins install user/repo             # install from Git, then prompt Enable? [y/N]
orion plugins install user/repo --enable    # install AND enable (no prompt)
orion plugins install user/repo --no-enable # install but leave disabled (no prompt)
orion plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
orion plugins remove my-plugin              # uninstall
orion plugins enable my-plugin              # add to allow-list
orion plugins disable my-plugin             # remove from allow-list + add to disabled
orion plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Orion Desktop registers the `orion://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
orion://plugin/install?repo=owner/repo            # main install link
orion://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
orion://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Orion and shows a **confirmation dialog** — the repo id,
...
Install from Git**. Legacy `orion://plugin-agent/…` and
`orion://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `orion-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="orion://plugin/install?repo=owner/repo&enable=1">Install in Orion</a>
```

MCP servers have the equivalent link form — see
[Add to Orion link](/reference/mcp-config-reference#add-to-orion-link).
...
When a plugin declares capabilities, `orion plugins install` (and
`orion plugins enable`) shows the list with one-line risk descriptions and
...
granted, `orion plugins update` surfaces the additions and asks again. New
...
`orion plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
orion plugins capabilities             # all plugins with declared/granted capabilities
orion plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Orion has not reviewed the plugin's code. Only install
...
`orion plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
orion plugins search telegram               # fuzzy search
orion plugins search                        # browse the whole index
orion plugins search --capability platform  # filter by declared capability
orion plugins search media --json           # machine-readable output
orion plugins search --refresh              # bypass the 24h local cache
```
...
```bash
orion plugins install orion-media-studio
```
...
(`https://raw.githubusercontent.com/Orion/orion-plugin-index/main/index.json`,
overridable via `orion config set plugins.index_url <url>`). Fetches are
cached under `~/.orion/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Orion — so search works fully offline.

**Index entry format.** Each entry is a JSON object:

```json
{
  "name": "orion-media-studio",
  "description": "Generative media workspace plugin.",
  "author": "Orion",
  "tags": ["media", "image-gen"],
  "repo": "Orion/orion-media-studio",
...
(`orion_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[orion-plugin-index](https://github.com/zacharyjleach-stack/Aries)
...
A **plugin pack** is a declarative, shareable YAML file (`orion-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: orion-media-studio            # bare community-index name…
...
```bash
orion plugins pack show ./orion-pack.yaml     # dry-run review
orion plugins pack install ./orion-pack.yaml  # review → confirm → install
orion plugins pack export > orion-pack.yaml   # snapshot the current install
orion plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `orion plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `orion plugins install`.
...
provenance (installed via `orion plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`orion skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `orion plugins install` and `orion plugins update` runs a static
...
On `orion plugins update`, a dangerous verdict on the updated tree
...
Running `orion plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `orion plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Orion reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Orion rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Orion can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Orion drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Orion checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Orion' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).

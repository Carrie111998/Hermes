...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Orion's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Orion instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
orion memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `orion` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Orion syncs them back at session init.
...
The `orion honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `orion memory setup honcho` (or run `orion memory setup` and pick it from the list); the `orion honcho` subcommand then appears on the next invocation.

```bash
orion memory setup honcho    # Configure Honcho directly (works before activation)
orion honcho status          # Connection status, config, and key settings
orion honcho setup           # Redirects to `orion memory setup` (post-activation alias)
orion honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
orion honcho peer            # Show or update peer names + dialectic reasoning level
orion honcho mode            # Show or set recall mode (hybrid/context/tools)
orion honcho tokens          # Show or set token budget for context and dialectic
orion honcho identity        # Seed or show the AI peer's Honcho identity
orion honcho sync            # Sync Honcho config to all existing profiles
orion honcho peers           # Show peer identities across all profiles
orion honcho sessions        # List known Honcho session mappings
orion honcho map             # Map current directory to a Honcho session name
orion honcho enable          # Enable Honcho for the active profile
orion honcho disable         # Disable Honcho for the active profile
orion honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `orion honcho`

If you previously used the standalone `orion honcho setup`:
...
No re-login or re-setup needed. Run `orion memory setup` and select "honcho" — the wizard detects your existing config.

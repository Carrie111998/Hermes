...
| `API_UPLOAD_FILES_URL` | _(none)_ | Remote file upload endpoint (e.g. `https://api.example.com/v1/files`). When set, files generated by agent skills are uploaded to this URL. |
| `API_UPLOAD_FILES_KEY` | _(none)_ | Bearer token for the `API_UPLOAD_FILES_URL` endpoint. |
| `API_UPLOAD_FILES_DOWNLOAD_URL` | _(none)_ | Download URL template with `{file_id}` placeholder (e.g. `https://api.example.com/v1/files/{file_id}/content`). When set, file links in responses point to this URL and annotations use the `url_citation` type. |
...
Agent skills (powerpoint, chart generation, PDF export, etc.) can generate files that need to be delivered to the API client. When `API_UPLOAD_FILES_URL` is configured, the API server automatically:

1. **Detects files** — scans the agent's response for `MEDIA:/absolute/path/to/file` tags and bare local file paths
2. **Uploads** — POSTs each file as `multipart/form-data` to `API_UPLOAD_FILES_URL`
3. **Replaces references** — replaces the `MEDIA:` tag in the response text with a `[filename](url)` markdown link at the original position
4. **Adds annotations** — includes structured metadata in the Responses API output via `output_text.annotations`

### Example

Agent response before processing:
```
Report generated.
MEDIA:/tmp/report.pdf
```

After processing (with download URL configured):
```json
{
  "type": "output_text",
...
When `API_UPLOAD_FILES_DOWNLOAD_URL` is not configured, annotations use the `file_citation` type instead:
```json
{
  "annotations": [
    {
...
### How the agent knows to use MEDIA:

The agent's system prompt instructs it to use `MEDIA:/absolute/path/to/file` syntax for file delivery. The prompt explicitly tells the agent not to wrap the path in backticks — write it as plain text so the extraction layer can detect it:

> When delivering files, include MEDIA:/absolute/path/to/file in your response — the file will be uploaded and made available for download. Important: do NOT wrap the MEDIA: tag or the file path in backticks (`) or code fences.

### Error resilience

If a file upload fails (network error, file not found, server rejects), the failure is logged and the response is delivered without that file. The MEDIA: tag is still stripped from the text so the user never sees raw markup. Other successfully uploaded files are unaffected.
...
- **Inline images only for input** — inline images are supported on both `/v1/chat/completions` and `/v1/responses`, but uploaded file *inputs* (`file`, `input_file`, `file_id`) and non-image document inputs are not supported through the API. See [File Delivery](#file-delivery) above for how agent-generated files are delivered to clients.

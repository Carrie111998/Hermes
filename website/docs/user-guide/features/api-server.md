...
## Multiplex Profile Isolation

When `gateway.multiplex_profiles` is configured, each profile gets an isolated execution context (separate sessions, separate secret scope, separate routing). However, the `GET /v1/runs/{id}` API endpoint does NOT enforce profile isolation — any authenticated caller can query any profile's run status. This is worth knowing when designing multi-tenant setups. See #90415.
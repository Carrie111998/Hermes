...
Sessions are stateless signed tokens; the directory is consulted at **login** and — in search-then-bind mode — at each **token refresh**, where a deleted or moved account is rejected. That bounds a disabled account's residual access to `session_ttl_seconds` (12h default). Direct-bind mode has no service credentials to check with, so there a disabled account keeps its session until the refresh token expires — prefer search-then-bind (or a short `refresh_ttl_seconds`) if that matters to you.
:::

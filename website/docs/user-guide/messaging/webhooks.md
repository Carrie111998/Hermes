...
| `session_key` | No | Template for a stable session key (for example, `"{id}"`). A rendered value matching `[A-Za-z0-9._-]{1,128}` keeps all matching events in the same Hermes conversation; omit it for the default independent session per delivery. |

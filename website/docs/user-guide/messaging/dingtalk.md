...
:::note upstream branding disclosure
Because DingTalk's `verification_uri_complete` is hardcoded to the upstream identity at the API layer, the QR currently authorizes under an `upstream` source string until Alibaba / DingTalk-Real-AI registers a Hermes-specific template server-side. This is purely how DingTalk presents the consent screen — the bot you create is fully yours and private to your tenant.
:::

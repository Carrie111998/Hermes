...
sidebar_position: 30
title: "Nastech Relay"
description: "Connect Nastech to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Nastech Relay (Connector)
...
Nastech Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nastech Portal access token from your existing login
   (`~/.nastech/auth.json`) — this proves which Nastech org (tenant) you own. If
   ...
   path, no Nastech Portal involved).
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.nastech/.env`:
...
`nastech gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.nastech/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates via generic OAuth2 client-credentials against your own IdP instead of Nastech Portal. |
...
token. Re-login with `nastech auth add nastech` (or `nastech setup`) and retry.
...
startup. Restart it (`nastech gateway restart`).

...
sidebar_position: 30
title: "Orion Relay"
description: "Connect Orion to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Orion Relay (Connector)
...
Orion Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.orion/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.orion/.env`:
...
`orion gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.orion/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `orion auth add nous` (or `orion setup`) and retry.
...
startup. Restart it (`orion gateway restart`).

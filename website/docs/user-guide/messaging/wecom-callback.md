...
Connect Orion to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Orion supports two WeCom integration modes:
...
> Run `orion gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Orion decrypts the message, queues it for the agent
...
(Use `orion gateway start` only after `orion gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Orion and the token the
...
in `~/.orion/.env` values around `=` will also break signature checks. After
fixing, restart `orion gateway run`.
...
Check `orion gateway run` logs for the bound host/port. If the adapter bound to

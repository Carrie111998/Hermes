...
| `DISCORD_DISABLE_DMS` | No | `false` | When `true`, ignore Discord DMs entirely (no `dm_messages` intent, admission drop, slash commands in DMs rejected). Use this for community/support bots that should only answer in public channels. Also settable as `discord.disable_dms` in `config.yaml`. |

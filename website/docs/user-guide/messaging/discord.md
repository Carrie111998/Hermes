...
| `HERMES_DISCORD_COMMAND_SYNC_TIMEOUT` | No | `600` | Wall-clock budget (seconds) for the `"safe"` sync reconciliation of one connect. Large command sets reconcile one HTTP mutation at a time and can exceed the default on rate-limited apps; raise it so a slow sync completes instead of timing out and re-running on every reconnect. |

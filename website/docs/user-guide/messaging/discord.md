...
## Interaction-scoped file uploads

Use `/ask prompt:<text>` when a conversational request may generate a file. `/ask`
runs the same Hermes agent pipeline and uses the same Discord session identity and
authorization rules as an ordinary message, but it also keeps the originating
Discord interaction available for that request's file delivery.

For files generated by `/ask`, Hermes reads the `attachment_size_limit` supplied
by Discord for that interaction and sends eligible files through the interaction
follow-up webhook. Discord calculates this per-interaction value from the upload
limits that apply to the invoking user and server, so it may reflect Discord Nitro
or the server boost tier. Hermes does not assume or promise a fixed upload size.
Files larger than the reported limit are rejected before upload with the actual
file size and effective limit in the error.

Ordinary messages, mentions, cron jobs, `hermes send`, expired interactions, and
other deliveries without request-scoped interaction context continue to use the
normal Discord bot/channel upload path. An interaction token is short-lived, so a
long-running `/ask` request may fall back to that normal path after the interaction
expires. Hermes uses only the bot's application interaction; it never needs or
uses a Discord user account token.

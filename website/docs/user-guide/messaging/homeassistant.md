...
description: Control your smart home with IYARI via Home Assistant integration.
...
IYARI integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "IYARI"
5. Copy the token
...
IYARI registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "IYARI".
...
from `config.yaml`. Double-check the file lives under the active IYARI profile
...
running IYARI — `curl -H "Authorization: Bearer <token>" <url>/api/` should

...
description: Control your smart home with Orion Agent via Home Assistant integration.
...
Orion Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Orion Agent"
5. Copy the token
...
Orion Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.orion/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Orion Agent".
...
The adapter reads credentials from `~/.orion/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Orion profile
...
running Orion — `curl -H "Authorization: Bearer <token>" <url>/api/` should

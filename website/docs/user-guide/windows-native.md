...
| **Node.js 24** | Required for the browser tool (`agent-browser`), the TUI's web bridge, and the WhatsApp bridge. |
...
3. **Installs Node.js 24** (winget if available, else a portable Node tarball unpacked under `%LOCALAPPDATA%\hermes\node`). Used for the browser tool and the WhatsApp bridge.

...
- **Curate the list with Edit Models.** The gear row at the bottom of the picker opens **Models**, where each model has a switch that shows/hides it in the dropdown (a per-device preference), and each *provider* has a switch that turns the provider off entirely. Switching a provider off writes [`model_catalog.excluded_providers`](../reference/model-catalog.md#hiding-providers-from-the-picker), so it disappears from every picker — desktop, TUI, and `hermes model` — even if it keeps authenticating itself from credentials left on the machine by other tools (a logged-in `gh` for Copilot, Claude Code's OAuth file). The row stays in the dialog with its switch off, so you can turn it back on later. The provider serving your current model is the one exception — it keeps its place in the picker until you switch models, because Hermes always shows the selection you saved.

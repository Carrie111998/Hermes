...
- **Keep computer awake** — **Settings → Advanced → Keep computer awake** holds an idle-sleep assertion (`PreventUserIdleSystemSleep`) so long or overnight agent runs keep going while the lid is open (the display can still dim). It does **not** override macOS clamshell sleep: a MacBook with no external display still sleeps when you close the lid, even on AC power. The Discord gateway then treats the pre-sleep socket as stale and reconnects on wake. This is a per-computer setting.

...
Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.hermes/.env` so the credentials are in the environment at boot. A ready-to-edit template unit file is included at [`systemd/hermes-serve.service`](https://github.com/NousResearch/hermes-agent/blob/main/systemd/hermes-serve.service) in the repo — copy it, set the bind address, and install it exactly like a `hermes-gateway.service` unit.

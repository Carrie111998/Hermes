...
- **Keyboard navigation.** Press `Cmd/Ctrl+Shift+M` to open the picker, then use `ArrowUp`/`ArrowDown` to highlight a model. Press `ArrowRight` to open that model's options, use `ArrowUp`/`ArrowDown` to highlight an effort level, and press `Enter` to apply it. Press `ArrowLeft` to return to the model list.

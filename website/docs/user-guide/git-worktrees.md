...
title: "Git Worktrees"
description: "Run multiple Orion agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Orion Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Orion so each session has a clean, isolated working directory.

## Why Use Worktrees with Orion?

Orion treats the **current working directory** as the project root:

- CLI: the directory where you run `orion` or `orion chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.orion/config.yaml`
...
Orion creates `.worktrees/my-experiment/` inside the repo (branch
`orion/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `orion-<id>`
...
git worktree add ../repo-feature feature/orion-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/orion-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Orion there:

```bash
cd ../repo-feature

# Start Orion in the worktree
orion
```
...
git worktree add ../repo-experiment-a feature/orion-a
git worktree add ../repo-experiment-b feature/orion-b
```
...
- Works on its own branch (`feature/orion-a` vs `feature/orion-b`).
...
- Orion checkpoint data under `~/.orion/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Orion experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/orion-checkpoints-docs`, `feature/orion-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Orion from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `orion -w` (Automatic Worktree Mode)

Orion has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `orion/orion-<hash>`).
...
```bash
orion -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `orion -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Orion session its own clean checkout.

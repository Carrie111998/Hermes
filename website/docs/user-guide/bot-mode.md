...
- **Direct messages** — every routed Bot Mode session carries the `message_agent` tool: a Bot messages a teammate by calling `message_agent(target="researcher", message="…")`. The tool validates the target against the live roster, prefixes the sender's `Message from 🤖 <sender> (@<sender>):` attribution automatically, and delivers into the teammate's canonical Bot Chat. Delivery is **fire-and-forget**: the sender gets an acknowledgement, finishes its turn, and the reply arrives later as a background completion notification. The message travels as a real parameter (nothing shell-interpreted — quotes, `$(...)`, and backticks arrive verbatim), and the Bot composes its own message rather than forwarding your words. The teammate roster — names **and roles** from each profile's title/description — is part of the system prompt, so Bots know who does what before choosing a recipient. The tool exists **only** in a participating install's real roster profiles, inside their canonical Bot Chat or classified human messaging chats. It is not exposed to CLI scratch sessions, cron/subagent sessions, webhooks, automation event streams, or agent/API task adapters.

The backend teaches each routed Bot Mode session the messaging protocol automatically at prompt-build time — including when a teammate opens it headlessly from the CLI. On a Bot Mode install, every profile shown by `profiles.list` is a roster member whether or not it has custom avatar/title metadata. The protocol appears only in that profile's canonical Bot Chat and classified human messaging chats (Discord, Telegram, Slack, and similar); CLI scratch sessions, webhooks, automation event sources, and agent/API task adapters such as A2A stay gated. Your other sessions and your SOUL.md remain untouched. This is controlled by `agent.bot_mode_protocol` in `config.yaml` (default: on):

```yaml
agent:
  bot_mode_protocol: true   # inject protocol into roster Bot Chats + classified human messaging chats
```

...
- **Sessions** (from a Bot's context menu) browses and filters that profile's 200 most recent stored conversations, without changing the primary click-to-chat flow. Choose **Make default** beside any conversation to make it the one that opens whenever you return to that Bot; the current choice is labelled **Default**.

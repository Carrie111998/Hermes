...
`permissions.deny` is the broader deny-policy namespace. `permissions.deny.commands` is an alias for command-deny rules, and `permissions.deny.paths` blocks matching paths from file tools (`read_file`, `write_file`, `patch`, and `search_files`) and implicit project-context readers before content is read or written. Recursive searches are rejected before backend enumeration when their root is denied or may overlap a denied descendant:

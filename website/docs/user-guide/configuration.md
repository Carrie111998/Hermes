...
delegation:
  ...
  # fallback_providers:                     # Independent fallback chain for subagents (empty = inherit the parent's top-level fallback_providers chain)
  #   - provider: custom                     # Same entry shape as the top-level fallback_providers list
  #     model: my-local-model                # (provider + model required; base_url/api_key optional)
...
**Subagent fallback chain:** By default, subagents inherit the parent's top-level `fallback_providers` chain. Set `delegation.fallback_providers` to give children an independent chain — e.g. route subagents to a cheap/fast cloud model that fails over to a local model, without changing the main agent's own fallback. Entries use the same shape as the top-level list (`provider` + `model`, optional `base_url`/`api_key`); malformed entries are ignored, and an empty list keeps the inheritance behavior.

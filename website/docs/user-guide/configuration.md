...
compression:
  ...
  proactive_prune_ratio: 0                          # Opt-in window-fraction trigger for the same prune (0 = off; see below)
...
`proactive_prune_ratio` is the window-fraction alternative trigger for the same prune (default `0` = off). Instead of an absolute token count, the prune fires when the prompt exceeds `ratio × effective input budget` (`context_length − max_tokens`) — so one setting works on any window size without guessing a token number: `0.4` triggers at ~400K tokens on a 1M model and ~48K on a 128K model. The budget is resolved live, so the trigger re-anchors automatically across `/model` switches and fallback activations. When both triggers are set, the **lower** one wins — combine a ratio for window-proportional behavior with an absolute number as a cap (or vice versa). Values above `1` clamp to `1.0`; values ≤ `0` disable. The same `proactive_prune_min_reclaim_tokens` reclaim gate and trigger-sized rearm runway apply, so prompt-cache breaks stay episodic either way.

...
compression:
  ...
  adaptive_context: false                           # Opt-in protected-working-set governor — see below
  adaptive_base_tokens: 231200                      # Efficient starting trigger
  adaptive_headroom_ratio: 0.20                     # Space above protected active material
  adaptive_step_tokens: 65536                       # Stable growth increment
  adaptive_hard_ratio: 0.85                         # Never exceed this fraction of effective input window
  ...
  idle_compact_min_tokens: 0                        # Optional absolute idle size floor (e.g. 128000)
...
`threshold_tokens` sets an optional **absolute token cap** for the compression trigger. When set, compression fires no later than this count regardless of model. In static mode it is the lower of the ratio-derived threshold and this cap; in adaptive mode it is also the governor's final hard ceiling. The cap survives model switches and fallback activations.

`adaptive_context` enables the opt-in protected-working-set governor. Instead of treating a fixed token count as both an efficiency target and a permanent ceiling, Hermes starts at `adaptive_base_tokens` and estimates the material it cannot safely compact: system/tool-schema overhead, the rolling handoff summary, the configured protected head/tail, and the complete active user/tool group. If that working set needs more room, the trigger grows with `adaptive_headroom_ratio`, rounded up in `adaptive_step_tokens` increments. Growth is hysteretic during the live context (it does not oscillate down on every small turn) and resets after compaction, a qualifying idle resume, or a model/route switch. The final trigger can never exceed `adaptive_hard_ratio` of the effective input window (`context_length - max_tokens`) or `threshold_tokens` when an absolute cap is set. Existing behavior is unchanged while `adaptive_context: false`.

`idle_compact_after_seconds` is an **opt-in, time-based** trigger that complements the size trigger. Default `0` (disabled). Resume age is read from the durable SessionDB activity heartbeat, so gateway watchdog-clock resets do not erase the idle gap. A qualifying session compacts up front before its first reply and honors the normal cooldown, anti-thrash, and per-session lock guards. `idle_compact_min_tokens` adds an absolute size floor; the effective floor is the larger of that value and `threshold × target_ratio`. For example, use `idle_compact_after_seconds: 86400` with `idle_compact_min_tokens: 128000` to compact only large sessions returning after a day.

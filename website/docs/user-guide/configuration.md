...
delegation:
  ...
  #     provider:
  #       sort: throughput
  inherit_fallback_providers: true           # Inherit the parent's fallback chain. Set false to pin subagents to their primary provider/endpoint.
...
**Fallback inheritance:** Subagents inherit the parent's fallback chain by default. This is independent of credential-pool endpoint matching: a correctly pinned primary can still fail transiently and activate a cross-provider fallback. Set `delegation.inherit_fallback_providers: false` when delegated work must stay on the primary route and fail rather than switching providers.

...
delegation:
  ...
  prefix_stagger_seconds: 0                 # Stagger first-wave sibling launches by N seconds each so batch children hit the provider prompt cache instead of all missing it at once. 0 = simultaneous (default).
...
**Prompt-cache launch stagger:** `prefix_stagger_seconds` (default `0` = launch simultaneously) delays each first-wave sibling's start by N seconds: child 0 starts immediately, child 1 after N seconds, child 2 after 2N, and so on up to `max_concurrent_children`. Batch children share a near-identical prompt prefix (system prompt, tools, skills); launched simultaneously, every child's first API call misses the provider prompt cache and re-pays the full prefix. A stagger of a few seconds (e.g. `5`) lets the first child's request write the cache so siblings read it — a large saving on providers that price cache reads at ~10% of input. Children queued beyond the first wave start when a pool slot frees and get no artificial delay (the prefix is cached by then). The delay wakes early if the parent is interrupted. Inspired by Claude Code's workflow fan-out prefix stagger (v2.1.229).

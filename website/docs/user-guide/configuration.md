...
| Streaming call ceiling | 1800s (never below the stale stream timeout) | Exempt when left implicit | `providers.<id>.max_call_seconds` or `HERMES_STREAM_MAX_CALL_SECONDS` |
...
The **stale stream detection** kills connections that receive SSE keep-alive pings but no actual content. Only real progress refreshes its window — content, reasoning, tool-call deltas, a finish reason or usage. Anthropic `ping` events and empty OpenAI-style deltas are ignored, so a heartbeat alone can no longer keep a dead stream alive. For local providers (which don't send keep-alive pings during prefill) the default is raised to a finite 900-second ceiling instead of the 180s base — configurable via `agent.local_stream_stale_timeout` or the `HERMES_LOCAL_STREAM_STALE_TIMEOUT` env var.

The **streaming call ceiling** bounds one streaming request end to end. Every other guard measures *silence*, so a provider that keeps emitting a trickle of real tokens resets all of them and can hold a single call open for as long as it likes — the 20-minute call that delayed a cron run in issue #83657 was exactly that shape. This ceiling never resets on provider activity: when it expires the connection is aborted and the error is handed to the normal retry / provider-fallback chain. It defaults to `HERMES_API_TIMEOUT` and is floored at the stale stream timeout so it can never preempt the detector. Local endpoints are exempt unless you set `providers.<id>.max_call_seconds` or `HERMES_STREAM_MAX_CALL_SECONDS` explicitly; `0` disables it everywhere.

...
`protect_last_n` protects the recent tail by **message count, not size**: the last N messages stay verbatim no matter how many tokens they carry. On sessions dominated by bulky tool outputs (large file reads, terminal dumps, web extracts) that tail can dwarf the post-compression target (`threshold × target_ratio`). When the protected tail alone exceeds the target, compaction mathematically cannot reach it — every attempt shrinks only the small compressible middle, reports `insufficient progress`, and retries, burning minutes per pass while the session stays pinned above the trigger. Other harnesses avoid this failure mode by token-clamping the retained tail instead of counting messages: Codex CLI keeps at most ~20K tokens of recent user messages next to its compaction summary (`COMPACT_USER_MESSAGE_MAX_TOKENS`), and Claude Code's context editing clears old tool results down to a ~40K-token recent window with an aggregate cap on total tool-result payload. Hermes' equivalent lever is `tail_mode`: `legacy` keeps the ratio-sized verbatim tail (a pressure pass additionally demotes oversized completed tool/file bodies inside it when they exceed the tail budget — issue #61932), while `lean` clamps the verbatim tail to a hard 10K–25K token band and carries continuity through chunked digests, a mechanical anchor index, verbatim user messages, and `session_search` recovery pointers embedded in the summary. If your workload is tool-heavy (agents, CI logs, bulk reads) and compression logs show repeated `insufficient progress` passes, set `tail_mode: lean`; keep `legacy` for conversational sessions where verbatim recent turns are worth more than headroom.

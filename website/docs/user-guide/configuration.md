...
- **Needs-attention escalation.** A platform whose instability has lasted past `agent.reconnect_attention_after` (default `7200` seconds = 2 hours, `0` disables) gets `needs_attention: true` and a `retrying_since` timestamp in gateway runtime status (`hermes status`), plus a WARNING log. The clock measures the episode, not continuous residency in the retry queue: a reconnect that does not hold carries the original start time forward instead of resetting it (see the flap horizon below). Retries continue unchanged — this is a signal, not a circuit breaker. Clearing follows the same rule, because a flag that dropped on a bind the clock does not trust would go quiet exactly while the platform is flapping. With a positive `agent.reconnect_stable_after` (the default), an escalated platform is reported `connected` again the moment it binds, but keeps `needs_attention` and `retrying_since` until that reconnect has *held* for the horizon; fail again inside the window and it was never marked healthy in between. Set `agent.reconnect_stable_after: 0` and there is no window to hold for and no clock to carry, so the flag clears on the first successful reconnect — the behaviour before the flap horizon existed.

...
compression:
  ...
  rotate_hint_tokens: 0                             # Opt-in chat nudge at this absolute session size (0 = disabled)
...
`rotate_hint_tokens` is an opt-in **absolute session-size nudge** for human chat surfaces that do not show a context meter. When the estimated or last reported prompt size reaches this value, Hermes sends a short reminder that `/reset` starts a clean session while keeping durable memory and searchable history, and that `/compress` compacts the current session. The reminder is cooldown-limited and is never sent to API, webhook, or Home Assistant event sources. Default `0` disables it. Set it above the normal compaction trigger so it signals that a session remains unusually large after automated hygiene. For example, with a 500K context window and `threshold: 0.50`, a value around `300000` avoids duplicating the normal compaction warning.

...
常见自动化模式的复制粘贴蓝图。每个蓝图使用 Orion 内置的 [cron 调度器](/user-guide/features/cron) 实现基于时间的触发，使用 [webhook 平台](/user-guide/messaging/webhooks) 实现事件驱动触发。
...
| **GitHub 事件** | PR 开启、推送、issue、CI 结果时触发 | Webhook 平台（`orion webhook subscribe`） |
| **API 调用** | 外部服务向你的端点 POST JSON | Webhook 平台（config.yaml 路由或 `orion webhook subscribe`） |
...
orion cron create "0 2 * * *" \
  "You are a project manager triaging the Orion/orion-agent GitHub repo.

1. Run: gh issue list --repo Orion/orion-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
orion webhook subscribe github-pr-review \
  --events "pull_request" \
...
orion cron create "0 9 * * 1" \
  "Scan the Orion/orion-agent repo for documentation drift.

1. Run: gh pr list --repo Orion/orion-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (orion_cli/commands.py, orion_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (orion_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
orion cron create "0 6 * * *" \
  "Run a dependency security audit on the orion-agent project.

1. cd ~/.orion/orion-agent && source .venv/bin/activate
...
```bash
orion webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
orion webhook subscribe alert-triage \
...
```python title="~/.orion/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Orion-Monitor/1.0"})
...
orion cron create "every 30m" \
  ...
  --script ~/.orion/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
orion cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Orion Agent development." \
...
```bash
orion webhook subscribe github-issues \
  --events "issues" \
...
```bash
orion webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
orion webhook subscribe stripe-payments \
...
```bash
orion cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the orion-agent codebase.

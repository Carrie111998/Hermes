...
你需要至少一种方式连接到 LLM。使用 `nastech model` 交互式切换提供商和模型，或直接配置：

| 提供商 | 配置方式 |
|----------|-------|
| **Nastech Portal** | `nastech model`（OAuth，订阅制） |
...
| **Arcee AI** | `~/.nastech/.env` 中的 `ARCEEAI_API_KEY`（provider: `arcee`；别名：`arcee-ai`、`arceeai`） |
| **GMI Cloud** | `~/.nastech/.env` 中的 `GMI_API_KEY`（provider: `gmi`；别名：`gmi-cloud`、`gmicloud`） |
| **MiniMax** | `~/.nastech/.env` 中的 `MINIMAX_API_KEY`（provider: `minimax`） |
| **MiniMax 中国** | `~/.nastech/.env` 中的 `MINIMAX_CN_API_KEY`（provider: `minimax-cn`） |
| **xAI（Grok）— Responses API** | `~/.nastech/.env` 中的 `XAI_API_KEY`（provider: `xai`） |
| **xAI Grok OAuth（SuperGrok）** | `nastech model` → "xAI Grok OAuth (SuperGrok / Premium+)"——浏览器登录，无需 API key。参见[指南](../guides/xai-grok-oauth.md) |
| **Qwen Cloud（阿里 DashScope）** | `~/.nastech/.env` 中的 `DASHSCOPE_API_KEY`（provider: `alibaba`） |
| **阿里云（Coding Plan）** | `DASHSCOPE_API_KEY`（provider: `alibaba-coding-plan`，别名：`alibaba_coding`）——独立计费 SKU，不同端点 |
| **Kilo Code** | `~/.nastech/.env` 中的 `KILOCODE_API_KEY`（provider: `kilocode`） |
| **小米 MiMo** | `~/.nastech/.env` 中的 `XIAOMI_API_KEY`（provider: `xiaomi`，别名：`mimo`、`xiaomi-mimo`） |
| **腾讯 TokenHub** | `~/.nastech/.env` 中的 `TOKENHUB_API_KEY`（provider: `tencent-tokenhub`，别名：`tencent`、`tokenhub`、`tencentmaas`） |
| **OpenCode Zen** | `~/.nastech/.env` 中的 `OPENCODE_ZEN_API_KEY`（provider: `opencode-zen`） |
| **OpenCode Go** | `~/.nastech/.env` 中的 `OPENCODE_GO_API_KEY`（provider: `opencode-go`） |
| **DeepSeek** | `~/.nastech/.env` 中的 `DEEPSEEK_API_KEY`（provider: `deepseek`） |
| **Hugging Face** | `~/.nastech/.env` 中的 `HF_TOKEN`（provider: `huggingface`，别名：`hf`） |
| **Google / Gemini** | `~/.nastech/.env` 中的 `GOOGLE_API_KEY`（或 `GEMINI_API_KEY`）（provider: `gemini`） |
| **LM Studio** | `nastech model` → "LM Studio"（provider: `lmstudio`，可选 `LM_API_KEY`） |
| **自定义端点** | `nastech model` → 选择"Custom endpoint"（保存在 `config.yaml`） |
...
[Nastech Portal](https://portal.nastechresearch.com) 是 Nastech Research 的统一订阅网关，也是**运行 Nastech Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等）以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）——费用从你的 Nastech 订阅中扣除，无需单独管理各提供商账户。

```bash
nastech setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
nastech model              # 已有安装——从列表中选择"Nastech Portal"
nastech portal info        # 随时查看登录状态和路由信息
```

还没有订阅？前往 [portal.nastechresearch.com/manage-subscription](https://portal.nastechresearch.com/manage-subscription) 购买。

**完整详情：** 参见专属的 [Nastech Portal 集成页面](/integrations/nastech-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 Nastech Portal 运行 Nastech Agent](/guides/run-nastech-with-nastech-portal)。
...
:::warning
即使使用 Nastech Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Nastech 将这些任务路由到你的**主聊天模型**——即你在 `nastech model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nastech Tool Gateway
付费 Nastech Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`nastech setup --portal` 一条命令即可完成登录、设置 Nastech 为提供商并开启网关。现有用户可通过 `nastech model` 或 `nastech tools` 按工具启用。随时使用 `nastech portal info` 查看路由状态。
:::
...
如果未找到 token，`nastech model` 会提供 **OAuth 设备码登录**——与 Copilot CLI 和 opencode 使用的流程相同。
...
| 类型 | 前缀 | 获取方式 |
|------|--------|------------|
| OAuth token | `gho_` | `nastech model` → GitHub Copilot → 使用 GitHub 登录 |
...
| `NASTECH_COPILOT_ACP_COMMAND` | 覆盖 Copilot CLI 二进制路径（默认：`copilot`） |
| `NASTECH_COPILOT_ACP_ARGS` | 覆盖 ACP 参数（默认：`--acp --stdio`） |
...
xAI 还提供专属 TTS 端点（`/v1/tts`）。在 `nastech tools` → 语音与 TTS 中选择 **xAI TTS**，或参见[语音与 TTS](../user-guide/features/tts.md#text-to-speech) 页面了解配置。
...
# → 浏览器打开；使用阿里巴巴账户登录
# → 确认——凭据保存到 ~/.nastech/auth.json

nastech chat   # 使用 portal.qwen.ai/v1 端点
```
...
仅在 portal 端点迁移时才需设置 `NASTECH_QWEN_BASE_URL`（默认：`https://portal.qwen.ai/v1`）。
...
nastech chat   # 使用 api.minimax.io/anthropic 端点
```
...
# 需要：~/.nastech/.env 中的 NVIDIA_API_KEY
...
```bash
# GMI Cloud
nastech chat --provider gmi --model deepseek-ai/DeepSeek-R1
# 需要：~/.nastech/.env 中的 GMI_API_KEY
```
...
# 需要：~/.nastech/.env 中的 STEPFUN_API_KEY
```
...
```bash
# 使用任意可用模型
nastech chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# 需要：~/.nastech/.env 中的 HF_TOKEN
...
| **vLLM** | 添加 `--enable-auto-tool-choice --tool-call-parser nastech` |
...
通过 `nastech model` → 自定义端点，或在 `config.yaml` 中配置任意上述服务：
...
7. **Nastech Portal** — 将 Nastech 模型 ID 后缀匹配到 OpenRouter 元数据
...
`nastech model` 在配置自定义端点时会提示输入上下文长度。留空则自动检测。
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # 由 `nastech model` → 自定义端点向导显式设置；自动检测仍作为回退
...
`nastech model` → 自定义端点向导现在会显式提示 `api_mode` 并将你的答案持久化到 `config.yaml`。当字段留空时，基于 URL 的自动检测（例如 `/anthropic` 路径 → `anthropic_messages`）仍作为回退。
...
也可以从交互式 `nastech model` 菜单中选择命名自定义提供商。

---

### 实战配置：Together AI、Groq、Perplexity

[其他兼容提供商](#other-compatible-providers) 中列出的云提供商都使用 OpenAI 的 REST 方言，因此在 `custom_providers:` 下的接入方式相同。以下是三个可直接使用的配置示例。每个示例放入 `~/.nastech/config.yaml`，对应的 API key 放入 `~/.nastech/.env`。
...
Together 的 `/v1/models` 端点可用，因此 `nastech model` 可以自动发现可用模型。
...
:::tip 故障排查
- `nastech doctor` 对于上述任何名称都不应打印 `Unknown provider` 警告（在 #15083 的 CLI 验证器修复之后）。
- 如果某个提供商的 `/v1/models` 端点不可达（Perplexity 是常见情况），`nastech model` 会在警告后持久化模型而不是硬性拒绝——参见 #15136。
...
| 使用场景 | 推荐方案 |
|----------|-------------|
| **只想让它工作** | OpenRouter（默认）或 Nastech Portal |
| **本地模型，简单配置** | Ollama |
...
:::tip
可以随时使用 `nastech model` 切换提供商——无需重启。无论使用哪个提供商，你的对话历史、记忆和技能都会保留。
:::
...
使用 OpenRouter 时，可以控制请求如何在提供商之间路由。在 `~/.nastech/config.yaml` 中添加 `provider_routing` 节：
...
OpenRouter 提供一个实验性编程模型路由器 `openrouter/pareto-code`，自动将请求路由到满足编程质量标准的最便宜模型（按 [Artificial Analysis](https://artificialanalysis.ai/) 排名）。选择此模型并在 `~/.nastech/config.yaml` 中调整 `min_coding_score` 参数：

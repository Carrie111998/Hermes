...
| `HERMES_KANBAN_DB` | 直接固定 kanban 数据库文件路径。在 dispatcher→worker 交接（`board=None`）时该环境变量无条件生效；当调用方显式传入 `board=`（或 `scoped_current_board`）时，通过 dispatcher 注入的 `HERMES_KANBAN_BOARD` 身份区分：同看板重定位 pin（保留 pin）、真正的跨看板读取（显式看板生效并告警）、或歧义重定位（抛错）。详见 `kanban_db_path()`（任务 t_e96dd0cb） |

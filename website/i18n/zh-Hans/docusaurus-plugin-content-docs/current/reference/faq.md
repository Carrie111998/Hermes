...
**原因：** 当前精确调用没有可消费的结构化 capability。Hermes 不通过关键字或命令文本模式作出这个判断。

**解决方案：** 出现提示时，检查完整调用并批准本次精确执行。您也可以：
- 要求智能体使用更安全的替代方案
- 在[安全文档](../user-guide/security.md)中了解精确终端授权

:::tip
这是预期行为。审批提示会显示将要执行的精确内容，签发的 capability 仅可使用一次且不可重放。
:::

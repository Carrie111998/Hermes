...
| `decompose <id>` / `decompose --all` | 将 triage 列中的任务按描述拆分为子任务图，路由到专业 profile（编排器驱动路径）。当 LLM 判断任务不适合拆分时，回退到 specify 风格的单任务提升。与 `specify` 相同的标志。在 `config.yaml` 的 `auxiliary.kanban_decomposer` 下配置模型。自动分解为显式选择功能：设置 `kanban.auto_decompose: true` 后，每次调度器 tick 才会自动运行；安全默认值为手动模式。参见 [自动与手动编排](/user-guide/features/kanban#auto-vs-manual-orchestration)。 |

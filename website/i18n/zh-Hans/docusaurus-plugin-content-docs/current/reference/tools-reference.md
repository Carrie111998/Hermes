...
| 工具 | 描述 | 所需环境 |
|------|------|----------|
| `image_analyze` | 使用已配置的辅助视觉模型一次分析 1 至 16 个图片 URL 或本地路径，并返回文本分析结果。 | — |

...
- **Worker/orchestrator toolset：** 调度器生成的 worker 使用 `kanban_show`、`kanban_complete`、`kanban_block`、`kanban_heartbeat`、`kanban_comment`；前台/编排器 profile 才使用 `kanban_create`、`kanban_link`，并在显式启用 `kanban` toolset 时获得 `kanban_list` 和 `kanban_unblock` 用于看板路由。

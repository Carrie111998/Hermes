...
| | |
|---|---|
| 来源 | 可选 — 通过 `orion skills install official/research/qmd` 安装 |
...
| 相关 skill | [`obsidian`](/user-guide/skills/bundled/note-taking/note-taking-obsidian), [`orion-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-orion-agent), [`arxiv`](/user-guide/skills/bundled/research/research-arxiv) |
...
qmd 提供 MCP 服务器，可通过原生 MCP 客户端直接向 Orion Agent 提供搜索工具。这是推荐的集成方式 — 配置完成后，agent 无需每次加载此 skill 即可自动获得 qmd 工具。

### 方案 A：Stdio 模式（简单）

在 `~/.orion/config.yaml` 中添加：

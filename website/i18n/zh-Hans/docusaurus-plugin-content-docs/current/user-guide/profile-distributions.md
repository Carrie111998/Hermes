...
**Profile 分发**将一个完整的 Orion agent——个性、技能、cron 任务、MCP 连接、配置——打包为一个 git 仓库。任何有权访问该仓库的人都可以用一条命令安装整个 agent，就地更新，并保持自己的记忆、会话和 API 密钥不受影响。

如果说 [profile](./profiles.md) 是本地 agent，那么分发就是让该 agent 可共享的形式。

## 这意味着什么

在分发功能出现之前，共享一个 Orion agent 意味着要发送：
...
```bash
orion profile install github.com/you/my-research-agent --alias
```

……他们就拥有了完整的 agent。填入自己的 API 密钥（`.env.EXAMPLE` → `.env`），即可运行 `my-research-agent chat`，或通过 Telegram / Discord / Slack / 任何 gateway 平台与其交互。当你推送新版本时，他们运行 `orion profile update my-research-agent` 即可拉取你的更改——他们的记忆和会话保持不变。
...
权衡之处：接收方需要安装 git。在 2026 年运行 Orion 的任何机器上，这已是既成事实。
...
- **你只想在自己的机器上备份一个 profile。** 使用 [`orion profile export` / `import`](../reference/profile-commands.md#orion-profile-export)——那正是这两个命令的用途。
...
```bash
orion profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.orion/profiles/research-bot/SOUL.md
...
创建 `~/.orion/profiles/research-bot/distribution.yaml`：

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
orion_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
```bash
cd ~/.orion/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
运行 `orion profile update research-bot` 的接收方将拉取最新版本。
...
```bash
orion profile install github.com/you/research-bot --alias
```
...
4. 请求确认。传入 `-y` / `--yes` 可跳过。
5. 将分发所有的文件复制到 `~/.orion/profiles/research-bot/`（或 manifest 中 `name` 解析到的位置）。
...
```bash
# GitHub shorthand
orion profile install github.com/you/research-bot

# Full HTTPS
orion profile install https://github.com/you/research-bot.git

# SSH
orion profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
orion profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
orion profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
orion profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
orion profile install github.com/acme/support-bot --name support-us --alias
# Bob（同一分发，不同本地名称）
orion profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Orion distribution.
...
```bash
cp ~/.orion/profiles/research-bot/.env.EXAMPLE ~/.orion/profiles/research-bot/.env
...
```bash
orion profile info research-bot
```
...
`orion profile list` 还会显示 `Distribution` 列，让你一眼看出哪些 profile 来自仓库，哪些是手动构建的：
...
```bash
orion profile update research-bot
```
...
```bash
orion profile delete research-bot
```
...
Path:    ~/.orion/profiles/research-bot
...
在笔记本上的任何迭代（`git commit && push`）都可以通过 `orion profile update research-bot` 同步到工作站。记忆按机器独立保存——笔记本记住自己的对话，工作站记住自己的，互不干扰。
...
# 每位工程师
orion profile install git@github.com:your-org/pr-reviewer.git --alias
# 填写 .env，使用自己的 API 密钥（费用由自己承担），.env.EXAMPLE 指明了所需内容
pr-reviewer chat
```

当负责人发布 v1.1（更好的 SOUL、新技能）时，工程师运行 `orion profile update pr-reviewer`，所有人在几分钟内就能用上新版本。
...
```bash
# 你
cd ~/.orion/profiles/polymarket-trader
# 在仓库根目录写一个完整的 README.md——GitHub 会在仓库页面展示它
git init && git add . && git commit -m "v1.0"
git tag v1.0.0
# 发布到公开 GitHub 仓库
git remote add origin https://github.com/you/orion-polymarket-trader.git
git push -u origin main --tags

# 任何人
orion profile install github.com/you/orion-polymarket-trader --alias
```
...
你在 Orion 之上构建了产品——也许是合规监控框架、客服技术栈、特定领域的研究平台。你想以产品形式分发它。
...
# 每位值班人员
orion profile install git@github.com:your-org/incident-2026-q2.git --alias

# 事故解决——清理
orion profile delete incident-2026-q2
```
...
:::note
Git ref 固定（`#v1.2.0`）已在规划中，但不在初始版本中——目前安装时跟踪默认分支。通过 `orion profile info <name>` 查看已安装版本，在准备好之前暂缓更新。
:::
...
```bash
# 你已安装的版本
orion profile info research-bot | grep Version
...
```yaml
# ~/.orion/profiles/research-bot/local/my-overrides.yaml
...
```bash
# 彻底删除并重新安装（记忆/会话也会丢失）
orion profile delete research-bot --yes
orion profile install github.com/you/research-bot --alias

# 更新到当前 main，但将 config.yaml 重置为分发默认值
orion profile update research-bot --force-config --yes
```
...
orion profile install github.com/yourname/forked-research-bot --alias

# 在 ~/.orion/profiles/forked-research-bot/ 中本地迭代
...
```bash
# 从本地目录安装（无需 git push）
orion profile install ~/.orion/profiles/research-bot --name research-bot-test --alias

# 调整、删除、重新安装，直到满意
orion profile delete research-bot-test --yes
orion profile install ~/.orion/profiles/research-bot --name research-bot-test
```
...
来自分发的 cron 任务**不会自动调度**——安装器会打印 `orion -p <name> cron list`，你需要显式启用它们。SOUL.md 和技能在你开始与 profile 对话后立即生效，因此如果你从不熟悉的来源安装，请在第一次运行前阅读它们。
...
- `install`、`update`、`info` 位于 `orion profile` 下——不是独立的命令树。
...
- 保留的 profile 名称（`orion`、`test`、`tmp`、`root`、`sudo`）在安装时会被拒绝，以避免与常见二进制文件冲突。

## 另请参阅

- [Profiles：运行多个 Agent](./profiles.md) — 基础概念
- [Profile 命令参考](../reference/profile-commands.md) — 每个标志、每个选项
- [`orion profile export` / `import`](../reference/profile-commands.md#orion-profile-export) — 本地备份 / 恢复（非分发）
- [在 Orion 中使用 SOUL](../guides/use-soul-with-orion.md) — 编写个性

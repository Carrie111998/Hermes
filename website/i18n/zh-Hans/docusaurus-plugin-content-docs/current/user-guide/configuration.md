...
```yaml
agent:
  tool_use_enforcement: true   # true | false；旧版 "auto"/列表不会注入
```
...
| `true` | 对所有模型启用相同的通用指导。 |
| `false` | 对所有模型禁用。 |
| `"auto"`（旧版默认值） | 禁用。仅为配置兼容而保留，不会解释模型名称。 |
| 列表（旧版） | 禁用。不再支持按模型名称子字符串路由。 |

### 注入的内容

显式启用后，Hermes 会添加一个与模型无关的通用指导块：要求模型实际调用工具、持续工作直到任务完成，并且不以承诺未来操作结束轮次。模型标识不会改变系统提示词。

### 何时开启

如需此指导，请显式启用；所有模型获得相同的行为：

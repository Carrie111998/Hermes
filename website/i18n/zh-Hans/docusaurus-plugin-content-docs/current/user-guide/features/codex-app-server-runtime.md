...
- **`kanban_show`** — 让 worker 读取自己的任务上下文。任务范围的 worker 不会收到 `kanban_list`。
- **前台/控制平面工具** — 未设置 `HERMES_KANBAN_TASK` 的前台会话可使用看板路由和拓扑变更工具；任务范围的 worker 不会收到 `kanban_list`、`kanban_create`、`kanban_unblock` 或 `kanban_link`。后续任务创建、解除阻塞和父子关联由前台/控制平面负责。

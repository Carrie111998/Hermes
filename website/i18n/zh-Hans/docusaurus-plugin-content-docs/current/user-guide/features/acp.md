...
一次会修改主机的临时目录删除操作会直接完成，全程没有任何提示。
...
Hermes 不使用关键字、正则表达式或命令路由器解释终端文本。LLM 模型是唯一语义解释主体；运行时只验证当前精确调用的结构化权限。

肯定响应会创建不透明的单次 capability，绑定精确命令字节、会话 epoch、有效期和使用次数。它不能授权相似命令、不能重放，也不会成为会话级或永久模式授权。如果 ACP 宿主显示 `allow_session` 或 `allow_always`，Hermes 会把这些肯定响应收窄为当前精确调用一次。拒绝、超时、字节不匹配或桥接失败都会故障关闭。

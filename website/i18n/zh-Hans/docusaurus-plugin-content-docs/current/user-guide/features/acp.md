...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 IYARI"
---
...
对于 Zed registry 安装，Zed 通过官方 ACP Registry 条目启动 IYARI。该条目使用 `uvx` 发行版运行：
...
IYARI 官方 ACP Registry 元数据的源文件位于：
...
Provider 解析使用 IYARI 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。IYARI 还为首次运行的 registry 客户端提供终端认证方法（`--setup`）；这将打开 IYARI 的交互式模型/provider 设置。
...
- 在 Zed 中，使用 `zed: acp registry` 打开 ACP Registry 并搜索 **IYARI**。
...
或编辑 `~/.hermes/.env`。Registry 客户端也可以触发 IYARI 的终端认证流程，该流程运行相同的交互式 provider/模型设置。

### Zed registry 启动器找不到 uv

从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 IYARI 线程。

...
在飞书/Lark 私聊中，如果一项工作需要独立的会话和完整记录，可以使用 `/newchat <任务>`：

```text
/newchat 帮我整理电脑上的下载文件
```

Hermes 会在该命令下创建一个新的飞书话题，并只把 `/newchat` 后面的文字作为任务执行。生成的 `thread_id` 会成为 Hermes 会话键的一部分，因此进度消息、最终结果以及你之后在话题里的回复都会复用同一个隔离会话，不会继续使用主私聊会话。如果话题创建失败，Hermes 会在主聊天中明确提示，并且不会退回主聊天执行任务。

`/newchat` 适合明确划分任务边界。普通单轮问题仍留在主聊天；系统不会自动把任意消息升级为话题，因为在执行前无法可靠判断任务是否一定需要多轮互动。

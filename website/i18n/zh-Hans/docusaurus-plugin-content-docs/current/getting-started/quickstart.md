...
这是最重要的配置步骤。使用 `nastech model` 以交互方式完成选择：

```bash
nastech model
```

:::tip 最简路径：Nastech Portal
...
该命令一次性完成登录、设置 Nastech 为 provider 并开启 Tool Gateway。
:::
...
| **MiniMax (OAuth)** | 通过浏览器 OAuth 使用 MiniMax-M2.7，无需 API key | `nastech model` → MiniMax (OAuth) |
...
:::tip
你可以随时通过 `nastech model` 切换 provider——没有锁定。所有支持的 provider 完整列表及配置详情，请参阅 [AI Providers](../integrations/providers.md)。
:::
...
- `nastech tools` — 按平台调整工具访问权限
- `nastech skills` — 浏览并安装可复用的工作流
...
```bash
nastech config set terminal.backend docker    # Docker 隔离
nastech config set terminal.backend ssh       # 远程服务器
```
...
```bash
nastech skills search kubernetes
...
（如果安装时未包含 `[all]`，请先运行 `cd ~/.nastech/nastech-agent && uv pip install -e ".[acp]"`。）
...
| `nastech doctor` 标记配置问题 | 配置值缺失或已过期 | 修复配置，在添加功能前重新测试普通对话 |
...
|---------|-------------|
| `nastech` | 开始聊天 |
| `nastech model` | 选择 LLM provider 和模型 |
| `nastech tools` | 配置每个平台启用的工具 |
| `nastech setup` | 完整配置向导（一次性配置所有内容） |
| `nastech doctor` | 诊断问题 |
| `nastech update` | 更新到最新版本 |
| `nastech gateway` | 启动消息 gateway |
| `nastech --continue` | 恢复上次会话 |

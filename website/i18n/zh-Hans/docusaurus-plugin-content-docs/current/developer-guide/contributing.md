...
```bash
# 标准安装器已经把 `nastech` 放到了 PATH 上。
nastech doctor
nastech chat -q "Hello"
```

如果你使用了手动克隆备用路径，可以在 checkout 中运行 `./nastech`，或显式把这个 clone 的 venv 链接到 PATH：
...
- **Profile 安全路径**：不得硬编码 `~/.nastech` — 代码路径使用 `nastech_constants` 中的 `get_nastech_home()`，面向用户的消息使用 `display_nastech_home()`。完整规则参见 [AGENTS.md](https://github.com/NastechResearch/nastech-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/NastechResearch/nastech-agent/issues)
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/NastechResearch/nastech-agent/blob/main/LICENSE) 授权。
...
- **Profile 安全路径**：不得硬编码 `~/.hermes` — 代码路径使用 `hermes_constants` 中的 `get_hermes_home()`，面向用户的消息使用 `display_hermes_home()`。完整规则参见[架构与运行时指南](https://github.com/NousResearch/hermes-agent/blob/main/docs/agents/architecture-and-runtime.md#profiles-multi-instance-support)。

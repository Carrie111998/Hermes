...
description: Готовые к использованию схемы автоматизации — запланированные задачи,
  триггеры событий GitHub, веб-перехватчики API и рабочие процессы с несколькими навыками.
---

# Чертежи автоматизации

Скопируйте и вставьте чертежи распространенных шаблонов автоматизации. В каждом проекте используется встроенный [планировщик cron](/user-guide/features/cron) Hermes для триггеров, основанных на времени, и [платформа веб-перехватчиков](/user-guide/messaging/webhooks) для триггеров, управляемых событиями.

Каждый проект работает с **любой моделью** и не привязан к одному поставщику.

Параметризированные схемы с формами вместо синтаксиса cron см. в [Каталоге схем автоматизации](/reference/automation-blueprints-catalog).

:::tip Три типа триггеров
| Триггер | Как | Инструмент |
|---------|-----|------|
| **Расписание** | Работает с определенной частотой (ежечасно, еженощно, еженедельно) | инструмент `cronjob` или косая черта `/cron` |
| **Событие GitHub** | Пожары по пиару открываются, продвигаются, выдаются, результаты КИ | Платформа вебхуков (`hermes webhook subscribe`) |
| **Вызов API** | Внешняя служба отправляет JSON в вашу конечную точку | Платформа Webhook (маршруты config.yaml или `hermes webhook subscribe`) |

Все три поддерживают доставку в Telegram, Discord, Slack, SMS, электронную почту, комментарии GitHub или локальные файлы.
:::
...
### Ночная сортировка невыполненных работ

Маркируйте, расставляйте приоритеты и суммируйте новые проблемы каждый вечер. Предоставляет дайджест на канал вашей команды.

**Триггер:** Расписание (ночное)
...
### Автоматическая проверка PR-кода

Автоматически просматривайте каждый запрос на включение при его открытии. Публикует комментарий к обзору непосредственно в PR.

**Триггер:** Вебхук GitHub

**Вариант А — Динамическая подписка (CLI):**
...
**Вариант Б. Статический маршрут (config.yaml):**
...
Затем в GitHub: **Настройки → Веб-перехватчики → Добавить веб-перехватчик** → URL-адрес полезной нагрузки: `http://your-server:8644/webhooks/github-pr-review`, Тип контента: `application/json`, Секрет: `github-webhook-secret`, События: **Запросы на извлечение**.

### Обнаружение смещения документов

Еженедельное сканирование объединенных запросов на запросы для поиска изменений API, требующих обновления документации.
...
Ежедневное сканирование на наличие известных уязвимостей в зависимостях проекта.
...
Запускайте дымовые тесты после каждого развертывания. Ваш конвейер CI/CD отправляет POST-запросы на веб-перехватчик после завершения развертывания.

**Триггер:** вызов API (вебхук)
...
Ваш конвейер CI/CD запускает его:
...
Сопоставьте оповещения мониторинга с недавними изменениями, чтобы подготовить ответ. Работает с Datadog, PagerDuty, Grafana или любой системой оповещения, которая может POST JSON.
...
Проверяйте конечные точки каждые 30 минут. Уведомляйте только тогда, когда что-то не работает.

**Триггер:** Расписание (каждые 30 минут)
...
### Конкурентный разведчик репозитория

Отслеживайте репозитории конкурентов на предмет интересных PR, функций и архитектурных решений.
...
Еженедельный обзор разработок AI/ML.
...
### Бумажный дайджест с примечаниями

Ежедневное сканирование arXiv, которое сохраняет сводки в вашей системе заметок.
...
## Автоматизация событий GitHub

### Автоматическая маркировка проблем

Автоматически отмечайте новые проблемы и реагируйте на них.
...
Анализируйте сбои CI и размещайте диагностику на PR.
...
### Изменения автоматического переноса в репозитории

Когда PR объединяется в один репозиторий, эквивалентное изменение автоматически переносится в другой.
...
Отслеживайте события платежей и получайте сводные данные о сбоях.
...
### Сводка ежедневных доходов

Каждое утро собирайте ключевые бизнес-показатели.
...
## Рабочие процессы с несколькими навыками

### Конвейер аудита безопасности

Объедините несколько навыков для комплексного еженедельного анализа безопасности.
...
Исследуйте, составляйте проекты и готовьте контент по графику.
...
| Цель | Флаг | Заметки |
|--------|------|-------|
| Тот же чат | `--deliver origin` | По умолчанию — доставка туда, где было создано задание |
| Локальный файл | `--deliver local` | Сохраняет вывод, нет уведомлений |
| Телеграмма | `--deliver telegram` | Домашний канал или `telegram:CHAT_ID` для конкретного |
| Раздор | `--deliver discord` | Домашний канал, или `discord:CHANNEL_ID` |
| слабый | `--deliver slack` | Домашний канал |
| СМС | `--deliver sms:+15551234567` | Прямо на номер телефона |
| Конкретная тема | `--deliver telegram:-100123:456` | Тема форума Telegram |

### Переменные шаблона веб-перехватчика

| Переменная | Описание |
|----------|-------------|
| `{pull_request.title}` | PR-заголовок |
| `{issue.number}` | Номер выпуска |
| `{repository.full_name}` | `owner/repo` |
| `{action}` | Действие события (открыто, закрыто и т.д.) |
| `{__raw__}` | Полная полезная нагрузка JSON (обрезанная до 4000 символов) |
| `{sender.login}` | Пользователь GitHub, инициировавший событие |

### Шаблон [ТИХИЙ]

Если ответ задания cron содержит `[SILENT]`, доставка подавляется. Используйте это, чтобы избежать спама в уведомлениях при тихом запуске:

```
If nothing noteworthy happened, respond with [SILENT].
```

Это означает, что вы получаете уведомление только тогда, когда агенту есть что сообщить.
...
2. The unified Orion Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                orion_meta = metadata.get("orion", {})
                if isinstance(orion_meta, dict):
                    tags = orion_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])

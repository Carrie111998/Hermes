#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Orion docs site.
...
LLM reads to learn what Orion does.

Both publish at:
  https://your-orion-docs.example/docs/llms.txt
  https://your-orion-docs.example/docs/llms-full.txt
...
SITE_BASE = "https://your-orion-docs.example/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Orion as a Python Library", None),
        ("guides/use-mcp-with-orion", "Use MCP with Orion", None),
        ("guides/use-voice-mode-with-orion", "Use Voice Mode with Orion", None),
        ("guides/use-soul-with-orion", "Use SOUL.md with Orion", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Orion"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Orion Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Orion. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/Orion/"
        "orion-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/zacharyjleach-stack/Aries")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Orion Agent — Full Documentation\n",
        (
            "This file is the entire Orion Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Orion, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://your-orion-docs.example/docs\n",
        "Short index: https://your-orion-docs.example/docs/llms.txt\n",
        "\n---\n\n",
    ]

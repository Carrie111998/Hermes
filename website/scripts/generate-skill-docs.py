...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/zacharyjleach-stack/Aries/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    orion_meta = (fm.get("metadata") or {}).get("orion") or {}
    tags = orion_meta.get("tags") or []
    related = orion_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `orion skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",

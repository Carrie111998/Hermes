...
class HermesACPAgent(acp.Agent):
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        state.agent = self.session_manager._make_agent(
            ...
            base_url=getattr(state.agent, "base_url", None) if same_provider else None,
            api_mode=getattr(state.agent, "api_mode", None) if same_provider else None,
            responses_transport=(
                getattr(state.agent, "responses_transport", None) if same_provider else None
            ),
        )
    ...
    async def set_session_model(
        ...
        if state:
            ...
            same_provider = bool(current_provider and requested_provider == current_provider)
            current_base_url = getattr(state.agent, "base_url", None) if same_provider else None
            current_api_mode = getattr(state.agent, "api_mode", None) if same_provider else None
            current_responses_transport = (
                getattr(state.agent, "responses_transport", None) if same_provider else None
            )
            state.agent = self.session_manager._make_agent(
                session_id=session_id,
                ...
                responses_transport=current_responses_transport,
            )
            self.session_manager.save_session(session_id)
            logger.info(

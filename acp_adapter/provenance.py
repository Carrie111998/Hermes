...
This is an additive Orion extension surfaced under ACP ``_meta.orion`` so
...
compression rotates the internal Orion head, ``build_session_provenance`` lets
...
def build_session_provenance(
    db: Any,
    acp_session_id: str,
    current_orion_session_id: str,
    *,
    previous_orion_session_id: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Build ``_meta.orion.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_orion_session_id: The live internal Orion DB session id
            (``state.agent.session_id``).
        previous_orion_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"orion": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_orion_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_orion_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_orion_session_id}
    ...
    rotated = bool(
        previous_orion_session_id
        and previous_orion_session_id != current_orion_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentOrionSessionId": current_orion_session_id,
        "rootOrionSessionId": root_id,
        "parentOrionSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_orion_session_id:
        provenance["previousOrionSessionId"] = previous_orion_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"orion": {"sessionProvenance": ...}}``."""
    prov = build_session_provenance(
        db,
        acp_session_id,
        current_orion_session_id,
        previous_orion_session_id=previous_orion_session_id,
    )
    if prov is None:
        return None
    return {"orion": {"sessionProvenance": prov}}

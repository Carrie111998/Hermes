"""ACP auth helpers — detect and advertise Orion authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "orion-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Orion runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Orion can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Orion.
    ...
    not have Orion provider credentials configured yet, so Orion always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Orion using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Orion provider",
            description=(
                "Open Orion' interactive model/provider setup in a terminal. "
                "Use this when Orion has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )

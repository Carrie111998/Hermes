"""CLI entry point for the orion-agent ACP adapter.

Loads environment variables from ``~/.orion/.env``, configures logging
...
try:
    ...
    # shadowing Orion's own modules — ``orion acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from ORION_HOME (default ``~/.orion``)."""
    from orion_cli.env_loader import load_orion_dotenv

    orion_home = get_orion_home()
    loaded = load_orion_dotenv(orion_home=orion_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", orion_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="orion-acp",
        description="Run Orion Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Orion version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import OrionACPAgent  # noqa: F401

    print("Orion ACP check OK")


def _run_setup() -> None:
    from orion_cli.main import main as orion_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "orion", "model"]
        orion_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting orion-agent ACP adapter")
    ...
    import acp
    from .server import OrionACPAgent
    ...
    if os.environ.get("ORION_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:

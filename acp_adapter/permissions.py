"""ACP permission bridging for Orion dangerous-command approvals."""
...
# Maps ACP permission option ids to Orion approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, smart_denied: bool = False,
    ...
    """Return ACP options that match Orion approval semantics."""
    ...
    if not smart_denied:
        options.append(PermissionOption(
            ...
            # hint while keeping Orion semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_orion(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Orion approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_ORION.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Orion-compatible approval callback that bridges to ACP.

...
- Interval resolution order: explicit constructor arg → ``config.yaml``
  (``acp.keepalive_interval_s``) → env var
  (``HERMES_ACP_KEEPALIVE_INTERVAL_S``) → default 45s. Interval <= 0 disables
  the feature (``make_turn_keepalive`` returns ``None``).
"""
...
_ENV_KEY = "HERMES_ACP_KEEPALIVE_INTERVAL_S"
...
def get_keepalive_interval(
    env_key: str = _ENV_KEY, default: float = _DEFAULT_INTERVAL_S
) -> float:
    """Resolve the keepalive interval in seconds.

    Precedence: env var (host override) → config.yaml → provided default.
    """
    raw_env = os.environ.get(env_key)
    if raw_env is not None:
        parsed = _coerce_float(raw_env)
        if parsed is not None:
            return parsed
        logger.warning("Invalid %s=%r; falling back to config/default", env_key, raw_env)

...
> **方案状态（2026-08-02）**：本文保留为 3-5 年候选架构和问题地图，不代表当前
> Hermes 生产已部署知识图谱或四层记忆。当前实施基线是可插拔、可回滚的
> `plugins/team_memory` SQLite + FTS5 MVP；先通过真实 Agent A/B 和人工相关性抽样
> 证明价值，再决定是否引入图数据库、向量检索或自动关系抽取。文中的投入、ROI、
> 实体关系数量和性能目标均为设计假设，不能直接作为已验证事实。

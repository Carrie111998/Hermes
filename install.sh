...
install_runtime() {
    if [[ "$USE_VENV" == true ]]; then
        ...
        uv pip install --python "$INSTALL_DIR/.venv/bin/python" -e "$INSTALL_DIR[all]"
    else
...
install_launcher() {
    local launcher="$ARES_BIN_DIR/ares"
    mkdir -p "$ARES_BIN_DIR"
    if [[ -e "$launcher" ]] && ! grep -q 'Ares compatibility launcher' "$launcher"; then
        die "refusing to overwrite an existing non-Ares launcher: $launcher"
    fi
    cat >"$launcher" <<EOF
#!/usr/bin/env sh
# Ares compatibility launcher — generated by the Ares installer.
export HERMES_HOME=$(printf '%q' "$HERMES_HOME")
exec $(printf '%q' "$(runtime_hermes)") "\$@"
EOF
    chmod 0755 "$launcher"
}

"""Run the read-tool eval through the REAL Orion AIAgent.

For each task: fresh temp ORION_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    orion_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".orion"
    ...
    try:
        ...
        shutil.rmtree(orion_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.orion/.env; set +a  — then relaunch."
        )

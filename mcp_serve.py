"""
Orion MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Orion-specific extra)

Usage:
    orion mcp serve
    orion mcp serve --verbose
...
logger = logging.getLogger("orion.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ORION_HOME."""
    try:
        from orion_constants import get_orion_home
        return get_orion_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ORION_HOME", Path.home() / ".orion")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from orion_constants import get_orion_home
        directory_file = get_orion_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ORION_HOME", Path.home() / ".orion")
...
class EventBridge:
    ...
    This is the Orion equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from orion_constants import get_orion_home
            db_file = get_orion_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ORION_HOME", Path.home() / ".orion")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Orion MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "orion",
        instructions=(
            "Orion Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Orion MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

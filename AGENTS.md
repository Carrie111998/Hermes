...
- **Behavior contracts over snapshots.** Tests should assert how two pieces of
  ...
  config version literals, enumeration counts). See
  [`tests/AGENTS.md`](tests/AGENTS.md#dont-write-change-detector-tests).
...
The root file contains repository-wide invariants only. Before editing a scoped
area, read its nearest `AGENTS.md`; do not assume the root duplicates those rules.
Context discovery loads `AGENTS.md` from the current working directory only — it
does not merge parent and child files — so scoped guides explicitly link back here.

- `apps/desktop/AGENTS.md` — Electron/React desktop architecture and testing.
- `ui-tui/AGENTS.md` — Ink TUI process model, JSON-RPC, and development commands.
- `tui_gateway/AGENTS.md` — shared backend boundaries across TUI clients.
- `web/AGENTS.md` — dashboard PTY embedding and chat-surface boundaries.
- `hermes_cli/AGENTS.md` — configuration and skin/theme implementation.
- `gateway/AGENTS.md` — messaging concurrency and background notifications.
- `tools/AGENTS.md` — built-in tools, tool exposure, and delegation.
- `plugins/AGENTS.md` — plugin contracts and in-tree policy.
- `plugins/kanban/AGENTS.md` — Kanban dispatcher and worker invariants.
- `skills/AGENTS.md` and `optional-skills/AGENTS.md` — skill authoring standards.
- `cron/AGENTS.md` — scheduler formats, delivery, and hardening invariants.
- `tests/AGENTS.md` — test runner, placement, and test-design rules.
...
Use `scripts/run_tests.sh`; do not invoke `pytest` directly. The wrapper isolates
credentials and `HERMES_HOME`, normalizes timezone/locale, and runs each test file
in a fresh subprocess. Put tests for JavaScript/TypeScript artifacts in the
relevant Vitest suite, not Python tests. Test behavior and invariants rather than
source text, changing catalogs, counts, or config-version snapshots. Read
`tests/AGENTS.md` before adding or changing tests.

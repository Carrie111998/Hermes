...
- When a restart explicitly requests llama, use `restart-hermes-stack.ps1 -StartLlama` with the llama hot-swap router when present (`scripts/windows/start-llama-hotswap.ps1` / `llama-hotswap-models.ini`) or existing Turboquant / RTX 5060 Ti scripts; wait until llama is warm/ready rather than treating a cold fallback start as success; if llama hot-swap is already healthy on `:8080` (`/v1/models` OK), preserve that process and omit `-StartLlama` so stack restart does not kill it; do not invent new non-secret `HERMES_*` env vars.
...
- On stack restarts, bring up needed Hermes services without leaving duplicate Desktop, backend, or watchdog processes; use `-SkipTunnels` on `restart-hermes-stack.ps1` when memory-graph/tunnels hang; if `-StartGoWatchdog` BuildIfMissing/prewarm hangs, recover with a direct packaged exe plus `hermes serve --skip-build` on `:9119` before relaunching Desktop. For a fully clean Desktop recovery: stop the Go watchdog first, clear wedged `:9118`/`:9119`, verify timed HTTP on serve (not only LISTEN), then launch a single Desktop — and do not disturb Hermes-WebUI `:8787` during that recovery.
- Run Hermes CLI, stack restarts, Desktop backend, and smoke tests from the uv-managed project `.venv` on Python 3.12 (`uv venv --python 3.12` / `uv sync`); do not use system 3.14 or a bare `py -3` that resolves to the wrong version.
...
- For upstream security/fix/Dependabot contributions: cut a dedicated branch/worktree from latest `upstream/main` (not fork `main` history), exclude `_docs`/fork-only noise, write PRs in King's English, and first confirm no duplicate Issue/PR; when a prior fix is incomplete, salvage and send a PR rather than opening a duplicate; harden env-leak / path-traversal / agent-runaway paths without gutting agent capability.
...
- After Go watchdog source changes, rebuild explicitly with `scripts/windows/Build-HermesGoWatchdog.ps1` (e.g. `-SkipTest`) before `-StartGoWatchdog`; an existing `hermes-watchdog.exe` can leave the old binary in place and skip rebuild.
- Desktop remote / Go-watchdog prewarm on `:9118` (or any managed port): `/api/status` alone is not enough, and LISTEN/TCP accept is not enough either — a wedged `hermes serve` can leave the port open while HTTP hangs at 0 bytes. If the desktop-backend manifest token drifts from the live `hermes serve` session token, `/api/sessions` returns 401 and Desktop shows "Could not connect to Hermes gateway" (UI may also show remote `127.0.0.1:9118` + Unauthorized). Go watchdog probes gated auth (`testBackendAuth`) before publishing the manifest and replaces the port occupant on drift. When `:9119` serve is HTTP-dead, a healthy dashboard on `:9120` can temporarily back Desktop via `HERMES_DESKTOP_REMOTE_URL` / `HERMES_DESKTOP_REMOTE_TOKEN`; stale remote overrides pointing at a dead `:9120` or drifted `:9118` must be cleared when recovering so Desktop returns to a healthy local `:9119` announcement.
...
- Local Windows port map for ops is in `fork/operations/AGENTS.md` (**8787 = Hermes-WebUI** via `HERMES_WEBUI_PORT` / `start-hermes-webui.ps1` — messaging gateway is a separate process and does **not** bind 8787; watchdog prewarm serve 9118, Desktop `hermes serve` 9119, dashboard 9120; llama often 8080/8081, harness 18794); reserved ops ports must not be treated as the Desktop backend or reaped by the watchdog.
- On this Windows host, `Get-CimInstance` / `netstat` (and sometimes `taskkill`) can hang for a long time during Hermes/watchdog cleanup — prefer `Stop-Process -Name` (e.g. `Hermes`, `hermes-watchdog`) with short-timeout HTTP probes (`curl.exe -m`) instead.
- Worktrees that cannot check out `main` publish with `git push origin HEAD:main`.
- For Hermes Agent with local llama.cpp, keep context length at least 65536 (operator target often ~100k); local GPU ops target RTX 5060 Ti 16GB with Turboquant-oriented start scripts under `scripts/windows/`. Dual-model hot-swap uses `scripts/windows/start-llama-hotswap.ps1` + `llama-hotswap-models.ini` (recent healthy lineup on `:8080` has included Qwen3.6-35B IQ3_M and Huihui-gemma-4-12B-agentic Q4_K_M as load/warm slots); do not assume nonexistent HF ids such as `NousResearch/Hermes-3-Llama-3.1-8B-GGUF:Q4_K_M`.

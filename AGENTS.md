...
- When a restart explicitly requests llama, use `restart-hermes-stack.ps1 -StartLlama` with existing Turboquant / RTX 5060 Ti scripts (e.g. `scripts/windows/start-hermes-llama-fallback-rtx5060ti.ps1`); wait until llama is warm/ready rather than treating a cold fallback start as success; do not invent new non-secret `HERMES_*` env vars.
...
- Desktop remote / Go-watchdog prewarm on `:9118` (or any managed port): `/api/status` alone is not enough. If the desktop-backend manifest token drifts from the live `hermes serve` session token, `/api/sessions` returns 401 and Desktop shows "Could not connect to Hermes gateway". Go watchdog probes gated auth (`testBackendAuth`) before publishing the manifest and replaces the port occupant on drift.

...
- When a restart explicitly requests llama, use `restart-hermes-stack.ps1 -StartLlama` with the llama hot-swap router when present (`scripts/windows/start-llama-hotswap.ps1` / `llama-hotswap-models.ini`) or existing Turboquant / RTX 5060 Ti scripts; wait until llama is warm/ready rather than treating a cold fallback start as success; do not invent new non-secret `HERMES_*` env vars.
...
- Desktop remote / Go-watchdog prewarm on `:9118` (or any managed port): `/api/status` alone is not enough, and LISTEN/TCP accept is not enough either — a wedged `hermes serve` can leave the port open while HTTP hangs at 0 bytes. If the desktop-backend manifest token drifts from the live `hermes serve` session token, `/api/sessions` returns 401 and Desktop shows "Could not connect to Hermes gateway". Go watchdog probes gated auth (`testBackendAuth`) before publishing the manifest and replaces the port occupant on drift. When `:9119` serve is HTTP-dead, a healthy dashboard on `:9120` can temporarily back Desktop via `HERMES_DESKTOP_REMOTE_URL` / `HERMES_DESKTOP_REMOTE_TOKEN`.
...
- For Hermes Agent with local llama.cpp, keep context length at least 65536 (operator target often ~100k); local GPU ops target RTX 5060 Ti 16GB with Turboquant-oriented start scripts under `scripts/windows/`. Dual-model hot-swap uses `scripts/windows/start-llama-hotswap.ps1` + `llama-hotswap-models.ini` (primary load-on-startup plus a warm-standby slot such as Huihui-gemma-4-12B-agentic Q4_K_M).

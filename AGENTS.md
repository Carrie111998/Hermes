...
## Required Scoped References

This root file intentionally keeps only the project intent and invariants that
every agent must receive. Detailed instructions live in `docs/agents/` and are
**normative extensions of this file, not optional background**. Before editing
or reviewing code, read every guide whose trigger matches the task:

| Trigger | Required guide |
|---|---|
| Any implementation, bug fix, refactor, test/CI change, or code review | [`docs/agents/testing-and-pitfalls.md`](docs/agents/testing-and-pitfalls.md) |
| Runtime architecture or work in `run_agent.py`, `agent/`, `model_tools.py`, `toolsets.py`, `cli.py`, `gateway/`, `apps/desktop/`, `ui-tui/`, `tui_gateway/`, plugins, skills, delegation, cron, kanban, or profiles | [`docs/agents/architecture-and-runtime.md`](docs/agents/architecture-and-runtime.md) |
| Adding or changing tools, dependencies, configuration, setup flows, environment variables, or extension surfaces | [`docs/agents/extensions-and-configuration.md`](docs/agents/extensions-and-configuration.md) |

Read all matching guides before deciding on an implementation. For Desktop
work, also read the scoped [`apps/desktop/AGENTS.md`](apps/desktop/AGENTS.md).

## Universal Implementation Invariants

These remain in the always-loaded root as a safety net; the scoped guides carry
the full rationale and examples.

- Preserve strict message-role alternation and a byte-stable system prompt for
  the life of a conversation. Never inject synthetic user messages mid-loop.
- Use `get_hermes_home()` for profile-scoped state and
  `display_hermes_home()` for user-facing paths. Never hardcode `~/.hermes`.
- Put behavioral settings in `config.yaml`; `.env` is for credentials only.
- Run Python tests through `scripts/run_tests.sh`, never direct `pytest`.
  Assert behavior and invariants, not source text, mutable catalog snapshots,
  or a faked host OS.
- For config propagation, security boundaries, remote backends, and
  file/network I/O, verify the real resolution path with real imports against a
  temporary `HERMES_HOME`; mocks alone are not sufficient evidence.
- Keep user-visible capability at the edges and session-scoped. Do not expose a
  new core model tool when an existing command, skill, gated tool, plugin, or
  MCP server is the correct lower-footprint surface.

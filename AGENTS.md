...
| Profiles, gateway invariants, or known pitfalls | `docs/development/repository-contracts.md` |
...
- Multiplex profile-scoped env reads fail closed. When a scoped value is
  missing, never borrow the default profile's value from `os.environ`.
...
- Tool schema paths use `display_hermes_home()`; persistent tool state uses
  `get_hermes_home()`.
- Surface capability is session-scoped. Gate GUI-only tools through the
  session platform/toolset, not `HERMES_DESKTOP` or a process-wide `check_fn`.
...
- **Stream-is-message adapters have one final.** Keep draft frames prefix-stable,
  mark interim sends, let the consumer declare the final, and reconcile by edit
  rather than a duplicate plain send.

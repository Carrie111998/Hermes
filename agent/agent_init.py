...
def init_agent(
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server", "agent_runtime"}:
    ...
    if agent.api_mode == "agent_runtime":
        # Whole-turn runtimes own provider authentication and model calls.
        # Keep the ordinary client slots explicit so no provider SDK is
        # constructed during AIAgent initialization.
        agent.api_key = ""
        agent.client = None

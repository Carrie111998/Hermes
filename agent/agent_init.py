...
def init_agent(
            ...
            if _routed_client is not None:
                ...
                if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                    ...
                    try:
                        ...
                        _fb_entries = normalize_fallback_entries(fallback_model)
                    except Exception:
                        _fb_entries = []
                    _fb_resolved = False
    ...
    agent._portable_memory_packet_enabled = True
    ...
    if not skip_memory or _memory_toolset_requested:
        try:
            ...
            agent._portable_memory_packet_enabled = mem_config.get("portable_memory_packet_enabled", True)
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        agent._memory_manager.configure_idle_sleep(
                            mem_config.get("sleep", {})
                        )
                    except Exception as _sleep_cfg_err:
                        _ra().logger.debug(
                            "Memory idle sleep config ignored: %s", _sleep_cfg_err
                        )
                    _ra().logger.info(
    ...
    if (
        ...
        and (agent.enabled_toolsets is None or "memory" in agent.enabled_toolsets)
    ):

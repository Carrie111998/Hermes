...
def nonnegative_token_count(value: Any) -> Optional[int]:
    """Return an exact non-negative token count, or ``None`` if malformed.

    Provider/token counters are integers.  Truncating fractional, negative,
    non-finite, or otherwise malformed values to zero manufactures measured
    evidence, so every producer and deserializer shares this fail-closed rail.
    """
...
# Backward-compatible private alias for the local construction paths.
_nonnegative_int = nonnegative_token_count
...
class UsageComponentReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        parsed_token_values = tuple(_nonnegative_int(item) for item in raw_token_values)
        invalid_token_value = any(
            raw is not None and parsed is None
            for raw, parsed in zip(raw_token_values, parsed_token_values)
        )
        (
            input_tokens,
            output_tokens,
        ...
        invalid_value = self.value is not None and value is None
        if (
            provenance is UsageProvenance.MEASURED
            and self.value is not None
            and _nonnegative_int(self.value) is None
        ):
            invalid_value = True
        ...
        # absence.
        if invalid_token_value or invalid_value:
            provenance = UsageProvenance.UNKNOWN
            reason = reason or "invalid_usage_value"
...
def aggregate_usage(
    ...
    for component in components:
        ...
        prior_index = seen.get(component.component_id)
        if prior_index is not None:
            if normalized[prior_index] != component:
                normalized[prior_index] = UsageComponentReceipt(
                    component_id=component.component_id,
                    source=component.source,
                    session_id=bound_session_id,
                    provenance=UsageProvenance.UNKNOWN,
                    authority=component.authority,
                    accepted_event_id=component.accepted_event_id,
                    reason="conflicting_duplicate_component",
                )
            continue
        seen[component.component_id] = len(normalized)
    ...
    *,
    ...
    if "components" in raw:
        ...
        if not isinstance(raw_components, (list, tuple)) or not raw_components:
            return (
                UsageComponentReceipt(
                    component_id=f"{default_component_id}:invalid-components",
                    source=default_source,
                    session_id=session_id,
                    provenance=UsageProvenance.UNKNOWN,
                    authority=default_authority,
                    accepted_event_id=default_accepted_event_id,
                    reason="malformed_usage_components",
                ),
            )
        components: list[UsageComponentReceipt] = []
        for index, item in enumerate(raw_components):
            if isinstance(item, Mapping):
                ...
                components.append(
                    UsageComponentReceipt(
                        component_id=f"{default_component_id}:{index}",
                        source=default_source,
                        session_id=session_id,
                        provenance=UsageProvenance.UNKNOWN,
                        authority=default_authority,
                        accepted_event_id=default_accepted_event_id,
                        reason="malformed_usage_component",
                    )
                )

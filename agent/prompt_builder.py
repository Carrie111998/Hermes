...
KANBAN_GUIDANCE = (
    ...
    "4. **Classify failure before blocking.** Use `kanban_block(kind=...)` only "
    "for a genuine human gate (`needs_input`), missing authority/credentials "
    "(`capability`), or a dependency you cannot act on (`dependency`). For a "
    "flaky provider/runtime failure, use `kind=\"transient\"`; the dispatcher "
    "will make a bounded recovery attempt with fresh evidence before any "
    "exhaustion block. For implementation findings, route to the responsible "
    "owner via the review/remediation flow instead of blocking.\n"
    ...
    "capability, dependency, or bounded transient recovery failures.\n"
    ...
    "- Do not complete a task you didn't actually finish. Classify the failure "
    "and use bounded transient recovery or remediation before blocking.\n"

...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        if _is_messaging:
            result = (
                "## Skills (mandatory)\n"
                "If a skill below clearly matches the user's request, load it "
                "with skill_view(name) and follow its instructions. Load only "
                "skills that are directly relevant to the current conversation "
                "\u2014 do not pre-load skills for tasks the user has not asked about. "
                ...
                "</available_skills>\n"
                "\n"
                "Only load a skill if it is clearly needed for the current request."
                + hidden_note
            )
        else:
            result = (

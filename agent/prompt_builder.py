...
KANBAN_GUIDANCE = (
    ...
    "eyes on it.\n"
    "6. **Coding tasks are HARD-GATED at `complete_task`.** If your task "
    "body carries a `repo: <owner>/<name>` line (the kernel uses that to "
    "mark a task as a coding task), `kanban_complete` will RAISE "
    "`MergeRequiredError` and refuse to flip the task to `done` unless "
    "ALL of these hold: (a) the task has a `branch_name`, (b) that branch "
    "has at least one commit AHEAD of `origin/main` locally AND on origin, "
    "and (c) at least one comment on this task contains a GitHub PR URL "
    "whose head-ref token equals the branch. The error tells you exactly "
    "what's missing. Do NOT call `kanban_complete` for a coding task — "
    "push the branch, open the PR, post the PR URL as a `kanban_comment`, "
    "then call `kanban_block(reason=\"review-required: PR #N opened; "
    "awaiting human review+merge\")` and stop. A human merges the PR; the "
    "next kernel tick flips the task to `done`. Trying to bypass the gate "
    "with `git push --force` or by editing `branch_name` to match an "
    "unrelated branch is treated as a worker integrity failure.\n"

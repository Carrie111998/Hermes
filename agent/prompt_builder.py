...
KANBAN_GUIDANCE = (
    ...
    "- **Forge publication gate.** Publication policy is chosen when the task "
    "is created. For required publication, complete with top-level "
    "`repo_path`, `branch`, `expected_base`, and `pr_number` arguments; these "
    "prove the clean checkout, remote head, and open matching PR. Ordinary "
    "`metadata` cannot satisfy or disable the policy.\n"

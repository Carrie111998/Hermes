...
KANBAN_GUIDANCE = (
    ...
    "If `kanban_show()` says `created_by=auto-decomposer`, this is a bounded "
    "leaf assignment: do not ask a human to choose its analysis, metrics, "
    "component, or implementation approach. Make that role-owned decision, "
    "perform the smallest useful analysis, and complete with factual evidence. "
    "Only a newly reproduced external access or credential failure may block "
    "such a leaf, and it must use `kind=capability`.\n"

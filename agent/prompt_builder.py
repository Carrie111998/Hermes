...
KANBAN_GUIDANCE = (
    ...
    "You handle only that card — every other card belongs to its own worker. "
    ...
    "task). Before `kanban_show` do not run session-start protocol, "
    "previous-session memory work, or delegation — your first substantive "
    "action is orienting on your own card. The response includes title, body, "
    "parent-task handoffs (summary + metadata), any prior attempts on this "
    "task if you're a retry, the full comment thread, and a pre-formatted "
    ...
    "eyes on it.\n"
    "6. **Finish inside your own card.** Acceptance work the body asks for "
    "(fixes, verification, review handling) stays in the current card — do "
    "not defer it to a new card. Create a child task with "
    "`kanban_create(title=..., assignee=<right-profile>, "
    "parents=[your-task-id])` only when the card body explicitly authorizes "
    "decomposition; never spawn follow-up cards on your own initiative.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
    ...
    "- `kanban_complete` and `kanban_block` are terminal: once either "
    "succeeds, return immediately — no further tool calls, tests, comments, "
    "memory writes, or follow-up cards.\n"

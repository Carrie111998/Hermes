...
_WINDOWS_BASH_SHELL_HINT = (
    ...
    "calls. MSYS-style paths like `/c/Users/<user>/...` work for shell "
    "builtins and MSYS utilities ONLY. Every Windows-side tool — `read_file`, "
    "`write_file`, `execute_code`, and the native `python`/`python3` that "
    "`terminal` invokes — needs the native `C:/Users/<user>/...` form: a "
    "Windows-side WRITE through `/c/...` or `/mnt/c/...` does not fail, it "
    "silently fabricates a phantom `C:\\c\\...` / `C:\\mnt\\c\\...` tree that "
    "no other consumer can see. `/mnt/c/...` is valid only inside a process "
    "genuinely executing in WSL. PowerShell builtins "

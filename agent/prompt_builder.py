...
_ORION_MD_NAMES = (".orion.md", "ORION.md")


def _find_orion_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.orion.md`` or ``ORION.md``.
    ...
    # could pick up a .orion.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ORION_MD_NAMES:
...
ORION_AGENT_HELP_GUIDANCE = (
    "You run on Orion Agent (by Orion). When the user needs help with "
    "Orion itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://your-orion-docs.example/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `orion-agent` skill with skill_view(name='orion-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.orion/kanban.db`. Your task id is in "
    "`$ORION_KANBAN_TASK`; your workspace is `$ORION_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ORION_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ORION_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ORION_KANBAN_BRANCH:-wt/$ORION_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ORION_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`orion profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `orion kanban <verb>` for board operations. Use "
...
#
# The orion-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to orion-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Orion YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Orion launches that runtime.\n\n"
        ...
        "run `orion computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Orion "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Orion floating over another application, so the
    user is rarely asking about Orion — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Orion "
        ...
        "rather than anything inside Orion. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Orion terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Orion desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.orion.send(\"prompt\") — or a "
        "data-orion-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the Orion WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Orion itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Orion runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Orion itself is running. The host OS, home, and cwd "
                f"of the Orion process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Orion "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Orion
    ...
    extra = (os.getenv("ORION_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_orion_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ORION_PLATFORM") or os.environ.get("ORION_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.orion/skills/`` directory.  External dirs
    ...
    # ORION_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_orion_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.orion/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_orion_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Orion Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `orion-agent` skill "
            "first. It has the actual commands (e.g. `orion config set …`, `orion tools`, "
            "`orion setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: orion setup, orion setup tools, orion setup terminal, orion status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from ORION_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the ORION_HOME ContextVar falls
    ...
    try:
        ...
        logger.debug("Could not ensure ORION_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_orion_home()
...
def _load_orion_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.orion.md / ORION.md — walk to git root."""
    orion_md_path = _find_orion_md(cwd_path)
    if not orion_md_path:
        return ""
    try:
        content = orion_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = orion_md_path.name
        try:
            rel = str(orion_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".orion.md", context_length=context_length,
            read_path=str(orion_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", orion_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .orion.md / ORION.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ORION_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Orion install/source
    ...
    # explicitly configured cwd is honored verbatim — the Orion tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Orion in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Orion install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_orion_md(cwd_path, context_length)
    ...
    # SOUL.md from ORION_HOME only — skip when already loaded as identity
    if not skip_soul:

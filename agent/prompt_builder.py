...
PLATFORM_HINTS = {
    ...
    "desktop": (
        ...
        "Diagrams draw themselves in the transcript: a ```mermaid fence "
        "renders as a real diagram (theme-aware, zoomable, copyable as PNG) "
        "and a ```svg fence renders as sanitised inline SVG. Reach for them "
        "whenever a flow, architecture, sequence, state machine, timeline, or "
        "relationship would land better drawn than described — you do not need "
        "a file, a tool, or a skill for it, and you should not write the "
        "diagram out as prose steps instead. "

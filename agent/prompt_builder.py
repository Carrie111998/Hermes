...
PLATFORM_HINTS = {
    ...
    "desktop": (
        ...
        "Fenced blocks are not all plain code — the transcript routes them: "
        "a ```mermaid fence always draws in place as a real diagram "
        "(theme-aware, zoomable, copyable as PNG); a small ```svg fence "
        "renders as sanitised inline SVG, while a large standalone graphic "
        "opens as an artifact card in the right rail instead; a ```html "
        "document, and any code fence past roughly 48 lines, promotes to an "
        "artifact card too — so a long file or a full page never drowns the "
        "conversation and you should not truncate it for fear that it will. "
        "Reach for mermaid whenever a flow, architecture, sequence, state "
        "machine, timeline, or relationship would land better drawn than "
        "described: it needs no file, no tool, and no skill, and writing the "
        "diagram out as prose steps instead is the wrong answer. "

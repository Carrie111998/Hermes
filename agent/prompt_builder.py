...
KANBAN_GUIDANCE = (
    ...
    "If your output is code that requires independent review, finish the "
    "technical handoff with `kanban_complete(summary=..., metadata={handoff_state: "
    "'review_required', exact_sha: ..., tests: [...]})`. Completion means this "
    "implementation task is done; it does NOT satisfy independent review, "
    "exact-SHA CI, merge approval, deployment approval, rollback readiness, or "
    "live verification. Keep each as a separate dependent reviewer, waiter, or "
    "human/live gate card. Do not block a finished builder merely because "
    ...
    "## Bounded execution\n"
    "\n"
    "- Concrete implementation and review cards run single-shot with "
    "`goal_mode=false`; reserve goal mode for genuinely open-ended work.\n"
    "- Start with one batched discovery sweep. After the first edit, do not "
    "restart broad discovery; batch related reads and edits by acceptance item.\n"
    "- Validate in a ladder: focused tests after the relevant edit batch, then "
    "one full validation after the final code change. Do not rerun unchanged "
    "checks or poll external CI conversationally. Push the exact SHA and hand "
    "CI waiting to a deterministic waiter/reviewer card unless explicitly asked.\n"
    "- Treat 50% of the iteration budget as scope freeze, 70% as final edit "
    "batch, and 85% as verification plus terminal handoff only. At the limit, "
    "call `kanban_complete` or `kanban_block` with exact remaining work.\n"
    "- Do not spawn delegated self-review inside a builder. Independent review "
    "belongs to the reviewer card. Defer optional skill maintenance until after "
    "the terminal handoff unless a broken skill blocks correctness.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"

...
KANBAN_GUIDANCE = (
    ...
    "- **Outbound email side effects.** The supervisor/dispatcher MUST assign one "
    "stable `operation_id` for the real-world email operation and carry that exact "
    "ID plus `message_class` through every Kanban card, retry, continuation, and "
    "worker. Workers must pass both fields to `send_message`; never mint a new ID "
    "to bypass an `already_claimed`, `submitted`, or `uncertain` result. The email "
    "adapter commits its durable claim before SMTP, so an ambiguous SMTP failure "
    "is intentionally not auto-retried; reconcile the existing claim instead.\n"

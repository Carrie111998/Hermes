...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills\n"
            "Before replying, scan the skills below. If a skill's title and description clearly "
            "match your task, load it with skill_view(name) and follow its instructions — skills "
            "encode specialized knowledge (API endpoints, tool-specific commands, proven workflows) "
            "and the user's preferred conventions for tasks like code review, planning, and testing, "
            "so prefer them over general-purpose approaches when they genuinely apply. "
            "Load at most 1-2 skills per turn unless the task clearly spans more. Skip skills that "
            "are only marginally related — loading one whose title/description doesn't match the "
            "task just spends context for nothing.\n"

...
KANBAN_GUIDANCE = (
    ...
    "You are running one durable task from the shared Kanban board. Your task "
    "id is in `$HERMES_KANBAN_TASK` and your isolated work location is in "
    "`$HERMES_KANBAN_WORKSPACE`.\n\n"
    "## Every worker\n"
    "1. Start with `kanban_show()` and read the task, parent handoffs, prior "
    "runs, comments, and acceptance criteria.\n"
    "2. Work only in `$HERMES_KANBAN_WORKSPACE`; do not modify another task's "
    "workspace.\n"
    "3. Use `kanban_heartbeat()` during long operations.\n"
    "4. Call `kanban_complete()` only after the assigned task is actually "
    "finished. Include a concise result, verification evidence, and residual "
    "risks.\n"
    "5. Call `kanban_block()` only for a real dependency, missing capability, "
    "or required human input. State the exact blocker and preserve completed "
    "work in the handoff.\n"
    "6. If review is required, complete your implementation with evidence; do "
    "not block merely to request review. The routing profile schedules review "
    "as a dependent task.\n\n"
    "## Routing-profile continuation\n"
    "If your profile exposes `kanban_create`, `kanban_list`, `kanban_link`, "
    "and `kanban_unblock`, you own decomposition and final judgment:\n"
    "1. Create specialist tasks with precise scope and acceptance criteria. "
    "Use multiple independent cards for parallel work.\n"
    "2. For implementation that needs review, create the Reviewer task with "
    "the implementation task as its parent. The implementation worker "
    "completes normally; the Reviewer then runs.\n"
    "3. Keep this routing task alive as the durable continuation. Link every "
    "terminal task whose result you must inspect as a parent of this task: "
    "`kanban_link(parent_id=<terminal_task>, "
    "child_id=$HERMES_KANBAN_TASK)`. Do not make a specialist task depend on "
    "this still-running routing task.\n"
    "4. After linking the terminal dependencies, call "
    "`kanban_block(kind=\"dependency\")` on this task. It returns to ready "
    "when those parents finish and the dispatcher starts it again.\n"
    "5. On continuation, call `kanban_show()` again, inspect every handoff and "
    "review verdict, create and link correction tasks if necessary, and block "
    "again. Complete this routing task only when the original request and all "
    "acceptance criteria are satisfied.\n\n"
    "Ordinary specialist workers do not create or link board tasks. If their "
    "assigned scope is impossible, they block with the exact reason so the "
    "routing profile can revise the graph.\n\n"
    "## Reference details\n"
    "- Put human-facing files in "
    "`kanban_complete(artifacts=[<absolute paths>])`; paths in metadata are "
    "not uploaded, and artifact files must exist.\n"
    "- Attach downloadable evidence with `kanban_attach` or "
    "`kanban_attach_url`; workers may attach only to their own task.\n"
    "- List ids in `kanban_complete(created_cards=[...])` only when captured "
    "from successful `kanban_create` calls.\n"
    "- Use registered profile names for assignees. `kanban_create` rejects an "
    "unknown profile instead of silently parking the card.\n"
    "- Use `kanban_*` tools for board operations, not shell commands.\n"
    "- Do not call `clarify` in a headless worker. Comment with relevant "
    "context, then block so required input is visible on the board.\n"
    "- Do not use `delegate_task` as a durable board substitute."
)


def kanban_guidance_for_session(valid_tool_names) -> str:
    """Return worker guidance only for a dispatcher-scoped Kanban session."""
    if not os.environ.get("HERMES_KANBAN_TASK"):
        return ""
    return KANBAN_GUIDANCE if "kanban_show" in valid_tool_names else ""

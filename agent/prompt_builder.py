...
KANBAN_GUIDANCE = (
    ...
    "6. **Close the checklists explicitly.** Every task body has `## Execution "
    "checklist` and `## Closeout criteria` sections using `- [ ]` / `- [x]` "
    "checkboxes. Before completing, check every criterion and pass "
    "`metadata.checklist_evidence` with `execution` and `closeout` lists, one "
    "non-empty evidence string per checked item. Missing, unchecked, malformed, "
    "or unsupported criteria keep the task open.\n"

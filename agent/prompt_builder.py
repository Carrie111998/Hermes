...
KANBAN_GUIDANCE = (
    ...
    "eyes on it. However, when the task body or dependency graph already "
    "names an explicit downstream audit or reviewer task, do not create a "
    "duplicate human review gate. After the required tests, receipts, "
    "manifests, and artifact checks pass, you must call `kanban_complete` "
    "so the downstream audit can run. A negative result is valid completed "
    "evidence, not a reason to block. Explicit task completion instructions "
    "override the generic review-required convention.\n"

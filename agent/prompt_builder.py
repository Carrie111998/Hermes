...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (targeted loading)\n"
            "Before replying, scan the skills below. Load a skill with `skill_view(name)` when "
            "it is directly relevant, the user names it, or the task depends on its specialized "
            "workflow, API, quality gate, or operating convention. Do not load skills for incidental "
            "keyword overlap, simple conversation, or a task already covered by the current context. "
            "Prefer the single most specific skill; load additional skills only when they provide "
            "distinct complementary requirements. This avoids unnecessary tool turns and context "
            "while preserving specialized procedures when they matter.\n"

...
KANBAN_GUIDANCE = (
    ...
    "For source discovery, stay inside that workspace: use `rg --files` "
    "with a narrow glob or `git ls-files` for a bounded filename search. Never run a recursive "
    "`find` from a home directory, project parent, or another broad root; it "
    "can consume the entire worker deadline without producing task evidence. "
    "After one successful `kanban_show`, reuse that context unless a lifecycle "
    "transition requires a fresh read.\n"

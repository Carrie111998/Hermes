...
KANBAN_GUIDANCE = (
    ...
    "you MUST call `kanban_heartbeat` at least once an hour** so humans see "
    "liveness and the dispatcher does not reclaim an apparently dead process. "
    "Heartbeats are liveness only: progress-stale recovery is based on durable "
    "evidenced progress receipts from real tool/file/board activity, not heartbeat "
    "age. A "
    ...
    "not trip unblock-loop detection. Routine review findings MUST use "
    "`kanban_request_changes` to atomically return work to the implementer; never "
    "represent them as `needs_input`. Reserve review-time blocking for a genuine "
    "human escalation, using `kanban_block(kind=\"authority\", ...)` for a decision "
    "only a human authority can make or `kanban_block(kind=\"integrity\", ...)` "
    "for a trust/integrity concern.\n"

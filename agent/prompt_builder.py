...
KANBAN_GUIDANCE = (
    ...
    "## Lifecycle\n"
    "\n"
    "0. **Your task is NOT running until the dispatcher spawns it.** "
    "A card created via ``kanban_create`` lands in ``ready`` or ``todo``. "
    "It stays there until the dispatcher picks it up, changes status to "
    "``running``, and injects ``HERMES_KANBAN_TASK`` into the worker's "
    "environment. "
    "Created/queued is not started. Track the actual lifecycle in events: "
    "``kanban_show`` returns an ``events`` array showing ``created`` \u2192 "
    "(``dispatch_routed`` or ``dispatch_exempted``) \u2192 ``claimed`` \u2192 "
    "``running`` \u2192 ``completed`` / ``blocked``. A card in ``ready`` with "
    "no ``claimed`` event has NOT been dispatched yet.\n"
    "\n"
    ...
    "### Dispatch decisions\n"
    "\n"
    "When creating a child task for a known worker profile (e.g. "
    "``worker-terra``), record the **dispatch decision** in "
    "``kanban_create(dispatch_decision={...})``. This creates an auditable "
    "event (``dispatch_routed`` or ``dispatch_exempted``) in the task's "
    "timeline, between the ``created`` event and any subsequent ``claimed``.\n"
    "\n"
    ...
    "The exemption vocabulary is bounded; valid keywords: ``tiny``, "
    "``requires_full_context``, ``security_critical``, "
    "``controller_judgment``, ``quality_escalation``, ``already_running``.\n"
    "* Route and exemption are mutually exclusive.\n"
    "* Omit ``dispatch_decision`` entirely for simple inline work that does "
    "not go through the board.\n"
    "\n"
    "Remember: a ``kanban_create`` call returns a ``task_id`` with "
    "``status='ready'`` (or ``'todo'`` if parents are pending). "
    "This means the task is **queued**, not running. "
    "It will stay in ``ready`` until the dispatcher's tick picks it up, "
    "transitions it to ``running``, and spawns the worker process. "
    "Do not conflate card creation with execution.\n"
    "\n"
    ...
    "- **Do not confuse card creation with execution.** A task created via "
    "``kanban_create`` starts in ``ready`` (queued), not ``running``. "
    "It is NOT being worked on until the dispatcher changes its status to "
    "``running``. Likewise, seeing a card in ``ready`` does NOT mean work "
    "has started — check the events list for a ``claimed`` event."
)

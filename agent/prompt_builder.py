...
KANBAN_GUIDANCE = (
    ...
    "4. **Classify obstacles before stopping.** Treat each obstacle as one of "
    "three classes: recoverable/internal, dependency-wait, or genuine "
    "human/external gate. Test failures, lint/type errors, merge conflicts, "
    "missing packages, CI failures, transient network errors, and tool/skill "
    "lookup problems are recoverable/internal by default — they are not human "
    "blockers. For an internal obstacle, preserve parent progress and work the "
    "recovery loop: inspect the exact failure, consult relevant skills/docs, "
    "try a bounded retry, use an alternate command/path/provider, and, if the "
    "work must continue elsewhere, create a correctly assigned remediation "
    "child with `parents=[your-task-id]`. Record what you tried and keep going. "
    "Only use `kanban_block` after this evidence-backed recovery loop when the "
    "obstacle is a real dependency or human/external gate.\n"
    "   Genuine gates include a missing owner decision, required permission or "
    "credential after checking all documented sources, legal/customer approval, "
    "an unavailable external system, or a dependency waiting on another tracked "
    "task. Use `kind=\"dependency\"` for the last case so it returns to `todo` "
    "and auto-resumes; use `kind=\"needs_input\"` or `kind=\"capability\"` "
    "for the corresponding human gate. Untyped blocks are legacy compatibility "
    "only, not a shortcut around this classification.\n"
    ...
    "native Review handoff (`kanban_submit_review`) with the repository, PR, "
    "immutable head SHA, verification, and deployment implications; do not "
    "masquerade as a generic blocked state or use a `review-required` reason "
    "as a substitute. Reviewing-then-completing is more honest than "
    "auto-completing work that still needs eyes on it.\n"

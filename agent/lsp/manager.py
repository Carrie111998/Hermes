...
class LSPService:
    ...
    async def _reap_idle_clients_async(self, now: Optional[float] = None) -> int:
        ...
        The background reaper calls this through ``_reap_idle_once``.  The
        synchronous ``reap_idle_clients`` safety valve and the next
        ``_get_or_spawn`` call also use it as a cheap checkpoint so stale
        clients can be cleared without waiting for the next loop interval.
        """
        if not self._enabled:
            return 0
        ...
        if not idle:
            return 0
        eventlog.log_reaped(
            [(client.server_id, client.workspace_root) for _key, client in idle],
            self._idle_timeout,
        )

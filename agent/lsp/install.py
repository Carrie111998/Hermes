...
appropriate.  All installs go to a Orion-owned bin staging dir,
``<ORION_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``orion lsp
  status``.
...
from orion_constants import find_node_executable
...
# ``<ORION_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `orion lsp status` reports the host's presence.
...
def orion_lsp_bin_dir() -> Path:
    """Return the Orion-owned bin staging dir for LSP servers."""
    from orion_constants import get_orion_home

    p = get_orion_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(orion_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $ORION_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Orion installed and
    ...
    if npm is None:
        ...
        return None
    staging = orion_lsp_bin_dir().parent  # <ORION_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a orion-owned target dir.
    ...
    console script.
    """
    pip_target = orion_lsp_bin_dir().parent / "python-packages"
    pip_target.mkdir(parents=True, exist_ok=True)
    try:
        logger.info("[install] pip install --target %s %s", pip_target, pkg)
        from orion_cli.tools_config import _pip_install
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = orion_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``orion lsp status`` CLI to give users a quick

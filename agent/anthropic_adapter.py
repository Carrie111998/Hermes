"""Anthropic Messages API adapter for Orion Agent.

Translates between Orion's internal OpenAI-style message format and
...
try:
    import orion_cli as _orion_cli

    _ORION_VERSION = str(_orion_cli.__version__)
except Exception:
    _ORION_VERSION = "0.0.0"
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in orion_cli/runtime_provider.py.
    """
...
# Orion effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_nous_portal_endpoint(base_url: str | None) -> bool:
    ...
    if base_url_host_matches(base_url or "", "inference-api.nousresearch.com"):
        return True
    try:
        from orion_cli.auth import _nous_inference_env_override
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to orion's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to orion's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + OrionAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://your-orion-docs.example")
        headers.setdefault("X-Title", "Orion Agent")
        headers.setdefault("User-Agent", f"OrionAgent/{_ORION_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Orion loads that into the process env from
    # ``~/.orion/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Orion notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Orion historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``orion models``) never mutates ``~/.orion/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Orion)
      ...
      5. Anthropic credential_pool OAuth entry (~/.orion/auth.json)
    ...
    # 1. Orion-managed OAuth/setup token env var
    ...
    # 5. Orion credential_pool OAuth entry.
...
# ── Orion-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.orion/.anthropic_oauth.json (our own file).
...
def _get_orion_oauth_file() -> Path:
    return get_orion_home() / ".anthropic_oauth.json"
...
def run_orion_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Orion-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Orion with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from orion_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_orion_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Orion-managed OAuth credentials from ~/.orion/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Orion OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See orion-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See orion-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See orion-agent#13848 (Kimi) and
    orion-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Orion Agent", "Claude Code")
                text = text.replace("Orion agent", "Claude Code")
                text = text.replace("orion-agent", "claude-code")
                text = text.replace("Orion", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Orion-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Orion surfaces in its CLI. We

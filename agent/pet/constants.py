...
row taxonomy is inferred from the atlas shape so Orion can render both legacy
...
# rows (1536x1872), while older Orion/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Orion' activity state names. They are not always identical to the
...
# Legacy Orion/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Orion activity names -> accepted row-name aliases in descending

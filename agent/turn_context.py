...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``orion logs``.
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # Orion only initiates compaction in "orion" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Orion preflight compression for codex app-server "
                "(mode=%s); Orion will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Orion (via _emit_status), so it always

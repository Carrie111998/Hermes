...
# ── Error-predicate leaf family (extracted to agent/auxiliary_client_errors.py) ──
# R2 slice, epic #78647 / #78635: the "recently 402'd" unhealthy-provider cache,
# transient-retry constants, and the error-classification predicates below now
# live in agent/auxiliary_client_errors.py and are re-exported here so every
# existing import / lazy-import / monkeypatch target keeps resolving to the
# same objects (seam identity: getattr identity holds per name).
from agent.auxiliary_client_errors import (

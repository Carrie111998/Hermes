    ...
    if pool_present:
        ...
        if or_key:
            ...
            return _create_openai_client(api_key=or_key, base_url=base_url,
                           default_headers=_headers_with_config(build_or_headers())), or_model
...
def set_runtime_main(
    ...
    *,
    ...
    global _RUNTIME_MAIN_DEFAULT_HEADERS, _RUNTIME_MAIN_AUTH_MODE, _RUNTIME_MAIN_COMPAT_SNAPSHOT
    runtime = {
        ...
        "default_headers": default_headers if isinstance(default_headers, dict) and default_headers else None,
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    _xai_default_headers = None
    from tools.xai_http import hermes_xai_default_headers

    _xai_default_headers = hermes_xai_default_headers()
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        **({"default_headers": merge_default_headers(_xai_headers, _xai_default_headers)}
           if _xai_headers or _xai_default_headers else {}),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _build_codex_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=codex_token,
        base_url=base_url,
        default_headers=_headers_with_config(
            _codex_cloudflare_headers(codex_token, base_url=base_url)
        ),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _resolve_auto(
    ...
    route_result = _resolve_auto_route(main_runtime=main_runtime, task=task)
    if len(route_result) == 2:
        client, model = route_result
    else:
        client, model, _provider = route_result
    return client, model
...
def resolve_provider_client(
    ...
    if provider == "auto":
        route_result = _resolve_auto_route(
            main_runtime=main_runtime,
            task=task,
        )
        if len(route_result) == 2:
            client, resolved = route_result
            effective_provider = None
        else:
            client, resolved, effective_provider = route_result
        if client is None:
            return None, None
...
def _runtime_cache_discriminator(field: str, value: Any) -> Any:
    ...
    if field == "default_headers" and isinstance(value, dict):
        return tuple(sorted(
            (str(k), str(v)) for k, v in value.items() if k and v
        )) if value else None
    return value

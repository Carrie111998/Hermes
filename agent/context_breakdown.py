...
def compute_session_context_breakdown(
    ...
    compression_threshold_tokens = int(getattr(comp, "threshold_tokens", 0) or 0) if comp else 0
    compression_threshold_percent = (
        max(0, min(100, round(compression_threshold_tokens / context_max * 100)))
        if context_max and compression_threshold_tokens
        else 0
    )

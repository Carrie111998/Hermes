...
def _close_failed_switch_clients(agent, state):
    ...
    if (
        ...
        try:
            # #70773: never hard-close a replaced shared client during
            # rollback — worker threads from stale-killed streaming attempts
            # may still be unwinding their SSL BIOs on the old pool. Retire
            # the sockets (FD-safe from any thread) and defer FD release to GC.
            agent._retire_shared_openai_client(
                current_client, reason="switch_model_rollback"
            )
            closed_ids.add(id(current_client))
        except Exception:
            logger.debug("failed to retire shared OpenAI client during switch rollback", exc_info=True)
...
def _close_committed_switch_resources(agent, state):
    ...
    if (
        ...
        try:
            # #70773: never hard-close the replaced shared client from here.
            # The turn thread may not be the thread whose request is still
            # unwinding on the old pool. Retire the sockets (FD-safe) and
            # defer the FD release to GC.
            agent._retire_shared_openai_client(
                old_client, reason="switch_model_commit"
            )
            closed_ids.add(id(old_client))
        except Exception:
            logger.debug("failed to retire replaced shared OpenAI client", exc_info=True)

...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Orion-Session-Id header), and an
        ...
        if env_var_enabled("ORION_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from orion_cli.config import load_config as _load_moa_cfg
    ...
    # canonicalized only after Orion sends the request. In that case model
    ...
    if is_anthropic_wire or _litellm_openai_wire:
        try:
            from orion_cli.config import get_custom_provider_model_capability
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to orion's outer conversation loop,
    ...
    if agent.provider == "opencode-free":
        from orion_cli.models import opencode_zen_free_headers
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    try:
        ...
        from orion_cli.config import load_config as _sm_load_config
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: orion' own dispatch loop intentionally keeps an
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    Orion' keepalive client (#10324 / ``_build_keepalive_http_client``) and

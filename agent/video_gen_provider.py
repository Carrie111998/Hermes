...
as ``kind: backend``) or ``~/.orion/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``orion tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ORION_HOME/cache/videos/``, creating parents as needed."""
    from orion_constants import get_orion_home

    path = get_orion_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ORION_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$ORION_HOME/cache/videos/``.

"""Upload a Orion session transcript to Hugging Face as an agent trace.

Orion stores sessions in its own SQLite store (``orion_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``orion trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Orion' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "orion-traces"
_ORION_VERSION = "orion-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Orion OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Orion' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Orion conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.orion/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `orion trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/orion-traces`` dataset. Returns a

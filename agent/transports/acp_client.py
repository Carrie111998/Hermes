...
class ACPClient:
    ...
    def __init__(
        ...
        # Route through the centralized env-sanitization helper so Tier-1
        # secrets (gateway bot tokens, GitHub auth, infra tokens) are always
        # stripped while provider credentials still flow through. This mirrors
        # codex_app_server.py (#29157) — the ACP agent subprocess is a
        # model-driving CLI that legitimately needs LLM provider creds but has
        # no use for Hermes internal secrets.

...
Owns one Codex thread per Orion session. Drives `turn/start`, consumes
...
# Orion' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ORION_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Orion/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Orion session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ORION_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ORION_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="orion",
            client_title="Orion Agent",
            client_version=_get_orion_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Orion' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Orion' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own orion-tools callback we
            # auto-accept — the user already approved Orion' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Orion
        approval-mode/timeout logic. The Orion-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Orion approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Orion approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Orion returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Orion approval mode/timeout resolution
    ...
    # "prompt expired"; the Orion-side messaging already distinguishes them.
    return "decline"
...
def _get_orion_version() -> str:
    """Best-effort Orion version string for codex's userAgent line."""
    try:

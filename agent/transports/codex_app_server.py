...
projection into Orion' display, approval bridging, transcript projection into
...
"codex_app_server"`. Orion' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Orion secret — gateway
        ...
        spawn_env = orion_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ORION_KANBAN_TASK"):
            kanban_db = spawn_env.get("ORION_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ORION_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ORION_HOME", os.path.expanduser("~/.orion")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "orion",
        client_title: str = "Orion Agent",

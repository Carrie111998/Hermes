...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        from toolsets import _ORION_CORE_TOOLS

        _core_tool_names = set(_ORION_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Orion expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile ORION_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped ORION_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``orion_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_orion_home()`` themselves.
        """
        if "orion_home" not in kwargs:
            from orion_constants import get_orion_home
            kwargs["orion_home"] = str(get_orion_home())
        for provider in self._providers:
            try:

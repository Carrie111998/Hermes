"""OpenAI-shape bridge for the ACP clients.

ACP agents (`copilot --acp`, `junie --acp=true`, …) speak the Agent Client
Protocol, which has no OpenAI-style ``tools``/``tool_calls`` channel: an ACP
prompt is text, and an ACP response is text plus the agent's *own* tool
notifications. Hermes' agentic surface — ``memory``, ``todo``,
``skill_manage`` and friends — is dispatched from OpenAI-shaped ``tool_calls``,
so on an ACP provider it can only work if the schemas travel *into* the prompt
as text and the calls are parsed back *out* of the response text.

This module owns both halves of that bridge, plus the streaming shape, so the
ACP clients share one implementation:
...
Both ``agent/copilot_acp_client.py`` and ``agent/junie_acp_client.py`` build on
this; they differ only in *which* tools they forward (Copilot: everything Hermes
offers; Junie: an agent-level allowlist, because Junie runs its own
read/edit/execute tools and re-offering those would make Hermes re-run finished
work).
"""
...
class StreamChunks(list):
    ...
    them on the ``stream=True`` path, so the ACP clients return this instead and
    copy the extras onto it.
    """
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that's how the Junie path forwards just the
    agent-level tools instead of Hermes' whole toolset.
    """

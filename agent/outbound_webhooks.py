...
(``gateway/platforms/webhook.py``): inbound wakes Orion when the world
changes; outbound tells the world when Orion does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Orion-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``ORION_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.orion/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/orion-events
          events: [on_session_end, subagent_stop]
          # secret literal (discouraged) or env var name (preferred):
          secret_env: ORION_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `orion hooks list`
...
Headers::
    ...
    User-Agent:              Orion-Agent-Outbound-Webhook
    X-Orion-Event:          <hook event name>
    X-Orion-Delivery:       <delivery_id>
    X-Orion-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("ORION_SAFE_MODE"):
        logger.info("ORION_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _parse_single_target(index: int, raw: Any) -> Optional[WebhookTarget]:
    from orion_cli.plugins import VALID_HOOKS
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Orion-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Orion-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,

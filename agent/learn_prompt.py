...
def build_learn_prompt(user_request: str) -> str:
    ...
    return (
        ...
        "1c. Before authoring, inspect the existing skill index with `skills_list()` "
        "and load the closest candidates with `skill_view(name=...)`. Prefer "
        "patching an existing skill that owns the workflow over creating a new "
        "sibling. Create a new skill only when its boundary is genuinely distinct. "
        "For an intentional companion, pass `allow_overlap=true` with a concise "
        "`overlap_reason` describing that boundary.\n"

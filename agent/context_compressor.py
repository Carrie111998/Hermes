...
Update the summary using this exact structure. PRESERVE all existing information that is still relevant. ADD new completed actions to the numbered list (continue numbering). Move answered questions to "Resolved Questions". Update "Active State" to reflect current state. Remove information only if it is clearly obsolete. CRITICAL: keep "## Goal" pointed at the user's most recent unfulfilled input — including any question, decision request, or discussion turn the assistant has not yet answered — and record whatever is still in flight under "## Active State". Only treat the goal as finished if the last exchange was fully resolved.

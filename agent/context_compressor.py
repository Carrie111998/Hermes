...
class ContextCompressor(ContextEngine):
    ...
    def should_compress_info(
        ...
        * ``"ineffective"`` — anti-thrashing has backed off because the last
        ...
        ``compress()`` directly and is never deferred.  A deferral returns
        ``(False, None)`` — unlike a cooldown or anti-thrash block it is
        transient and self-resolving (bounded by the ceiling and the
        consecutive-check cap), so it must not trigger the callers'
        stuck-over-threshold warning.
        """

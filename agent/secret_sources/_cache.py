...
cache problem must never block Orion startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Orion home used for cache paths.

    ``home_path`` is whatever ``load_orion_dotenv()`` already resolved;
    falling back to ``$ORION_HOME`` / ``~/.orion`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from orion_constants import get_orion_home

        home_path = get_orion_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<orion_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ORION_HOME is ever made traversable.
            try:

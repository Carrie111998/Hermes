...
def _call_tool_callback(callback, positional: tuple, **metadata) -> None:
    ...
    if positional_names:
        values = {
            ...
            if key not in positional_names
        }
...
def _persisted_tool_activity(metadata: dict[str, object]) -> dict[str, object] | None:
    """Keep a strictly sanitized replay sidecar only for opted-in fields."""
...
def _is_guardrail_block_result(result: Any) -> bool:
    ...
    try:
        ...
        return False
    return isinstance(data, dict) and isinstance(data.get("guardrail"), dict)


def _presentation_allowed_for_result(blocked: bool, result: Any) -> bool:
    return not blocked or not _is_guardrail_block_result(result)


def _emit_tool_start_presentation(
    agent,
    *,
    function_name: str,
    ...
    if agent.tool_progress_callback:
        try:
            ...
            _call_tool_callback(
                ...
                **_start_metadata(agent, tool_call_id),
            )
...
def _finalize_cancelled_tool_activity(
    ...
    *,
    ...
    presentation_result = _presentation_completion_value(completion_metadata)
    ...
    if agent.tool_complete_callback:
        try:
            ...
            _call_tool_callback(
                ...
                (tool_call_id, function_name, display_args, presentation_result),
                **completion_metadata,
            )
        except Exception as cb_err:
    ...
    *,
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            ...
            cancel_state = getattr(agent, "_active_tool_cancel_state", None)
            if isinstance(cancel_state, dict):
                cancel_state["function_args"] = final_args
                cancel_state["middleware_trace"] = list(state["middleware_trace"])
    ...
    *,
    ...
    if not agent.quiet_mode and getattr(agent, "tool_progress_mode", "all") != "off":
        ...
        if agent.verbose_logging:
            ...
            print(
                f"  📞 {prefix}: {function_name}({list(display_args.keys())}) - "
                f"{args_preview}"
            )
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for tool_call in tool_calls:
        ...
        if _tool_reason_enabled(agent, function_name):
            ...
            agent._tool_call_reasons[tool_call.id] = extract_tool_reasoning(
                function_args
            )
    ...
    cancelled_indices_lock = threading.Lock()
    ...
    for i, (tc, name, args, middleware_trace, _parse_error, _scope_block) in enumerate(
        ...
        if i in timed_out_indices and r is None:
            ...
            if blocked and _parse_error is None:
                effect_disposition = "none"
        ...
        presentation_allowed = (
            _parse_error is None
            and _presentation_allowed_for_result(blocked, function_result)
        )
        ...
        tool_message = make_tool_result_message(
            ...
            tool_activity=(
                ...
                if _presentation_allowed_for_result(blocked, display_function_result)
                else None
            ),
        )
        ...
        if presentation_allowed and agent.tool_progress_callback:
            try:
                _call_tool_callback(
                    agent.tool_progress_callback,
                    (
        ...
        if presentation_allowed and agent.tool_complete_callback:
            try:
                display_args = _agent_activity_display_args(agent, name, args)
                _call_tool_callback(
                    agent.tool_complete_callback,
                    (tc.id, name, display_args, presentation_result),
                    **completion_metadata,
                )
            except Exception as cb_err:
                logging.debug(f"Tool complete callback error: {cb_err}")
...
def execute_tool_calls_sequential(
    ...
    *,
    ...
    previous_cancel_state = getattr(agent, "_active_tool_cancel_state", None)
    agent._active_tool_cancel_state = cancel_state
    try:
        ...
        if cancel_state and not cancel_state.get("completion_finalized", False):
            function_result = cancel_state.get("function_result")
            ...
            cancel_state["completion_finalized"] = True
            try:
                agent.interrupt("keyboard interrupt")
            except Exception:
                pass
        if cancel_state:
            _clear_tool_reason(agent, cancel_state.get("tool_call_id", ""))
        raise
    finally:
        if previous_cancel_state is None:
            try:
                delattr(agent, "_active_tool_cancel_state")
            except AttributeError:
                pass
        else:
            agent._active_tool_cancel_state = previous_cancel_state
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if function_name == "todo":
            ...
            try:
                ...
                _spinner_result = function_result
                cancel_state["function_result"] = function_result
                cancel_state["terminal_emitted"] = True
                try:
                    agent.interrupt("keyboard interrupt")
                except Exception:
        ...
        # Registry-dispatched tools emitted their post hook inside
        # handle_function_call; inline/runtime tools emitted it above; blocked
        # calls were closed by the middleware. A later presentation callback
        # interrupt must not re-close the tool as cancelled.
        cancel_state["terminal_emitted"] = True
        ...
        presentation_result = _presentation_completion_value(completion_metadata)
        presentation_allowed = _presentation_allowed_for_result(
            _execution_blocked,
            function_result,
        )
        ...
        if presentation_allowed and agent.tool_complete_callback:
            try:
                display_args = _agent_activity_display_args(
                    agent,
                    function_name,
                    function_args,
                )

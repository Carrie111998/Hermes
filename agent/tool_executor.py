...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if _block_msg is not None:
        ...
        elif function_name == "suggest_actions":
            def _execute(next_args: dict) -> Any:
                from tools.suggest_actions_tool import suggest_actions_tool as _suggest_actions_tool
                return _suggest_actions_tool(
                    message=next_args.get("message", ""),
                    actions=next_args.get("actions"),
                    callback=getattr(agent, "suggest_actions_callback", None),
                )
            function_result, function_args = _run_agent_tool_execution_middleware(
                agent,
                function_name=function_name,
            ...
            tool_duration = time.time() - tool_start_time
            if agent._should_emit_quiet_tool_messages():
                agent._vprint(f"  {_get_cute_tool_message_impl('suggest_actions', function_args, tool_duration, result=function_result)}")

...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        ...
        # the session is planning / awaiting approval. A guard that raises must
        # NOT allow the tool — _plan_mode_block_reason fails closed for mutating
        # tools. Both executors route through this middleware, so the guard
        # lives at the single chokepoint. See hermes_cli/plan_mode.
        if block_message is None:
            _plan_block = _plan_mode_block_reason(
                agent, function_name, final_args, effective_task_id
            )
            if _plan_block is not None:
                block_message = _plan_block
                block_error_type = "plan_mode_block"

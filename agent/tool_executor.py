    ...
    for i, (tc, name, args, middleware_trace, _parse_error, _scope_block) in enumerate(
        ...
        if i in timed_out_indices and r is None:
            ...
            if is_error:
                ...
                result_preview = sanitize_tool_result_for_sink(_err_text[:200] if len(_err_text) > 200 else _err_text)
...
def execute_tool_calls_sequential(
    ...
    api_call_count: int = 0, *, finalize: bool = True
) -> None:
    """Compatibility wrapper for the extracted sequential execution shard."""
    from tools.tool_execution_sequential import execute_tool_calls_sequential as _impl
...
def execute_tool_calls_segmented(
    agent, assistant_message, messages: list, effective_task_id: str,
    api_call_count: int = 0, segments=None
) -> None:
    """Compatibility wrapper for the extracted segmented dispatcher."""
    from tools.tool_execution_sequential import execute_tool_calls_segmented as _impl

    return _impl(
        agent, assistant_message, messages, effective_task_id,
        api_call_count, segments=segments
    )

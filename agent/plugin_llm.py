...
Plugins built on Orion Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Orion
...
:class:`~orion_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Orion' main config
...
handles every provider, fallback chain, and per-task override Orion
supports.
"""
...
def _resolve_task_ownership(plugin_id: str) -> tuple[frozenset, frozenset]:
    ...
    builtin: set = set()
    try:
        from orion_cli.plugins import get_plugin_auxiliary_tasks
...
class PluginLlm:
    ...
    Instances are constructed by :class:`orion_cli.plugins.PluginContext`

...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Orion with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.orion/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Orion
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``orion tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``orion setup`` for

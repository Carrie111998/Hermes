...
CURATOR_REVIEW_PROMPT = (
    ...
    "      Re-home it with the LEDGERED tool surface, never with terminal "
    "`mv`/`cp`: `skill_manage action=write_file` on the umbrella to place "
    "the content (subdirectories are created for you), then "
    "`skill_manage action=remove_file` on the source to drop the original, "
    "then archive the old sibling with `skill_manage action=delete`. A "
    "terminal move writes the same bytes with NO ledger entry, so the "
    ...
    "Terminal writes under the skills tree are refused in this pass.\n\n"
    ...
    "  - terminal                       — READ-ONLY over the skills tree "
    "(cat, ls, find, grep). Every write under a skills root — mv, cp, rm, "
    "mkdir, tee, sed -i, shell redirect — is REFUSED in this pass, "
    "including for bundled, hub-installed, and external-dir skills: those "
    "writes leave no ledger entry and cannot be rolled back. Re-home "
    "content with skill_manage write_file/remove_file instead\n\n"

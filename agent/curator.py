...
# The LLM review fork — prompt text, provider/model resolution, and the
# forked-AIAgent execution itself — lives in its own bounded module rather
# than growing this orchestrator further. Re-exported here so every existing
# `curator.X` / `from agent.curator import X` reference (including test
# monkeypatches of `_run_llm_review`) keeps working unchanged.
from agent.curator_review_fork import (  # noqa: F401

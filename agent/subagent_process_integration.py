...
def strict_worker_runtime_mounts() -> tuple[Any, Any]:
    ...
    return (
        RuntimeMount(source=python_prefix, target=python_prefix),
        RuntimeMount(source=repository_root, target=repository_root),
    )
...
def _tool_calls(normalized: Any) -> list[dict[str, Any]]:
    ...
    for call in getattr(normalized, "tool_calls", None) or []:
        ...
        if not isinstance(call_id, str) or not call_id or call_id in seen:
            raise ProcessIntegrationError("model emitted missing or duplicate tool call id")
        ...
        if isinstance(arguments, str):
            try:
                ...
                raise ProcessIntegrationError("model emitted invalid tool arguments") from exc
...
class ParentBrokerAdapter:
    ...
    def __init__(self, *, broker: SubagentBroker, child: Any, profile: Any, task: str) -> None:
        self.broker = broker
        self.child = child
    ...
    def _dispatch(self, operation: str, body: Any) -> Mapping[str, Any]:
        ...
        if operation == "tool.execute":
            ...
            if not isinstance(name, str) or not isinstance(args, Mapping):
                raise ProcessIntegrationError("tool.execute name/arguments are malformed")

...
_MEMORY_REVIEW_PROMPT = (
    ...
    "injecting into every future prompt. Be selective.\n\n"
    "Save to USER memory only stable identity, global preferences, or recurring "
    "expectations about how the user wants the agent to communicate. Save to "
    "MEMORY only stable environment facts or cross-project conventions that are "
    "expensive to rediscover.\n\n"
    "Do not save completed work, mutable project state, task progress, artifact "
    "IDs, dated outcomes, or session summaries. Those belong in session history "
    "or the project's own state/docs. Procedures belong in skills, not memory.\n\n"
    "If a qualifying fact stands out, save it using the memory tool. Otherwise "
    "say 'Nothing to save.' and stop."
)

_SKILL_REVIEW_PROMPT = (
    "Review the conversation above for a durable procedural lesson. Be "
    "selective: 'Nothing to save.' is the default outcome for routine, "
    "successful work with no correction, reusable technique, or missing skill "
    "instruction. Never manufacture a skill update merely because a turn "
    "completed.\n\n"
    "Target shape of the library: CLASS-LEVEL skills, each with a rich "
    "SKILL.md and a `references/` directory for session-specific detail. "
    "Not a long flat list of narrow one-session-one-skill entries. Apply this "
    "shape only after a qualifying durable signal exists.\n\n"
    "Route corrections by scope before acting. A global user preference about "
    "tone, format, verbosity, or communication belongs in USER memory. A "
    "task-class workflow correction belongs in the skill governing that class "
    "of task. Do not duplicate a preference into a skill unless it changes the "
    "procedure for that task class.\n\n"
    "Signals that warrant a skill action:\n"
    "  • The user corrected a reusable workflow, approach, or sequence of "
    "steps for a class of tasks. Encode it as a pitfall or explicit step.\n"
    ...
    "Keep the boundary strict: memory captures stable user/environment facts; "
    "skills capture reusable procedures. Project state, completed work, and "
    "session outcomes belong in project files or session history.\n\n"
    ...
    "If no durable procedural signal exists, say 'Nothing to save.' and stop."
)

_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above and classify each durable candidate before "
    "using a tool. Most routine turns should produce no write. Route information "
    "to exactly one primary home:\n"
    "  • USER memory: stable identity and global user preferences.\n"
    "  • MEMORY: stable environment facts and cross-project conventions.\n"
    "  • Skill: reusable task-class procedures, corrections, and verified "
    "workarounds.\n"
    "  • Project state/docs: mutable plans, decisions, task progress, and "
    "artifact IDs.\n"
    "  • Session history: completed work, dated outcomes, and conversational "
    "context that can be recalled on demand.\n\n"
    "Do not duplicate the same preference or fact across homes. If no durable "
    "signal exists, say 'Nothing to save.' and stop.\n\n"
    "**Memory**: save only the qualifying USER or MEMORY facts above with the "
    "memory tool.\n\n"
    "**Skills**: update only when a reusable task-class lesson exists.\n\n"
    "Target shape of the skill library: CLASS-LEVEL skills with a rich "
    "SKILL.md and a `references/` directory for session-specific detail. "
    "Not a long flat list of narrow one-session-one-skill entries.\n\n"
    "Signals that warrant a skill update:\n"
    "  • The user corrected a reusable task-class workflow or approach. A "
    "global user style or communication preference belongs in USER memory, "
    "not automatically in a skill.\n"
    ...
    "(2), or (3).\n\n"
    "Do not copy a global preference into a skill unless it changes the "
    "procedure for that task class.\n\n"
    ...
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "Act only on a qualifying durable signal. Otherwise say "
    "'Nothing to save.' and stop."
)

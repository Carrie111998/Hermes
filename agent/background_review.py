...
_SKILL_REVIEW_PROMPT = (
    ...
    "WRITE HYGIENE — follow these rules on every skill update:\n"
    "  • READ BEFORE WRITE: call skill_view for the exact file before "
    "patching or editing it. Never write from memory.\n"
    "  • NO DUPLICATES: if the concept already exists under any wording, "
    "modify the existing entry instead of adding another copy.\n"
    "  • PREFER patch OVER edit: use a targeted find-and-replace unless "
    "a full section genuinely needs restructuring.\n"
    "  • NO DATE STAMPS: do not add 'Updated YYYY-MM-DD', "
    "'NEW — Session YYYY-MM-DD', HTML added-date comments, or other "
    "changelog markers. Skills are timeless guidelines.\n"
    "  • DELETE DEPRECATED content instead of retaining it under a "
    "DEPRECATED label.\n\n"
    ...
    "'Nothing to save.' is a valid and correct outcome when the session "
    "ran smoothly with no corrections and produced no new technique. "
    "Use it freely when none of the signals above fired."
)
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "there is a clear, concrete signal that genuinely new knowledge "
    "exists. Quality over quantity — a session with no signal should "
    "produce no skill update.\n\n"
    ...
    "  • READ BEFORE WRITE: call skill_view for the exact file before "
    "patching or editing it. Never write from memory.\n"
    "  • NO DUPLICATES: modify an existing entry instead of adding a "
    "semantically duplicate copy.\n"
    "  • PREFER patch OVER edit for targeted changes; use a full rewrite "
    "only when a section genuinely needs restructuring.\n"
    "  • NO DATE STAMPS or changelog markers in SKILL.md. Skills are "
    "timeless guidelines.\n"
    "  • DELETE DEPRECATED content instead of retaining dead text under "
    "a DEPRECATED label.\n\n"
    ...
    "'Nothing to save.' is a valid and correct outcome when neither "
    "dimension has a concrete signal."
)

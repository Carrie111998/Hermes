...
_SKILL_REVIEW_PROMPT = (
    ...
    "READ-BEFORE-WRITE (mandatory): every patch / edit / write_file / remove_file "
    "on a skill MUST load that exact target's current content first via "
    "skill_view — skill_view(name) for SKILL.md, or "
    "skill_view(name, file_path='references/...') for a supporting file. "
    "Writes against content you did not load in THIS review run are refused. "
    "Build the patch from the content just returned; if a write is refused with "
    "'not been loaded in this review turn', call skill_view for that exact "
    "target and retry.\n\n"

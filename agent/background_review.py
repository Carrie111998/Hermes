...
_SKILL_WRITE_QUALITY_GATE = (
    "Before any skill write, perform this critical QUALITY GATE. ALL criteria "
    "must pass:\n"
    "  • NOVEL: the learning is not already covered by the target skill or "
    "one of its support files.\n"
    "  • DURABLE: it is likely to remain true after this session and is not "
    "temporary environment or task state.\n"
    "  • EVIDENCE-BACKED: the conversation confirms the lesson through a "
    "successful outcome or explicit user correction; it is not speculation.\n"
    "  • REUSABLE: it is likely to help across multiple future sessions, not "
    "just explain today's incident.\n"
    "  • WELL-SCOPED: it describes a task class and belongs in this skill "
    "rather than user memory, SOUL/persona guidance, or session history.\n"
    "A correction or non-trivial technique is a candidate for evaluation, "
    "not permission to write. If any gate fails, do not write. 'Nothing to "
    "save.' is a normal, healthy outcome.\n\n"
)


_SKILL_REVIEW_PROMPT = (
    "Review the conversation above and update the skill library only when a "
    "candidate earns durable persistence. Prefer precision over activity.\n\n"
    + _SKILL_WRITE_QUALITY_GATE
    ...
    "Signals to evaluate (any one warrants evaluation, not automatic action):\n"
    "  • User corrected your style, tone, format, legibility, or "
    "verbosity. Frustration signals like 'stop doing X', 'this is too "
    "verbose', 'don't format like this', 'why are you explaining', "
    "'just give me the answer', 'you always do Y and I hate it', or an "
    "explicit 'remember this' are FIRST-CLASS candidates for evaluation, "
    "not automatic skill writes. Route them using the quality gate.\n"
    "  • User corrected your workflow, approach, or sequence of steps. "
    "Evaluate whether the correction is a durable pitfall or explicit step "
    "for the skill that governs that class of task.\n"
    "  • Non-trivial technique, fix, workaround, debugging path, or "
    "tool-usage pattern emerged that a future session would benefit "
    "from. Evaluate it against every quality-gate criterion.\n"
    ...
    "directory per kind:\n"
    "     • `references/<topic>.md` — reusable troubleshooting detail "
    "(reproduction recipes, provider quirks, evidence patterns) AND "
    ...
    "User-preference routing: a stable cross-task style or behavior "
    "preference belongs in user memory, not duplicated across skills. If this "
    "skill-only review cannot update memory, leave that candidate untouched. "
    "A task-specific or task-class workflow correction may belong in the "
    "governing skill, but only when every quality-gate criterion passes.\n\n"
    ...
    "If no candidate passes every quality-gate criterion, say 'Nothing to "
    "save.' and stop. Do not create a write merely because the review ran."
)
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "**Skills**: how to do this class of task. Update the library only when a "
    "candidate earns durable persistence. Prefer precision over activity.\n\n"
    + _SKILL_WRITE_QUALITY_GATE
    + "Target shape of the skill library: CLASS-LEVEL skills with a concise "
    "SKILL.md and a `references/` directory for reusable supporting detail. "
    "Not a long flat list of narrow one-session-one-skill entries.\n\n"
    "Signals to evaluate (any one warrants evaluation, not automatic action):\n"
    "  • User corrected your style, tone, format, legibility, "
    "verbosity, or approach. Frustration is a FIRST-CLASS candidate for "
    "evaluation, not an automatic skill write. 'stop doing X', 'don't "
    "format like this', and 'I hate when you Y' should trigger routing "
    "through the quality gate.\n"
    "  • Non-trivial technique, fix, workaround, or debugging path "
    "emerged.\n"
    "  • A skill that was loaded or consulted turned out wrong, missing, "
    "or outdated. Treat the proven gap as a candidate and patch only if "
    "every quality-gate criterion passes.\n\n"
    "After the quality gate passes, use this preference order for skills:\n"
    ...
    "`references/<topic>.md` for reusable troubleshooting detail OR condensed "
    ...
    "(2), or (3).\n\n"
    "User-preference routing: save stable cross-task style or behavior "
    "preferences in user memory; do not duplicate them across skills. A "
    "task-specific or task-class workflow correction may also belong in the "
    "governing skill, but only when every quality-gate criterion passes.\n\n"
    ...
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "Act only on dimensions with evidence that passes their persistence "
    "criteria. If no skill candidate passes every quality-gate criterion and "
    "there is no durable memory to save, say 'Nothing to save.' and stop."
)

...
_SKILL_REVIEW_PROMPT = (
    ...
    "the session produced a durable, reusable procedure. 'Nothing to save.' "
    "is correct when the signal is personal, transient, or unverified.\n\n"
    "Target shape of the library: CLASS-LEVEL skills, each with a concise "
    "SKILL.md and support files containing generic reusable material. Never "
    "preserve a session transcript or one-off task state in the live skill "
    "package.\n\n"
    "Signals that warrant a skill update:\n"
    "  • The user corrected a workflow, approach, or sequence in a way that "
    "applies to the whole class of task. Encode the correction as a pitfall "
    "or explicit step.\n"
    "  • A non-trivial technique, fix, debugging path, or tool-usage pattern "
    "was demonstrated and verified during the session.\n"
    "  • A loaded skill was proven wrong, incomplete, or outdated. Patch the "
    "curator-managed skill with the verified correction.\n"
    "Personal identity, preferences, and communication style belong in memory, "
    "not in a reusable skill. A personal correction alone is not a skill "
    "update signal.\n\n"
    ...
    "packaged with three kinds of reusable support files — use the right "
    "directory per kind:\n"
    "     • `references/<topic>.md` — concise authoritative excerpts, API "
    "docs, provider behavior, or domain notes that remain valid beyond this "
    "session. Do not store transcripts, current project state, or private "
    "material.\n"
    ...
    "Privacy and evidence boundary: skills are shareable procedure, not a "
    "user profile or session archive. Store personal facts and preferences "
    "only through the memory system. Generalize verified workflow lessons "
    "without retaining who supplied them or where they were discovered.\n\n"
    ...
    "a skill.\n\n"
    "  • Unresolved failures: if the session ended without finding a "
    "working method — every attempted approach failed and you told the user "
    "to check manually — do not recast those attempts as a reliable workflow "
    "or validated guidance. Say 'Nothing to save.', unless you are "
    "independently confident of a real working alternative; then capture only "
    "that alternative, never the dead ends or unverified guesses.\n"
    "  • Personal names, concrete user-home paths, non-placeholder email "
    "addresses, credentials, private URLs, or session identifiers.\n"
    "  • Current project status, remembered benchmark/pricing numbers, or "
    "vendor claims presented as independently verified facts. Record how to "
    "re-check volatile evidence instead of caching its value.\n"
    "  • Destructive or guard-bypass recipes such as hidden worktree state, "
    "force-updating remote refs, broad home-directory scans, or secret "
    "propagation through prompts/task context.\n"
    "  • Backup, snapshot, or timestamped copies inside a live skill package. "
    "Use the curator backup store outside the active package.\n"
    "If a tool failed because of setup state, capture the FIX (install "
    "command, config step, env var to set) under an existing setup or "
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "If the session produced no durable, verified, non-personal procedure, "
    "say 'Nothing to save.' and stop. Otherwise, act within these boundaries."
)

_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above and update two things:\n\n"
    "**Memory**: personal identity, desires, communication preferences, and "
    "durable expectations belong only in memory. Save valid updates with the "
    "memory tool; do not duplicate them into a reusable skill.\n\n"
    "**Skills**: update the library only when the session produced a durable, "
    "verified procedure for a class of tasks. 'Nothing to save.' is correct "
    "when the signal is personal, transient, or unverified.\n\n"
    "Target shape of the skill library: CLASS-LEVEL skills with concise "
    "SKILL.md files and generic reusable support material. Never preserve a "
    "session transcript or one-off task state in the live package.\n\n"
    "Signals that warrant a skill update:\n"
    "  • A workflow correction applies to the whole class of task.\n"
    "  • A non-trivial technique, fix, or debugging path was demonstrated "
    "and verified.\n"
    "  • A curator-managed skill was proven wrong, incomplete, or outdated.\n"
    "Personal identity, preferences, and communication style are memory "
    "signals, not skill-update signals.\n\n"
    ...
    "`references/<topic>.md` for concise authoritative excerpts or durable "
    "domain notes; `templates/<name>.<ext>` for generic starter files; "
    "`scripts/<name>.<ext>` for statically re-runnable verification, fixture "
    "generation, or probes. Do not store transcripts, current project state, "
    "private material, or backup copies. Add a one-line pointer in SKILL.md "
    "so future agents find the support file.\n"
    ...
    "(2), or (3).\n\n"
    "Privacy and evidence boundary: skills are shareable procedure, not a "
    "user profile or session archive. Personal facts and preferences stay in "
    "memory. Generalize verified workflow lessons without retaining who "
    "supplied them or where they were discovered.\n\n"
    ...
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "For skills, act only on durable, verified, non-personal signal. Memory "
    "may retain durable personal facts and preferences under its own policy. "
    "If neither store has a valid update, say 'Nothing to save.' and stop."
)

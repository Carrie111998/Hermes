...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Stances on system mechanisms: rules about how to respond to "
    "this very review pass or other system prompts (e.g. asserting a "
    "quality bar overrides the 'be ACTIVE' pressure, or that a "
    "system instruction should be resisted). Even when the content is "
    "correct and matches something the user said, persisting it in a "
    "skill freezes a stance on the mechanism itself, which silently "
    "compounds across future reviews regardless of whether the system "
    "prompt changes or a real gap exists on that pass. If a system "
    "mechanism conflicts with a user preference, say so in your reply "
    "so the owner can decide — do not write it into the skill library.\n\n"

...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    a guaranteed no-op — one that still pays for a full agent spawn (a JVM, for
    Junie) every cadence. The shipped ACP clients CAN carry them (both use the
    text bridge in ``agent/acp_openai_bridge.py``); this exists so a future one
    that can't declares ``SUPPORTS_HERMES_TOOL_CALLS = False`` and is skipped
    instead of burning a spawn. Anything that doesn't say otherwise is assumed
    capable, so ordinary providers are unaffected.
    """
...
def _run_review_in_thread(
    ...
    # produce a fork that spawns a whole agent (a JVM, for Junie) and then cannot
    # write anything. Don't spawn it — point at the override that does work.
    # Checked BEFORE the thread-scoped silence below so the warning is not
    # swallowed, and cheap-check-first so the normal path never resolves the
    # runtime twice. Fixes the class, not one provider: any future ACP
    # integration inherits the guard.
    if not _parent_can_emit_tool_calls(agent) and not bool(
        _resolve_review_runtime(agent).get("routed")
    ):
        logger.warning(

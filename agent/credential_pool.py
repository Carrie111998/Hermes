...
class CredentialPool:
    ...
    def acquire_lease(self, credential_id: Optional[str] = None) -> Optional[str]:
        ...
        if pending_refresh:
            self._refresh_pending_entries(pending_refresh)
        # Match select(): when every candidate was deferred, the first pass
        # necessarily returns no choice. Re-run the in-lock choice after the
        # refreshed entries have been merged back into the pool.
        if chosen_id is None and pending_refresh:
            chosen_id, _remaining_pending = self._acquire_lease_under_lock(None)
        return chosen_id

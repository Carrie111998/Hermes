...
# un-throttled log fires on *every* selection. On Windows several Orion
...
# readiness handshake ("Timed out connecting to Orion backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``orion_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``orion model`` / ``orion auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Orion process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``orion`` invocation that defaults to the active provider
        (e.g. setup wizard, ``orion auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Orion
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ORION_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ORION_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ORION_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("orion_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Orion
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("orion_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Orion process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Orion profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `orion model` / `orion auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `orion model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``orion auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `orion auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `orion auth remove <provider> <N>` is stable across all source types.
    try:
        from orion_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Orion PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `orion setup` ("Claude
        ...
        # Claude Code CLI, orion_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.orion/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `orion setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"orion_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_orion_oauth_credentials

        for source_name, creds in (
            ("orion_pkce", read_orion_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `orion auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (orion auth remove copilot gh_cli) must not pay either on
        ...
        # Orion auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from orion_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.orion/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `orion auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `orion auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Orion auth store.  Without this gate the removal is instantly
        ...
        # Orion owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``orion model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``orion auth list`` reflects the logged-in state and so the pool
...
# Prefer ~/.orion/.env over os.environ — the user's config file is the
# authoritative source for Orion credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_orion_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `orion auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.orion/.env.
    ...
    try:
        from orion_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    if provider == "openrouter":
        # Prefer ~/.orion/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `orion auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Orion

...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    1. Credential-bearing files (secret disclosure). Added in Phase 9 / Packet C.
    2. Hermes internal cache files and other denied paths (prompt-injection
       prevention and credential stores; pre-existing upstream categories).

    Chaining the credential check here rather than adding a parallel function
    is deliberate: every existing caller of this function is already a read
    chokepoint, so the credential boundary reaches tools/file_tools.py and the
    ACP shim with no new call sites and no risk of a caller being missed.
    ...
    Blocked categories include:
      ...
      * Credential / secret stores under HERMES_HOME and the global Hermes
        ...
        and anything under ``mcp-tokens/``.
      * Project-local environment files anywhere on disk (``.env`` family,
        ``.envrc``) — they routinely hold API keys and database passwords.
      * Internal Hermes cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata an attacker could use as a prompt-injection carrier.

    **This is NOT a security boundary.** The terminal tool runs as the same
    OS user with shell access; the agent can still ``cat auth.json`` and
    exfiltrate it. The read-deny exists as defense-in-depth that returns a
    clear error to models that respect tool denials and surfaces a visible
    audit trail. Treat any user-visible framing as "may help" rather than
    "stops attackers."

...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ORION_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Orion runtime / deployment
    "ORION_HOME", "ORION_PROFILE", "ORION_GATEWAY_LOCK_DIR",
    "ORION_MAX_ITERATIONS", "ORION_MAX_TOKENS", "ORION_API_TIMEOUT",
    "ORION_REDACT_SECRETS", "ORION_NOUS_TIMEOUT_SECONDS",
    "_ORION_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ORION_KANBAN_DB", "ORION_KANBAN_WORKSPACES_ROOT", "ORION_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ORION_KANBAN_",
    "ORION_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Orion writes
    ...
    ``orion_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Orion parser: save_env_value
    ...
    from orion_cli.config import _parse_env_value
...
def build_profile_secret_scope(orion_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(orion_home)
    secrets = load_env_file(home / ".env")

    try:
        from orion_cli.env_loader import get_secret_source_values

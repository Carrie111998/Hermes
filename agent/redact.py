...
def _redact_registry_patterns(text: str, *, file_read: bool,
    ...
    # code_file=True (source-code / terminal-output contexts) skips KEY=value
    # masking to match the built-in ENV-assignment contract (#33801): code
    # constants like MAX_TOKENS=*** must survive, and token-shaped values are
    # still caught by the built-in _PREFIX_RE pass. file_read content is
    # exempt (file_read implies code_file=True upstream): .env/config reads
    # must keep key-form masking — gate on file_read explicitly.

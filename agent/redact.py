...
def redact_sensitive_text(
    ...
    *,
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Registry-fed exact-value pass (Hermes-sec credential firewall; see PR
    # "exact-value secret redaction from user pattern file").
    # Masks exact registered secrets and registered KEY=value / KEY: value
    # forms from the pattern file written by `hermes-sec matcher build`
    # (default ~/.hermes/state/credential-firewall/redact_patterns.json,
    # overridable via HERMES_REDACT_PATTERNS). Fail-safe: a missing or
    # unreadable file is a no-op — the built-in families above still run.
    # Sentinel: non-reusable for file content (issue #35519 semantics) so a
    # masked value can never be written back over the real one.
    text = _redact_registry_patterns(text, file_read=file_read)

    return text


_PATTERNS_CACHE: dict = {"mtime": None, "lit_re": None, "key_re": None}


def _redact_registry_patterns(text: str, *, file_read: bool) -> str:
    """Exact-value + registered-key masking from the user patterns file.

    The file is JSON (``{"literals": [...], "key_patterns": {...}}``)
    generated by the ``hermes-sec`` credential-firewall tooling. Loaded
    lazily and cached by mtime so rotations are picked up without restart.
    Any failure degrades to a no-op — this pass enhances the built-in
    pattern families; it is never a required boundary by itself.
    """
    if len(text) < 8:
        return text
    try:
        ...
        path = _Path(os.environ.get(
            ...
            os.path.expanduser(
                "~/.hermes/state/credential-firewall/redact_patterns.json")))
    ...
    if not (_PATTERNS_CACHE.get("mtime") == mtime
        ...
        try:
            ...
            keys = list(data.get("key_patterns", {}).keys())
            _PATTERNS_CACHE["lit_re"] = (
                re.compile("|".join(re.escape(l) for l in lits)) if lits else None)
    ...
    if _PATTERNS_CACHE["lit_re"] is not None:
        if file_read:
            text = _PATTERNS_CACHE["lit_re"].sub(
                lambda m: _mask_token_nonreusable(m.group(0)), text)
        else:
            text = _PATTERNS_CACHE["lit_re"].sub("***", text)
    if _PATTERNS_CACHE["key_re"] is not None:
        def _k(m):
            val = m.group(4)
            return m.group(1) + m.group(2) + m.group(3) + (
                _mask_token_nonreusable(val) if file_read else "***")

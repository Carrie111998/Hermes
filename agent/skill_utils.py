...
from orion_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.orion/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    ORION_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ORION_KANBAN_TASK`` / ``ORION_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ORION_KANBAN_TASK") or os.getenv("ORION_KANBAN_BOARD"):
        ...
        # The Orion Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``orion_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ORION_PLATFORM`` or
            ``ORION_SESSION_PLATFORM`` env vars.  Returns the global
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``orion config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``orion`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.orion/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``orion`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ORION_HOME, not cwd
        if not p.is_absolute():
            p = (orion_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.orion/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.orion/skills`` at the git root) are
...
#   <root>/.orion/skills/   — Orion-native location
...
# a one-line "run `orion skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when ORION_HOME itself lives inside a git checkout:
    ``<root>/.orion/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``orion skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`orion skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under ORION_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_orion_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.orion/skills/`` (including via symlinks) or configured
    ...
    # ~/.orion/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Orion can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    orion = metadata.get("orion") or {}
    if not isinstance(orion, dict):
        orion = {}
    return {
        "fallback_for_toolsets": orion.get("fallback_for_toolsets", []),
        "requires_toolsets": orion.get("requires_toolsets", []),
        "fallback_for_tools": orion.get("fallback_for_tools", []),
        "requires_tools": orion.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    orion = metadata.get("orion")
    if not isinstance(orion, dict):
        return []
    raw = orion.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Orion metadata, VCS, virtualenv/dependency, cache, and skill

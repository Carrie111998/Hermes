    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                # Promise gate: when the agent is about to finish with a
                # natural-language future commitment ("I will do X next"), let
                # a plugin keep the turn going immediately so the agent can
                # execute/register/cancel the promise now instead of waiting
                # for the user's next message. Mirrors the pre_verify
                # continuation shape, but is not tied to code edits.
                _promise_nudge = None
                _promise_attempt = getattr(agent, "_promise_gate_nudges", 0)
                try:
                    from hermes_cli.lifecycle import has_hook
                    from hermes_cli.plugins import get_promise_gate_continue_message

                    # Allow multiple promise-gate continuations per turn (a
                    # summary may introduce several distinct next steps), but
                    # cap the loop so a pathological reply cannot spin forever.
                    if has_hook("promise_gate") and _promise_attempt < 3:
                        _promise_nudge = get_promise_gate_continue_message(
                            session_id=getattr(agent, "session_id", None) or "",
                            platform=getattr(agent, "platform", "") or "",
                            model=getattr(agent, "model", "") or "",
                            attempt=_promise_attempt,
                            final_response=final_response,
                        )
                except Exception:
                    logger.debug("promise_gate hook check failed", exc_info=True)
                    _promise_nudge = None

                if _promise_nudge:
                    agent._promise_gate_nudges = _promise_attempt + 1
                    final_msg["finish_reason"] = "promise_gate_continue"
                    # The attempted answer is real content; surface it as an
                    # interim assistant message, then append a synthetic user
                    # nudge for the internal promise-confirmation turn.
                    agent._emit_interim_assistant_message(final_msg)
                    messages.append(final_msg)
                    try:
                        agent._flush_messages_to_session_db(messages, conversation_history)
                    except Exception:
                        logger.debug("promise_gate interim flush failed", exc_info=True)
                    messages.append({
                        "role": "user",
                        "content": _promise_nudge,
                        "_promise_gate_synthetic": True,
                    })
                    agent._session_messages = messages
                    logger.debug("promise_gate nudge issued (attempt %d)",
                                 agent._promise_gate_nudges)

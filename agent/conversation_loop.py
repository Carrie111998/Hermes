...
def run_conversation(
    ...
    # [rosenblatt] copilot_remote auto-delegation: if the prompt is a clear
    # implementation request and the copilot_remote toolset is loaded, short-
    # circuit the conversation loop and launch a tracked Copilot session
    # directly. Mirrors the pre-extraction early-exit from run_agent.py.
    if agent._should_auto_delegate_to_copilot_remote(user_message):
        logger.info(
            "Auto-delegating implementation request to copilot_remote: session=%s platform=%s",
            agent.session_id or "none",
            agent.platform or "unknown",
        )
        return agent._auto_delegate_to_copilot_remote(

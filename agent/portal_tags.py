...
Every Orion request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Orion Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=orion-agent",
        "client=orion-client-v<__version__>",
    ]

The version is sourced live from ``orion_cli.__version__`` so it auto-aligns
...
``orion_cli.__version__`` is the canonical source of truth.
"""
...
def _orion_version() -> str:
    """Return the current Orion release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``orion_cli`` cannot be imported (should
...
def orion_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=orion-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=orion-client-v{_orion_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Orion session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Orion conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.orion/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Orion
    ...
    """
    tags = ["product=orion-agent", orion_client_tag()]

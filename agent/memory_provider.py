...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `orion backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - orion_home (str): The active ORION_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.orion``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "orion").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'orion memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], orion_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'orion memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``orion_home`` is the active ORION_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ORION_HOME.

        ``orion backup`` only walks ORION_HOME, so any provider state kept
        ...
        of the archive, and ``orion import`` restores them to their original

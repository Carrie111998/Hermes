...
``orion pause`` writes a sentinel file at ``$ORION_HOME/ESTOP``;
``orion resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Orion is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.orion/ESTOP``.
...
def _orion_home() -> Path:
    """Resolve the active ORION_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active ORION_HOME."""
    return _orion_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on ORION_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Orion is paused ({reason}). New work is on hold; "
            "run `orion resume` to pick things back up."
        )
    return (
        "⏸️ Orion is paused. New work is on hold; "
        "run `orion resume` to pick things back up."
    )

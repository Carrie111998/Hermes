...
Storage
-------
Bundles live in ``~/.orion/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ORION_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``orion bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ORION_HOME.

    Honors ``ORION_BUNDLES_DIR`` for tests; falls back to
    ``<ORION_HOME>/skill-bundles``.
    """
    override = os.environ.get("ORION_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_orion_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `orion bundles` CLI subcommand.
# ---------------------------------------------------------------------------

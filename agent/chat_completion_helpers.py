...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # Chunking recovery for truncated tool args WITH a finish_reason
        # (#74798): when the provider reports finish_reason but the tool
        # arguments are still truncated, the accumulated call would be
        # silently discarded.  Build a partial-stream stub so the caller
        # can attempt chunked retry for payload-heavy tools.

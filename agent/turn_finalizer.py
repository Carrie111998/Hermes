...
def finalize_turn(
    ...
    *,
    ...
    # Triage policy still uses the normal durable transcript finalizer, but it
    # must not turn a toolless local notification into plugin/memory/context
    # side effects. Existing cleanup and persistence remain enabled.
    is_triage_hold = bool(getattr(agent, "_fallback_triage_state", None))
    ...
    try:
        ...
        if not interrupted and not failed and not is_triage_hold:
            try:
    ...
    if final_response and not interrupted and not is_triage_hold:
        try:
    ...
    if not interrupted and not is_triage_hold:
        try:

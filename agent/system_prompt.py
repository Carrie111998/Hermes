...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    _r = _ra()
    _prompt_tool_names = set(agent.valid_tool_names or ())
    ...
    if (
        ...
        if _task_scoped_claude_capability:
            _prompt_tool_names = set(
                CAPABILITY_SETS[_task_scoped_claude_capability]
            )
    ...
    if getattr(agent, "_task_completion_guidance", True) and _prompt_tool_names:
    ...
    if getattr(agent, "_parallel_tool_call_guidance", True) and _prompt_tool_names:
        stable_parts.append(PARALLEL_TOOL_CALL_GUIDANCE)

    # Tool-aware behavioral guidance: only inject when the tools are loaded
    tool_guidance = []
    if "memory" in _prompt_tool_names:
        tool_guidance.append(MEMORY_GUIDANCE)
    if "session_search" in _prompt_tool_names:
        tool_guidance.append(SESSION_SEARCH_GUIDANCE)
    if "skill_manage" in _prompt_tool_names:
    ...
    elif _kanban_guidance is None and "kanban_show" in _prompt_tool_names:
    ...
    if "computer_use" in _prompt_tool_names:
        from agent.prompt_builder import computer_use_guidance
        stable_parts.append(computer_use_guidance())

    nous_subscription_prompt = _r.build_nous_subscription_prompt(_prompt_tool_names)
    ...
    has_skills_tools = any(name in _prompt_tool_names for name in ['skills_list', 'skill_view', 'skill_manage'])
    if has_skills_tools:
        avail_toolsets = {
            toolset
            for toolset in (
                _r.get_toolset_for_tool(tool_name) for tool_name in _prompt_tool_names
            )
            if toolset
        }
        ...
        skills_prompt = _r.build_skills_system_prompt(
            available_tools=_prompt_tool_names,

...
upstream prefix cache warm.  See ``orion-agent-dev``'s
...
from orion_constants import get_default_orion_root, get_orion_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- orion-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ORION_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ORION_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the ORION_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from orion_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from orion_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound ORION_HOME ContextVar override wins. Surfaces that multiplex
    ...
    """
    try:
        from orion_constants import get_orion_home_override
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Orion never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ORION_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the ORION_HOME ContextVar
    ...
    # Pointer to the orion-agent skill + docs for user questions about Orion itself.
    stable_parts.append(ORION_AGENT_HELP_GUIDANCE)
    ...
    # ORION_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Orion, any interactive coding surface in a code
    ...
    # `orion -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['orion-bots']).
    ...
    # Active-profile hint — names the Orion profile the agent is running
    # under so it doesn't conflate ~/.orion/skills/ (default profile) with
    # ~/.orion/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient ORION_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_orion_root(): get_orion_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_orion_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_orion_root())
    else:
        _home_str = _root_str = str(get_orion_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Orion profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_orion_root()), which in ambient
        # profile mode is NOT get_orion_home().
        profile_home = _home_str
        default_root = get_default_orion_root()
        post_workspace_parts.append(
            f"Active Orion profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Orion). Every other surface (desktop chat panel,
    ...
    from orion_time import get_timezone as _orion_tz, now as _orion_now
    now = _orion_now()

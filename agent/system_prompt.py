...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    try:
        soul_ext = agent._memory_manager.get_soul_extension_prompt()
        if isinstance(soul_ext, str) and soul_ext.strip():
            stable_parts.append(soul_ext.strip())
    except Exception:
        pass
    ...
    if agent._memory_store:
        ...
        try:
            profile_ext = agent._memory_manager.get_user_profile_extension_prompt()
            if isinstance(profile_ext, str) and profile_ext.strip():
                volatile_parts.append(profile_ext.strip())
        except Exception:
            pass

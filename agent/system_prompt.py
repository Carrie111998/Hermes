...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Full Kanban lifecycle guidance is dispatcher-scoped. Normal profiles may
    # expose routing tools without being assigned a task.
    _kanban_guidance = getattr(agent, "_kanban_worker_guidance", None)
    if _kanban_guidance:
        tool_guidance.append(_kanban_guidance)
    elif _kanban_guidance is None:
        fallback_guidance = kanban_guidance_for_session(agent.valid_tool_names)
        if fallback_guidance:
            tool_guidance.append(fallback_guidance)
    if tool_guidance:

...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Orion starts and finds ``~/.openclaw/``.

    Points users at ``orion claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``orion claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Orion, run "
        "`orion claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `orion claw cleanup` (renames it to ~/.openclaw.pre-migration — "

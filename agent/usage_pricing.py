...
def _usage_field(value: Any, name: str, default: Any = 0) -> Any:
    """Read a usage field from an SDK object or decoded JSON mapping."""
...
def normalize_usage(
    ...
    *,
    ...
    if mode == "anthropic_messages" or provider_name == "anthropic":
        input_tokens = _usage_count(_usage_field(response_usage, "input_tokens"))
        output_tokens = _usage_count(_usage_field(response_usage, "output_tokens"))
        cache_read_tokens = _usage_count(_usage_field(response_usage, "cache_read_input_tokens"))
        cache_write_tokens = _usage_count(_usage_field(response_usage, "cache_creation_input_tokens"))
    elif mode == "codex_responses":
        input_total = _usage_count(_usage_field(response_usage, "input_tokens"))
        output_tokens = _usage_count(_usage_field(response_usage, "output_tokens"))
        details = _usage_field(response_usage, "input_tokens_details", None)
        cache_read_tokens = _usage_count(_usage_field(details, "cached_tokens") if details else 0)
        cache_write_tokens = _usage_count(
            _usage_field(details, "cache_creation_tokens") if details else 0
        )
        input_tokens = max(0, input_total - cache_read_tokens - cache_write_tokens)
    else:
        prompt_total = _usage_count(_usage_field(response_usage, "prompt_tokens"))
        output_tokens = _usage_count(_usage_field(response_usage, "completion_tokens"))
        ...
        if not cache_read_tokens:
            ...
            cache_read_tokens = _usage_count(
                _usage_field(response_usage, "prompt_cache_hit_tokens")
            )
        cache_write_tokens = _usage_count(
        ...
        if not cache_write_tokens:
            cache_write_tokens = _usage_count(
                _usage_field(details, "cache_creation_input_tokens")
                if details else 0
            )
        if not cache_write_tokens:
            cache_write_tokens = _usage_count(
        ...
        if not cache_write_tokens:
            cache_write_tokens = _usage_count(
                _usage_field(response_usage, "cache_write_tokens")
            )
    ...
    if output_details:
        reasoning_tokens = _usage_count(_usage_field(output_details, "reasoning_tokens"))
    if not reasoning_tokens:
        completion_details = _usage_field(response_usage, "completion_tokens_details", None)
        if completion_details:
            reasoning_tokens = _usage_count(_usage_field(completion_details, "reasoning_tokens"))

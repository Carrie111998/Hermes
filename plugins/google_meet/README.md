...
Let the orion agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where orion runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `orion meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `orion meet` CLI |
...
| `cli.py` | `orion meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ORION_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `orion meet node {run,list,approve,remove,status,ping}` |
...
orion meet install                                      # pip + Chromium
orion meet setup                                        # preflight
orion meet auth                                         # optional
orion meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
orion meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.orion/.env
orion meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
orion meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
orion meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.orion/.env
orion meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, orion will **not** switch your system audio input automatically — the
...
orion plugins enable google_meet
orion meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
orion meet node approve my-mac ws://<mac-ip>:18789 <token>
orion meet node ping my-mac

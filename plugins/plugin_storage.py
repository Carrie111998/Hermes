...
``<orion home>/plugins/<name>/``. That tree is the plugin *install* dir —
``orion plugins remove`` deletes it and ``orion plugins update`` git-pulls
...
``<orion home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Orion.
...
# Mirrors the plugin-name shape `orion plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<orion home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`orion_constants.get_orion_home` on every call. Don't cache the
    result across profile switches.
    """
    from orion_constants import get_orion_home

    root = get_orion_home() / "plugin-data" / _validate_name(name)

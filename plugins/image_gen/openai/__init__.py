...
three virtual model IDs so the ``orion tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `orion tools` → Image "
                    "Generation → OpenAI to configure, or `orion setup` "
                    "to add the key."
                ),

"""Meta Model API (Muse Spark) provider plugin for Orion Agent.
...
Bundled from https://github.com/albertodepaola/orion-meta-provider. Orion'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to orion' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Orion' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in orion_cli/providers.py and the retention hint in

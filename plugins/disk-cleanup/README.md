...
Auto-tracks and cleans up ephemeral files created during Orion Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ORION_HOME` and `/tmp/orion-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ORION_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ORION_HOME` or `/tmp/orion-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ORION_HOME/disk-cleanup/` is itself excluded
- `$ORION_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

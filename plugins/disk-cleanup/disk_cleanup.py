"""disk_cleanup — ephemeral file cleanup for Orion Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ORION_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ORION_HOME and /tmp/orion-*
Never touches: ~/.orion/logs/ or any system directory.
"""
...
try:
    ...
    def get_orion_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ORION_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".orion").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ORION_HOME/logs/``."""
    return get_orion_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ORION_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ORION_HOME or ``/tmp/orion-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    orion_home = get_orion_home()
    try:
        path.resolve().relative_to(orion_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/orion-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("orion-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "orion-agent", "backups", "profiles", ".worktrees",
...
# Paths under $ORION_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ORION_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        orion_home = get_orion_home()
        for parent in ("cron", "cronjobs"):
            base = orion_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ORION_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ORION_HOME, but never recurse into known
    # durable state trees.  Some installs place the Orion checkout, venv,
    # and desktop build under ORION_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    orion_home = get_orion_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in orion_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    orion_home = get_orion_home()
    try:
        rel = path.resolve().relative_to(orion_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "orion-agent",
        ...
        # Path isn't under ORION_HOME (e.g. /tmp/orion-*) — fall through.
        pass

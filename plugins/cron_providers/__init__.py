...
1. Bundled providers: ``plugins/cron_providers/<name>/`` (shipped with orion-agent)
2. User-installed providers: ``$ORION_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_orion_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_orion_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_orion_user_cron'`` — the same
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ORION_HOME/plugins/`` or None if unavailable."""
    try:
        from orion_constants import get_orion_home
        d = get_orion_home() / "plugins"
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$ORION_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.

...
``{title, url, description, position}`` rows every other Orion web
provider produces.
...
prefers Orion-managed xAI Grok OAuth (via ``orion auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.orion/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``orion tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `orion auth` to sign in with "

...
This plugin ships bundled with Orion but is **opt-in** — it only loads when
...
orion tools  # → Langfuse Observability
...
Set these in `~/.orion/.env` (or via `orion tools`):
...
orion chat -q "hello"              # then check Langfuse for a "Orion turn" trace
```

Generation observations include the Orion system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `ORION_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
ORION_LANGFUSE_ENV=production       # environment tag
ORION_LANGFUSE_RELEASE=v1.0.0       # release tag
ORION_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
ORION_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
ORION_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
ORION_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`ORION_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
orion plugins disable observability/langfuse
```

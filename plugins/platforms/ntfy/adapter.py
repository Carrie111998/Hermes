"""ntfy platform adapter (Orion plugin).
...
``plugins/platforms/ntfy/``. The Orion plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "orion-in"              # subscribe topic (incoming)
          publish_topic: "orion-out"     # optional — defaults to topic
...
_ECHO_TAG = "orion-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``orion cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Orion dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `orion gateway status` without

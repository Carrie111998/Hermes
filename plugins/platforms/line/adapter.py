"""
LINE Messaging API platform adapter for Orion Agent.
...
to Orion Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ORION_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            orion_home = Path(get_orion_home()).resolve()
        except Exception:
            orion_home = Path.home().joinpath(".orion").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``orion status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``orion status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``orion setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.orion/.env`` via ``orion_cli.config``.
    """
    print()
    ...
    try:
        from orion_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("orion_cli.config not available; set LINE_* vars manually in ~/.orion/.env")
        return

...
# ``model_tools`` import time, so ``orion status``, ``orion chat``, etc.
...
# ``orion_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Orion is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a orion MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "orion-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ORION_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _orion_home = _get_orion_home()
        except (ModuleNotFoundError, ImportError):
            _orion_home = _Path.home() / ".orion"
        self._thread_count_store = _ThreadCountStore(
            _orion_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Orion is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ORION_HOME", str(_Path.home() / ".orion"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ORION_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a orion MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Orion is thinking…' marker message.
        ...
        "Orion is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Orion is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Orion is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Orion is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``orion setup``.

    The setup wizard at ``orion_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.orion/.env`` so the next
    gateway restart picks them up.
    """
    from orion_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    from orion_cli.config import get_env_value, save_env_value
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. orion-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.orion/.env")
    print_info("Restart the gateway: orion gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``orion cron`` running as a
    separate process from ``orion gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Orion plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `orion setup` to install Google Chat support.",
        ...
        # the "Orion is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Orion is thinking…' marker message "

"""SimpleX Chat platform adapter (Orion plugin).
...
This adapter ships as a Orion platform plugin under
``plugins/platforms/simplex/``. The Orion plugin loader scans the
...
Optional environment variables:
    ...
    ORION_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``orion setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Orion — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ORION_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``orion setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.orion/.env`` via
    ``orion_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "orion_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.orion/.env"
        )
        return

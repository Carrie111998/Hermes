"""
IRC Platform Adapter for Orion Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Orion agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "orion-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Orion Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Orion Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: orion_, orion_1, orion_2...
...
def check_requirements() -> bool:
    ...
    # orion setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `orion gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Orion to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #orion — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.orion/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "orion-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "orion-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Orion Agent (cron)")

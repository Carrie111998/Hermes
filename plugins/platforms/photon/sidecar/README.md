...
Small Node helper that bridges Orion Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Orion is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Orion-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Orion can receive inbound messages
...
The Orion plugin's `orion photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Orion is Python, so replies go through this sidecar

// Orion Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Orion): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Orion -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Orion-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `orion photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Orion can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "orion-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Orion'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Orion allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Orion, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-orion-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Orion supplied them so a known-good

...
under an immutable ``/opt/orion`` (read-only for the orion user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$ORION_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_orion_home() / "photon" / "sidecar"
    try:

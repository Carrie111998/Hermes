...
This plugin connects Orion Agent to iMessage (and other Spectrum
...
TypeScript-only, Orion runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Orion channel,
...
Run `orion photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.orion/.env` (the same place every other
...
Management metadata lives in `~/.orion/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `orion photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Orion
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Orion coalesces
...
- **Native polls are supported.** Orion posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `orion send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<orion-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `orion photon status`, a DM and a group roundtrip,

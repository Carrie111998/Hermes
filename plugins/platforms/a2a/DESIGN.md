...
- **Untrusted-input framing:** ALL inbound text (including `/`-prefixed —
  remote peers can never reach operator slash commands) is preserved verbatim
  behind a model-visible privacy boundary. The primary model, not host-side
  keyword or regex logic, remains the sole semantic authority.
...
├── security.py      # auth/identity, inbound framing, redaction, audit
├── DESIGN.md
└── README.md
```

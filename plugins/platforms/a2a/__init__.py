"""
A2A (Agent-to-Agent) plugin for Orion Agent.

Registers:
  - The ``a2a`` platform adapter (inbound: exposes Orion as an A2A agent,
    protocol v1.0).
...
def interactive_setup() -> None:
    """`orion gateway setup` flow for A2A."""
    ...
    print_info("Expose Orion as an A2A-discoverable agent and call other A2A agents.")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Orion plugin system."""

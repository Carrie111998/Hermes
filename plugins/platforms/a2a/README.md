# A2A — Agent-to-Agent protocol for Orion
...
peer (another Orion, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
orion gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Orion serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.orion/a2a_audit.jsonl`.
- Conversations persist to `~/.orion/a2a_conversations/` — they survive context

...
Layers (all opt-out-able only by explicit config, never silently):
  ...
  3. Inbound framing    — preserve peer text verbatim behind an explicit
                         model-visible untrusted-input boundary
...
# --------------------------------------------------------------------------
# Inbound untrusted-input framing
# --------------------------------------------------------------------------
...
def wrap_inbound(peer: str, text: str) -> str:
    """Frame inbound task text without semantically rewriting peer content.

    EVERY inbound message is framed — including text starting with "/".
    Remote peers must never reach the gateway's operator slash commands. The
    primary model receives the peer text verbatim and remains the sole semantic
    authority for interpreting it.
    """
    return PRIVACY_PREFIX.format(peer=peer or "unknown") + (text or "")

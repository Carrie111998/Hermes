...
holds the native media delivery cluster: size validation helpers (``_telegram_media_size_allowed`` / ``_telegram_media_too_large_note`` / ``_missing_media_path_error``) and the ``send_*`` family (voice, media groups, image files, documents, video, images, animations), plus the module-level ``_coerce_duration_seconds`` / ``_probe_voice_duration_seconds`` and ``_MEDIA_SEND_READ_TIMEOUT`` primitives they depend on. Those three helpers are re-exported by the adapter module so existing imports (``tests/gateway/test_telegram_voice_duration.py``, ``tests/gateway/test_telegram_media_read_timeout.py``) keep resolving.

Behavior-neutral: every method is lifted verbatim from ``TelegramAdapter``.
``self.*`` calls resolve unchanged via the MRO, and ``MediaSendMixin`` precedes
...
class MediaSendMixin:
    """Media delivery cluster lifted verbatim from ``TelegramAdapter``."""

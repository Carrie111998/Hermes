"""Polling/updates transport mixin for the Telegram adapter (adapter god-file slice).

Extracted from ``plugins/platforms/telegram/adapter.py``: the getUpdates
polling lifecycle (start/drain/reconnect/conflict ladders), the CLOSE-WAIT
heartbeat and pending-update probe, polling progress generations and their
verifiers, webhook startup, and the teardown fencing that serializes them.
``TelegramAdapter`` imports ``TelegramPollingMixin`` back and inherits from it
(the mixin pattern proven by the gateway authorization/topic mixins).

Adapter-local module globals the moved methods read at call time (error
redaction, polling constants, the generation ContextVar, the thread-deadline
helpers, the runtime-rebound ``Update``) stay on the adapter and are imported
lazily inside each method body, so this module never imports the adapter at
import time -> no import cycle, and monkeypatches of ``adapter.<name>`` keep
working. Shared error classifiers (``_looks_like_network_error`` /
``_looks_like_polling_conflict``) stay on the adapter class and resolve via
``self`` (MRO).
"""
...
class TelegramPollingMixin:
    """Polling/updates-transport methods for TelegramAdapter."""

    def _init_polling_state(self) -> None:
        """Initialize polling/updates-transport instance state.

        Extracted verbatim from ``TelegramAdapter.__init__`` so the polling
        mixin owns its transport state fields.
        """
    ...
    async def _drain_polling_connections(self) -> None:
        ...
        """
        from plugins.platforms.telegram.adapter import _DRAIN_TIMEOUT
    ...
    def _instrument_polling_request(self, request):
        ...
        instance.
        """
        from plugins.platforms.telegram.adapter import _POLLING_GENERATION_CONTEXT
        adapter = self
    ...
    async def _start_polling_once(
        ...
        *,
        ...
        """
        from plugins.platforms.telegram.adapter import Update, _POLLING_GENERATION_CONTEXT, _PollingLifecycleAbort, _UPDATER_START_TIMEOUT, _await_with_thread_deadline, _shutdown_abandoned_app
        if getattr(self, "_polling_teardown_started", False):
    ...
    async def _delete_webhook_best_effort(
        ...
        """
        from plugins.platforms.telegram.adapter import _UPDATER_START_TIMEOUT, _await_with_thread_deadline, _redact_telegram_error_text
        if not self._bot:
            return False
    ...
    async def _start_polling_resilient(
        ...
        *,
        ...
        """
        from plugins.platforms.telegram.adapter import _INITIAL_POLLING_PROGRESS_TIMEOUT, _PollingLifecycleAbort, _await_with_thread_deadline, _first_completed, _redact_telegram_error_text
        if getattr(self, "_polling_teardown_started", False):
            return False
    ...
    async def _handle_polling_network_error(self, error: Exception) -> None:
        ...
        """
        from plugins.platforms.telegram.adapter import _PollingLifecycleAbort, _UPDATER_STOP_TIMEOUT, _redact_telegram_error_text
        if getattr(self, "_polling_teardown_started", False):
            return
        if self.has_fatal_error:
            return
    ...
    async def _polling_heartbeat_loop(self) -> None:
        ...
        *general* request path (not the getUpdates pool), so a healthy long-poll
        ...
        """
        from plugins.platforms.telegram.adapter import _POLLING_ERROR_TASK_STUCK_TIMEOUT
    ...
    async def _verify_polling_after_reconnect(
        ...
        """
        from plugins.platforms.telegram.adapter import _POLLING_PROGRESS_TIMEOUT, _redact_telegram_error_text
        PROBE_TIMEOUT = 10
        if getattr(self, "_polling_teardown_started", False):
            return
    ...
    async def _start_webhook(self, *, is_reconnect: bool) -> bool:
        """Start the webhook transport when ``TELEGRAM_WEBHOOK_URL`` is set.

        Extracted from ``TelegramAdapter.connect`` so the webhook-vs-polling
        decision and its security checks live with the polling/updates
        transport mixin. Returns True when webhook mode is active (callers
        skip polling startup); False when polling mode should start instead.
        """
        from plugins.platforms.telegram.adapter import Update

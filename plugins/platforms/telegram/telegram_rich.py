"""Rich-message delivery methods for ``TelegramAdapter`` (Bot API 10.1/10.2).

Extracted from ``plugins/platforms/telegram/adapter.py`` as part of the god-file
decomposition campaign (telegram adapter shard).  This mixin holds the rich-message
delivery cluster: capability/content eligibility gates, TDesktop client-workaround
detectors, the raw ``sendRichMessage`` / ``editMessageText rich_message`` /
``sendRichMessageDraft`` senders, and the streaming protocol overrides.

Behavior-neutral: every method is lifted verbatim from ``TelegramAdapter``.
``self.*`` calls resolve unchanged via the MRO (thread routing, notification
kwargs, error classifiers stay on the adapter).  Neutral dependencies import at
module top; the one adapter-local helper (``_redact_telegram_error_text``) is
imported lazily inside the methods that use it so this module never imports the
adapter at import time -> no import cycle.  The module-level ``logger`` keeps the
adapter's exact logger name (``"plugins.platforms.telegram.adapter"``) so log
records are unchanged.

Tri-state contract (preserved): ``_try_send_rich`` / ``_try_edit_rich`` return
``None`` to signal "fall back to the legacy MarkdownV2 path"; a ``SendResult``
with ``success=False`` means transient (do NOT legacy-resend).  The capability
latches (``_rich_send_disabled`` / ``_rich_draft_disabled``) are initialized in
``TelegramAdapter.__init__`` and read/written only by this cluster.
"""
...
class TelegramRichMixin:
    """Rich-message delivery cluster for ``TelegramAdapter`` (Bot API 10.1/10.2)."""

"""Telegram c4 mixin for ``TelegramAdapter``.

Extracted verbatim from ``plugins/platforms/telegram/adapter.py`` as part
of the god-file decomposition campaign. Outbound messaging cluster: chunked sends, in-place edits with
overflow split-and-deliver, status message coalescing, and
message deletion.
...
``MessagingMixin`` precedes ``BasePlatformAdapter`` in the bases so
...
class MessagingMixin:
    """c4 cluster lifted verbatim from ``TelegramAdapter``."""

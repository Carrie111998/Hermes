...
def _apply_yaml_config(yaml_cfg: dict, mattermost_cfg: dict) -> dict | None:
    ...
    if "thread_context" in mattermost_cfg:
        _tc = _normalize_onoff(mattermost_cfg["thread_context"], default="on")
        seeded["thread_context"] = _tc
        if not _skip_env_bridge and not os.getenv("MATTERMOST_THREAD_CONTEXT"):
            os.environ["MATTERMOST_THREAD_CONTEXT"] = _tc

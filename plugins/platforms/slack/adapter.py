...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _handle_slack_message_impl(
        ...
        if is_thread_reply and not has_active_thread_session:
            ...
            # Ride the channel_context lane (not text): prepending to text
            # would move a recognized command away from character zero and
            # break command routing — see the channel_context init above.
            if nearby_context:
                channel_context = nearby_context
            elif nearby_notice:
                channel_context = nearby_notice

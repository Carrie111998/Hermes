"""
Email platform adapter for the Orion gateway.

Allows users to interact with Orion by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "orion-agent" "version" "{_orion_version}" '
            '"vendor" "Orion" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `orion config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `orion gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Orion Agent")
        ...
        msg_id = f"<orion-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# orion_cli/gateway.py, and the _send_email dispatch in

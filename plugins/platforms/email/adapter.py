        ...
        try:
            ...
            try:
                ...
                if is_reconnect and snapshot is not None:
                    ...
                    # adapter's consumed UID baseline. UIDs above its watermark
                    # arrived during the outage and remain eligible for dispatch.
                    self._seed_seen_uids(snapshot)
                    logger.info(
                    ...
                    # First connect (or no snapshot): seed all existing messages
                    # so BODY.PEEK[] polling processes only later arrivals.
        ...
        try:
            ...
            try:
                ...
                for uid in data[0].split():
                    ...
                    # Mark a UID consumed only after the server returned a
                    # response. A failed fetch must remain eligible next poll.
                    self._seen_uids.add(uid)
                    if len(self._seen_uids) > self._seen_uids_max:
                        self._trim_seen_uids()

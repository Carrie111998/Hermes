/**
 * Orion Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.orion/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ORION_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "orion-kanban-dot-triage",
    todo: "orion-kanban-dot-todo",
    ready: "orion-kanban-dot-ready",
    running: "orion-kanban-dot-running",
    blocked: "orion-kanban-dot-blocked",
    review: "orion-kanban-dot-review",
    done: "orion-kanban-dot-done",
    archived: "orion-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-orion-task";
  ...
  const DOCS_URL = "https://your-orion-docs.example/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://your-orion-docs.example/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "orion.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="orion-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])orion-kanban-md-code\1/i.test(attrs)) {
      return ' class="orion-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "orion-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "orion-kanban-md",
  ...
  // standard `drop` event and our `orion-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("orion-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("orion-kanban-column--drop");
          if (target) target.classList.add("orion-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("orion-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("orion-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("orion-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ORION_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "orion-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "orion-kanban-attention",
        "orion-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "orion-kanban-attention-bar" },
        h("span", { className: "orion-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "orion-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "orion-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "orion-kanban-attention-row",
                  "orion-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "orion-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "orion-kanban-attention-row-id" }, task.id),
                h("span", { className: "orion-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "orion-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "orion-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "orion-kanban-diag-action-btn",
      action.suggested ? "orion-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " orion-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".orion-kanban-drawer-comment-row input, .orion-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "orion-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("orion-kanban-diag", sevClass) },
      h("div", { className: "orion-kanban-diag-header" },
        h("span", { className: "orion-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "orion-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "orion-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "orion-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "orion-kanban-diag-data-row" },
                  h("span", { className: "orion-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "orion-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "orion-kanban-diag-data-row" },
                h("span", { className: "orion-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "orion-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "orion-kanban-diag-reassign-row" },
            h("span", { className: "orion-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "orion-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "orion-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "orion-kanban-diag-msg",
              msg.ok ? "orion-kanban-diag-msg--ok" : "orion-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "orion-kanban-section" },
      h("div", { className: "orion-kanban-section-head-row" },
        h("span", { className: "orion-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "orion-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "orion-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "orion-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "orion-kanban-docs-link",
      title: "Open Orion Kanban docs in a new tab",
      "aria-label": "Orion Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `orion kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "orion-kanban-msg-ok" : "orion-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "orion-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "orion-kanban-boardswitcher" },
      h("div", { className: "orion-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "orion-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "orion-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "orion-kanban-dialog-title" },
        ...
        h("div", { className: "orion-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Orion profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "orion-kanban-bulk" },
      h("span", { className: "orion-kanban-bulk-count" },
      ...
      h("div", { className: "orion-kanban-bulk-priority",
      ...
      h("div", { className: "orion-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Orion profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "orion-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("orion-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("orion-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "orion-kanban-trash",
        dragOver ? "orion-kanban-trash--drop" : "",
        props.draggingTaskId ? "orion-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "orion-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "orion-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".orion-kanban-card")) return true;
      if (target.closest && target.closest(".orion-kanban-column-add")) return true;
      if (target.closest && target.closest(".orion-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("orion-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("orion-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".orion-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "orion-kanban-columns",
        isScrollable ? "orion-kanban-columns--scrollable" : "",
        isPanning ? "orion-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("orion-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("orion-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "orion-kanban-column",
        dragOver ? "orion-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "orion-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "orion-kanban-col-check",
        ...
        h("span", { className: cn("orion-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "orion-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "orion-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "orion-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "orion-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "orion-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "orion-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "orion-kanban-lane" },
                  h("div", { className: "orion-kanban-lane-head" },
                    h("span", { className: "orion-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "orion-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "orion-kanban-card--stale-red";
    if (age >= tier.amber) return "orion-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".orion-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "orion-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "orion-kanban-card",
        props.selected ? "orion-kanban-card--selected" : "",
        props.failed ? "orion-kanban-card--failed" : "",
        props.draggingSource ? "orion-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "orion-kanban-card-content" },
          h("div", { className: "orion-kanban-card-row" },
            h("label", {
              className: "orion-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "orion-kanban-card-check",
            ...
            h("span", { className: "orion-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or orion kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "orion-kanban-warning-badge",
                    "orion-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "orion-kanban-needs-assignee",
          ...
          h("div", { className: "orion-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "orion-kanban-card-row orion-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "orion-kanban-assignee",
                            title: `Assigned to Orion profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "orion-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "orion-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "orion-kanban-ago",
  ...
  // centered modal reusing the orion-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "orion-kanban-dialog orion-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Orion profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Orion profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "orion-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "orion-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "orion-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "orion-kanban-drawer-close",
        ...
        data ? h("div", { className: "orion-kanban-drawer-comment-foot" },
          h("div", {
            className: "orion-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "orion-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "orion-kanban-section" },
      h("div", { className: "orion-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "orion-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "orion-kanban-drawer-body" },
      h("div", { className: "orion-kanban-drawer-title" },
        h("span", { className: cn("orion-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "orion-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "orion-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "orion-kanban-section" },
            h("div", { className: "orion-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "orion-kanban-section" },
            h("div", { className: "orion-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "orion-kanban-done-no-result orion-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "orion-kanban-section" },
            h("div", { className: "orion-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "orion-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "orion-kanban-section" },
        h("div", { className: "orion-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "orion-kanban-comment" },
            h("div", { className: "orion-kanban-comment-head" },
              h("span", { className: "orion-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "orion-kanban-diag-action-btn",
      ...
      h("div", { className: "orion-kanban-section" },
        h("div", { className: "orion-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "orion-kanban-comment" },
                h("div", { className: "orion-kanban-comment-head" },
                  h("span", { className: "orion-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "orion-kanban-comment-ago" },
      ...
      h("div", { className: "orion-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "orion-kanban-event",
              isDiag ? "orion-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "orion-kanban-event-header" },
                  h("span", { className: "orion-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "orion-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "orion-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "orion-kanban-event-header-plain" },
                  h("span", { className: "orion-kanban-event-kind" }, e.kind),
                  h("span", { className: "orion-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "orion-kanban-event-phantom-row" },
                  h("span", { className: "orion-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "orion-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "orion-kanban-event-payload" },
    ...
    return h("div", { className: "orion-kanban-section" },
      h("div", { className: "orion-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "orion-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `orion-kanban-run--${r.outcome || r.status || "ended"}`
          : "orion-kanban-run--active";
        return h("div", { key: r.id, className: cn("orion-kanban-run", outcomeClass) },
          h("div", { className: "orion-kanban-run-head" },
            h("span", { className: "orion-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "orion-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "orion-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "orion-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "orion-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "orion-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "orion-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "orion-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "orion-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "orion-kanban-pre orion-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "orion-kanban-meta-row" },
      h("span", { className: "orion-kanban-meta-label" }, props.label),
      h("span", { className: "orion-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "orion-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "orion-kanban-meta-row" },
        h("span", { className: "orion-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "orion-kanban-meta-value orion-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "orion-kanban-meta-row" },
        h("span", { className: "orion-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "orion-kanban-meta-value orion-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "orion-kanban-meta-row" },
        h("span", { className: "orion-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "orion-kanban-meta-value orion-kanban-editable",
    ...
    return h("div", { className: "orion-kanban-meta-row" },
      h("span", { className: "orion-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "orion-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "orion-kanban-section" },
      h("div", { className: "orion-kanban-section-head-row" },
        h("span", { className: "orion-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "orion-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "orion-kanban-section" },
      h("div", { className: "orion-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "orion-kanban-deps-row" },
        h("span", { className: "orion-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "orion-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "orion-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "orion-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "orion-kanban-dep-chip-x",
      ...
      h("div", { className: "orion-kanban-deps-row" },
        h("span", { className: "orion-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "orion-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "orion-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "orion-kanban-section" },
      ...
      h("div", { className: "orion-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "orion-kanban-home-sub orion-kanban-home-sub--on"
              : "orion-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ORION_PLUGINS__ && typeof window.__ORION_PLUGINS__.register === "function") {
    window.__ORION_PLUGINS__.register("kanban", KanbanPage);
  }
})();

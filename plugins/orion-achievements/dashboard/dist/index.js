(function () {
  "use strict";
  // orion-achievements dashboard plugin
  // Originally authored by @PCinkusz — https://github.com/PCinkusz/orion-achievements (MIT).
  // Bundled into orion-agent. Upstream repo remains the staging ground for new
  // badges and UI iteration; the in-progress scan banner below is a small addition
  // layered on top of the original dist bundle.
  const SDK = window.__ORION_PLUGIN_SDK__;
  if (!SDK || !window.__ORION_PLUGINS__) return;
  ...
  function api(path, options) {
    ...
    // BOTH dashboard modes: loopback (X-Orion-Session-Token header) and
    // gated OAuth (orion_session_at cookie via credentials:'include').
    // Hand-rolling fetch + reading window.__ORION_SESSION_TOKEN__ directly
    ...
    const url = "/api/plugins/orion-achievements" + path;
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("ORION AGENT  ·  your-orion-docs.example", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function download() {
      ...
      a.href = url;
      a.download = "orion-achievement-" + (achievement.id || "badge") + ".png";
    ...
    function tweetText() {
      ...
      const tmpl = tx(t, "share.tweet_text", "Just unlocked {tier_part}\"{name}\" in Orion Agent ☤", {
        tier_part: tierPart,
        name: achievement.name,
      });
      return tmpl + "\n\n@Orion · https://your-orion-docs.example";
    }
    ...
    return React.createElement("div", { className: "ha-page ha-page-loading" },
      React.createElement("section", { className: "ha-hero ha-loading-hero" },
        React.createElement("div", null,
          ...
          React.createElement("h1", null, tx(t, "hero.title", "Orion Achievements")),
          React.createElement("p", null, tx(t, "hero.scan_subtitle", "Scanning Orion session history. First scan can take 5–10 seconds on large histories."))
        ),
      ...
      React.createElement("section", { className: "ha-guide ha-loading-guide" },
        React.createElement("div", null,
          ...
          React.createElement("p", null, tx(t, "guide.scan_status_body", "Orion is scanning local history once, then cards will appear automatically. Nothing is stuck if this takes a few seconds."))
        ),
        React.createElement("div", null,
    ...
    return React.createElement("div", { className: "ha-page" },
      React.createElement("section", { className: "ha-hero" },
        React.createElement("div", null,
          React.createElement("div", { className: "ha-kicker" }, tx(t, "hero.kicker", "Agentic Gamerscore")),
          React.createElement("h1", null, tx(t, "hero.title", "Orion Achievements")),
          React.createElement("p", null, tx(t, "hero.subtitle", "Collectible Orion badges earned from real session history. Known unfinished achievements are shown as Discovered; Secret achievements stay hidden until the first matching behavior appears."))
        ),
      ...
      React.createElement("div", { className: "ha-stats" },
        ...
        React.createElement(StatCard, { label: tx(t, "stats.latest", "Latest"), value: latest[0] ? latest[0].name : tx(t, "stats.none_yet", "None yet"), hint: latest[0] ? latest[0].category : tx(t, "stats.latest_hint_empty", "run Orion more") })
      ),
      React.createElement("section", { className: "ha-guide" },
        ...
        React.createElement("div", null,
          ...
          React.createElement("p", null, tx(t, "guide.secret_body", "Secrets hide their exact trigger. Once Orion sees a related signal, the card becomes Discovered and shows its requirement."))
        )
      ),
  ...
  window.__ORION_PLUGINS__.register("orion-achievements", AchievementsPage);
})();

...
```bash
orion memory setup    # select "byterover"
```

Or manually:
```bash
orion config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.orion/.env
```
...
Working directory: `$ORION_HOME/byterover/` (profile-scoped).

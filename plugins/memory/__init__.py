...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with orion-agent)
2. User-installed providers: ``$ORION_HOME/plugins/<name>/``
3. Project-local providers: ``./.orion/plugins/<name>/``, opt-in via
   ``ORION_ENABLE_PROJECT_PLUGINS``
4. Pip-installed providers: ``orion_agent.memory_providers`` entry points
...
ENTRY_POINTS_GROUP = "orion_agent.memory_providers"
...
_USER_NAMESPACE = "_orion_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_orion_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_orion_user_memory'`` — the
...
def _get_project_plugins_dir() -> Optional[Path]:
    """Return ``./.orion/plugins/`` or None if unavailable or not opted in.

    Gated on ``ORION_ENABLE_PROJECT_PLUGINS`` exactly as the general
    ...
    """
    try:
        from orion_cli.plugins import _env_enabled

        if not _env_enabled("ORION_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".orion" / "plugins"
        return d if d.is_dir() else None
    except Exception:
        return None
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 2. User-installed providers ($ORION_HOME/plugins/<name>/)
    # 3. Project-local providers (./.orion/plugins/<name>/), opt-in
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    ``orion <provider>`` subcommands — working, but a second-class citizen next
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from orion_cli.plugins import resolve_module_origin
...
def load_memory_provider(
    ...
    *,
    ...
    (``$ORION_HOME/plugins/<name>/``), and pip entry-point providers.
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        provider — ``orion memory status``, the setup picker — leaves no
    ...
    def _plugin_context(self):
        ...
        plugin manager, which discovery touches on every orion startup.
        """
        if self._context is None:
            from orion_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _orion_user_memory.<name>.cli, usually

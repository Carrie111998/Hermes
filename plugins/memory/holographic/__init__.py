"""orion-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $ORION_HOME/config.yaml (profile-scoped):
  plugins:
    orion-memory-store:
      db_path: $ORION_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "orion-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, orion_home):
        """Write config to config.yaml under plugins.orion-memory-store."""
        from pathlib import Path
        config_path = Path(orion_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["orion-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_orion_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _orion_home = str(get_orion_home())
        _default_db = _orion_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $ORION_HOME in user-supplied paths so config values like
        # "$ORION_HOME/memory_store.db" or "~/.orion/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$ORION_HOME", _orion_home)
            db_path = db_path.replace("${ORION_HOME}", _orion_home)

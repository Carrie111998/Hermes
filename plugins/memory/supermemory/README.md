...
- Hosted: API key from [app.supermemory.ai/integrations?connect=orion](http://app.supermemory.ai/integrations?connect=orion)
...
Before running `orion memory setup`, add the local endpoint to
...
Then run `orion memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ORION_HOME/supermemory.json`
...
| `container_tag` | `orion` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `orion-{identity}` → `orion-coder`). |
...
`https://api.supermemory.ai`. Orion resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: orion`, and document writes stamp
`metadata.sm_source: orion`. This is a **functional routing key, not telemetry**:
it groups Orion-written memories into a dedicated "Orion" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Orion profile:

```json
{
  "container_tag": "orion-{identity}"
}
```

For a profile named `coder`, this resolves to `orion-coder`. The default profile resolves to `orion-default`. Without `{identity}`, all profiles share the same container.

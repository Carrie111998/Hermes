...
- OpenViking server running and reachable from Orion

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Orion can identify older servers that expose the legacy status-only health
...
connection values into Orion, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.orion/.env`; for a named profile use
...
OpenViking's server config is separate from Orion:
...
Orion-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `orion` | Orion peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Orion lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Orion sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Orion built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Orion action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Orion
...
file URIs, such as
`viking://user/peers/orion/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/orion/memories/preferences/mem_abc123.md`. Files

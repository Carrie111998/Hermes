...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Orion peer ID in OpenViking (default: orion)
...
_DEFAULT_AGENT = "orion"
_AGENT_PROMPT_LABEL = "Orion peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "ORION_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Orion slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Orion to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Orion refused the connection."
        ) from exc
...
def _load_orion_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ORION_HOME", "")).expanduser() if os.environ.get("ORION_HOME") else Path.home() / ".orion"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Orion did not start "
    ...
    try:
        ...
        # The Orion Desktop backend can include the Orion venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Orion venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Orion venv,
        # aborting `orion update` with access-denied on .pyd files.
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Orion session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Orion only", "write values only to Orion .env"),
    ...
    _save_orion_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Orion .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See orion-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_orion_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Orion peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, orion_home: str, config: dict) -> None:
        ...
        from orion_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        orion_home_path = Path(orion_home)
        env_path = orion_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_orion_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        orion_home = str(kwargs.get("orion_home") or "").strip()
        if not orion_home:
            try:
                from orion_constants import get_orion_home
                orion_home = str(get_orion_home())
            except Exception:
                orion_home = str(Path.home() / ".orion")
        self._orion_home = orion_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Orion will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.orion/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts orion (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Orion will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._orion_home:
            return None
        return Path(self._orion_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._orion_home:
            return None
        return Path(self._orion_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_orion_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Orion' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Orion canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See orion-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/orion/.overview.md).

...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # orion_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(orion_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(orion_home) / "mem0.json"
    existing = {}
...
def _setup_platform(orion_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "orion-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "orion"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.orion/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.orion/.env (or unset it) or Orion will keep "
    ...
    if env_writes:
        _write_env(Path(orion_home) / ".env", env_writes)
...
def _setup_selfhosted(orion_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "orion-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "orion")
...
def _setup_oss(orion_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(orion_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "orion-user")
    ...
    if env_writes:
        _write_env(Path(orion_home) / ".env", env_writes)
    _save_mem0_json(orion_home, {"mode": "oss", "user_id": user_id, "agent_id": "orion", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, orion_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # orion_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "orion-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "orion"
...
def _setup_oss_interactive(orion_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], orion_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], orion_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'orion-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "orion-user")

    agent_id = input("  Agent ID [orion]: ").strip()
    agent_id = agent_id or "orion"
    ...
    if env_writes:
        _write_env(Path(orion_home) / ".env", env_writes)
    _save_mem0_json(orion_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(orion_home: str, config: dict) -> None:
    """Entry point called by orion memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(orion_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(orion_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(orion_home, config, flags)
        return

...
```bash
orion memory setup    # select "mem0"
```
...
Behavioral settings live in `$ORION_HOME/mem0.json` (set them via `orion memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.orion/.env`.
...
| `user_id` | `orion-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Orion session and call `mem0_search` — it connects to your server.
...
```bash
orion memory setup mem0 --mode oss \
...
```bash
orion memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ORION_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.orion/mem0_qdrant"}}
  }
}
```
...
```bash
orion memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ORION_HOME/mem0.json`).

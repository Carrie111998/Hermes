...
``orion://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Orion OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "orion-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``orion-desktop`` / ``orion-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Orion.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ORION_HOME override can't reach.

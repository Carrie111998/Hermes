...
| `curatedContextPath` | string | — | Optional Markdown file under `HERMES_HOME/memory` that replaces raw Honcho base context. In tools mode this is an explicit exception to no automatic injection |
| `curatedContextTokens` | int | `1000` | Hard token cap for the curated file |
...
#### Curated base context

`curatedContextPath` is intended for compact context generated by another trusted workflow. Paths must resolve to a `.md` file under `HERMES_HOME/memory`; traversal, symlink escapes, other file types, and all paths outside that directory are rejected. Content is bounded by `curatedContextTokens`, stripped of nested memory fences, and scanned with Hermes' strict prompt-injection rules before injection. Unsafe or unreadable files fail closed for the curated source and raw Honcho base-context refresh remains suppressed while the option is configured. Dialectic supplements remain enabled in `hybrid` and `context` modes.

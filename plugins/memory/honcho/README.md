...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/orion>
...
```bash
orion memory setup honcho   # configure Honcho directly (works on a fresh install)
orion memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
orion config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.orion/.env
```

> `orion honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `orion memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `orion model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ORION_HOME/honcho.json` | Profile-local (isolated Orion instances) |
| 2 | `~/.orion/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Orion profile: `orion` (default) or `orion_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `orion honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Orion session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`orion-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`orion`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-orion-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening orion in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Orion session ID (timestamp + hex). Every `orion` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Orion profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`orion`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Orion profile: `orion` (default) or `orion_<profile>` (e.g. `orion -p coder` -> host key `orion_coder`). Older `orion.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ORION_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `orion-agent`) |
...
|---------|-------------|
| `orion memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `orion honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `orion memory setup`) |
| `orion honcho status` | Show resolved config for active profile |
| `orion honcho enable` / `disable` | Toggle Honcho for active profile |
| `orion honcho mode <mode>` | Change recall or observation mode |
| `orion honcho peer --user <name>` | Update user peer name |
| `orion honcho peer --ai <name>` | Update AI peer name |
| `orion honcho tokens --context <N>` | Set context token budget |
| `orion honcho tokens --dialectic <N>` | Set dialectic max chars |
| `orion honcho map <name>` | Map current directory to a session name |
| `orion honcho sync` | Create host blocks for all Orion profiles |

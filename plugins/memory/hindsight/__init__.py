...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    IMPORTANT: ``import hindsight`` (shipped by hindsight-all) eagerly imports
    the full hindsight-api stack, which transitively requires fastmcp and mcp
    1.x. Hermes pins mcp 2.0.0 in its own extras (tools/mcp_tool.py needs the
    new httpx2-based client). The two versions are mutually exclusive, so the
    in-venv probe can NEVER succeed — import the light dependencies directly
    instead. The daemon itself runs out-of-process (hindsight-api binary or
    uvx fallback) with an isolated dependency tree that resolves correctly.
    """
    try:
        importlib.import_module("hindsight_embed.daemon_embed_manager")
        # Verify sentence_transformers for local ML embeddings; without it,
        # the daemon aborts on startup. Importing here surfaces the failure
        # early so is_available() gates initialize() out (#7718).
        importlib.import_module("sentence_transformers")
        # Optional sanity check: import hindsight_client.Hindsight (light,
        # already pinned in plugin.yaml as hindsight-client==0.6.1).
        importlib.import_module("hindsight_client").Hindsight
        return True, None
    except Exception as exc:
        # Detect the fastmcp/mcp 1.x vs mcp 2.0 conflict pattern. When this
        # ImportError occurs, the root cause is NOT a missing package — it's
        # a known incompatibility that will never resolve in the same venv.
        # Surface actionable guidance instead of the generic hint.
        msg = str(exc) if exc else ""
        if "fastmcp" in msg.lower() or "mcp" in msg.lower() and "request_ctx" in msg:
            return (
                False,
                "fastmcp requires mcp 1.x but Hermes pins mcp 2.0.0 for its own "
                "MCP client. The two are mutually exclusive, so local_embedded mode "
                "cannot run in the same venv. Use the cloud or local_external mode "
                "instead, or run the daemon out-of-process via uvx (hindsight-api "
                "already falls back to uvx when the in-venv runtime is unavailable).",
            )
        return False, msg
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and (
        "hindsight'" in text or 'hindsight"' in text or "hindsight_embed" in text
    ):
        return (
...
def _load_config() -> dict:
    ...
    return {
        ...
        "retain_source": os.environ.get(
            "HINDSIGHT_RETAIN_SOURCE", _DEFAULT_RETAIN_SOURCE
        ),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def _offer_starter_template(
        self, mode: str, provider_config: dict, env_writes: dict
    ) -> None:
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_source = str(
            ...
            or os.environ.get("HINDSIGHT_RETAIN_SOURCE", _DEFAULT_RETAIN_SOURCE)
        ).strip()
    ...
    def recall_status(self) -> Optional[RecallStatus]:
        ...
        if not self._recall_indicator or not self._last_recall_returned:
            return None
        return RecallStatus(
            provider_label="Hindsight",
            count=self._last_recall_count,
            glyph=_HINDSIGHT_GLYPH,
        )

...
def _is_valid_memory_id(value: str) -> bool:
    """Validate memory_id against known Hindsight UUID format."""
    global _MEMORY_ID_RE  # noqa: PLW0603 — lazily compiled regex
    if _MEMORY_ID_RE is None:
        import re
        _MEMORY_ID_RE = re.compile(r'^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$')
    return bool(_MEMORY_ID_RE.match(value))

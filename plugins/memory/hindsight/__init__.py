...
_mental_model_cache: dict[_MentalModelCacheKey, tuple[str, float]] = {}
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # Mental model state
        self._agent_model_id: str = ""
        self._user_model_id: str = ""
        self._cache_ttl: int = 300
        self._mental_model_max_chars: int = 2000
        self._use_agent_model_for_soul: bool = False
        self._use_user_model_for_profile: bool = False
    ...
    def _fetch_mental_model_from_api(self, model_id: str) -> str:
        """Fetch mental model string from client.mental_models."""
    ...
    def _get_mental_models(
        ...
        agent_model_id = getattr(self, "_agent_model_id", "")
        if agent_model_id:
            agent_content = _resolve_mental_model(
                getattr(self, "_bank_id", "hermes"),
                agent_model_id,
                getattr(self, "_cache_ttl", 300),
                lambda: self._fetch_mental_model_sync(agent_model_id),
                block_on_missing=block_on_missing,
            )

        user_content: str | None = None
        user_model_id = getattr(self, "_user_model_id", "")
        if user_model_id:
            user_content = _resolve_mental_model(
                getattr(self, "_bank_id", "hermes"),
                user_model_id,
                getattr(self, "_cache_ttl", 300),
                lambda: self._fetch_mental_model_sync(user_model_id),
                block_on_missing=block_on_missing,
            )
    ...
    def get_soul_extension_prompt(self) -> str:
        ...
        if not getattr(self, "_use_agent_model_for_soul", False):
            return ""
        return getattr(self, "_baked_initial_agent_model_content", None) or ""
    ...
    def get_user_profile_extension_prompt(self) -> str:
        ...
        if not getattr(self, "_use_user_model_for_profile", False):
            return ""
        return getattr(self, "_baked_initial_user_model_content", None) or ""

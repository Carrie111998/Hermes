...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: orion)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: orion)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $ORION_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "orion").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Orion can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``orion memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'orion memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ORION_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_orion_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "orion": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "orion"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Orion config."""
    profile = config.get("profile", "orion")
    return str(profile or "orion")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Orion .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Orion profile name (from agent_identity)
      {workspace} — Orion workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``orion-{user}`` with no user becomes ``orion``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Orion Agent and the User"
    ...
    def save_config(self, values, orion_home):
        """Write config to $ORION_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(orion_home) / "hindsight"
    ...
    def post_setup(self, orion_home: str, config: dict) -> None:
        ...
        from orion_cli.config import save_config
        from orion_cli.secret_prompt import masked_secret_prompt

        from orion_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        # data-volume target instead of writing to /opt/orion (NS-605).
        ...
        provider_config.setdefault("bank_id", "orion")
        ...
        self.save_config(provider_config, orion_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(orion_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(orion_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Orion starter template (best-effort)."""
        from orion_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "orion")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "orion"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: orion-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Orion Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "orion"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "orion"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/orion (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "orion", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "orion")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Orion Agent and the User")
        ...
        # Orion emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Orion as a non-root user, or switch "
                    "to cloud / local_external mode via 'orion memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Orion get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See Orion/orion-agent#5820.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Orion created/used that client on the shared async loop,

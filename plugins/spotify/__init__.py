...
``_check_spotify_available()`` — when the user has not run ``orion auth
spotify``, the tools remain registered (so they appear in ``orion
...
The Spotify auth flow (``orion auth spotify``), CLI plumbing, and docs

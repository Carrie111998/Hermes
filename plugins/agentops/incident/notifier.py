...
class NotificationGate:
    def __init__(self, *, min_interval_seconds: int = 900, max_per_period: int = 20) -> None:
        self.interval = timedelta(seconds=min_interval_seconds); self.max_per_period = max_per_period; self.sent = 0; self._period = None
    def allow(self, incident: Incident, now: datetime) -> bool:
        period = now.date()
        if period != self._period: self._period = period; self.sent = 0

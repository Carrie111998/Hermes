...
`orion config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.orion/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ORION_HOME`, secret scope, and

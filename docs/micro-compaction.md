...
has to be thrown away or summarized. Orion has always done this in one batch:
...
Orion folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Orion's normal session flush is
...
With `micro_compact` unset or `false` Orion behaves exactly as it always has:
...
Defaults to `$ORION_HOME/logs/agent.log`. It reports passes, outcome mix, net

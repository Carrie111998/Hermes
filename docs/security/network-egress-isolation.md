...
When running Orion inside Docker, the default `network_mode: host` gives the
...
The Orion [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ORION_UID=$(id -u) ORION_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=orion,orion-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://orion-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Orion trust model and vulnerability reporting

...
|---|---|
| Fingerprint/correlation | `test_stable_cross_target_correlation_and_window_split`, `test_signal_id_is_atomic_idempotency_and_payload_is_redacted_and_frozen`, `test_fingerprint_drops_volatile_ids_and_normalizes_uuid` |
| State/history controls | `test_split_merge_suppress_keep_public_history_and_rebind_seen` (split/merge/suppress, bounded evidence and idempotent rebind) |
...
| Dashboard auth | `test_dashboard_auth_expiry_and_direct_reads_fail_closed` |

...
| State/history controls | `test_signal_and_split_budgets_fail_closed`, `test_split_merge_suppress_keep_public_history_and_rebind_seen`, `test_merge_marks_source_and_excludes_it_from_digest`, `test_merge_is_idempotent_and_merged_source_cannot_be_suppressed`, `test_suppression_identity_survives_window_until_expiry_then_reopens` (hard `_seen`/split caps, merge exclusion/idempotency, suppression identity across windows and expiry reopen) |

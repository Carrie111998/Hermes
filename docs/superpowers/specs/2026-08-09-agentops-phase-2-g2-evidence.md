...
| Final P0/P1 closure | Existing legacy DBs are refused before writable migration because Python SQLite cannot bind a verified inode to a later path reopen; bootstrap attempts bounded, owner-checked LaunchAgent plist binding and marks unavailable profiles unmanaged | `test_store_rejects_legacy_trigger_object_before_migration`, `test_bootstrap_coverage_reports_process_observer_unmanaged` |

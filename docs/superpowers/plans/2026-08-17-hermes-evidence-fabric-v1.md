...
**Architecture:** Extend the existing `SessionDB` SQLite schema and migration path with dedicated Evidence Fabric tables, while keeping DTOs, validation, ownership checks, and repository/service methods in a new `research` domain package. Reuse Hermes WAL, foreign keys, `BEGIN IMMEDIATE` retry behavior, and `hermes_home_key()` scope identity. Expose one non-core `research_fabric` tool through the existing registry after the service invariants are complete.
...
- Create: `tools/evidence_fabric_tool.py` — compact registered `research_fabric` tool.
- Create: `tests/tools/test_evidence_fabric_tool.py` — registry, schema, toolset, and authorization tests.
...
- Modify: `toolsets.py` — non-core `research` toolset containing only `research_fabric`.
...
- [ ] **Step 3: Write the direct-SQL cross-run test.** Insert run A/B and evidence A/B with `sqlite3.connect()`; attempt an evidence row in A whose `derived_from_evidence_id` is evidence B; assert `sqlite3.IntegrityError`. This bypasses service validation.
- [ ] **Step 4: Write the terminal immutability test.** Directly attempt evidence/claim/link inserts and claim-status updates after a run is `COMPLETED`; assert database rejection.
...
- [ ] **Step 4: Add terminal-run write protection.** Add triggers that reject graph inserts/updates when the parent run status is not `OPEN`; terminal status itself remains immutable.
...
- [ ] **Step 3: Implement pure helpers and frozen DTOs.** `EvidenceScope` defaults `scope_key` from `hermes_home_key()` and carries runtime profile, connection, and agent identity. The service—not callers—computes `content_hash`.
...
**Interfaces:** Produces `EvidenceFabricService.create_research_run`, `get_research_run`, `list_research_runs`, and terminal transition methods using `SessionDB` and `EvidenceScope`.
...
- [ ] **Step 2: Implement run writes/reads.** Check ownership and status inside the same `BEGIN IMMEDIATE` transaction; persist `owner_scope_key`, profile, connection ID, agent, metadata, and timestamps; return DTOs, never raw rows.
...
**Interfaces:** Produces `add_evidence`, `get_evidence`, and `list_evidence` with exact duplicate convergence, source-version preservation, derived provenance, and untrusted-content marking.
...
- [ ] **Step 2: Implement service-owned ingestion.** Require source content bytes/text for hashing; reject mismatched caller hashes; normalize URI conservatively; execute authorization/status/insert in one transaction.
...
**Interfaces:** Produces `create_claim`, `get_claim`, `list_claims`, `link_evidence_to_claim`, and `set_claim_status`.
...
- [ ] **Step 2: Implement transactional checks and writes.** Check IDs, same run, ownership, and terminal status within the write transaction. Populate link `created_by_agent/profile` and claim `updated_by_agent/profile` from `EvidenceScope`; reject or ignore model-supplied identity fields.
...
- [ ] **Step 1: Write the identical-evidence race test.** Use a barrier and two threads, each with its own `SessionDB` against one temporary DB, calling `add_evidence` with identical run/URI/content. Assert one row, one durable ID returned by both, no raw integrity error, and distinct evidence from both writers preserved.
...
## Task 8: Add the compact non-core model-facing tool

**Files:** Create `tools/evidence_fabric_tool.py` and `tests/tools/test_evidence_fabric_tool.py`; modify `toolsets.py`.

**Interfaces:** Produces one registered `research_fabric` tool in a non-core `research` toolset with a bounded operation enum for approved run/evidence/claim/link/status actions.

- [ ] **Step 1: Write failing registry/tool tests.** Assert discovery, compact schema, absence from `_HERMES_CORE_TOOLS`, opt-in `research` toolset, malformed-operation rejection, active `hermes_home_key()` use rather than model-supplied scope, and no SQL/database-path parameters.
- [ ] **Step 2: Implement registration and dispatch.** Follow `tools/registry.py`; resolve active home/profile/agent identity inside the handler; pass `EvidenceScope` to the service; dispatch only approved methods; bound JSON and return controlled errors.
- [ ] **Step 3: Run and commit.** `python -m pytest tests/tools/test_evidence_fabric_tool.py tests/test_evidence_fabric.py -q`; commit `feat: expose compact Evidence Fabric tool`.
...
```powershell
python -m compileall hermes_state_common.py hermes_state_schema.py research tools/evidence_fabric_tool.py

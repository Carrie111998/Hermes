...
The canonical transcript store is SQLite via `SessionDB` (from `orion_state`). The
...
Counts consecutive restarts via a JSON file (`{ORION_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `orion update`, `orion gateway restart`,
or `/restart`.

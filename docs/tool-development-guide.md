...
`packages/charterforge-stripe-rail/`, outside the core package. Install the
checkout and rail together with `pip install -e . -e
packages/charterforge-stripe-rail`, then set the secret-only
`STRIPE_SECRET_KEY`. It creates inbound Checkout Sessions and

文档路径：docs/ultra-studio-zh/infra-design/02-boundary-map.md

# 基础设施边界图

状态：基建设计中文详细版  
来源：`docs/ultra-studio-infra-design/02-boundary-map.md` 与中文阅读层 `docs/ultra-studio-docs-zh/infra-02-boundary-map.html`  
日期：2026-06-11

## 定位

按层列出 Edge/Gateway、Identity、Policy、Sandbox、Filesystem、Event Bus、GPU、Asset、Storage、Observability 等边界。

本页把源文档转换成中文实施视角：先说明它解决的问题，再拆出当前结论、执行影响、验证方式和开放问题。它用于中文阅读和排期，英文源保留为核对材料。

## 核心结论

- P0 需要明确 session、media_job、asset、credential shim、event 的 owner。
...
- 这是“谁拥有状态”的地图。以后新增能力时先找 owner，避免所有东西塞进 chat 或 plugin。
### P0 相关结论

- P0 需要明确 session、media_job、asset、credential shim、event 的 owner。
- 未实现的 cloud layer 只保留接口，不假装存在。
### 阅读位置
...
| 英文章节 | 中文章节 |
|---|---|
| Ownership Principle | 所有权原则 |
| Layer Map | 层级地图 |
| Control Plane Boundaries | 控制面边界 |
| Execution Plane Boundaries | 执行面边界 |
| Data Plane Boundaries | 数据面边界 |
| Source-of-Truth Map | 真实来源地图 |
| Status Truth For Earlier Questions | 前面问题的状态真相 |

文档路径：docs/ultra-studio-zh/infra-design/03-control-plane-design.md

# 控制面设计

状态：基建设计中文详细版  
来源：`docs/ultra-studio-infra-design/03-control-plane-design.md` 与中文阅读层 `docs/ultra-studio-docs-zh/infra-03-control-plane-design.html`  
日期：2026-06-11

## 定位

覆盖 Edge Gateway、Identity、OPA、TokenRouter、quota、approval、Gateway/Session 合同。

本页把源文档转换成中文实施视角：先说明它解决的问题，再拆出当前结论、执行影响、验证方式和开放问题。它用于中文阅读和排期，英文源保留为核对材料。

## 核心结论

- 控制面决定谁能做什么、花多少钱、用哪个凭证、哪里需要人批准。
- 自建 P0 可用 server env credential shim。
- 多用户/云模式前必须补 TokenRouter、policy、quota、操作记录。
...
| Objective | 目标 |
| Request Flow | 请求流 |
| Edge Gateway Contract | 边缘网关合约 |
| Identity and Tenant Access | 身份与租户访问 |
| OPA Policy Contract | OPA 策略合约 |
| TokenRouter Contract | TokenRouter 合约 |
| Gateway / Session API | Gateway / Session API |
| Approval Boundary | 审批边界 |
| Current Implementation Truth | 当前实现真相 |

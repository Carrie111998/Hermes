文档路径：docs/ultra-studio-zh/infra-design/07-validation-roadmap.md

# 基础设施验证路线

状态：基建设计中文详细版  
来源：`docs/ultra-studio-infra-design/07-validation-roadmap.md` 与中文阅读层 `docs/ultra-studio-docs-zh/infra-07-validation-roadmap.html`  
日期：2026-06-11

## 定位

定义 P0/P1/P2 基建验证、失败测试和 launch gates。

本页把源文档转换成中文实施视角：先说明它解决的问题，再拆出当前结论、执行影响、验证方式和开放问题。它用于中文阅读和排期，英文源保留为核对材料。

## 核心结论

- P0 launch gate：真实 media job、真实 asset、真实错误、刷新恢复、下载可用。
...
- 基建不是画图结束，要能用失败场景验证。比如 provider 失败、刷新恢复、权限拒绝、secret 泄漏检测。
### P0 相关结论

- P0 launch gate：真实 media job、真实 asset、真实错误、刷新恢复、下载可用。
- P1/P2 再加多用户、sandbox、TokenRouter、CometAPI。
### 阅读位置
...
| 英文章节 | 中文章节 |
|---|---|
| Launch Gate Rule | 上线门禁规则 |
| P0: Real Chat Agent With Media Jobs | P0：真实聊天 Agent + 媒体任务 |
| P1: Cloud Isolation and Durable Jobs | P1：云隔离与持久任务 |
| P2: Supercomputer-Grade Operations | P2：Supercomputer 级运营 |
| Validation Matrix | 验证矩阵 |
| Implementation Order | 实现顺序 |
| Failure Injection Tests | 故障注入测试 |
| Open Questions | 开放问题 |

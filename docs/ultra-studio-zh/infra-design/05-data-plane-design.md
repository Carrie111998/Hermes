文档路径：docs/ultra-studio-zh/infra-design/05-data-plane-design.md

# 数据面设计

状态：基建设计中文详细版  
来源：`docs/ultra-studio-infra-design/05-data-plane-design.md` 与中文阅读层 `docs/ultra-studio-docs-zh/infra-05-data-plane-design.html`  
日期：2026-06-11

## 定位

覆盖 CometAPI、Asset Library、object storage、Postgres RLS、media 来源链路、memory/files/marketplace。
...
- 创作产品的价值在复用资产。数据面决定上传、生成、检索、角色、合集和溯源怎么长期存在。
### P0 相关结论

- 先实现最小 media_assets。
- CometAPI、semantic search、RLS、object plane 是后续升级。
### 阅读位置
...
|---|---|
| Objective | 目标 |
| Data Plane Overview | 数据面总览 |
| Core Entities | 核心实体 |
| Media Job Contract | 媒体任务合约 |
| Asset Contract | 资产合约 |
| CometAPI Boundary | CometAPI 边界 |
| Upload Flow | 上传流程 |
| Asset Library Behaviors | 资产库行为 |
| Memory / Files / Marketplace | Memory / Files / Marketplace |
| Postgres RLS | Postgres RLS |
| Required Indexes | 必需索引 |

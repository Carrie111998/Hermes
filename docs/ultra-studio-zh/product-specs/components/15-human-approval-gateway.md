文档路径：docs/ultra-studio-zh/product-specs/components/15-human-approval-gateway.md

# 15 人工审批网关

状态：中文详细版（由英文核对版与中文阅读页整理）  
来源：`docs/ultra-studio-product-specs/components/15-human-approval-gateway.md` 与 `docs/ultra-studio-product-specs/components-zh/15-human-approval-gateway.html`  
日期：2026-06-11

## 目的与范围

对高风险动作、付费调用、删除、外发、重试消耗等操作做明确审批。要可恢复、可检查。

本页不是英文原文的逐词镜像，而是面向实施的中文详细版：保留原文的组件边界、实现状态、状态机、API/事件、数据模型、权限错误和验收要求，方便后续直接拆任务。

## 实现状态

- 已有 approval 和 clarify 机制，chat UI 有 pending prompt cards。
- 已有 sudo/secret prompt 相关 UI。
- 代码证据：tools/clarify_tool.py、PendingPromptPanel.tsx、tui_gateway/server.py。

## 主要缺口

- 当前审批偏内存态，刷新后不稳定。
- spec 的 approve/edit/reject/respond 与代码的 once/session/deny 不一致。
- 没有 durable decision record 和审批作用域。
...
- 生成前如有高成本或破坏性动作，显示明确审批卡。
- 审批决定写入 session/job record。
- 刷新后 pending approval 可恢复或明确过期。

P1 / P2 后续：

- 团队审批、预算审批、policy-based auto approve、操作记录导出。
- 编辑参数后再批准。

文档路径：docs/ultra-studio-zh/product-specs/components/16-observation-provenance-ledger.md

# 16 观察与溯源账本

状态：中文详细版（由英文核对版与中文阅读页整理）  
来源：`docs/ultra-studio-product-specs/components/16-observation-provenance-ledger.md` 与 `docs/ultra-studio-product-specs/components-zh/16-observation-provenance-ledger.html`  
日期：2026-06-11

## 目的与范围

记录每次生成、工具调用、观察结果、资产来源、QA 证据和决策链。它用来防止 agent 空口声称完成。

本页不是英文原文的逐词镜像，而是面向实施的中文详细版：保留原文的组件边界、实现状态、状态机、API/事件、数据模型、权限错误和验收要求，方便后续直接拆任务。

## 实现状态

- 已有部分 logs、insights、tool call records，但不是统一 ledger。
- docs 要求 artifact/provenance ledgers。
- 邻近实现分散在工具日志、session store、Atlas provider 返回。

## 主要缺口

- 没有统一 observation/provenance 数据模型。
- 没有 job_id -> asset_id -> prompt -> input_refs -> provider response 的完整链。
- 没有 QA evidence 与最终结果绑定。
...
- 先在 media_jobs 和 media_assets 上写足 provenance 字段。
- 每个完成声明必须能指向 asset 或 observation。
- 失败也要记录 provider error 和请求摘要。

P1 / P2 后续：

- 统一 ledger API、可视化时间线、导出、diff、检查模式。
- 跨任务资产 来源链路图。
...
- 用户问“这个视频怎么来的”，能查到输入、模型、prompt、时间和输出。
- agent 不能只说完成但没有 artifact id。
- QA 结论能区分观察事实和主观判断。

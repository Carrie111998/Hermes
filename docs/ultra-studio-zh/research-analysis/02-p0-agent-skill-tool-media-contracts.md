文档路径：docs/ultra-studio-zh/research-analysis/02-p0-agent-skill-tool-media-contracts.md

# P0 Agent / Skill / Tool / Media 合同

状态：调研分析中文详细版  
来源：`docs/ultra-studio-research-analysis/02-p0-agent-skill-tool-media-contracts.md` 与中文阅读层 `docs/ultra-studio-docs-zh/research-02-p0-agent-skill-tool-media-contracts.html`  
日期：2026-06-11

## 定位

收敛 P0 需要的 router、skill、media job、asset、provider constraints 合同。
...
- 这是从“技能文档”落到“运行时可以执行”的桥。没有这个，agent 仍可能只是在 prompt 里讲计划。
### P0 相关结论

- router 输出结构化 route，不直接生成。
- tool wrapper 写 media_job，provider adapter 负责 Atlas。
- 最终输出必须注册 asset。
### 阅读位置
...
| 英文章节 | 中文章节 |
|---|---|
| Canonical Endpoint Namespace | 规范 endpoint 命名空间 |
| Canonical Events | 规范事件 |
| Media Job Schema | 媒体任务 Schema |
| Asset Schema | 资产 Schema |
| Router Output | Router 输出 |
| P0 Skill Allowlist | P0 Skill 允许列表 |
| Atlas Tool Contract | Atlas 工具合约 |
| Typed Errors | 类型化错误 |
| Verification | 验证 |

文档路径：docs/ultra-studio-zh/research-analysis/01-p0-mvp-vertical-slice.md

# P0 MVP 垂直切片

状态：调研分析中文详细版  
来源：`docs/ultra-studio-research-analysis/01-p0-mvp-vertical-slice.md` 与中文阅读层 `docs/ultra-studio-docs-zh/research-01-p0-mvp-vertical-slice.html`  
日期：2026-06-11

## 定位

定义最小真实产品切片：React chat、upload、workflow-router、Atlas wrapper、media_jobs、media_assets、media cards。

本页把源文档转换成中文实施视角：先说明它解决的问题，再拆出当前结论、执行影响、验证方式和开放问题。它用于中文阅读和排期，英文源保留为核对材料。

## 核心结论

- P0 的目标不是 demo，而是一个真实可恢复的生成链路。它要能失败、能恢复、能显示真实资产。
- 不要引入 CometAPI、TokenRouter、Temporal、NATS 作为第一步。
- 必须新增 job/asset 持久层和 UI media card。
...
|---|---|
| Goal | 目标 |
| P0 User Stories | P0 用户故事 |
| P0 Non-Goals | P0 非目标 |
| Existing Code To Use | 可复用现有代码 |
| Required P0 Work | P0 必做工作 |
| P0 Acceptance | P0 验收 |
| Build Order | 建设顺序 |

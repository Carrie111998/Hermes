文档路径：docs/ultra-studio-zh/standalone/hermes-real-chat-agent-ui.md

# 真实聊天 Agent UI 合同

状态：独立专题中文详细版  
来源：`docs/hermes-real-chat-agent-ui.md` 与中文阅读层 `docs/ultra-studio-docs-zh/standalone-real-chat-ui.html`  
日期：2026-06-11

## 定位

定义网页聊天必须连接真实 Hermes，而不是 fake demo 或 hardcoded 视频结果。
...
- 这是之前 UI 争论的边界：不能一打开就生成视频，不能 hardcode brief，不能只做 demo 站。
### P0 相关结论

- 网页聊天走真实 session/gateway。
- 上传图片能进入真实工具链。
- 生成结果在前端真实显示，不只是 markdown URL。
### 阅读位置
...
|---|---|
| Contract | 合约 |
| Media Attachment Flow | 媒体附件流程 |
| References And Asset UI | 引用与资产 UI |
| Non-Goals | 非目标 |

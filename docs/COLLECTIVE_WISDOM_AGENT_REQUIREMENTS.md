...
| INST-001–009; LOCAL-001–008 | Explicit authenticated install with every exact-content download bound to the registered installation identity and current takedown generation; local compatibility, staging/atomic replacement, final record journal, and no unattended background update mutation | `hermes_wisdom/service.py`, `client.py`, `compatibility.py`, `consumption.py` | `tests/wisdom/test_client.py`, `test_compatibility.py`, `test_service.py`, `test_consumption.py` | Agent #94266 + Gateway #213 | implemented against the pinned generation-fenced content route; macOS/Windows atomic-filesystem execution remains release evidence |

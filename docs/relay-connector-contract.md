...
This document is the formal interface between the **Orion gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`Orion/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `orion gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`orion gateway enroll` → connector `/relay/enroll`, or
...
(`Orion/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `orion` CLI.

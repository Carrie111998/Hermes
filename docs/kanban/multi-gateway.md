...
Orion supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.orion/config.yaml`:
...
Or set the env var: `ORION_KANBAN_DISPATCH_IN_GATEWAY=false`

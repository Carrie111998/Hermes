...
**Task:** Task 26C — Approved marker disposition (Path A implementation candidate)
**Status:** Task 25 checkpointed `c6a9e305`; Task 26A **closed**; Task 26B **checkpoint approved and complete** (parent `8de2b29b`); **Task 26C Path-A v1 checkpoint approved and complete** (parent `a40ec2d0`); **Task 26 complete for approved v1 scope**; Path B deferred and unimplemented; **Task 27/28 not started**; marker disposition available only through the approved high-risk protocol; no generic unlock, stale takeover, force, retry, repair, invoke, Recovery Run, or outcome-rewrite authority; successful disposition does not grant ordinary Run mutation authority; finalized-run immutability remains enforced; all outcome non-permission booleans remain false

---

## 1. One-paragraph state

Phase 1 remains **semantically closed** at Task 17.1 `8fea4daa0`. Task 19 (`57a1ed651`) delivered read-only observe. Task 20 (`2fa580b5`) accepted **Policy C** (docs). Task 21 delivered read-only derived action plans. Task 22 enforces Policy C immutable seal at all 25 public/run-aware mutation boundaries. Task 23 adds a run-scoped durable write barrier for those same 25 mutators. **Task 24** (`af4868054`) adds authoritative approval control. **Task 24.1** (`40f4d016`) repaired the execution-lock contention test harness. **Task 25** (`c6a9e305`) adds `invoke_approved_run_completion` for approved `complete_run_manually` only. **Task 26A** (closed) adds read-only `inspect_run_completion_reconciliation`. **Task 26B** (checkpoint approved and complete) adds durable reconciliation case control records at `.control/reconciliation/` — decisions grant reconciliation posture only; all six non-permission booleans remain `false`; no marker disposition, retry, repair, invoke, Recovery Run creation, or outcome rewrite. **Task 26C Path-A v1 checkpoint approved and complete** — marker disposition available only through the approved high-risk protocol; successful disposition does not grant ordinary Run mutation authority; finalized-run immutability remains enforced; all outcome non-permission booleans remain false; Path B deferred. **Task 26 complete for approved v1 scope.** **Task 27/28 not started.**
...
→ 26C marker disposition ✅ (**Path-A v1 checkpoint approved and complete**) → 27 Recovery/Successor (not started)
...
Task 25 covers re-observe + stale rejection + single-API invoke + post-verification for `complete_run_manually` only. **Task 26B checkpoint approved and complete** — durable reconciliation cases grant posture only. **Task 26C Path-A v1 checkpoint approved and complete** (Path A marker disposition only). **Task 27/28 not started**; Task 26 complete for approved v1 scope (Path B deferred). General, unattended, and multi-API lifecycle invocation remain disabled.

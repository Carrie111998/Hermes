...
Orion gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Orion's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `orion.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `orion.platform.up/degraded` with bounded `error_code` attributes |
...
`orion.gateway.active_agents`, `orion.gateway.background_work`, and
`orion.gateway.background_delegations` are complementary. `active_agents`
...
```bash
orion monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'orion-agent[otlp]'`),
...
a plain `orion gateway run`: the exporter lives in the gateway process, so
...
`orion.gateway.up`, `orion.platform.up`, and `orion.platform.degraded`.
...
absent_over_time(orion_gateway_up[5m])

# Locally owned bridge is explicitly down.
orion_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
orion_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
orion_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
orion_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(orion_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `orion.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
orion.status = failed|unknown
orion.delivery_outcome = failed|not_configured
orion.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `orion.job_key`;
...
Orion Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/orion_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/orion_otel_capture.jsonl --wait 8
...
`orion.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
orion monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `orion monitoring` CLI intentionally exposes `status` only. This first
release covers only Orion Agent-owned service-health and operational-diagnostic
signals, including Orion Agent-owned Relay transport health. Team Gateway's

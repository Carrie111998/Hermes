...
Orion includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Orion and does not require a Orion observability plugin. Orion remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Orion execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `orion-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Orion `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `ORION_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `ORION_NEMO_RELAY_ATOF_*` and `ORION_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Orion does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Orion requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Orion installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Orion process so configured interceptors can
...
Collection remains off unless Orion policy enables it:
...
opt-in. Set `ORION_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Orion does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Orion
...
retain the same `session_id` and add `orion.session.segment` plus
`orion.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Orion
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Orion process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Orion logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Orion process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Orion
process and share the same policy boundary.

Run profiles in separate Orion processes when they require different trust
...
Orion core owns one Relay host and one isolated Relay session scope per Orion
session. Core lifecycle producers use
`orion_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Orion plugin registration. Shared-metrics marks must
...
```text
Orion turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Orion shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Orion sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Orion used for the logical call, such as
...
New calls use `orion.model_route.count`. The previous
`orion.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `orion.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Orion
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Orion processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `orion.task_run`, parented to
the owning Orion session. The start counter contains only bounded execution
...
provider attempts for the same Orion API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Orion tool-call
...
cancellations. Active task ownership follows the task ID if Orion rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`orion.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Orion
...
tool or plugin names. Orion does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `orion.tool_approval` marks and
...
Successful skill mutations emit `orion.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `orion.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Orion derives reuse and patch-generation
...
```text
$ORION_HOME/telemetry/shared_metrics/metrics.sqlite3
$ORION_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Orion version,
...
schema field name, its current scope is one `ORION_HOME`, so it is more
...
Deleting `$ORION_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ORION_HOME`. To reset it, stop Orion
processes and remove `$ORION_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Orion CLI turn against the deterministic local model server:

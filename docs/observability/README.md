# Orion Observer Hooks

Orion observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Orion' planner, model providers, memory, tool registry,
...
Orion also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "orion.observer.v1"
```

Hook callbacks are fail-open. Orion catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Orion only builds
...
The native NeMo Relay SDK integration maps Orion session, turn, LLM, and tool

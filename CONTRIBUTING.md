...
Thank you for contributing to Orion Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Orion to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo Orion/orion-agent "<your terms>"
  gh search prs --repo Orion/orion-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/zacharyjleach-stack/Aries/issues?q=) · [PRs (all states)](https://github.com/zacharyjleach-stack/Aries/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Orion itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Orion install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `orion skills browse` (labeled "official") and install it with `orion skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Orion Discord](https://github.com/zacharyjleach-stack/Aries). Users can install it with `orion skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.orion/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(orion_home, config)` for setup-wizard integration
...
- Integrate with `orion memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Orion ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.orion/plugins/`, project `.orion/plugins/`, or a pip entry point) — see [Build a Orion Plugin](https://your-orion-docs.example/docs/guides/build-a-orion-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Orion Discord](https://github.com/zacharyjleach-stack/Aries) `#plugins-skills-and-skins` channel so users can find and install it
...
echo "OPENROUTER_API_KEY=***" >> ~/.orion/.env
```
...
├── cli.py                    # OrionCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (orion-cli, orion-telegram, etc.)
├── orion_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── orion_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.orion/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (your-orion-docs.example)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.orion/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.orion/`)

| Path | Purpose |
|------|---------|
| `~/.orion/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.orion/.env` | API keys and secrets |
| `~/.orion/auth.json` | OAuth credentials (Nous Portal) |
| `~/.orion/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.orion/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.orion/state.db` | SQLite session database |
| `~/.orion/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.orion/cron/` | Scheduled job data |
| `~/.orion/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`orion_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.orion/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_ORION_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.orion`:
...
The user may skip setup and keep loading the skill. Orion only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `orion setup` or update `~/.orion/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Orion tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Orion Agent" is the secondary collaborator. If the contributor's commit shows "Orion Agent" as author because they used Orion to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Orion tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `orion --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Orion uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.orion/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `orion_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `orion_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Orion runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the orion wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `orion_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `orion_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `orion_cli/gateway_windows.py::
...
- **Discord**: [github.com/zacharyjleach-stack/Aries](https://github.com/zacharyjleach-stack/Aries) — for questions, showcasing projects, and sharing skills

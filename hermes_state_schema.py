    ...
    def _init_schema(self):
        ...
        if fts5_available:
            ...
            if self._db_has_legacy_inline_fts(cursor):
                ...
                # Trigram FTS5 for CJK/substring search. Enabled by default for
                # upstream parity, but operators can quarantine it without
                # touching canonical messages or the base FTS surface.
                if self._fts_enabled:
                    if trigram_enabled_for_open:
                        trigram_enabled = self._ensure_fts_schema(
                            cursor, "messages_fts_trigram", FTS_TRIGRAM_SQL

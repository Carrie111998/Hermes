# nix/desktop.nix — Orion Desktop (Electron) app build + wrapper
#
# `orionAgent` is the fully-built `.#default` package — it ships the
# `orion` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ORION_DESKTOP_ORION` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Orion CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # ORION_HOME and ORION_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "orion-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "orion-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = orionNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "orion-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe orionNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/orion-desktop $out/bin
    cp -r ${renderer}/* $out/share/orion-desktop/
    ...
    substituteInPlace $out/share/orion-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/orion-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ORION_DESKTOP_ORION to the absolute path of the nix-built `orion`
    # binary so the desktop's resolver step 4 ("existing Orion CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/orion-desktop \
      --add-flags "$out/share/orion-desktop" \
      --set ORION_DESKTOP_ORION "${lib.getExe orionAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/orion.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../orion_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/orion-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/orion.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/orion.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Orion Agent";
    homepage = "https://github.com/zacharyjleach-stack/Aries";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "orion-desktop";
  };
}

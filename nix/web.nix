# nix/web.nix — Orion Web Dashboard (Vite/React) frontend build
{ orionNpmLib, ... }:
orionNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @orion/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../orion_cli/web_dist for the

...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      orion-agent = self'.packages.default;
      orionVenv = orion-agent.orionVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "orion-check";
                homeDirectory = "/home/orion-check";
                stateVersion = "24.11";
              };
            }
            { services.orion-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.orion-agent = programs;
              services.orion-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.orion-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.orion-agent);
      ...
      configKeys = pkgs.runCommand "orion-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${orionVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "orion-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "orion-build-package" { } ''
          echo "PASS: package built at ${orion-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "orion-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.ORION_TEST = "1";
                environmentFiles = [ "/run/secrets/orion-env" ];
                orionHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "orion" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "orion" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.orionAgentSetup.data;

            failures =
              lib.optional (names != [
                "orion-agent"
                "orion-backend"
              ]) "expected orion-agent + orion-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/orion gateway" (argvOf "orion-agent")
              ) "gateway process does not run `orion gateway`: ${argvOf "orion-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/orion serve" (argvOf "orion-backend")
              ) "backend process does not run `orion serve`: ${argvOf "orion-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "orion-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/orion-check/.orion" (envOf n)) names
              ) "gateway and backend must share one ORION_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.ORION_HOME or null != "/home/orion-check/.orion"
              ) "programs.orion-agent.enable must export ORION_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "orion-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/orion-check/.orion/SOUL.md" activation
              ) "orionHomeFiles must install into ORION_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads ORION_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "ORION_MANAGED=home-manager" (envOf n)) names
              ) "processes must report ORION_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "orion-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "orion-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/orion-check";
            ...
            cases = [
              ...
              {
                name = "orionHomeFiles needs no directory";
                ok = accepts { orionHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "orion-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one ORION_HOME ───────────────
        # `programs.enable` exports ORION_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.orion
        # while the services use the ORION_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/orion-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                orionHome = "/home/orion-check/.orion-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "orion-desktop") cfg.home.packages;
            ...
            # second Orion runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "orion-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.orion-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.orion-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one orion-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "ORION_HOME" != "/home/orion-check/.orion-work"
              ) "the launcher must carry ORION_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "ORION_HOME")})"
              ++ lib.optional (
                setValue "ORION_MANAGED" != "home-manager"
              ) "the launcher must report ORION_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "ORION_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "ORION_DESKTOP_ORION" != "${lib.head agentPackages}/bin/orion"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "ORION_DESKTOP_ORION")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "ORION_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "ORION_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "ORION_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "ORION_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "ORION_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "orion-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares ORION_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set ORION_HOME" wrapper
              ) "the launcher must carry ORION_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "ORION_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "ORION_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? orion-backend || cfg.launchd.agents ? orion-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "orion-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `orion`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.orion-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.orion-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "orion-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "orion-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "orion" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."orion-agent-setup".text;

            failures =
              lib.optional (names != [
                "orion-agent"
                "orion-backend"
              ]) "expected orion-agent + orion-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/orion gateway" (execOf "orion-agent")
              ) "gateway unit does not run `orion gateway`: ${execOf "orion-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/orion dashboard" (execOf "orion-backend")
              ) "backend unit does not run `orion dashboard`: ${execOf "orion-backend"}"
              ++ lib.optional (
                units.orion-agent.environment.ORION_HOME != units.orion-backend.environment.ORION_HOME
              ) "gateway and backend must share one ORION_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/orion/.orion/SOUL.md" activation
              ) "orionHomeFiles must install into ORION_HOME";
          ...
          in
          pkgs.runCommand "orion-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs orion, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.orion-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/orion serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec orion directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "orion-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "orion-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec orion, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/orion dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "orion-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $ORION_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                ORION_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "orion-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'ORION_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--orion-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "orion-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "orion-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${orion-agent}/bin/orion || (echo "FAIL: orion binary missing"; exit 1)
          test -x ${orion-agent}/bin/orion-agent || (echo "FAIL: orion-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${orion-agent}/bin/orion --version 2>&1 | grep -qi "orion" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "orion-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in orion orion-agent orion-acp; do
            test -x ${orion-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "orion-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking orion --help ==="
          ${orion-agent}/bin/orion --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${orion-agent}/bin/orion --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "orion-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${orion-agent}/share/orion-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${orion-agent}/share/orion-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ORION_BUNDLED_SKILLS" ${orion-agent}/bin/orion || \
            (echo "FAIL: ORION_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ORION_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${orion-agent}/share/orion-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${orion-agent}/share/orion-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ORION_OPTIONAL_SKILLS" ${orion-agent}/bin/orion || \
            (echo "FAIL: ORION_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ORION_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "orion-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${orion-agent}/share/orion-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${orion-agent}/share/orion-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ORION_BUNDLED_PLUGINS" ${orion-agent}/bin/orion || \
            (echo "FAIL: ORION_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ORION_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "orion-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${orion-agent}/share/orion-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${orion-agent}/share/orion-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${orion-agent}/share/orion-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ORION_BUNDLED_LOCALES" ${orion-agent}/bin/orion || \
            (echo "FAIL: ORION_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ORION_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ORION_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ORION_BUNDLED_LOCALES=${orion-agent}/share/orion-agent/locales \
            ${orionVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ORION_BUNDLED_LOCALES set"; exit 1)
        ...
        # ORION_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "orion-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${orion-agent}/share/orion-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${orion-agent}/share/orion-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ORION_OPTIONAL_MCPS" ${orion-agent}/bin/orion || \
            (echo "FAIL: ORION_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ORION_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${orion-agent}/bin/orion mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: orion mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "orion-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${orion-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${orion-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ORION_TUI_DIR" ${orion-agent}/bin/orion || \
            (echo "FAIL: ORION_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ORION_TUI_DIR set in wrapper"
        ...
        # Verify ORION_NODE is set in wrapper and points to Node 26+
        # (Orion pins its toolchain to Node 26 everywhere)
        orion-node = pkgs.runCommand "orion-node-version" { } ''
          set -e
          echo "=== Checking ORION_NODE in wrapper ==="
          grep -q "ORION_NODE" ${orion-agent}/bin/orion || \
            (echo "FAIL: ORION_NODE not set in wrapper"; exit 1)
          echo "PASS: ORION_NODE present in wrapper"

          ORION_NODE=$(sed -n "s/^export ORION_NODE='\(.*\)'/\1/p" ${orion-agent}/bin/orion)
          test -x "$ORION_NODE" || (echo "FAIL: ORION_NODE=$ORION_NODE not executable"; exit 1)
          echo "PASS: ORION_NODE executable at $ORION_NODE"

          NODE_MAJOR=$("$ORION_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Orion requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ORION_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ORION_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "orion-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ORION_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ORION_MANAGED guards ==="
          check_blocked "config set" ${orion-agent}/bin/orion config set model foo
          check_blocked "config edit" ${orion-agent}/bin/orion config edit
        ...
        extra-python-packages = let
          ...
          orionWithExtra = orion-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "orion-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${orionWithExtra}/bin/orion || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${orionWithExtra}/bin/orion || \
          ...
          if grep -q "PYTHONPATH" ${orion-agent}/bin/orion; then
        ...
        extra-dependency-groups = let
          orionWithGroups = orion-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "orion-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${orionWithGroups}"
          echo "venv: ${orionWithGroups.orionVenv}"
        ...
        messaging-variant = pkgs.runCommand "orion-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.orionVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "orion-config-roundtrip" {
          ...
          merge_and_load() {
            local orion_home="$1"
            export ORION_HOME="$orion_home"
            ${configMergeScript} ${nixSettings} "$orion_home/config.yaml"
            ${orionVenv}/bin/python3 -c '
import json, sys

# nix/nixosModules.nix — the NixOS module for orion-agent
#
...
#
# Container mode: orion runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ORION_HOME/.env
# and read by orion at startup — no container recreation needed for env changes.
#
# Tool resolution: the orion wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      # ORION_HOME via addToSystemPackages, so they can save settings through the
      ...
      # orion user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "orion-container-entrypoint" ''
        set -eu

        ORION_UID="''${ORION_UID:?ORION_UID must be set}"
        ORION_GID="''${ORION_GID:?ORION_GID must be set}"

        # ── Group: ensure a group with GID=$ORION_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$ORION_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="orion"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$ORION_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$ORION_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$ORION_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$ORION_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="orion"
          TARGET_HOME="/home/orion"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$ORION_UID" -g "$ORION_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$ORION_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$ORION_UID:$ORION_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure ORION_HOME is owned by the target user.
        ...
        if [ -n "''${ORION_HOME:-}" ] && [ -d "$ORION_HOME" ]; then
          find "$ORION_HOME" \! -user "$ORION_UID" -exec chown "$ORION_UID:$ORION_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/orion-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/orion-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/orion ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/orion
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$ORION_UID" --regid="$ORION_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $ORION_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "orion-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        orion_bin=${containerDataDir}/current-package/bin/orion
      '';

      # Default: /var/lib/orion/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the orion group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit orionHome; };
    ...
    in
    {
      options.services.orion-agent =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/orion";
              description = "State directory. Contains .orion/ subdir (ORION_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the orion CLI to environment.systemPackages and export
                ORION_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.orion symlink to the service
                  stateDir. These users are automatically added to the orion group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the orion CLI to system PATH and export ORION_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.orion/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.ORION_HOME = orionHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.orion-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the orion user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.orion-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed orion
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure orion is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${orionHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${orionHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."orion-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${orionHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${orionHome} ${cfg.workingDirectory}
                  ...
                  find ${orionHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${orionHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${orionHome}/$_subdir"
                    chmod 2770 "${orionHome}/$_subdir"
                    find "${orionHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg orionHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${orionHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${orionHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${orionHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.orion";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${orionHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "orion-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.orion -> stateDir/.orion for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.orion-agent = {
              ...
              # $ORION_HOME/.env by the activation script. load_orion_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.orion-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.orion-agent = {
              description = "Orion Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  ORION_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  ORION_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env ORION_UID="$ORION_UID" \
                    --env ORION_GID="$ORION_GID" \
                    --env ORION_HOME=${containerDataDir}/.orion \
                    --env ORION_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/orion gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}

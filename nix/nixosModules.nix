...
{ inputs, ... }: {
  ...
  let
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      #   Node ${toString nodeMajor} via NodeSource — Ubuntu 24.04 ships Node 18 which is EOL.
      ...
      if [ ! -f /var/lib/hermes-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        echo "deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_${toString nodeMajor}.x nodistro main" \

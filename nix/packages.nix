# nix/packages.nix — Orion Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./orion-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.orionTui;
        web = full.orionWeb;
        desktop = full.orionDesktop;

        update-npm-lockfile = full.orionNpmLib.updateNpmLockfile;
      };
    };
}

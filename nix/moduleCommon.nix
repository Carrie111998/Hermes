...
#
# `services.orion-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  orionHome, programs.orion-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "orion-config-attrs";
    description = "Orion YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $ORION_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Orion Agent";
      ...
      package = mkOption {
        ...
        description = "The orion-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Orion configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `orion config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $ORION_HOME/.env. Orion reads that file at each start, with
          load_orion_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."orion/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Orion refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Orion reads SOUL.md and memories/ from ORION_HOME, so put those
          files in `orionHomeFiles`.
      ...
      orionHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into ORION_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Orion reads SOUL.md and the memory files from ORION_HOME and not
          from the working directory. Declare those files here, or Orion
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the orion plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Orion discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          orion_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as orion (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in orion-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `orion gateway`.";
      };
      ...
      # ── The backend: `orion serve` or `orion dashboard` ──────────────
      # `orion serve` and `orion dashboard` are the same entry point,
      # orion_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Orion Desktop
      ...
      # controls an external gateway, with `orion gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Orion
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            ORION_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Orion Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."orion/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `orion` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.orion-agent.installPackage was removed. Orion now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.orion-agent.enable = ${lib.boolToString (value != false)};  # the orion CLI, and ORION_HOME for your shells
        programs.orion-agent.desktop.enable = true;  # the desktop application

      `services.orion-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "orion-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "orion-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "orion-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "orion-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "orion-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # ORION_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Orion make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${orionHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Orion
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${orionHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${orionHome}/config.yaml
            ${run}chmod ${modes.config} ${orionHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "orion-managed" managedSystem} ${orionHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${orionHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${orionHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${orionHome}/auth.json"
        else
          ''
            if [ ! -e ${orionHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${orionHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree orionHome cfg.orionHomeFiles}
      ...
      ${run}find ${orionHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "orion-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${orionHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/orion"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps orion as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "orion-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "orion-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "orion-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          ORION_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export ORION_DASHBOARD_SESSION_TOKEN

          if [ -z "$ORION_DASHBOARD_SESSION_TOKEN" ]; then
            echo "orion-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "orion-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "orion-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "orion-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "orion-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "orion-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "orion-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Orion Agent web dashboard and desktop backend"
    else
      "Orion Agent backend for Orion Desktop";

  # The environment that each Orion process needs, from either module.
  #
  # managedSystem gives the value of ORION_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Orion an identity and a memory, use
          ${optionPath}.orionHomeFiles instead. Those files go to
          ORION_HOME. Orion reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of ORION_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"

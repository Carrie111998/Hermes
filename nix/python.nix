...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Orion derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            orion-agent = prev.orion-agent.overrideAttrs (_old: {
              ORION_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $ORION_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ORION_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        orion-agent = prev.orion-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "orion-agent-env" {
    orion-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "orion-agent-editable-env" {
    orion-agent = dependency-groups;
  };
}

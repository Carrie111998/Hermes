        ...
        try:
            ...
            if pull_result.returncode != 0:
                ...
                if _cur_branch and _cur_branch != branch:
                    ...
                    #
                    # Divergence comes in two shapes: ordinary (a common
                    # ancestor still exists — the reset below is a normal,
                    # safe rebase-onto-remote) and orphan (no common ancestor
                    # at all, e.g. a corrupted local HEAD or a repo re-init —
                    # see #87694). In the orphan case `reset --hard` would
                    # silently discard the entire local commit graph with no
                    # way back, so park pre_pull_sha behind a rescue ref first.

...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if behind == 0:
        ...
        # produced the count above, so the list is always consistent with the
        # number (upstream/main when available, origin/main as fallback, or
        # origin/<branch> for --branch). --max-count bounds the git process
        # itself rather than fetching every pending commit and truncating in
        # Python.
        try:
            log_result = subprocess.run(
                git_cmd + [
                    "log", "--oneline", "--no-decorate", "--max-count=10",
            ...
            if log_result.returncode == 0:
                ...
                if log_lines:
                    ...
                    for line in log_lines:
                        print(f"    • {line}")
                    if behind > len(log_lines):
                        print(f"    … and {behind - len(log_lines)} more commit(s)")
        except Exception:

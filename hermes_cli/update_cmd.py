...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # A checkout on a real branch is updated IN PLACE, whatever its name.
        # The historical behaviour switched to the update target and updated
        # THAT, silently abandoning the user's branch (and, before the
        # divergence guard below, discarding its commits via reset --hard).
        #
        ...
        #
        # The two concerns are distinguished by whether the branch carries
        # work of its own, which is exactly what the guard already measures:
        #
        #   fully merged  -> a stale leftover with nothing to lose: switch
        #                    back to the target (parked-branch guard).
        #   unmerged: N   -> a real branch the user is working on: update it
        #                    IN PLACE from origin/<target> so the running
        #                    code advances AND the local commits survive.
        #                    Never switch: an update must not change what the
        #                    user has checked out.
        #   anything else -> dirty / unverifiable / opted out: touch nothing,
        #                    warn loudly, and stop before the post-update
        #                    steps reinforce a stale tree.
        #
        # Only a detached HEAD — where there is no branch to preserve — keeps

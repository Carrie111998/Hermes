...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        try:
            ...
            # still hit EOF (stdin closed mid-prompt), or stdin sent bytes
            # that are not valid text. Same treatment as non-interactive --
            # never a permanent decision. KeyboardInterrupt is handled
            # separately below so Ctrl+C keeps its own "Cancelled." message.
            print()
            logger.info(

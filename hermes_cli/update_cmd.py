...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/kotasiddharth/hermes-agent/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/kotasiddharth/hermes-agent.git",
    "git@github.com:kotasiddharth/hermes-agent.git",
    "https://github.com/kotasiddharth/hermes-agent",
    "git@github.com:kotasiddharth/hermes-agent",
}

OFFICIAL_REPO_URL = "https://github.com/kotasiddharth/hermes-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your repository is not tracking the configured Hermes repository.")
        print("  This means you may miss updates from kotasiddharth/hermes-agent.")
        print()
        try:
            response = (
                input("Add configured repo as 'upstream' remote? [Y/n]: ").strip().lower()
            )
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/kotasiddharth/hermes-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/kotasiddharth/hermes-agent.git' to add later."
            )

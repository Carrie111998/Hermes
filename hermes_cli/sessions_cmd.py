...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "list":
        ...
        # Manual retitle entry point — mirrors the /retitle slash handler but
        # for any historical session, not just the current one. Reuses the
        # shared retitle_session worker so trigger surface (auto / slash /
        # CLI) all write through the same code path.
        from agent.title_generator import retitle_session, _retitle_config
        ...
        # Pull the last N user+assistant messages (skip tool rows). Fetch
        # 2N to account for interleaved tool messages the worker filters out.
        ...
        if dry_run:
            # Dry-run mode: show what would happen without calling the LLM.
            # The actual title candidate requires the LLM call, so we surface
            # the guardrails instead of the candidate.

"""Safe Charterforge Console command engine.

This module backs ``hermes console`` and is intentionally narrower than the
full Charterforge CLI. It exposes a curated set of native adapters that can later be
...
class HermesConsoleEngine:
    """Curated line-command executor for Charterforge Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Charterforge Console does not run shell syntax. Use one supported "
                    "Charterforge command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Charterforge component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Charterforge logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Charterforge configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Charterforge Console."""
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Charterforge configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Charterforge Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Charterforge Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`hermes {first}` is not available in Charterforge Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Charterforge Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Charterforge Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Charterforge Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Charterforge Console.",
            ("skills", "publish"): "`skills publish` is not available in Charterforge Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Charterforge Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Charterforge Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Charterforge Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Charterforge Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Charterforge Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Charterforge Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Charterforge Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Charterforge Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Charterforge Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Charterforge Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Charterforge Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Charterforge Console.")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Charterforge Console.")
...
def run_console_repl(
    *,
    ...
    engine = HermesConsoleEngine()
    if interactive:
        print("Charterforge Console. Type `help` for commands, `exit` to quit.", file=stdout)

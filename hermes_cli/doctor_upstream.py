...
Pure READONLY inspection of:
  ...
  - scope health (UH1..UH5, UH9);
  - safety of a future update operation (UH10).

This module never invokes a mutating Git command. It uses an explicit
allowlist (:data:`READONLY_GIT_SUBCOMMANDS`) and only ever calls ``git`` via
argv through ``subprocess.run`` (no shell), captures stdout/stderr, and has a
hard timeout. Anything that does not match the allowlist raises
:class:`GitCommandForbidden`.

Diagnostic-only: no network, no fetch, no persistence, no working-tree writes.
``run_upstream_health`` is the single entry point and returns a serializable
result suitable for ``render_text`` / ``render_compact`` / ``serialize_json``.
"""
...
# Subcommands that may be invoked. Anything outside this set is forbidden at
# runtime — see ``_run_git``. This is the *only* surface through which the
# module touches Git; it never uses ``shell=True``.
...
# Subcommands explicitly forbidden even if someone extends the allowlist by
...
def _run_git(args: Iterable[str], *, cwd: Optional[str] = None,
    ...
    try:
        ...
        raise GitCallError(
            ...
            f"{timeout or GIT_COMMAND_TIMEOUT_SECONDS}s",
            argv=argv,
        ) from e
...
# The behavior Hermes *actually* exhibits today. This is the canonical profile
# passed into ``update_safety_check`` when a profile isn't supplied (e.g. from
# tests that need to inject a different one).
...
# --------------------------------------------------------------------------- #
# Critical paths (heuristic, conservative). A mutual path on any of these
# surfaces but does not itself cause BLOCKED — only the update_safety rules
# can produce BLOCKED.
# --------------------------------------------------------------------------- #
    ...
    *, branch: str, cwd: Optional[str] = None,
    ...
    for hint in (
        ...
        if hint.endswith("/HEAD"):
            ...
            # Local test repos and some clones do not materialize origin/HEAD.
            # Fall through to origin/main/master.
            continue
...
def _divergence_info(cwd: Optional[str], upstream_ref: Optional[str]) -> DivergenceInfo:
    ...
    if merge_base_sha and head_sha and merge_base_sha == head_sha:
        ...
        # commit; use wall-clock "now" minus the oldest unique commit's
        # timestamp.
...
def update_safety_check(
    ...
    *,
    ...
    Mutual paths never directly cause BLOCKED. WARN never changes exit code.
    ``requires_manual_confirmation`` is True whenever a PASS result is given
    for a non-trivial ahead-only or branch-switch case.
    """
    reasoning: list[str] = []
    ...
    # BranchHealth; we still need to BLOCKED because the operation can reset work.

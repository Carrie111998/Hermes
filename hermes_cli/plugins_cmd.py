...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Charterforge subprocess resolution: :func:`shutil.which` first,
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Charterforge.",
                ) from None
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Charterforge plugin.",
        )
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Charterforge core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Charterforge

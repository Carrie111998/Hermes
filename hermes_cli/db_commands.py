...
def cmd_upgrade(args: argparse.Namespace) -> int:
    ...
    For Postgres, runs Alembic migrations.
...
def cmd_downgrade(args: argparse.Namespace) -> int:
    ...
    if backend == "postgres":
        # For now, downgrade is not implemented
        # Future: support alembic downgrade steps
        print("ERROR: Downgrade not yet implemented for Postgres", file=sys.stderr)
        print("  Run migrations manually: alembic downgrade -1", file=sys.stderr)
        return 1
    
    else:
        print("SQLite does not support schema downgrade. Re-init to reset.")
        return 1

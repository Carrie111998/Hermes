...
class PluginContext:
    ...
    def register_command(
        ...
        ``session_id`` (the persisted physical id, or ``None`` before a session
        exists), durable ``session_key``, and ``platform``); legacy one-argument handlers remain
        supported unchanged.
...
def invoke_plugin_command(handler: Callable, raw_args: str, **context: Any) -> Any:
    ...
    metadata such as ``session_id`` (which may be ``None`` before session
    creation), durable ``session_key``, and ``platform``. Signature inspection is

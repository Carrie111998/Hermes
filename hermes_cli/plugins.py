...
VALID_HOOKS: Set[str] = {
    ...
    "pre_llm_call",
    "post_llm_call",
    # Promise gate. Fired when the agent is about to finish a turn with a
    # textual answer. A callback can keep the turn going by returning:
    #   {"action": "continue", "message": "<internal follow-up>"}
    # This is intentionally similar to pre_verify, but scoped to natural
    # language future commitments ("I will do X next") rather than code edits.
    "promise_gate",
...
def get_promise_gate_continue_message(
    *,
    session_id: str = "",
    platform: str = "",
    model: str = "",
    ...
    """Check ``promise_gate`` hooks for a directive to keep the agent going.

    A hook keeps the same turn going by returning::

        {"action": "continue", "message": "<follow-up for the model>"}

    The first directive carrying a non-empty message wins. This gives plugins
    a zero-user-interaction seam for "I will do X" promise confirmation: the
    attempted final answer is surfaced as interim content, then the model is
    immediately prompted to execute/register/cancel the promise before the
    turn completes.
    """
    hook_results = invoke_hook(
        "promise_gate",

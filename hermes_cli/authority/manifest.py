...
def _decision(
    ...
    *,
    ...
    return AuthorityDecision(
        ...
        policy_version=artifact.manifest.policy_version,
        manifest_sha256=artifact.manifest_sha256,
    )


def evaluate_authority_operation(
    artifact: AuthorityManifestArtifact, request: AdmissionRequest
) -> AuthorityDecision:
    ...
    if operation is None:
        return _decision(artifact, request, "unsupported_operation", None)

    if request.actor_class not in operation.allowed_actor_classes:
        return _decision(artifact, request, "actor_forbidden", matched_rule)

    if request.resource_state not in operation.allowed_resource_states:
        return _decision(artifact, request, "resource_state_denied", matched_rule)
    ...
    if any(
        ...
        return _decision(artifact, request, "missing_capability", matched_rule)

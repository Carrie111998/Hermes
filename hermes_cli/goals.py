...
def resolve_goal_input(
    ...
    *,
    ...
    Shared by the Classic CLI, TUI/Desktop/Dashboard Chat backend, and
    messaging gateway so file loading and inline contract parsing give one
    interpretation everywhere. ``raw`` is the argument string after known ``/goal``
    ...
    if not allow_file:
        raise GoalInputError(
            "Reading a goal from a file is only available in the local CLI, "
            "TUI, Desktop, and Dashboard Chat — not from messaging platforms."
        )

...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        # ``disabled_toolsets`` is applied at *tool* granularity, so a toolset
        # that shares any tool with a kept one would silently strip that shared
        # tool from the blank-slate surface. Drop such overlapping candidates —
        # this covers read-only subsets like ``file_readonly`` (read_file +
        # search_files ⊂ ``file``) the same way the posture skip above covers
        # ``coding`` (#57315, #58281).
        kept_tools: set = set()
        for ts in keep:
            try:

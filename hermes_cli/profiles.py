...
def remove_wrapper_script(name: str, *, target: Optional[str] = None) -> bool:
    """Remove the wrapper script for a profile. Returns True if removed."""
    wrapper_dir = _get_wrapper_dir()
    canon = normalize_profile_name(name)
    target_canon = normalize_profile_name(target) if target is not None else None
        ...
        if wrapper_path.exists():
            try:
                ...
                if target_canon is not None:
                    if _extract_wrapper_target(content, is_windows=is_windows) != target_canon:
                        continue
                elif "hermes -p" not in content:
                    continue
                wrapper_path.unlink()
                return True
            except Exception:
...
def _extract_wrapper_target(content: str, *, is_windows: bool) -> Optional[str]:
    """Return the profile target from a wrapper generated by Hermes."""
    for line in content.splitlines():
        if is_windows:
            parts = line.strip().split()
            if len(parts) == 4 and parts[:2] == ["hermes", "-p"] and parts[3] == "%*":
                return normalize_profile_name(parts[2])
            continue
        ...
        if (
            ...
            return normalize_profile_name(parts[3])
    return None


def _iter_wrapper_alias_targets() -> Iterator[tuple[str, str]]:
    """Yield ``(alias, canonical_profile)`` pairs for Hermes wrapper scripts."""
    wrapper_dir = _get_wrapper_dir()
    ...
    for entry in sorted(wrapper_dir.iterdir()):
        ...
        target = _extract_wrapper_target(content, is_windows=is_windows)
        if target is None:
            continue
...
def _retarget_custom_wrapper_aliases(old_name: str, new_name: str) -> list[str]:
    """Update every custom wrapper that points at a renamed profile."""
    updated: list[str] = []
    for alias, target in _iter_wrapper_alias_targets():
        if alias == old_name or target != old_name:
            continue
        if create_wrapper_script(alias, target=new_name):
...
def build_alias_map() -> dict[str, str]:
    ...
    """
    result: dict[str, str] = {}
    for alias, canon in _iter_wrapper_alias_targets():
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    remove_wrapper_script(old_canon, target=old_canon)
    updated_aliases = _retarget_custom_wrapper_aliases(old_canon, new_canon)
    if updated_aliases:
        print(f"✓ Updated custom aliases: {', '.join(updated_aliases)}")

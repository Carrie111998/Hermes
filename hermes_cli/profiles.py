"""
Profile management for multiple isolated Charterforge instances.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Charterforge infrastructure directories that only the default profile
...
# into the export. The set below identifies the *known Charterforge profile
...
# Charterforge subcommands that cannot be used as profile names/aliases
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Charterforge installation itself or a common system binary.  "
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Charterforge. If we
...
# ``config.yaml``. ``config.yaml`` is the user-facing Charterforge config
...
def create_profile(
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Charterforge profile.\n"
    ...
    # Cloned configs can be older than the running Charterforge (or predate schema
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Charterforge *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Charterforge backend subcommand
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Charterforge process: either an entrypoint marker in argv, or
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      first and proved unable to anticipate every non-Charterforge file the

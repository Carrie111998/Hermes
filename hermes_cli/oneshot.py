...
def _run_agent(
    ...
    try:
        ...
        # MCP discovery runs in the background so provider/config setup can
        # overlap with connection startup. Before AIAgent takes its one-time
        # tool snapshot, join that thread using the existing bounded timeout.
        # Without this gate, one-shot sessions nondeterministically omit MCP
        # tools depending on which side wins the startup race.
        wait_for_mcp_discovery()

...
def update_claim_belongs_to_current_process(holder: UpdateHolder | None) -> bool:
    """Return whether *holder* is the update transaction we run under.

    Web-bundle builders use this to honor the install-wide update marker
    without deadlocking builds intentionally launched by that same updater.
    The ownership rules deliberately match :meth:`UpdateLock.acquire`: our
    own PID, the explicit Tauri handoff PID, or a live process ancestor is the
    current transaction.  Missing or unprovable ownership fails closed.
    """

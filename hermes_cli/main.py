...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    try:
        ...
        def first_resumable(sessions) -> Optional[str]:
            for session in sessions:
                session_id = session.get("id")
                if not session_id or session_id in skipped:
                    continue
                if callable(archive_stale) and archive_stale(session_id):
                    skipped.add(session_id)
                    continue
                return session_id
            return None
        ...
        if ws_key:
            session_id = first_resumable(
                db.search_sessions(source=source, limit=20, workspace_key=ws_key)
            )
            if session_id:
                return session_id
        # Fallback: global MRU for this source.
        return first_resumable(db.search_sessions(source=source, limit=20))
    except Exception:
        pass
    finally:
        if db is not None:

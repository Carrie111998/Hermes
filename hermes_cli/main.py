...
def cmd_dashboard(args):
    ...
    # 2026-04-17: skip rebuild if the web UI is already built (avoids the
    # subprocess.run([npm, "install"]) returning nonzero on Windows when the
    # build is already valid). Also skip if HERMES_WEB_DIST is set (external
    # override) or if --skip-build was passed (upstream v2026.5.16).
    ...
    elif "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False) and not _already_built:

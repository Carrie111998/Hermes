...
def cmd_chat(args):
    ...
    if in_dir:
        _expanded_in_dir = os.path.expanduser(in_dir)
        if in_dir == "~" or in_dir.startswith(("~/", "~\\")):
            _home = os.environ.get("HOME", "").strip()
            if _home:
                _expanded_in_dir = (
                    _home if in_dir == "~" else os.path.join(_home, in_dir[2:])
                )
        _target_dir = os.path.abspath(_expanded_in_dir)

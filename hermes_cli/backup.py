...
# Ceiling for the sqlite3 backup watchdog. ``Connection.backup()`` loops while
# the source reports SQLITE_BUSY and does not accumulate the connection's busy
# timeout across retries, so a DB locked by another process (e.g. a live
# Chromium profile DB opened with exclusive locking) makes it spin forever.
# The watchdog interrupts the source connection after this long. Legitimate
# multi-GB database snapshots finish far below it.
_SQLITE_BACKUP_WATCHDOG_SECONDS = 900.0
...
def _safe_copy_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        # Fail fast on a locked source: a plain statement honors the busy
        # timeout (default 5s) and raises, while ``conn.backup()`` would retry
        # SQLITE_BUSY forever (see _SQLITE_BACKUP_WATCHDOG_SECONDS).
        conn.execute("SELECT 1")
        deadline = time.monotonic() + _SQLITE_BACKUP_WATCHDOG_SECONDS

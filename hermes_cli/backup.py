...
class _CredentialSnapshotError(RuntimeError):
    pass
...
def _write_full_zip_backup(
    ...
    try:
        with _backup_operation_lock(hermes_root):
            return _write_full_zip_backup_locked(
                out_path,
                hermes_root,
...
def _write_full_zip_backup_locked(
    ...
    logger.info(
        ...
        (time.monotonic() - scan_started) * 1000,
        len(files_to_add) + len(auth_to_add),
    )
        ...
        with _atomic_output_path(out_path) as archive_path, zipfile.ZipFile(
            ...
            for credential in auth_to_add:
                try:
                    ...
                    raise _CredentialSnapshotError(
                        str(credential.archive_path)
                    ) from exc
    except (OSError, _SQLiteSnapshotError, _CredentialSnapshotError) as exc:

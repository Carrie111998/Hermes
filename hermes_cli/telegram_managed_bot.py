...
manual BotFather token copy-paste. Charterforge talks only to the Nous onboarding
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Charterforge Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Charterforge Telegram onboarding service.")

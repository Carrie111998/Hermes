"""
Charterforge Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Charterforge.app`` or ``~/Applications/Charterforge.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Charterforge`` (NSIS per-user)
...
the app name ("Charterforge"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Charterforge``
  - Windows: ``%APPDATA%\\Charterforge``
  - Linux:   ``$XDG_CONFIG_HOME/Charterforge`` (default ``~/.config/Charterforge``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Charterforge"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Charterforge"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Charterforge" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Charterforge.app"),
            home / "Applications" / "Charterforge.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Charterforge).
            local_base / "Programs" / "Charterforge",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Charterforge")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Charterforge.desktop",
        ]
    return paths

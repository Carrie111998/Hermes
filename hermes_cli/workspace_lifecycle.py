...
class Registry:
    ...
    def open(self) -> sqlite3.Connection:
        ...
        try:
            ...
            conn.execute("""CREATE TABLE IF NOT EXISTS workspace_observations (
                evidence_hash TEXT PRIMARY KEY, workspace_id TEXT, payload TEXT NOT NULL
            )""")
            conn.execute("""CREATE TABLE IF NOT EXISTS workspace_receipts (
                receipt_hash TEXT PRIMARY KEY, workspace_id TEXT NOT NULL, operation TEXT NOT NULL,
                payload TEXT NOT NULL
            )""")
            # A process can die after reserving PREPARING.  Reopening the sole
            # authority reconciles that incomplete transition conservatively;
            # it never resumes or deletes the workspace.
            stale_rows = conn.execute(
                "SELECT id, reasons FROM workspaces WHERE state=?",
                (WorkspaceState.PREPARING.value,),
            ).fetchall()
            for workspace_id, encoded_reasons in stale_rows:
                reasons = json.loads(encoded_reasons)
                if "interrupted_preparing" not in reasons:
                    reasons.append("interrupted_preparing")
                conn.execute(
                    "UPDATE workspaces SET state=?, disposition=?, reasons=? WHERE id=?",
    ...
    def create_or_get(self, *, workspace_id: str, idempotency_key: str, evidence: Evidence) -> dict[str, Any]:
        ...
        try:
            ...
            if existing:
                ...
                prior_evidence = prior["record"].get("evidence", {})
                requested = json.loads(evidence.canonical_json())
                identity_fields = ("canonical_path", "repo_common_dir", "head", "branch")
                if any(prior_evidence.get(name) != requested.get(name) for name in identity_fields):
                    raise RuntimeError("idempotency conflict: key maps to different workspace identity")
                conn.execute("COMMIT")
                return prior
            record = json.dumps({"schema_version": SCHEMA_VERSION, "evidence": json.loads(evidence.canonical_json())}, sort_keys=True, separators=(",", ":"))
            conn.execute("INSERT INTO workspaces VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                         (workspace_id, idempotency_key, str(Path(evidence.canonical_path).resolve(strict=False)),
                          ...
                          Disposition.BLOCKED_REVIEW.value, json.dumps(["preparing"]),
                          evidence.observation_hash, record))
            ...
            conn.execute("UPDATE workspaces SET state=?, disposition=?, reasons=?, evidence_hash=? WHERE id=?",
                         (decision.state.value, decision.disposition.value, json.dumps(decision.reasons),
                          decision.evidence_hash, workspace_id))
            conn.execute("COMMIT")
            return self.get(workspace_id) or {"id": workspace_id, **decision.as_dict()}
        except Exception:
    ...
    def reconcile_preparing(self) -> int:
        ...
        try:
            conn.execute("BEGIN IMMEDIATE")
            rows = conn.execute("SELECT id, reasons FROM workspaces WHERE state=?", (WorkspaceState.PREPARING.value,)).fetchall()
            for workspace_id, reasons in rows:
                ordered = json.loads(reasons)
                if "interrupted_preparing" not in ordered:
                    ordered.append("interrupted_preparing")
                conn.execute("UPDATE workspaces SET state=?, disposition=?, reasons=? WHERE id=?",
                             (WorkspaceState.BLOCKED_REVIEW.value, Disposition.BLOCKED_REVIEW.value,
                              json.dumps(ordered), workspace_id))
            conn.execute("COMMIT")
            return len(rows)
        except Exception:
            conn.execute("ROLLBACK")
    ...
    def receipt(self, workspace_id: str, operation: str, payload: dict[str, Any], *, receipt_hash: str | None = None) -> str:
        ...
        try:
            ...
            conn.execute("INSERT OR IGNORE INTO workspace_receipts VALUES (?, ?, ?, ?)",
                         (digest, workspace_id, operation, encoded))
        finally:
            conn.close()
        return digest

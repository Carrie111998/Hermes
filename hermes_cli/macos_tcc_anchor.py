...
def _bundle_libpython(venv_dir: Path, source_file: Path) -> Path | None:
    """Copy the store's libpython dylib into ``venv/lib/``.

    The anchored copy keeps its build-time ``LC_RPATH`` of
    ``@executable_path/../lib`` — valid inside the uv store, but inside a venv
    that dir holds only ``python3.x/site-packages``, so without libpython the
    anchor dies in dyld before ``main`` (issue #95425).  A real copy (not a
    symlink) keeps the anchor bootable even after the store generation it came
    from is garbage-collected.
    """
    ...
    for dylib in store_lib.glob("libpython3.*.dylib"):
        ...
        try:
            if target.is_file() and not target.is_symlink():
                if target.stat().st_size == dylib.stat().st_size:
                    continue
            tmp = venv_lib / f".{dylib.name}.tcc-tmp"
            tmp.unlink(missing_ok=True)
            shutil.copy2(dylib, tmp)
            os.replace(tmp, target)
            return target
        except OSError as exc:
            logger.warning("libpython bundle for %s failed: %s", dylib.name, exc)
            continue
    return None


def _verify_anchor_boot(venv_py: Path, env: dict[str, str]) -> bool:
    """Boot the staged anchor copy before it goes live.

    The failure this guards against (#95425) is invisible to anything except
    actually loading the binary: dyld resolves the rpath, loads libpython and
    the stdlib, and only then does the interpreter reach ``main``.  A staged
    copy that cannot start must never replace the working symlink.
    """
    try:
        probe = subprocess.run(
            [str(venv_py), "-c", "import sys, encodings; sys.exit(0)"],
            env=env,
            capture_output=True,
            timeout=30,
        )
    except (OSError, subprocess.TimeoutExpired) as exc:
        logger.warning("anchor boot verification failed to run: %s", exc)
        return False
    if probe.returncode != 0:
        stderr = probe.stderr.decode(errors="replace")[-500:]
        logger.warning("staged anchor failed to boot: %s", stderr)
        return False
    return True


def _repoint_aliases(venv_bin: Path, source_file: Path) -> None:
    """Point uv alias symlinks at the store interpreter, not the anchor copy.

    Aliases that resolve through the copied anchor lose the venv prefix and
    die looking for the stdlib under the build-time ``/install`` prefix
    (#95541).  Keeping ``python3``/``python3.11`` on the original store
    binary preserves stdlib resolution for every launcher that invokes the
    alias.  Only symlinks that resolve into a managed store are touched.
    """
    ...
    for name in sorted(names):
        ...
        try:
            ...
            # Only aliases that currently resolve into a managed store (or
            # through the anchor copy, which resolves to the venv itself)
            # are ours to touch; anything else is a user-managed link.
            points_at_source = (
                str(resolved) == str(source_file) or resolved == venv_bin / "python"
            )
            if not (_is_uv_macos_store(str(resolved)) or points_at_source):
                continue
...
def _install_anchor(venv_dir: Path, source_file: Path) -> None:
    ...
    interpreter half-written.  The copy is dylib-complete (libpython bundled
    into ``venv/lib/``) and boot-verified before it goes live, so the anchor
    can never replace a working interpreter with one that dies in dyld
    (#95425).  Writes the source marker; versioned alias symlinks stay on the
    store binary so stdlib resolution through them is unchanged (#95541).
    """
    venv_py = venv_python_path(venv_dir)
    ...
    try:
        ...
        # Boot-verify the staged copy against the venv it will live in: the
        # dyld/stdlib failure modes (#95425, #95541) only show up when the
        # binary actually loads.  libpython must be bundled FIRST so the
        # staged copy can resolve its rpath from its final location's parent.
        # PYTHONHOME is deliberately unset: a venv interpreter derives its
        # prefix from pyvenv.cfg next to itself, and forcing PYTHONHOME makes
        # the probe pass while the real (env-less) launch would still fail.
        _bundle_libpython(venv_dir, source_file)
        env = dict(os.environ)
        env.pop("PYTHONHOME", None)
        if not _verify_anchor_boot(tmp_path, env):
            raise RuntimeError(
                "staged anchor copy failed to boot (dyld/libpython); "
                "keeping the existing interpreter"
            )

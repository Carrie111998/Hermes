...
def _endpoint_probe_blocked_reason(url: str) -> Optional[str]:
    ...
    ``create_ssrf_safe_client(allow_private_urls=True)`` so DNS rebinding
...
def _endpoint_probe_client(**kwargs):
    """httpx client for dashboard endpoint probes (LAN ok, metadata blocked)."""
    from tools.url_safety import create_ssrf_safe_client

    kwargs.setdefault("follow_redirects", False)
    kwargs.setdefault("allow_private_urls", True)
    return create_ssrf_safe_client(**kwargs)
...
async def validate_custom_endpoint(body: CustomEndpointUpdate):
    ...
    try:
        ...
        # create_ssrf_safe_client still enforces the metadata floor at connect.
        with _endpoint_probe_client(timeout=httpx.Timeout(8.0)) as client:

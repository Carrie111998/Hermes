...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # On Windows a get_running_pid() cache miss may invoke tasklist, which
        # can take seconds under process pressure. Keep that subprocess off
        # the asyncio loop so status polling cannot starve websocket flushes
        # or unrelated dashboard requests. Use the cached wrapper (upstream)
        # so high-frequency polling doesn't churn file descriptors on every
        # call, while still resolving the active/selected profile's explicit
        # gateway.pid path (fork) instead of silently probing the root profile.
        if _gateway_pid_path is not None:
            gateway_pid = await asyncio.to_thread(get_running_pid_cached, _gateway_pid_path)
        else:
            ...
            gateway_pid = await asyncio.to_thread(get_running_pid_cached)
...
try:
    from agent.anthropic_adapter import (
        ...
        _OAUTH_TOKEN_URL as _ANTHROPIC_OAUTH_TOKEN_URL,  # noqa: F401 - availability probe; the import IS the test
...
def _trigger_cron_job_sync(job_id: str, profile: Optional[str] = None, reason: Optional[str] = None):
    # v0.15.1 catch-up: combine upstream multi-profile routing + threadpool
...
async def trigger_cron_job(job_id: str, profile: Optional[str] = None, reason: Optional[str] = None):
    return await _run_cron_dashboard_io(_trigger_cron_job_sync, job_id, profile, reason)

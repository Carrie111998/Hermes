...
async def speak_stream_ws(ws: "WebSocket") -> None:
    ...
    if not _ws_auth_ok(ws):
        await _reject_ws(ws, code=4401)
        return
    if not _ws_request_is_allowed(ws):
        await _reject_ws(ws, code=4403)
        return
    await ws.accept()
...
async def _reject_ws(
    ...
    *,
    ...
    """Reject a WebSocket while preserving its application close code.

    Starlette turns ``close()`` before ``accept()`` into an HTTP 403 denial.
    Browsers surface that denial as the generic abnormal-close code 1006, so
    clients cannot distinguish terminal auth/host failures from a transient
    network drop and may reconnect forever. Complete the upgrade first, then
    immediately close without exchanging application data so the browser sees
    the intended 44xx code and can stop retrying.
    """
    await ws.accept()
...
async def console_ws(ws: WebSocket) -> None:
    ...
    if not _DASHBOARD_EMBEDDED_CHAT_ENABLED:
        ...
        await _reject_ws(ws, code=4404, reason="embedded chat disabled")
        return
    ...
    if auth_reason is not None:
        ...
        await _reject_ws(
            ws,
            code=4401,
            reason=_ws_close_reason(f"auth: {auth_reason}"),
        )
        return
    ...
    if host_origin_reason is not None:
        ...
        await _reject_ws(
            ws,
            code=4403,
            reason=_ws_close_reason(host_origin_reason),
        )
        return
    ...
    if client_reason is not None:
        ...
        await _reject_ws(
            ws,
            code=4408,
            reason=_ws_close_reason(client_reason),
        )
        return
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    # --- auth + host/origin/peer check (before any application data or PTY
    #     work). Rejections use _reject_ws to finish the handshake solely so
    #     the browser receives the close code + reason instead of generic 1006.

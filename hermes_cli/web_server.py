...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    # A separate authenticated dashboard may bridge an iPad spectator into
    # this loopback-only Desktop backend. The credential is generated by
    # Electron for this one backend process and is always mapped to the same
    ...
    if spectator_relay:
        ...
        if expected and hmac.compare_digest(
            ...
            return None, "spectator"
        return "spectator_relay_mismatch", "spectator"
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    if credential == "spectator":
        from hermes_cli.spectator_relay import (
            has_live_desktop_descriptors,
            relay_spectator_ws,
        )

        # The authenticated iPad dashboard is a separate process. When a live
        # Desktop descriptor exists, relay into that owning process instead of
        # consulting this dashboard's unrelated in-memory session registry.
        # A Desktop backend never relays to itself.
        if not os.environ.get("HERMES_DESKTOP") and has_live_desktop_descriptors():
            await relay_spectator_ws(ws)
            return
...
def start_server(
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            from hermes_cli.spectator_relay import write_desktop_descriptor

            spectator_descriptor = (
                write_desktop_descriptor(actual_port)
                if os.environ.get("HERMES_DESKTOP") == "1"
                else None
            )
            ...
            try:
                ...
                from hermes_cli.spectator_relay import remove_desktop_descriptor

                remove_desktop_descriptor(spectator_descriptor)

...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # PID file can be missing after a crash/restart race while the gateway
        # process is still alive. Fall back to a process scan so Desktop /
        # dashboard status stays truthful (fork: 1f147e98bf).
...
def _provider_models_probe_url(base_url: str) -> tuple[str | None, str | None]:
    """Fork SSRF-hardening advantage: validate/normalize provider probe URLs."""
...
def _provider_probe_block_reason(url: str, *, strict_private: bool = False) -> str | None:
    """Fork SSRF-hardening advantage: block metadata/link-local and private probes."""
    try:
        ...
        for child in sorted(plugins_root.iterdir()):
            ...
            try:
                ...
                if name in seen_names:
                    continue
                seen_names.add(name)
                config_name, config_key = _dashboard_plugin_config_identity(
                    child, str(name)
                )
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "user":
            ...
            if not is_plugin_enabled_by_config(
                ...
                key=str(
                    plugin.get("_key") or plugin.get("_config_name") or plugin["name"]
                ),
            ):
                _log.info(

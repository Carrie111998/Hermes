...
CREATE TABLE IF NOT EXISTS task_runs (
    ...
    claim_expires       INTEGER,
    worker_pid          INTEGER,
    -- Kernel process identity captured immediately after spawn. PID start
    -- time + boot id prevent reuse; SID/PGID identify ordinary descendants
    -- even if they scrub environment markers. Authorization itself executes
    -- only in the live root-gateway PID, so reparenting cannot turn a worker
    -- into an operator.
...
_REBUILD_SPECS = {
    ...
    "task_runs": (
        ...
        " worker_pid INTEGER, worker_pid_started INTEGER, worker_boot_id TEXT,"
        " worker_sid INTEGER,"
        " worker_pgid INTEGER, max_runtime_seconds INTEGER,"
...
def claim_task(
    ...
    *,
    ...
    # active-PR continuation controls by skipping dispatch_once. A continuation
    # grant is verified here immediately before the write transaction, then
    # consumed inside that same transaction after the run row is created.
    has_active_pr = bool(
            ...
            if active_urls:
                ...
                if authorization_id is not None:
                    ...
                    if continuation_authorization is not None:
                        status = continuation_authorization.status(now=authorization_now)
                        if status != "active":
    ...
    with write_txn(conn):
        ...
        if run_id is not None:
            conn.execute(
                ...
                "worker_boot_id = ?, worker_sid = ?, worker_pgid = ? WHERE id = ?",
                (
                    int(pid),
...
def evaluate_respawn_guard(
    ...
    The ordinary rate-limit/auth/recent-success guards are unchanged. An
    ...
    if auth_status != "active":
        return RespawnGuardDecision(
            reason="active_pr",
            continuation_denial=f"authorization_{auth_status}",
...
def check_respawn_guard(
    ...
    return evaluate_respawn_guard(conn, task_id).reason

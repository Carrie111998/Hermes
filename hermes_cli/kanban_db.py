...
def _normalise_completion_verdict(metadata: Optional[dict]) -> tuple[bool, str]:
    """Return ``(present, normalized)`` for completion verdict metadata."""
    ...
    return True, normalized or "MISSING"
...
def _enforce_completion_verdict(
    ...
    *,
    ...
    """Fail closed for review completions and explicit non-PASS verdicts.
    ...
    with write_txn(conn):
        ...
        if context is None:
            return False
        prior_status, current_run_id, is_review = context
        verdict_present, verdict = _normalise_completion_verdict(metadata)
        if (is_review or verdict_present) and verdict != "PASS":
            _append_event(
                conn,
                task_id,
                ...
                {
                    "verdict": verdict,
                    "source_status": "review" if is_review else prior_status,
                },
...
def complete_task(
    ...
    *,
    ...
    callers do not have to pass both. ``metadata`` is a free-form dict except
    that ``verdict`` is reserved for exact-PASS review semantics (e.g.
    ``{"changed_files": [...], "tests_run": [...]}``) — workers are
    encouraged to use it for structured handoff facts.
    ...
    # ordinary completions remain unchanged unless they explicitly submit a
    # verdict, in which case that verdict is also fail-closed.
    ...
    with write_txn(conn):
        ...
        if context is None:
            return False
        prior_status, current_run_id, completion_is_review = context
        verdict_present, verdict = _normalise_completion_verdict(metadata)
        if (completion_is_review or verdict_present) and verdict != "PASS":
            _append_event(
                conn,
                task_id,
                "completion_blocked_nonpassing_verdict",
                {
                    "verdict": verdict,
                    "source_status": (
                        "review" if completion_is_review else prior_status
                    ),
                },
                run_id=current_run_id,
            )
            # A normal return commits the rejection event while leaving the
            # task and dependents untouched. The ordinary preflight path still
            # raises NonPassingVerdictError with actionable caller text.
            return False

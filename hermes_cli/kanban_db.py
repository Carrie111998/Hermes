...
# The lifecycle ledger is deliberately narrower than task_events: these are
# stable pipeline milestones, not the board's internal state-transition log.
LIFECYCLE_LEDGER_SCHEMA_VERSION = 1
VALID_LIFECYCLE_EVENT_TYPES = frozenset({
    "ticket_created",
    "ticket_ready",
    "ticket_started",
    "work_started",
...
_LIFECYCLE_METADATA_MAX_BYTES = 16 * 1024
_LIFECYCLE_METADATA_MAX_DEPTH = 6
_LIFECYCLE_METADATA_MAX_ITEMS = 256
_LIFECYCLE_TEXT_MAX_CHARS = 4096
...
class LifecycleLedgerEvent:
    """One immutable row from lifecycle-ledger schema version 1.

    Rows are ordered by the server-assigned ``id``. ``recorded_at`` is UTC
    generated by :func:`append_lifecycle_event`; callers cannot supply it.
    Duplicate identity tuples return the original row unchanged.
    """
    ...
    @classmethod
    def from_row(cls, row: sqlite3.Row) -> "LifecycleLedgerEvent":
...
-- Version-1 external pipeline lifecycle ledger. This is intentionally
-- separate from task_events so measurement writes cannot change notifier,
-- dispatch, or task-history behavior. Timestamps are supplied only by the
-- core writer API and identity is normalized across nullable PR/SHA values.
CREATE TABLE IF NOT EXISTS kanban_lifecycle_events (
    id             INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_lifecycle_event_identity
    ON kanban_lifecycle_events(
        ticket_id,
        COALESCE(pr_number, -1),
        COALESCE(merge_sha, ''),
        event_type
    );
CREATE INDEX IF NOT EXISTS idx_lifecycle_event_ticket
    ON kanban_lifecycle_events(ticket_id, id);
...
BEFORE UPDATE ON kanban_lifecycle_events
BEGIN
    SELECT RAISE(ABORT, 'kanban lifecycle ledger is append-only');
END;

CREATE TRIGGER IF NOT EXISTS trg_lifecycle_events_no_delete
BEFORE DELETE ON kanban_lifecycle_events
BEGIN
    SELECT RAISE(ABORT, 'kanban lifecycle ledger is append-only');
END;

-- REPLACE deletes a conflicting row without firing DELETE triggers when
-- recursive_triggers is disabled. Reject collisions before conflict handling.
CREATE TRIGGER IF NOT EXISTS trg_lifecycle_events_no_replace
BEFORE INSERT ON kanban_lifecycle_events
WHEN EXISTS (
    SELECT 1 FROM kanban_lifecycle_events
    WHERE id = NEW.id
       OR (ticket_id = NEW.ticket_id
           AND pr_number IS NEW.pr_number
           AND merge_sha IS NEW.merge_sha
...
def _normalize_lifecycle_metadata(metadata: Optional[Mapping[str, Any]]) -> Optional[dict]:
    if metadata is None:
        return None
    ...
    def normalize(value: Any, depth: int) -> Any:
        nonlocal item_count
        if depth > _LIFECYCLE_METADATA_MAX_DEPTH:
            raise ValueError(
                f"metadata exceeds maximum depth {_LIFECYCLE_METADATA_MAX_DEPTH}"
            )
        item_count += 1
        if item_count > _LIFECYCLE_METADATA_MAX_ITEMS:
            raise ValueError(
                f"metadata exceeds maximum item count {_LIFECYCLE_METADATA_MAX_ITEMS}"
            )
        ...
        if isinstance(value, str):
            if len(value) > _LIFECYCLE_TEXT_MAX_CHARS:
                raise ValueError(
                    f"metadata strings must not exceed {_LIFECYCLE_TEXT_MAX_CHARS} characters"
                )
            return value
        ...
        raise ValueError("metadata values must be JSON objects, arrays, or scalar values")

    normalized = normalize(metadata, 0)
    encoded = json.dumps(
        normalized,
        ensure_ascii=False,
        allow_nan=False,
    ...
    if len(encoded.encode("utf-8")) > _LIFECYCLE_METADATA_MAX_BYTES:
        raise ValueError(
            f"metadata must not exceed {_LIFECYCLE_METADATA_MAX_BYTES} encoded bytes"
        )
    return normalized


def _lifecycle_utc_now() -> str:
...
def append_lifecycle_event(
    ...
    *,
    ...
    """Validate and append one version-1 pipeline lifecycle event.

    The server writer assigns the UTC ``recorded_at`` timestamp. Identity is
    transactionally unique by ``(ticket_id, pr_number-or-null,
    merge_sha-or-null, event_type)``; a duplicate retry returns the original
    immutable row, including its timestamp and metadata. The independent
    table and API preserve all existing ``task_events`` behavior.
    """
    if not isinstance(ticket_id, str) or not ticket_id.strip():
        raise ValueError("ticket_id must be a non-empty string")
    ticket_id = ticket_id.strip()
    if len(ticket_id) > 128:
        raise ValueError("ticket_id must not exceed 128 characters")

    if not isinstance(event_type, str) or event_type not in VALID_LIFECYCLE_EVENT_TYPES:
        raise ValueError(f"unsupported lifecycle event_type: {event_type!r}")

    if pr_number is not None:
        if (
            isinstance(pr_number, bool)
            or not isinstance(pr_number, int)
            or not 1 <= pr_number <= _SQLITE_MAX_INTEGER
        ):
            raise ValueError(
                f"pr_number must be an integer from 1 through {_SQLITE_MAX_INTEGER}"
            )

    if merge_sha is not None:
        if not isinstance(merge_sha, str):
            raise ValueError("merge_sha must be a 40- or 64-character hexadecimal string")
        merge_sha = merge_sha.strip().lower()
        if not re.fullmatch(r"(?:[0-9a-f]{40}|[0-9a-f]{64})", merge_sha):
            raise ValueError("merge_sha must be a 40- or 64-character hexadecimal string")

    normalized_metadata = _normalize_lifecycle_metadata(metadata)
    encoded_metadata = (
        json.dumps(
            normalized_metadata,
            ensure_ascii=False,
    ...
    with write_txn(conn):
        row = conn.execute(
            "SELECT * FROM kanban_lifecycle_events "
            "WHERE ticket_id = ? AND pr_number IS ? AND merge_sha IS ? AND event_type = ?",
            (ticket_id, pr_number, merge_sha, event_type),
        ).fetchone()
        if row is None:
            cursor = conn.execute(
                "INSERT INTO kanban_lifecycle_events "
                "(schema_version, ticket_id, event_type, pr_number, merge_sha, metadata, recorded_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
            ...
            row = conn.execute(
                "SELECT * FROM kanban_lifecycle_events WHERE id = ?",
                (cursor.lastrowid,),
            ).fetchone()
        if row is None:  # pragma: no cover - guarded by schema + transaction
            raise sqlite3.IntegrityError("lifecycle event insert did not produce a row")
        result = LifecycleLedgerEvent.from_row(row)
    return result

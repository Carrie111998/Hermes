...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    # Cache only the exact generation left in a corrupt state. A replacement or
    # subsequent edit changes its identity and forces the next open to re-probe.
    final_identity = _db_file_identity(resolved)
...
def create_task(
    ...
    *,
    ...
    # duplicate. The lookup is repeated under BEGIN IMMEDIATE below so two
    # concurrent creators with the same key cannot both insert.
    if idempotency_key:
        row = conn.execute(
...
def _append_event(
    ...
    *,
    ...
    event_id = int(cur.lastrowid or conn.execute("SELECT last_insert_rowid()").fetchone()[0])
    try:
...
def _default_spawn(
    ...
    *,
    ...
    # doing the task. `--cli` is the highest-precedence interface override;
    # dropping the env var also covers older Hermes builds on PATH.
    ...
    if external_harness:
        ...
        if task.goal_mode:
            ...
            # the kanban goal-loop hook only runs in cli.py's quiet branch.
            cmd.append("-Q")

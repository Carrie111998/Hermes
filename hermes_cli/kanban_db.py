...
def claim_review_task(
    ...
    *,
    ...
    Like ``claim_task``, this rechecks parent dependencies transactionally.
    Review status can outlive the evidence that originally satisfied the
    task, so stale approval must not grant a new worker lease.
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    for row in stale:
        ...
        if _worker_survived_termination(termination):
            ...
            continue
        with write_txn(conn):
            requeue_status = _dependency_requeue_status(conn, row["id"])
            cur = conn.execute(
...
def reclaim_task(
    ...
    *,
    ...
    with write_txn(conn):
        requeue_status = _dependency_requeue_status(conn, task_id)
        cur = conn.execute(
            "UPDATE tasks SET status = ?, claim_lock = NULL, "
            "claim_expires = NULL, worker_pid = NULL "
...
def complete_task(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        if run_id is None and (summary or metadata or result or outcome_code):
...
def unblock_task(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    with write_txn(conn):
        ...
        # kanban/boards/cookai/workspaces/t_a6acd07d/root-cause.md.
        new_status = "todo" if dependency_blockers(conn, task_id) else "ready"

    ...
    *,
    ...
    with write_txn(conn):
        conn.execute(
            """
            INSERT OR IGNORE INTO kanban_notify_subs
                (subscription_id, task_id, platform, chat_id, chat_type, thread_id, user_id,
...
def stage_unseen_notifications_for_sub(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        task = conn.execute(
            "SELECT title, assignee, result, session_id FROM tasks WHERE id = ?",
            (sub["task_id"],),
        ).fetchone()
        ...
        for row in rows:
            ...
            if isinstance(event_payload, dict):
                payload_fields = {
                    ...
                    "blocked": ("reason",),
                    "block_loop_detected": ("reason", "recurrences"),
                    "gave_up": ("error",),

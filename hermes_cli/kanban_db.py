...
# ---------------------------------------------------------------------------
# Re-exported extraction modules (godfile decomposition, wave 1, shard s5)
#
# The functions below were moved VERBATIM into their extraction modules
# (``notify_subs_mixin.py``, ``worker_spawn_mixin.py``). Re-importing them
# here keeps the ``kanban_db.<name>`` public API surface — used by
# ``kanban.py`` / ``kanban_decompose.py`` / ``kanban_specify.py`` /
# ``kanban_swarm.py``, the gateway kanban-notifier, and the test suite —
# unchanged. These imports sit at the bottom of the file so the extraction
# modules can bind the shared helpers they need from this module without a
# circular-import deadlock.
# ---------------------------------------------------------------------------

from hermes_cli.notify_subs_mixin import (  # noqa: E402,F401
...
from hermes_cli.worker_spawn_mixin import (  # noqa: E402,F401

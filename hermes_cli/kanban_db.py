    ...
    * ``transient`` — treated like a generic block for routing, but a worker
    ...
    with write_txn(conn):
        ...
        if kind == "dependency":
            ...
            # Deliberately no lifecycle dispatch here: fall through to the
            # shared post-transaction dispatch below so kanban_task_blocked
            # fires only after write_txn has committed and released the write
            # lock, exactly like the ordinary and loop-detected branches.

...
# ---------------------------------------------------------------------------
# Re-exports from extracted sibling modules (godfile shard-s2 extraction)
# ---------------------------------------------------------------------------
# The functions below were moved VERBATIM to sibling modules
# (hermes_cli/kanban_db_tasks.py, hermes_cli/kanban_db_events.py,
# hermes_cli/kanban_db_links.py, hermes_cli/kanban_db_attachments.py).
# They are re-exported here so ``import hermes_cli.kanban_db as kb`` and
# ``from hermes_cli.kanban_db import X`` callers are unaffected. Imports live
# at the bottom of both sides of the cycle so the module pair resolves in any
# import order. Order matters: kanban_db_events must be imported first
# (it binds ``_append_event``, which the tasks/links/attachments siblings
# import through this module's re-exports), then kanban_db_tasks (binds the
# task helpers), then links and attachments.

from hermes_cli.kanban_db_events import (  # noqa: E402
    _append_event,
    _current_run_id,
    _end_run,
...
from hermes_cli.kanban_db_tasks import (  # noqa: E402
    _canonical_assignee,
...
from hermes_cli.kanban_db_links import (  # noqa: E402
    _would_cycle,
    child_ids,
    link_tasks,
    parent_ids,

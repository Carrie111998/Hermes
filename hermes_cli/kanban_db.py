...
def recompute_ready(
    ...
    1. The most recent terminal stop event was a worker/operator
       ``kanban_block`` or a circuit-breaker ``gave_up`` — those stay
       blocked until an explicit ``kanban_unblock`` (#28712).  Without
       that guard, a review-required handoff or max-failure breaker can
       auto-respawn, the fresh worker hits the same deterministic
       failure, and the loop repeats indefinitely.

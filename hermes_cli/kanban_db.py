        ...
        for offset, run in enumerate(shown):
            ...
            if run.metadata:
                try:
                    ...
                    pass
        # The most recent attempt is the one a retry has to reconcile with. When
        # it ended without a verdict, name what it left in the workspace instead
        # of asking the retry to go and find it.
        if shown and _run_was_protocol_violation(shown[-1]):
            evidencia = _protocol_violation_evidence(task, shown[-1])
            if evidencia:
                lines.append("")

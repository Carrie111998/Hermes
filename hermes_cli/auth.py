...
# SSH / remote-session detection and graphical-browser helpers moved to
# auth_browser_helpers.py (epic #78647, target #78637). Every name is
# re-exported here so the monolith namespace keeps resolving unchanged:
# bare-name call sites inside this module, downstream
# `from hermes_cli.auth import ...` importers, and test monkeypatches all
# go through this module. The eager import binds the names into this
# module's globals; the PEP 562 __getattr__ below is a cycle-safe
# fallback for attribute access.
_AUTH_BROWSER_HELPER_EXPORTS = (
...
from hermes_cli.auth_browser_helpers import (
    _CONSOLE_BROWSER_NAMES,
    _can_open_graphical_browser,
    _is_remote_session,
    _print_loopback_ssh_hint,
...
def __getattr__(name: str):
    """PEP 562 re-export of the SSH/browser helpers in auth_browser_helpers."""
    if name in _AUTH_BROWSER_HELPER_EXPORTS:
        from hermes_cli import auth_browser_helpers  # noqa: PLC0415

        return getattr(auth_browser_helpers, name)

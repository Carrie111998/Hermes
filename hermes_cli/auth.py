...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    This function is side-effect-safe: it does not trigger token refreshes.
    """
    ...
    try:
        ...
        if pool and pool.has_credentials():
            # Use a local snapshot to avoid side effects from pool.select()
            # which can trigger refreshing single-use-token entries.
            entries = pool.entries()
            now = time.time()
            for entry in entries:
                # Skip borrowed credentials without runtime key
                if entry.auth_type == AUTH_TYPE_API_KEY and not entry.runtime_api_key:
                    continue
                # For openai-codex, check if the access token is usable (not expiring)
                # using the same logic as pool.select() but without side effects.
                if (entry.source == "device_code"
                        and entry.last_status in {STATUS_EXHAUSTED, STATUS_DEAD}):
                    # Sync from auth.json to get fresh tokens (read-only)
                    synced = pool._sync_codex_entry_from_auth_store(entry)
                    if synced is not entry:
                        entry = synced
                # Skip exhausted/dead entries
                if entry.last_status == STATUS_DEAD:
                    continue
                if entry.last_status == STATUS_EXHAUSTED:
                    exhausted_until = _exhausted_until(entry)
                    if exhausted_until is not None and now < exhausted_until:
                        # Check if Codex quota restored upstream (issue #43747)
                        if not (pool._codex_quota_restored_upstream(entry)):
                            continue
                # Skip if token is expiring
            ...
            # No non-expiring entry found in pool; check for rate limit status
    ...
    # Fall back to legacy provider state — ensure read_only mode to avoid refresh
    try:

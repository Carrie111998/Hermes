...
def _extend_registry_from_provider_plugins() -> None:
    ...
    try:
        ...
        for _pp in _list_providers_for_registry():
            ...
            _profile_mode = str(getattr(_pp, "api_mode", "") or "")
            _pconfig = PROVIDER_REGISTRY.get(_pp.name)
            if _pconfig is not None:
                # A built-in API-key entry may already own the canonical name.
                # Enrich only that exact canonical family; never let a plugin
                # name collision mutate an OAuth/special provider or another
                # provider's alias target.
                if _pconfig.auth_type != "api_key" or _pconfig.id != _pp.name:
                    continue
                if _profile_mode:
                    # Validated at read time by
                    # runtime_provider._parse_api_mode().
                    _pconfig.api_mode = _profile_mode
            else:
                _api_key_vars = tuple(v for v in _pp.env_vars if not v.endswith("_BASE_URL") and not v.endswith("_URL"))
                _base_url_var = next((v for v in _pp.env_vars if v.endswith("_BASE_URL") or v.endswith("_URL")), None)
                _pconfig = ProviderConfig(
                    id=_pp.name,
                ...
                PROVIDER_REGISTRY[_pp.name] = _pconfig

            # Also register aliases so resolve_provider() and the persisted-mode
            # family guard resolve them to the same canonical ProviderConfig.
            # Existing keys always win; profile aliases never overwrite a
            # built-in provider or another provider's alias.
            for _alias in _pp.aliases:
                if _alias not in PROVIDER_REGISTRY:
                    PROVIDER_REGISTRY[_alias] = _pconfig
    except Exception:
        pass

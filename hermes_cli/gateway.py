...
def generate_launchd_plist() -> str:
    ...
    # A plist can be regenerated by the gateway itself, whose launchd PATH may
    # already be the macOS minimum. Keep standard Homebrew/local prefixes in
    # the generated definition even when the writer cannot currently discover
    # a binary there; otherwise a self-update permanently strips tools such as
    # ``gh`` from the next gateway process.
    launchd_fallback_dirs = [
        "/opt/homebrew/bin",
        "/opt/homebrew/sbin",
        "/usr/local/sbin",
        "/usr/local/bin",

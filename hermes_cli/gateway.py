...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    # Fork's substring patterns feed the older ``Key : Value`` (Format-List)
    # branch below, which the upstream strict matcher doesn't cover. Includes
    # the Windows quoted-path form ``"...\hermes.exe" gateway`` (fork).
    patterns = [
        ...
        # Fork: also match the quoted-path form ``"...\hermes.exe" gateway``.
        "hermes.exe\" gateway",
        "hermes.exe gateway",
        "hermes-gateway.exe",
        "gateway/run.py",
    ...
    # actual ``gateway run`` subcommand (or the dedicated entrypoints), so the
    # Key=Value branches no longer false-match ``gateway status``/``dashboard``
    # siblings or unrelated processes like ``python -m tui_gateway``. Lazy
    # import mirrors the circular-import avoidance used elsewhere in this module.
    ...
    try:
        if is_windows():
            ...
            for line in result.stdout.split("\n"):
                line = line.strip()
                # wmic and the PowerShell ForEach-Object fallback both emit
                # ``Key=Value``; older Format-List output uses ``Key : Value``.
                # Handle BOTH defensively (fork preserved the colon form).
...
def stop_profile_gateway() -> bool:
    ...
    # Wait briefly for it to exit.  Must use the cross-platform existence
    # check rather than os.kill(pid, 0): on Windows the latter calls
    # TerminateProcess and would nuke any live process that sits at this PID.
    import time as _time
    from gateway.status import _pid_exists

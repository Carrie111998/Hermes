"""On-demand supply-chain audit for Charterforge installs.

Scans three surfaces a Charterforge user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Charterforge venv (every PyPI dist via ``importlib.metadata``).

...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Charterforge-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Charterforge refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Charterforge that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Charterforge that doesn't ship
...
#
# Some vendors expose several Charterforge provider slugs (one per endpoint /
...
# In-repo fallback for the model Charterforge silently lands on when the user never
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Charterforge lands on when none is selected.
            desc = "default"
        else:
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Charterforge' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Charterforge should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Charterforge release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Charterforge release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Charterforge release.
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Charterforge' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Charterforge' native Anthropic adapter. The live catalog may
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Charterforge will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Charterforge cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Charterforge actually ships.  (#46850)

...
# Turn-route middleware runs once before Hermes resolves provider credentials or
# constructs an agent. It is intentionally separate from llm_request: the
# latter only rewrites an already selected provider request.
...
def apply_turn_route_middleware(
    ...
    **context: Any,
    ...
    limited to public selection metadata (model/provider/runtime hints); host
    credentials are resolved only after this pass. Callers must invoke this

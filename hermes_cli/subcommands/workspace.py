...
def build_workspace_parser(subparsers: Any, *, cmd_workspace: Callable[[Any], int]) -> None:
    ...
    for name, help_text in (
        ...
        if name == "classify":
            child.add_argument("--baseline", help="Optional 54-registration post-cleanup baseline to compare")
        if name == "import":
...
def cmd_workspace(args: Any) -> int:
    ...
    elif action == "classify" and args.baseline:
        report = compare_post_cleanup_baseline(args.repo, args.baseline)
    else:

...
def _add_server_runtime_args(parser) -> None:
    ...
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Charterforge web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Charterforge web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Charterforge web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Charterforge desktop app shells (<= 0.15.x) spawn the
    ...
    # showed "Charterforge couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Charterforge Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Charterforge backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Charterforge backend server — the JSON-RPC/WebSocket gateway the "

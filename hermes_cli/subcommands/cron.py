...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Script path. Scheduler scripts, and backend scripts with a local "
            "terminal, must be relative to ~/.hermes/scripts/. Other backend "
            "scripts require an absolute path visible to the terminal backend. "
            "Default mode: script stdout is injected into the agent's prompt "
            "each run. "

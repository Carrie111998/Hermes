...
def resolve_exec_command() -> str:
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # sys.executable is the interpreter actually running Hermes. Keep
            # the path spelling intact: resolving a venv's `python` symlink to
            # uv's base interpreter drops the venv site-packages at launch.
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    exe_dir = str(Path(sys.executable).absolute().parent).lower()

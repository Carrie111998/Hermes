...
def _persistent_workspace_has_partial_work(task: object) -> bool:
    """Detect existing dirty Git work without mutating or broadening task schema."""
    if getattr(task, "workspace_kind", None) not in {"dir", "worktree"}:
        return False
    ...
    if not workspace.is_dir():
        return False
    git_metadata = os.path.lexists(workspace / ".git")
    try:
...
def decompose_task(
    ...
    *,
    ...
    if _persistent_workspace_has_partial_work(task):
        return DecomposeOutcome(
            task_id,
            False,
            "persistent Git workspace contains partial dirty work",
        )

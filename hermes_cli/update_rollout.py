...
* snapshot the exact pre-update Git identity and the complete venv outside the
  checkout;
...
* on any canary/batch failure, restore both Git and the venv before restarting
  and verifying every already-advanced profile on the old SHA, canary first.
...
def _restore_git_identity(
    ...
    *,
    ...
    # Prove the object exists before changing HEAD.
    _run_git(project, "cat-file", "-e", f"{sha}^{{commit}}")
    if detached or not branch:
        args = ["checkout"]
        if force:
            args.append("--force")
        args.extend(("--detach", sha))
        ...
        args.append(branch)
        _run_git(project, *args)
        _run_git(project, "reset", "--hard", sha)
...
def restore_checkpoint(
    ...
    *,
    ...
    """Restore exact code + dependency state with a compensating swap.

    The replacement venv is fully staged before Git moves.  If the venv swap
    fails, both the previous venv and the current Git identity are restored.
    On Windows an interpreter running from the live venv may prevent directory
    renames; that fails closed and leaves the checkpoint intact.

    ``transaction_owned_reset`` is an explicit caller assertion that tracked
    working-tree mutations occurred after this transaction's admission and
    checkpoint.  It permits a forced Git checkout for automatic compensation;
    interactive rollback keeps the clean-tree requirement by default.
    """
    ...
    if (
        ...
        and _run_git(project, "status", "--porcelain")
    ):
        raise RollbackError(
    ...
    current_sha, current_branch, current_detached = _git_identity(project)
    ...
    try:
        ...
        # between the two directory renames must not strand a split Git/venv
        # generation.
    ...
    *,
    ...
    if rollback is None:
        rollback = lambda path: restore_checkpoint(
            path, project_root, transaction_owned_reset=True
        )

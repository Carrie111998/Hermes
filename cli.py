...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin, CLIBillingMixin):
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif canonical == "token-efficiency":
            self._show_token_efficiency(cmd_original)
    ...
    def _show_token_efficiency(self, command: str):
        """Print the local read-only token-efficiency report."""
        limit = 20
        parts = command.split()
        if len(parts) > 1:
            try:
                ...
                print("  Usage: /token-efficiency [limit]")
                return
        try:
            from agent.token_telemetry import render_token_efficiency_report

            session_id = getattr(self.agent, "session_id", None) if self.agent else self.session_id
            report = render_token_efficiency_report(limit=limit, session_id=session_id)
        except Exception as exc:
            print(f"  ❌ Token efficiency report failed: {exc}")
            return
    ...
    def _show_subscription(self):
        """`/subscription` — show plan-change handoff without taking billing action."""
        try:
            from agent.billing_view import build_billing_state

            state = build_billing_state()
        except Exception as exc:
            print(f"  ❌ Could not load subscription state: {exc}")
            return
        if not state.logged_in:
            print("  Not logged into Nous Portal. Run `hermes portal` to log in, then /subscription.")
            return
        print()
        print("  💳 Nous subscription")
        if state.org_name:
            role = (state.role or "").title()
            print(f"  Org: {state.org_name}{f' · {role}' if role else ''}")
        if not state.can_change_plan:
            print("  Plan changes require an org owner/admin or an account with billing permissions.")
        if state.portal_url:
            print(f"  Manage subscription on portal: {state.portal_url}")
        else:
            print("  No portal URL available from the current billing state.")

...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update Orion Desktop and try again.')
    }
...
export interface OrionWebSocketUrlOptions {
...
export function buildOrionWebSocketUrl(options: OrionWebSocketUrlOptions): string {

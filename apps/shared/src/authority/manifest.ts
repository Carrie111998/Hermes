...
function decision(
  ...
  return deepFreeze({
    ...
    policy_version: artifact.manifest.policyVersion,
...
export function evaluateAuthorityOperation(
  ...
  if (!operation) {
    return decision(artifact, request, "unsupported_operation", null);
  }

  if (!operation.allowed_actor_classes.includes(request.actor_class)) {
    return decision(artifact, request, "actor_forbidden", matchedRule);
  }

  if (!operation.allowed_resource_states.includes(request.resource_state)) {
    return decision(artifact, request, "resource_state_denied", matchedRule);
  }
  ...
  for (const grant of request.capabilities) {
    if (
      ...
      return decision(artifact, request, "invalid_capability_proof", matchedRule);
    }
  ...
  if (
    ...
    return decision(artifact, request, "missing_capability", matchedRule);
  }
  ...
  return decision(artifact, request, "allowed", matchedRule, {
    allowed: true,
    capabilityProofs,
  });
}

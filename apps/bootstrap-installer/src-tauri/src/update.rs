...
//!
//! Driven when the installer is launched as `Orion-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Orion desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `orion update`
//!      or repair bootstrap can race locked files),
//!   2. run `orion update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in orion_cli/main.py),
//!   3. run `orion desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `orion update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_orion) and
...
/// `orion update` exit code meaning "another orion process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// orion_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `orion update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `orion update` claims
/// the same file (see `orion_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `orion update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in orion_cli/update_lock.py — all three read
...
/// a dashboard-spawned `orion update`) still blocks.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let orion_home = crate::paths::orion_home();
    let install_root = orion_home.join("orion-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_orion`
    ...
    //
    // The same marker is the cross-process update lock (orion_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `orion update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Orion update is already running (PID {}, started {} ago). \
    ...
    let orion = resolve_orion(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the orion CLI under {}. Is Orion installed? \
    ...
    // async on Windows. If it still holds the venv shim, `orion update`
    ...
    // ---- stage 2: orion update -----------------------------------------
    // Pass --branch so `orion update` targets the branch this installer was
    ...
    // without --branch, `orion update` switches the checkout to `main` (a
    ...
    // --force skips `orion update`'s Windows running-exe guard (which would
    ...
    // time `orion update` runs there is no legitimate orion.exe to protect,
    // and the guard would only produce a false "Orion is still running" stop.
    //
    ...
    // probe only checks the orion.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Orion windows" message.
    ...
    // Retry-once for the update-boundary crash. `orion update` lazily imports
    ...
    // second `orion update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Orion" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Orion is still running. Close all Orion windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "orion update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::orion_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: orion desktop --build-only ----------------------------
    // `orion update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `orion update` already does above, and `orion update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `orion desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Orion manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_orion_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Orion to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend orion.exe (or the desktop Orion.exe
            ...
            // force-kill every Orion.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Orion still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_orion();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_orion(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Orion.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Orion.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `orion.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `orion.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\orion.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startOrion()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// orion.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named orion.exe.)
fn force_kill_other_orion() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `orion <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `orion update` is precisely the shape that
...
/// Path to the venv orion shim under an install root, regardless of existence.
fn venv_orion(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("orion.exe")
    } else {
        install_root.join("venv").join("bin").join("orion")
    }
}

/// Resolve the orion CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `orion` on PATH.
fn resolve_orion(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_orion(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "orion.exe" } else { "orion" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let orion_home = crate::paths::orion_home();
    let mut envs = vec![(
        "ORION_HOME".to_string(),
        orion_home.as_os_str().to_os_string(),
    )];
    // `orion update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `orion update` child claims that SAME lock (orion_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Orion is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in orion_cli/update_lock.py.
    envs.push((
        "ORION_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        orion_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_orion_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Orion.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.orion-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.orion-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::orion_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_orion_is_under_install_root() {
        let root = Path::new("/x/orion-agent");
        let shim = venv_orion(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/orion-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/orion-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "ORION_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the orion update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_orion(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/orion-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".orion-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `orion update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(orionHome, child.pid) races ahead
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Orion.app"]),
            Some(PathBuf::from("/Applications/Orion.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Orion.app");
        let tmp = base.join("Orion.app.orion-update-new");
        let old = base.join("Orion.app.orion-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Orion.app");
        let tmp = base.join("Orion.app.orion-update-new"); // intentionally absent
        let old = base.join("Orion.app.orion-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Orion.app");
        let tmp = base.join("Orion.app.orion-update-new"); // absent
        let old = base.join("Orion.app.orion-update-old");
        write_marker(&target, "OLD");

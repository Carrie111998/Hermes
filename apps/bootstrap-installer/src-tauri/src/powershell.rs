...
/// read side does not see EOF until the last of them closes it. `orion update`
...
/// stranded with it.
///
/// Same bound `Invoke-OrionStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `orion_home_override` propagates to the child as $ORION_HOME so the
/// install script writes to the same directory the installer is reading from.
pub async fn run_script(
    script_path: &Path,
    args: &[String],
    sink: StreamSink,
    orion_home_override: Option<&str>,
    cancel_rx: &mut Option<CancelRx>,
) -> Result<ScriptResult> {
    ...
    if let Some(cwd) = stable_script_cwd(script_path, orion_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = orion_home_override {
        cmd.env("ORION_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, orion_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = orion_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_orion_home() {
    ...
    /// child. `orion update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]

...
//!
//! Mirrors `orion_constants.get_orion_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\orion
//!   macOS:   ~/.orion
//!   Linux:   ~/.orion  (override via $ORION_HOME)
//!
//! NOTE (macOS): Python's get_orion_home(), scripts/install.sh, and the
//! Electron desktop's resolveOrionHome() ALL use ~/.orion on macOS — there
...
/// Returns the canonical Orion home directory, respecting $ORION_HOME if set.
pub fn orion_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ORION_HOME") {
    ...
    {
        // %LOCALAPPDATA%\orion — matches scripts/install.ps1's $OrionHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("orion");
        }
    }

    // macOS + Linux + fallback: ~/.orion (matches Python get_orion_home(),
    // install.sh, and the Electron desktop's resolveOrionHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".orion");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    orion_home().join("bootstrap-cache")
}
...
/// ORION_HOME so it survives repo checkout deletion (unlike anything under
/// orion-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\orion\orion-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_orion`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ORION_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ORION_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    orion_home().join(".orion-update-in-progress")
}
...
pub fn copy_self_to_orion_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ORION_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ORION_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".orion-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ORION_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[orion-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ORION_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_orion_home() -> String {
    orion_home().to_string_lossy().into_owned()
}

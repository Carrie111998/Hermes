...
export const $orionHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.orion/logs/bootstrap-installer.log')
    $orionHome.set('~/.orion')
  ...
  try {
    const [logPath, orionHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_orion_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $orionHome.set(orionHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Orion" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Orion-Setup.exe --update);
...
export async function launchOrionDesktop(): Promise<void> {
  ...
  await invoke('launch_orion_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Orion repository' },

...
**The native desktop app for [Charterforge](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Charterforge surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Charterforge and hear it back.</td></tr>
...
### Install with Charterforge (recommended)

Already have the Charterforge CLI? Just run:

```bash
hermes desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Charterforge gateway or install Charterforge locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Charterforge Desktop website.](https://hermes-agent.nousresearch.com/).
...
```bash
# throwaway CHARTERFORGE_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
../scripts/dev-sandbox.sh npm run dev
CHARTERFORGE_DESKTOP_CHARTERFORGE_ROOT=/path/to/clone npm run dev
CHARTERFORGE_HOME=/tmp/throwaway npm run dev
...
first launch it can install the Charterforge runtime into `CHARTERFORGE_HOME`
...
- **Charterforge** runs as a headless `hermes serve` process and exposes the
...
1. `CHARTERFORGE_DESKTOP_CHARTERFORGE_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `CHARTERFORGE_DESKTOP_HERMES`, or `hermes` on `PATH`
5. a system Python that can import the Charterforge runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Charterforge
...
screen offers **Connect to existing Charterforge** before starting the local installer.
...
terminal commands, and file operations run against the remote Charterforge host, not
...
Boot logs land in `CHARTERFORGE_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
> The default Charterforge home on Windows is `%LOCALAPPDATA%\hermes`. Set the `CHARTERFORGE_HOME` env var if you've relocated it.

...
How to build Orion Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Orion surface can also
...
- A **runtime home change** (switching the underlying `ORION_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Orion — do not assume

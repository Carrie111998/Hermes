...
How to build Charterforge Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Charterforge surface can also
...
- A **runtime home change** (switching the underlying `CHARTERFORGE_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Charterforge — do not assume

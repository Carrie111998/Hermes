...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('orion:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('orion:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('orion:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('orion:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('orion:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('orion:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('orion:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('orion:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('orion:pet-overlay:control', payload)
  })
}

 ...
 *  - handshake gate: WebSocket.OPEN alone never publishes (§7)
...
export type ActivationReceipt =
  | Readonly<{ status: 'activated'; route: RouteKey; lease: RouteLease }>
  | Readonly<{ status: 'already-active'; route: RouteKey; lease: RouteLease }>
  | Readonly<{
...
// ── Supervisor
export type ActivateFn = (route: RouteKey) => Promise<{ gatewayEpoch: string; socketInstanceId: string }>
export type IsCurrentFn = (route: RouteKey) => boolean

export class GatewaySupervisor {
  readonly #flights = new Map<string, Promise<ActivationReceipt>>()
  readonly #states = new Map<string, GatewaySupervisorState>()
  readonly #activationEpochByKey = new Map<string, number>()
  readonly #leases = new Map<string, RouteLease>()
  #globalEpoch = 0

  constructor(
    private readonly deps: {
      activateTransport: ActivateFn
  ...
  leaseFor(route: RouteKey): RouteLease | null {
   ...
   * never publish.
   */
  activate(route: RouteKey): Promise<ActivationReceipt> {
    const key = supervisorKey(route)
    ...
    const pending = (async (): Promise<ActivationReceipt> => {
      ...
      let transport: { gatewayEpoch: string; socketInstanceId: string }
      try {
        transport = await this.deps.activateTransport(route)
      } catch (error) {
  ...
  reconnect(route: RouteKey): Promise<ActivationReceipt> {
  ...
  dispose(route: RouteKey): void {
    ...
    if (cur === 'Live' || cur === 'Degraded') this.transition(route, 'Draining')

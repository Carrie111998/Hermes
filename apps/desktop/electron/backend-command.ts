// Backend subcommand routing for the desktop-managed Orion process.
//
// The desktop app launches its own headless backend via `orion serve` — it
...
// app hasn't updated yet, or an older `orion` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Orion profile to pin via `--profile`.
 */
 ...
 * `-m orion_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `orion_cli/subcommands/dashboard.py` source registers

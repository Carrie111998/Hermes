...
function resolveUnpackedRelease(execPath, updateRoot, platform) {
  ...
  const unpacked = pathModule.resolve(releaseDir, unpackedDirName(platform))
  const normalizedExec = pathModule.resolve(String(execPath))
  // execPath must be the unpacked dir itself or a descendant of it.
  const withSep = unpacked.endsWith(pathModule.sep) ? unpacked : unpacked + pathModule.sep

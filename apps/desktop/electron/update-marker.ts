 ...
 *
 * The Tauri updater writes ORION_HOME/.orion-update-in-progress for the whole
 ...
 * (`force_kill_other_orion`, taskkill /IM orion.exe) kills it, the launch
...
export function markerPath(orionHome) {
  return path.join(orionHome, '.orion-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(orionHome)
  let raw
 ...
 *
 * The Tauri-based orion-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(orionHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(orionHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Orion to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(orionHome, opts)

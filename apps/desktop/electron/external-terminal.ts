// Launching the Orion TUI in the user's OWN terminal emulator.
//
...
// `orion --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Orion runtime is often a venv Python invoked
//     as `python -m orion_cli.main`, not a `orion` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Orion

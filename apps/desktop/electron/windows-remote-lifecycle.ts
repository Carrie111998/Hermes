...
const READY_RE = /^ORION_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitOrionPath = '') {
  const explicit = psLiteral(explicitOrionPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$orionHome=$env:ORION_HOME',
    'if(-not $orionHome){$orionHome=Join-Path $env:LOCALAPPDATA "orion"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command orion.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $orionHome "orion-agent\\venv\\Scripts\\orion.exe")',
    '$candidates+=(Join-Path $HOME "orion-agent\\.venv\\Scripts\\orion.exe")',
    '$orion=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $orion){throw "Orion is not installed on the remote Windows host."}',
    'if($explicit -and $orion -ne $explicit){throw "The configured Orion path is not an executable file."}',
    '$python=Join-Path (Split-Path $orion) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Orion Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;orionHome=$orionHome;orionPath=$orion;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitOrionPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitOrionPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'orion_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.orionPath === 'string' &&
    typeof lock.orionHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.orionPath === runtime.orionPath &&
    lock.orionHome === runtime.orionHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteOrionPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.orionPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Orion on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.orionPath = inspection.path
  const orionVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located orion at ${runtime.orionPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, orionPath: runtime.orionPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForOrion(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Orion SSH"', 'powershell.exe -NoLogo')

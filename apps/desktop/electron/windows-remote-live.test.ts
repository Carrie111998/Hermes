...
//   CHARTERFORGE_WIN_SSH_HOST   ssh alias/host of the Windows box
//   CHARTERFORGE_WIN_SSH_USER   remote user
//   CHARTERFORGE_WIN_SSH_HERMES absolute path to the remote hermes.exe under test
const liveHost = process.env.CHARTERFORGE_WIN_SSH_HOST || ''
const liveUser = process.env.CHARTERFORGE_WIN_SSH_USER || ''
const configuredCharterforge = process.env.CHARTERFORGE_WIN_SSH_HERMES || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Charterforge-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredCharterforge)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredCharterforge.replace('hermes.exe', 'python.exe')}' -m hermes_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredCharterforge.replace('hermes.exe', 'python.exe')

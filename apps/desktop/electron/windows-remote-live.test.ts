...
//   ORION_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ORION_WIN_SSH_USER   remote user
//   ORION_WIN_SSH_ORION absolute path to the remote orion.exe under test
const liveHost = process.env.ORION_WIN_SSH_HOST || ''
const liveUser = process.env.ORION_WIN_SSH_USER || ''
const configuredOrion = process.env.ORION_WIN_SSH_ORION || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Orion-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredOrion)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredOrion.replace('orion.exe', 'python.exe')}' -m orion_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredOrion.replace('orion.exe', 'python.exe')
        const terminate = `& '${python}' -m orion_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.orionPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m orion_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m orion_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }

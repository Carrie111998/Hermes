...
import { buildDesktopBackendEnv, orionManagedNodePathEntries, normalizeOrionHomeRoot } from './backend-env'
import { isReauthRequiredError, makeNousCloudBackendDownError, waitForOrionReady } from './backend-health'
...
const USER_DATA_OVERRIDE = process.env.ORION_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ORION_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ORION_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ORION_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[orion] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[orion] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ORION_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[orion] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[orion] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via ORION_DESKTOP_PASSWORD_STORE, bridged by the
// `orion desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[orion] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[orion] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `orion desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Orion sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[orion] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[orion] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[orion] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[orion] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[orion] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('orion:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Orion burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[orion] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[orion] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[orion] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[orion] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ORION_HOME — the user-facing root for everything Orion-related. Mirrors
// scripts/install.ps1's $OrionHome and scripts/install.sh's $ORION_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\orion (matches install.ps1)
//   macOS / Linux: ~/.orion (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.orion directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\orion yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ORION_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ORION_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.orion / %LOCALAPPDATA%\orion.
function resolveOrionHome() {
  if (process.env.ORION_HOME) {
    return normalizeOrionHomeRoot(process.env.ORION_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'orion-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ORION_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\orion and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ORION_HOME')

    if (fromRegistry) {
      return normalizeOrionHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'orion')
    const legacy = path.join(app.getPath('home'), '.orion')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.orion setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.orion')
}

const ORION_HOME = resolveOrionHome()

function pathWithOrionManagedNode(...entries) {
  const managed = orionManagedNodePathEntries(ORION_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ORION_ROOT — the canonical mutable Orion install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ORION_ROOT = path.join(ORION_HOME, 'orion-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ORION_ROOT, 'venv')
...
// boots, going straight to `resolveOrionBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ORION_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ORION_ROOT, '.orion-bootstrap-complete')
...
// active-profile.json records which Orion profile the desktop launches its
// local backend as. When set, startOrion() passes `orion --profile <name>
// dashboard …`, which deterministically pins ORION_HOME (see
// _apply_profile_override in orion_cli/main.py) and bypasses the sticky
// ~/.orion/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors orion_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// orionDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ORION_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by orion_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ORION_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (orion_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ORION_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ORION_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ORION_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ORION_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ORION_DESKTOP_APP_NAME || 'Orion'
...
// 'orion:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.orion) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.orion')
}

// Seed the native About panel with the live Orion version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `orion update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveOrionVersion(),
  copyright: 'Copyright © 2026 Orion'
})
...
// startOrion(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ORION_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ORION_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startOrion() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning orion serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Orion backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (orionLog.length > 300) {
    orionLog.splice(0, orionLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-orion-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('orion:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the orion:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('orion:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ORION_ROOT
  })
}
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ORION_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Orion will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(ORION_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Orion update did not finish',
        `${result.message}\n\nDetails: ${path.join(ORION_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `orion` — must not shadow `orion.cmd`/`orion.exe`.
...
function unwrapWindowsVenvOrionCommand(command, backendArgs) {
  return resolveVenvOrionCommand(command, backendArgs, {
...
// spawns `orion serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `orion` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'orion_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `orion --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ORION_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveOrionBackend step 4); execFileSync of a .cmd without
...
function isOrionSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'orion_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ORION_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Orion deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the orion-import
...
// findGitBash — locate bash.exe on Windows. Resolves ORION_GIT_BASH_PATH
...
// normal ORION_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\orion\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'orion', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'orion', 'git', 'bin', 'git.exe'))
  }
...
function recentOrionLog() {
  return orionLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's orion root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ORION_ROOT.
// ORION_DESKTOP_ORION_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ORION_DESKTOP_ORION_ROOT && path.resolve(process.env.ORION_DESKTOP_ORION_ROOT),
    !IS_PACKAGED && isOrionSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isOrionSourceRoot(ACTIVE_ORION_ROOT) ? ACTIVE_ORION_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ORION_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('orion:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'orion-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `orion update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ORION_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `orion update` can write
// fresh entry points. On Windows this is the file the running backend
// `orion.exe` holds open; on POSIX it's never mandatory-locked.
function venvOrionShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'orion.exe')
    : path.join(updateRoot, 'venv', 'bin', 'orion')
}
...
// only signals the direct child, so on Windows a backend `orion.exe` that
// spawned its own grandchildren (a `orion` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    child.orionBackendIdentity = identity

    return identity
  } catch (error) {
    stopBackendChild(child)
    await waitForBackendExit(child)
    throw new Error(`Could not persist ownership for the Orion backend: ${error.message}`)
  }
}

function releaseBackendChild(child) {
  const identity = child?.orionBackendIdentity
...
// `orionProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `orion.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a orion
// REPL, a pty terminal, the gateway) keep `orion.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const orionProcess = backendConnectionState.getProcess()

  stopBackendTreesForUpdate(orionProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvOrionShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentOrionProcess = backendConnectionState.getProcess()

    if (currentOrionProcess && Number.isInteger(currentOrionProcess.pid)) {
      stragglers.push(currentOrionProcess.pid)
    }
...
// `orion update`). Instead we spawn the staged Orion-Setup binary with
// --update and quit, so it can run `orion update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `orion update` → relaunch),
      ...
      // in: the ORION_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `orion update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `orion desktop`, never the Tauri installer that self-copies
      // orion-setup.exe into ORION_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `orion` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `orion update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `orion update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, orionRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(ORION_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Orion — this window will close and the updater will open. Don’t reopen Orion yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(ORION_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // orion.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running orion, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Orion install open ' +
        '(a second Orion window or a terminal running orion?). Close it and retry.'
    ...
    // Orion processes running from this venv.  The updater normally refuses
    ...
    // `orion update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `orion update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          ORION_HOME,
          ORION_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithOrionManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `orion update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(ORION_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Orion update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(ORION_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Orion will keep running — try again, or run \`orion update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvOrion = path.join(venvBin, IS_WINDOWS ? 'orion.exe' : 'orion')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Orion launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the orion
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.orion-bootstrap-complete')),
      hasVenvOrion: fileExists(venvOrion),
...
// Python-level pre-update snapshot inside `orion update`.
function preflightStateDb(orionHome, rememberLog) {
  const stateDbPath = path.join(orionHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(orionHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(orionHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(orionHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `orion update`, swaps/relaunches the app bundle, and writes
// .orion-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'orion update', percent: null })

    return { ok: true, manual: true, command: 'orion update', orionRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      ORION_HOME,
      ORION_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithOrionManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Orion — this window will close. Don’t reopen Orion yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_ORION_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isOrionSourceRoot(ACTIVE_ORION_ROOT) &&
    fileExists(venvPython) &&
    canImportOrionCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ORION_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `orion
...
function resolveWebDist() {
  const override = process.env.ORION_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ORION_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: orion desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: orion desktop --force-build`
  )
...
function resolveOrionCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Orion.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveOrionCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_ORION_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Orion at ${ACTIVE_ORION_ROOT}`,
    command,
    args: ['-m', 'orion_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      orionHome: ORION_HOME,
      pythonPathEntries: [ACTIVE_ORION_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ORION_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveOrionBackend(backendArgs) {
  // 1. Explicit override -- ORION_DESKTOP_ORION_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ORION_DESKTOP_ORION_ROOT && path.resolve(process.env.ORION_DESKTOP_ORION_ROOT)

  if (overrideRoot && isOrionSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Orion source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `orion` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isOrionSourceRoot.)
  if (!IS_PACKAGED && isOrionSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Orion source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_ORION_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\orion\\orion-agent (Windows) or ~/.orion/orion-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Orion runtime at ${ACTIVE_ORION_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `orion` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ORION_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ORION_DESKTOP_IGNORE_EXISTING !== '1') {
    let orionCommand = null
    const orionOverride = process.env.ORION_DESKTOP_ORION

    if (orionOverride) {
      const resolvedOverride = findOnPath(orionOverride)

      if (resolvedOverride) {
        orionCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(orionOverride, { isWsl: IS_WSL })) {
        orionCommand = orionOverride
      } else {
        rememberLog(`Ignoring Windows Orion override under WSL: ${orionOverride}`)
      }
    } else {
      orionCommand = findOnPath('orion')
    }

    if (orionCommand) {
      if (looksLikeDesktopAppBinary(orionCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Orion CLI: ${orionCommand}`)
        orionCommand = null
      }
    }

    if (orionCommand) {
      const unwrapped = unwrapWindowsVenvOrionCommand(orionCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `orion` shim
      ...
      const shellForProbe = isCommandScript(orionCommand)

      // ORION_DESKTOP_ORION is an explicit deployment override (used by
      ...
      if (shouldTrustOrionOverride(orionOverride) || verifyOrionCli(orionCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvOrionCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Orion CLI at ${orionCommand}`,
          command: orionCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Orion CLI at ${orionCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed orion_cli module via system Python.
  ...
  if (python) {
    ...
    // having orion_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\orion\orion-agent\venv.
    if (canImportOrionCli(python)) {
      return {
        kind: 'python',
        label: `installed orion_cli module via ${python}`,
        command: python,
        args: ['-m', 'orion_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: orion_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveOrionBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Orion Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveOrionBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Orion install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Orion recovery was handed off to Orion Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      orionHome: ORION_HOME,
      logRoot: path.join(ORION_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Orion install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Orion bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ORION_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startOrion() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // orion:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveOrionBackend(backend.args))
  }
  ...
  if (!isOrionSourceRoot(ACTIVE_ORION_ROOT)) {
    throw new Error(
      `Orion install at ${ACTIVE_ORION_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Orion' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\orion\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `orion` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Orion on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isOrionSourceRoot()
    // plus an importable orion_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Orion venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Orion at ${ACTIVE_ORION_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Orion runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Orion backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Orion backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Orion backend after ${timeoutMs}ms`))
    })
...
function downloadViaTokenToFile(url, token, ctx, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      ...
      {
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Orion-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Orion-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('orion:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveOrionCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('orion:preview-file-changed', payload)
}
...
// waitForOrionReady how to read a 401 — rejected session vs gated route.
...
async function waitForOrion(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForOrionReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('orion:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('orion:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('orion:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('orion:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Orion...".
function sendPowerResume() {
  ...
  webContents.send('orion:power-resume')
}
...
// 'orion:power-battery' push below.
ipcMain.handle('orion:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('orion:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('orion:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('orion:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('orion:context-menu-spellcheck', {
...
//
// Hosted Orion gateways gate the dashboard behind an OAuth provider (e.g.
// Orion) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``orion_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / orion #37247).
//     Both are set as HttpOnly cookies (``orion_session_at`` ~15 min,
//     ``orion_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:orion-remote-oauth'
...
// "not signed in", which on the initial boot path (startOrion → the renderer's
// single-shot boot() with no retry) surfaced as the "Orion couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Orion
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Orion Cloud agent…' : 'Sign in to Orion gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Orion backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in orion_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Orion Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:orion-remote-oauth`:
...
// convention (orion_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Orion surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.ORION_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Orion gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for orion_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Orion Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Orion Cloud sign-in.'))
...
// Discover the hosted (Orion Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Orion Cloud. Open Settings → Gateway, choose Orion Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Orion Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Orion Cloud session has expired. Sign in to Orion Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Orion Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every orion:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteOrionPath`/`remoteProfile` —
...
// a valid stored value (pins the root ORION_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ORION_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ORION_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Orion Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteOrionPath: (ssh || savedSsh)?.remoteOrionPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ORION_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Orion Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Orion Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteOrionPath: input.sshRemoteOrionPath ?? existingBlock.remoteOrionPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (orion #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Orion gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Orion gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Orion gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.ORION_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteOrionPath || '')
    ...
    result = await lifecycle({
      ...
      remoteOrionPath: sshConfig.remoteOrionPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForOrion: (baseUrl, token) => waitForOrion(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    orionVersion: result.orionVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.orionVersion || 'orion (version unknown)'} at ${result.orionPath || '?'}`
  )
  ...
  return { ...connection, remoteOrionVersion: result.orionVersion || '' }
}
...
//   2. env override (ORION_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.ORION_DESKTOP_REMOTE_TOKEN,
      url: process.env.ORION_DESKTOP_REMOTE_URL
    },
    profile,
...
// env → global) so a startOrion() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Orion
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Orion" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteOrionPath: input.sshRemoteOrionPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteOrionPath || '')
          let orionPath
          let orionVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            orionVersion = inspection.version
            supported = inspection.supported
          } else {
            orionPath = await remoteLifecycle.locateOrion(ssh, sshConfig.remoteOrionPath || '')
            orionVersion = await remoteLifecycle.probeOrionVersion(ssh, orionPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, orionPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Orion on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteOrionPath: orionPath,
            remoteOrionVersion: orionVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startOrion())
  ...
  // connect to Orion gateway". Mirror the renderer's connect here so the test
...
function resetOrionConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const orionProcess = backendConnectionState.invalidate()
  stopBackendChild(orionProcess)
...
// startOrion() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetOrionConnection() invalidates it.
  const orionProcess = backendConnectionState.getProcess()
  const dying = orionProcess && !orionProcess.killed ? orionProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('orion:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('orion:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startOrion()
  ...
  // spawning, so two children never share one profile's ORION_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Orion backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile) {
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Orion backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(source, profile, key, poolEntry) {
  ...
  if (source.kind === 'ssh') {
    ...
    const sshConfig = normalizeSshConfig({
      ...
      remoteOrionPath: source.remoteOrionPath,
  ...
  await waitForOrion(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
// local-spawn portion of startOrion() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForOrion(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited ORION_HOME env (see _apply_profile_override
  // step 3 in orion_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveOrionBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const orionCwd = resolveOrionCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(ORION_HOME, 'profiles', key))
  )

  rememberLog(`Starting Orion backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: orionCwd,
        ORION_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        ORION_WEB_DIST: webDist,
        ...(readyFile ? { ORION_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Orion backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Orion backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)

    if (!ready) {
      rejectStart?.(
        new Error(`Orion backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForOrion(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Orion backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Orion backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_orion_home() recreates the deleted profile directory.
//
...
async function startOrion() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Orion Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startOrion() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Orion backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Orion backend at ${remote.baseUrl}`, 24)
      await waitForOrion(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Orion backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, orionLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.orion/active_profile file) and
    // resolves ORION_HOME the same way `orion -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Orion runtime', 28)

        return resolveOrionBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Orion backend via ${backend.label}`, 84)
    rememberLog(`Starting Orion backend via ${backend.label}`)
    ...
    const orionProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ORION_HOME for the child so Python's get_orion_home()
          // resolves to the SAME location our resolveOrionHome() picked. Without
          // this pin, Python falls back to ~/.orion on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\orion, which differs from C:\Users\<u>\.orion.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ORION_HOME via setx; the desktop
    ...
    await claimBackendChild(orionProcess, `${backend.command} ${backend.args.join(' ')}`, profile, backendNonce)
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, orionProcess)

    if (!processOwner) {
      stopBackendChild(orionProcess)
      await waitForBackendExit(orionProcess)
      releaseBackendChild(orionProcess)
      throw new Error('Orion backend start was superseded by a newer connection attempt.')
    }

    orionProcess.stdout.on('data', rememberLog)
    orionProcess.stderr.on('data', rememberLog)
    ...
    orionProcess.once('error', error => {
      releaseBackendChild(orionProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Orion backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Orion backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Orion backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Orion backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    orionProcess.once('exit', (code, signal) => {
      releaseBackendChild(orionProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Orion backend exit (${signal || code})`)
      ...
      rememberLog(`Orion backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Orion backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Orion backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentOrionLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Orion backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(orionProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Orion backend to become ready', 90)
    await Promise.race([waitForOrion(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => orionProcess.exitCode === null && !orionProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Orion backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Orion backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Orion is
...
// pushes pet state over IPC (orion:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via orion:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Orion app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Orion icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see orion:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Orion in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('orion:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Orion can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('orion:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('orion:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `orion:hud:set-bounds`, which flips resizable on for the call — the
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('orion:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('orion:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('orion:quick-entry:shown')
}
...
function createWindow() {
  ...
  startOrion().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('orion:connection', async (_event, profile) => {
  const connection = await ensureBackend(profile)
...
ipcMain.handle('orion:connection:for', async (_event, payload) => {
...
// "Starting Orion…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('orion:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetOrionConnection,
...
ipcMain.handle('orion:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('orion:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('orion:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('orion:window:openInstance', async () => {
...
ipcMain.handle('orion:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveOrionBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Orion is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `orion-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, ORION_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('orion:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('orion:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('orion:zoom:get', event => {
...
ipcMain.on('orion:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('orion:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startOrion() call restarts the
...
ipcMain.handle('orion:bootstrap:repair', async () => {
  // Forceful repair: force the next startOrion() through the full installer
  ...
  // "bypass active runtime" path inside startOrion() and fall through
...
ipcMain.handle('orion:boot-progress:get', async () => bootProgressState)
ipcMain.handle('orion:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('orion:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('orion:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('orion:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('orion:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('orion:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('orion:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('orion:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('orion:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('orion:connections:set-primary', async (_event, id) => {
  const registry = setPrimaryConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('orion:connections:set-launch-mode', async (_event, mode) => {
  const registry = setConnectionLaunchMode(readDesktopConnectionsRegistry(), String(mode || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('orion:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('orion:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteOrionPath: entry.remoteOrionPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteOrionPath: connection.remoteOrionPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteOrionProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root ORION_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('orion:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// orion:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
ipcMain.handle('orion:gateway:ws-url-for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
...
// Fan out `orion update` to every eligible registered connection at once.
...
// /api/orion/update endpoint (the dashboard updater), which runs
// `orion update` on THAT machine.
ipcMain.handle('orion:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/orion/update', {}, { timeoutMs: 15_000 })
...
// analogue of the orion:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('orion:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('orion:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startOrion() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('orion:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Orion Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('orion:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('orion:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('orion:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('orion:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('orion:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ORION_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('orion:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('orion:requestMicrophoneAccess', async () => {
...
ipcMain.handle('orion:window:readBelow', async event => {
...
async function handleOrionApiRequest(request) {
  ...
  // backend calls ensure_orion_home() which recreates the profile directory,
...
ipcMain.handle('orion:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its ORION_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleOrionApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleOrionApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('orion:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('orion:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('orion:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as orion://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('orion:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('orion:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('orion:data-url-read-max:get', () => ({
...
ipcMain.handle('orion:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('orion:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('orion:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('orion:readFileText', async (_event, filePath) => {
...
// `orion:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('orion:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('orion:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('orion:writeClipboard', (_event, text) => {
...
ipcMain.handle('orion:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('orion:readClipboard', () => clipboard.readText())

ipcMain.handle('orion:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('orion:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('orion:context-menu:edit', (event, command) => {
...
ipcMain.handle('orion:context-menu:copy-image', event => {
...
ipcMain.handle('orion:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('orion:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('orion:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('orion:saveClipboardImage', async () => {
...
ipcMain.handle('orion:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('orion:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('orion:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('orion:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('orion:active-work', (event, payload) => {
...
ipcMain.on('orion:titlebar-theme', (_event, payload) => {
...
ipcMain.on('orion:native-theme', (_event, mode) => {
...
ipcMain.on('orion:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('orion:translucency', (_event, payload) => {
...
ipcMain.on('orion:keep-awake', (_event, on) => {
...
ipcMain.handle('orion:quick-entry:settings:get', async () => {
...
ipcMain.handle('orion:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('orion:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('orion:quick-entry:submit', {
...
ipcMain.on('orion:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('orion:quick-entry:state', payload)
  }
})

ipcMain.on('orion:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('orion:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('orion:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `orion:window:openSession`).
...
ipcMain.handle('orion:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('orion:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('orion:preview:reach', async (_event, url) => reachablePreviewUrl(String(url || '')))

ipcMain.handle('orion:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveOrionCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('orion:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveOrionCwd()
}))

ipcMain.handle('orion:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('orion:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('orion:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('orion:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('orion:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('orion:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('orion:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: orionLog.slice(-200) }))
...
ipcMain.on('orion:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (orion:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('orion:updates:check', async () =>
...
ipcMain.handle('orion:updates:apply', async (_event, payload) =>
...
ipcMain.handle('orion:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('orion:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Orion version (the one `release.py` bumps in
// orion_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Orion version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveOrionVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'orion_cli', '__init__.py')
...
// Renderer-bundle skew: `orion update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Orion version and push it into the native About panel
// just before showing it, so an in-place `orion update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveOrionVersion()} — app build out of date, update the desktop app`
        : resolveOrionVersion(),
      copyright: 'Copyright © 2026 Orion'
    })
    app.showAboutPanel()
...
ipcMain.handle('orion:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveOrionVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    orionRoot: resolveUpdateRoot(),
...
// the actual removal via `orion uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (orion_cli/uninstall.py + orion_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ORION_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isOrionSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'orion_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ORION_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Orion agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import orion_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ORION_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `orion-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `orion-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('orion:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('orion:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('orion:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('orion:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// orion:// deep links (e.g. orion://blueprint/morning-brief?time=08:00,
// orion://mcp/install?name=NAME&config=B64 — the vendor "Add to Orion"
// button, or orion://plugin/install?repo=owner/repo). Dev
// (`ORION_DESKTOP_DEV_SERVER`) registers orion-dev:// instead — bare
// Electron or a stale OS handler often owns orion:// on dev machines.
...
// ---------------------------------------------------------------------------
const ORION_PROTOCOL = DEV_SERVER ? 'orion-dev' : 'orion'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['orion-dev', 'orion'] : ['orion']
...
function handleDeepLink(url) {
  ...
  // orion://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('orion:deep-link', payload)
...
ipcMain.handle('orion:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ORION_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ORION_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(ORION_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${ORION_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `orion://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startOrion), whose
...
app.whenReady().then(() => {
  ...
  // startOrion() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching orion:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Orion never keeps another app's chord hostage.

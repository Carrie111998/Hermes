...
if (IS_WINDOWS) {
  ...
  // expose Ares sessions/config to every packaged app on the machine.
...
// HERMES_HOME — the user-facing root for everything Ares-related. Mirrors
...
// ACTIVE_HERMES_ROOT — the canonical mutable Ares install. Same path
...
// active-profile.json records which Ares profile the desktop launches its
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Ares'
...
// Seed the native About panel with the live Ares version. This is refreshed
...
let bootProgressState = {
  ...
  message: 'Waiting to start Ares backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Ares will start automatically when it completes…',
      12
    )
...
function findSystemPython() {
  ...
  // CPython but several Ares deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Ares' terminal tool requires
...
// `hermes update`). Instead we spawn the staged Ares-Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Ares — this window will close and the updater will open. Don’t reopen Ares yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Ares install open ' +
        '(a second Ares window or a terminal running hermes?). Close it and retry.'
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  // Put the Ares-managed Node and the venv on PATH so `hermes desktop`'s
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Ares (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Ares to retry.',
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Ares…', percent: 100 })
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Ares to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Ares desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Ares to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Ares.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Ares.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Ares to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Ares to load the new version.',
      percent: 100
    })
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Ares.app/Contents/...`
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Ares at ${ACTIVE_HERMES_ROOT}`,
    command,
    args: ['-m', 'hermes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: HERMES_HOME,
...
function resolveHermesBackend(backendArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Ares source at ${overrideRoot}`, backendArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Ares source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Ares override under WSL: ${hermesOverride}`)
      }
    } else {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Ares CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvHermesCommand(hermesCommand, backendArgs) || {
            label: `existing Ares CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Ares CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'Ares Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Ares install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Ares recovery was handed off to Ares Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Ares install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Ares bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `Ares install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Ares' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Ares on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Ares venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Ares at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Ares runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Ares backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Ares backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Ares backend after ${timeoutMs}ms`))
    })
...
async function waitForHermes(baseUrl, token, signal?) {
  ...
  throw new Error(`Ares backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
// signal so the chat composer doesn't stay stuck on "Starting Ares...".
...
//
// Hosted Ares gateways gate the dashboard behind an OAuth provider (e.g.
...
// single-shot boot() with no retry) surfaced as the "Ares couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Ares
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Ares Cloud agent…' : 'Sign in to Ares gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Ares backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Ares Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// so a single override flips every Ares surface to the same portal.
...
// authenticates via PRIVY, not the Ares gateway session cookies, so this
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Ares Cloud sign-in.'))
...
// Discover the hosted (Ares Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Ares Cloud. Open Settings → Gateway, choose Ares Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Ares Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Ares Cloud session has expired. Sign in to Ares Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Ares Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    remoteUrl,
    // The persisted Ares Cloud org (slug/id) for a cloud connection, or '' for
...
// `org` (optional) is the Ares Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Ares Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Ares gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Ares gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Ares gateway is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Ares backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Ares
...
async function testDesktopConnectionConfig(input: any = {}) {
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Ares on the remote host before connecting with Desktop SSH.'
            }
          }
  ...
  // connect to Ares gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry) {
  ...
  rememberLog(`Starting Ares backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Ares backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Ares backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Ares backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Ares backend for profile "${profile}"`,
    rememberLog
  })
...
async function startHermes() {
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Ares backend', 8)
  ...
  const connectionPromise = (async () => {
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Ares backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Ares backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Ares runtime', 28)
    const backend = await ensureRuntime(resolveHermesBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const hermesCwd = resolveHermesCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Ares backend via ${backend.label}`, 84)
    rememberLog(`Starting Ares backend via ${backend.label}`)
    ...
    if (!processOwner) {
      stopBackendChild(hermesProcess)
      throw new Error('Ares backend start was superseded by a newer connection attempt.')
    }
    ...
    hermesProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Ares backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Ares backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Ares backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Ares backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Ares backend exit (${signal || code})`)
      ...
      rememberLog(`Ares backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Ares backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Ares backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Ares backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Ares backend to become ready', 90)
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Ares backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Ares is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Ares app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Ares icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Ares in the normal
...
// "Starting Ares…". Before the renderer's backoff loop reconnects, it asks us
...
// --- Ares Cloud (cloud-auto-discovery Phase 3) ---
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Ares' TUI into
...
// Resolve the canonical Ares version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Ares version instead of the Electron app's own package.json version,
...
// Re-resolve the live Ares version and push it into the native About panel
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Ares agent venv at ${VENV_ROOT}.`
    }
  }

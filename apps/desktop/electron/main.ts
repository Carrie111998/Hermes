...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    // runBootstrap()'s return type is inferred as a union of its several
    // return-site object literals (ok/cancelled/failedStage/error/marker/
    // installScriptUnavailable are each present on only some branches), so
    // TS can't narrow property access below without an explicit discriminated
    // union. Loosen to `any` here rather than fighting the inference --
    // upstream's own bootstrapResult.cancelled/.failedStage accesses have the
    ...
    if (!bootstrapResult.ok) {
      ...
      const bootstrapError = (
        bootstrapResult.installScriptUnavailable

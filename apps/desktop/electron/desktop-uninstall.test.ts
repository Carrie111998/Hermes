...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'orion_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'orion_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'orion_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Orion.app/Contents/MacOS/Orion', 'darwin'),
    '/Applications/Orion.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Orion.app/Contents/MacOS/Orion', 'darwin'),
    '/Users/x/Applications/Orion.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Orion\\Orion.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Orion'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\orion-desktop\\Orion.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\orion-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Orion.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_OrionXXXX/orion', 'linux', { APPIMAGE: '/home/x/Apps/Orion.AppImage' }),
    '/home/x/Apps/Orion.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/orion/linux-unpacked/orion', 'linux', {}), '/opt/orion/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/orion', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Orion.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Orion.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.orion/orion-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.orion/orion-agent',
    uninstallArgs: ['-m', 'orion_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/orion/linux-unpacked',
    orionHome: '/home/x/.orion'
  })
  ...
  assert.match(script, /'-m' 'orion_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/orion\/linux-unpacked'/)
  assert.match(script, /export ORION_HOME='\/home\/x\/\.orion'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.orion/orion-agent',
    agentRoot: '/home/x/.orion/orion-agent',
    uninstallArgs: ['-m', 'orion_cli.uninstall', '--mode', 'full'],
    appPath: null,
    orionHome: '/home/x/.orion'
  })

  // System python + source on PYTHONPATH so import orion_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.orion\/orion-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'orion_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'orion_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    orionHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'orion_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\orion',
    uninstallArgs: ['-m', 'orion_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Orion',
    orionHome: 'C:\\Users\\x\\AppData\\Local\\orion'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import orion_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\orion;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "orion_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Orion" >nul 2>&1/)

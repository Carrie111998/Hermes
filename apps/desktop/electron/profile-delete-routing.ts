// Profile-delete routing logic for the `orion:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_orion_home() and
...
/** Parse a `orion:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its ORION_HOME.
 *
...
/**
 * Route the next `orion:api` request away from the primary/window backend
 ...
 * ensure_orion_home() recreates the directory the delete just removed.
 */

 ...
 *     `X-Charterforge-Session-Token` header; WS uses `?token=`.
...
// The Nous portal (NAS) does NOT use Charterforge gateway session cookies — it is a
...
// Charterforge Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteCharterforgePath = String(entry.remoteCharterforgePath || '').trim()

  if (remoteCharterforgePath) {
    out.remoteCharterforgePath = remoteCharterforgePath
  }
 ...
 * the Charterforge gateway session cookies, so cloud sign-in / discovery liveness

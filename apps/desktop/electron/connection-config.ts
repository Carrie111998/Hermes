 ...
 *     `X-Orion-Session-Token` header; WS uses `?token=`.
...
// orion_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - orion_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - orion_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / orion #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-orion_session_at', '__Secure-orion_session_at', 'orion_session_at']
const RT_COOKIE_VARIANTS = ['__Host-orion_session_rt', '__Secure-orion_session_rt', 'orion_session_rt']

// The Nous portal (NAS) does NOT use Orion gateway session cookies — it is a
...
// Keep this aligned with orion_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['orion', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Orion profile the remote SSH dashboard should actually run as.
...
// Orion Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteOrionPath = String(entry.remoteOrionPath || '').trim()

  if (remoteOrionPath) {
    out.remoteOrionPath = remoteOrionPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Orion installation. Preserve an explicit mapping
  // when it is a valid Orion profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_orion_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `ORION_HOME` scopes it.
 *
 ...
 * and by the registry SSH branch of the `orion:api` handler — both routes
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://orion.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `orion:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a orion session ACCESS-token cookie
 ...
 * the Orion gateway session cookies, so cloud sign-in / discovery liveness

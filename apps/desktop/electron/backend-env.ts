 ...
 * `iter_orion_node_dirs()` in orion_constants.py, which the Electron main
 * process cannot import.
 */
function orionManagedNodePathEntries(
  orionHome,
  ...
  const root = pathModule.join(orionHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const orionNodeDirs = orionManagedNodePathEntries(orionHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([orionNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeOrionHomeRoot(orionHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!orionHome) {
    return orionHome
  }

  const resolved = pathModule.resolve(String(orionHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). orion_bootstrap sets this inside the child too, but only

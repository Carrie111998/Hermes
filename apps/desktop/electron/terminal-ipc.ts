...
// registry, and the orion:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // ORION_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.ORION_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Orion' TUI into
    ...
    env.TERM_PROGRAM = 'Orion'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a orion/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from ORION_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.ORION_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `orion:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('orion:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('orion:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('orion:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('orion:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('orion:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

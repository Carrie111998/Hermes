 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Orion Agent by spawning
 ...
 *     activeRoot,          // ACTIVE_ORION_ROOT
 *     sourceRepoRoot,      // SOURCE_REPO_ROOT (for dev install.ps1 lookup)
 *     orionHome,          // ORION_HOME
 *     logRoot,             // ORION_HOME/logs
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.orion-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(orionHome) {
  return path.join(orionHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.orion/orion-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(orionHome) {
  if (!orionHome) {
    return null
  }

  const candidate = path.join(orionHome, 'orion-agent', 'scripts', installScriptName())
...
function cachedScriptPath(orionHome, commit) {
  return path.join(bootstrapCacheDir(orionHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/Orion/orion-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(orionHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(orionHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, orionHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ORION_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ORION_HOME: orionHome || process.env.ORION_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, orionHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, orionHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--orion-home', orionHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, orionHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, orionHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, orionHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(orionHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, orionHome, emit })

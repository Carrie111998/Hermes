    ...
    https
      .get(url, res => {
          ...
          https
            .get(res.headers.location, res2 => {
              if (res2.statusCode !== 200) {
                const redirectErr: Error & { statusCode?: number } = new Error(
        ...
        if (res.statusCode !== 200) {
          ...
          const statusErr: Error & { statusCode?: number } = new Error(
            `Failed to download ${scriptName}: HTTP ${res.statusCode} from ${url}`
          )
...
function installScriptUnavailableError(commit) {
  const err: Error & { installScriptUnavailable?: boolean; pinnedCommit?: string } = new Error(
...
async function resolveInstallScript({
  ...
  // Pinned (SHA) refs get the stamp-404 short circuit below: raw.githubusercontent.com
  // URLs for a fixed SHA are immutable, so once a pinned commit 404s this
  // session there is no point re-hitting GitHub for it (and the "not
  // available upstream" message only makes sense for a real pin). Unpinned
  // fallback branch refs (all-zero commit stamps, #50823) don't get that
  // treatment -- a branch ref can legitimately start resolving later, and a
  // 404 there is just a normal fetch failure.
  let downloadError = null

  if (installRef.pinned && NOT_FOUND_COMMITS.has(installRef.ref)) {
    emit({
      type: 'log',
      line:
        `[bootstrap] skipping GitHub fetch for ${installRef.ref.slice(0, 12)}: ` +
        'it already returned HTTP 404 this session (SHA raw URLs are immutable)'
    })
    downloadError = installScriptUnavailableError(installRef.ref)
  } else {
    emit({
      type: 'log',
      line:
    ...
    try {
      ...
      if (installRef.pinned && isNotFoundDownloadError(err)) {
        NOT_FOUND_COMMITS.add(installRef.ref)
      }
    }
  }
  ...
  // A permanent 404 on a pinned commit with nothing to fall back to is
  // unrecoverable by retrying -- replace the bare HTTP error with
  // instructions the user can actually act on. Transient failures (offline,
  // DNS, 5xx) and unpinned-ref 404s rethrow as-is so the retry path stays
  // honest for them.
  if (installRef.pinned && isNotFoundDownloadError(downloadError)) {
    throw installScriptUnavailableError(installRef.ref)
  }
...
async function runBootstrap(opts) {
  ...
  try {
    ...
    const failure: { ok: false; error: string; installScriptUnavailable?: boolean } = {
      ok: false,
    ...
    if (err && err.installScriptUnavailable) {
      ...
      // fallback): let main.ts surface the actionable message directly

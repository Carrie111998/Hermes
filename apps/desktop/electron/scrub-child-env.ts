...
export function childEnvDestinationKey(key: string): string {
  ...
  for (;;) {
    ...
    if (upper.startsWith('_HERMES_FORCE_')) {
      current = current.slice('_HERMES_FORCE_'.length); continue
    }
    if (upper.startsWith('APPTAINERENV_')) {
      current = current.slice('APPTAINERENV_'.length); continue
    }
    if (upper.startsWith('SINGULARITYENV_')) {
      current = current.slice('SINGULARITYENV_'.length); continue
    }
    return current
  }
}
...
const HERMES_83565_ALWAYS_STRIP = new Set([
  ...
  'HERMES_DASHBOARD_SESSION_TOKEN', 'GATEWAY_RELAY_ID', 'GATEWAY_RELAY_SECRET',

 ...
 * origin=git@gitee.com:wintao-storm/QIJI-agent.git. A background
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  assert.equal(canonicalGitHubRemote('git@gitee.com:wintao-storm/QIJI-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('git@gitee.com:wintao-storm/QIJI-agent'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@gitee.com/wintao-storm/QIJI-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://gitee.com/wintao-storm/QIJI-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@gitee.com:wintao-storm/QIJI-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://gitee.com/wintao-storm/QIJI-agent/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isSshRemote detects scp-like and ssh:// forms only', () => {
  assert.equal(isSshRemote('git@gitee.com:wintao-storm/QIJI-agent.git'), true)
  assert.equal(isSshRemote('ssh://git@gitee.com/wintao-storm/QIJI-agent.git'), true)
  assert.equal(isSshRemote('https://gitee.com/wintao-storm/QIJI-agent.git'), false)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  assert.equal(isOfficialSshRemote('git@gitee.com:wintao-storm/QIJI-agent.git'), true)
  assert.equal(isOfficialSshRemote('git@gitee.com:wintao-storm/QIJI-agent'), true)
  assert.equal(isOfficialSshRemote('ssh://git@gitee.com/wintao-storm/QIJI-agent.git'), true)
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  assert.equal(isOfficialSshRemote('https://gitee.com/wintao-storm/QIJI-agent.git'), false)

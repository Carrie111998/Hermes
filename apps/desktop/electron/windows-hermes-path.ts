...
export function resolveVenvHermesCommand(
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Douglas Agent at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Douglas Agent Python at ${python}`,
    command: python,

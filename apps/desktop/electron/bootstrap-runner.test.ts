...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'orion-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/orion-runner-test',
    sourceRepoRoot: null,
    orionHome: '/tmp/orion-runner-test',
    logRoot: '/tmp/orion-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'orion-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'orion-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/orion-agent',
      orionHome: '/tmp/orion'
    }),
    ['--dir', '/tmp/orion-agent', '--orion-home', '/tmp/orion', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/orion-agent', '--orion-home', '/tmp/orion', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/orion',
      orionHome: '/tmp/home'
    }),
    ['--dir', '/tmp/orion', '--orion-home', '/tmp/home', '--branch', 'main']
  )
})

...
const SCAN_MODULE = 'orion_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Orion processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'orion_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Orion process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Orion installation is free.\n' +
    '\n' +
    'Close other Orion windows and terminals, then retry.  If the problem\n' +
    'persists, run `orion update` in a terminal for detailed diagnostics.'
  )
}

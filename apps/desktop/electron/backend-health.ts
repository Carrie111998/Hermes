...
export interface CharterforgeReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForCharterforgeReady`).
   */
...
export async function waitForCharterforgeReady(baseUrl: string, options: CharterforgeReadyOptions): Promise<void> {
  ...
  throw new Error(`Charterforge backend did not become ready: ${detail}`)
}

 ...
 * runtime, remote gateways, Orion Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@orion/shared`) — tsconfig project
...
/** Turn `ls ~/.orion/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Orion Cloud
 * instances are platform-managed — we never run `orion update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteOrionPath: input.remoteOrionPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Orion-managed names; an empty result stores nothing.
 ...
 * `remoteOrionPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Orion Cloud' : 'Remote gateway'),

/**
 * Authoritative connection-identity facade.
 *
 * The registry's storage, migration, roster, and mutation implementation stays
 * byte-for-byte in connection-registry-base.ts. This facade owns only the
 * security-sensitive translation from a resolved route descriptor back to a
 * live registry identity, so every caller imports one canonical contract.
 */

import { normalizeRemoteBaseUrl, normalizeSshConfig, normAuthMode } from './connection-config'
import { matchingConnectionId, type StoredRoute } from './connection-route-identity'
import type { ConnectionRegistry } from './connection-registry-base'

export * from './connection-registry-base'

 ...
 *
 * The staged Tauri `orion-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `orion update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `orion update`, swaps/relaunches
 * the app, and writes .orion-update-result.json. With the app gone before
 * the update starts, the ORION_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `orion update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into ORION_HOME on *every* platform
 * (`orion-setup.exe` on Windows, `orion-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_orion_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `orion update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(orionHome, 'orion-setup.exe')
 ...
 *
 * `copy_self_to_orion_home` deliberately no-ops during `--update`
 ...
 * "Another Orion update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `orion update`, which
 ...
 * the installer is written to ORION_HOME at install/repair time, making mtime

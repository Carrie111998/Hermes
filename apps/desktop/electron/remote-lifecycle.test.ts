...
test('listRemoteOrionProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/ORION_HOME/, '/Users/zillajr/.orion\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteOrionProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteOrionProfiles rejects a hostile ORION_HOME', async () => {
  const ssh = fakeSsh([[/ORION_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteOrionProfiles(ssh),
    (err: any) => {
...
test('locateOrion prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/orion/, 'OK']])
  assert.equal(await locateOrion(ssh, '/opt/orion'), '/opt/orion')
})

test('locateOrion throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong orion" bug.
  const ssh = fakeSsh([
    [/command -v orion/, '/home/u/.local/bin/orion\n'],
    [/\[ -x .*\.local\/bin\/orion/, 'OK']
  ])

  await assert.rejects(
    () => locateOrion(ssh, '/bad/path/orion'),
    (err: any) => {
      assert.equal(err.kind, 'orion-not-found')
      assert.match(err.message, /\/bad\/path\/orion/)
...
test('locateOrion falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v orion/, '/home/u/.local/bin/orion\n'],
    [/\[ -x .*\.local\/bin\/orion/, 'OK']
  ])

  assert.equal(await locateOrion(ssh, ''), '/home/u/.local/bin/orion')
})

test('locateOrion preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$ORION_BIN" "$ORION_ENTRYPOINT" "$@",
  // where $ORION_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.orion/orion-agent/venv/bin/python\n']
  ])

  assert.equal(await locateOrion(ssh, ''), '/home/u/.local/bin/orion')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateOrion must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateOrion returns an explicit remoteOrionPath unchanged', async () => {
  // The override half of #74411: an explicit remoteOrionPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/orion changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/orion/, 'OK'],
    [/python3 -c/, '/home/u/.orion/orion-agent/venv/bin/python\n']
  ])

  assert.equal(await locateOrion(ssh, '~/.local/bin/orion'), '~/.local/bin/orion')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteOrionPath must never be rewritten')
})

test('locateOrion falls back to ~/.local/bin/orion when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateOrion(ssh, ''), '~/.local/bin/orion')
})

test('locateOrion tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/orion/, 'OK']])
  assert.equal(await locateOrion(ssh, ''), '~/.orion/orion-agent/venv/bin/orion')
})

test('locateOrion throws a orion-not-found error with an install hint', async () => {
...
test('locateOrion uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v orion/, '/x/orion'],
    [/\[ -x/, 'OK']
  ])

  await locateOrion(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.orion/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.orion/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.orion/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/orion'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/orion serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/orion'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/orion', '/Users/cd9c/.orion', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /orion-agent.*venv.*bin.*orion/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'orion wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'orion')
    const launcher = path.join(temp, 'orion launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/orion-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/orion', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ORION_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { orionPath: '/x/orion', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ORION_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ORION_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { orionPath: '/x/orion', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nORION_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ORION_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'ORION_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.orion\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/print\("OWNED"/, 'OWNED\n'],
    [/kill 333/, ''],
    [/--version/, 'Orion Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'ORION_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile orionPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ orionPath: '/old/stale/orion', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteOrionPath: '/new/orion', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ORION_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes orionHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ORION_HOME/, '/home/alice/.orion\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ORION_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"orionHome":"\/home\/alice\/\.orion"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ORION_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ORION_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/orion'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.orion/orion-agent/venv/bin/orion'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/orion'))
  assert.doesNotThrow(() => validateRemotePath('~/.orion/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/orion'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('orion'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/orion'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/orion\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/orion\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/orion\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/orion', '/usr/`id`/orion', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.orion/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/orion')
  assert.ok(result.includes('/usr/local/bin/orion'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/orion')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ORION_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/orion', 'work', {
    tokenFilePath: `~/.orion/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.orion\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/orion', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Orion', () => {
  const cmd = buildSpawnCommand('/x/orion', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env ORION_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { orionPath: '/x/orion', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { orionPath: '/x/orion', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile rejects a log path outside the exact ownership and spawn path', async () => {
  const lock = ownedLock({ logPath: '~/.orion/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.orion/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/orion serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/orion serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/orion'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/orion'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/orion'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/orion'), false)
})

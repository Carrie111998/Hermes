export type ParentWatchdogEnv = {
  ORION_PARENT_PID: string
  ORION_PARENT_START_MARKER?: string
  ORION_PARENT_NONCE?: string
  /** Spawn tag consumed by orion_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { ORION_PARENT_PID: String(pid) }
  env.ORION_SPAWN = spawnTag(pid, startMarker)
  ...
  env.ORION_PARENT_START_MARKER = startMarker
  env.ORION_PARENT_NONCE = nonce

  return env
}

/** Build the `ORION_SPAWN` tag mirrored by orion_cli/process_identity.py.

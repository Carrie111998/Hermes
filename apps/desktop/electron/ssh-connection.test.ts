...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env ORION_DASHBOARD_SESSION_TOKEN=abc123deadbeef ORION_DESKTOP=1 orion dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /ORION_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /ORION_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Orion-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Orion-Session-Token: hdr_888'), /X-Orion-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Orion-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('orion', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v orion', 15000)
  assert.equal(args[args.length - 1], 'command -v orion')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `orion-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `orion-ssh-test-${process.pid}-${Date.now()}`)

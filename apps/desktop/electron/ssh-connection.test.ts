...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env CHARTERFORGE_DASHBOARD_SESSION_TOKEN=abc123deadbeef CHARTERFORGE_DESKTOP=1 hermes dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /CHARTERFORGE_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /CHARTERFORGE_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Charterforge-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Charterforge-Session-Token: hdr_888'), /X-Charterforge-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Charterforge-Session-Token: hdr_888').includes('hdr_888'))
})

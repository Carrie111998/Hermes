...
test('Windows snapshot script uses one shell and matching Get-Process start markers', () => {
  ...
  assert.match(script, /Get-Process -Id \$requestedPid -ErrorAction Stop/)
  ...
  assert.match(script, /if\(\$unknown\.ContainsKey\(\$requestedPid\)\)/)
  assert.doesNotMatch(script, /\$PID\b/i)
  assert.doesNotMatch(script, /@\{;/)
  assert.match(script, /catch \{ \$unknown\[\$requestedPid\] = \$true \}/)
  assert.doesNotMatch(script, /\$unknown\.ContainsKey\(\$requestedPid\) -and \$rowsByPid\.ContainsKey/)

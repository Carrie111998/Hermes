 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Charterforge dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Charterforge install on the remote (login-shell probe),
 ...
 *     `CHARTERFORGE_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForCharterforge in.
 *
 * The minted CHARTERFORGE_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^CHARTERFORGE_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
...
async function locateCharterforge(ssh, remoteCharterforgePath) {
  ...
  if (remoteCharterforgePath) {
    if (await isExecutable(remoteCharterforgePath)) {
      return resolveLauncher(remoteCharterforgePath)
    }

    const err: any = new Error(
      `The Charterforge path you set is not an executable on the remote host: "${remoteCharterforgePath}". ` +
  ...
  const err: any = new Error(
    'Charterforge is not installed on the remote host (could not find a `hermes` executable). ' +
      'Install it on the remote with:  curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Charterforge path explicitly in the SSH connection settings.'
  )
...
// e.g. "Charterforge v0.18.2 ..."), or '' on failure. Surfaces WHICH hermes a
// connection uses, so a stale/unexpected install is visible.
async function probeCharterforgeVersion(ssh, hermesPath) {
  try {
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Charterforge Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The CHARTERFORGE_HOME the remote dashboard will use (explicit env wins, else
// ~/.hermes). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteCharterforgeHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${CHARTERFORGE_HOME:-$HOME/.hermes}"')).trim().split('\n').pop()

    return out || '~/.hermes'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Charterforge home.')
...
function buildSpawnCommand(hermesPath, profile, opts: any = {}) {
  ...
  const dashCmd = `env CHARTERFORGE_DESKTOP=1 ${hermes} ${profileArgs}${subCmd}`
...
async function spawnRemoteDashboard(ssh, { hermesPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, hermesPath))) {
    const err: any = new Error(
      'The remote Charterforge install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Charterforge on the remote host to continue using Desktop SSH mode.'
    )
 ...
 * opened SshConnection, forward/pickLocalPort/waitForCharterforge, a token-gated
...
async function connect(deps) {
  ...
  const hermesPath = await locateCharterforge(ssh, remoteCharterforgePath)
  log(`located hermes at ${hermesPath}`)
  const hermesVersion = await probeCharterforgeVersion(ssh, hermesPath)
  ...
  const hermesHome = await probeRemoteCharterforgeHome(ssh)
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForCharterforge(baseUrl, spawnToken)
    assertNotAborted(signal)
...
export {
  ...
  ownershipDirectory,
  pidIsOurDashboard,
  probeCharterforgeVersion,
  probeRemoteCharterforgeHome,
  probeRemotePlatform,
  PROTOCOL_VERSION,
  readLockfile,
  READY_RE,

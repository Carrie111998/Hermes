 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Orion dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Orion install on the remote (login-shell probe),
 ...
 *     `ORION_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForOrion in.
 *
 * The minted ORION_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ORION_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.orion/desktop-ssh'
...
// Resolve the remote orion executable. An EXPLICIT path is honored strictly
...
async function locateOrion(ssh, remoteOrionPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The orion binary or wrapper script
    ...
    //     the Orion version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/zacharyjleach-stack/Aries/issues/74411
    return candidate
  }
  ...
  if (remoteOrionPath) {
    if (await isExecutable(remoteOrionPath)) {
      return resolveLauncher(remoteOrionPath)
    }

    const err: any = new Error(
      `The Orion path you set is not an executable on the remote host: "${remoteOrionPath}". ` +
        'Check the path (it must be the full path to the `orion` binary on the remote, e.g. ' +
        '~/orion-agent/.venv/bin/orion), or clear it to auto-detect.'
    )

    err.kind = 'orion-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v orion')}`)).trim()
  ...
  candidates.push('~/.local/bin/orion')
  candidates.push('/usr/local/bin/orion')
  candidates.push('~/.orion/orion-agent/venv/bin/orion')
  ...
  const err: any = new Error(
    'Orion is not installed on the remote host (could not find a `orion` executable). ' +
      'Install it on the remote with:  curl -fsSL https://your-orion-docs.example/install.sh | sh  ' +
      '— or set the Orion path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<orion> --version`,
// e.g. "Orion Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH orion a
// connection uses, so a stale/unexpected install is visible.
async function probeOrionVersion(ssh, orionPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(orionPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Orion Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ORION_HOME the remote dashboard will use (explicit env wins, else
// ~/.orion). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteOrionHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ORION_HOME:-$HOME/.orion}"')).trim().split('\n').pop()

    return out || '~/.orion'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Orion home.')
...
async function listRemoteOrionProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteOrionHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Orion profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Orion home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'orionPath', 'orionHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !orionPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(orionPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/orion`, leaving neither
      // launcher nor ORION_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `orion_home=os.path.expanduser(${shq(orionHome)}) if ${shq(orionHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if orion_home:\n' +
      ' expected_entries.add(os.path.join(orion_home,"orion-agent","venv","bin","orion"))\n' +
...
function buildSpawnCommand(orionPath, profile, opts: any = {}) {
  const orion = expandRemotePath(orionPath)
  ...
  const dashCmd =
    ...
    `exec env ORION_DESKTOP=1 ${orion} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, orionPath) {
  const orion = expandRemotePath(orionPath)

  const out = await ssh.exec(
    `help="$(${orion} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { orionPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, orionPath))) {
    const err: any = new Error(
      'The remote Orion install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Orion on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(orionPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForOrion, a token-gated
...
async function connect(deps) {
  ...
  const orionPath = await locateOrion(ssh, remoteOrionPath)
  log(`located orion at ${orionPath}`)
  const orionVersion = await probeOrionVersion(ssh, orionPath)

  if (orionVersion) {
    log(`remote orion version: ${orionVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const orionHome = await probeRemoteOrionHome(ssh)
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.orionPath === orionPath &&
      lock.orionHome === orionHome
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForOrion(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)

 ...
 * the three user-facing uninstall modes to the `orion uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m orion_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `orion_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'orion_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Orion.app/Contents/MacOS/Orion  → …/Orion.app
 *   Windows: …\Orion\Orion.exe                 → …\Orion  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Orion.app/Contents/MacOS/Orion → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Orion.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Orion.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Orion$/i.test(dir) || /[\\/]orion-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/orion
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import orion_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, orionHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import orion_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Orion install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ORION_HOME=${String(orionHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]

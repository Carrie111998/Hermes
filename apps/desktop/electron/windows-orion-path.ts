/**
 * windows-orion-path.ts
 *
 * Pure, dependency-injected pieces of Windows `orion` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvOrionCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `orion` shim shadowed the real
 *      orion.cmd/orion.exe; the shim then failed the --version probe and
 ...
 *      and Orion launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvOrionCommand() — unwrapWindowsVenvOrionCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `orion`) must
 * not shadow `orion.cmd`/`orion.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Orion launcher that drives `orion update`.
...
export interface ResolveVenvOrionCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportOrionCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `orion`/`orion.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/orion(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m orion_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportOrionCli(). A
 * venv whose update died mid-`pip install` still has python.exe + orion.exe
 ...
 *
 * Returns null when `command` is not a venv orion shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvOrionCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvOrionCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^orion(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Orion at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Orion Python at ${python}`,
    command: python,
    args: ['-m', 'orion_cli.main', ...backendArgs],
    bootstrap: false,

...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ORION_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Orion-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.orion/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'orion-desktop-ssh')
  }

  return path.join(os.homedir(), '.orion', 'desktop-ssh')
}

// Unit tests for the pure Windows `orion` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvOrionCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `orion` shim shadows
//      the real orion.cmd/orion.exe.
...
//   3. resolveVenvOrionCommand() — must probe the venv python via
//      canImportOrionCli() before trusting it, or a broken venv gets
...
import {
  ...
  resolveVenvOrionCommand
} from './windows-orion-path'
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvOrion: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvOrion: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvOrion: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvOrion: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvOrion: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvOrion: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvOrionCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportOrionCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    orionHome: '/fake/orion-home',
...
test('resolveVenvOrionCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvOrionCommand('/root/venv/Scripts/orion.exe', [], deps), null)
})

test('resolveVenvOrionCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvOrionCommand('/root/venv/Scripts/orion.cmd', [], deps), null)
})

test('resolveVenvOrionCommand: returns null when the basename is not orion/orion.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvOrionCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvOrionCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvOrionCommand('/root/venv/bin/orion.exe', [], deps), null)
})

test('resolveVenvOrionCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvOrionCommand('/root/venv/Scripts/orion.exe', [], deps), null)
})

test('resolveVenvOrionCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportOrionCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvOrionCommand('/root/venv/Scripts/orion.exe', ['serve'], deps)
...
test('resolveVenvOrionCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvOrionCommand('/root/venv/Scripts/orion.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'orion_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvOrionCommand: is case-insensitive on orion.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvOrionCommand('/root/venv/Scripts/ORION.EXE', [], deps))
  assert.ok(resolveVenvOrionCommand('/root/venv/SCRIPTS/orion.exe', [], deps))
})

...
// hardening, ORION_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('orion:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('orion:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('orion:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('orion:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<ORION_HOME>/desktop-plugins`,
  // resolved from the main-process ORION_HOME (see resolveOrionHome) — NOT from
  // the connected backend. A remote backend reports its own `orion_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped orion_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(orionHome, 'profiles', profile) : orionHome
  ...
  ipcMain.handle('orion:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<ORION_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where ORION_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('orion:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<ORION_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('orion:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('orion:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('orion:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('orion:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('orion:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('orion:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()

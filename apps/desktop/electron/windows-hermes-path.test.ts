...
// unwrapWindowsVenvCharterforgeCommand(). These pin the two Windows resolution bugs
...
//   3. resolveVenvCharterforgeCommand() — must probe the venv python via
//      canImportCharterforgeCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvCharterforgeCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportCharterforgeCli: () => true,
...
test('resolveVenvCharterforgeCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvCharterforgeCommand('/root/venv/Scripts/hermes.exe', [], deps), null)
})

test('resolveVenvCharterforgeCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvCharterforgeCommand('/root/venv/Scripts/hermes.cmd', [], deps), null)
})

test('resolveVenvCharterforgeCommand: returns null when the basename is not hermes/hermes.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvCharterforgeCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvCharterforgeCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvCharterforgeCommand('/root/venv/bin/hermes.exe', [], deps), null)
})

test('resolveVenvCharterforgeCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvCharterforgeCommand('/root/venv/Scripts/hermes.exe', [], deps), null)
})

test('resolveVenvCharterforgeCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportCharterforgeCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvCharterforgeCommand('/root/venv/Scripts/hermes.exe', ['serve'], deps)
...
test('resolveVenvCharterforgeCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvCharterforgeCommand('/root/venv/Scripts/hermes.exe', ['serve', '--port', '0'], deps)
...
test('resolveVenvCharterforgeCommand: is case-insensitive on hermes.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvCharterforgeCommand('/root/venv/Scripts/HERMES.EXE', [], deps))
  assert.ok(resolveVenvCharterforgeCommand('/root/venv/SCRIPTS/hermes.exe', [], deps))
})

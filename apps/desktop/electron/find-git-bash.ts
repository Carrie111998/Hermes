 ...
 *   1. ORION_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\orion\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ORION_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ORION_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'orion', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'orion', 'git', 'usr', 'bin', 'bash.exe'))
  }

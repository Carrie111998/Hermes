 ...
 * console. It writes ORION_HOME/.orion-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Orion within 30 minutes.
...
export function handoffResultPath(orionHome: string): string {
  return path.join(orionHome, '.orion-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(orionHome)
  let raw: string

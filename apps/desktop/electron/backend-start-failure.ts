 ...
 * startCharterforge() re-throw the cached error without re-attempting the connect —
...
/**
 * Whether a startCharterforge() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startCharterforge`, re-emits

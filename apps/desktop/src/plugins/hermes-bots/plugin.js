...
// The persisted namespace binds connection-local route ids to one Desktop
// installation. A fresh runtime courier id owns each renewable claim.
let botRelaySupervisor = null
let botRelayCourierNamespace = null

function startBotRelay() {
  // The worker is exported through the live plugin-SDK shim so a runtime/disk
  // plugin remains one self-contained plugin.js. A relative dynamic import
  // cannot resolve from the runtime loader's blob URL.
  if (typeof sdk === 'undefined') {
    return
  }

  const { createBotRelaySupervisor } = sdk

  // Older Desktop SDKs can still load the plain ESM plugin; relay v2 simply
  // stays unavailable until the shell exposes the durable courier runtime.
  if (
    typeof createBotRelaySupervisor !== 'function' ||
    typeof host.botRelayLeadership?.acquire !== 'function' ||
    typeof host.botRelayLeadership?.release !== 'function'
  ) {
    return
  }

  const previous = botRelaySupervisor
  let supervisor = null
  supervisor = createBotRelaySupervisor({
    host,
    resolveHandle: botHandle,
    onLeadershipChange: namespace => {
      if (botRelaySupervisor === supervisor) {
        botRelayCourierNamespace = namespace
      }
    },
    onError: error => {
      if (botRelaySupervisor === supervisor) {
        host.notifyError?.(error, 'Bot relay could not start')
      }
    }
  })

  botRelaySupervisor = supervisor
  botRelayCourierNamespace = null
  void previous?.stop()
  supervisor.start()
}

function stopBotRelay() {
  const supervisor = botRelaySupervisor
  botRelaySupervisor = null
  botRelayCourierNamespace = null
    ...
    ctx.register({
      ...
      data: {
        handler: async draft => {
          ...
          // connection targets carry the Desktop namespace as well as their
          // Desktop-local connection id, so two Desktops whose route is named
          // "local" can never make a user-picked mention ambiguous.
          const lines = mentionedBots.map(bot => {
            const handle = botHandle(bot.name, bot)
            const title = String(botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['hermes-bots']?.title || bot.title || '').trim()
            const target = bot.remoteSource && bot.connectionId
              ? `${handle}@${bot.connectionId}${botRelayCourierNamespace ? `~${botRelayCourierNamespace}` : ''}`
              : handle

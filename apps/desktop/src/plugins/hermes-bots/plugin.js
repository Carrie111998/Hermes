...
/** An explicit open landed: {key, storedId}. storedId '' is the legacy
 *  newChat draft fallback (no stored session to compare focus against). */
...
/** Select one exact roster owner, then open its canonical chat only when the
 *  current Desktop can route that owner without guessing. Remote rows remain
 *  selected on the Bots home until owner-routed session opening lands. */
async function openRosterBot(bot) {
  const generation = ++botOpenGeneration
  const key = botRosterKey(bot)
  const meta = botRosterMeta(bot, $botMeta.get())
  const previewSession = bot.preferred_session || bot.last_session
  ...
  if (bot.remoteSource) {
    // Selection only. A remote bot must never be opened through whatever
    // gateway happens to be live — exact owner-routed remote sessions are
    // upstream work, not this UI's to improvise (#90149, #90006).
    $openBotChat.set(null)

    if (botsHomeEnabled()) {
      // The click is an explicit gesture at this owner: front the home even
      // over a focused chat (which stays alive as a hidden sibling layer).
      openBotsHomeWorkspace(true)
    } else {
      // No home surface on this shell: keep the previous guidance toast so
      // older desktops lose nothing.
      host.notify?.({
        kind: 'info',
        title: displayName(bot, meta),
        message: `Stay in this chat and @${botHandle(bot.name, bot)} to message them. Gateway stays on this device.`
      })
    }
  ...
  try {
    ...
    if (id) {
      $openBotChat.set({ key, storedId: String(id) })
...
function releaseStaleOpenBotChat(focusedStoredId) {
  ...
  const stale = open.storedId ? focused !== open.storedId : Boolean(focused)

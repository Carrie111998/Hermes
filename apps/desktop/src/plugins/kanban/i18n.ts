...
const pl: KanbanMessages = {
  ...
  boardExported: path => `Wyeksportowano tablicę do ${path}`,
  boardImported: name => `Zaimportowano ${name}`,
  boardImportedAs: slug => `Ta nazwa była zajęta — zaimportowano jako ${slug}`,
  renameBoardTitle: 'Zmień nazwę tablicy',
  deleteBoardTitle: name => `Usunąć „${name}”?`,
  deleteBoardConfirm:
    'Tablica zostanie zarchiwizowana, a nie wymazana — jej zadania i załączniki pozostaną na dysku i będzie można je przywrócić.',
  boardArchived: path => `Zarchiwizowano tablicę w ${path}`,

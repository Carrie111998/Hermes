...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `OrionPlugin` and
...
`$ORION_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`orion-desktop-plugins` skill.

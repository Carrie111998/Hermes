/**
 * Orion Bot Mode — a "one chat per agent" roster for the Orion desktop.
 *
 * Left pane "Bots": one row per Orion profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Orion cron jobs) scoped to the
 ...
 * bot-initiated sends use `orion -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import * as sdk from '@orion/plugin-sdk'
import {
  atom,
  Button,
  Checkbox,
...
/** Mirrors host.paneVisibility('orion-bots:pane') — wired in register(). */
...
const GROUP_CHAT_SYNC_META_KEY = 'orion-bots-groups'
 ...
 *  clients (Orion Go, headless backends) see rooms regardless of which
...
// to (local, remote URL, SSH, Orion Cloud, docker) must be able to find
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['orion-bots']?.title || profile?.display_name || ''),
...
async function saveBotMeta(owner, patch) {
  ...
  try {
    ...
    const request = route ? requestForBot(bot, 'profiles.configure', { name, ui_meta: { 'orion-bots': rest } }) :
      host.request('profiles.configure', { name, ui_meta: { 'orion-bots': rest } })
...
// handoffs (`orion -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      Promise.resolve(request)
        .then(() => queryClient.invalidateQueries({ queryKey: ['orion-bots', 'roster'] }))
...
function mergeServerMeta(roster, fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['orion-bots']
...
/** Permanently delete a bot's Orion profile, then remove plugin-local state
 ...
 * directory (orion-agent#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('orion-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'orion-bots-roster-css'
  style.textContent =
    '.orion-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.orion-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes orion-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.orion-bots-pulse { animation: orion-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run orion mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.orionDesktop && window.orionDesktop.openExternal) {
        window.orionDesktop.openExternal(authUrl)
      } else {
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated Orion), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `orion pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (orion-agent bot_mode_probe).
...
function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (orion-agent #86875) also expose the union
 ...
 *  callable alias is 'orion' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'orion' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`orion profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Orion"
 *  can never hijack the primary profile's @orion alias. */
function mentionNameForms(value) {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'orion'].includes(form)
  )
}
...
function botFriendlyNames(bot) {
  ...
  return [bot?.ui_meta?.['orion-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
function newBotChat(bot) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({ kind: 'error', message: 'Update Orion Desktop to open another Bot chat.' })
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'orion') {
      token = byForm.has('orion') ? 'orion' : token
    }
...
// raw Cloud hostname, or to generic 'Orion' in Cloud-only mode.
//
...
// grandfather, or recover. Stored-id pins (ui_meta['orion-bots'].chat) were
...
async function openStoredBotChat(owner, storedId, summary) {
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Orion Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (orion-agent#89617) — clicking Retry
...
async function prepareBotSource(bot) {
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error('Update Orion Desktop to chat with agents on other connections.')
  }
...
async function ensureBotMetadata(bot) {
  ...
  const server = row?.ui_meta?.['orion-bots']
...
function displayName(bot, meta) {
  ...
  // "Orion". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `orion profile rename
  ...
  // never generic "Orion" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Orion (the agent it is)
...
function durableGroupChatMembers(bots) {
  return (bots || []).map(bot => {
    ...
    const title = String(botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['orion-bots']?.title || bot.title || '').trim()
...
// turn in its OWN persistent per-group Orion session and is fed only the
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Orion"
 *  (community report, Aug 21 2026: renamed default still read "Orion is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Orion (matching displayName and the
 *  @orion handle) so the main agent never loses its name in rooms. */
function groupSpeakerLabel(name) {
  ...
  // Core profile display_name (`orion profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Orion' : trimmed
}
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'orion -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `orion profile list` for the LIVE',
...
function previewKind(preview) {
  ...
  if (match) {
    ...
    // profile reads @orion, never @default (#89484).
 ...
 *  (orion-agent#90268). Older gateways omit worker_session — always false. */
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating Orion).'
    })
  }
  ...
  // (orion-agent#87317): Skills (installed list + one-click hub installs +
...
// https://your-orion-docs.example/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'orion-skill-pick', ...} to us (orion-agent#86243). We validate
...
const HUB_ORIGIN = 'https://your-orion-docs.example'
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'orion-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'orion-bots': look } }).catch(() => undefined)
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating Orion).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// orion -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `orion -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Orion-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the Orion schedule string from picker state. */
...
function GroupChatWorkspace({ group, members, onBack, visible = true }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as Orion, remote dupes
...
function BotsHomeView() {
  ...
  const gateway = bot.connectionLabel || (bot.connectionId === 'local' ? 'This device' : 'Orion gateway')
...
function GroupRow({ active, group, members, needsYou, onOpen, onDisband }) {
  ...
  // and the group prompt: the primary profile is @orion, not @default.
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = botRosterMeta(bot, allMeta)?.created || bot.ui_meta?.['orion-bots']?.created || 0
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update Orion and restart the gateway.`
          ...
          : roster.length === 0
            ...
            : allBotsHidden && !hiddenExpanded
              ...
              : rosterRows.length === 0 && matchingHiddenBots.length === 0
                ...
                : jsx(ScrollArea, {
                    className: 'orion-bots-roster min-h-0 flex-1',
                    children: jsx('div', {
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated Orion profile at ',
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @orion.
    ctx.register({
    ...
    if (!document.getElementById('orion-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'orion-bots-keyframes'
      style.textContent = '@keyframes orion-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }

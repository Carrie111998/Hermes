...
function loadProvide({ roster, active = 'default', focused = active, meta = {} } = {}) {
  ...
  const context = {
    ...
    queryClient: {
      ...
      // key-ignoring mock.
      getQueryData: key =>
        key.length === 3 && key[0] === 'orion-bots' && key[1] === 'roster'
          ? roster
          : undefined,
      getQueriesData: () => (Array.isArray(roster?.profiles) ? [[['orion-bots', 'roster', 'local'], roster]] : []),
...
test('offers roster handles for a matching prefix, excluding the active profile', () => {
  ...
  assert.ok(inserts.includes('@orion'), 'default surfaces as @orion')

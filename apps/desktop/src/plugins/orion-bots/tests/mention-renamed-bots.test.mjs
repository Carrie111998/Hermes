...
// Bot Mode title or `orion profile rename` display_name — must update what
...
test('mentionNameForms: slugged + collapsed, reserved tokens dropped', () => {
  ...
  // A bot renamed "Orion" or "everyone" cannot hijack reserved tags.
  assert.equal(mentionNameForms('Orion').length, 0)
...
test('botMentionTag: renamed slug wins, profile handle is the fallback', () => {
  ...
  assert.equal(botMentionTag({ name: 'default' }), 'orion')
  // display_name (orion profile rename) drives the tag too.
...
test('parseGroupChatMentions: display_name and ui_meta title forms address a member', () => {
  ...
  const members = [
    ...
    { name: 'ops', title: '', ui_meta: { 'orion-bots': { title: 'Night Watch' } } }
  ]

...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Orion". Regression:
  ...
  assert.equal(name(active, undefined), 'Orion')
})

test('botHandle: precomputed multi-source handle wins; default stays orion', () => {
  ...
  assert.equal(botHandle('default'), 'orion')
})
...
test('displayName: local default stays Orion; remote default uses the device label', () => {
...
test('resolveRosterMentions: @orion in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'orion' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @orion and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })

...
test('render: BotRow previews the pinned canonical chat, not an unrelated latest session', () => {
  // orion-agent#88200: the row opens the pinned chat on click, so the
...
test('previewKind: the primary profile surfaces as @orion, never @default', () => {
  ...
  assert.equal(fromBotOf("Message from agent 'default': deploy is green"), 'orion')
})

...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Douglas Agent gateway connection closed',
      connectErrorMessage: 'Could not connect to Douglas Agent gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Douglas Agent gateway is not connected',
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.hermesDesktop?.api) {
    throw new Error('Douglas Agent desktop bridge unavailable')
  }

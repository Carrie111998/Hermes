...
export const ja = defineLocale({
  ...
  composer: {
    ...
    rewritePrompt: 'プロンプトを書き換え',
    rewritingPrompt: 'プロンプトを書き換え中…',
    rewriteBasic: '基本的な書き換え',
    rewriteBasicDesc: '範囲を広げず、明確さと文法を改善します。',
    rewriteBrief: '簡潔に展開',
    rewriteBriefDesc: 'タスクを実行可能にするための要点だけを追加します。',
    rewriteDetailed: '詳細に展開',
    rewriteDetailedDesc: 'テストと受け入れ基準を含む実装可能な仕様にします。',
    rewriteEnhance: '強化',
    rewriteEnhanceDesc: '関連する技術的考慮事項を追加し、利用可能なら検出したプロジェクト情報を使用します。',
    rewriteFailed: 'プロンプトを書き換えられませんでした',
    rewriteDraftChanged: '書き換え中に下書きが変更されたため、現在の内容をそのまま残しました。',
    tipPre: 'ヒント: ',

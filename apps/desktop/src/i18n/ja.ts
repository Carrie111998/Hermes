...
export const ja = defineLocale({
  ...
  notifications: {
    ...
    voice: {
      ...
      routeAmbiguous: destination => `「${destination}」に一致するセッションが複数あります。より長いタイトルを指定してください。`,
      routeFailed: title => `「${title}」に送信できませんでした`,
      routeInvalidSyntax: 'セッション名と依頼内容を「and」または間を置いて区切ってください。',
      routeMissingPrompt: destination => `「${destination}」に送信する内容を話してください。`,
      routeNotFound: destination => `「${destination}」に一致するセッションがありません。完全なタイトルを試してください。`,
      routedToSession: title => `「${title}」に送信しました。`,
      routeUnavailable: 'セッションルーティングの準備ができていません。もう一度お試しください。',

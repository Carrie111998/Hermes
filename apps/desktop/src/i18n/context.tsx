...
export interface I18nConfigClient {
  getConfig: () => Promise<CharterforgeConfigRecord>
  saveConfig: (config: CharterforgeConfigRecord) => Promise<{ ok: boolean }>
}
...
const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    ...
    return getCharterforgeConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.hermesDesktop?.api) {
    ...
    return saveCharterforgeConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: CharterforgeConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: CharterforgeConfigRecord, locale: Locale): CharterforgeConfigRecord {

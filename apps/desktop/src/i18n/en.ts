...
export const en: Translations = {
  ...
  settings: {
    ...
    fleetUpdates: {
      title: 'Backend fleet updates',
      intro:
        'Each row is one registered backend. Checks and actions stay pinned to that gateway; aliases of the same install update only once.',
      refresh: 'Refresh backends',
      updateAll: 'Update all',
      updatingAll: 'Updating all…',
      backendTitle: label => `${label} backend`,
      backendVersion: version => `Backend ${version}`,
      deployment: kind => `Install: ${kind}`,
      deploymentKinds: {
        cloud: 'cloud managed',
        desktop: 'desktop managed',
        external: 'externally managed',
        'git-venv': 'git virtualenv',
        image: 'container image',
        launchd: 'launchd service',
        mutable: 'mutable checkout',
      ...
      failed: 'Check failed',
      cloudManaged: 'Managed by Hermes Cloud',
      manual: 'Manual update',
      partial: 'Partially updated',
      applying: 'Updating backend…',
      updated: 'Backend updated',
      restartRequired: 'Gateway restart required',
      restartGateway: 'Restart gateway',
      restarted: 'Gateway restarted',
      applyBackend: 'Update backend',
      copyCommand: 'Copy exact command',
      retry: 'Retry backend',
      manualToast: 'This backend needs the shown manual update command.',
      partialToast: 'The backend update completed only partially. Review this row before retrying.',
      availabilityUnknown: 'Couldn’t verify whether this backend has an update. Refresh it before applying anything.',
      cannotApply: 'This backend cannot apply its update in place.',
      checkFailed: 'Backend update check failed.',
      checkUnavailable: 'Backend update check is unavailable.',
      restartFailed: 'Gateway restart failed.',
      restartNoReturn: 'Gateway did not return with current code.',
      restartRefused: 'Gateway restart was refused.',
      sshOwnedServe: label =>
        `${label} is served through Desktop’s SSH lifecycle. Disconnect it, run the shown command on that host, then reconnect.`,
      updateFailed: 'Backend update failed.',
      updateNoReturn: 'Backend did not return after updating.',
      updateRefused: 'Backend refused the update.',
      botContext: (profiles, platforms) =>
        [
          profiles.length ? `Bot profiles: ${profiles.join(', ')}` : '',
  ...
  updates: {
    ...
    everythingBackendTitle: label => `${label} backend`,
    everythingBackendUpdated: 'Backend update completed.',
    everythingBackendRestarted: 'Gateway restarted with the current backend code.',
    everythingBackendPartial: 'Backend update completed with follow-up work remaining.',
    everythingBackendManualCommand: command => `Update this backend manually: ${command}`,
    everythingBackendManual: 'This backend requires a manual update.',
    everythingBackendManaged: 'This backend is managed by Hermes Cloud.',
    restartSkewedGatewayTitle: 'Backend gateway restart required',
    restartSkewedGatewayBody:
      'This gateway is still running older backend code. Restart only its owning profile before changing models or continuing.',
    restartSkewedGateway: 'Restart backend gateway',
    applyStatus: {
      ...
      partial: 'The backend update completed only partially. Review the backend before retrying.',
      restartingSkewedGateway: 'Restarting the scoped backend gateway with current code…',
      restartFailed: 'The backend gateway restart failed.',
      restartComplete: 'The backend gateway restarted with current code.',
      restartNoReturn: 'The backend gateway did not return with current code.'
    }
  },

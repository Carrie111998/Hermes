...
export const en: Translations = {
  ...
  boot: {
    ready: 'Ares Desktop is ready',
    desktopBootFailedWithMessage: message => `Desktop boot failed: ${message}`,
    steps: {
      connectingGateway: 'Connecting live desktop gateway',
      loadingSettings: 'Loading Ares settings',
      loadingSessions: 'Loading recent sessions',
      startingDesktopConnection: 'Starting desktop connection',
      startingHermesDesktop: 'Starting Ares Desktop…'
    },
    errors: {
      backgroundExited: 'Ares background process exited.',
      backgroundExitedDuringStartup: 'Ares background process exited during startup.',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Your Ares backend is older than this desktop build and may not work correctly. Update to align them.',
    ...
    errors: {
      ...
      methodNotAllowed:
        'The desktop backend rejected that request (405 Method Not Allowed). Try restarting Ares Desktop.',
    ...
    native: {
      ...
      inputBody: 'Ares is waiting for your response.',
      turnDoneTitle: 'Ares finished',
  ...
  settings: {
    ...
    resetConfirm: 'Reset all settings to Ares defaults?',
    ...
    plugins: {
      ...
      blurb:
        'UI extensions loaded into this app — bundled with the build, or dropped into the desktop-plugins folder (including ones Ares writes). Disabling unloads a plugin live and survives restarts.',
      count: n => `${n} installed`,
    ...
    notifications: {
      ...
      focusedHint: 'Completion alerts only fire while Ares is in the background.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Input needed',
          description: 'Ares asked a question or needs a password or secret.'
        },
        turnDone: {
          label: 'Response ready',
          description: 'A turn finished while Ares was in the background.'
        },
        turnError: {
          label: 'Turn failed',
    ...
    appearance: {
      ...
      colorModeDesc: 'Pick a fixed mode or let Ares follow your system setting.',
      ...
      pet: {
        title: 'Pet',
        intro:
          'Adopt an animated petdex mascot that floats over the app and reacts to what Ares is doing — running while tools execute, celebrating on success, sulking on errors.',
        restartHint:
          'Pets need a quick restart — the running app started before this feature was added. Quit and reopen Ares, then come back here.',
        on: 'On',
        off: 'Off',
        scaleTitle: 'Size',
    ...
    about: {
      ...
      automaticUpdatesDesc:
        'Ares checks for updates automatically in the background and lets you know when one is ready.',
    ...
    config: {
      ...
      loading: 'Loading Ares configuration...',
    ...
    gateway: {
      ...
      intro:
        'Ares Desktop starts its own local gateway by default. Use a remote gateway when you want this app to control an already-running Ares backend on another machine or behind a trusted proxy. Pick a profile below to give it its own remote host.',
      ...
      localDesc: 'Start a private Ares backend on localhost. This is the default and works offline.',
      remoteTitle: 'Remote gateway',
      remoteDesc: 'Connect this desktop shell to a remote Ares backend.',
      remoteAuthHint: 'Hosted gateways use OAuth or a username and password; self-hosted ones may use a session token.',
      cloudTitle: 'Ares Cloud',
      cloudDesc: 'Sign in once to Ares Cloud and pick from the agents on your account — no URL to paste.',
      cloudSignInTitle: 'Ares Cloud',
      cloudSignIn: 'Sign in to Ares Cloud',
      cloudSignedIn: 'Signed in to Ares Cloud',
      cloudNeedsSignIn: 'Sign in to Ares Cloud to discover the agents on your account.',
      ...
      cloudDiscoverFailed: 'Could not load your Ares Cloud agents',
      cloudConnectFailed: 'Could not connect to that agent',
      cloudSignInFailed: 'Ares Cloud sign-in failed',
      cloudSignedOutTitle: 'Signed out of Ares Cloud',
      cloudSignedOutMessage: 'Cleared the Ares Cloud session.',
      ...
      restartingMessage: 'Ares Desktop will reconnect using the saved settings — the shell stays open.',
      savedMessage: 'Saved for the next restart.',
      connectedTo: (baseUrl, version) => `Connected to ${baseUrl}${version ? ` · Ares ${version}` : ''}`,
      ...
      sshDesc:
        'Ares is launched on the remote over SSH and tunneled to this app — nothing to start or expose yourself. Requires working key-based SSH access to the host.',
      ...
      sshHermesPathTitle: 'Ares path (optional)',
      ...
      sshReachable: (host, platform) => `Reachable: ${host} (${platform}) — Ares found`,
      ...
      sshErrAuth:
        'SSH authentication failed. Load your key into the ssh-agent (ssh-add) or set an IdentityFile in ~/.ssh/config — Ares runs ssh non-interactively.',
      sshErrHostKey:
        'The host key has CHANGED since you last connected. Verify this is expected, then run ssh-keygen -R <host> and reconnect.',
      sshErrNotInstalled:
        'Ares is not installed on the remote host. Install it there (curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh) or set the Ares path.',
      sshErrPlatform:
        'Unsupported remote platform. Ares Desktop SSH mode supports Linux, macOS, and Windows remote hosts.',
      sshErrTimeout: 'SSH connection timed out. The host may be unreachable or asleep.',
      sshErrUpdateRequired: 'Update Ares on the remote host before connecting with Desktop SSH.',
    ...
    providers: {
      ...
      intro:
        'Sign in with a subscription — no API key to copy. Ares runs the browser sign-in for you, right here in the app.',
      connected: 'Connected',
      collapse: 'Collapse',
      ...
      localEndpoint: {
        ...
        description: 'Point Ares at any OpenAI-compatible endpoint (Zyphra, vLLM, llama.cpp, Ollama, etc).'
      },
  ...
  starmap: {
    ...
    emptyDesc: 'As Ares builds skills and memories for your work, they appear here.',
    share: 'Share map',
    shareHint:
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Restart Ares to use pets — the backend predates this feature.',
    ...
    generatePet: {
      ...
      staleBackend: 'Update Ares to generate pets.',
      backgroundHint: 'You can close this — Ares will notify you when it’s done.',
    ...
    nav: {
      ...
      settings: { title: 'Settings', detail: 'Configure Ares desktop' },
    ...
    hermesActiveSessions: (version, count) => `Ares ${version} · Active sessions ${count}`,
  ...
  profiles: {
    ...
    deleting: 'Deleting...',
    createDesc: 'Profiles are independent Ares environments: separate config, skills, and SOUL.md.',
    nameLabel: 'Name',
  ...
  cron: {
    ...
    newCron: 'New cron',
    emptyDescNew:
      'Schedule a prompt to run on a cron expression. Ares will run it and deliver results to the destination you pick.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Update the Ares backend to create projects — your backend is older than this desktop app (Settings → Updates → Backend).',
      deleteConfirm: 'This removes the saved project from Ares. Files, git repos, and worktrees stay untouched.',
  ...
  composer: {
    ...
    placeholderStarting: 'Starting Ares...',
    placeholderReconnecting: 'Reconnecting to Ares…',
    ...
    attachUrlDesc: 'Ares will fetch the page and include it as context for this turn.',
  ...
  updates: {
    stages: {
      ...
      pull: 'Almost there…',
      pydeps: 'Finishing up…',
      update: 'Updating Ares…',
      rebuild: 'Rebuilding the desktop app…',
      restart: 'Restarting Ares…',
    ...
    unsupportedMessage: 'This version of Ares can’t update itself from inside the app.',
    ...
    availableBody: 'A new version of Ares is ready to install.',
    availableTitleBackend: 'Backend update available',
    availableBodyBackend: 'A newer version of the connected Ares backend is ready to install.',
    availableBodyNoChangelog: 'A newer version is ready. Release notes aren’t available for this install type.',
    updateNow: 'Update now',
    maybeLater: 'Maybe later',
    moreChanges: count => `+ ${count} more change${count === 1 ? '' : 's'} included.`,
    manualTitle: 'Update from your terminal',
    manualBody: 'You installed Ares from the command line, so updates run there too. Paste this into your terminal:',
    manualPickedUp: 'Ares will pick up the new version next time you launch it.',
    guiSkewTitle: 'Update the desktop app',
    guiSkewBody:
      'The backend was updated, but this desktop app package wasn’t changed. Update or reinstall the Ares desktop app (your AppImage / .deb / .rpm) to match.',
    copy: 'Copy',
    copied: 'Copied',
    done: 'Done',
    applyingBody:
      'The Ares updater takes over in its own window and reopens Ares automatically when it’s done. Please don’t reopen Ares yourself while it’s updating.',
    applyingBodyBackend:
      'The remote backend is applying the update and will restart. Ares reconnects automatically when it’s back.',
    applyingClose: 'This window will close while the update runs, then Ares reopens on its own.',
  ...
  install: {
    ...
    oneTimeTitle: 'Ares needs a one-time install',
    ...
    settingUpTitle: 'Setting up Ares Agent',
    finishingTitle: 'Finishing up',
    failedDesc:
      'One of the install steps failed. On Windows, this can happen if another Ares CLI or desktop instance is running. Stop any running Ares instances, then retry. Check the details below or the desktop log for the full transcript.',
    activeDesc:
      'This is a one-time setup. The Ares installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.',
  ...
  onboarding: {
    headerTitle: "Let's get you setup with Ares Agent",
    headerDesc: 'Connect a model provider to start chatting. Most options take one click.',
    preparingInstall: 'Ares is finishing install. This usually takes under a minute on first run.',
    starting: 'Starting Ares…',
    ...
    recommended: 'Recommended',
    connected: 'Connected',
    featuredPitch: 'One subscription, 300+ frontier models — the recommended way to run Ares',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description: 'Point Ares at a local or self-hosted OpenAI-compatible endpoint (vLLM, llama.cpp, Ollama, etc).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Opens a verification page in your browser — Ares connects automatically',
    ...
    authorizeThere: 'Authorize Ares there.',
    ...
    autoBrowser: provider =>
      `We opened ${provider} in your browser. Authorize Ares there and you'll be connected automatically — nothing to copy or paste.`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      connectionCloudTooltip: host => `Connected to Ares Cloud at ${host} · click to manage`,
      ...
      gatewayTitle: 'Ares inference gateway status',
      agents: 'Agents',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} is ${size}. Ares will only show the first 512 KB.`,
    ...
    web: {
      ...
      tryAgain: 'Try again',
      restarting: 'Ares is restarting...',
      askRestart: 'Ask Ares to restart the server',
      lookingRestart: taskId => `Ares is looking for a preview server to restart (${taskId})`,
      restartingTitle: 'Restarting preview server',
      restartingMessage: 'Ares is working in the background. Watch the preview console for progress.',
      ...
      finishedRestarting: message => `Ares finished restarting the preview server${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Ares could not restart the server.',
      stillWorking:
        'Ares is still working, but no restart result has arrived yet. The server command may be running in the foreground.',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Ares is loading a response',
    ...
    approval: {
      gatewayDisconnected: 'Ares gateway is not connected',
      ...
      alwaysDescription: pattern =>
        `This adds the “${pattern}” pattern to your permanent allowlist (~/.hermes/config.yaml). Ares won’t ask again for commands like this — in this session or any future one.`,
  ...
  prompts: {
    ...
    sudoDesc: 'Ares needs your sudo password to run a privileged command. It is sent only to your local agent.',
    sudoPlaceholder: 'sudo password',
    secretTitle: 'Secret required',
    secretDesc: 'Ares needs a credential to continue.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Restart Ares Desktop to use Save Image.',
    restartToSaveImages: 'Restart Ares Desktop to save images',

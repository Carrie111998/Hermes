...
export const en: Translations = {
  ...
  notifications: {
    ...
    voice: {
      ...
      routeAmbiguous: destination => `More than one session matched "${destination}". Try a longer title.`,
      routeFailed: title => `Could not send to "${title}"`,
      routeInvalidSyntax: 'Separate the session title from the request with "and" or a pause.',
      routeMissingPrompt: destination => `Say what to send to "${destination}".`,
      routeNotFound: destination => `No session matched "${destination}". Try the full title.`,
      routedToSession: title => `Sent to "${title}".`,
      routeUnavailable: 'Session routing is not ready. Try again.',

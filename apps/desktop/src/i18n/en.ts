...
export const en: Translations = {
  ...
  settings: {
    ...
    plugins: {
      ...
      couldNotResolveFolder: 'Could not resolve the plugins folder',
      couldNotOpenFolder: 'Could not open the plugins folder',
    ...
    memory: {
      ...
      saved: label => `${label} saved`,
      configUpdated: 'Memory provider configuration updated.',
      saveSettingsFailed: label => `Failed to save ${label} settings`,
      fullConfigTitle: label => `${label} — full configuration`,
      ...
      configReference: label => `${label} configuration reference`,
      saveChanges: 'Save changes',
      loadFailed: 'Memory provider settings failed to load',
      retry: 'Retry',
      loading: 'Loading memory provider settings...',
      settingsFor: label => `${label} settings`,
      setPill: label => `${label} set`,
      notSetPill: label => `${label} not set`,
      fullConfig: 'Full config…',
      saveFieldFailed: label => `Failed to save ${label}`
    },
    customEndpoints: {
      ...
      reachableFound: count => `Endpoint is reachable. Found ${count} models.`,
    ...
    computerUse: {
      ...
      installDriverThenGrant: ' Then grant Accessibility and Screen Recording here.',
      grantsAttach:
    ...
    uninstall: {
      ...
      guiDesc: 'Remove this desktop app. The Hermes agent, your config, and chats all stay.',
      guiConsequence: 'the desktop Chat GUI (this app and its data)',
      liteTitle: 'Uninstall GUI + agent, keep my data',
      liteDesc: 'Remove the app and the Hermes agent, but keep config, chats, and secrets for a future reinstall.',
      liteConsequence: 'the Chat GUI and the Hermes agent (config, chats, and secrets are kept)',
      fullTitle: 'Uninstall everything',
      fullDesc: 'Remove the app, the agent, and all user data — config, chats, scheduled jobs, secrets, logs.',
    ...
    billingPlans: {
      ...
      cannotMakeHere: 'That change cannot be made here.',
      cannotScheduleHere: 'This change cannot be scheduled here.',
      alreadyOn: name => `You are already on ${name} — nothing to change.`,
      changeTo: (name, date) => `Change to ${name} — takes effect ${date}. No charge now; you keep your current plan until then.`,
      monthlyCreditsChange: delta => ` Monthly credits change: ${delta}.`,
      tryAgain: 'Try again',
    ...
    billing: {
      ...
      planValue: (tier, price) => `${tier} · ${price}/mo`,
      ...
      creditsPerMonth: credits => `$${credits} credits/mo`,
      ...
      paymentMethodDesc: 'Manage the card used for top-ups and subscription renewals.',
      buyCreditsNow: 'Buy credits now',
      buyCreditsDesc: 'A single charge on your card, added to your balance today.',
      refillWhenLow: 'Refill when low',
      refillManageDesc: 'Manage auto-refill from the portal.',
      refillTurnOn: 'Turn on auto-refill from the portal',
      refillCharges: card => `Auto-refill charges ${card} — reconcile on the portal`,
      refillChargesConfigured: (reloadTo, threshold) =>
        `Charges ${reloadTo} automatically when your balance falls below ${threshold}.`,
      refillKeepTopped: 'Keep your balance topped up when it drops below your threshold.',
      ...
      planCancelsOn: date => `Cancels on ${date}.`,
      planChangesTo: (tier, date) => `Changes to ${tier} on ${date}.`,
      planRenews: date => `Renews ${date}`,
      subCredits: 'Subscription credits',
      subCreditsRemaining: 'Subscription credits remaining',
      subResets: date => `Resets ${date}`,
      subLeft: (remaining, monthly) => `${remaining} of ${monthly} left`,
      subOver: (remaining, monthly, over) => `${remaining} of ${monthly} left · ${over} over`,
      topupCredits: 'Top-up credits',
      topupNoExpire: 'Does not expire',
      monthlyCap: 'Monthly spend cap',
      monthlyCapUsed: 'Monthly spend cap used',
      monthlyCapUsedOf: (spent, limit) => `${spent} of ${limit} used`,
      capDefault: 'Default ceiling',
      capCustom: 'Monthly remote spending',
      usageFallback: label => `${label} usage`,
      processingSettlement: 'Processing… checking settlement',
      addedRefreshing: amount => `${amount} added. Balance is refreshing.`,
      openPortalLabel: 'Open portal',
      retry: 'Retry',
      customCreditAmount: 'Custom credit amount',
      billingPreviewFixture: 'Billing preview fixture (dev only)',
      preview: 'preview',
      live: 'live',
      provenanceAutoRefill: 'auto-refill card',
      provenanceCustomerDefault: 'customer default',
      provenanceSubPin: 'subscription card'
    },
    billingErrors: {
      consentRequiredTitle: 'Card confirmation needed',
      consentRequiredMessage: 'Confirm this card for terminal charges in the portal',
      insufficientScopeTitle: 'Remote Spending needs approval',
      insufficientScopeMessage: 'This needs Remote Spending allowed. Start a top-up to allow it, then retry.',
      revokedByAdmin: 'An admin stopped remote spending for this terminal.',
      revokedByUser: 'You stopped remote spending for this terminal.',
      revokedTitle: 'Remote spending was stopped',
      revokedMessage: who => `${who} Reconnect from Settings → Gateway to re-authorize this device.`,
      ...
      roleRequiredTitle: 'Admin role required',
      roleRequiredMessage: 'Adding funds needs an org admin/owner. Ask an admin, or manage on the portal.',
      idempotencyConflictTitle: 'Start a fresh top-up',
      idempotencyConflictMessage: '🔴 That charge key was already used for a different amount. Start a fresh top-up.',
      noPaymentMethodTitle: 'No saved card',
      ...
      monthlyCapReachedTitle: 'Monthly spend cap reached',
      monthlyCapReachedMessage: '🔴 Monthly spend cap reached.',
      monthlyCapReachedHeadroom: remaining => `🔴 Monthly spend cap reached — $${remaining} headroom left.`,
      ...
      stripeUnavailableTitle: 'Stripe is having trouble',
      stripeUnavailableMessage: mins => `Stripe is having trouble — try again shortly${mins}`,
      upgradeCapTitle: 'Daily plan-change limit reached',
      upgradeCapMessage: 'Daily plan-change limit reached — try again tomorrow',
      ...
      transportMessage: 'Billing request failed before reaching the gateway.',
      genericFailedTitle: 'Billing request failed',
      genericFailedMessage: 'Billing request failed.',
      retryInMinutes: mins => ` (try again in ~${mins} min)`
    },
    billingCharge: {
      creditsAdded: 'Credits added.',
      amountAdded: amount => `$${amount} added.`,
      failedTitle: 'Charge failed',
      unconfirmedTitle: 'Charge outcome unconfirmed',
      unconfirmedMessage: refusal =>
        `${refusal} Your last charge's outcome is unconfirmed - check your balance/history before retrying.`,
      couldNotCheckTitle: 'Could not check charge',
      couldNotCheckMessage: 'Could not check the charge.',
      noChargeIdMessage: 'The billing service accepted the request but did not return a charge id.',
      noChargeIdTitle: 'Charge could not be tracked',
      timedOutTitle: 'Still processing after 5 minutes',
      timedOutMessage: 'Charge may still settle. Check the portal before retrying.',
      bankVerification:
      ...
      genericFailed: reason => `The charge didn't go through (${reason}).`
    },
    billingStepUp: {
      notApprovedTitle: 'Verification was not approved',
      notApprovedMessage: 'Verification finished without allowing Remote Spending for this terminal.',
      completeTitle: 'Verification complete',
      completeMessage: 'Remote Spending is allowed for this terminal.'
    },
    billingAutoReload: {
      thresholdLabel: 'Threshold',
      thresholdAria: 'Auto-refill threshold',
      reloadToLabel: 'Reload to',
      reloadToAria: 'Auto-refill reload-to amount',
      updated: 'Auto-refill updated.',
      turnedOff: 'Auto-refill turned off.',
      confirmTurnOff: 'Turn off auto-refill?',
      turnOff: 'Turn off',
      cancel: 'Cancel',
      disable: 'Disable',
      saving: 'Saving…',
    ...
    billingAmounts: {
      thresholdLabel: 'Threshold',
      reloadToLabel: 'Reload-to',
      mustBeGreater: 'Reload-to amount must be greater than the threshold.',
      enterAmount: label => `${label}: enter a dollar amount with at most 2 decimal places.`,
      greaterThanZero: label => `${label}: amount must be greater than $0.`,

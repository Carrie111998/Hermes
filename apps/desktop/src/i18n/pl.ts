...
const plTranslatedOverrides = {
  ...
  skills: {
    ...
    toggleToolset: (label, enabled) => `Spróbuj ${enabled ? 'włączyć' : 'wyłączyć'} zestaw narzędzi ${label}`,
  ...
  webhooks: {
    ...
    toggleFailed: (name: string, enabled: boolean) => `Nie udało się ${enabled ? 'włączyć' : 'wyłączyć'} „${name}”`,

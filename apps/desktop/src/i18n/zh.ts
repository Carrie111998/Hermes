...
export const zh: Translations = {
  ...
  settings: {
    ...
    customEndpoints: {
      ...
      saved: '自定义端点已保存。',
      saveFailed: '保存失败',
      reachableModels: count => `端点可访问，发现 ${count} 个模型。`,
      reachable: '端点可访问。',
      ...
      deleteEndpoint: '删除端点',
      emptyTitle: '暂无自定义端点',
      emptyDescription: '请在下方添加兼容 OpenAI 的端点。',
      editTitle: '编辑端点',
      addTitle: '添加端点',
      name: '名称',
    ...
    billing: {
      ...
      threshold: '触发阈值',
      autoRefillThreshold: '自动充值触发阈值',
      reloadTo: '充值至',
      autoRefillReloadTo: '自动充值目标金额',
      disable: '停用',
      turnOffPrompt: '关闭自动充值？',
      turnOff: '关闭',
      customCreditAmount: '自定义充值金额',
      buy: '购买',
      processingSettlement: '处理中…正在确认结算结果',
      addedBalanceRefreshing: amount => `已充值 ${amount}，正在刷新余额。`,
      openPortal: '打开门户',
      openVerification: '打开验证页面',
      dismiss: '关闭提示',
      waitingVerification: '正在等待验证链接…',
      verifyContinue: '验证后继续',
      usageFallback: label => `${label}用量`,
      fixture: name => `测试数据：${name}`,
      updatedAgo: time => `更新于${time}`,
      billingFixture: '账单测试数据',
      live: '实时',
      openPortalExternal: '打开门户 ↗',
      connectAccountMessage: '请在 TUI 中运行 /portal，或打开 Nous 门户连接账户。',
      connectAccountTitle: '连接 Nous 账户',
      balance: '余额',
      plan: '套餐',
      autoRefill: '自动充值',
      enabled: '已启用',
      ...
      updateExternal: '更新 ↗',
      paymentMethodMissingDescription: '请先在门户中添加付款方式，然后再购买充值额度。',
      paymentMethod: '付款方式',
      noCard: '尚未绑定银行卡',
      paymentMethodDescription: '管理用于充值和续订套餐的银行卡。',
      adjustPlanExternal: '调整套餐 ↗',
      subscriptionUnavailable: '订阅详情暂不可用，但仍可打开门户进行管理。',
      renews: date => `续订日期：${date}`,
      subscriptionDescription: '查看套餐并通过账单门户进行更改。',
      opensPortal: '打开门户',
      subscription: '订阅',
      buyCredits: '购买额度',
      buyCreditsDescription: '为套餐外的智能体运行购买充值额度。',
      manage: '管理',
      manageAutoRefillPortal: '请在门户中管理自动充值。',
      autoRefillDescription: '余额低于阈值时自动补充余额。',
      turnOnAutoRefillPortal: '请在门户中启用自动充值',
      differentCard: '另一张银行卡',
      reconcileExternal: '核对 ↗',
      autoRefillCharges: card => `自动充值使用 ${card}，请在门户中核对`,
      refillWhen: (amount, threshold) => `余额低于 ${threshold} 时充值至 ${amount}`,
      subscriptionRemainingOver: (remaining, monthly, over) => `剩余 ${remaining} / ${monthly} · 超用 ${over}`,
      subscriptionRemaining: (remaining, monthly) => `剩余 ${remaining} / ${monthly}`,
      subscriptionCreditsRemaining: '剩余订阅额度',
      resets: date => `重置日期：${date}`,
      subscriptionCredits: '订阅额度',
      topUpCreditsRemaining: '剩余充值额度',
      doesNotExpire: '不会过期',
      topUpCredits: '充值额度',
      amountOfLimitUsed: (amount, limit) => `已用 ${amount} / ${limit}`,
      monthlySpendCapUsed: '月度支出上限使用量',
      defaultCeiling: '默认上限',
      monthlyTerminalBillingSpend: '终端账单月度支出',
      monthlySpendCap: '月度支出上限',
      planMonthly: (tier, price) => `${tier} · ${price}/月`,
      autoRefillCard: '自动充值银行卡',
      customerDefault: '客户默认银行卡',
      subscriptionCard: '订阅银行卡',
      creditsAdded: amount => `已充值 ${amount}。`,
      creditsAddedGeneric: '额度已充值。',
      chargeFailedTitle: '充值失败',
      chargeOutcomeUnconfirmedMessage: message => `${message} 上一次充值结果尚未确认，重试前请检查余额或历史记录。`,
      chargeOutcomeUnconfirmedTitle: '充值结果尚未确认',
      couldNotCheckCharge: '无法检查充值结果。',
      couldNotCheckChargeTitle: '无法检查充值',
      acceptedNoChargeId: '账单服务已接受请求，但未返回充值 ID。',
      chargeCouldNotBeTracked: '无法跟踪充值',
      chargeMayStillSettle: '充值可能仍在结算，重试前请先检查门户。',
      stillProcessing: '处理已超过 5 分钟',
      bankVerificationRequired: '银行要求完成 3DS 验证，请在门户中完成后继续购买。',
      cardExpired: '银行卡已过期，请在门户中更新。',
      cardDeclined: '银行卡被拒，请在门户中改用其他银行卡。',
      chargeProcessingError: reason => `充值未成功（${reason}）。`,
      verificationNotGrantedMessage: '验证已结束，但未授予账单管理权限。',
      verificationNotGrantedTitle: '验证未获批准',
      billingAccessVerifiedMessage: '账单管理权限已验证。',
      billingAccessVerifiedTitle: '验证完成',
      reloadGreaterThanThreshold: '充值目标金额必须大于触发阈值。',
      invalidAmount: label => `${label}：请输入最多保留 2 位小数的美元金额。`,
      amountGreaterThanZero: label => `${label}：金额必须大于 $0。`,
      minimumAmount: (label, minimum) => `${label}：最低金额为 ${minimum}。`,
      maximumAmount: (label, maximum) => `${label}：最高金额为 ${maximum}。`,
      errors: {
        tooManyChargesRetry: minutes => `🟡 当前充值请求过多（约 ${minutes} 分钟后重试）。这不是付款失败。`,
        tooManyCharges: '🟡 当前充值请求过多。这不是付款失败。',
        stripeRetry: minutes => `Stripe 暂时异常，请约 ${minutes} 分钟后重试`,
        ...
        idempotencyTitle: '重新发起充值',
        noPaymentMethodMessage: '💳 尚未保存终端充值银行卡。请在门户中设置（单次购买不会保存可复用银行卡）。',
        noPaymentMethodTitle: '未保存银行卡',
        orgAccessMessage: '此令牌未绑定到你可以管理的组织',
        orgAccessTitle: '无组织访问权限',
        monthlyCapRemaining: remaining => `🔴 已达到月度支出上限，剩余可用空间 $${remaining}。`,
        monthlyCapMessage: '🔴 已达到月度支出上限。',
        monthlyCapTitle: '已达到月度支出上限',
        tooManyChargesTitle: '当前充值请求过多',
        stripeTroubleTitle: 'Stripe 暂时异常',
        planLimitMessage: '已达到每日套餐变更上限，请明天重试',
        planLimitTitle: '已达到每日套餐变更上限',
        endpointUnavailableMessage: '账单端点返回了非 JSON 响应，此部署可能未提供该端点。',
        endpointUnavailableTitle: '账单端点不可用',
        timeoutMessage: '账单请求超时。',
        timeoutTitle: '账单请求超时',
        transportMessage: '账单请求在到达网关前失败。',
        transportTitle: '账单连接失败',
        defaultMessage: '账单请求失败。',
        defaultTitle: '账单请求失败'
      }
    },
    plugins: {
    ...
    memoryProvider: {
      ...
      loadFailedDetail: message => `记忆提供方设置加载失败：${message}`,
      savedTitle: provider => `${provider} 已保存`,
      savedMessage: '记忆提供方配置已更新。',
      saveFailed: provider => `无法保存 ${provider} 设置`,
      saveFieldFailed: field => `保存 ${field} 失败`,
      loading: '正在加载记忆提供方设置…',
      settings: provider => `${provider} 设置`,
      fieldSet: field => `${field} 已设置`,
      fieldNotSet: field => `${field} 未设置`,
      fieldAbout: field => `关于 ${field}`,
      fullConfig: '完整配置…',
      fullConfigurationTitle: provider => `${provider} — 完整配置`,
      fullConfigurationDescription: (provider, profile) =>
        `${provider} 在“${profile}”配置中的全部选项。留空字段将回退到解析后的主机值或内置默认值。`,
      configReference: provider => `${provider} 配置参考`,
      otherGroup: '其他',
      saveChanges: '保存更改',

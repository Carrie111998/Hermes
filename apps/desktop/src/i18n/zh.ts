...
export const zh: Translations = {
  ...
  install: {
    ...
    connectExistingDesc: 'HTTPS 网关，或 ssh://host 通过 SSH 连接。不会启动本地安装。',
    ...
    remoteSetupDesc: '粘贴 https:// 网关 URL，或 ssh://host 通过 SSH 连接。',
    remoteUrlTitle: '网关 URL',
    remoteUrlDesc: '公开仪表盘用 https://，SSH 用 ssh://botnet（user@host，可选 :port 和 ?profile=）。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes 或 ssh://botnet',
    sshDetected: host =>
      `将通过 SSH 连接 ${host}。测试并应用后，Desktop 会用 ssh-agent 或 ~/.ssh/config 挂到该机器上的 Hermes serve。`,

...
export const zh: Translations = {
  ...
  settings: {
    ...
    model: {
      ...
      provider: '提供方',
      model: '模型',
      openrouterModelInput: 'OpenRouter 模型',
      openrouterModelShapeHint: '请使用 author/slug 格式的 OpenRouter 模型 ID；可以包含可选的 :suffix。',
      applying: '应用中...',
      defaultsLabel: '默认值',
      reasoning: '推理',
      reasoningOff: '关闭',
      ...
      openrouterRouting: {
        allowFallbacks: '允许备用提供方',
        automatic: '自动 — 由 OpenRouter 选择',
        blocked: '已屏蔽',
        block: '屏蔽',
        discoveryFailed: '端点发现失败',
        endpoint: 'OpenRouter 端点',
        lock: '锁定到此端点',
        manual: '手动输入提供方标签',
        noEndpoints: '未报告符合条件的端点',
        notCurrentlyReported: '当前未报告',
        prefer: '优先使用此端点',
        providerTag: '提供方标签',
        quantization: '量化',
        refresh: '刷新端点',
        refreshing: '正在加载端点…',
        routingMode: 'OpenRouter 路由模式',
        selected: '已选择',
        subtitle: '选择要使用的提供方，或屏蔽不想使用的提供方。',
        title: 'OpenRouter 路由',
        undo: '撤销',
        unblock: '取消屏蔽',
        summaryAutomatic: 'OpenRouter 会选择最佳可用提供方。',
        summarySelectedOnly: '请求仅使用 {endpoint}。',
        summarySelectedPrefer: '优先使用 {endpoint}；不可用时可以使用其他提供方。',
        summaryBlockedOnly: 'OpenRouter 会选择除 {blocked} 之外的任何提供方。',
        summarySelectedPreferBlocked: '优先使用 {endpoint}；不可用时可回退到除 {blocked} 之外的其他提供方。',
        summaryEndpointWithQuantization: '{provider}（{quantization}）',
        summaryBlockedJoinTwo: '{first}和{second}',
        summaryBlockedJoinMany: '{items}、{last}',

...
export const zh: Translations = {
  ...
  providerManager: {
    title: '提供方管理器',
    description: '启用提供方、添加自定义端点，并管理在选择器中显示的模型。',
    filters: '筛选',
    providerListHeading: '提供方',
    modelsHeading: '模型',
    searchPlaceholder: '搜索模型',
    noProviders: '没有可用模型的提供方。',
    noModels: '未找到模型。',
    activeCount: (n: number) => `${n} 个已启用`,
    allHidden: '所有模型已隐藏 — 不会出现在选择器里。',
    addProvider: '添加提供方…',
    selectProviderHint: '选择一个提供方以管理其模型。',
    editProvider: '编辑提供方',
    providerName: '名称',
    ...
    apiKey: 'API 密钥',
    apiKeyPlaceholder: '可选 — 留空以保留已保存的密钥',
    editProviderCredentials: '编辑凭据',
    baseUrlOverride: 'Base URL 覆盖',
    baseUrlOverridePlaceholder: '留空以使用默认端点',
    apiKeyDescription: '存储在配置文件的 .env 文件中',
    configureProvider: '配置提供方',
    configureDescription: '输入 API 密钥以启用此提供方并发现其模型。',
    saveAndDiscover: '保存并发现模型',
    apiKeyRequired: 'API 密钥为必填项。',
    configuring: '保存中…',
    apiMode: 'API 模式',
    apiModeChat: '聊天补全（兼容 OpenAI）',
    apiModeAnthropic: 'Anthropic 消息',
    models: '模型',
    modelsPlaceholder: '每行一个模型 ID，例如\ngpt-4o\nclaude-sonnet-4',
    confirmDelete: '删除此提供方？此操作无法撤销。',
    nameExists: '已存在同名提供方。',
    invalidUrl: '基础 URL 必须是有效的 http(s) 地址。',
    enableProvider: '启用提供方',
    disableProvider: '禁用提供方',
    discoverModels: '自动发现模型',
    updateList: '更新列表',
    addModel: '添加模型',
    discoveryFailed: '模型发现失败',
    discoveryEmpty: '未发现新模型',
    listRefreshed: '模型列表已刷新',
    discoverySuccess: (n: number) => `已添加 ${n} 个模型`,
    manualModelTitle: '添加模型',
    manualModelDescription: '注册此提供方暴露的单个模型。',
    modelId: '模型 ID',
    modelIdPlaceholder: '例如 gpt-4o',
    modelName: '显示名称',
    modelNamePlaceholder: '在应用中显示的可选名称',
    advancedParameters: '高级参数',
    advancedParametersEmpty: '暂无高级参数。',
    modelIdRequired: '模型 ID 为必填项',
    modelExists: '已存在该 ID 的模型',
    groupLocal: '本地',
    groupConfigured: '已配置',

...
export const zh: Translations = {
  ...
  settings: {
    ...
    about: {
      ...
      onLatest: '你已是最新版本。',
      installing: '正在安装更新。',
      gatewayRestartRequired: '网关正在运行旧代码，需要重启。',
    ...
    fleetUpdates: {
      title: '后端集群更新',
      intro: '每行代表一个已注册后端。检查和操作始终限定到该网关；同一安装的别名只更新一次。',
      refresh: '刷新后端',
      updateAll: '全部更新',
      updatingAll: '正在全部更新…',
      backendTitle: label => `${label} 后端`,
      backendVersion: version => `后端 ${version}`,
      deployment: kind => `安装类型：${kind}`,
      deploymentKinds: {
        cloud: '云端托管',
        desktop: '桌面客户端管理',
        external: '外部管理',
      ...
      copyCommand: '复制准确命令',
      retry: '重试后端',
      manualToast: '此后端需要运行所显示的手动更新命令。',
      partialToast: '后端更新仅部分完成。重试前请检查此行。',
      availabilityUnknown: '无法确认此后端是否有可用更新。应用任何更新前请先刷新。',
      cannotApply: '此后端无法直接应用更新。',
      checkFailed: '后端更新检查失败。',
      checkUnavailable: '后端更新检查不可用。',
      restartFailed: '网关重启失败。',
      restartNoReturn: '网关未能以当前代码恢复。',
      restartRefused: '网关拒绝重启。',
      sshOwnedServe: label =>
        `${label} 由 Desktop 的 SSH 生命周期提供服务。请断开连接，在该主机上运行显示的命令，然后重新连接。`,
      updateFailed: '后端更新失败。',
      updateNoReturn: '更新后后端未恢复在线。',
      updateRefused: '后端拒绝更新。',
      botContext: (profiles, platforms) =>
        [
          profiles.length ? `Bot 配置档案：${profiles.join('、')}` : '',
  ...
  updates: {
    ...
    everythingBackendTitle: label => `${label} 后端`,
    everythingBackendUpdated: '后端更新已完成。',
    everythingBackendRestarted: '网关已使用当前后端代码重启。',
    everythingBackendPartial: '后端更新已完成，但仍需后续处理。',
    everythingBackendManualCommand: command => `手动更新此后端：${command}`,
    everythingBackendManual: '此后端需要手动更新。',
    everythingBackendManaged: '此后端由 Hermes Cloud 管理。',
    restartSkewedGatewayTitle: '需要重启后端网关',
    restartSkewedGatewayBody: '此网关仍在运行旧版后端代码。切换模型或继续操作前，仅重启其所属配置档案。',
    restartSkewedGateway: '重启后端网关',
    applyStatus: {
      preparing: '正在更新后端…',
      pulling: '后端更新中…',
      ...
      partial: '后端更新仅部分完成。重试前请检查后端。',
      restartingSkewedGateway: '正在用当前代码重启指定的后端网关…',
      restartFailed: '后端网关重启失败。',
      restartComplete: '后端网关已用当前代码重启。',
      restartNoReturn: '后端网关未能以当前代码恢复。'
    }
  },

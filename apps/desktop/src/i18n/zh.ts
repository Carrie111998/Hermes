...
export const zh: Translations = {
  ...
  settings: {
    ...
    moa: {
      title: 'MoA 工作室',
      description: '创建命名的 Mixture of Agents 预设，调整每个席位，并直接切换当前对话。',
      preset: '预设',
      presetName: '预设名称',
      newPresetName: '新预设名称',
      renamePreset: '重命名预设',
      ...
      deletePreset: '删除预设',
      deleteConfirm: name => `从草稿中移除 MoA 预设“${name}”？请点击“保存更改”以应用。`,
      setDefault: '设为默认',
      defaultBadge: '默认',
      activeBadge: '当前',
      useInThisChat: '在当前对话中使用',
      presetEnabled: '启用预设',
      enabledDescription: '禁用的预设会保留配置，但不能用于 MoA 执行。',
      referencesTitle: '参考席位',
      reference: index => `参考 ${index}`,
      referenceProvider: index => `参考 ${index} 提供方`,
      referenceModel: index => `参考 ${index} 模型`,
      referenceEffort: index => `参考 ${index} 推理强度`,
      referenceEnabled: (n: number) => `启用参考 ${n}`,
      aggregator: '聚合器',
      aggregatorProvider: '聚合器提供方',
      aggregatorModel: '聚合器模型',
      aggregatorEffort: '聚合器推理强度',
      providerDefault: '提供方默认',
      addReference: '添加参考',
      removeReference: index => `移除参考 ${index}`,
      moveReferenceUp: index => `上移参考 ${index}`,
      moveReferenceDown: index => `下移参考 ${index}`,
      moveUp: '上移',
      moveDown: '下移',
      remove: '移除',
      executionTitle: '执行与输出',
      cadence: '顾问运行频率',
      cadenceDescription: '选择顾问是每个用户回合运行一次、每次工具迭代后运行，还是每 N 次工具迭代运行。',
      oncePerUserTurn: '每个用户回合一次',
      everyNIterations: '每 N 次工具迭代',
      everyNCount: 'N（次数）',
      everyToolIteration: '每次工具迭代后',
      advisorOutputCap: '顾问输出上限',
      advisorOutputCapDescription: '每个参考席位返回的最大词元数。',
      uncappedAdvisorOutput: '顾问输出不设上限',
      aggregatorOutputLimit: '聚合器输出上限',
      aggregatorOutputLimitDescription: '聚合器返回的最大词元数。',
      advisorTemperature: '顾问温度',
      advisorTemperatureDescription: '留空以使用各参考提供方的默认值。',
      aggregatorTemperature: '聚合器温度',
      aggregatorTemperatureDescription: '留空以使用聚合器提供方的默认值。',
      saveChanges: '保存更改',
      unsaved: '有未保存的更改。设置完成后请点击“保存更改”。',
      saving: '保存中…',
      saved: '已保存',
      saveFailedRetained: '保存失败 — 更改仍保留为未保存状态。',
      incomplete: '请先补全提供方、模型或数值，再进行保存。',
      nameBlank: '请输入预设名称。',
      nameDuplicate: '已存在同名预设。',
      unavailable: '此后端不支持 MoA 预设。'
    },
    providers: {
      connectAccount: '连接账号',

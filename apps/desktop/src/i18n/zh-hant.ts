...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    moa: {
      title: 'MoA 工作室',
      description: '建立具名的 Mixture of Agents 預設，調整每個席位，並直接切換目前聊天。',
      preset: '預設',
      presetName: '預設名稱',
      newPresetName: '新預設名稱',
      renamePreset: '重新命名預設',
      ...
      deletePreset: '刪除預設',
      deleteConfirm: name => `從草稿中移除 MoA 預設「${name}」？請點選「儲存變更」以套用。`,
      setDefault: '設為預設',
      defaultBadge: '預設',
      activeBadge: '目前',
      useInThisChat: '在目前聊天中使用',
      presetEnabled: '啟用預設',
      enabledDescription: '停用的預設會保留設定，但無法用於 MoA 執行。',
      referencesTitle: '參考席位',
      reference: index => `參考 ${index}`,
      referenceProvider: index => `參考 ${index} 提供方`,
      referenceModel: index => `參考 ${index} 模型`,
      referenceEffort: index => `參考 ${index} 推理強度`,
      referenceEnabled: (n: number) => `啟用參考 ${n}`,
      aggregator: '聚合器',
      aggregatorProvider: '聚合器提供方',
      aggregatorModel: '聚合器模型',
      aggregatorEffort: '聚合器推理強度',
      providerDefault: '提供方預設',
      addReference: '新增參考',
      removeReference: index => `移除參考 ${index}`,
      moveReferenceUp: index => `上移參考 ${index}`,
      moveReferenceDown: index => `下移參考 ${index}`,
      moveUp: '上移',
      moveDown: '下移',
      remove: '移除',
      executionTitle: '執行與輸出',
      cadence: '顧問執行頻率',
      cadenceDescription: '選擇顧問是每個使用者回合執行一次、每次工具迭代後執行，或每 N 次工具迭代執行。',
      oncePerUserTurn: '每個使用者回合一次',
      everyNIterations: '每 N 次工具迭代',
      everyNCount: 'N（次數）',
      everyToolIteration: '每次工具迭代後',
      advisorOutputCap: '顧問輸出上限',
      advisorOutputCapDescription: '每個參考席位回傳的最大詞元數。',
      uncappedAdvisorOutput: '顧問輸出不設上限',
      aggregatorOutputLimit: '聚合器輸出上限',
      aggregatorOutputLimitDescription: '聚合器回傳的最大詞元數。',
      advisorTemperature: '顧問溫度',
      advisorTemperatureDescription: '留空以使用各參考提供方的預設值。',
      aggregatorTemperature: '聚合器溫度',
      aggregatorTemperatureDescription: '留空以使用聚合器提供方的預設值。',
      saveChanges: '儲存變更',
      unsaved: '有未儲存的變更。設定完成後請點選「儲存變更」。',
      saving: '儲存中…',
      saved: '已儲存',
      saveFailedRetained: '儲存失敗 — 變更仍保留為未儲存狀態。',
      incomplete: '請先補齊提供方、模型或數值，再進行儲存。',
      nameBlank: '請輸入預設名稱。',
      nameDuplicate: '已存在同名預設。',
      unavailable: '此後端不支援 MoA 預設。'
    },
    providers: {
      connectAccount: '連結帳號',

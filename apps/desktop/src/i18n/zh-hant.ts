...
export const zhHant = defineLocale({
  ...
  notifications: {
    ...
    voice: {
      ...
      routeAmbiguous: destination => `有多個工作階段符合「${destination}」。請使用更長的標題。`,
      routeFailed: title => `無法傳送至「${title}」`,
      routeInvalidSyntax: '請用「and」或停頓分隔工作階段標題與要求。',
      routeMissingPrompt: destination => `請說出要傳送至「${destination}」的內容。`,
      routeNotFound: destination => `找不到符合「${destination}」的工作階段。請嘗試完整標題。`,
      routedToSession: title => `已傳送至「${title}」。`,
      routeUnavailable: '工作階段路由尚未就緒。請再試一次。',

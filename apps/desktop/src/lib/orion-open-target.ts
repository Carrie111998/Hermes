 ...
 * `orion://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `orion://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `orion://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type OrionOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }
...
export function normalizeOrionOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('orion://') || trimmed.startsWith(`${ORION_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // orion://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // orion://index-network/intent/1 → /index-network/intent/1
...
export function resolveOrionOpenPath(target: OrionOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeOrionOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeOrionOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeOrionOpenString(target.path)
 ...
 * (`orion://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromOrionDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveOrionOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveOrionOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `orion://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromOrionDeepLink('open', name, params)
}

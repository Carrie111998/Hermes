...
export function installBrowserDesktopBridge(): boolean {
  ...
  const startBrowserTerminal = async (options?: {
    ...
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:'
    const url = new URL(`${protocol}//${window.location.host}${bootstrap.basePath}/api/terminal`)
    ...
    return new Promise<HermesTerminalSession>((resolve, reject) => {
        ...
        if (typeof event.data === 'string') {
          try {
            ...
            if (payload.type === 'ready') {
              ...
              state.shell = payload.shell || state.shell
            ...
            if (payload.type === 'error') {
              rejectStart(payload.message || 'Browser terminal failed to start')

              return
            }
          } catch {
            // A text PTY frame is ordinary terminal output, not metadata.
          }
      ...
      socket.onerror = () => rejectStart('Browser terminal WebSocket failed to connect')

      socket.onclose = event => {
        emitTerminalData(state, state.decoder.decode())

        if (!settled) {
          rejectStart(event.reason || `Browser terminal closed before startup (${event.code})`)
        }
  ...
  const bridge: Window['hermesDesktop'] = {
    ...
    terminal: {
      cwd: async (id: string) => {
        ...
        try {
          const result = await api<{ cwd?: null | string }>({
            path: queryPath('/api/terminal/cwd', { id }),
            profile: browserProfile()
          })

          return result.cwd || state.cwd || null
        } catch {
      ...
      dispose: async (id: string) => {
        ...
        try {
          ...
          // Socket may already be closed after the shell exited.
        }

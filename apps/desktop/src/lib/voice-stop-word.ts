...
//
// When someone is in a hands-free "Hey Orion" voice chat, the natural way to
...
// phrase (optionally addressed to Orion), so a real turn that merely contains
...
// Optional address prefixes so "orion stop" / "ok stop" / "hey orion, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey orion', 'hey orion,', 'orion', 'orion,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("orion") is not a stop command on its own.
      continue
    }
 ...
 * to Orion). Returns false for anything that merely contains "stop" as part of

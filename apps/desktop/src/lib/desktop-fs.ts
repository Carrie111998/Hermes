import type {
  OrionConnection,
  OrionReadDirResult,
  OrionReadFileTextResult,
  OrionSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: OrionConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: OrionConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<OrionReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<OrionReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<OrionReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<OrionReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.orionDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: OrionSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()

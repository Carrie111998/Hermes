...
/** Why a known Charterforge command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Charterforge profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Charterforge version', surface: exec() },
...
function isKnownCharterforgeSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Charterforge' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownCharterforgeSlashCommand(normalized)
}

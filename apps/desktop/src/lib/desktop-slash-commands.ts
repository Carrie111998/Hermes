...
/** Why a known Orion command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Orion profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Orion Agent version', surface: exec() },
...
function isKnownOrionSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Orion' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownOrionSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Orion and were never asked for. It is

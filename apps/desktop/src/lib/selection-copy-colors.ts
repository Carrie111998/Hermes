...
// Orion is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.orionMode
 ...
 * the only carriers of Orion' palette — are dropped so the paste target's

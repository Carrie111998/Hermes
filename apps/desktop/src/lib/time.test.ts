...
describe('sessionBucketLabel', () => {
  ...
  it('formats month (same year) and month + year (prior year) via Intl', () => {
    expect(labelAt(2026, 2, 3)).toBe(
      new Intl.DateTimeFormat(undefined, { month: 'long' }).format(new Date(2026, 2, 3))
    )
    expect(labelAt(2025, 11, 3)).toBe(
      new Intl.DateTimeFormat(undefined, { month: 'long', year: 'numeric' }).format(new Date(2025, 11, 3))
    )
  })
})

...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __orionWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__orionWatch || (win.__orionWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('orion-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('orion-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __orionWatchTag?: number }).__orionWatchTag

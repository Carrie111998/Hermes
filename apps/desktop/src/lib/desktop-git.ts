...
// (window.nastechDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['nastechDesktop']>['git']>
...
const remoteGit: GitBridge = {
  ...
  review: {
    ...
    prList: (repoPath, branches, numbers) =>
      gitPost<NastechRepoPullRequests>('review/pr-list', { branches, numbers: numbers ?? [], path: repoPath }),

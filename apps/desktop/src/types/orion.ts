...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Orion doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in orion_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `orion model` uses — instead of
...
export type OrionConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `orion model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Orion simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (orion.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors orion_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Orion, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `orion tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Orion); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** orion-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** orion-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/orion/update/check` — the backend's own update state.

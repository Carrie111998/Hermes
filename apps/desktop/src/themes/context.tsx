...
const SKIN_KEY = 'orion-desktop-theme-v2'
const MODE_KEY = 'orion-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'orion-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'orion-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'orion-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.orionTheme = skinName
  root.dataset.orionMode = rendered
  ...
  window.orionDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('orion-boot-background', chromeBg)
    window.localStorage.setItem('orion-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.orionThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.orionDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Orion authoring/activating a skin from a

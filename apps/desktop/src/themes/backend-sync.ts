/**
 * Live skin sync from the Orion backend.
 *
 * The backend resolves the active skin (built-in or `$ORION_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Orion
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { OrionSkin } from '@orion/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: OrionSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as OrionSkin)

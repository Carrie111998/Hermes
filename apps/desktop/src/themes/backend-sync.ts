/**
 * Live skin sync from the Charterforge backend.
 *
 * The backend resolves the active skin (built-in or `$CHARTERFORGE_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Charterforge
 ...
 */
export function ingestBackendSkin(skin: CharterforgeSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as CharterforgeSkin)

...
function sessionsBotsTree() {
  ...
  registerPane('orion-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'orion-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('orion-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('orion-bots:pane')
    expect($hiddenStripTabs.get()).toContain('orion-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('orion-bots:pane')

    expect(setStripTabHidden('orion-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('orion-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('orion-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('orion-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('orion.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('orion-bots:pane')
    ...
    revealTreePane('orion-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('orion-bots:pane')
    expect(window.localStorage.getItem('orion.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'orion-bots:pane', title: 'orion-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'orion-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))

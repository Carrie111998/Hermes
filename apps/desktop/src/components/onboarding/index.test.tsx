...
describe('onboarding Picker', () => {
  it('features Nastech Portal and hides other providers behind a disclosure', () => {
    ...
    // Fireworks is the always-visible #2 slot (after Nastech), even while OAuth
  ...
  it('shows Fireworks in slot #2 ahead of other OAuth providers', () => {
    setProviders([
      ...
      provider('nastech', 'Nastech Portal')
    ])
    render(<Picker ctx={ctx} />)
    fireEvent.click(screen.getByRole('button', { name: 'Other providers' }))
    ...
    const labels = screen
      ...
      .filter(text => /Nastech Portal|Fireworks AI|OpenAI OAuth|MiniMax|OpenRouter/.test(text))

    const indexOf = (needle: string) => labels.findIndex(text => text.includes(needle))
    expect(indexOf('Nastech Portal')).toBeGreaterThanOrEqual(0)
    expect(indexOf('Fireworks AI')).toBeGreaterThan(indexOf('Nastech Portal'))

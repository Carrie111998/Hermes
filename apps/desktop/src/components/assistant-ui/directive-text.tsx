...
const ORION_REF_TYPES = WIRE_REFERENCE_KINDS
type OrionRefType = (typeof ORION_REF_TYPES)[number]
...
const ORION_DIRECTIVE_RE = referenceRe()
...
export const orionDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ORION_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...orionDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Orion directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.orionDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.orionDesktop?.readFileDataUrl(id)

 ...
 * own design.
 *
 * PREVIEWS ARE DISPLAY-ONLY. No callable composer bridge or hidden-prompt
 * submission path is exposed to preview scripts.
 *
...
/** Preview content is display-only. It has no bridge into the composer. */
export function intentScript(token: string): string {
  void token
  return ''
}

/** Retained compatibility parser: preview content is display-only, so intents
 * are always rejected and can never become composer turns. */
export function intentFromMessage(_data: unknown, _token: string, _allowlistedSource = false): string | null {
  return null
}

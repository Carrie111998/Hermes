...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's ORION_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.orionDesktop?.logsRoot?.()
      ...
      const result = await window.orionDesktop?.openDir?.(root)

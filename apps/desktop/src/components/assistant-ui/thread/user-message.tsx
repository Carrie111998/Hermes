...
import { ArrowBarUp, StopFilled } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { $gateway } from '@/store/gateway'
import { notifyThreadEditOpen, requestScrollToAnswerStart } from '@/store/thread-scroll'
...
export const UserMessage: FC<{
  ...
  // One transcript walk supplies both restore identity and answer availability.
  // Encode them as a primitive so streaming token updates rerun the cheap
  // selector without re-rendering every historical user bubble.
  const userTurnState = useAuiState(s => {
    let ordinal = 0
    ...
    return currentOrdinal === null ? null : currentOrdinal * 2 + Number(hasAssistantResponse)
  })

  const runtimeUserOrdinal = userTurnState === null ? null : Math.floor(userTurnState / 2)
  const hasAssistantResponse = userTurnState !== null && userTurnState % 2 === 1
  ...
  const showAnswerJump = hasAssistantResponse
          ...
          <div className="human-message-with-todos-wrapper flex w-full flex-col gap-0">
            ...
            >
              ...
              >
                ...
                {(showAnswerJump || showStop || showRestore) && (
                    ...
                    {showAnswerJump && (
                      <TooltipIconButton
                        ...
                        onClick={event => {
                          ...
                          event.stopPropagation()
                          triggerHaptic('selection')
                          requestScrollToAnswerStart(event.currentTarget, messageId)
                        }}
                        onPointerDown={event => event.stopPropagation()}
                        side="top"
                        tooltip={copy.jumpToAnswerStart}
                      >

...
import { orionDirectiveFormatter } from '@/components/assistant-ui/directive-text'
import {
...
import type { OrionGateway } from '@/orion'
...
export const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const replaceTriggerWithChip = useCallback(
    (item: Unstable_TriggerItem) => {
      ...
      rememberInitialDraft()
      recordUndoPoint()
      const serialized = orionDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLElement>) => {
    if (!dragHasAttachments(event.dataTransfer, ORION_PATHS_MIME)) {
      return
    }

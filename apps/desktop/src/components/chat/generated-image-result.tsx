...
async function resolveImageSrc(path: string): Promise<string> {
  ...
  if (window.orionDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.orionDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.orionDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
    ...
    return (
      <a
        ...
        onClick={event => {
          ...
          void window.orionDesktop?.openExternal(mediaExternalUrl(image))
        }}
      >

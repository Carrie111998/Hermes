import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@orion/shared'
import { atom } from 'nanostores'

import type { OrionConnection } from '@/global'
import { OrionGateway, setApiRequestConnection } from '@/orion'
...
const isOpen = (gateway: OrionGateway | null): boolean => gateway?.connectionState === 'open'

interface RegistryConfig {
  onEvent: (event: GatewayEvent) => void
  onActiveConnectionInvalidated?: (fallbackProfile: string, activationEpoch: number) => void
  onActiveConnectionChanged?: (connection: OrionConnection) => void
  /**
...
interface Secondary {
  ...
  connection: OrionConnection | null
...
interface GatewayRegistryState {
  ...
  primaryGateway: OrionGateway | null
  primaryProfile: string
  activeKey: string
  activationEpoch: number
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<OrionGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('orion.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<OrionGateway | null>(null),
...
export function setPrimaryGateway(gateway: OrionGateway | null, profile = 'default'): void {
...
export function activeGateway(): OrionGateway | null {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the orion module (null
...
function publishActiveConnection(connection: OrionConnection): void {
...
async function reconnectSecondary(entry: Secondary): Promise<void> {
  ...
  try {
    ...
    // under partial vi.mock('@/orion') harnesses the transitive graph can
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new OrionGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: OrionGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Orion gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.orionDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Orion Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<OrionGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Orion gateway is not connected" on the Sessions `+` action (#88880).

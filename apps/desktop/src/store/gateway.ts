import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@charterforge/shared'
...
const isOpen = (gateway: CharterforgeGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: CharterforgeGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<CharterforgeGateway | null>>
}
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<CharterforgeGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: CharterforgeGateway | null, profile = 'default'): void {
...
export function activeGateway(): CharterforgeGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new CharterforgeGateway()
...
export async function ensureActiveGatewayOpen(): Promise<CharterforgeGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }

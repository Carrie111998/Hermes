...
function connectionEndpoint(connection: unknown): string | null {
  if (!connection || typeof connection !== 'object') {
    return null
  }

  const value = connection as { baseUrl?: unknown; port?: unknown; wsUrl?: unknown }

  if (typeof value.baseUrl === 'string' && value.baseUrl) {
    return value.baseUrl
  }

  if (typeof value.wsUrl === 'string' && value.wsUrl) {
    try {
  ...
  return typeof value.port === 'number' ? `port:${value.port}` : null
}

// True when `profile` is served by the SHARED primary backend (global-remote
// case 3). Both pooled and shared descriptors carry `profile`: it is required
// to mint the correct WebSocket URL. The endpoint comparison distinguishes
// the shared primary from a profile-specific pooled backend.
async function sharedPrimaryRoute(profile: string): Promise<boolean> {
  ...
  try {
    const [conn, primary] = await Promise.all([desktop.getConnection(profile), desktop.getConnection()])
    const endpoint = connectionEndpoint(conn)

    return Boolean(
      conn &&
        typeof conn === 'object' &&
        (conn as { profile?: string }).profile &&
        endpoint &&
        endpoint === connectionEndpoint(primary)
    )
  } catch {
    return false
  }

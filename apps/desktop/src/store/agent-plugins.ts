...
/**
 * Feature store for backend (agent) plugins — the native Orion plugins plus
 ...
 * primitives `orion plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Orion. */
  portable?: boolean
}
 ...
 *  ORION_HOME. Always refetches (it's a cheap local disk scan on the

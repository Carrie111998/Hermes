...
describe('goal store', () => {
  ...
  it('hydrates a waiting status whose reason contains parentheses', () => {
    applyGoalStatusText('s1', '⏳ Goal (parked 60s — retry (HTTP 401), 3/20 turns): ship it')

    expect($goalsBySession.get().s1).toMatchObject({ status: 'waiting', title: 'ship it' })
  })

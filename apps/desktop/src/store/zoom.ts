...
export function setZoomPercent(percent: number): void {
  window.orionDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.orionDesktop?.zoom) {
  void window.orionDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.orionDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}

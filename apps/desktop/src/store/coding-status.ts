...
import type { OrionGitWorktree, OrionRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: OrionGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, OrionRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, OrionGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<OrionRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<OrionGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<OrionRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<OrionGitWorktree[]>>()
const $noRepoStatus = atom<OrionRepoStatus | null>(null)
const $noWorktrees = atom<OrionGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<OrionRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<OrionGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: OrionRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: OrionGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<OrionRepoStatus | null>
  seq: number
  target: string
}

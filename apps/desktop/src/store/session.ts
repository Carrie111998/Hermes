import type { ConnectionState } from '@orion/shared'
...
import type { SessionInfo, UsageStats } from '@/types/orion'
...
const COMPOSER_MODEL_KEY = 'orion.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'orion.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'orion.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'orion.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'orion.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'orion.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'orion.desktop.lastRoute'
...
function workspaceCwdKey(connection: OrionConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.orionDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.orionDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<OrionConnection | null>(null)
...
export const setConnection = (next: Updater<OrionConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useOrionConfig.

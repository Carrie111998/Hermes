...
const COMPOSER_MODEL_KEY = 'nastech.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'nastech.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'nastech.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'nastech.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'nastech.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'nastech.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'nastech.desktop.lastRoute'
...
function workspaceCwdKey(connection: NastechConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.nastechDesktop?.settings?.getDefaultProjectDir
...
// until config loads, and re-seeded on every profile switch by useNastechConfig.

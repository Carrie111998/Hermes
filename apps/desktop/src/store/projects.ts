...
import type { OrionGitBaseBranch, OrionGitBranch } from '@/global'
import { getOrionConfig, orionApi, type OrionGateway } from '@/orion'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/orion'
...
const PROJECT_SCOPE_KEY = 'orion.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Orion gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Orion profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await orionApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<OrionGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<OrionGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Orion
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getOrionConfig(context.profile))
...
// repo, returning where Orion should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<OrionGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<OrionGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.orionDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.orionDesktop?.writeClipboard?.(path)
  }
}

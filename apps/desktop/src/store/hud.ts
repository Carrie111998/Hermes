 ...
 * composer with the reply scrolling above it, so Orion can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.orionDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.orionDesktop?.hud
...
export const reportHudSession = (sessionId: null | string): void => window.orionDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.orionDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)

...
const { getProfiles } = await import('@/orion')
...
const remoteConn = (over: Partial<OrionConnection> = {}): OrionConnection =>
  ({ baseUrl: 'https://orion-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as OrionConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { orionDesktop: { getConnection } })

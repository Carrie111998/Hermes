...
const REMOTE_SESSION_ENDED_ERROR = 'The remote session ended.'
...
export function redactRemoteText(value: string): string {
  const bearerRedacted = value.replace(
    ...
    (match, prefix: string, token: string) =>
      token === '[REDACTED]' || token === '***' ? match : `${prefix}***`
  )

  const apiKeysRedacted = bearerRedacted.replace(/\bsk-(?:[A-Za-z0-9_]+-)*[A-Za-z0-9_]{8,}\b/g, 'sk-***')

  return apiKeysRedacted.replace(
    /(?<![A-Za-z0-9+/_=-])[A-Za-z0-9+/_-]{24,}={0,2}(?![A-Za-z0-9+/_=-])/g,
    '***'
  )
}

function redactRemoteValue(value: unknown): unknown {
  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, nested]) => [key, redactRemoteValue(nested)]))
  }

  return value
}

function redactedSessionEvent(event: RemoteSessionEvent): RemoteSessionEvent {
  return redactRemoteValue(event) as RemoteSessionEvent
}
...
function parsedSession(value: unknown): RemoteSession | null {
  ...
  return {
    id: row.id,
    title: typeof row.title === 'string' ? redactRemoteText(row.title) : null,
...
function applySessionEvent(sessionId: string, event: RemoteSessionEvent, generation: number): void {
  ...
  const safeEvent = redactedSessionEvent(event)
  const events = [...existing.events, safeEvent].slice(-MAX_SESSION_EVENTS)

  const nextSession: RemoteSession = {
    ...existing,
    status:
      safeEvent.event === 'session.status' && typeof safeEvent.status === 'string' ? safeEvent.status : existing.status,
    updated_at: eventUpdatedAt(safeEvent, existing.updated_at),
    events
  }
  ...
  $remoteAttach.set({ ...state, sessions, status: 'connected', error: undefined, reconnecting: undefined })
}
...
function isCurrentStream(sessionId: string, generation: number): boolean {
  return generation === streamGeneration && $remoteAttach.get().attachedSessionId === sessionId
}
...
async function scheduleReconnect(
  ...
  if (!isCurrentStream(sessionId, generation) || controller.signal.aborted) {
    return
  }

  if (retriesUsed >= RECONNECT_DELAYS_MS.length) {
  ...
  const completed = await waitForReconnect(RECONNECT_DELAYS_MS[retriesUsed], controller.signal)

  if (!completed || !isCurrentStream(sessionId, generation)) {
    return
  }

  await openEventStream(sessionId, generation, retriesUsed + 1)
}

async function openEventStream(sessionId: string, generation: number, retriesUsed: number): Promise<void> {
  if (!isCurrentStream(sessionId, generation)) {
    return
  }

  const connection = usableConnection()

  if (!connection || !isCurrentStream(sessionId, generation)) {
    return
  }
  ...
  try {
    const response = await checkedFetch(
      `${remoteBaseUrl(connection.host, connection.port)}/api/remote/sessions/${encodeURIComponent(sessionId)}/events`,
      {
        method: 'GET',
    ...
    void consumeEventStream(response.body, sessionId, generation).then(
      () =>
        scheduleReconnect(
          sessionId,
          generation,
          retriesUsed,
          controller,
          new Error('Remote session stream disconnected')
        ),
      error => scheduleReconnect(sessionId, generation, retriesUsed, controller, error)
    )
  } catch (error) {
    if (!isCurrentStream(sessionId, generation) || controller.signal.aborted) {
      return
    ...
    if (error instanceof RemoteRequestError && error.status === 401) {
      ...
      await scheduleReconnect(sessionId, generation, retriesUsed, controller, error)
    } else {
      publishError(error)
    }
  }
...
export async function refreshSessions(): Promise<void> {
  ...
  try {
    ...
    const attachedMissing = Boolean(
      current.attachedSessionId &&
        current.sessions.some(session => session.id === current.attachedSessionId) &&
        !incoming.some(session => session.id === current.attachedSessionId)
    )

    let sessions = mergeSessions(current.sessions, incoming, current.attachedSessionId)

    if (attachedMissing) {
      const attachedId = current.attachedSessionId

      closeStream()
      sessions = sessions.map(session => (session.id === attachedId ? { ...session, status: 'ended' } : session))
    }
    ...
    $remoteAttach.set({
      ...
      sessions,
      status: 'connected',
      error: attachedMissing ? REMOTE_SESSION_ENDED_ERROR : undefined,
      attachedSessionId: attachedMissing ? undefined : current.attachedSessionId,
      reconnecting: undefined
    })
  } catch (error) {
    const current = $remoteAttach.get()
...
export async function attachToSession(id: string): Promise<void> {
  ...
  await openEventStream(id, generation, 0)
}

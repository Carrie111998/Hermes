...
// When switching to Nastech auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your Nastech subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `nastech model` does (for Nastech it honors the user's free/paid tier, so a

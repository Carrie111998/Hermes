...
type PreviewArtifactBackendIdentity = Pick<
  PreviewArtifactPublicationIdentity,
  'baseUrl' | 'mode' | 'remoteIdentity'
>

const canonicalBackendIdentity = (mode: 'local' | 'remote', baseUrl: string, remoteIdentity: string): string => {
  if (mode === 'local') {
    // The loopback port is ephemeral across Desktop launches.
    return 'local'
  }
  ...
  try {
    ...
    if (remoteIdentity.trim()) {
      // SSH connections expose their durable ownership id while host/port point
      // at a new local tunnel each launch. Keep the gateway path as scope.
      return `remote:${remoteIdentity.trim()}:${pathname}`
    }

    // URL.host excludes userinfo; search and fragment are intentionally omitted.
    return `${parsed.protocol}//${parsed.host}${pathname}`
  } catch {
    // Connections are validated before reaching the renderer. Stay secret-safe
    // for malformed legacy state instead of hashing its raw value.
    return remoteIdentity.trim() ? `remote:${remoteIdentity.trim()}` : 'remote'
  }
}
...
export const previewArtifactBackendIdentity = ({
  baseUrl = '',
  mode,
  remoteIdentity = ''
}: PreviewArtifactBackendIdentity): string =>
  canonicalBackendIdentity(mode, baseUrl.trim(), remoteIdentity)
...
export function previewArtifactOwnerId(
  ...
  // compression tip or an id claimed by another tagged profile waits for its
  // own lineage row instead of minting a transient owner.
  return !hasScopedCandidate && unscopedMatch ? sessionPinId(unscopedMatch) : null
}
...
export const previewArtifactPublicationKey = ({
  ...
  `p:${stableScopeHash(
    JSON.stringify([
      mode,
      previewArtifactBackendIdentity({ baseUrl, mode, remoteIdentity }),

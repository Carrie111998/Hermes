...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('nastech-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('nastech-desktop-onboarded-v1')).toBe('1')
  })
  ...
  it('enters setup when the selected OpenRouter credential is genuinely empty', async () => {
    ...
    expect($desktopOnboarding.get().reason).toContain('No usable credentials found for openrouter.')
    expect(window.localStorage.getItem('nastech-desktop-onboarded-v1')).toBeNull()
  })
...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    installApiMock(async ({ body, path }: { body?: unknown; path: string }) => {
      ...
      if (path === '/api/providers/oauth/nastech/submit') {
      ...
      if (path.startsWith('/api/model/recommended-default?')) {
        return { provider: 'nastech', model, free_tier: false }
      }

      if (path === '/api/model/set') {
        return { ok: true, provider: 'nastech', model, gateway_tools: [] }
      }
    ...
    const requestGateway: OnboardingContext['requestGateway'] = async (method, params) => {
      ...
      if (method === 'setup.runtime_check') {
        expect(params).toEqual({ provider: 'nastech' })
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('nastech', 'Nastech Portal'),
          start: {
        ...
        reason:
          'No access token found for Nastech Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('Nastech Portal')

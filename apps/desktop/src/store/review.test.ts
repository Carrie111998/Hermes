...
import type { OrionReviewFile, OrionReviewShipInfo } from '@/global'
...
// reaches the gateway); everything else routes through window.orionDesktop.git,
...
function file(path: string, over: Partial<OrionReviewFile> = {}): OrionReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as OrionReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.orionDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as OrionReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.orionDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: OrionReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as OrionReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { orionDesktop?: unknown }).orionDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as OrionReviewShipInfo)

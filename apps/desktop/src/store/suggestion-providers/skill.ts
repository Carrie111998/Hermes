 ...
 *  Working in `~/www/orion-agent`, the draft says "orion-agent" constantly
 *  — a "Use skill: orion-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`orion-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/orion-agent-dev`, `plugin:skill`). */

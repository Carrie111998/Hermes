...
describe('syncPreviewArtifacts', () => {
  ...
  it('canonicalizes remote ownership without persisting URL credentials or transient parameters', () => {
    const identity = {
      mode: 'remote' as const,
      ownerId: 'lineage-root',
      profile: 'work',
      publicationId: 'tool:call-1'
    }

    const credentialed = previewArtifactPublicationKey({
      ...identity,
      baseUrl: 'https://user:secret@EXAMPLE.test:443/hermes/?token=temporary#fragment'
    })

    const canonical = previewArtifactPublicationKey({ ...identity, baseUrl: 'https://example.test/hermes' })
    const otherGateway = previewArtifactPublicationKey({ ...identity, baseUrl: 'https://example.test/other' })

    expect(credentialed).toBe(canonical)
    expect(otherGateway).not.toBe(canonical)
  })

  it('uses stable SSH ownership instead of an ephemeral forwarded port', () => {
    const identity = {
      mode: 'remote' as const,
      ownerId: 'lineage-root',
      profile: 'work',
      publicationId: 'tool:call-1',
      remoteIdentity: '0123456789abcdef'
    }

    const firstTunnel = previewArtifactPublicationKey({ ...identity, baseUrl: 'http://127.0.0.1:41000/hermes/' })
    const nextTunnel = previewArtifactPublicationKey({ ...identity, baseUrl: 'http://127.0.0.1:42000/hermes' })
    const otherGateway = previewArtifactPublicationKey({ ...identity, baseUrl: 'http://127.0.0.1:42000/other' })

    expect(firstTunnel).toBe(nextTunnel)
    expect(otherGateway).not.toBe(firstTunnel)
  })

  it('ignores the ephemeral port of a restarted local backend', () => {
    const identity = {
      mode: 'local' as const,
      ownerId: 'lineage-root',
      profile: 'default',
    ...
    const firstLaunch = previewArtifactPublicationKey({ ...identity, baseUrl: 'http://127.0.0.1:41000' })
    const nextLaunch = previewArtifactPublicationKey({ ...identity, baseUrl: 'http://127.0.0.1:42000' })

    expect(firstLaunch).toBe(nextLaunch)
  })
  ...
  it('resolves the lineage root within the owning profile', () => {
    ...
    expect(previewArtifactOwnerId('same-tip', sessions, 'other')).toBeNull()

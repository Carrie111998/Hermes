...
import { getCharterforgeConfigRecord, saveCharterforgeConfig } from '@/hermes'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getCharterforgeConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveCharterforgeConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {

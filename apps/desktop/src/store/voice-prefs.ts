...
import { getOrionConfigRecord, saveOrionConfig } from '@/orion'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getOrionConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveOrionConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {

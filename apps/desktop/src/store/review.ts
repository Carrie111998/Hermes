...
//
// Scope is always "uncommitted" — Orion' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'orion.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'orion.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'orion.desktop.reviewTreeMode'
const SELECTED_KEY = 'orion.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<OrionReviewFile[]>([])
...
export const $reviewShipInfo = atom<OrionReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['orionDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: OrionReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly OrionReviewFile[], path: string): OrionReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.orionDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.orionDesktop?.openExternal?.(url)
    }

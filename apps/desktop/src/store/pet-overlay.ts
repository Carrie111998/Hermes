 ...
 * app's bounds and stays visible while Orion is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'orion.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'orion.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.orionDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.orionDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.orionDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.orionDesktop?.petOverlay?.close()
}

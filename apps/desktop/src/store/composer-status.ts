...
/** Compatibility export for the status-stack tests and callers. */
export const isSessionGoneForBackgroundPolling = isSessionNotFoundError
...
export async function refreshBackgroundProcesses(sid: string): Promise<void> {
  ...
  try {
    ...
    if (isSessionGoneForBackgroundPolling(error)) {
      // Stop requests to the dead runtime before asking the existing recovery
      // path to rebind the durable session. A successful rebind clears the RPC
      // guard; until then, no sibling poller can restart the request storm.
      markSessionRpcBlocked(sid)
      // Latching stops the storm and carries the gateway's terminal verdict to
      // the existing tile/primary-chat recovery path.
      markRuntimeGone(sid)
...
export function resetSessionBackground(sid: string) {
  ...
  for (const item of list) {
    ...
    if (item.state === 'running') {
      if (gateway && !isSessionRpcBlocked(sid)) {
        const ambientRequest = <T>(method: string, params?: Record<string, unknown>) =>
          gateway.request<T>(method, params ?? {})

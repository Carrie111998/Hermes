 ...
 * `@orion/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-orion-glass]`
 * block in styles.css).
 */
...
} from '@orion/shared/translucency'
 ...
 * sets `orionDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.orionDesktop?.glassSupported === 'boolean'
    ? window.orionDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.orionDesktop?.translucencySupported === 'boolean'
    ? window.orionDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-orion-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-orion-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-orion-glass-on', glassLive)
  root.toggleAttribute('data-orion-glass', glassOn)
  root.toggleAttribute('data-orion-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-orion-glass-scope', scope)
  } else {
    root.removeAttribute('data-orion-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.orionDesktop?.setTranslucency?.(state)
  })

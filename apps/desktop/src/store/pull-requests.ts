...
import { scanSessionPullRequests, type SessionInfo } from '@/orion'
...
export const $pullRequestsByBranch = atom<Record<string, OrionBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'orion.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('orion.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: OrionBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }

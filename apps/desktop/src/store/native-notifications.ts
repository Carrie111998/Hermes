...
import { type OrionOpenTarget, resolveOrionOpenPath } from '@/lib/orion-open-target'
...
const STORAGE_KEY = 'orion:native-notifications'
...
// "Backgrounded" = the user isn't on Orion. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `orion://index-network/intent/1`.
   */
  activate?: string
...
export function dispatchNativeNotification(input: NativeNotificationInput): boolean {
  ...
  void window.orionDesktop?.notify({
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `orion://index-network/intent/1`). */
  activate?: OrionOpenTarget
 ...
 *  user is away from Orion — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveOrionOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveOrionOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))

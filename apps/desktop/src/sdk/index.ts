/**
 * @orion/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__ORION_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, type OrionGateway } from '@/orion'
import {
  $gateway,
...
import type { UsageStats } from '@/types/orion'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Orion profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.orionDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.orionDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and OrionGateway.connect() has
...
export const host = {
  ...
  connections: async () => {
    const bridge = window.orionDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Orion Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.orionDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Orion Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Orion Desktop to open another Bot chat.' })
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Orion Desktop connection routing unavailable')
    }
...
export type { OrionGateway } from '@/orion'
...
export type { OrionOpenTarget } from '@/lib/orion-open-target'
...
/** Orion' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/orion'

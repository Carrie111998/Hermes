...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'orion:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.orionDesktop?.saveImageFromUrl) {
        if (await window.orionDesktop.saveImageFromUrl(src)) {

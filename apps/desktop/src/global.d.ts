import type { GatewayWsUrlResult } from '@orion/shared'
import type { TranslucencyState } from '@orion/shared/translucency'
...
declare global {
  interface Window {
    orionDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<OrionConnection>
      ...
      // Resume this session in the user's own terminal emulator (`orion --tui
      ...
      // Orion Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ORION_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: OrionApiRequest) => Promise<T>
      notify: (payload: OrionNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<OrionReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<OrionReadFileTextResult>
      selectPaths: (options?: OrionSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<OrionPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<OrionPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<OrionPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: OrionActiveWork) => void
      setTitleBarTheme?: (payload: OrionTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<OrionReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ORION_HOME>/desktop-plugins),
      ...
      /** LOCAL `<ORION_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<ORION_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<OrionGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<OrionGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<OrionGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<OrionRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: OrionReviewScope, baseRef?: null | string) => Promise<OrionReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: OrionReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<OrionReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<OrionRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<OrionPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: OrionTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<OrionTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: OrionWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: OrionPreviewFileChanged) => void) => () => void
...
export interface OrionTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface OrionTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `orion update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  orionRoot?: string
...
export interface OrionTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface OrionActiveWork {
  count: number
  titles: string[]
}

export interface OrionWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Orion Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Orion Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Orion Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the orion gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Orion Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface OrionApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface OrionPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface OrionReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface OrionPreviewWatch {
  id: string
  path: string
}
...
export interface OrionGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface OrionGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface OrionGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface OrionRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface OrionRepoStatus {
  branch: null | string
...
// turn began.
export type OrionReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface OrionReviewFile {
  path: string
  added: number
  removed: number
...
export interface OrionReviewList {
  files: OrionReviewFile[]
...
export interface OrionReviewPr {
  url: string
  state: string
  number: number
}
...
export interface OrionBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface OrionRepoPullRequests {
  ghReady: boolean
  prs: OrionBranchPullRequest[]
}
...
export interface OrionPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface OrionReviewShipInfo {
  ghReady: boolean
  pr: OrionReviewPr | null
}

export interface OrionReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface OrionReadDirResult {
  entries: OrionReadDirEntry[]
  error?: string
}

export interface OrionPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface OrionSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean

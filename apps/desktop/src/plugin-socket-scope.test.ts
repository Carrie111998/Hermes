...
// (#73044). Exercises the REAL orion + store/gateway + store/profile chain:
...
vi.mock('@/orion', async importOriginal => {
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/orion')
...
const conn = (over: Partial<OrionConnection> = {}): OrionConnection =>
  ({
    authMode: 'oauth',
...
beforeEach(() => {
  ...
  Object.defineProperty(window, 'orionDesktop', {
    configurable: true,
    value: {
      getConnection,
      getConnectionFor,
...
afterEach(() => {
  ...
  Reflect.deleteProperty(window, 'orionDesktop')
  vi.restoreAllMocks()
})

import { JsonRpcGatewayClient } from '@orion/shared'

import type { OrionApiRequest } from '@/global'
...
// "Timed out connecting to Orion backend" that hangs the UI (#48504).
//
...
export class OrionGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Orion gateway connection closed',
      connectErrorMessage: 'Could not connect to Orion gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Orion gateway is not connected',
...
// process to route them to that host (orion:api's registry branch). Null
...
export function orionApi<T>(request: OrionApiRequest): Promise<T> {
  return window.orionDesktop.api<T>({ ...connectionScoped(), ...request })
}

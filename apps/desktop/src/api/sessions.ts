...
import { capabilityScoped, orionApi, type ProfileScope } from './client'
...
export async function listSessions(
  ...
  const result = await orionApi<PaginatedSessions>({
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await orionApi<SidebarSessionsResponse>({
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return orionApi<SessionSearchResponse>({
...
export function getSessionMessages(
  ...
  return orionApi<SessionMessagesResponse>({
    ...sessionScope,
...
export function renameSession(
  ...
  return orionApi<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',

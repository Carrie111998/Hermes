...
import { getApiRequestConnection, getApiRequestProfile, orionApi, profileScoped } from './client'
...
async function activeConnection(): Promise<OrionConnection> {
  const getConnectionFor = window.orionDesktop.getConnectionFor
  ...
  return window.orionDesktop.getConnection(getApiRequestProfile())
}

/** Options for a plugin REST call — mirrors the app's own `orionDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see OrionApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.orionDesktop?.api) {
    throw new Error('Orion desktop bridge unavailable')
  }

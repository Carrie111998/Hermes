...
import { capabilityScoped, orionApi, type ProfileScope, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getStatus(): Promise<StatusResponse> {
  return orionApi<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return orionApi<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getOrionConfig(profile?: string): Promise<OrionConfig> {
  return orionApi<OrionConfig>({
...
export function getOrionConfigRecord(profile?: ProfileScope): Promise<OrionConfigRecord> {
  return window.orionDesktop.api<OrionConfigRecord>({
    ...capabilityScoped(profile),
    path: '/api/config'
  })
}

export function getOrionConfigDefaults(): Promise<OrionConfigRecord> {
  return orionApi<OrionConfigRecord>({
    ...profileScoped(),
...
export function getOrionConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return orionApi<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function saveOrionConfig(config: OrionConfigRecord, profile?: null | string): Promise<{ ok: boolean }> {
...
export function getEnvVars(profile?: null | string): Promise<Record<string, EnvVarInfo>> {
  return orionApi<Record<string, EnvVarInfo>>({
    ...profileScoped(profile),
    path: '/api/env'
  })
}
...
export function revealEnvVar(key: string, profile?: ProfileScope): Promise<{ key: string; value: string }> {
  return window.orionDesktop.api<{ key: string; value: string }>({
    ...capabilityScoped(profile),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function validateProviderCredential(
  ...
  return orionApi<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return orionApi<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return orionApi<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return orionApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return orionApi<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return orionApi<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string, profile?: ProfileScope): Promise<OAuthStartResponse> {
  return window.orionDesktop.api<OAuthStartResponse>({
    ...capabilityScoped(profile),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return orionApi<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(
  ...
  return window.orionDesktop.api<OAuthPollResponse>({
    ...capabilityScoped(profile),

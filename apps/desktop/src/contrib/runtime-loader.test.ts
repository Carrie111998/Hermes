...
import type { OrionReadDirResult } from '@/global'
...
// getStatus would supply the connected backend's orion_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ orion_home: '/remote/box/.orion' }))
...
const readDir = vi.fn<(path: string) => Promise<OrionReadDirResult>>()
...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend orion_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.orion/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.orion/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.orion/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.orion/plugins')
    // The remote backend's orion_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.orion/desktop-plugins')
  })
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.orion/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.orion/plugins/my-feature' }] }
      }

      if (dir === '/local/.orion/plugins/my-feature') {
        return {
          entries: [{ isDirectory: false, name: 'plugin.yaml', path: '/local/.orion/plugins/my-feature/plugin.yaml' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.orion/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.orion/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.orion/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.orion/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.orion/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.orion/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.orionDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.orion/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.orion/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.orion/plugins/uni' }] }
      }

      if (dir === '/local/.orion/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.orion/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.orion/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.orion/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
    ...
    try {
      ...
      // ~/.orion/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend orion_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.orion/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.orion/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.orion/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.orion/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.orion/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.orion/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.orionDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.orion/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.orion/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.orion/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'orion-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "orion-bots", name: "Bot Mode", register() {} }',
        'orion-bots',
        { file: '/local/.orion/desktop-plugins/orion-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['orion-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['orion-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.orion/desktop-plugins/orion-bots/plugin.js'
      })

 ...
 *    a `OrionPlugin` registers automatically (vite glob — drop a folder in).
 *    `orion-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `orion-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<orion home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<orion home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: OrionPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default OrionPlugin export — skipped`)

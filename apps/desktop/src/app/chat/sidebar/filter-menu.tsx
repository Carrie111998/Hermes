...
export function SidebarFilterMenu({ className }: { className?: string }) {
  ...
  const groupings: Option<SidebarGrouping>[] = [
    { icon: 'clock', id: 'date', label: fm.updated },
    { icon: 'root-folder', id: 'project', label: fm.project },
    { icon: 'pulse', id: 'status', label: fm.status },
    { icon: 'account', id: 'profile', label: fm.profile }
  ]

  const orderingsAll: Option<SidebarOrdering>[] = [
    { icon: 'clock', id: 'updated', label: fm.updated },
    { icon: 'add', id: 'created', label: fm.created },
    { icon: 'pulse', id: 'status', label: fm.status },
    { icon: 'symbol-numeric', id: 'tokens', label: fm.tokens },
    { icon: 'credit-card', id: 'cost', label: fm.cost },
    { icon: 'list-ordered', id: 'manual', label: fm.manual }
  ]

  const rowMetaDefs: Option<SidebarRowMeta>[] = [
    { icon: 'clock', id: 'updated', label: fm.updated },
    { icon: 'comment', id: 'preview', label: fm.preview },
    { icon: 'symbol-numeric', id: 'tokens', label: fm.tokens },
    { icon: 'credit-card', id: 'cost', label: fm.cost },
    { icon: 'git-pull-request', id: 'pr', label: fm.pr },
    { icon: 'account', id: 'profile', label: fm.profile }
  ]

  const prFilters: Option<PullRequestBucket>[] = [
    { icon: 'git-pull-request', id: 'open', label: fm.open },
    { icon: 'git-pull-request-draft', id: 'draft', label: fm.draft },
    { icon: 'git-merge', id: 'merged', label: fm.merged },
    { icon: 'git-pull-request-closed', id: 'closed', label: fm.closed },
    { icon: 'circle-slash', id: 'none', label: fm.noPr }
  ]

  const statusFilters: Option<SessionStatusBucket>[] = [
    { dot: sessionDotClassName('needs-input'), id: 'needs-input', label: fm.needsInput },
    { dot: sessionDotClassName('working'), id: 'working', label: fm.working },
    { dot: sessionDotClassName('unread'), id: 'unread', label: fm.unread },
    { dot: sessionDotClassName('draft'), id: 'draft', label: fm.draft },
    { dot: cn(sessionDotClassName('idle'), 'bg-(--ui-text-quaternary)'), id: 'idle', label: fm.idle }
  ]
  ...
  const orderings = orderingsAll.filter(option => {
    if (option.id === 'manual') {
  ...
  const rowMetaOptions = rowMetaDefs.filter(option => {
    if (option.id === 'cost') {

...
const branchActionLabel = (branch: CharterforgeGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<CharterforgeGitBranch[]>([])
  ...
  const convert = async (branch: CharterforgeGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }

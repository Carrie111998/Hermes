...
beforeEach(() => {
  resetStores()
  mocks.focusOpenSession.mockReset().mockReturnValue(false)
  mocks.navigate.mockReset()
  mocks.revealTreePane.mockReset()
...
describe('SessionForkOriginNotice', () => {
  it('renders only under the anchored branchable message and opens the parent live tip', () => {
    ...
    expect(mocks.focusOpenSession).toHaveBeenCalledWith('parent-tip-2')
    expect(mocks.navigate).toHaveBeenCalledWith('/parent-tip-2')
    expect(mocks.revealTreePane).toHaveBeenCalledWith('workspace')
    expect($forkOriginNotices.get()['child-1']).toBeTruthy()
  })

  it('fronts an already-open parent surface without replacing the main route', () => {
    mocks.focusOpenSession.mockReturnValue(true)
    $forkOriginNotices.set({
      'child-1': { branchMessageOrdinal: 1, createdAt: 1, parentSessionId: 'parent-root-1' }
    })
    $messages.set([
    ...
    renderBanner('en')
    fireEvent.click(screen.getByRole('button', { name: 'Open source conversation: Parent session' }))

    expect(mocks.focusOpenSession).toHaveBeenCalledWith('parent-tip-2')
    expect(mocks.navigate).not.toHaveBeenCalled()
    expect(mocks.revealTreePane).not.toHaveBeenCalled()
  })

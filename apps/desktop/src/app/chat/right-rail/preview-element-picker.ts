/**
 * Guest-page element picker, matching ZCode's `__zcodeWebElementPicker`.
 *
 * Injected via `webview.executeJavaScript`. Hover highlights a node; click
 * returns a structured snapshot that PreviewPane inserts into the composer.
 * Cancel with Escape or by calling `window.__hermesWebElementPicker.cancel()`.
 *
 * Password field values are never captured. Scripts/styles are stripped from
 * the HTML excerpt.
 */
...
/** Injected into the guest. Must stay self-contained — no renderer imports. */

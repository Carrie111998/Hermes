        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.orionDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Orion to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.orionDesktop?.watchPreviewFile ||
      !window.orionDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.orionDesktop.onPreviewFileChanged(payload => {
    ...
    void window.orionDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.orionDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.orionDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:orion-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.orionDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.orionDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.orionDesktop?.contextMenuCopyImage?.(),
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        {isWebPreview && !isRemoteHtml && (
          <PreviewBrowserBar
            ...
            onOpenExternal={() => void window.orionDesktop?.openExternal(currentUrl)}

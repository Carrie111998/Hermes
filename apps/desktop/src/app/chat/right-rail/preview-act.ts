...
const preamble = () => `  var w = window;
  ...
  w.__orionActHolder = w.__orionActHolder || {};
  w.__orionAct = ${actEngineSource()};
  w.__orionWatch_fn = (${watchInPage.toString()});
  w.__orionWatchTag = ${WATCH_TAG};
  var holder = w.__orionActHolder;
  var act = function (a) { return w.__orionAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__orionWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__orionHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'ORION-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}

import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@charterforge/shared'
import { useEffect, useRef } from 'react'

import type { CharterforgeConnection } from '@/global'
import { CharterforgeGateway } from '@/hermes'
...
interface GatewayBootOptions {
  ...
  onGatewayReady: (gateway: CharterforgeGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: CharterforgeConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Charterforge...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Charterforge…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Charterforge gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshCharterforgeConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshCharterforgeConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startCharterforge re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new CharterforgeGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshCharterforgeConfig(),
          ...
          // retry it), not brick boot behind the "Charterforge couldn't start"

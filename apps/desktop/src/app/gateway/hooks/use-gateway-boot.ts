import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@orion/shared'
import { useEffect, useRef } from 'react'

import type { OrionConnection } from '@/global'
import { OrionGateway } from '@/orion'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.orionDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: OrionGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: OrionConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Orion...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Orion…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Orion gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshOrionConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshOrionConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startOrion re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new OrionGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshOrionConfig(),
          ...
          // retry it), not brick boot behind the "Orion couldn't start"

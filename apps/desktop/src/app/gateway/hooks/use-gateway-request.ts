...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as OrionGateway | null
  const gatewayRef = useRef<OrionGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.orionDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<OrionGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as OrionGateway | null
      }),
    []
  )

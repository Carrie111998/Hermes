...
// bug that drives the REAL useGatewayBoot hook + REAL OrionGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshOrionConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { orionDesktop?: unknown }).orionDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForOrion) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startOrion()'s remote branch awaits waitForOrion() for 45s before it
    ...
    ;(window as { orionDesktop?: unknown }).orionDesktop = desktop
    ...
    // After ~45s waitForOrion gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Orion backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Orion couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Orion')
    expect($currentCwd.get()).toBe('C:\\Orion')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startOrion() rejects with a

...
// bug that drives the REAL useGatewayBoot hook + REAL CharterforgeGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshCharterforgeConfig: async () => undefined,
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForCharterforge) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startCharterforge()'s remote branch awaits waitForCharterforge() for 45s before it
    ...
    // After ~45s waitForCharterforge gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Charterforge backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Charterforge couldn't start" overlay — the socket is open

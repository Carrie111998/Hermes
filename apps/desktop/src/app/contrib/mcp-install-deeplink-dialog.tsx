...
import { getOrionConfigRecord, saveMcpServers } from '@/orion'
...
import { setOrionConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `orion://mcp/install` deep links. The payload is
 ...
 * confirming lets Orion spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getOrionConfigRecord())
      ...
      setOrionConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))

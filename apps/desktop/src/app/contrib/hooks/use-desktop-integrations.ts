...
import { pathFromOrionDeepLink, resolveOrionOpenPath } from '@/lib/orion-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.orionDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.orionDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.orionDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.orionDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // orion://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.orionDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future orionDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveOrionOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.orionDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (orion://index-network/intent/1, orion://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromOrionDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.orionDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.orionDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.orionDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.orionDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())

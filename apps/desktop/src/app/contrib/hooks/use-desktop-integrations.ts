...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    const unsubscribe = window.hermesDesktop?.onNotificationAction?.(({ actionId, approvalId, sessionId }) => {
      void respondToApprovalAction(sessionId ?? null, actionId, approvalId)
    })

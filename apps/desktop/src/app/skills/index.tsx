...
import type { SkillInfo, ToolsetInfo } from '@/types/orion'
...
export function SkillsView({
  ...
  const registryBridge = window.orionDesktop?.connections
  const rosterBridge = window.orionDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Orion (default)' : p.name,
      value: p.name
    }))

...
function ResumeHarness({
  ...
  const transcriptViewGateRef = useRef(new Map<string, symbol>())

  const holdSessionTranscriptView = (runtimeId: string): (() => void) => {
    const token = Symbol(runtimeId)
    transcriptViewGateRef.current.set(runtimeId, token)

    return () => {
      if (transcriptViewGateRef.current.get(runtimeId) === token) {
        transcriptViewGateRef.current.delete(runtimeId)
      }
    }
  }

  const projectViewState = (sessionId: string, state: ClientSessionState) =>
    suppressTranscriptForView(state, transcriptViewGateRef.current.has(sessionId))
  ...
  const actions = useSessionActions({
    ...
    syncSessionStateToView: (sessionId, state) => onViewSync?.(sessionId, projectViewState(sessionId, state)),
    updateSessionState: (sessionId, updater, storedSessionId) => {
      ...
      // Production useSessionStateCache publishes every changed active state
      // to the primary view as part of updateSessionState itself.
      onViewSync?.(sessionId, projectViewState(sessionId, next))
...
describe('resumeSession warm-cache mapping integrity', () => {
  ...
  it.each(['rewrite', 'stored-id rotation'] as const)(
    'does not apply or mint an older REST proof after a concurrent %s',
    async mutation => {
      const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
        current: new Map([['stored-A', 'rt-A']])
      }
      ...
      state.messages = [
        ...
        { id: 'cached-assistant', parts: [{ text: 'cached answer', type: 'text' }], role: 'assistant' }
      ]
      ...
      const nextEpoch =
        ((current as ClientSessionState & { transcriptAuthorityEpoch?: number }).transcriptAuthorityEpoch ?? 0) + 1

      sessionStateByRuntimeIdRef.current.set('rt-A', {
        ...current,
        ...(mutation === 'rewrite'
          ? {
              messages: [{ id: 'rewritten-user', parts: [{ text: 'rewritten prompt', type: 'text' }], role: 'user' }]
            }
          : { storedSessionId: 'stored-A-next' }),
        transcriptAuthorityEpoch: nextEpoch,
      ...
      expect(finalState.transcriptProvenance).toBeUndefined()

      if (mutation === 'rewrite') {
        expect(JSON.stringify(finalState.messages)).toContain('rewritten prompt')
      } else {
        expect(finalState.storedSessionId).toBe('stored-A-next')
      }
    }
  )

  it.each(['Method not found', 'request timed out: session.activate'])(
    'hydrates an unproven warm transcript after activation fails with %s',
    async activationError => {
      ...
      expect(getLatestSessionMessages).toHaveBeenCalledWith('stored-A', undefined)
      expect(JSON.stringify(viewSnapshots.at(-1)?.messages)).toContain('persisted prompt')
      expect(viewSnapshots.at(-1)?.transcriptProvenance).toEqual(
        createPersistedDisplayTranscriptProvenance({
          lineageRootId: 'root-A',
          scope: undefined,
          storedSessionId: 'stored-A'
  ...
  it('releases an unproven cached fallback after activation and REST both fail', async () => {
    const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
      current: new Map([['stored-A', 'rt-A']])
    }
    ...
    const state = clientState('stored-A')
    state.messages = [
      { id: 'cached-user', parts: [{ text: 'cached fallback prompt', type: 'text' }], role: 'user' },
      { id: 'cached-assistant', parts: [{ text: 'cached fallback answer', type: 'text' }], role: 'assistant' }
    ]
    ...
    expect(getLatestSessionMessages).toHaveBeenCalledWith('stored-A', undefined)
    expect(JSON.stringify(viewSnapshots.at(-1)?.messages)).toContain('cached fallback prompt')
    expect(viewSnapshots.at(-1)?.transcriptProvenance).toBeUndefined()
  })

  it('publishes a matching proven warm transcript immediately', async () => {
    const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
      current: new Map([['stored-A', 'rt-A']])
    }
    ...
    const state = clientState('stored-A')
    state.messages = [
      { id: 'cached-user', parts: [{ text: 'proven prompt', type: 'text' }], role: 'user' },
      { id: 'cached-assistant', parts: [{ text: 'proven answer', type: 'text' }], role: 'assistant' }
    ]
    ...
    await waitFor(() => expect(viewSnapshots.length).toBeGreaterThanOrEqual(2))
    expect(JSON.stringify(viewSnapshots[0].messages)).toContain('proven prompt')
    expect(viewSnapshots[0].transcriptProvenance).toEqual(state.transcriptProvenance)

    persisted.resolve({
      messages: [
        { content: 'proven prompt', role: 'user', timestamp: 1 },
        { content: 'proven answer', role: 'assistant', timestamp: 2 }
      ],
      session_id: 'stored-A'
    })
    await resumePromise

    expect(viewSnapshots.at(-1)?.transcriptProvenance).toEqual(state.transcriptProvenance)
  })

  it('keeps a proven persisted base when activation returns runtime messages and REST fails', async () => {
    const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
      current: new Map([['stored-A', 'rt-A']])
    }
    ...
    const requestGateway = vi.fn(async (method: string) => {
      if (method === 'session.activate') {
        return {
          ...
          message_count: 2,
          messages: [
            { content: 'runtime-only prompt', role: 'user', timestamp: 3 },
            { content: 'runtime-only answer', role: 'assistant', timestamp: 4 }
          ],
    ...
    expect(JSON.stringify(viewSnapshots.at(-1)?.messages)).toContain('proven prompt')
    expect(JSON.stringify(viewSnapshots.at(-1)?.messages)).not.toContain('runtime-only prompt')
    expect(viewSnapshots.at(-1)?.transcriptProvenance).toEqual(state.transcriptProvenance)
  })

  it.each([
    ['missing', null],
    ['wrong connection', { connectionId: 'remote-B' }],
    ['wrong profile', { profile: 'work' }],
    ['wrong stored session', { storedSessionId: 'stored-B' }],
    ['wrong lineage', { lineageRootId: 'root-B' }],
    ['malformed', { source: 'runtime' }]
  ] as const)('rejects a %s warm transcript proof before authority settles', async (_label, override) => {
    const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
      current: new Map([['stored-A', 'rt-A']])
    }
    ...
    const state = clientState('stored-A')
    state.messages = [
      { id: 'cached-user', parts: [{ text: 'unproven prompt', type: 'text' }], role: 'user' },
      { id: 'cached-assistant', parts: [{ text: 'unproven answer', type: 'text' }], role: 'assistant' }
    ]
    ...
    persisted.resolve({
      messages: [
        { content: 'unproven prompt', role: 'user', timestamp: 1 },
        { content: 'unproven answer', role: 'assistant', timestamp: 2 }
      ],
      session_id: 'stored-A'
    })
    await resumePromise

    expect(viewSnapshots.at(-1)?.messages).toHaveLength(2)
  })

  it('rejects proof from another owner when stored session ids are profile-local', async () => {
    const ownerRoute: SessionProfileRoute = {
    ...
    vi.mocked(getSession).mockResolvedValue(ownedRow)
    ...
    state.messages = [
      ...
      { id: 'cached-assistant', parts: [{ text: 'remote answer', type: 'text' }], role: 'assistant' }
    ]
    state.transcriptProvenance = createPersistedDisplayTranscriptProvenance({
      lineageRootId: 'root-A',
      scope: { connectionId: 'remote-B', profile: 'default' },
      storedSessionId: 'stored-A'
    })
    ...
    const resumePromise = resume!('stored-A', true, ownerRoute)
    ...
    persisted.resolve({
      messages: [
        { content: 'remote prompt', role: 'user', timestamp: 1 },
        { content: 'remote answer', role: 'assistant', timestamp: 2 }
      ],
      session_id: 'stored-A'
    })
    await resumePromise
  ...
  it.each(['request failure', 'identity mismatch', 'empty race'] as const)(
    'publishes an unproven cache only after persisted refresh fails: %s',
    async failureMode => {
      const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
        current: new Map([['stored-A', 'rt-A']])
      }
      ...
      const latestState = sessionStateByRuntimeIdRef.current.get('rt-A')!
      sessionStateByRuntimeIdRef.current.set('rt-A', {
        ...latestState,
        messages: [
          ...latestState.messages,
          { id: 'live-user', parts: [{ text: 'live update', type: 'text' }], role: 'user' }
        ]
      })

      if (failureMode === 'request failure') {
        persisted.reject(new Error('backend unavailable'))
      } else if (failureMode === 'identity mismatch') {
        persisted.resolve({
          messages: [{ content: 'wrong session', role: 'user', timestamp: 1 }],
          session_id: 'stored-B'
        })
      } else {
        persisted.resolve({ messages: [], session_id: 'stored-A' })
      }
      ...
      const transcriptPublications = viewSnapshots.filter(snapshot => snapshot.messages.length > 0)
      expect(transcriptPublications).toHaveLength(1)
      expect(JSON.stringify(transcriptPublications[0].messages)).toContain('live update')
      expect(transcriptPublications[0].transcriptProvenance).toBeUndefined()
    }
  )

  it('does not blank a same-selected-session re-resume', async () => {
    const runtimeIdByStoredSessionIdRef: MutableRefObject<Map<string, string>> = {
      current: new Map([['stored-A', 'rt-A']])
    }
    ...
    await waitFor(() => expect(viewSnapshots.length).toBeGreaterThanOrEqual(2))
    expect(viewSnapshots.every(snapshot => JSON.stringify(snapshot.messages).includes('active prompt'))).toBe(true)

    persisted.resolve({
      messages: [
        { content: 'active prompt', role: 'user', timestamp: 1 },
        { content: 'active answer', role: 'assistant', timestamp: 2 }
      ],
      session_id: 'stored-A'
    })
    await resumePromise
  ...
  it('preserves live cache updates that arrive while the persisted transcript is loading', async () => {
    ...
    setSessions([storedSession({ _lineage_root_id: 'root-A', id: 'stored-A', message_count: 4 })])
    ...
    expect(resumedState?.messages.filter(message => JSON.stringify(message).includes('current prompt'))).toHaveLength(1)

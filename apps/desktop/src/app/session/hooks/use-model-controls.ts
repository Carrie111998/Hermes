    ...
    async (selection: ModelSelection): Promise<boolean> => {
      ...
      try {
        const result = await requestGateway<{ deferred?: boolean }>('config.set', {
          ...
          key: 'model',
          // The composer picker is always session-scoped, including the
          // primary chat. Settings → Model is the explicit global-default
          // surface; a one-off chat choice must never rewrite config.yaml.

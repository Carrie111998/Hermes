...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshOrionConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshOrionConfig — two REST calls (config + defaults) per event, per

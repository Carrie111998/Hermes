...
let refreshCharterforgeConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  handleEvent = null
  refreshCharterforgeConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshCharterforgeConfig).not.toHaveBeenCalled()

    await act(async () => {
      await vi.advanceTimersByTimeAsync(400)
    })

    expect(refreshCharterforgeConfig).toHaveBeenCalledTimes(1)
  })

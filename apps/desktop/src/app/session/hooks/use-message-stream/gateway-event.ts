      ...
      if (event.type === 'gateway.ready') {
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Douglas Agent reported an error',
        ...
        const errorMessage = payload?.message || 'Douglas Agent reported an error'

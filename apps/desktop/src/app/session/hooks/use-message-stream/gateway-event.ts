      ...
      if (event.type === 'gateway.ready') {
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'evaOS Agent reported an error',
        ...
        const errorMessage = payload?.message || 'evaOS Agent reported an error'

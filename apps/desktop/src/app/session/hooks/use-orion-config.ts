...
import { getOrionConfig, getOrionConfigDefaults } from '@/orion'
...
interface OrionConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useOrionConfig({ activeSessionIdRef }: OrionConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshOrionConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getOrionConfig(), getOrionConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Orion' built-in medium over the user's config.
  ...
  return { refreshOrionConfig, sttEnabled, voiceMaxRecordingSeconds }
}

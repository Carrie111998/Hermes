...
export function useSubmitPrompt(deps: SubmitPromptDeps) {
  ...
  return useCallback(
    async (rawText: string, options?: SubmitTextOptions) => {
      ...
      // Tracks targetStoredSessionId until the dead-draft recovery in
      // recoverDeadRuntime below re-homes the send to a freshly minted chat —
      ...
      const recoverDeadRuntime = async (
        ...
        try {
          createdId = await createBackendSessionForSend(bubbleText)
        } catch {
          createdId = null
        }
        ...
        if (!createdId) {
          ...
          if (createNullDrift) {
            console.warn('[submit-drift-abort]', createNullDrift, { phase: 'post-recovery-create-null' })
      ...
      try {
        ...
        // submitText overrides `text` when a recovery re-stages attachments
        // into a freshly minted session (the @file: refs change with it).
        const submitParams = (targetId: string, submitText: string = text) => ({
          session_id: targetId,
          text: submitText,
        ...
        // Pinned outside the retry callback: recoverDeadRuntime reassigns
        // sessionId from a nested closure, so TS drops its non-null narrowing
        // inside callbacks.
        ...
        try {
          ...
          if (recovery === null) {
            ...
            if (recovery.minted) {
              ...
              const resyncDrift = sessionDriftReason()

              if (resyncDrift) {
                console.warn('[submit-drift-abort]', resyncDrift, { phase: 'post-recovery-attachments' })
            ...
            await withSessionBusyRetry(() =>
              requestGateway('prompt.submit', submitParams(retryId, retryText), PROMPT_SUBMIT_REQUEST_TIMEOUT_MS)
            )
          }
        }

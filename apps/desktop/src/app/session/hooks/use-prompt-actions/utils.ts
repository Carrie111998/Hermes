...
export async function readImageForRemoteAttach(
  ...
  const dataUrl = await window.orionDesktop?.readFileDataUrl(filePath)
...
export async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.orionDesktop?.readFileDataUrlForAttach ?? window.orionDesktop?.readFileDataUrl

...
import { getOrionConfig } from '@/orion'
...
import { useOrionConfig } from './use-orion-config'

vi.mock('@/orion', () => ({
  getOrionConfig: vi.fn(),
  getOrionConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'orion.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getOrionConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getOrionConfig>>)

describe('useOrionConfig refreshOrionConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Orion' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useOrionConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshOrionConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useOrionConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getOrionConfig>>>()
    vi.mocked(getOrionConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useOrionConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshOrionConfig(true)
    })
    expect(getOrionConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getOrionConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getOrionConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getOrionConfig>>>()
    vi.mocked(getOrionConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshOrionConfig(true)
      refreshC = result.current.refreshOrionConfig(true)
    })

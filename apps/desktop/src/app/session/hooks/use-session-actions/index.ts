    ...
    async (storedSessionId: string, replaceRoute = false, capturedOwner?: SessionProfileRoute) => {
      ...
      try {
        ...
        if (resumed.messages_omitted && !usablePrefetch) {
          try {
            const fallback = await getLatestSessionMessages(resumedStoredSessionId || storedSessionId, sessionRestScope)

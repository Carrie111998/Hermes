...
function responseMatchesCurrentDisplayAuthority(
  ...
  // The list is our current connection/profile authority. A missing or
  // ambiguous alias is unsafe: the same lineage may exist on multiple registry
  // backends, so fail closed rather than selecting first.
    ...
    async (storedSessionId: string, replaceRoute = false, capturedOwner?: SessionProfileRoute) => {
              ...
              const activatedState = updateSessionState(
                ...
                state => ({
                  ...
                  transcriptProvenance:
                    acceptedPersistedProvenance ?? (hasValidProvenance ? (expectedProvenance ?? undefined) : undefined),
      ...
      try {
        ...
        const displayLineageRootId =
          cachedTailProvenance?.lineageRootId ?? storedForCache?._lineage_root_id ?? storedForCache?.id ?? null
        const displayResolvedTipId = cachedTailProvenance?.resolvedTipId ?? storedForCache?.id ?? null

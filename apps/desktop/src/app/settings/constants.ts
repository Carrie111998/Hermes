...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before Douglas Agent stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Douglas Agent can read from one file request.',
  approvals: {
    mode: 'How Douglas Agent handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Douglas Agent updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})

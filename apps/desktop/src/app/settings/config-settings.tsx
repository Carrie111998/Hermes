...
import { getElevenLabsVoices, getOrionConfigSchema, saveOrionConfig } from '@/orion'
...
import type { ConfigFieldSchema, OrionConfigRecord } from '@/types/orion'

import { orionConfigCacheWriter, useOrionConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<OrionConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useOrionConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => orionConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['orion-config-schema'] : ['orion-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getOrionConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveOrionConfig(config, scopeProfile)
  ...
  const applyConfig = (next: OrionConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: OrionConfigRecord) => {

...
import type { ConfigFieldSchema, CharterforgeConfigRecord } from '@/types/hermes'

import { setCharterforgeConfigCache, useCharterforgeConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: CharterforgeConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useCharterforgeConfigRecord()
  ...
  const {
    ...
    queryFn: getCharterforgeConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveCharterforgeConfig(config)
          ...
          setCharterforgeConfigCache(config)
  ...
  const updateConfig = (next: CharterforgeConfigRecord) => {

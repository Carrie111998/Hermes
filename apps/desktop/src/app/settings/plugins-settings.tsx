...
// User-installed plugins first — mirrors `orion plugins list --user`.
...
function reveal(file: string) {
  void window.orionDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // orion_home breaks against a remote backend (#66899).
    const dir = await window.orionDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.orionDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's orion home (profile-aware), so the
// path comes from the gateway — not from the renderer's local ORION_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.orionDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Orion (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>

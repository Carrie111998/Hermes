...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nastech', model: 'nastech-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nastech',
        slug: 'nastech',
        models: ['nastech-4', 'nastech-4-mini'],
        authenticated: true,
        capabilities: { 'nastech-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nastech', model: 'nastech-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ provider: 'nastech', model: 'nastech-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'nastech', model: 'nastech-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getNastechConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "Nastech" shows in both the trigger and the open list.
    expect((await screen.findAllByText('Nastech')).length).toBeGreaterThan(0)
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'nastech', model: 'nastech-4' })
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('Nastech'))
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getNastechConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveNastechConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['nastech-4'],
          authenticated: true,
          capabilities: { 'nastech-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nastech', model: 'nastech-4' }]
    })
    ...
    expect(screen.getByText('nastech')).toBeTruthy()
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'nastech', model: 'nastech-4', enabled: false }),

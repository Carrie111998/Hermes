...
import { disconnectOAuthProvider, listOAuthProviders } from '@/orion'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.orionDesktop?.terminal)
...
//      in orion_cli/provider_catalog.py) — the SAME provider identity
//      `orion model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Orion can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Orion can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Orion never deletes creds another tool owns behind a silent API call.

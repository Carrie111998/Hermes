...
import type { ToolsetConfig } from '@/types/orion'
...
const getOrionConfigSchema = vi.fn()
...
vi.mock('@/orion', () => ({
  ...
  getOrionConfigSchema: () => getOrionConfigSchema(),
...
beforeEach(() => {
  ...
  getOrionConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getOrionConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveOrionConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveOrionConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>

...
import { setOrionConfigCache, useOrionConfigRecord } from '../hooks/use-config-record'
...
export function VoiceProviderFields({ section, providerKey }: { section: 'tts' | 'stt'; providerKey: string }) {
  ...
  const { data: loadedConfig } = useOrionConfigRecord()

  const { data: schemaResponse } = useQuery({
    queryKey: ['orion-config-schema'],
    queryFn: () => getOrionConfigSchema(),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void saveOrionConfig(config)
        .then(() => setOrionConfigCache(config))

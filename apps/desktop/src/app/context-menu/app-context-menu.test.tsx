...
import { ContextMenu, ContextMenuTrigger, ORION_CONTEXT_MENU_TRIGGER_ATTR } from '@/components/ui/context-menu'
...
function installBridge(partial: Partial<Window['orionDesktop']> = {}) {
...
describe('AppContextMenu', () => {
  ...
  it('offers the resolved copy only for loopback links on a remote gateway', async () => {
    ...
    installBridge({
      reachPreviewUrl: reachPreviewUrl as unknown as Window['orionDesktop']['reachPreviewUrl'],
      writeClipboard: writeClipboard as unknown as Window['orionDesktop']['writeClipboard']
    })
    mountMenu()
  ...
  it('runs edit verbs after the menu closed, with focus back on the editable', async () => {
    ...
    installBridge({ contextMenuEdit: contextMenuEdit as unknown as Window['orionDesktop']['contextMenuEdit'] })
    mountMenu()
  ...
  it('grays out paste until the clipboard reports text', async () => {
    ...
    installBridge({ readClipboard: readClipboard as unknown as Window['orionDesktop']['readClipboard'] })
    mountMenu()
    const host = attach('<textarea>text</textarea>')
...
describe('ContextMenuTrigger asChild', () => {
  it('keeps the coordinator marker when the child overwrites data-slot', () => {
    ...
    expect(footer.hasAttribute(ORION_CONTEXT_MENU_TRIGGER_ATTR)).toBe(true)
  })
})

...
const ORION_PATHS_MIME = 'application/x-orion-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ORION_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ORION_PATHS_MIME)
  ...
  const getPath = window.orionDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.orionDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (orion --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.orionDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[orion-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.orionDesktop?.terminal?.write(sessionId, `${command}\r`)

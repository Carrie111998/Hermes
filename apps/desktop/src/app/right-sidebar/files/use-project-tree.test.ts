...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): OrionReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { orionDesktop: unknown }).orionDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: OrionReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<OrionReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { orionDesktop: unknown }).orionDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.orionDesktop is missing', async () => {
    delete (window as unknown as { orionDesktop?: unknown }).orionDesktop

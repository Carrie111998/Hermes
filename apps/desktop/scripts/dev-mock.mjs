...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `orion-dev-mock-${Date.now()}`))
  ...
  return { root, orionHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(orionHome, mockUrl) {
  fs.writeFileSync(
    path.join(orionHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(orionHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.orionHome, mock.url)
  console.log(`  ORION_HOME: ${sandbox.orionHome}`)
  ...
  const env = {
    ...
    ORION_DESKTOP_APP_NAME: `OrionDevMock-${Date.now()}`,
  }

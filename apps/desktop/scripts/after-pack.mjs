 ...
 *
 * Stamps the Charterforge icon + identity onto the packed Windows Charterforge.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Charterforge')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Charterforge'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Charterforge.exe keeps the stock Electron icon`)
  }
}

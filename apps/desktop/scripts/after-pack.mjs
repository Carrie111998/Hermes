 ...
 *
 * Stamps the Orion icon + identity onto the packed Windows Orion.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `orion desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Orion')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Orion'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Orion.exe keeps the stock Electron icon`)
  }
}

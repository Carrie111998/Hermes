...
// no longer bundles the Charterforge Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Charterforge.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Charterforge'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Charterforge.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Charterforge'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default CHARTERFORGE_HOME for non-sandboxed runs -- matches main.ts's
// resolveCharterforgeHome(). On Windows it's %LOCALAPPDATA%\hermes; elsewhere
// it's ~/.hermes. The fresh-install sandbox launchFresh() sets its own
// CHARTERFORGE_HOME and never touches this.
const DEFAULT_CHARTERFORGE_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'hermes')
  }
  return path.join(os.homedir(), '.hermes')
})()
const VENV_ROOT = path.join(DEFAULT_CHARTERFORGE_HOME, 'hermes-agent', 'venv')
...
function ensurePackagedApp() {
  if (process.env.CHARTERFORGE_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Charterforge-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Charterforge-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Charterforge-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Charterforge-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CHARTERFORGE_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CHARTERFORGE_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  env.CHARTERFORGE_DESKTOP_CWD = cwd
  env.CHARTERFORGE_DESKTOP_IGNORE_EXISTING = '1'
  env.CHARTERFORGE_DESKTOP_TEST_MODE = 'fresh-install'
  env.CHARTERFORGE_DESKTOP_USER_DATA_DIR = userDataDir
  env.CHARTERFORGE_HOME = hermesHome
  delete env.CHARTERFORGE_DESKTOP_HERMES
  delete env.CHARTERFORGE_DESKTOP_CHARTERFORGE_ROOT
  ...
  console.log(`  CHARTERFORGE_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)
...
//   - The Charterforge Python payload is NOT shipped (it's fetched at first
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Charterforge
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CHARTERFORGE_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CHARTERFORGE_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}

...
// no longer bundles the Orion Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Orion.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Orion'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Orion.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Orion'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ORION_HOME for non-sandboxed runs -- matches main.ts's
// resolveOrionHome(). On Windows it's %LOCALAPPDATA%\orion; elsewhere
// it's ~/.orion. The fresh-install sandbox launchFresh() sets its own
// ORION_HOME and never touches this.
const DEFAULT_ORION_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'orion')
  }
  return path.join(os.homedir(), '.orion')
})()
const VENV_ROOT = path.join(DEFAULT_ORION_HOME, 'orion-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'orion-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ORION_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Orion-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Orion-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Orion-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Orion-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ORION_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ORION_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const orionHome = path.join(sandbox, 'orion-home')
  ...
  env.ORION_DESKTOP_CWD = cwd
  env.ORION_DESKTOP_IGNORE_EXISTING = '1'
  env.ORION_DESKTOP_TEST_MODE = 'fresh-install'
  env.ORION_DESKTOP_USER_DATA_DIR = userDataDir
  env.ORION_HOME = orionHome
  delete env.ORION_DESKTOP_ORION
  delete env.ORION_DESKTOP_ORION_ROOT
  ...
  console.log(`  ORION_HOME: ${orionHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(orionHome, 'orion-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Orion Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of orion_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'orion-agent', 'orion_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Orion
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ORION_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ORION_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}

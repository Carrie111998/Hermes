...
const UPSTREAM_URL = 'https://github.com/zacharyjleach-stack/Aries.git'
...
const LEGACY_STORAGE_KEY = 'orion.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'orion.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.orionDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `orion-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const orionHome = join(sandbox, 'orion-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(orionHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(orionHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        ORION_DESKTOP_APP_NAME: `OrionShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        ORION_DESKTOP_ORION_ROOT: target.targetRoot,
        ORION_DESKTOP_IGNORE_EXISTING: '1',
        ORION_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'orion-short-session-ab-'))
  const prepared = []

...
//   · its own CHARTERFORGE_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · CHARTERFORGE_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated CHARTERFORGE_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate CHARTERFORGE_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  try {
    ...
    // CHARTERFORGE_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set CHARTERFORGE_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...process.env,
      CHARTERFORGE_HOME: home,
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.CHARTERFORGE_DESKTOP_DEV_SERVER = devUrl
    }

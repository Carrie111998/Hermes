...
// delta flush does — via the __CHARTERFORGE_SESSION_TILES__ hook.
//
...
const setup = (tiles, seedTurns, streamSeed) => `
  (() => {
    const hook = window.__CHARTERFORGE_SESSION_TILES__
...
const reveal = sid => `window.__CHARTERFORGE_LAYOUT_TREE__.reveal(${JSON.stringify(`session-tile:${sid}`)})`
  ...
  (() => {
    if (window.__MT__) {
      ...
      for (const { sid, rid } of window.__MT__.ids) {
        window.__CHARTERFORGE_SESSION_TILES__.publish(rid, {
          ...window.__CHARTERFORGE_SESSION_TILES__.states()[rid], busy: false, streamId: null
        })
        window.__CHARTERFORGE_SESSION_TILES__.close(sid)
      }
      window.__MT__ = null
    }
    return 'cleaned'

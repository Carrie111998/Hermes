...
const setup = (tiles, seedTurns) => `
  (() => {
    const hook = window.__ORION_SESSION_TILES__
...
const reveal = sid => `window.__ORION_LAYOUT_TREE__.reveal(${JSON.stringify(`session-tile:${sid}`)})`
  ...
  (() => {
    if (window.__IDLE__) {
      for (const { sid, rid } of window.__IDLE__.ids) {
        const states = window.__ORION_SESSION_TILES__.states()
        window.__ORION_SESSION_TILES__.publish(rid, { ...states[rid], busy: false, streamId: null })
        window.__ORION_SESSION_TILES__.close(sid)
      }

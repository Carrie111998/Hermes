 ...
 * product name (`Orion`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Orion'
 *
 * This is a hard failure with no obvious cause for the user — `orion desktop`
 ...
 * (the mac-unpacked Orion.app bundle) and Windows (win-unpacked), so we
 ...
 * Orion.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `orion desktop --build-only` (orion_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Orion.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Orion'}.exe`

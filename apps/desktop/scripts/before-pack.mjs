 ...
 * product name (`Charterforge`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Charterforge'
 *
 ...
 * (the mac-unpacked Charterforge.app bundle) and Windows (win-unpacked), so we
 ...
 * Charterforge.exe that Windows can't load (truncated PE from a corrupt cached
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Charterforge.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Charterforge'}.exe`

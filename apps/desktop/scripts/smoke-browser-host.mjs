...
const terminalExpression = `(async () => {
  ...
  try {
    ...
    return { cwd, ok: output.includes(marker), outputTail: output.slice(-800), shell: session.shell, skipped: false }
  } catch (error) {
...
try {
  ...
  for (const viewport of [
    ...
    if (requireTerminal && viewport.width === 390 && !terminalState.ok) {
      failures.push(
        `browser Desktop shell terminal failed: ${terminalState.error || terminalState.outputTail || 'marker missing'}`
      )
    }

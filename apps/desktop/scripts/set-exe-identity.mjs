#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Charterforge icon + version metadata onto the
// built Charterforge.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Charterforge.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Charterforge.exe>
//
...
// Stamp the Charterforge icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Charterforge',
      FileDescription: 'Charterforge',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Charterforge icon + identity stamped')
}

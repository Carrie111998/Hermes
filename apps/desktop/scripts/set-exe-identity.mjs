#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Orion icon + version metadata onto the
// built Orion.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Orion.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `orion desktop`, the installer's
...
// path (which rebuilds via `orion desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Orion.exe>
//
...
// Stamp the Orion icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      CompanyName: 'Orion',
      LegalCopyright: 'Copyright (c) 2026 Orion'
    }
  })

  console.log('[set-exe-identity] done — Orion icon + identity stamped')
}

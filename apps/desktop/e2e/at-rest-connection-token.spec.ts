 ...
 * dir (macOS `~/Library/Application Support/Orion/connection.json`, Windows
 * `AppData\Roaming\Orion\connection.json`). Anything that can read the file
 ...
 *      or in ORION_HOME (logs included).
 ...
 * `X-Orion-Session-Token` header it receives — a dropped or mangled token
 ...
 * gateway token: `51c68d4ab` ("Add Orion desktop app (#20059)"), the commit
 ...
 */
const SENTINEL_TOKEN = 'orion-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'OrionE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-Orion-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote Orion gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-orion-session-token']
 ...
 * backend, so we skip spawning `orion serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startOrion() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    ORION_DESKTOP_APP_NAME: STABLE_APP_NAME,
    ORION_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { orionDesktop?: Record<string, unknown> }).orionDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture Orion should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { orionDesktop: any }).orionDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // ORION_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.orionHome, needles),
      'the gateway session token leaked into a ORION_HOME file (logs included)',
    ).toEqual([])

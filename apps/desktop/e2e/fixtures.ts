/**
 * Shared E2E fixtures for the Orion desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → orion serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `orion desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `orion-e2e-${prefix}-${Math.random()}`))
  const orionHome = path.join(root, 'orion-home')
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(orionHome, 'config.yaml')
 ...
 */
export function writeEnvFile(orionHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(orionHome, '.env')
 ...
 */
function writeEmptyConfig(orionHome: string): void {
  const configPath = path.join(orionHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ORION_HOME → sandbox orion-home (isolated config/sessions)
 *  - ORION_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ORION_DESKTOP_IGNORE_EXISTING=1 → don't pick up `orion` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ORION_DESKTOP_ORION_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ORION_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ORION_HOME: sandbox.orionHome,
    ORION_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ORION_DESKTOP_IGNORE_EXISTING: '1',
    ORION_DESKTOP_ORION_ROOT: REPO_ROOT,
    ORION_DESKTOP_APP_NAME: `OrionE2E-${Date.now()}`,
    ...
    ORION_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ORION_HOME + userData
 * @param env      - the process environment (already has ORION_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.orionHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ORION_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`orion serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.orionHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ORION_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Orion backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Orion.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Orion.app', 'Contents', 'MacOS', 'Orion')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'orion')
}
 ...
 * progress without spawning a real Orion backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ORION_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ORION_DESKTOP_BOOT_FAKE: '1',
    ORION_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + orion-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ORION_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ORION_DESKTOP_ORION
  delete (env as Record<string, string | undefined>).ORION_DESKTOP_ORION_ROOT

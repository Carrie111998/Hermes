 ...
 * packaged binary) with a real `orion serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → orion serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Orion title', async () => {
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { orionDesktop?: Record<string, unknown> }).orionDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → orion serve is spawned → WS connects → config

...
/**
 * E2E smoke tests for the packaged Orion desktop app.
 *
 ...
 * (credential stripping, isolated ORION_HOME + userData, unique app name).
 *
...
test('window opens with the Orion title', async () => {
  const title = await fixture!.page.title()
  expect(title).toContain('Orion')
})
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      orionDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).orionDesktop?.hud?.open({ sessionId: null })
  )

 ...
 * AIAgent loop, using the E2E mock provider configured in `orionHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(orionHome: string) {
  ...
  static async start(orionHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(orionHome)

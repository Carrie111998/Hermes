...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Orion E2E'], { cwd: repo })
...
function configureRepoCwd(orionHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(orionHome, mockUrl)
  fs.appendFileSync(path.join(orionHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(orionHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.orionHome, mock.url, repo)

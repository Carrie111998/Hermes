...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.orionHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.orionHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.orionHome)
  const builder = await RealSessionBuilder.start(sandbox.orionHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.orionHome, mock.url)
  fs.appendFileSync(path.join(sandbox.orionHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.orionHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))

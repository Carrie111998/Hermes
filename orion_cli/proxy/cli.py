"""CLI handlers for the ``orion proxy`` subcommand."""
...
from orion_cli.proxy.adapters import ADAPTERS, get_adapter
from orion_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "orion proxy requires aiohttp. Run `orion setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"orion auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Orion proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Orion proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: orion proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``orion proxy <subcommand>``."""
    ...
    print(
        "orion proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  orion proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  orion proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  orion proxy providers\n"

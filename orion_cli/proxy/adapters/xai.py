...
from orion_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# xAI's public API is OpenAI-compatible for the endpoints Orion commonly
# uses. The Responses endpoint is included because Orion' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Orion-managed OAuth credentials."""

    auth_hint = "orion auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`orion auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`orion auth reset xai-oauth` or re-authenticate with "
                    "`orion auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `orion auth add xai-oauth --type oauth`."
            )

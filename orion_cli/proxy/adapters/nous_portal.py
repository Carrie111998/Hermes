...
Reads the user's Nous OAuth state from ``~/.orion/auth.json`` through the
...
from orion_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `orion auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `orion auth add nous` to re-authenticate."
                )
    ...
    # to orion_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------

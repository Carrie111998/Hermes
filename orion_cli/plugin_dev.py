"""Runtime-backed validation behind ``orion plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary ORION_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="orion-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            ...
            {
                "ORION_HOME": str(home),
                "ORION_BUNDLED_PLUGINS": str(bundled),
                "ORION_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Orion discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "orion_plugins" or name.startswith("orion_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_orion_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".orion" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from orion_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Orion supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Orion never auto-installs plugin dependencies; "
    ...
    if schema:
        from orion_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Orion' real scanner and registration path."""
    try:

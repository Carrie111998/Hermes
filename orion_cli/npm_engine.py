...
Scope of the repair is deliberately narrow. Orion only upgrades an npm that
lives inside its **own** managed Node tree (``$ORION_HOME/node``), installing
...
and their other projects; Orion never modifies those. When the failing npm is
one of those foreign installs, Orion instead provisions its own managed Node
...
from orion_constants import (
    bootstrap_orion_managed_node,
    get_orion_home,
    managed_node_tree_in_use,
    with_orion_node_path,
)
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Orion-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$ORION_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$ORION_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Orion-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Orion-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="orion-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Orion could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Orion-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$ORION_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Orion-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_orion_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Orion-managed install, or
    ...
    if prefix is not None:
        # Orion owns this npm — upgrade it in place. Only an npm-range

...
``$ORION_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-orion-setup
(the stage2 hook) has chowned the volume and seeded $ORION_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ORION_HOME``,
    not under ``profiles/``). The dispatcher in ``orion_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``orion gateway start`` (no ``-p``) targets. Without it,
    bare ``orion gateway start`` inside the container would land on
    ...
    ``$ORION_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        orion_home: The container's ORION_HOME (typically /opt/data).
            Profiles live under ``<orion_home>/profiles/<name>/``;
            the default profile lives at ``<orion_home>`` itself.
    ...
    # ``orion gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(orion_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(orion_home)
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(orion_home),
    ))

    profiles_root = orion_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `orion profile create` (config.yaml
            # is not — that comes later via `orion setup`). Use it as the
            ...
            # via ``orion -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(orion_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = orion_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("ORION_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the orion args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/orion/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/orion/docker/main-wrapper.sh
    ...
    ``orion`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``orion <subcommand>``, so an
    explicit leading ``orion`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `orion <subcommand>`; peel an explicit orion.
    if args and Path(args[0]).name == "orion":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``orion dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted ORION_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `orion -p <profile> gateway start`
        # (which routes through the Phase 4
        # _dispatch_via_service_manager_if_s6 helper to `s6-svc -u`).
        if not start:
            (tmp_dir / "down").touch()

        # Pre-create the supervise/ skeleton with orion ownership
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit orion
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the orion user) won't hit EACCES. See
...
def _write_reconcile_log(
    orion_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $ORION_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted ORION_HOME, both race to flock() the same s6-log lock
    ...
    orion_home = Path(os.environ.get("ORION_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        orion_home=orion_home, scandir=scandir,
    )
    for a in actions:
        print(

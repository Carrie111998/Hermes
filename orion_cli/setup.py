"""
Interactive setup wizard for Orion Agent.
...
from orion_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://your-orion-docs.example/docs"
...
# display_orion_home imported lazily at call sites (stale-module safety during orion update)
...
from orion_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from orion_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Orion Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Orion using environment variables or config commands:")
    print_info("  orion config set model.provider custom")
    print_info("  orion config set model.base_url http://localhost:8080/v1")
    print_info("  orion config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'orion setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``ORION_NONINTERACTIVE=1`` (see ``orion_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ORION_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ORION_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'orion setup')")


def _print_setup_summary(config: dict, orion_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Orion cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    orion model            (pick any provider/model)")
        print_info("    orion setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'orion setup' to configure"))
    ...
    # Video generation — opt-in via `orion tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from orion_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'orion setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'orion setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'orion tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'orion setup terminal'"))
    ...
    # Spotify (OAuth via orion auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'orion setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {orion_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('orion setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('orion setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('orion setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('orion setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('orion setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('orion config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('orion config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('orion config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('orion', Colors.GREEN)}              Start chatting")
    print(f"   {color('orion gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('orion doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `orion tools` and `orion setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``orion model``)
    ...
    provider added to ``orion model`` is automatically available here.
    ...
    # Delegate to the shared orion model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: orion model")
    ...
    # on demand via `orion auth add`, `orion setup` vision, and
    # `orion setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``orion model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.orion/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Orion auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via orion setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'orion setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Orion runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `orion setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `orion setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `orion egress setup` then `orion egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `orion egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'orion-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $ORION_HOME/bin is never on PATH, so a bare
            # which() misses the uv Orion installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'orion-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ORION_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ORION_MAX_ITERATIONS")
    ...
    print_info("  Run `orion setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ORION_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.orion/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from orion_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_orion_home(Path(get_orion_home()))
    except Exception:
        pass
...
def _profile_name_from_orion_home(orion_home) -> str | None:
    """Return the active profile name when ORION_HOME is a profile dir."""
    if orion_home.parent.name == "profiles":
        return orion_home.name
    return None
...
def _setup_bluebubbles():
    ...
    print_info("Connects Orion to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from orion_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://your-orion-docs.example/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://your-orion-docs.example/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  orion config edit")
    print_info("   Open config in your editor:  orion config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from orion_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Orion from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'orion setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     orion config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `orion import` or `orion setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `orion setup tools` and `orion tools` use the same flow:
    ...
    """
    from orion_cli.tools_config import tools_command
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Orion profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``orion_cli.auth`` — lists every supported
    ...
    """
    try:
        from orion_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from orion_cli.config
    ...
    if section_key == "model":
        ...
        from orion_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_orion migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_orion", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Orion.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Orion to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Orion at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Orion at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Orion at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Orion at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Orion equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Orion config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Orion.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Orion's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(orion_home: Path) -> bool:
    ...
    print_info("Orion can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: orion claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: orion claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Orion configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Orion config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Orion (use orion claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``orion setup --portal`` and ``orion portal``. This is the
    ...
    Orion session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``orion setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``orion model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``orion portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Orion Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `orion portal` == quick setup's Nous step.
    try:
        from orion_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `orion portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `orion portal`: %s", exc)
        print()
    ...
    print_info("  Run `orion portal info` to inspect routing.")
    print_info("  Run `orion` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      orion setup           — full or quick (auto-detected)
      orion setup model     — just model/provider
      orion setup tts       — just text-to-speech
      orion setup terminal  — just terminal backend
      orion setup gateway   — just messaging platforms
      orion setup tools     — just tool configuration
      orion setup telemetry — just local shared metrics
      orion setup agent     — just agent settings
    """
    from orion_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Orion Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Orion Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Orion Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, orion_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Orion configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'orion setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(orion_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Orion?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, orion_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, orion_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {orion_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'orion config edit'")
    ...
    # Tune later with `orion setup agent`.
    if not is_existing:
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from orion_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    _print_setup_summary(config, orion_home)


def _run_first_time_quick_setup(config: dict, orion_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``orion setup <section>``
    or switch providers with ``orion model``.
    """
    from orion_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'orion setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `orion import` from another machine).
        from orion_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    orion setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  orion setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``orion tools`` (which
    ...
    try:
        ...
        from orion_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``orion setup agent`` /
    ``orion config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, orion_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `orion update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        orion tools")
        print_info("  Seed skills:         orion skills opt-in --sync")
        print_info("  Add MCP servers:     orion mcp add")
        print_info("  Enable plugins:      orion plugins")
        print_info("  Tune agent settings: orion setup agent")
        print()
        _print_setup_summary(config, orion_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, orion_home)


def _blank_slate_walkthrough(config: dict, orion_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`orion update` runs from re-injecting them. Opt back in any")
            print_info("time with `orion skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `orion tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `orion plugins list` / `orion plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `orion plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `orion mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `orion mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   orion tools")
    print_info("  Seed skills:         orion skills opt-in --sync")
    print_info("  Add MCP servers:     orion mcp add")
    print_info("  Tune agent settings: orion setup agent")
    print()
...
def _run_quick_setup(config: dict, orion_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'orion setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Orion to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'orion setup gateway'.")

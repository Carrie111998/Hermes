"""``orion skin`` — list, switch, and tweak skins from the CLI.
...
from orion_constants import display_orion_home, get_orion_home

_HEX_RE = re.compile(r"^#[0-9a-fA-F]{6}$")


def _skins_dir() -> Path:
    return get_orion_home() / "skins"
...
def _use(name: str) -> None:
    ...
    from orion_cli.config import config_command
...
def _skin_set(key: str, value: str, skin: str | None) -> int:
    ...
    print(f"✓ {key} = {value} in {display_orion_home()}/skins/{target}.yaml (live within ~1s)")
    return 0

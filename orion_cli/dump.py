"""
Dump command for orion CLI.

Outputs a compact, plain-text summary of the user's Orion setup
...
from orion_cli.config import get_orion_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.orion/.env.
    ...
    exports. ``orion debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.orion_build_sha`` by the Dockerfile's
    ``ORION_GIT_SHA`` build-arg (see ``orion_cli/build_info.py``).
    ...
    try:
        from orion_cli.build_info import get_build_sha
...
def _redact(value: str) -> str:
    ...
    ``orion dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from orion_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(orion_home: Path) -> int:
    """Count installed skills."""
    skills_dir = orion_home / "skills"
...
def _cron_summary(orion_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = orion_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"orion_home:      {display_orion_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.orion/.env: a managed
        ...
        # A credential added via `orion auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `orion auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["orion-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(orion_home)}")
    lines.append(f"  skills:             {_count_skills(orion_home)}")

...
Usage:
    orion pairing list              # Show all pending + approved users
    orion pairing approve <platform> <request-id|code>  # Approve a pairing request
    orion pairing revoke <platform> <user_id> # Revoke user access
    orion pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle orion pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: orion pairing {list|approve|revoke|clear-pending}")
        print("Run 'orion pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: orion pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.orion/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'orion pairing list' to see pending requests.\n")

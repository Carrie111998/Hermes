...
- ``orion_cli.providers``        -- canonical provider identity + overlays
- ``orion_cli.model_normalize``  -- per-provider name formatting
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Orion wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Orion.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Orion versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``orion model`` wizard — not a
    ...
    while ``orion model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Orion itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Orion itself discovered (``models_discovered: true``
...
_ORION_MODEL_WARNING = (
    "Orion Orion 3 & 4 models are NOT agentic and are not designed "
    "for use with Orion Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Orion Orion 3 / Orion 4 chat families.
# The previous substring check (`"orion" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``orion-brain:qwen3-14b-ctx16k`` that just
# happen to carry "orion" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   Orion/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/orion3:70b
# Negative examples it must NOT match:
#   orion-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ORION_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])orion[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_orion_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Orion 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ORION_NON_AGENTIC_RE.search(model_name))


def _check_orion_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Orion 3/4 chat model."""
    if is_nous_orion_non_agentic(model_name):
        return _ORION_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``orion config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from orion_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``orion_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'orion model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'orion doctor' — config issues detected:"
        ...
        from orion_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from orion_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from orion_cli.config import load_config
            from orion_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from orion_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from orion_cli.models import should_use_ollama_native_catalog
    ...
    # orion_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from orion_cli.models import opencode_provider_family as _oc_family_fn
    ...
    orion_warn = _check_orion_model_warning(new_model)
    if orion_warn:
        warnings.append(orion_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from orion_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Orion provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from orion_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from orion_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from orion_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from orion_cli.providers import ORION_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from orion_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Orion-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for orion_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(orion_id)
        if (
            _alias_target
            and _alias_target != orion_id
        ...
        try:
            ...
            pass
        if _canonical != orion_id:
            continue
        if orion_id.lower() in seen:
            continue
        if orion_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(orion_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from orion_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(orion_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(orion_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        orion_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(orion_id)
            seen.add(orion_id.lower())

    # --- Section 2: Orion-only providers (ORION_OVERLAYS) ---
    _mdev_to_orion = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in ORION_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        orion_slug = _mdev_to_orion.get(pid, pid)
        if orion_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or orion_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or orion_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(orion_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(orion_slug)
            seen.add(pid.lower())
            seen.add(orion_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from orion_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against orion_id / mdev_id (section 1), pid / orion_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from orion_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by orion provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://your-orion-docs.example/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Orion release. Falls back to the in-repo
    ...
    # --- 1. Check Orion-mapped providers ---
    from orion_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from orion_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for orion_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip orion_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose orion_id matches the canonical profile name so
        ...
        # same orion_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if orion_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (ORION_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Orion may not route yet.
        ...
        if not has_creds:
            try:
                from orion_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(orion_id)
                )
        ...
        # /model picker sees the SAME list `orion model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(orion_id)
        if not model_ids:
            model_ids = curated.get(orion_id, [])
            if orion_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(orion_id, model_ids)
    ...
    # --- 2. Check Orion-only providers (nous, openai-codex, copilot, opencode-go) ---
    from orion_cli.providers import ORION_OVERLAYS
    from orion_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Orion provider ID.
    # ORION_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Orion IDs ("copilot").
    _mdev_to_orion = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in ORION_OVERLAYS.items():
        ...
        # Resolve Orion slug — e.g. "github-copilot" → "copilot"
        orion_slug = _mdev_to_orion.get(pid, pid)
        if orion_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or orion_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(orion_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or orion_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(orion_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(orion_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", orion_slug, exc)
        ...
        # configured.
        if not has_creds and orion_slug == "anthropic":
            try:
                ...
                if (orion_creds and orion_creds.get("accessToken")) or \
        ...
        if orion_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(orion_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(orion_slug)
                model_ids = _ids if _ids else (curated.get(orion_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(orion_slug, []) or curated.get(pid, [])
        elif orion_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `orion model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from orion_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(orion_slug, []) or curated.get(pid, [])
                if orion_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(orion_slug, model_ids)
        total = len(model_ids)
        if orion_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": orion_slug,
            "name": get_label(orion_slug),
            "is_current": orion_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "orion",
        ...
        seen_slugs.add(orion_slug.lower())
        _record_builtin_endpoint(orion_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ORION_OVERLAYS (keeps /model in sync
    # with `orion model`).
    try:
        from orion_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Orion writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Orion's own writer uses (mirrors section-4 grouping).
            ...
            # ``orion model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``orion model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from orion_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Orion's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, orion_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``orion model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from orion_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`orion_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from orion_cli.models import fetch_openrouter_models

"""Shell completion script generation for orion CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_orion_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Orion Agent bash completion
# Add to ~/.bashrc:
#   eval "$(orion completion bash)"

_orion_profiles() {{
    local profiles_dir="$HOME/.orion/profiles"
...
_orion_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_orion_profiles)" -- "$cur"))
        return
    fi
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef orion
# Orion Agent zsh completion
# Add to ~/.zshrc:
...
_orion_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.orion/profiles" ]]; then
        profiles+=($HOME/.orion/profiles/*(N/:t))
    fi
...
_orion() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_orion_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'orion command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Orion Agent fish completion",
        "# Add to your config:",
        "#   orion completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __orion_profiles",
        "    echo default",
        "    if test -d $HOME/.orion/profiles",
        "        for d in $HOME/.orion/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c orion -f -s p -l profile"
        " -d 'Profile name' -xa '(__orion_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__orion_profiles)' -d 'Profile name'"
                )

"""``orion worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``orion_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    orion worktree list             # audit: verdict + reason per tree
    orion worktree prune            # reap safe trees + merged branches
    orion worktree prune --dry-run  # show the plan, change nothing
    orion worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `orion worktree prune`."
        )

"""Dependency-light venv recovery that runs BEFORE orion_cli.main's imports.

The ``orion`` console entry point is ``orion_cli.main:main``.  Importing
``orion_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``orion_cli.env_loader``, ``yaml`` via ``orion_cli.config``, ...).  In
...
a corrupted venv.  ``orion_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``orion_cli.main`` to become
...
#
# ``orion update`` renames the live ``orion*.exe`` shims aside
# (``orion.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``orion`` on PATH, and the command that would repair it IS ``orion
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY orion
    invocation, and ``orion acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Orion already vendors
    (``~/.orion/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".orion" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``orion acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``orion_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``orion_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``orion update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `orion update`: CLI and Desktop retries preserve
...
# every `orion` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``orion update`` (or the self-lock preflight, #83569) left the
    ...
    ``orion_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``orion acp`` included).
    ...
    try:
        ...
        try:
            print(
                "⚠ A previous `orion update` was interrupted mid-install — "
            ...
            print(
                "  The next launch will retry; orion will keep working from "

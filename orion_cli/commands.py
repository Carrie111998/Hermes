"""Slash command definitions and autocomplete for the Orion CLI.
...
class CommandDef:
    ...
    # Names a key in ``orion_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``orion_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Orion config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Orion works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Orion' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Orion is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.orion/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Orion Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Orion Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`orion_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/orion`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Orion ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Orion
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``orion skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``orion skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``orion skills list`` and
...
# unrelated command landed. These claim their slots right after /orion,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /orion <command>).
...
# native slot, the alias spelling stays reachable via /orion reset).
...
# ``/orion <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/orion" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /orion topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /orion moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /orion debug on Slack.
#   - egress: Docker-only proxy status; reachable as /orion egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /orion init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /orion version on
...
#   - diff: git working-tree diff; reached via /orion diff on Slack so it
...
#     /orion update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /orion heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /orion refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /orion pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /orion whoami on Slack.
...
#     /orion platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_ORION_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/orion <verb>`` subcommand.
    ...
    can still reach them via ``/orion <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/orion`` is always reserved as the first entry so the
    legacy ``/orion <subcommand>`` form keeps working for anything that
    ...
    # Reserve /orion as the catch-all top-level command.
    entries.append(("orion", "Talk to Orion or run a subcommand", "[subcommand] [args]"))
    seen.add("orion")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ORION_ONLY:
            # Intentionally Slack-via-/orion only (see _SLACK_VIA_ORION_ONLY).
            return
    ...
    # everything except /orion, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://orion-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /orion handler.

    Maps both canonical names and aliases so /orion bg do stuff works
    the same as /orion background do stuff.

    Plugin-registered slash commands are included so ``/orion <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via orion_cli.personality."""
        try:

"""
Top-level argparse construction for the orion CLI.
...
# argparse runs (it sets ``ORION_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``orion_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    orion                        Start interactive chat
    orion chat -q "Hello"        Single query mode
    orion --tui                  Launch the modern TUI (or set display.interface: tui)
    orion --cli                  Force the classic REPL (overrides display.interface: tui)
    orion -c                     Resume the most recent session
    orion -c "my project"        Resume a session by name (latest in lineage)
    orion --resume <session_id>  Resume a specific session by ID
    orion --resume latest        Resume the most recent session (same as -c)
    orion --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    orion setup                  Run setup wizard
    orion logout                 Clear stored authentication
    orion auth add <provider>    Add a pooled credential
    orion auth list              List pooled credentials
    orion auth remove <p> <t>    Remove pooled credential by index, id, or label
    orion auth reset <provider>  Clear exhaustion status for a provider
    orion model                  Select default model
    orion fallback [list]        Show fallback provider chain
    orion fallback add           Add a fallback provider (same picker as `orion model`)
    orion fallback remove        Remove a fallback provider from the chain
    orion config                 View configuration
    orion config edit            Edit config in $EDITOR
    orion config set model gpt-4 Set a config value
    orion gateway                Run messaging gateway
    orion -s orion-agent-dev,github-auth
    orion -w                     Start in isolated git worktree
    orion gateway install        Install gateway background service
    orion sessions list          List past sessions
    orion sessions browse        Interactive session picker
    orion sessions rename ID T   Rename/title a session
    orion logs                   View agent.log (last 50 lines)
    orion logs -f                Follow agent.log in real time
    orion logs errors            View errors.log
    orion logs --since 1h        Lines from the last hour
    orion debug share             Upload debug report for support
    orion console                Open the safe Orion command console
    orion update                 Update to latest version
    orion dashboard              Start web UI dashboard (port 9119)
    orion dashboard --stop       Stop running dashboard processes
    orion dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="orion",
        description="Orion Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `orion chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via ORION_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `orion setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to ORION_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.orion/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Orion Agent",
    )
    ...
    # top-level parser: when the user writes `orion -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/orion_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also ORION_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.orion/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Orion itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,

...
``orion_cli.config.migrate_config``. Each step is a function
...
``orion_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``orion_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``orion_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``orion_cli.config`` (e.g.
``patch("orion_cli.config.read_raw_config", ...)``) keep working, because
...
#: ORION_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_orion_home()}/config.yaml and run `orion setup` to "
...
def _cfg():
    """Return the live ``orion_cli.config`` module (lazy, cycle-free)."""
    from orion_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_orion_home = _c.get_orion_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$ORION_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `orion plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `orion curator status` has no
    ...
    #   3. Creates `~/.orion/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_orion_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_orion_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `orion config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `orion config set`"
                )
...
# behavior set it to true explicitly via `orion config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from orion_cli.relay_plugin_cutover import legacy_relay_plugin_keys

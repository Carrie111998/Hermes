...
Used by ``orion kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``orion_cli/goals.py`` — same aux
...
ORION_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("ORION_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Orion Agent board.
...
def _profile_author() -> str:
    """Mirror of ``orion_cli.kanban._profile_author``. Kept local to
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=ORION_KANBAN_SPECIFY_MAX_TOKENS,

...
Config files are stored in ~/.orion/ for easy access:
- ~/.orion/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.orion/.env         - API keys and secrets

This module provides:
- orion config          - Show current configuration
- orion config edit     - Open config in editor
- orion config get      - Print a resolved configuration value
- orion config set      - Set a specific value
- orion config unset    - Remove a user configuration value
- orion config wizard   - Re-run setup wizard
"""

import copy
from orion_cli.cli_output import line_input
import json
import logging
import os
import platform
...
from orion_cli.route_identity import normalize_route_base_url
from orion_cli.secret_prompt import masked_secret_prompt
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``orion config set`` (which rewrites
    ...
    ``config.yaml`` in place: orion never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.orion/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``orion logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  orion config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Orion makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Orion itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``orion update``, the TUI build.
...
#   on every plugin install / ``orion update``.
...
# * ``ORION_HOME`` / ``ORION_PROFILE`` / ``ORION_CONFIG`` /
#   ``ORION_ENV`` — Orion runtime location flags. Writing these into
...
#
# IMPORTANT: ``ORION_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ORION_LANGFUSE_PUBLIC_KEY,
# ORION_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Orion runtime location — never via dashboard env writer.
    # NOT a ORION_* blanket: integration credentials (ORION_GEMINI_*,
    # ORION_LANGFUSE_*, ORION_SPOTIFY_*, ...) ARE allowed.
    "ORION_HOME", "ORION_PROFILE", "ORION_CONFIG", "ORION_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Orion runtime location "
            "(ORION_HOME, ORION_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # ORION_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `orion plugins enable
    # observability/langfuse` or `orion tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ORION_LANGFUSE_ENV",
    "ORION_LANGFUSE_RELEASE",
    "ORION_LANGFUSE_SAMPLE_RATE",
    "ORION_LANGFUSE_MAX_CHARS",
    "ORION_LANGFUSE_CAPTURE",
    "ORION_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "ORION_ACP_AUTH_METHOD",
    "ORION_ACP_AUTO_APPROVE",
    "ORION_COPILOT_ACP_COMMAND",
    "ORION_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
    "COPILOT_ACP_BASE_URL",
})
import yaml

from orion_cli.colors import Colors, color
from orion_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set ORION_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ORION_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_orion_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # ORION_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Orion is running in package-manager-managed mode.

    Two signals: the ORION_MANAGED env var (set by the systemd service),
    or a .managed marker file in ORION_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Orion through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``orion_cli/`` — i.e. the git checkout for source
    installs, ``/opt/orion`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ORION_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Orion was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$ORION_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ORION_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.orion``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ORION_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.orion:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ORION_HOME``, a
    ...
    ``docker`` and ``orion update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/orion-agent`` image bakes a ``docker``
        stamp into ``/opt/orion`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ORION_HOME.
    try:
    ...
    #    host install whose shared $ORION_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``orion update``.
    try:
        method = (
            (get_orion_home() / ".install_method")
    ...
    # detect Nix installs that don't set ORION_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``orion_constants.is_container`` (import-safe)."""
    try:
        from orion_constants import is_container
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ORION_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/orion`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``orion update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# orion_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Orion, it doesn't update
...
✗ ``orion update`` doesn't apply inside the Docker container.

Orion Agent runs as a published image (nousresearch/orion-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/orion-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate orion-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/orion-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/orion-agent/tags
  • Your config and session history live under ``$ORION_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``orion update`` inside Docker.
...
def format_managed_message(action: str = "modify this Orion installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Orion installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Orion."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ORION_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, orion_bin
    or None if container mode is not active, we're already inside the
    container, or ORION_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ORION_DEV") == "1":
        return None
    ...
    container_mode_file = get_orion_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "orion-agent")
    exec_user = info.get("exec_user", "orion")
    orion_bin = info.get("orion_bin", "/data/current-package/bin/orion")
...
# Re-export from orion_constants — canonical definition lives there.
from orion_constants import get_orion_home, get_process_orion_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_orion_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_orion_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_orion_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ORION_UID / ORION_GID env vars set by Docker deployments.

    Docker containers running Orion commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ORION_HOME once, but
    subdirectories created at runtime by ``ensure_orion_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ORION_UID", "").strip()
    gid_str = os.environ.get("ORION_GID", "").strip()
    try:
...
def _chown_to_orion_uid(path) -> None:
    """Chown ``path`` to ``ORION_UID:ORION_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_orion_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_orion_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the orion group can
    share state with the gateway service.

    The mode can be overridden via the ORION_HOME_MODE environment variable
    (e.g. ORION_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ORION_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ORION_UID``/``ORION_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ORION_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_orion_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Orion runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ORION_CONTAINER") or os.environ.get("ORION_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ORION_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ORION_HOME, upgrading legacy empty templates.
...
# ensure_orion_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_ORION_HOME_ENSURED: set = set()


def ensure_orion_home():
    """Ensure ~/.orion directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``ORION_HOME`` we only re-run
    ...
    ``get_orion_home()`` and therefore re-run for the new path.
    """
    home = get_orion_home()
    key = str(home)

    if key in _ORION_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``orion profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old ORION_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_orion_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _ORION_HOME_ENSURED.add(key)


def _ensure_orion_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ORION_HOME {home} does not exist."
        )
...
from orion_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `orion config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.orion.config`` entries, then checks
    ...
    try:
        ...
        # should never break `orion update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Orion' own config writer has historically emitted it
    ...
    # Orion (``_save_discovered_models_to_config``), not hand-curated.
    # Older Orion versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Orion versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``orion_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (orion_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (orion_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (orion_cli/setup.py)
    "known_plugin_toolsets", # written/read by orion_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (orion_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'orion setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Orion won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, orion send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'orion doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "orion update / orion -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from orion_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ orion config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in orion_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from orion_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: orion config set <key> <value>")
    ...
    # `orion update`, but only the version bump is persisted.
    ...
    # metadata.orion.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.orion/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_orion_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``OrionCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``orion config set``
    ...
    backends) — while display paths (``orion status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the ORION_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _orion_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in orion_cli.personality.
from orion_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`orion_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from orion_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from orion_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``ORION_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`orion_cli.personality` (single owner).
    """
    from orion_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.orion/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_orion_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.orion/config.yaml.
    ...
    (which change ``ORION_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Orion host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/orion/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_orion_dotenv()
...
#   openai-codex (OAuth — orion auth) — OpenAI Codex
#   nous         (OAuth — orion auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.orion/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Orion writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.orion/.env.
    ...
    interactive menu render (`orion tools`, `orion setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `orion tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.orion/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.orion/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``ORION_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"ORION_CUSTOM_{slug}_API_KEY" if slug else "ORION_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.orion/.env and os.environ.
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from orion_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.orion/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Orion — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Orion vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.orion/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.orion/.env`` over ``os.environ``.

    Used for Orion-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Orion Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from orion_cli.auth import get_anthropic_key
    ...
    # Warn on stale ORION_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ORION_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ORION_MAX_ITERATIONS={_env_ghost} "
                f"(run 'orion doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from orion_cli.personality import active_personality_name
    ...
    print(color("  orion config edit     # Edit config file", Colors.DIM))
    print(color("  orion config set <key> <value>", Colors.DIM))
    print(color("  orion setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``ORION_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("ORION_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `orion cron list`, then pin the "
        "intended values with `orion cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (orion_cli/plugins_cmd.py, orion_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `orion config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``orion config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``orion config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``orion config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (orion config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   orion config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   orion config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``orion config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``orion config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    orion config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    orion config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `orion config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_orion_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `orion config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `orion config unset <KEY>` fully removes
        # the provider instead of leaving it resurrectable (#51071 family).
        from orion_cli.credential_lifecycle import remove_provider_env_credential
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: orion config get <key> [--json]")
            print()
            print("Examples:")
            print("  orion config get model")
            print("  orion config get terminal.backend")
            print("  orion config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: orion config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  orion config set model anthropic/claude-sonnet-4")
            print("  orion config set terminal.backend docker")
            print("  orion config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: orion config unset <key>")
            print()
            print("Examples:")
            print("  orion config unset model")
            print("  orion config unset terminal.backend")
            print("  orion config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'orion config migrate' to add them")
        ...
        print("  orion config           Show current configuration")
        print("  orion config edit      Open config in editor")
        print("  orion config get <key>          Print a resolved config value")
        print("  orion config set <key> <value>   Set a config value")
        print("  orion config unset <key>        Remove a config value")
        print("  orion config check     Check for missing/outdated config")
        print("  orion config migrate   Update config with new options")
        print("  orion config path      Show config file path")
        print("  orion config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``orion config`` UI so users

"""
Gateway subcommand for orion CLI.

Handles: orion gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_orion_home is imported lazily at call sites to avoid ImportError
# when orion_constants is cached from a pre-update version during `orion update`.
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``ai.orion.gateway*`` LaunchAgent — the update path needs the whole
    ...
    systemd branch has always been fleet-wide (``orion-gateway*``) and is
    unaffected.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    # systemd always lists every orion-gateway* unit regardless of scope.
    ...
    if is_macos():
        ...
        if all_profiles:
            ...
            # mirrors the systemd branch's ``orion-gateway*`` pattern above.
        ...
        if all_profiles:
            ...
            # ``launchctl list`` prefix scan also catches ai.orion.gateway*
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "orion CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `orion update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``orion gateway status`` from falsely counting the ``orion`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``orion gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_orion_home().resolve())
    # Forward slashes on both sides of the ORION_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"orion_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ORION_HOME
        ...
        # ORION_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "orion_home=" in command_lc
            and f"orion_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``orion update`` hung exactly there
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``orion update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Orion profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Orion profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "orion_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m orion_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``orion update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Orion's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose ORION_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``orion update`` users
    # with no running gateway until they re-invoke ``orion gateway``
    ...
    # interpreter and captures the stable cwd + env overlay (ORION_HOME,
    ...
    if sys.platform == "win32":
        try:
            from orion_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / ORION_HOME) the windowless
        # base interpreter needs to import orion_cli.  Empty on POSIX, where
...
def _orion_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``ORION_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("ORION_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_orion_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ORION_HOME``.

    Under ``sudo``, ``ORION_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_orion_home` falls back to ``/root/.orion`` — the wrong
    profile. The unit file pins ``ORION_HOME`` for the actual gateway
    ...
    unit_home = (_orion_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("ORION_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ORION_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ORION_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}orion gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}orion gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from orion_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: orion gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `orion gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``orion gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from orion_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (orion_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Orion_Gateway /
            # Orion_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `orion gateway install` deployment.
            from orion_cli.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "Orion_Gateway"
    ...
    try:
        ...
        # whole ai.orion.gateway* fleet — not just the current profile's
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ORION_HOME-scoped).
...
from orion_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Orion Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``orion gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ORION_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ORION_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from orion_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "orion-gateway"
SERVICE_DESCRIPTION = "Orion Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ORION_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ORION_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from orion_constants import get_default_orion_root

    home = get_orion_home().resolve()
    default = get_default_orion_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ORION_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(orion_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when ORION_HOME is a named profile.

    For ``~/.orion/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        orion_home: Optional explicit ORION_HOME path. Defaults to the current
            ``get_orion_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Orion root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_orion_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from orion_constants import get_default_orion_root

    home = Path(orion_home or str(get_orion_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_orion_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(orion_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".orion"
    try:
        Path(orion_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(orion_home, default_root=target_root)
    except ValueError:
        return _profile_arg(orion_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this ORION_HOME.

    Default ``~/.orion`` returns ``orion-gateway`` (backward compatible).
    Profile ``~/.orion/profiles/coder`` returns ``orion-gateway-coder``.
    Any other ORION_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Orion installs that predate the
# orion-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (orion-gateway-*.service) and unrelated third-party
# "orion" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("orion.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "orion_cli.main gateway",
    "orion_cli/main.py gateway",
    "gateway/run.py",
    " orion gateway ",
    "/orion gateway ",
)
...
def _find_legacy_orion_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Orion gateway units.

    Detects unit files installed by older Orion versions that used a
    different service name (e.g. ``orion.service`` before the rename to
    ``orion-gateway.service``). When both a legacy unit and the current
    ``orion-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``orion-gateway-coder.service`` and unrelated third-party
      ``orion-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``orion.service`` running an unrelated
...
def has_legacy_orion_units() -> bool:
    """Return True when any legacy Orion gateway unit files exist."""
    return bool(_find_legacy_orion_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Orion gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_orion_units()
    if not legacy:
        return
    print_warning("Legacy Orion gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current orion-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    orion gateway migrate-legacy")


def remove_legacy_orion_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Orion gateway unit files.

    Iterates over whatever ``_find_legacy_orion_units()`` returns — which is
    ...
    legacy = _find_legacy_orion_units()
    if not legacy:
        print("No legacy Orion gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Orion gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: orion gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo orion gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    orion gateway uninstall")
    print_info("    sudo orion gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: orion gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``orion setup`` (end of wizard)
    and ``orion import``: if this host supports a service manager and no
    ...
    ``orion gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   orion gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: orion gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: orion gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Orion often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.orion`` → ``ai.orion.gateway.plist`` (backward compatible).
    Profile ``~/.orion/profiles/coder`` → ``ai.orion.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.orion.gateway-{suffix}" if suffix else "ai.orion.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    ``get_default_orion_root()``), NOT by globbing ``~/Library/LaunchAgents``:
    the LaunchAgents directory is shared per-user, so a sandboxed
    ``ORION_HOME`` (tests, capture sandboxes, side-by-side installs) must
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.orion.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.orion.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # orion_constants cached from before this symbol existed. See
            # _reload_orion_constants() in orion_cli/managed_uv.py.
            from orion_cli.managed_uv import _reload_orion_constants

            venv_python_path = _reload_orion_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.orion/orion-agent  -> /home/alice/.orion/orion-agent
      /opt/orion                 -> /opt/orion  (kept as-is)
...
def _orion_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ORION_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_orion_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.orion                    → /home/alice/.orion
      /root/.orion/profiles/coder     → /home/alice/.orion/profiles/coder
      /opt/custom-orion               → /opt/custom-orion  (kept as-is)
    """
    current_orion_raw = os.environ.get("ORION_HOME", "").strip()
    current_orion = (
        Path(current_orion_raw).expanduser()
        if current_orion_raw
        else get_orion_home()
    )
    ...
    # different ORION_HOME into the generated service unit.
    current_default = Path.home() / ".orion"
    target_default = Path(target_home_dir) / ".orion"

    # Default ~/.orion → remap to target user's default
    if current_orion == current_default:
        return str(target_default)

    # Profile or subdir of ~/.orion → preserve the relative structure
    try:
        relative = current_orion.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.orion) — keep as-is
        return str(current_orion)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    orion_node = orion_home / "node" / "bin"
    if _is_dir(orion_node):
        candidates.append(str(orion_node))
    orion_nm = orion_home / "node_modules" / ".bin"
    if _is_dir(orion_nm):
        candidates.append(str(orion_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m orion_cli.main``, so module
    ...
    dir, or a clone that ``orion update`` later relocates/removes — the path
    ...
    ``ORION_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ORION_HOME
...
def _systemd_watchdog_seconds(orion_home: str | Path | None = None) -> int:
    ...
    if orion_home is not None:
        ...
        override_token = set_orion_home_override(orion_home)
        reset_home_override = reset_orion_home_override
    try:
...
def _systemd_watchdog_service_fields(
    orion_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(orion_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], orion_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Orion-managed Node under ``$ORION_HOME/node`` goes first when it
    ...
    *orion_root* is the Orion home the unit will run against. System units
    ...
    """
    from orion_constants import (
        orion_managed_node_tree_present,
        iter_orion_node_dirs,
    )

    managed_node_present = orion_managed_node_tree_present(orion_root)
    for directory in iter_orion_node_dirs(orion_root) if managed_node_present else ():
        entry = str(directory)
        try:
    ...
    # target Orion home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Orion home is known — probing here would stat the calling
    ...
    if system:
        ...
        orion_home = _orion_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            orion_home
        )
        profile_arg = _profile_arg_for_target_user(orion_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ORION_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(orion_home) if orion_home else _remap_path_for_user(working_dir, home_dir)
        venv_dir = _remap_path_for_user(venv_dir, home_dir)
        path_entries = [_remap_path_for_user(p, home_dir) for p in path_entries]
        # Managed Node for the TARGET user's tree (see the skip above): probe
        # the remapped orion_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(orion_home) if orion_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m orion_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="ORION_HOME={orion_home}"
Restart=always
RestartSec=5
...
"""
    ...
    orion_home = str(get_orion_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        orion_home
    )
    profile_arg = _profile_arg(orion_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m orion_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="ORION_HOME={orion_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned ORION_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo orion gateway … --system``, ORION_HOME is often stripped
    # and falls back to ``/root/.orion``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / ORION_HOME comparisons
    ...
    _sync_orion_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ORION_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ORION_HOME
    ...
    exported a throwaway ``ORION_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``orion gateway restart``
    with ``ORION_HOME=/tmp/orion-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ORION_HOME=..."`` lines and launchd
    ``<key>ORION_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ORION_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ORION_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ORION_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: ORION_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ORION_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single ORION_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only ORION_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ORION_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../orion_test`` path into
    # ``Environment="ORION_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/orion_test"' in new_unit
        or "/orion_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # ORION_HOME into the unit (manual E2E homes like /tmp/orion-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Orion install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo orion gateway uninstall --system")
    print_info("         orion gateway install")
    print_info("         orion gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("ORION_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("ORION_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Offer to remove legacy units (orion.service from pre-rename installs)
    # before installing the new orion-gateway.service. If both remain, they
    ...
    if has_legacy_orion_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_orion_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin ORION_HOME; adopt it before any
    ...
    # ``sudo orion gateway install --system --force`` would bake /root/.orion
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}orion gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}orion gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}orion gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # ORION_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `orion gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `orion gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}orion gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}orion gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}orion gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}orion gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.orion.gateway-{suffix}" if suffix else "ai.orion.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``orion gateway start/stop/status`` call).
...
#      background process (the `nohup orion gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_orion_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_orion_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m orion_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ORION_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "orion_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``orion update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup orion gateway run --replace`
    ...
    working.
    """
    from orion_cli._subprocess_compat import windows_detach_popen_kwargs
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup orion gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `orion gateway start`)
    ...
    nofile_block = ""
    try:
        from orion_cli.resource_limits import configured_nofile_soft_limit
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.orion/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See orion-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_orion_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Orion install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  orion gateway status             # Check status")
    from orion_constants import display_orion_home as _dhh
...
def launchd_stop():
    ...
    # `orion gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `orion update`
...
def launchd_status(deep: bool = False):
    ...
    # Orion PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Orion PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Orion install")
    else:
        print("⚠ Service definition is stale relative to the current Orion install")
        print("  Run: orion gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: orion gateway stop")
            else:
    ...
    if deep:
        log_file = get_orion_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``ORION_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``orion gateway run`` is always a misconfiguration, so we
    ...
    try:
        ...
        from orion_cli.profiles import normalize_profile_name
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``orion gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``orion gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other ORION_HOME roots.
    """
    ...
    print("  Use 'orion gateway restart' to replace it,")
    print("  or 'orion gateway stop' first.")
    print("  Or use 'orion gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ORION_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Orion gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'orion' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/orion/docker/entrypoint.sh before the Orion command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ORION_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set ORION_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `orion gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ORION_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("ORION_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `orion gateway restart` which already
    ...
    # `orion gateway start/restart` — leaving the gateway vulnerable to
    ...
    # ORION_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ORION_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``ORION_GATEWAY_MAX_STARTS`` /
    # ``ORION_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("ORION_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("ORION_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``orion gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. orion) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Orion delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Orion connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: orion pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Orion will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``orion setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run orion under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.orion/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_orion_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    from orion_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from orion_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'orion pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: orion pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'orion gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Orion will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Orion will store the returned account_id/token in ~/.orion/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `orion gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_orion_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `orion pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `orion pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "OrionAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``orion_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from orion_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.orion/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.orion/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: orion gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: orion gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: orion gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo orion gateway install --system"
                        )
                    print_info("  Or run in foreground:  orion gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: orion gateway run")
                print_info(
                    "  For persistence:   tmux new -s orion 'orion gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: orion gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup orion gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'orion gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`orion_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from orion_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ORION_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``orion gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``orion gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``orion gateway run`` are unaffected.
      2. ``ORION_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``orion gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``ORION_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ORION_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("ORION_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${ORION_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if ORION_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ORION_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-orion service's
    # pattern in docker/s6-rc.d/main-orion/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: orion gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: orion gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s orion 'orion gateway run'"
                )
                print()
            ...
            print()
            print(
                "  orion gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s orion 'orion gateway run'         # persistent via tmux"
            )
            print(
                "  nohup orion gateway run > ~/.orion/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  orion profile create <name>     # creates the s6 service slot")
                print("  orion -p <name> gateway start   # bring it up via s6")
                print("  orion status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: orion gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: orion gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: orion gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  orion profile delete <name>     # tears down the s6 service slot")
                print("  orion -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `orion -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: orion gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_ORION_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `orion gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_ORION_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `orion gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: orion gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  orion gateway install")
                    print("  sudo orion gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  orion gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup orion gateway run > ~/.orion/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s orion 'orion gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  orion gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  orion gateway install  # Install as user service")
                    print(
                        "  sudo orion gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Orion gateway unit files from
        # pre-rename installs (e.g. orion.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_orion_units(interactive=not yes, dry_run=dry_run)

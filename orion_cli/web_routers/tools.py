...
late-binding seam in :mod:`orion_cli.web_deps` (``late`` for callables,
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from orion_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `orion tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `orion computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``orion tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `orion tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``orion tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.orion/.env`` via ``save_env_value`` —
    the same store ``orion tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``orion tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``orion -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ORION_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``orion computer-use permissions grant`` as a background action.

"""
Orion CLI - Unified command-line interface for Orion Agent.

Provides subcommands for:
- orion chat          - Interactive chat (same as ./orion)
- orion gateway       - Run gateway in foreground
- orion gateway start - Start gateway service
- orion gateway stop  - Stop gateway service
- orion setup         - Interactive setup wizard
- orion status        - Show status of all components
- orion cron          - Manage cron jobs
"""
...
def _ensure_utf8():
    ...
    can even start — e.g. `orion setup` on a fresh Pi.
    ...
    orion_cli/stdio.py::configure_windows_stdio() runs later from the entry

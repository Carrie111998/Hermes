"""Durable aggregation and local export for Orion shared metrics."""
...
from orion_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "orion.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_orion_home() / "telemetry" / "shared_metrics"
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Orion processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "orion_version": period_row["orion_version"],

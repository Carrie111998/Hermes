"""Bounded product contract for the first Orion shared-metrics slice."""
...
SCHEMA_KEY = "orion.metrics.schema_version"
SCHEMA_VERSION = "orion.metrics.event.v2"
MODEL_CALL_SCOPE = "orion.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "orion.task_run"
TOOL_CALL_SCOPE = "orion.tool_call"
CLIENT_ACTIVE_MARK = "orion.client.active"
TOOL_APPROVAL_MARK = "orion.tool_approval"
SKILL_LIFECYCLE_MARK = "orion.skill.lifecycle"
SKILL_LOAD_MARK = "orion.skill.load"
SUBSCRIBER_NAME = "orion.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "orion.client.active"
LEGACY_MODEL_CALL_METRIC = "orion.model_call.count"
MODEL_ROUTE_METRIC = "orion.model_route.count"
TASK_STARTED_METRIC = "orion.task_run.started"
TASK_FINISHED_METRIC = "orion.task_run.finished"
TOOL_CALL_METRIC = "orion.tool_call.count"
TOOL_APPROVAL_METRIC = "orion.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "orion.skill.lifecycle.count"
SKILL_LOAD_METRIC = "orion.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Orion installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(orion_version or "").strip()
    ...
    return {
        ...
        "orion_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("orion_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Orion logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("orion.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from orion_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Orion terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Orion registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "orion-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Orion tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Orion."""

"""
Status command for orion CLI.

Shows the status of all Orion Agent components.
"""
...
from orion_cli.auth import AuthError, resolve_provider
from orion_cli.colors import Colors, color
from orion_cli.config import get_env_path, get_env_value, get_orion_home, load_config
...
from orion_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``orion config``'s
...
def _format_relative_ts(ts: float) -> str:
    ...
    from orion_cli.timefmt import relative_time
...
def _estop_status_line():
    """One-line pause banner for `orion status`, or None when not paused.

    Cheap: a single stat on $ORION_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `orion resume` to lift)"


def show_status(args):
    """Show status of all Orion Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Orion Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `orion status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: orion portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: orion model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: orion auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: orion auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: orion model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from orion_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'orion-agent[vercel]')"
    ...
    try:
        from orion_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   orion gateway")
    ...
    jobs_file = get_orion_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_orion_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'orion doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'orion setup' to configure", Colors.DIM))
    print()

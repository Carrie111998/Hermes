"""Install and remove the Linux desktop entry (``orion.desktop``).

``orion desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "orion.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``orion.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``orion desktop``.

    Prefer the real ``orion`` executable (argv[0] or PATH). When Orion
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from orion_cli.relaunch import resolve_orion_bin
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `orion` script with
            ...
            # sys.executable is the interpreter actually running Orion (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "orion_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Orion' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Orion\n"
        "GenericName=Orion Desktop\n"
        "Comment=Launch Orion Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Orion desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "orion"

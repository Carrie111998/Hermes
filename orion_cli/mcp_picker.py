"""MCP picker — interactive `orion mcp picker` (also the default `orion mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``orion mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `orion plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from orion_cli.cli_output import prompt_yes_no
from orion_cli.curses_ui import curses_single_select
from orion_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Orion session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from orion_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `orion mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: orion mcp install <name>    Picker: orion mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Orion is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Orion — run `orion update` "
...
def show_catalog() -> None:
    """`orion mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`orion mcp picker` (and default `orion mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`orion mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `orion mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:

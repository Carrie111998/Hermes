"""
Orion Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``orion desktop`` / ``orion gui``)
     Built inside the agent checkout under ``$ORION_HOME/orion-agent/``:
       ...
       - ``$ORION_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Orion.app`` or ``~/Applications/Orion.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Orion`` (NSIS per-user)
...
the app name ("Orion"), separate from ``$ORION_HOME``:
  - macOS:   ``~/Library/Application Support/Orion``
  - Windows: ``%APPDATA%\\Orion``
  - Linux:   ``$XDG_CONFIG_HOME/Orion`` (default ``~/.config/Orion``)
...
``orion uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(orion_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return orion_home / "orion-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Orion"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Orion"
...
def source_built_gui_artifacts(orion_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``orion desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``orion-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(orion_home)
    ...
    return [
        ...
        orion_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Orion" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Orion).
            local_base / "Programs" / "Orion",
            # Older / alternate layout some builds used.
            local_base / "orion-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Orion")
    else:
        ...
        # ``uninstall_gui``.
        from orion_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `orion desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Orion.desktop",
        ]
    return paths


def agent_is_installed(orion_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ORION_HOME.
    ...
    if (agent_root / "orion_cli").is_dir():
        return True
...
def gui_is_installed(orion_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(orion_home):
        if p.exists():
            return True
...
def gui_install_summary(orion_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = orion_home if orion_home is not None else get_orion_home()
...
def uninstall_gui(orion_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``orion-agent/orion_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ORION_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Orion disappears from the launcher.
        try:
            from orion_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove orion' or "
            "'sudo dnf remove orion'). AppImage builds are a single file you "

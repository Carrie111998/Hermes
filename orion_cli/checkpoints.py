"""`orion checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.orion/checkpoints/``.  Actions:

    orion checkpoints               # same as `status`
    orion checkpoints status        # total size, project count, breakdown
    orion checkpoints list          # per-project checkpoint counts + workdir
    orion checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    orion checkpoints clear [-f]    # nuke the entire base (asks first)
    orion checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    orion checkpoints
    orion checkpoints prune --retention-days 3 --max-size-mb 200
    orion checkpoints clear -f
...
from orion_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: orion checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``orion checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `orion checkpoints` → status

...
Usage:
    orion                     # Interactive chat (default)
    orion chat                # Interactive chat
    orion gateway             # Run gateway in foreground
    orion gateway start       # Start gateway as service
    orion gateway stop        # Stop gateway service
    orion gateway status      # Show gateway status
    orion gateway install     # Install gateway service
    orion gateway uninstall   # Uninstall gateway service
    ...
    orion cron list           # List cron jobs
    orion cron status         # Check if cron scheduler is running
    orion doctor              # Check configuration and dependencies
    orion honcho setup                    # Configure Honcho AI memory integration
    orion honcho status                   # Show Honcho config and connection status
    orion honcho sessions                 # List directory → session name mappings
    orion honcho map <name>               # Map current directory to a session name
    orion honcho peer                     # Show peer names and dialectic settings
    orion honcho peer --user NAME         # Set user peer name
    orion honcho peer --ai NAME           # Set AI peer name
    orion honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    orion honcho mode                     # Show current memory mode
    orion honcho mode [hybrid|honcho|local]  # Set memory mode
    orion honcho tokens                   # Show token budget settings
    orion honcho tokens --context N       # Set session.context() token cap
    orion honcho tokens --dialectic N     # Set dialectic result char cap
    orion honcho identity                 # Show AI peer identity representation
    orion honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    orion honcho migrate                  # Step-by-step migration guide: OpenClaw native → Orion + Honcho
    orion --version           Show version and update status
    orion update              Update to latest version
    orion uninstall           Uninstall Orion Agent
    orion acp                 Run as an ACP server for editor integration
    orion sessions browse     Interactive session picker with search

    orion claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: orion_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``orion_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``orion update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``orion_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, orion crashes on import and the user can't run
# ``orion update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
from orion_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python orion_cli/main.py`` (script
# mode — sys.path[0] is orion_cli/, not the repo root) can import the
# canonical helpers; everything else lives in orion_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from orion_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``orion update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from orion_cli.env_loader import ...`` / ``from orion_cli.config import
...
# package directory, so if IT can't import, nothing else in orion_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from orion_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from orion_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'orion' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``orion tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``orion.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("orion")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"orion", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"orion")
        # Windows: the .exe name is already ``orion.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* orion_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".orion", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``ORION_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``orion … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("ORION_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``ORION_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ORION_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`orion --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(orion_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(orion_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``orion --version`` before config/logging imports."""
...
from orion_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from orion_cli.subcommands.cron import build_cron_parser
from orion_cli.subcommands.sync import build_sync_parser
...
from orion_cli.subcommands.pause import build_pause_parser
...
from orion_cli.subcommands.verify import build_verify_parser
...
from orion_cli.subcommands.skin import build_skin_parser
from orion_cli.subcommands.console import build_console_parser
...
from orion_cli.subcommands.monitoring import build_monitoring_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (orion tools, orion setup, orion model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'orion {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any orion module import.
#
# Many modules cache ORION_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ORION_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.orion/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ORION_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `orion mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Orion' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo orion -p <name>` against the invoking user's home.
        ...
        candidate = home / ".orion" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`orion chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors orion_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ORION_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.orion/profiles/coder or
    # /opt/data/profiles/coder).  If ORION_HOME points to the orion root
    # instead (e.g. systemd hardcodes ORION_HOME=/root/.orion), we must
    # still read active_profile — the user may have switched profiles via
    # `orion profile use` and the gateway should honour that choice.
    # See issue #22502.
    orion_home_env = os.environ.get("ORION_HOME", "")
    if profile_name is None and orion_home_env:
        if Path(orion_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the orion root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ORION_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``orion gateway run`` to mean
    # "the root ORION_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ORION_S6_SUPERVISED_CHILD"):
        try:
            from orion_constants import get_default_orion_root

            active_path = get_default_orion_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ORION_HOME
    if profile_name is not None:
        try:
            from orion_cli.profiles import resolve_profile_env

            orion_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            orion_home = _resolve_sudo_user_profile_env(profile_name)
            if not orion_home:
            ...
            # A bug in profiles.py must NEVER prevent orion from starting
            print(
...
# Windows launcher self-heal — the ``orion`` command users run is a COPY of
# the venv console script, staged into the managed binary dir (the default
# Orion root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``orion update``'s autostash
# (``git stash push --include-untracked``) swept them off disk; with the
# desktop updater's ``--keep-stash`` nothing restored them and ``orion``
...
# ``python -m orion_cli.main``. Costs a few stat calls when healthy; gates
# fail toward inaction so source checkouts are untouched. Sits AFTER the
# profile override on purpose — no orion module may be imported before
# profiles resolve. The launcher dir itself is per-machine (the helper
# anchors on the DEFAULT root, not ORION_HOME), so profile sessions heal
# the same shared dir.
if sys.platform == "win32":
    try:
        from orion_cli import _install_repair as _install_repair_mod
...
# Bridge security.redact_secrets from config.yaml → ORION_REDACT_SECRETS env
# var BEFORE orion_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves orion_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from orion_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_orion_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "ORION_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ORION_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `orion` subcommands
...
try:
    from orion_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from orion_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if orion_constants not importable yet
...
# Provider model-selection wizard flows extracted to orion_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (orion_cli.main._model_flow_*) keep resolving unchanged.
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_orion_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ORION_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ORION_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`orion_cli.timefmt` so lightweight consumers don't have to
...
def _has_any_provider_configured() -> bool:
    ...
    from orion_cli.config import get_env_path, get_orion_home, load_config
    from orion_cli.auth import get_auth_status

    # Determine whether Orion itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_orion_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_orion_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Orion has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Orion to use their tokens.
    if _has_orion_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``orion -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, orion_bin
        cli_args: the original CLI arguments (everything after 'orion')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    orion_bin = container_info["orion_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo orion {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo orion {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'orion sessions list' to see available sessions, or "
            ...
            try:
                from orion_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _print_tui_exit_summary(
    ...
    print(f"  orion --tui --resume {target}")
    if title:
        print(f'  orion --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @orion/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@orion" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/orion-ink/package.json",
    "packages/orion-ink/index.js",
    "packages/orion-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ORION_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ORION_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ORION_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ORION_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ORION_HOME": orion_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(orion_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(orion_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if orion_cli_dir is None:
        orion_cli_dir = Path(__file__).parent
    bundled = orion_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``orion update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ORION_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Orion checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `orion update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Orion checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `orion --tui`\n"
        "If the checkout is still inconsistent, run `orion update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ORION_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ORION_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $ORION_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Orion installed,
    ...
    ext_dir = os.environ.get("ORION_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ORION_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ORION_TUI_DIR (e.g. `unset ORION_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # orion_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_orion_node_path()),
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Orion-managed
            ...
            from orion_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @orion/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @orion/ink resolves through
        # packages/orion-ink/dist/entry-exports.js. If that dist bundle is
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from orion_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("ORION_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["ORION_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("ORION_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["ORION_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("ORION_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["ORION_CWD"]) / python_path
...
def _launch_tui(
    ...
    # TUI child is a orion process: propagate the profile-home contract via
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="orion-tui-active-session-", suffix=".json"
    )
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ORION_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["ORION_MODEL"] = model
        env["ORION_INFERENCE_MODEL"] = model
    if provider:
        env["ORION_TUI_PROVIDER"] = provider
        env["ORION_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ORION_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ORION_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ORION_TUI_SKILLS"] = value
    if query:
        env["ORION_TUI_QUERY"] = query
    if image:
        env["ORION_TUI_IMAGE"] = image
    if checkpoints:
        env["ORION_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ORION_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ORION_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ORION_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ORION_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ORION_ACCEPT_HOOKS"] = "1"
    ...
    # ORION_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `orion --tui` try to
    ...
    # still set ORION_TUI_RESUME themselves.
    env.pop("ORION_TUI_RESUME", None)
    if resume_session_id:
        env["ORION_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `orion update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ORION_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``orion kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``orion kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ORION_KANBAN_BOARD"):
        return
    try:
        from orion_cli.kanban_db import get_current_board

        os.environ["ORION_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.orion/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Orion — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Orion still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``orion … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'orion sessions list' to see available sessions.")
            sys.exit(1)
    ...
    # Code / Codex CLI) and resume the newly created Orion session.
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: orion --resume {_imported_id})")
    ...
    try:
        ...
        from orion_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'orion doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Orion isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  orion setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'orion setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from orion_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.orion/skills the banner
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from orion_cli.config import get_orion_home
            skills_dir = Path(get_orion_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import orion_cli.banner as _banner_mod
    ...
    # user's ~/.orion/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["ORION_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from orion_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from orion_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Orion?")
        print()
    ...
    # has no creds.json.  Every subsequent `orion gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: orion gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `orion gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  orion gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Orion Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  orion gateway")
        ...
        print()
        print("  Or install as a service: orion gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'orion whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``orion whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``orion_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from orion_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``orion model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("ORION_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'orion model' for "
                "available providers, or run 'orion doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in orion_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``orion model``
    ...
    # matching against orion_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    # a leftover OPENAI_BASE_URL in ~/.orion/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.orion/.env if the active provider is not 'custom'.
    ...
    """
    from orion_cli.config import get_env_value, save_env_value, load_config
...
#
# Orion uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `orion model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `orion model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`orion_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Orion only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``orion model`` flow, then route aux tasks
    to them here.
    """
    from orion_cli.config import load_config
    from orion_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Orion URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# downstream call sites read `orion_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `orion` invocation — including
# fast paths like `orion --version` and slash-command dispatch that never
...
# surface so argparse wiring and test monkeypatches on orion_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `orion` invocation, including fast paths like `orion --version`
...
def _self():
    ...
    keeps monkeypatches on orion_cli.main.<name> working, exactly like a
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``orion setup`` / ``orion model``.
    ...
    recover from a malformed paste without editing ``~/.orion/.env`` by hand.
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `orion setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Orion will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Orion CLI with a provider."""
    from orion_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from orion_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from orion_cli.auth_commands import auth_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.orion/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `orion sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set ORION_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "ORION_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="orion sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="orion sync now")
...
def cmd_webhook(args):
    ...
    from orion_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``orion slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: orion slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `orion slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from orion_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from orion_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from orion_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from orion_cli.hooks import hooks_command
...
def cmd_security(args):
    """Dispatch `orion security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from orion_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `orion approvals <subcmd>`."""
...
def cmd_skin(args):
    ...
    from orion_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Orion home directory to a zip file."""
    if getattr(args, "quick", False):
        from orion_cli.backup import run_quick_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `orion --version` pre-import fast path (the `version` subcommand
...
def cmd_uninstall(args):
    """Uninstall Orion Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from orion_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from orion_cli.uninstall import run_gui_uninstall
    ...
    from orion_cli.uninstall import run_uninstall
...
# Update pipeline lives in orion_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on orion_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in ORION_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``orion update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``orion update``
    ...
    ``orion`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``orion update``
    ...
    ``orion_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "orion_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $ORION_HOME."""
    from orion_constants import get_orion_home
    return get_orion_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``orion`` launcher present but ``orion_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the orion venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — orion venv python3, no nix-shell overhead
    ...
    # a orion venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``orion update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Orion-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_orion_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "orion_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``orion web``).
    ...
    # build path usable on a stock `py -m orion_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_orion_node_path())
    ...
    # as `orion update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (orion web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``orion update`` can unconditionally call ``orion desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``orion desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ORION_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ORION_HOME."""
    from orion_constants import get_orion_home
    return get_orion_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Orion.app/Contents/MacOS/Orion → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``orion desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``orion update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Orion.app/Contents/MacOS/Orion"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Orion.exe",
            release_dir / "win-ia32-unpacked" / "Orion.exe",
            release_dir / "win-arm64-unpacked" / "Orion.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "orion",
            release_dir / "linux-unpacked" / "Orion",
            release_dir / "linux-arm64-unpacked" / "orion",
            release_dir / "linux-arm64-unpacked" / "Orion",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Orion.exe
...
# Orion.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Orion.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Orion.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 orion-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Orion.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Orion.exe from backup.")
        print("    Your existing version was kept and still works. Run `orion desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `orion desktop --force-build` to rebuild, or re-run the Orion")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Orion'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_orion_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Orion.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Orion.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Orion.exe: Access is
    ...
    an unrelated "Orion" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Orion.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Orion is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Orion.app/Contents/MacOS/Orion  ->  app bundle = .../Orion.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``orion desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Orion Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Orion Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `orion desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the ORION_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Orion Desktop (Linux only, best-effort).
    ...
    shell. ``orion uninstall --gui`` removes it.
    """
    try:
        from orion_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from orion_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["ORION_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ORION_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "orion_root", None):
        env["ORION_DESKTOP_ORION_ROOT"] = str(Path(args.orion_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ORION_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["ORION_DESKTOP_CWD"] = os.getcwd()
    ...
    # `ORION_DESKTOP_DISABLE_GPU=... orion desktop` keeps working.
    config_electron_flags, config_disable_gpu, config_password_store = _desktop_launch_options()
    if config_disable_gpu != "auto" and "ORION_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ORION_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `ORION_DESKTOP_PASSWORD_STORE=... orion desktop` keeps working.
    if sys.platform == "linux" and "ORION_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["ORION_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  orion gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Orion-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → orion-setup →
            # orion update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `orion update` path).
            nixos_env = with_orion_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Orion.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Orion.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Orion.exe, close any")
                    print("  running Orion desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> orion desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Orion is
                ...
                # success on a Orion.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Orion shows up
    ...
    if source_mode:
        print("→ Launching Orion Desktop from source build...")
    ...
    print(f"→ Launching packaged Orion Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers live in orion_cli/dashboard_procs.py
...
# orion_cli.main.<name> keep resolving unchanged.
...
_DASHBOARD_SYSTEMD_UNIT = "orion-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Orion installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``orion-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/orion-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but orion launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``orion update``.
    ...
    log_path = get_orion_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `orion update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $ORION_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``orion update``.
    ...
    protocol stream (``orion acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a normal ``orion.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_orion_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from orion.exe; applying package-only first aid, "
    ...
    try:
        from orion_cli import _install_repair as _ir
        ...
        if self_locked:
            print(
                "  Orion is still running from the launcher that needs "
                "replacing. Close other Orion windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\orion.exe`` is a launcher that runs the interpreter with
    ...
    ``orion ...`` command holds its own shim, and an editable install run
    ...
    ``orion.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _orion_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_orion_launcher_locked() -> bool:
    """True when a venv ``orion*.exe`` shim is this process or an ancestor.
...
_UPDATE_REEXEC_ENV = "ORION_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``orion update``, so the whole remaining flow — the
    ...
    cmd = [str(python_exe), "-m", "orion_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from orion_constants import project_venv_dir
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``orion update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    from orion_constants import project_venv_dir, venv_bin_dir
...
def _orion_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_orion_exe(
    ...
    when ``orion update`` runs, ``orion.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``orion.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next orion invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Orion Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Orion Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``orion.exe`` re-runs itself under the venv Python before
    ...
    for shim in _orion_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Orion Desktop, exit other `orion` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `orion update`."
        )
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Orion versions queued for our shims.

    Orion used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _orion_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_orion_exe`` if uv didn't write replacements.

    This is the safety-critical direction. A failed *quarantine* only aborts an
    update; a failed *restore* leaves the install with no ``orion`` on PATH,
...
class ShimQuarantineError(RuntimeError):
    """A live ``orion*.exe`` shim could not be renamed aside (#87331).
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``orion.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``orion.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``orion`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_orion_exe(scripts_dir, failed_out=failed)
    ...
    try:
        ...
        # quarantined aside and `orion` missing from PATH after a green
...
def _quarantine_stamp_ms(stale: Path) -> int | None:
    ...
    :func:`_quarantine_running_orion_exe`. We neither rescue nor delete those:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``orion.exe.old.*`` from updates.

    Called early on every orion invocation. Two cases the old unconditional
    ``unlink()`` got wrong, both ending with ``orion`` gone from PATH:

    1. **Orphan rescue.** If ``orion.exe`` is missing while
       ``orion.exe.old.*`` is present, that .old file is the ONLY surviving
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``orion update`` does exactly this). Leave anything inside the grace
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``orion.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``orion.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `orion update`:")
    print(
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    """
    from orion_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``orion.exe`` / ``orion-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_orion_exe`` for the rationale.
    ...
    if (
        ...
        # Quarantining the wrong dir means the running orion.exe stays locked
    ...
    # hours later inside ``orion update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``orion.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``orion.exe`` shim is locked during ``orion update``, or when uv/distlib
    skips a launcher write. The symptom is ``orion-agent.exe`` and
    ``orion-acp.exe`` present but ``orion.exe`` missing, so ``orion`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `orion update --force` after "
            "closing other orion processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m orion_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``orion update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``orion.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``orion`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `orion update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `orion update --force` after closing other orion processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``orion update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.orion/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``orion update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``orion update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.orion/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``orion update --gateway``) the update is already
    ...
    try:
        ...
        # orion_cli.config.get_orion_home to simulate setup failure.
        from orion_cli.config import get_orion_home as _get_orion_home

        logs_dir = _get_orion_home() / "logs"
...
def cmd_update(args):
    """Update Orion Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Orion Agent")
        return
    ...
    if getattr(args, "plan", False):
        ...
        # Orion runtime across profiles, its supervisor, and its running
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `orion update --branch=<x>` would actually pull.
    ...
    from orion_cli.update_lock import (
    ...
    try:
        ...
        try:
            from orion_cli.update_receipt import finalize_pending_update_receipt
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``orion -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `orion profile` — show current profile status
        from orion_cli.profiles import format_profile_label

        profile_name = get_active_profile_name()
        dhh = display_orion_home()
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → orion -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.orion)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  orion profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     orion -p {name} chat")
                else:
        ...
        from orion_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from orion_constants import get_orion_home as _hh
        ...
        from orion_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `orion profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → orion -p {name}  ({wrapper})")
        print()
        ...
        from orion_cli.profiles import profile_exists, validate_alias_name
        ...
        from orion_cli.profiles import normalize_profile_name, rename_profile
        ...
        from orion_cli.profiles import export_profile
        ...
        from orion_cli.profiles import import_profile
        ...
        import tempfile
        from orion_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="orion_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  orion -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      orion -p {plan.manifest.name} chat")
        ...
        from orion_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `orion profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"orion -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from orion_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("orion_requires"):
            print(f"Requires:     Orion {data['orion_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from orion_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.orion_requires:
        print(f"  Requires: Orion {mf.orion_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No orion dashboard processes running.")
        return 0

    print(f"{len(live)} orion dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``orion dashboard register`` for OAuth.
    ...
    print("    [2] OAuth via Nous Portal (run `orion dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "ORION_DASHBOARD_OAUTH_CLIENT_ID into ~/.orion/.env for you.\n"
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from orion_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.orion/desktop-ssh: a
    # literal "~/.orion/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of ORION_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_orion_home(): a non-default sticky profile (or any ORION_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_orion_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".orion" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``ORION_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``orion dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No orion dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `orion update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with orion serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with ORION_DESKTOP=1 plus
    # ORION_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # ORION_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `orion dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited ORION_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # ORION_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (ORION_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets ORION_SERVE_HEADLESS below.
    if os.environ.get("ORION_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("ORION_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("ORION_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("ORION_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → ORION_HOME points into
    ...
    if (
        ...
        and os.environ.get("ORION_DESKTOP") != "1"
    ):
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: ORION_HOME is explicitly pinned to the
        ...
        # ORION_HOME.  We must resolve the root explicitly instead of just
        # dropping ORION_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ORION_HOME=/opt/data`), so an unset
        # ORION_HOME falls back to $HOME/.orion = /opt/data/.orion — an
        ...
        # update-button guard also misfires).  get_default_orion_root()
        # returns the root for both layouts: ~/.orion for a standard install
        ...
        try:
            from orion_constants import get_default_orion_root
            env["ORION_HOME"] = str(get_default_orion_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ORION_HOME) rather than block the reroute.
    ...
    # the same logs directory as every other Orion surface.
    try:
        from orion_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["ORION_SERVE_HEADLESS"] = "1"
    elif "ORION_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ORION_WEB_DIST"])
            if "ORION_WEB_DIST" in os.environ
            else PROJECT_ROOT / "orion_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom ORION_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "ORION_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # ORION_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["ORION_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ ORION_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset ORION_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads ORION_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["ORION_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from ORION_WEB_DIST: {_dist_root}")
    ...
    from orion_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from orion_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from orion_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from orion_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from orion_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Orion log files."""
    from orion_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Orion command console."""
    from orion_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from orion_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``orion -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``orion_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``orion -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``orion`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``orion <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``orion photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ORION_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that ORION_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from orion_cli.plugins import start_background_plugin_discovery
...
def _try_fast_chat_launch() -> bool:
    ...
    ``orion`` / ``orion -w -s foo --yolo`` / ``orion chat`` don't need the
    ...
    """
    if os.environ.get("ORION_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from orion_cli.config import get_container_exec_info
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("ORION_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ORION_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ORION_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ORION_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `orion --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_orion_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_orion_home()}/memories/\n")
    else:
        from orion_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Orion Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from orion_cli.tools_config import run_post_setup_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: orion-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from orion_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    elif getattr(args, "skills_action", None) in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from orion_cli.skills_hub import skills_command

        skills_command(args)


def _cmd_skills_trust(args):
    """``orion skills trust [path]`` / ``orion skills untrust [path]``.
...
def cmd_plugins(args):
    from orion_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from orion_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``orion-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``ORION_AGENT`` is the Orion-specific marker. setdefault: never
    clobber an outer harness (e.g. Orion running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "orion-agent")
    os.environ.setdefault("ORION_AGENT", "true")


def main():
    """Main entry point for orion CLI."""
    # Cosmetic: make the process show up as 'orion' instead of 'python3.11'
    ...
    # Sweep stale ``orion.exe.old.*`` quarantine files left by previous
    # ``orion update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_orion_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (orion update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``orion update``
    ...
    # ``orion skills install update``) merely defers recovery one launch;
    # under-matching (missing ``orion -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in orion_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from orion_cli.moa_cmd import cmd_moa
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://your-orion-docs.example/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `orion model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory orion -w sessions "
            "accumulate. Never deletes uncommitted tracked changes, unique "
            "unpushed commits, or in-use trees; untracked-only scratch is "
            "archived to ~/.orion/archive/worktree-prune/ before removal. See: "
            "https://your-orion-docs.example/docs/user-guide/cli#worktree-cleanup"
        ),
    )
    ...
    def _dispatch_worktree(_args):
        from orion_cli.worktree_cmd import cmd_worktree
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.orion/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://your-orion-docs.example/docs/user-guide/secrets/"
        ),
    )
    ...
    from orion_cli import secrets_cli as _secrets_cli
    from orion_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `orion proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://your-orion-docs.example/docs/user-guide/egress/iron-proxy"
        ),
    )

    from orion_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``orion proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from orion_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in orion_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in orion_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in orion_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `orion whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in orion_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from orion_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in orion_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in orion_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in orion_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in orion_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in orion_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in orion_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in orion_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Orion gateways)
    # =========================================================================
    from orion_cli.subcommands.peer import build_peer_parser
    ...
    # =========================================================================
    from orion_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from orion_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from orion_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in orion_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in orion_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in orion_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in orion_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in orion_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in orion_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in orion_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in orion_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.orion/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo orion uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from orion_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in orion_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: orion_cli/subcommands/import_agent.py)
    # =========================================================================
    ...
    # =========================================================================
    # config command  (parser built in orion_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in orion_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in orion_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in orion_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in orion_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from orion_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in orion_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``orion --help``, ``orion logs``,
    ...
    try:
        from orion_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Orion shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from orion_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from orion_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in orion_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in orion_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `orion computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `orion tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `orion computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Orion app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from orion_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("ORION_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from orion_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: orion computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: orion computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: orion computer-use install --upgrade")
                return 0
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: orion computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in orion_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <orion home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Orion",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Orion session store "
            "so it can be resumed with 'orion --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in orion_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on orion_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in orion_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in orion_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # NOTE: the `orion version` subcommand was removed — `orion --version`
    # / `-V` now carries the full output including update status.

    # =========================================================================
    # update command  (parser built in orion_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in orion_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in orion_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in orion_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in orion_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Orion-Setup.exe success screen tells users to
    # run `orion desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in orion_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in orion_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in orion_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``orion -c Pokemon Agent Dev`` → ``orion -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'orion -c model' where 'model' is consumed as the
    ...
    # --yolo: set ORION_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (orion hooks list, cron
    ...
    # ``orion egress start`` refusing when credential_source=bitwarden

...
Add, remove, or reorder entries here — both `orion setup` and
`orion` provider-selection will pick up the change automatically.
"""
...
from orion_cli.urllib_security import open_credentialed_url, url_origin
...
_ORION_USER_AGENT = f"orion-cli/{_ORION_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `orion model`
    flow without maintaining a separate static list.
    """
    from orion_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $ORION_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Orion-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ORION_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # "opencode/latest"; we send honest Orion attribution and don't
    ...
    # endpoint expects (see orion_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Orion that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Orion that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_orion_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$ORION_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  orion model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `orion model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `orion model` TUI
...
#
# Some vendors expose several Orion provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `orion model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``orion model``, the
...
# In-repo fallback for the model Orion silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from orion_cli.model_catalog import get_default_model_from_cache
...
# ``orion model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in orion_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``orion auth add openai-codex`` without
    ``orion model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``orion model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    orion-agent is tool-calling-first — every provider path assumes the model
...
# copy, `orion -p`, a cron job, or a freshly booted gateway answers
...
def _reasoning_caps_disk_path() -> Path:
    from orion_constants import get_orion_home
    return get_orion_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _ORION_USER_AGENT}
    try:
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from orion_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — orion-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Orion lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from orion_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return orion-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; orion's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``orion model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from orion_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _ORION_USER_AGENT}
...
def _resolve_static_model_alias(
    ...
    try:
        from orion_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Orion' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``orion_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Orion should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``orion auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.orion/.env``.
    ...
    """
    try:
        from orion_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Orion release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        ...
        # a Orion release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from orion_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Orion release.
...
#
# Cache strategy:
#   - One JSON file at $ORION_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from orion_constants import get_orion_home
    return get_orion_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ORION_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `orion config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ORION_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Orion' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Orion' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from orion_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ORION_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"orion-agent/{_ORION_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=orion"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from orion_constants import get_orion_home
    return get_orion_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `orion moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Orion will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Orion will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Orion
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Orion cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Orion actually ships.  (#46850)
            #

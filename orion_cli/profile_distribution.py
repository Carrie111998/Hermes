"""Profile distributions — shareable, packaged Orion profiles via git.

A distribution is a Orion profile published as a git repository (or
...
* ``orion profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``orion skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``orion profile``, not a parallel tree):

    orion profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "orion-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            orion_requires=str(data.get("orion_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.orion_requires:
            out["orion_requires"] = self.orion_requires
        if self.author:
...
def check_orion_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Orion {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``orion profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Orion profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Orion distribution."
        )

    # Version check up-front so we fail fast
    check_orion_requires(manifest.orion_requires, orion_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.orion).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="orion_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `orion profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`orion profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="orion_dist_update_") as tmp:
        plan = plan_install(

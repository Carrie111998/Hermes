...
def active_session_limit_message(
    ...
    return (
        f"Orion is at the active session limit ({active_count}/{max_sessions})."
        f"{detail} Try again when another session finishes."
    )


def _state_dir() -> Path:
    return Path(get_orion_home()) / "runtime"
...
class ActiveSessionLease:
    ...
    # root ``ORION_HOME`` must release against the same registry even when
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile ORION_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``orion dashboard`` / ``serve``) never trips that

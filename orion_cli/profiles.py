"""
Profile management for multiple isolated Orion instances.

Each profile is a fully independent ORION_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.orion/profiles/<name>/`` by default.

The "default" profile is ``~/.orion`` itself — backward compatible,
zero migration needed.

Usage::

    orion profile create coder          # fresh profile + bundled skills
    orion profile create coder --clone  # also copy config, .env, SOUL.md, skills
    orion profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    orion -p coder chat                 # or via flag
    orion profile use coder             # set as sticky default
    orion profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See orion_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `orion profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `orion
...
# `orion skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Orion infrastructure directories that only the default profile
         (``~/.orion``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_orion_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.orion) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "orion-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when ORION_HOME equals the
...
# into the export. The set below identifies the *known Orion profile
# artifacts* at the root of ORION_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``orion_constants``.
...
_RESERVED_NAMES = frozenset({
    "orion", "default", "test", "tmp", "root", "sudo",
})

# Orion subcommands that cannot be used as profile names/aliases
_ORION_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the orion root, NOT to the current ORION_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ORION_HOME points outside
    ``~/.orion``, profiles live under ``ORION_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_orion_home() / "profiles"


def _get_default_orion_home() -> Path:
    """Return the default (pre-profile) ORION_HOME path.

    In standard deployments this is ``~/.orion``.
    In Docker/custom deployments where ORION_HOME is outside ``~/.orion``
    (e.g. ``/opt/data``), returns ORION_HOME directly.
    """
    from orion_constants import get_default_orion_root
    return get_default_orion_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_orion_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Orion installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ORION_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_orion_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Orion home
    (:func:`orion_constants.get_orion_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, orion subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ORION_SUBCOMMANDS:
        return f"'{canon}' conflicts with a orion subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            orion_exe = shutil.which("orion") or "orion"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(orion_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Orion. If we
    ...
    try:
        ...
        from orion_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `orion doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``orion -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``orion profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``orion -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``orion profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ORION_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Orion config
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, orion_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ORION_HOME. When
    ...
    The returned ``orion_home`` is the path to pass to
    ``set_orion_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_orion_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``orion update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.orion)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.orion/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `orion -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Orion profile.\n"
    ...
    if not soul_path.exists():
        try:
            from orion_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `orion update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`orion profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `orion update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Orion (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `orion profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `orion -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ORION_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``orion profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ORION_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_orion_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Orion *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Orion backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ORION_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `orion -p <canon> profile
    ...
    orion_markers = ("orion_cli.main", "orion-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('orion')` resolution) spawns the
    ...
    # interpreter, not "orion", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _ORION_CONSOLE_SCRIPT_NAMES = frozenset({"orion", "orion-agent", "orion-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Orion process: either an entrypoint marker in argv, a
            # resolved executable named `orion`, or a python interpreter
            # directly exec'ing a `orion`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_orion = (
                any(marker in joined for marker in orion_markers)
                or exe_name == "orion"
                or exe_name.startswith("orion")
            )
            if not is_orion and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: orion, orion-agent,
                # orion-acp) rather than a bare `startswith("orion")` --
                ...
                # "orion-notes.py" or "orion-unrelated-tool" as the shim,
                ...
                is_orion = script_stem in _ORION_CONSOLE_SCRIPT_NAMES
            if not is_orion:
                continue
            ...
            # ...or by ORION_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ORION_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.orion).\n"
            "To remove everything, use: orion uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_orion_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ORION_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    """
    try:
        from orion_cli.service_manager import detect_service_manager
        if detect_service_manager() != "s6":
            return  # host path — silent, no registration needed
        from orion_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ORION_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ORION_HOME")
    try:
        os.environ["ORION_HOME"] = str(profile_dir)
        from orion_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ORION_HOME"] = old_home
        elif "ORION_HOME" in os.environ:
            del os.environ["ORION_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.orion/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: orion profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ORION_HOME.

    Returns ``"default"`` if ORION_HOME is not set or points to ``~/.orion``.
    Returns the profile name if ORION_HOME points into ``~/.orion/profiles/<name>``.
    Returns ``"custom"`` if ORION_HOME is set to an unrecognized path.
    """
    from orion_constants import get_orion_home
    orion_home = get_orion_home()
    resolved = orion_home.resolve()

    default_resolved = _get_default_orion_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ORION_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Orion file the
      user may have lying alongside ORION_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``orion sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``ORION_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.orion itself — its parent is ~/ and its
        # directory name is ".orion", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: orion profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.orion itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.orion). "
            "Specify a different name: orion profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="orion_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"orion_{old_name}"
    legacy_old_host = f"orion.{old_name}"
    new_host = f"orion_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_orion_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("orion_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ORION_HOME path string.

    Called early in the CLI entry point, before any orion modules
    are imported, to set the ORION_HOME environment variable.

    When ORION_HOME is already set, the configured spelling IS the

"""Per-terminal session breadcrumbs for ``orion -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$ORION_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``orion -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_orion_home() / "terminal-sessions"

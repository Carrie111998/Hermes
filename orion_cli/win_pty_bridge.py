"""Windows ConPTY bridge for the `orion dashboard` chat tab.

Drop-in counterpart to ``orion_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``orion_cli.web_server`` consumes: ``spawn``, ``read``,

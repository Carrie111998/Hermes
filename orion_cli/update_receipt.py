...
1. **Update receipt** — a machine-readable JSON record of what one
   ``orion update`` run discovered, did, skipped (and why), written to
   ``<ORION_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``orion_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``orion update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``orion update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_orion_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``orion update`` has many early-termination paths (Windows

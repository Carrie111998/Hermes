"""
orion fallback — manage the fallback provider chain.
...
Subcommands:
  orion fallback [list]   Show the current fallback chain (default when no subcommand)
  orion fallback add      Pick provider + model via the same picker as `orion model`,
                           then append the selection to the chain
  orion fallback remove   Pick an entry to delete from the chain
  orion fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.orion/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `orion model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  orion fallback add")
        print()
        return
    ...
    print("  Docs: https://your-orion-docs.example/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `orion model`, then append the selection to the chain."""
    from orion_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `orion model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `orion fallback list` to view, or `orion fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``orion fallback [subcommand]``."""

"""Unified provider-credential lifecycle across every store Orion reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.orion/.env``                     — the canonical secret store
    2. ``~/.orion/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.orion/config.yaml``              — inline mirrors written by the
...
def _prune_env_pool_entries(env_var: str) -> List[str]:
    ...
    """
    from orion_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `orion auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``orion auth add``.
    """
    from orion_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from orion_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from orion_cli.config import load_env, remove_env_value

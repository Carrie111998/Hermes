"""
MCP Server Management CLI — ``orion mcp`` subcommand.

Implements ``orion mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.orion/config.yaml under the ``mcp_servers`` key.
"""
...
from orion_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_orion_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from orion_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.orion/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``orion mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import OrionTokenStorage
        return OrionTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # orion_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  orion mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  orion mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  orion mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_orion_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: orion mcp test " + name)
        return
    ...
    if choice in {"s", "select"}:
        ...
        from orion_cli.curses_ui import curses_checklist
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_orion_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `orion mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  orion mcp add <name> --url <endpoint>')
        _info('  orion mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``orion mcp login`` and
    ``orion mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `orion mcp remove` + `orion mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `orion mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Orion desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `orion mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``orion mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``orion mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: orion mcp reauth <name>   |   orion mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'orion mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``orion mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `orion plugin`.
        from orion_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("orion mcp                                    Open the catalog picker (default)")
        _info("orion mcp catalog                            List Nous-approved MCPs")
        _info("orion mcp install <name>                     Install a catalog MCP")
        _info("orion mcp serve                              Run as MCP server")
        _info("orion mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("orion mcp add <name> --command <cmd>         Add a stdio server")
        _info("orion mcp add <name> --preset <preset>       Add from a known preset")
        _info("orion mcp remove <name>                      Remove a server")
        _info("orion mcp list                               List configured servers")
        _info("orion mcp test <name>                        Test connection")
        _info("orion mcp configure <name>                   Toggle tools")
        _info("orion mcp login <name>                       Re-authenticate OAuth")
        _info("orion mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()

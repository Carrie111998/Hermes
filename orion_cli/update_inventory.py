...
One read-only pass that answers, BEFORE any mutation: what Orion runtimes
...
`detect_install_method`) — so `orion update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `orion update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `orion serve`; it respawns
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Orion runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "orion update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set) -> str:
    ...
    if pid in service_pids:
        try:
            from orion_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"orion -p {profile} gateway restart"
    return "orion gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        from orion_cli.build_info import get_code_identity
    ...
    try:
        from orion_cli.gateway import _get_service_pids
    ...
    try:
        from orion_cli.gateway import find_profile_gateway_processes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Orion services: none detected — code swap only.")
        return
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.profile in relaunched or r.profile in external:
                ...
                r.profile in unit or (r.profile == "default" and "orion-gateway" in unit)
                for unit in failed_set
            ):
                outcome = "failed"
            elif any(
                r.profile in svc or (r.profile == "default" and "orion-gateway" in svc)
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    print("      orion gateway restart                # active profile")
    print("      orion -p <profile> gateway restart   # named profile")
    return True

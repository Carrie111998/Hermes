"""On-demand supply-chain audit for Orion Agent installs.

Scans three surfaces a Orion user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Orion venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.orion/plugins``
...
def _discover_plugins(orion_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.orion/plugins``.
    ...
    """
    plugins_dir = orion_home / "plugins"
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from orion_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def _discover_components(
    *,
    ...
    home = orion_home or Path(get_orion_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `orion security audit`."""
    home = Path(get_orion_home())
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, orion_home=home
    )

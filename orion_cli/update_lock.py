"""Cross-process mutual exclusion for in-flight Orion updates.

Three different surfaces can start an update of the same install tree:

* ``orion update`` from a terminal,
* the dashboard's Update button (``POST /api/orion/update`` →
  ``_spawn_orion_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``orion-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``orion update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ORION_HOME>/.orion-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``orion update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Orion is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``orion-setup`` binary under ``~/.orion`` is only refreshed by a full
  installer run (``copy_self_to_orion_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Orion is still running") on every
  GUI update forever, with no Orion process actually running.
"""
...
MARKER_NAME = ".orion-update-in-progress"

# Set by an orchestrating updater (the Tauri `orion-setup --update` flow) to
# its own pid before spawning `orion update` as a child stage. The parent
...
HANDOFF_PID_ENV = "ORION_UPDATE_HANDOFF_PID"
...
# to show "Orion is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Orion home (never the context-local profile override):
    the Rust updater resolves ``$ORION_HOME`` or the platform default, and the
    ...
    return get_process_orion_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``orion update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``orion-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Orion update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `orion update` as a stage): we run under ITS claim

...
the tool registry or launching Orion (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``orion-cli`` to the non-existent ``orion``. ``resolve_toolset('orion')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``orion-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``orion`` instead of ``orion-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `orion tools` to reconfigure."
        )
    return warnings

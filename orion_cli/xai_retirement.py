...
Pure logic: walks a Orion config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `orion doctor` and a future `orion migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Orion config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Orion config and return retirement issues.
...
def apply_migration(
    ...
    from orion_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `orion migrate xai` reports nothing to migrate

"""
Doctor command for orion CLI.

Diagnoses issues with Orion Agent setup.
"""
...
_DHH = display_orion_home()  # user-facing display path (e.g. ~/.orion or ~/.orion/profiles/coder)

# Load environment variables from ~/.orion/.env so API key checks work
_env_path = get_env_path()
load_orion_dotenv(orion_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from orion_cli.colors import Colors, color
from orion_cli.models import _ORION_USER_AGENT
from orion_cli.vercel_auth import describe_vercel_auth
...
from orion_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Orion containers"
...
def _orion_database_paths(orion_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Orion-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from orion_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, orion_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((orion_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(orion_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``orion_cli.sqlite_safe_read``).
...
def _report_database_journal_modes(
    ...
    from orion_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = orion_home if orion_home is not None else ORION_HOME
    try:
        databases = _orion_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Orion databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.orion/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ORION_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from orion_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from orion_cli.auth import get_xai_oauth_auth_status
...
from orion_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'orion sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    # ORION_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor removed its only consumer (the v3→4 migration) — it is silently
    # ignored. ORION_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("ORION_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("ORION_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Orion Relay plugin."""
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches orion_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``orion --version`` reporting a stale version while
    ...
    """
    try:
        from orion_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != orion_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'orion update', or set "
            "orion_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-orion and dashboard static services are up
    ...
    for static in ("main-orion", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `orion profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `orion doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from orion_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the ORION_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ORION_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ORION_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `orion`.
    os.environ.setdefault("ORION_INTERACTIVE", "1")

    # Handle `orion doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.orion/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Orion Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`orion doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from orion_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Orion already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and orion_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.orion/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        # matches orion_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'orion setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'orion setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'orion setup' to create one")
                issues.append("Run 'orion setup' to create .env")
    
    # Check ~/.orion/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from orion_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from orion_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'orion config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from orion_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.orion/.env or run 'orion setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'orion setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'orion config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from orion_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'orion setup' to migrate config")
                else:
                    issues.append("Run 'orion doctor --fix' or 'orion setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from orion_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'orion doctor --fix'")
        except Exception:
            pass

        # Detect stale ORION_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ORION_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from orion_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("ORION_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ORION_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `orion setup` run)",
                )
                if should_fix:
                    if remove_env_value("ORION_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ORION_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ORION_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ORION_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ORION_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'orion doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from orion_cli.config import load_env as _load_env_depr
            from orion_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Orion' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = orion_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Orion a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Orion Agent Persona\n\n"
                "<!-- Edit this file to customize how Orion communicates. -->\n\n"
                "You are Orion, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = orion_home / "memories"
    ...
    state_db_path = orion_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from orion_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'orion doctor --fix' "
                        "(or 'orion sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from orion_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'orion doctor --fix' "
                        "(or 'orion sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Orion
        ...
        try:
            from orion_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'orion sessions optimize-storage' "
    ...
    wal_path = orion_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'orion doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "orion"
        ...
        _cmd_link = _cmd_link_dir / "orion"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(orion not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/orion → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/orion points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/orion → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/orion — run 'orion doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/orion exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/orion not found",
                    "(orion command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/orion → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/orion symlink — run 'orion doctor --fix'")
    ...
    try:
        from orion_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'orion-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/Orion-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `orion update` does, so a session's first browser call
            ...
            # `orion update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `orion doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable ORION_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Orion one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'orion config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``orion doctor``.
    ...
    # never the legitimate source for `orion doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'orion setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ORION_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: orion skills list)")
    ...
    try:
        from orion_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = ORION_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: orion memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: orion memory setup",
                    "No Honcho API key — run 'orion memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run orion memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: orion memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: orion memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from orion_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "orion -p" in content:
                            _m = _re.search(r"orion -p (\S+)", content)
    ...
    # `orion doctor --live` (real network calls; bounded + read-only).
    try:
        from orion_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'orion doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))

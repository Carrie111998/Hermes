...
``ORION_HOME/cache/`` with a TTL, with a bundled seed file as the offline
...
def _cache_path() -> Path:
    return get_orion_home() / "cache" / "plugin_index.json"


def get_index_url() -> str:
    """Resolve the index URL: config override ``plugins.index_url`` or default."""
    try:
        from orion_cli.config import cfg_get, load_config_readonly

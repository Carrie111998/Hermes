...
from orion_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /orion``, the
    ``login_url`` is prefixed (``/orion/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from orion_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["orion.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``orion_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``orion_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from orion_cli.dashboard_auth.cookies import clear_session_cookies
        from orion_cli.dashboard_auth.prefix import prefix_from_request

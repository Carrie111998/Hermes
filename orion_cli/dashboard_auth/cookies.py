...
Three cookies in play:
  ...
  - orion_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/orion) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/orion`` covers
...
setter wrote ``__Secure-orion_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "orion_session_at"
SESSION_RT_COOKIE = "orion_session_rt"
...
SESSION_PROVIDER_COOKIE = "orion_session_provider"
PKCE_COOKIE = "orion_session_pkce"
...
SSO_ATTEMPT_COOKIE = "orion_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /orion`` we want ``Path=/orion`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (orion #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/orion``)

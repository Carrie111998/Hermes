...
from ``orion_constants`` or other orion_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ORION_HOME/logs/dashboard-auth.log``.

    Uses ``orion_constants.get_orion_home()`` (a leaf module — no import
    ...
    return get_orion_home() / "logs" / "dashboard-auth.log"

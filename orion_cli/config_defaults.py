"""Default configuration data for Orion Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from orion_cli/config.py. Must not import from orion_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["orion-cli"],
    # SQLite journal mode used by every Orion database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Orion server processes.
    ...
    "session": {
        # Per-terminal `orion -c`: each CLI session drops a breadcrumb file
        # under $ORION_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by orion_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `orion chat --run-budget N`.
        ...
        # an interactive `orion gateway restart` must never block for hours
        ...
        # the Orion-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Orion
        ...
        # identity slot (SOUL.md). Empty by default. The ORION_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Orion adds a coding operating brief
        ...
        # `orion status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``ORION_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # bot_relay from a short-lived `orion -p <bot> chat -Q` recipient
        ...
        #             ORION_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ORION_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Orion auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Orion sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Orion
        ...
        # bundled Orion image, which drops to the `orion` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `orion tools` picker's
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Orion sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Orion should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``orion chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.orion/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: orion sweeps the checkpoint base at startup
        ...
        # ``orion checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .orion.md, or .cursorrules before Orion applies
    ...
    # Single-query (``orion -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Orion truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Orion' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); orion =
                                      # Orion' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Orion' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Orion does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``orion kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``orion kanban decompose`` and the kanban
        ...
        # ``orion profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `orion model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `orion` (and `orion chat`) launches by default:
        ...
        # REPL and `--tui` (or ORION_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `orion --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `orion -c` muscle memory.  Default off so existing
        # users aren't surprised.  ORION_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `orion --tui` drops a one-time hint
        ...
        # model — see orion_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to ORION_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/orion`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `orion pets`. Disabled until
        ...
        "pet": {
            ...
            # get_orion_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``ORION_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ORION_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ORION_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ORION_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ORION_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/orion``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Orion appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.orion/cache/piper-voices/
    ...
    # "Hey Orion" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey orion",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_orion" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.orion/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.orion/cache/delegation/
    ...
    # response. If not, Orion feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Orion auto-pauses the goal and
    ...
    "moa": {
        ...
        # <orion_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.orion/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.orion/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``orion skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``orion skills trust`` / ``orion skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${ORION_SKILL_DIR} and ${ORION_SESSION_ID} in SKILL.md
        ...
        # (`orion skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.orion/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.orion/.curator_backups/blobs/. Enables `orion curator ledger`
        # and single-mutation `orion curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `orion curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `orion curator run --consolidate` overrides
        ...
        # normally restored on every `orion update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `orion curator purge` deletes
        ...
        # skipped), snapshot ~/.orion/skills/ into
        # ~/.orion/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `orion curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for orion-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Orion Agent*" header.
    ...
    # dangerous command. -q runs export ORION_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; ORION_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Orion' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.orion/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ORION_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .orion config) always require human
        ...
        # longer trigger the startup banner. Add via `orion doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``orion_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Orion to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ORION_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ORION_CRON_MAX_PARALLEL env var.
        ...
        # ORION_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # ORION_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via ORION_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `orion -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `orion` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `orion kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     orion-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Orion tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.orion/logs/.
    ...
    # update model picker lists without shipping a orion-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://your-orion-docs.example/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `orion model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Orion provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal ORION_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.orion/sessions/sessions.json entirely.
        ...
        # the ORION_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.orion.gateway` that defenses 1-2 don't
        ...
        # `orion update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``ORION_GATEWAY_MAX_STARTS`` / ``ORION_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.orion/.env,
        ...
        # behavior: files must live under the Orion cache, under
        ...
        # user. Bridged to ORION_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Orion
        # cache (~/.orion/cache/{documents,images,audio,video,screenshots})
        ...
        # to ORION_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ORION_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ORION_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.orion/state.db.
    ...
    "sessions": {
        ...
        # the default of ``orion sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.orion/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `orion sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `orion update` prints a one-line notice with
        ...
        # ~/.orion/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ORION_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ORION_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `orion sessions export`. Checked
    ...
    # ``orion doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `orion doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``orion update`` behaviour.
    "updates": {
        ...
        #     DBs) into <ORION_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``orion backup``-style zip
        #     of ORION_HOME into <ORION_HOME>/backups/, restorable with
        #     ``orion import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `orion update` does with uncommitted local changes to the
        ...
        # When `orion update` finds the source checkout parked on a feature
        ...
        #                         merge. `orion update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<ORION_HOME>/lsp/bin/`` on first use.
    ...
    # `orion tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.orion/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.orion/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.orion/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Orion may use this encrypted
            ...
            # ~/.orion/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.orion/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Orion disables it for our users unless they
        # explicitly opt in here. When false (default), Orion sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Orion also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Orion computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Orion passes the
    ...
    #
    # Configure with `orion egress setup`.  Disabled by default — the rest of
    # Orion works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.orion/bin/ on
    ...
    # Orion Desktop (Electron app) launch options. These only affect
    # `orion desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ORION_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the ORION_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Orion mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'orion model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Orion auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Orion' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Orion, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @orion:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ORION_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #orion)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: orion-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'orion-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Orion API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Orion API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Orion API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ORION_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ORION_TOOL_PROGRESS_MODE for backward

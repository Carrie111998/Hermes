"""``orion sync`` subcommand parser — Skill Sync.

Cloned from ``orion_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    orion sync status                 show gate/opt-in/head state
    orion sync pull                   pull and materialize opted-in skills
    orion sync push                   push opted-in skills
    orion sync now                    reconcile: pull then push
    orion sync enable <skill>         opt a skill into sync
    orion sync disable <skill>        opt a skill out of sync
    orion sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    orion sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  orion sync status            what is synced, and from where\n"
            "  orion sync enable my-skill   include a skill in your sync\n"
            "  orion sync now               pull, then push\n"
            "  orion sync propose my-skill  share a skill with your team\n"
        ),

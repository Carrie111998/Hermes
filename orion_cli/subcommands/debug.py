...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Orion Agent. Use 'orion debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    orion debug share              Upload debug report (asks for confirmation)
    orion debug share --yes        Skip confirmation (for scripts/CI)
    orion debug share --lines 500  Include more log lines
    orion debug share --expire 30  Keep paste for 30 days
    orion debug share --local      Print report locally (no upload)
    orion debug share --no-redact  Disable upload-time secret redaction
    orion debug share --nous       Upload to Nous-internal storage (private)
    orion debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'orion debug share'",
    )

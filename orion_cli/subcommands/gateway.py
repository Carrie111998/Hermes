"""``orion gateway`` and ``orion proxy`` subcommand parsers.

Extracted verbatim from ``orion_cli/main.py:main()`` (god-file Phase 2).
...
from orion_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ORION_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy orion.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Orion gateway unit files "
            "(e.g. orion.service) left over from older installs. Profile "
            "units (orion-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.orion/.env. "
            "Requires being logged in (orion setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.orion/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `orion proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,

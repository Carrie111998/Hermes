...
These were module-level helpers in ``orion_cli/main.py``. They are pulled
into a neutral module so both ``main.py`` and every
``orion_cli/subcommands/<group>.py`` builder can import them without an
...
def add_accept_hooks_flag(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ORION_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )

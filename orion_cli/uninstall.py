"""
Orion Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.orion/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Orion PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing orion-agent or orion PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Orion Agent" comment and following line
                if '# Orion Agent' in line or '# orion-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('orion' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing orion
                if 'orion' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Orion-owned file, and
...
def remove_wrapper_script():
    """Remove the orion wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "orion",
        Path.home() / ".local" / "bin" / "orion-acp",
        Path.home() / ".local" / "bin" / "orion-agent",
        Path("/usr/local/bin/orion"),
        Path("/usr/local/bin/orion-acp"),
        Path("/usr/local/bin/orion-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains orion_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'orion_cli' in content or 'orion-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(orion_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``orion`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Orion home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (orion_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``orion gateway run`` processes
    ...
    try:
        from orion_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from orion_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ORION_HOME`` and ``ORION_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\orion\git\cmd``, ``%LOCALAPPDATA%\orion\git\bin``,
#      ``%LOCALAPPDATA%\orion\git\usr\bin``, ``%LOCALAPPDATA%\orion\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\orion\git\`` and Node to
#      ``%LOCALAPPDATA%\orion\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``orion dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\orion\gateway-service\`` and sometimes
...
def _orion_path_markers(orion_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Orion-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the orion launchers and the managed uv) — only wanted when
    ...
    """
    root = str(orion_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare orion-agent install dir.
    markers = [root + "\\orion-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if ORION_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "orion".  We don't want to catch
    # unrelated entries like "corion-foo" or "ephermeral", so we look for
    # backslash-orion as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(orion_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Orion-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<orion_home>\\bin`` (the managed binary
    dir holding the orion launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _orion_path_markers(orion_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_orion_env_vars_windows() -> list[str]:
    """Delete ORION_HOME and ORION_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ORION_HOME", "ORION_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``orion`` launchers install.ps1 staged in the managed
    binary dir (the default Orion root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``orion`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_orion_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from orion_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from orion_constants import get_default_orion_root

        bin_dir = get_default_orion_root() / "bin"
    except Exception as e:
...
def _is_default_orion_home(orion_home: Path) -> bool:
    """Return True when ``orion_home`` points at the default (non-profile) root."""
    try:
        from orion_constants import get_default_orion_root
        return orion_home.resolve() == get_default_orion_root().resolve()
    except Exception:
        return False
...
def _discover_named_profiles():
    ...
    default root."""
    try:
        from orion_cli.profiles import list_profiles
    except Exception:
        return []
    try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ORION_HOME directory.

    We shell out to ``orion -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ORION_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m orion_cli.main` so we don't depend on a `orion`
    #    wrapper that may be half-removed mid-uninstall.
    orion_invocation = [_sys.executable, "-m", "orion_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                orion_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's ORION_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``orion uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ORION_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from orion_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(orion_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Orion Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Orion Chat GUI installation was found.")
        print(f"  Checked: {orion_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Orion agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(orion_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Orion agent at {orion_home / 'orion-agent'}")
        print(f"  • Your config, sessions, and secrets under {orion_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Orion agent is still installed. Run 'orion' to use the CLI,")
    print("or 'orion uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.orion/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.orion/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ORION_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_orion_home(orion_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.orion data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Orion Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {orion_home / 'config.yaml'}")
    print(f"  Secrets: {orion_home / '.env'}")
    print(f"  Data:    {orion_home / 'cron/'}, {orion_home / 'sessions/'}, {orion_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ORION_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Orion data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Orion code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, orion_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Orion PATH entries from shell configs / Windows User PATH")
    print("  • Orion wrapper scripts and Orion-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Orion config/data: {orion_home}")
        if _is_default_orion_home(orion_home):
        ...
        print(f"  • Keep Orion config/data: {orion_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``orion`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ORION_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in orion_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\orion\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (orion\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_orion_home(orion_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(orion_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Orion-owned PATH entries in User environment")

        log_info("Removing ORION_HOME / ORION_GIT_BASH_PATH User env vars...")
        removed_env = remove_orion_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Orion-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing orion command...")
    ...
    #     would dangle — `orion` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows orion launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows orion launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Orion home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Orion-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(orion_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Orion-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(orion_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ORION_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from orion_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(orion_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.orion/, just remove the orion-agent subdir
            if orion_home in project_root.parents or project_root.parent == orion_home:
    ...
    #     under ORION_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(orion_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(orion_home)
    ...
    # 5. Optionally remove ~/.orion/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ORION_HOME dirs live under
        ...
        try:
            if orion_home.exists():
                shutil.rmtree(orion_home)
                log_success(f"Removed {orion_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {orion_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {orion_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://your-orion-docs.example/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://your-orion-docs.example/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Orion Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m orion_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import orion_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``orion_constants`` + ``orion_cli.colors``
    (and lazily ``orion_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m orion_cli.uninstall")
    parser.add_argument(
        "--mode",

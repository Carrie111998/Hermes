"""``orion logs`` — view and filter Orion log files.
...
Usage examples::
    ...
    orion logs                    # last 50 lines of agent.log
    orion logs -f                 # follow agent.log in real time
    orion logs errors             # last 50 lines of errors.log
    orion logs gateway -n 100    # last 100 lines of gateway.log
    orion logs gui -f            # follow gui.log (dashboard/pty/ws)
    orion logs desktop -f        # follow desktop.log (Electron app boot/backend)
    orion logs --level WARNING    # only WARNING+ lines
    orion logs --session abc123   # filter by session ID substring
    orion logs --component tools  # only tool-related lines
    orion logs --since 1h         # lines from the last hour
    orion logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_orion_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Orion runs — try 'orion chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_orion_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_orion_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_orion_home()}/logs/")
        return

    print(f"Log files in {display_orion_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'orion chat' to generate logs)")

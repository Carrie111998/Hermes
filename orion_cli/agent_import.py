"""orion import-agent — import Claude Code / Codex CLI setups into Orion.

Usage:
    orion import-agent                       # auto-detect ~/.claude or ~/.codex
    orion import-agent claude-code           # import from ~/.claude
    orion import-agent codex                 # import from ~/.codex
    orion import-agent claude-code --dry-run # preview only, no changes
    orion import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``orion claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_orion.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in ORION_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → ORION_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in ORION_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in ORION_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → ORION_HOME/skills/codex-imports/<name>/
...
deliberately via ``orion setup`` or config.yaml.
"""
...
# Same entry delimiter as the Orion memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`orion config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_orion.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Orion command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Orion glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Orion equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        orion_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → ORION_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``orion import-agent`` (invoked from orion_cli.main)."""
    from orion_cli.config import get_config_path, load_config, save_config
    from orion_constants import get_orion_home
    from orion_cli.setup import (
        Colors,
        color,
        print_header,
        print_info,
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: orion import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: orion import-agent claude-code   or   orion import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Orion — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: orion import-agent "
    ...
    print_info(f"Target:      {orion_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'orion setup' for credentials")
    ...
    try:
        preview = AgentImporter(
            ...
            target_root=orion_home.resolve(),
            execute=False,
            overwrite=overwrite,
        ).run()
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: orion import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'orion setup' "
               "to configure providers, or add them to ~/.orion/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from orion_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'orion setup' or ~/.orion/.env.")
        print()

"""Dashboard process-hygiene helpers — extracted from ``orion_cli/main.py``.
...
``_detect_concurrent_orion_instances``) are lifted verbatim. References to
helpers that STAY in ``orion_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``orion_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``orion_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``orion_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``orion dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``orion update`` replaces files on disk, the running
    ...
    ``orion-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Orion Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``orion serve`` as
    ...
    environment variable ``ORION_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "orion dashboard",
        "orion_cli.main dashboard",
        "orion_cli/main.py dashboard",
        # The headless backend (`orion serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "orion serve",
        "orion_cli.main serve",
        "orion_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from orion_cli._subprocess_compat import bounded_probe_run
            ...
            # (rather than `pgrep -f "orion.*dashboard"`) keeps us consistent
            # with `orion_cli.gateway._scan_gateway_pids` and avoids the
...
def _orion_home_for_pid(pid: int) -> str | None:
    """Best-effort ``ORION_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("ORION_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"ORION_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Orion Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``orion update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], orion_home: str | None = None
) -> str:
    """Stable owner key: ``ORION_HOME`` when known, else ``--profile`` / ``-p``.

    ``ORION_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.orion`` roots) keep a resolved ``home:`` key so
    ...
    if orion_home:
        try:
            home_path = Path(orion_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(orion_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(orion_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``orion update``.

    Each candidate is ``(pid, argv, orion_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``ORION_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``ORION_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``orion update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, orion_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, orion_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``orion dashboard`` / ``orion serve`` processes.

    Called at the end of ``orion update`` (default ``reason``) and also
    from ``orion dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``orion update`` path), a detected
    ``orion-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``orion-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``orion update``'s systemd
    fleet-restart loop, which restarts ``orion-serve*`` units before this
    ...
    # When the Orion Desktop Electron app spawns this dashboard as a
    # backend child, it sets ORION_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ORION_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `orion update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot ORION_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _orion_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. orion update's
    ...
    #    remote backend (orion serve) under Restart=on-failure never comes
...
def _detect_concurrent_orion_instances(
    ...
    The Orion Desktop Electron app spawns ``orion.EXE`` as a backend child,
    so during ``orion update`` the user-invoked process and the desktop's
    ...
    shims (``orion.exe`` / ``orion-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``orion update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``orion.exe``) is a separate process from the Python
    ...
    for shim in _m()._orion_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated orion.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``orion update``
    ...
    #      orion.exe sitting *under* a non-Orion parent (e.g. a Orion
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        orion serve --host 127.0.0.1 --port 0
        orion serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "orion" not in cmd and "orion_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (ORION_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("ORION_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``orion serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``orion serve`` on a Mac
...
def _orion_home_dir() -> Path:
    """Resolved Orion home (ORION_HOME override or ~/.orion)."""
    override = os.environ.get("ORION_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "orionPath", "orionHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated ORION_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{orion_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _orion_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local orion serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``orion serve`` backends with no parent.
    ...
    running under ORION_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never ORION_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports ORION_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # ORION_BACKEND_READY, so a sibling spawned seconds ago is not yet

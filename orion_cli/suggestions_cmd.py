...
def _resolve_origin() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        platform = get_session_env("ORION_SESSION_PLATFORM")
        chat_id = get_session_env("ORION_SESSION_CHAT_ID")
        if platform and chat_id:
            return {
                "platform": platform,
                "chat_id": chat_id,
                "chat_name": get_session_env("ORION_SESSION_CHAT_NAME") or None,
                "thread_id": get_session_env("ORION_SESSION_THREAD_ID") or None,
            }
    except Exception:
        pass
    return None

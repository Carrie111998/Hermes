"""Offline, non-destructive recovery for a damaged Orion session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``orion_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``orion_state._backup_db_file``.
    """
    from orion_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Orion process using this profile and retry. "
                "This includes the interactive `orion` CLI session this "
                ...
                "Orion process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `orion` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from orion_cli.session_lost_and_found import (

"""orion webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.orion/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _orion_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: orion gateway run
"""
...
def webhook_command(args):
    """Entry point for 'orion webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: orion webhook {subscribe|list|remove|test}")
        print("Run 'orion webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (orion gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: orion webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from orion webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (orion gateway run)")

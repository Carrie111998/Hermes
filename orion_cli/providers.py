"""
Single source of truth for provider identity in Orion Agent.
...
2. **Orion overlays** — transport type, auth patterns, aggregator flags,
...
# -- Orion overlay ----------------------------------------------------------
# Orion-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class OrionOverlay:
    """Orion-specific provider metadata layered on top of models.dev."""
...
ORION_OVERLAYS: Dict[str, OrionOverlay] = {
    "moa": OrionOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": OrionOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": OrionOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": OrionOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": OrionOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": OrionOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ORION_QWEN_BASE_URL",
    ),
    "lmstudio": OrionOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": OrionOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": OrionOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": OrionOverlay(
    ...
    "kimi-for-coding": OrionOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": OrionOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": OrionOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": OrionOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": OrionOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": OrionOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": OrionOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": OrionOverlay(
        transport="openai_chat",
    ...
    "opencode": OrionOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": OrionOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": OrionOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": OrionOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": OrionOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "orion", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Orion overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Orion overlay
    ...
    overlay = ORION_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + orion extra
    ...
    if overlay is not None:
        # Orion-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and ORION_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Orion overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in ORION_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Orion provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from orion_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY

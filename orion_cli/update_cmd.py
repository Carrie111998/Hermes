"""Orion update pipeline — extracted from ``orion_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``orion_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``orion_cli.main`` reference — so existing call sites and test monkeypatches
that target ``orion_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``orion_cli.main``.
...
Imports are one-way: ``orion_cli.main`` imports this module, never the reverse
...
from orion_cli.config import get_orion_home
from orion_constants import venv_python_path

logger = logging.getLogger(__name__)


def _m():
    """Lazy ``orion_cli.main`` reference.

    Lets callers keep patching ``orion_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``orion_cli.main`` -> ``orion_cli.update_cmd`` stays one-way
    at import time.
    """
    from orion_cli import main
...
#: ``_purge_stale_orion_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        "orion_cli",
        "orion_cli.main",
        "orion_cli.update_cmd",
        "orion_cli.orion_logging",
    }
)


def _purge_stale_orion_modules() -> None:
    """Evict every cached Orion module after the checkout changed in-place.

    ``orion update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from orion_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``orion_cli.gateway`` does
    ``from orion_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Orion
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Orion module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Orion modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``orion update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``orion update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``orion_cli.config_defaults``,
    ``orion_cli.config``, and ``orion_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``orion_cli._subprocess_compat`` and
    ``orion_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "orion_cli.config_defaults",
        "orion_cli.config",
        "orion_cli.config_migrations",
        "orion_cli._subprocess_compat",
        "orion_cli.dashboard_procs",
    ):
...
def _run_config_check_fresh() -> tuple:
    ...
    from orion_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from orion_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``orion update`` historically
    ...
    caller): scope config reads/writes via the context-local ORION_HOME
    ...
    try:
        from orion_constants import (
            get_process_orion_home,
            reset_orion_home_override,
            set_orion_home_override,
        )
        from orion_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_orion_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_orion_home_override(entry)
            try:
...
# Critical files that Orion must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "orion_cli/main.py",
    "orion_cli/config.py",
    "orion_cli/__init__.py",
    "orion_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``orion.exe`` has
    ...
    Skipping is safe because Orion pins its editable finder to a *static*
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``orion`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="orion-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``orion update`` can be driven by a
    ...
    """
    from orion_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from orion_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('orion_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``orion update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from orion_constants import get_orion_home
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `orion update`.
    ...
    print("  Preview now:  orion curator run --dry-run")
    print("  Pause it:     orion curator pause")
    print(
        "  Docs:         https://your-orion-docs.example/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `orion update`.
    ...
    try:
        ...
        return
    db_path = get_orion_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `orion sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    orion sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  orion sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``orion update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``orion update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: orion curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from orion_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    orion dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``orion_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.orion-update-staging"
    backup = f"{dst}.orion-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `orion update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``orion_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``orion update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && orion update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("ORION_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== orion-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Orion launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["orion.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Orion")
    print("  Desktop, a gateway, or another orion REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `orion` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``orion.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.orion-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".orion-update-staging", ".orion-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `orion update`.")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Orion Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `orion update "
            f"--branch {branch}`, or update against main with `orion update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/zacharyjleach-stack/Aries/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="orion-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"orion-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for orion-agent itself
        ...
        # GitHub ZIPs extract to orion-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"orion-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # Orion.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `orion update` to retry; if the agent won't start, "
            "reinstall from https://your-orion-docs.example"
        )
        _m().sys.exit(1)
    finally:
    ...
    from orion_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from orion_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `orion update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: orion skills list-modified  "
    ...
    try:
        from orion_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from orion_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_orion_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_orion_home())
    ...
    try:
        from orion_cli.update_receipt import finalize_update_receipt
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "orion-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``orion update --keep-stash`` (the desktop updater's mode): the
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Orion behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes orion completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Orion couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Orion couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Orion behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Orion couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Orion couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/zacharyjleach-stack/Aries.git",
    "git@github.com:Orion/orion-agent.git",
    "https://github.com/zacharyjleach-stack/Aries",
    "git@github.com:Orion/orion-agent",
}

OFFICIAL_REPO_URL = "https://github.com/zacharyjleach-stack/Aries.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_orion_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_orion_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Orion repository.")
        print("  This means you may miss updates from Orion/orion-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/zacharyjleach-stack/Aries.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/zacharyjleach-stack/Aries.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``orion update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from orion_constants import get_default_orion_root

    default_home = get_default_orion_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "orion.exe"
    lines = ["✗ Another orion.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Orion Desktop, exit any open `orion` REPLs, and")
    lines.append("  stop the gateway (`orion gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `orion update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``orion tools``."""
    try:
        from orion_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Orion Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``orion tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Orion Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Orion Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``orion update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `orion update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `orion update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Orion' editable-install extras or
    ...
    try:
        from orion_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from orion_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ORION_HOME/bin/uv``, but on Termux the official
    ...
    """
    from orion_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `orion update` is exactly the step
...
def _npm_lockfile_changed(orion_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `orion
    ...
    try:
        ...
        cache_file = orion_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(orion_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    orion update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `orion update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from orion_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `orion update`.")
            failed = []
            if any(
    ...
    # Orion profile using this checkout. Keep one per-checkout cache under the
    # shared Orion root rather than rerunning npm once per named profile.
    shared_orion_root = get_default_orion_root()
    ...
    # `orion update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_orion_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `orion update`.")
        return list(labels)
    ...
    from orion_constants import with_orion_node_path

    nixos_env = with_orion_node_path(_m()._nixos_build_env())
    ...
    # `orion update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_orion_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.orion/logs/update.log`` only, never the terminal.

    During ``orion update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``orion update --check``: fetch and report without installing.
    ...
    from orion_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from orion_cli.banner import _github_compare_behind
            from orion_cli.config import recommended_update_command
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``orion update`` without requiring a reinstall.  The
    ...
    ``orion`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/orion.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v orion'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/orion, code at /usr/local/lib/orion-agent).
    fhs_link = Path("/usr/local/bin/orion")
    ...
    path_comment = (
        "# Orion Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``orion-acp`` launcher next to the ``orion`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``orion update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``orion-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Orion as not installed even when it is.

    The shim simply delegates to the sibling ``orion`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``orion`` / ``orion-acp``
    launchers into the managed binary dir ``$OrionHome\bin`` and puts THAT
    ...
    ``orion_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``orion-acp`` is already present next to the
    ``orion`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        orion_cmd = bin_dir / "orion"
        acp_cmd = bin_dir / "orion-acp"
        try:
            if not (orion_cmd.is_file() or orion_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ORION_HOME under
      ``backups/`` (restorable via ``orion import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from orion_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from orion_constants import
        # get_orion_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from orion_cli.config import get_orion_home as _get_home
        ...
        try:
            from orion_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from orion_cli.update_receipt import record_step
    ...
    try:
        from orion_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from orion_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_orion_home so the user sees ~/.orion/...
    try:
        from orion_constants import get_orion_home, display_orion_home

        home = get_orion_home()
        try:
            display_path = f"{display_orion_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  orion import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``orion update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``orion update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run orion from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.orion-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".orion-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The orion.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m orion_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``orion
    ...
    for proc in proc_iter:
        ...
        # the `-m orion_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``orion_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``orion.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``orion`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `orion` launch will complete it")
...
def _holder_value_flags() -> frozenset:
    ...
    try:
        from orion_cli._parser import build_top_level_parser
...
def _orion_holder_subcommand(cmdline: str) -> str | None:
    """The actual Orion SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``orion_cli.main`` / ``orion(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("orion_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("orion", "orion.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``orion dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Orion processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Orion backend (if the Desktop app is open, close it)",
        "dashboard": "  ← orion dashboard (stop it: orion dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _orion_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the Orion desktop app / other Orion terminals, then re-run:"
    )
    lines.append("    orion update")
    lines.append("  (or use `orion update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``orion update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from orion_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning orion-setup, and the update-in-progress marker
    ...
    the update with "Orion is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Orion backend (``orion_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.orion-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "orion_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``orion_cli.process_identity``). A holder qualifies when ALL of:
    ...
    """
    try:
        from orion_cli.process_identity import (
            REAPABLE_PURPOSES,
            ledger_entries,
            spawner_is_dead,
        )
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Orion ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``orion.exe``) is open, NOTHING legitimate
    ...
    - Only Orion backends (``orion_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``orion.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.orion-runtime`` interpreter child alive and holding the install open
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``orion serve`` / ``orion dashboard`` argv.
    ...
    """
    if "orion_cli.main" not in (cmdline or "").lower():
        return False
    return _orion_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _desktop_owns_gateway_lifecycle() -> bool:
    ...
    """
    try:
        from orion_cli.process_identity import ledger_entries, spawner_is_dead
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    orion.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from orion_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Orion...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m orion_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: orion gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``orion gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from orion_cli import gateway_windows
        from orion_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``orion-gateway*.service``/``orion-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("orion-serve")`` alone would also accept the
        # unrelated ``orion-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "orion-gateway.service"
            or unit.startswith("orion-gateway-")
            or unit == "orion-serve.service"
            or unit.startswith("orion-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``orion-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``orion-serve*`` units (#83438) don't, so sending them
    ...
    (``orion-gateway-helper`` is fine — profile units are
    ``orion-gateway-<profile>`` — but ``orion-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "orion-gateway" or svc_name.startswith("orion-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from orion_cli.gateway import is_macos
    ...
    print("    orion gateway status")
    if any(not name.startswith("ai.orion.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.orion.") for name in ordered):
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.orion.gateway*`` LaunchAgent must reload it — restarting only the
            ...
            try:
                ...
                if wait_for_launchd_gateway_supervision(label=current_label):
                    ...
                    print(
                        ...
                        "supervising it.\n"
                        "    Check logs, then: orion gateway restart"
                    )
    except (FileNotFoundError, subprocess.TimeoutExpired):
        pass
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``orion_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``orion_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``orion update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``orion-setup.exe``) executes
    ``$ORION_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_orion_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``orion update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``orion update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `orion desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "orion_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Orion-managed Node on PATH: when
    # `orion update` runs inside the desktop updater chain (Desktop →
    # orion-setup → orion update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from orion_constants import with_orion_node_path

    build_env = with_orion_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `orion desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from orion_constants import display_orion_home as _dhh
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from orion_cli.update_receipt import begin_update_receipt
    ...
    # running Orion runtime, its supervisor, and its running code version —
    ...
    try:
        from orion_cli.update_inventory import (
    ...
    # On Windows, abort early if another orion.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_orion_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `orion serve` backend) will keep .pyd
    ...
    # the orion.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Orion backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Orion backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .orion-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Orion is still
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Orion backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_orion_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Orion backend process(es) "
    ...
    # process by design (a CLI `orion update` IS the venv python), and an
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://your-orion-docs.example/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if current_branch != branch and current_branch != "HEAD":
            ...
            if switch_block_reason.startswith("unmerged:"):
                ...
                try:
                    from orion_cli.config import load_config as _load_cfg
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by orion.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                _write_update_incomplete_marker()
                from orion_cli.managed_uv import ensure_uv
                ...
                if healthy_after:
                    ...
                    print("  Close all Orion windows/gateways and re-run: orion update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Orion gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in orion_cli/config.py bricked
        # every user who ran ``orion update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``orion
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``orion update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``orion update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``orion doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && orion update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_orion_home added to orion_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``orion`` launch finishes the install
        ...
        # symbols in orion_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `orion` again "
        ...
        if not import_ok:
            ...
            print("    Run `orion update` again — if it persists, reinstall:")
            print("    https://your-orion-docs.example")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_orion_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.orion/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit ORION_HOME so
        # it is not affected by sync_skills()'s module-level ORION_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ORION_HOME env var points at the default or a named profile.
        try:
            from orion_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from orion_cli.profiles import backfill_profile_envs
        ...
        # ``orion update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``orion_cli.config`` and
        # ``orion_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from orion_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'orion config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'orion config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: orion config migrate")
            else:
                print()
                print("Skipped. Run 'orion config migrate' later to configure.")
        else:
        ...
        # context-local ORION_HOME override (never os.environ — other
        ...
        # it and warn loudly.
        try:
            from orion_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            from orion_cli.backup import restore_cron_jobs_all_profiles
        ...
        # check `orion curator status`. Self-stamps after printing so it
        ...
        # Self-heal the orion-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Orion on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("orion-acp launcher self-heal failed: %s", e)

        # Migrate the Windows orion launchers to the managed binary dir
        # (the default Orion root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (orion-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `orion`
        ...
        # the managed clone under the default Orion root).
        try:
            from orion_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``orion update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from orion_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `orion update`
                # must stay fast; `orion computer-use install --upgrade`
        ...
        # When running as ``orion update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Orion modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... orion-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... orion-gateway*)
            ...
            try:
                from orion_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all orion-gateway* units (default + profiles) plus
            # orion-serve* units (the Desktop app's backend, #83438).
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: orion gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # orion-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `orion update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.orion.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: orion gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: orion -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_orion_home() / ".update_exit_code"
                    try:
            ...
            try:
                from orion_cli.update_receipt import record_gateway_restart
        ...
        # Warn if legacy Orion gateway unit files are still installed.
        # When both orion.service (from a pre-rename install) and the
        # current orion-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `orion update` surfaces the issue until the user migrates.
        try:
            from orion_cli.gateway import (
                has_legacy_orion_units,
                _find_legacy_orion_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_orion_units():
                print()
                print("⚠ Legacy Orion gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_orion_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (orion.service) fight the current")
                print("  orion-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    orion gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes orion-serve*,
        ...
        print("  orion model              # Select provider and model")
        ...
        try:
            from orion_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import orion_cli.update_receipt as _ur
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from orion_cli.main import main; main()" update --yes'
                    )
            try:
                from orion_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``orion_cli.gateway`` no longer imports,

...
This module is imported by ``orion_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no orion_cli.config,
...
``orion_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(orion_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ORION_HOME?"""
    active_profile = os.path.join(orion_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    orion_home = os.environ.get("ORION_HOME", "").strip()
    if orion_home:
        return orion_home
    return os.path.join(os.path.expanduser("~"), ".orion")
...
def container_mode_may_be_active() -> bool:
    ...
    orion_home = os.environ.get("ORION_HOME", "").strip()
    if orion_home:
        if os.path.exists(os.path.join(orion_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(orion_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(orion_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".orion")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect ORION_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``orion --version`` output (also used by /version).
    ...
    ``orion version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        ...
        from orion_cli import __release_date__, __version__

        print(f"Orion Agent v{__version__} ({__release_date__})")
    ...
    try:
        from orion_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``orion --version`` before the heavy import wall.
    ...
    container. Termux keeps the ORION_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("ORION_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:

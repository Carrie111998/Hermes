...
on Japanese locales, etc.).  Orion's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``orion_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Orion is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ORION_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ORION_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Orion-managed Git install directories so
    ...
    # already-running shells don't see the change — which means orion
    ...
    # install without Orion-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Orion's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Orion's
    ``orion config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Orion (User env var in Windows Settings, or
    export in a PowerShell profile) and Orion picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Orion-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\orion\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``orion`` from right after install) retain their old PATH.

    Any subprocess Orion spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Orion-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "orion", "git", "cmd"),
        os.path.join(local_appdata, "orion", "git", "bin"),
        os.path.join(local_appdata, "orion", "git", "usr", "bin"),
        # Orion venv Scripts directory — host of the orion.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes orion, but harmless to include.
        os.path.join(local_appdata, "orion", "orion-agent", "venv", "Scripts"),

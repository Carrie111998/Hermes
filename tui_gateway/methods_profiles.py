...
handlers delegate to the same `orion_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Orion profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(profile_path):
        ...
        describe the same session (orion-agent#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(profile_path):
        ...
        sessions never surface in conversation lists (orion-agent#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's orion-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``orion setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (orion_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local ORION_HOME override — the same
        ...
        try:
            ...
            token = set_orion_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from orion_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from orion_cli.config import load_config_readonly, read_user_config_raw
            from orion_cli.web_routers.profiles import _write_profile_model
            from orion_constants import (
                reset_orion_home_override,
                set_orion_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the ORION_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `orion tools`
            ...
            # composites (orion-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from orion_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `orion tools` / Settings is where
            ...
            description = ""
            try:
                from orion_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from orion_cli.profiles import write_profile_meta

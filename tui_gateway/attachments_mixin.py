"""Desktop / session file-attachment helpers (moved verbatim from server.py).

Shard-plan s4 clusters c9 + c10 (the two highest-agreement move clusters of
the shard). Function bodies are byte-identical to their pre-split server.py
...
# Mirror of the module-level constant in server.py (rebound copies resolve it
# against server.py's namespace at runtime; this local copy lets the module's
# own functions run standalone, e.g. in tests).
...
def register(server) -> None:
    """Bind this module's helpers onto ``server``'s globals (see method_ctx.py)."""
    g = vars(server)
    for _fn in (

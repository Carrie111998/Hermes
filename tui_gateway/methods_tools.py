...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.orion/.env`` into the gateway process via
    ``orion_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m orion_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "orion_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m orion_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from orion_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind ORION_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_orion_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("ORION_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ORION_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_orion_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off ORION_HOME, so scoping the
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``orion journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "browse":
            from orion_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from orion_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `orion mcp` offers, so
...
# (orion_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes ORION_HOME via set_orion_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses orion_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    (``window.orionDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``orion mcp login`` uses
    ...
    try:
        ...
        orion_home = str(get_orion_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(orion_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``orion plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.orion/plugins/`` (non-interactive).
    ...
    plugins live under each profile's ORION_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Orion plugin.

    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero Orion sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from orion_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_orion_home), "display": display_orion_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _orion_home / "config.yaml"
        try:
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Orion provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    collection + force-redaction pipeline as ``orion debug share --nous``

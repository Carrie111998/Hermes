...
def _default_registry_path() -> Path:
    return get_orion_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_orion_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_orion_home = expected_orion_home if expected_orion_home is not None else str(get_orion_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["ORION_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("orion_home") or "")
        if got_home and got_home != self.expected_orion_home:
            raise RuntimeError(f"compute host ORION_HOME mismatch: {got_home} != {self.expected_orion_home}")

...
# Re-export names moved into methods_projects.py that are still resolved
# through this namespace — sibling methods_* consumers (methods_config.py)
# and the moved handlers themselves, which install() rebinds onto these
# globals (method_ctx.py seam contract). Identity-preserving: same objects.
...
# The moved plain helpers and inner handler bodies keep their home-module
# globals (install() only rebinds the outer @method wrappers); the
# server-level names they close over are delegated at call time through
# methods_projects.py's wrapper bindings, so monkeypatches of this module's
# bindings stay visible. No static bridge needed — the re-export above
# keeps the moved-handler direction covered.

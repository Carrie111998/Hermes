...
# Change watcher extracted to tui_gateway/change_watcher.py (god-file slice
# R2-S1). The names are re-exported here so every existing import site
# (entry.py, ws.py, the R5 config.set path, the test suite) keeps resolving
# them off this module. The moved functions read the registry state below and
# other server deps through this module object at call time (patch-inert).
from tui_gateway.change_watcher import (  # noqa: E402,F401
    resolve_skin,
    _skin_sig,
...
# live color edit to the active skin — and nothing else. Stays here (not in
# change_watcher.py) because test_protocol.py patches it via
# monkeypatch.setattr(server, "_last_skin_sig", ...) — the moved skin watcher
# reads/writes it through this module object at call time.

...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    # Resolve config first so an empty policy remains empty.
    if not explicit:
        try:
            from hermes_cli.config import (
                load_config,
                platform_toolsets_explicitly_empty,
            )
    ...
    try:
        ...
        if not enabled:
            # Only None selects AIAgent defaults; [] remains deny-all.
            return []
    ...
    kwargs.update(
        {
            "enabled_toolsets": (
                []
                if kwargs["enabled_toolsets"] is not None
                and not kwargs["enabled_toolsets"]
                else ["terminal", "file"]
            ),
            "session_db": None,
            "skip_memory": True,

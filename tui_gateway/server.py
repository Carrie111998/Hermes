...
def _mirror_slash_side_effects(sid: str, session: dict, command: str) -> str:
    ...
    try:
        if name == "model" and arg and agent:
            ...
            # Mirrors the `config.set key=fast` path. auto/cold windows are
            # applied per request by agent.fast_mode, so they clear any pinned
            # tier keys without resolving new ones.

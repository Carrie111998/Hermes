...
def _call_apply_model_switch(sid: str, session: dict, raw_input: str, **kwargs) -> dict:
    """Call _apply_model_switch while preserving older monkeypatch signatures."""
    try:
        import inspect

        sig = inspect.signature(_apply_model_switch)
        ...
        pass
    return _apply_model_switch(sid, session, raw_input, **kwargs)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    if agent is None or session.get("model_override"):
        return
    if (
        not hasattr(agent, "switch_model")
        and getattr(_apply_model_switch, "__name__", "") == "_apply_model_switch"
    ):
        return

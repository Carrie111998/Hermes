...
class ComputeHost:
    def __init__(
        ...
        *,
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("ORION_COMPUTE_HOST_HEARTBEAT_SECS") or "15")
        )
    ...
    def _run_real_turn(self, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            try:
                ...
                orion_undo.on_user_message_appended(session["session_key"])
            except Exception:
                pass
            try:
    ...
    def _ensure_server_session(self, server: Any, frame: dict[str, Any]) -> dict:
        ...
        try:
            if profile_home:
                from orion_constants import set_orion_home_override
            ...
            if home_token is not None:
                try:
                    from orion_constants import reset_orion_home_override
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("ORION_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["ORION_COMPUTE_HOST_CHILD"] = "1"
    ...
    host.emit(
        {
            ...
            "cwd": os.getcwd(),
            "orion_home": os.environ.get("ORION_HOME", ""),
        }
    )

#!/usr/bin/env python3
"""
Nastech CLI - Main entry point.

Usage:
    nastech                     # Interactive chat (default)
    nastech chat                # Interactive chat
    nastech gateway             # Run gateway in foreground
    nastech gateway start       # Start gateway as service
    nastech gateway stop        # Stop gateway service
    nastech gateway status      # Show gateway status
    nastech gateway install     # Install gateway service
    nastech gateway uninstall   # Uninstall gateway service
    nastech setup               # Interactive setup wizard
    nastech logout              # Clear stored authentication
    nastech status              # Show status of all components
    nastech cron                # Manage cron jobs
    nastech cron list           # List cron jobs
    nastech cron status         # Check if cron scheduler is running
    nastech doctor              # Check configuration and dependencies
    nastech honcho setup                    # Configure Honcho AI memory integration
    nastech honcho status                   # Show Honcho config and connection status
    nastech honcho sessions                 # List directory → session name mappings
    nastech honcho map <name>               # Map current directory to a session name
    nastech honcho peer                     # Show peer names and dialectic settings
    nastech honcho peer --user NAME         # Set user peer name
    nastech honcho peer --ai NAME           # Set AI peer name
    nastech honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    nastech honcho mode                     # Show current memory mode
    nastech honcho mode [hybrid|honcho|local]  # Set memory mode
    nastech honcho tokens                   # Show token budget settings
    nastech honcho tokens --context N       # Set session.context() token cap
    nastech honcho tokens --dialectic N     # Set dialectic result char cap
    nastech honcho identity                 # Show AI peer identity representation
    nastech honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    nastech honcho migrate                  # Step-by-step migration guide: OpenClaw native → Nastech + Honcho
    nastech version             Show version
    nastech update              Update to latest version
    nastech uninstall           Uninstall Nastech Agent
    nastech acp                 Run as an ACP server for editor integration
    nastech sessions browse     Interactive session picker with search

    nastech claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: nastech_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``nastech_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``nastech update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``nastech_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, nastech crashes on import and the user can't run
...
from nastech_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python nastech_cli/main.py`` (script
# mode — sys.path[0] is nastech_cli/, not the repo root) can import the
# canonical helpers; everything else lives in nastech_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from nastech_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``nastech update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from nastech_cli.env_loader import ...`` / ``from nastech_cli.config import
...
# package directory, so if IT can't import, nothing else in nastech_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from nastech_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from nastech_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'nastech' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``nastech tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``nastech.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("nastech")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"nastech", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"nastech")
        # Windows: the .exe name is already ``nastech.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* nastech_cli.config is importable. Mirrors the explicit
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``NASTECH_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``nastech … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("NASTECH_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``NASTECH_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("NASTECH_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`nastech --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(nastech_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(nastech_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``nastech --version`` before config/logging imports."""
...
from nastech_cli.sessions_cmd import cmd_sessions  # noqa: F401
...
from nastech_cli.subcommands.sync import build_sync_parser
...
from nastech_cli.subcommands.pause import build_pause_parser
...
from nastech_cli.subcommands.verify import build_verify_parser
from nastech_cli.subcommands.security import build_security_parser
from nastech_cli.subcommands.approvals import build_approvals_parser
...
from nastech_cli.subcommands.import_agent import build_import_agent_parser
from nastech_cli.subcommands.config import build_config_parser
from nastech_cli.subcommands.skin import build_skin_parser
from nastech_cli.subcommands.console import build_console_parser
...
from nastech_cli.subcommands.monitoring import build_monitoring_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (nastech tools, nastech setup, nastech model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'nastech {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any nastech module import.
#
# Many modules cache NASTECH_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("NASTECH_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.nastech/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set NASTECH_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `nastech mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Nastech' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo nastech -p <name>` against the invoking user's home.
        ...
        candidate = home / ".nastech" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`nastech chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors nastech_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If NASTECH_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.nastech/profiles/coder or
    # /opt/data/profiles/coder).  If NASTECH_HOME points to the nastech root
    # instead (e.g. systemd hardcodes NASTECH_HOME=/root/.nastech), we must
    # still read active_profile — the user may have switched profiles via
    # `nastech profile use` and the gateway should honour that choice.
    # See issue #22502.
    nastech_home_env = os.environ.get("NASTECH_HOME", "")
    if profile_name is None and nastech_home_env:
        if Path(nastech_home_env).parent.name == "profiles":
            return
    ...
    if profile_name is None and not os.environ.get("NASTECH_S6_SUPERVISED_CHILD"):
        try:
            ...
            active_path = get_default_nastech_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set NASTECH_HOME
    if profile_name is not None:
        try:
            from nastech_cli.profiles import resolve_profile_env

            nastech_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            nastech_home = _resolve_sudo_user_profile_env(profile_name)
            if not nastech_home:
            ...
            # A bug in profiles.py must NEVER prevent nastech from starting
            print(
...
# Bridge security.redact_secrets from config.yaml → NASTECH_REDACT_SECRETS env
# var BEFORE nastech_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves nastech_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from nastech_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_nastech_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from nastech_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "NASTECH_REDACT_SECRETS" not in os.environ:
...
# Initialize centralized file logging early — all `nastech` subcommands
...
try:
    from nastech_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from nastech_constants import apply_ipv4_preference as _apply_ipv4
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_nastech_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("NASTECH_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("NASTECH_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`nastech_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from nastech_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_nastech_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Nastech Portal OAuth credentials
    ...
    # Only count these if Nastech has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Nastech to use their tokens.
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``nastech -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, nastech_bin
        cli_args: the original CLI arguments (everything after 'nastech')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    nastech_bin = container_info["nastech_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo nastech {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
...
def _print_tui_exit_summary(
    ...
    print(f"  nastech --tui --resume {target}")
    if title:
        print(f'  nastech --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @nastech/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@nastech" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/nastech-ink/package.json",
    "packages/nastech-ink/index.js",
    "packages/nastech-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``NASTECH_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("NASTECH_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``NASTECH_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("NASTECH_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "NASTECH_HOME": nastech_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _find_bundled_tui(nastech_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if nastech_cli_dir is None:
        nastech_cli_dir = Path(__file__).parent
    bundled = nastech_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``nastech update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("NASTECH_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Nastech checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `nastech update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Nastech checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `nastech --tui`\n"
        "If the checkout is still inconsistent, run `nastech update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (NASTECH_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()
    ...
    def _node_bin(bin: str) -> str:
        if bin == "node":
            ...
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $NASTECH_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Nastech installed,
        ...
        if not path and bin == "node":
            try:
                from nastech_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("NASTECH_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with NASTECH_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset NASTECH_TUI_DIR (e.g. `unset NASTECH_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # nastech_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env={**with_nastech_node_path(), "CI": "1"},
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Nastech-managed
            ...
            from nastech_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @nastech/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @nastech/ink resolves through
        # packages/nastech-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "nastech-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from nastech_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("NASTECH_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["NASTECH_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("NASTECH_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["NASTECH_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("NASTECH_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["NASTECH_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["NASTECH_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a nastech process: propagate the profile-home contract via
    ...
    env["NASTECH_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["NASTECH_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["NASTECH_MODEL"] = model
        env["NASTECH_INFERENCE_MODEL"] = model
    if provider:
        env["NASTECH_TUI_PROVIDER"] = provider
        env["NASTECH_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["NASTECH_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["NASTECH_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["NASTECH_TUI_SKILLS"] = value
    if query:
        env["NASTECH_TUI_QUERY"] = query
    if image:
        env["NASTECH_TUI_IMAGE"] = image
    if checkpoints:
        env["NASTECH_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["NASTECH_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["NASTECH_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["NASTECH_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["NASTECH_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["NASTECH_ACCEPT_HOOKS"] = "1"
    ...
    # NASTECH_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `nastech --tui` try to
    ...
    # still set NASTECH_TUI_RESUME themselves.
    env.pop("NASTECH_TUI_RESUME", None)
    if resume_session_id:
        env["NASTECH_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `nastech update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``NASTECH_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``nastech kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``nastech kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("NASTECH_KANBAN_BOARD"):
        return
    try:
        from nastech_cli.kanban_db import get_current_board

        os.environ["NASTECH_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.nastech/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Nastech — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Nastech still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``nastech … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("NASTECH_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'nastech sessions list' to see available sessions.")
            sys.exit(1)
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Nastech isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  nastech setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'nastech setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from nastech_cli.banner import prefetch_update_check
    ...
    # user's ~/.nastech/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["NASTECH_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["NASTECH_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["NASTECH_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from nastech_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from nastech_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Nastech?")
        print()
    ...
    # has no creds.json.  Every subsequent `nastech gateway` then paid a 30s
    ...
    session_dir = get_nastech_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: nastech gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `nastech gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'nastech whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``nastech whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``nastech_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from nastech_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from nastech_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``nastech model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'nastech model' for "
    ...
    # PROVIDER_GROUPS in nastech_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``nastech model``
    ...
    # matching against nastech_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    elif selected_provider == "nastech":
        _model_flow_nastech(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.nastech/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.nastech/.env if the active provider is not 'custom'.
...
#
# Nastech uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `nastech model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `nastech model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`nastech_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Nastech Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``nastech model`` flow, then route aux tasks
    to them here.
    """
    from nastech_cli.config import load_config
    from nastech_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from nastech_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from nastech_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Nastech URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# downstream call sites read `nastech_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `nastech` invocation — including
# fast paths like `nastech --version` and slash-command dispatch that never
...
def _prompt_api_key(
    ...
    recover from a malformed paste without editing ``~/.nastech/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from nastech_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
...
def cmd_login(args):
    """Authenticate Nastech CLI with a provider."""
    from nastech_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from nastech_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from nastech_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from nastech_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from nastech_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.nastech/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `nastech sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            print("\nNot logged into Nastech Portal — sync is inert.", file=sys.stderr)
        elif not status.get("nastech_admin"):
            print(
            ...
            print(
                "\nSync feature is off for this instance (set NASTECH_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "NASTECH_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        ...
        return 1
    if not identity.get("nastech_admin"):
        print(
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="nastech sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="nastech sync now")
...
def cmd_webhook(args):
    ...
    from nastech_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``nastech slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: nastech slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `nastech slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from nastech_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from nastech_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from nastech_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from nastech_cli.hooks import hooks_command
...
def cmd_verify(args):
    ...
    from nastech_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `nastech security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from nastech_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `nastech approvals <subcmd>`."""
    from nastech_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from nastech_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from nastech_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from nastech_cli.config import config_command
...
def cmd_skin(args):
    ...
    from nastech_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Nastech home directory to a zip file."""
    if getattr(args, "quick", False):
        from nastech_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from nastech_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Nastech backup from a zip file."""
    from nastech_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from nastech_cli.config import detect_install_method
    from nastech_cli.slash_exec import CommandContext, execute_command
...
def cmd_uninstall(args):
    """Uninstall Nastech Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from nastech_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from nastech_cli.uninstall import run_gui_uninstall
    ...
    from nastech_cli.uninstall import run_uninstall
...
# Update pipeline extracted to nastech_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (nastech_cli.main._cmd_update_impl,
# nastech_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from nastech_cli.update_cmd import (  # noqa: F401
...
# validated against. Lives next to the checkout (NOT in NASTECH_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``nastech update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``nastech update``
    ...
    ``nastech`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``nastech update``
    ...
    ``nastech_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $NASTECH_HOME."""
    from nastech_constants import get_nastech_home
    return get_nastech_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``nastech`` launcher present but ``nastech_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the nastech venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — nastech venv python3, no nix-shell overhead
    ...
    # a nastech venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``nastech update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Nastech-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_nastech_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "nastech_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``nastech web``).
    ...
    # build path usable on a stock `py -m nastech_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_nastech_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (nastech web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``nastech update`` can unconditionally call ``nastech desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``nastech desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $NASTECH_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $NASTECH_HOME."""
    from nastech_constants import get_nastech_home
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``nastech update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            ...
            release_dir / "win-ia32-unpacked" / "Nastech.exe",
            release_dir / "win-arm64-unpacked" / "Nastech.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "nastech",
            release_dir / "linux-unpacked" / "Nastech",
            release_dir / "linux-arm64-unpacked" / "nastech",
            release_dir / "linux-arm64-unpacked" / "Nastech",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Nastech.exe
...
#
# The desktop self-update chain (Desktop → nastech-setup --update →
# `nastech update` → `nastech desktop --build-only` → relaunch) rebuilds
# Nastech.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Nastech.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Nastech.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 nastech-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Nastech.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Nastech.exe from backup.")
        print("    Your existing version was kept and still works. Run `nastech desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `nastech desktop --force-build` to rebuild, or re-run the Nastech")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Nastech'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_nastech_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Nastech.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Nastech.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Nastech.exe: Access is
    ...
    an unrelated "Nastech" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Nastech.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Nastech is damaged and can't be opened" — and macOS TCC forgets every
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``nastech desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the NASTECH_DESKTOP_DISABLE_GPU env var the Electron app
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Nastech Desktop (Linux only, best-effort).
    ...
    shell. ``nastech uninstall --gui`` removes it.
    """
    try:
        from nastech_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from nastech_logging import setup_logging as _setup_logging_gui
    ...
    # with_nastech_node_path() copies os.environ when called with no arg.
    env = with_nastech_node_path()
    if getattr(args, "fake_boot", False):
        env["NASTECH_DESKTOP_BOOT_FAKE"] = "1"
    ...
    if getattr(args, "cwd", None):
        ...
        env["NASTECH_DESKTOP_CWD"] = os.getcwd()
    ...
    # `NASTECH_DESKTOP_DISABLE_GPU=... nastech desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "NASTECH_DESKTOP_DISABLE_GPU" not in os.environ:
        env["NASTECH_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  nastech gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Nastech-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → nastech-setup →
            # nastech update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `nastech update` path).
            nixos_env = with_nastech_node_path(_nixos_build_env())
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> nastech desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Nastech is
                ...
                # success on a Nastech.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Nastech shows up
...
# Dashboard process-hygiene helpers extracted to nastech_cli/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on nastech_cli.main.<name> keep resolving unchanged.
from nastech_cli.dashboard_procs import (  # noqa: F401
    _detect_concurrent_nastech_instances,
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "nastech dashboard",
            "nastech_cli.main dashboard",
            "nastech_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "nastech serve",
            "nastech_cli.main serve",
            "nastech_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "nastech-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Nastech installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``nastech-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/nastech-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but nastech launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``nastech update``.
    ...
    log_path = get_nastech_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `nastech update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $NASTECH_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``nastech update``.
    ...
    protocol stream (``nastech acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `nastech update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``nastech.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_nastech_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from nastech.exe; applying package-only first aid, "
    ...
    try:
        ...
        if self_locked:
            print(
                "  Nastech is still running from the launcher that needs "
                "replacing. Close other Nastech windows, restart from a "
...
def _windows_running_nastech_launcher_locked() -> bool:
    """True when a venv ``nastech*.exe`` shim is this process or an ancestor.
    ...
    if scripts_dir is None:
        return False
    shims = _nastech_exe_shims(scripts_dir)
    if not shims:
        return False
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``nastech update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from nastech_constants import venv_bin_dir

    scripts = venv_bin_dir(venv_dir, windows=_is_windows())
    return scripts if scripts.is_dir() else None


def _nastech_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_nastech_exe(
    ...
    when ``nastech update`` runs, ``nastech.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``nastech.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next nastech invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Nastech Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Nastech Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _nastech_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Nastech Desktop, exit other `nastech` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `nastech update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_nastech_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``nastech.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``nastech.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``nastech`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_nastech_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``nastech.exe.old.*`` left by prior updates.

    Called early on every nastech invocation. The .old files are unlocked once
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``nastech.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``nastech.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `nastech update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``nastech.exe`` / ``nastech-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_nastech_exe`` for the rationale.
    """
    ...
    # hours later inside ``nastech update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``nastech.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``nastech.exe`` shim is locked during ``nastech update``, or when uv/distlib
    skips a launcher write. The symptom is ``nastech-agent.exe`` and
    ``nastech-acp.exe`` present but ``nastech.exe`` missing, so ``nastech`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `nastech update --force` after "
            "closing other nastech processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m nastech_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``nastech update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``nastech.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``nastech`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `nastech update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `nastech update --force` after closing other nastech processes."
        )
        return
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from nastech_constants import venv_python_path
...
class _UpdateOutputStream:
    """Stream wrapper used during ``nastech update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.nastech/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``nastech update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``nastech update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.nastech/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``nastech update --gateway``) the update is already
...
def cmd_update(args):
    """Update Nastech Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Nastech Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `nastech update --branch=<x>` would actually pull.
    ...
    from nastech_cli.update_lock import (
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``nastech -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `nastech profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_nastech_home()
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.nastech)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  nastech profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     nastech -p {name} chat")
                else:
        ...
        from nastech_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from nastech_constants import get_nastech_home as _hh
        ...
        from nastech_cli import profile_describer as _pd
        ...
        from nastech_cli.profiles import profile_exists, validate_alias_name
        ...
        from nastech_cli.profiles import rename_profile
        ...
        from nastech_cli.profiles import import_profile
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="nastech_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `nastech profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"nastech -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("nastech_requires"):
            print(f"Requires:     Nastech {data['nastech_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from nastech_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.nastech_requires:
        print(f"  Requires: Nastech {mf.nastech_requires}")
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} nastech dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``nastech dashboard register`` for OAuth.
    ...
    try:
        from nastech_cli.web_server import should_require_auth
    ...
    print("    [2] OAuth via Nastech Portal (run `nastech dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from nastech_cli.config import load_config, save_config
        from nastech_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from nastech_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.nastech/desktop-ssh: a
    # literal "~/.nastech/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of NASTECH_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_nastech_home(): a non-default sticky profile (or any NASTECH_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_nastech_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".nastech" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``NASTECH_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``nastech dashboard`` that
...
def cmd_dashboard(args):
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with nastech serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with NASTECH_DESKTOP=1 plus
    # NASTECH_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # NASTECH_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `nastech dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited NASTECH_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # NASTECH_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (NASTECH_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets NASTECH_SERVE_HEADLESS below.
    if os.environ.get("NASTECH_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("NASTECH_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("NASTECH_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("NASTECH_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → NASTECH_HOME points into
    ...
    if (
        ...
        and os.environ.get("NASTECH_DESKTOP") != "1"
    ):
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: NASTECH_HOME is explicitly pinned to the
        ...
        # NASTECH_HOME.  We must resolve the root explicitly instead of just
        # dropping NASTECH_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV NASTECH_HOME=/opt/data`), so an unset
        # NASTECH_HOME falls back to $HOME/.nastech = /opt/data/.nastech — an
        ...
        # update-button guard also misfires).  get_default_nastech_root()
        # returns the root for both layouts: ~/.nastech for a standard install
        ...
        try:
            from nastech_constants import get_default_nastech_root
            env["NASTECH_HOME"] = str(get_default_nastech_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop NASTECH_HOME) rather than block the reroute.
    ...
    # the same logs directory as every other Nastech surface.
    try:
        from nastech_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["NASTECH_SERVE_HEADLESS"] = "1"
    elif "NASTECH_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["NASTECH_WEB_DIST"])
            if "NASTECH_WEB_DIST" in os.environ
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom NASTECH_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "NASTECH_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # NASTECH_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["NASTECH_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ NASTECH_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset NASTECH_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads NASTECH_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["NASTECH_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from NASTECH_WEB_DIST: {_dist_root}")

    # Discover and load plugins so any DashboardAuthProvider plugin
    # (e.g. plugins/dashboard_auth/nastech) registers BEFORE start_server's
    ...
    from nastech_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with Nastech Portal."""
...
def cmd_gateway_enroll(args):
    ...
    from nastech_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_prompt_size(args):
    ...
    from nastech_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_console(args):
    """Open the safe Nastech command console."""
    from nastech_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from nastech_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "nastech", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
# so that in ``nastech -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``nastech_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``nastech -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``nastech`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``nastech <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``nastech photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that NASTECH_YOLO_MODE is set before ANY
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["NASTECH_SAFE_MODE"] = "1"
    os.environ["NASTECH_IGNORE_USER_CONFIG"] = "1"
    os.environ["NASTECH_IGNORE_RULES"] = "1"
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("NASTECH_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["NASTECH_DEFER_AGENT_STARTUP"] = "1"
            os.environ["NASTECH_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["NASTECH_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `nastech --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        print(f"  Files were in: {display_nastech_home()}/memories/\n")
    else:
        from nastech_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Nastech Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from nastech_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from nastech_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: nastech-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from nastech_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from nastech_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from nastech_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from nastech_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from nastech_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from nastech_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for nastech CLI."""
    # Cosmetic: make the process show up as 'nastech' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from nastech_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``nastech.exe.old.*`` quarantine files left by previous
    # ``nastech update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_nastech_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (nastech update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``nastech update``
    ...
    # ``nastech skills install update``) merely defers recovery one launch;
    # under-matching (missing ``nastech -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in nastech_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from nastech_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from nastech_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nastech-agent.nastechresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `nastech model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nastech-agent.nastechresearch.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from nastech_cli import secrets_cli as _secrets_cli
    from nastech_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `nastech proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nastech-agent.nastechresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``nastech proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from nastech_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # setup command  (parser built in nastech_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in nastech_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `nastech whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in nastech_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from nastech_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in nastech_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in nastech_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in nastech_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in nastech_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in nastech_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in nastech_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in nastech_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # portal command — Nastech Portal status + Tool Gateway routing
    # =========================================================================
    ...
    # =========================================================================
    from nastech_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from nastech_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in nastech_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in nastech_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in nastech_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in nastech_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in nastech_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in nastech_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in nastech_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in nastech_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.nastech/checkpoints/",
    ...
    # =========================================================================
    # import command  (parser built in nastech_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: nastech_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from nastech_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in nastech_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in nastech_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in nastech_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in nastech_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in nastech_cli/subcommands/skills.py)
    # =========================================================================
    ...
    # =========================================================================
    # plugins command  (parser built in nastech_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``nastech --help``, ``nastech version``, ``nastech logs``,
    ...
    try:
        from nastech_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Nastech shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from nastech_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from nastech_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # tools command  (parser built in nastech_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `nastech computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `nastech tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `nastech computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Nastech app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from nastech_cli.tools_config import install_cua_driver
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from nastech_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: nastech computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: nastech computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: nastech computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: nastech computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: nastech computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: nastech computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in nastech_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def _add_session_filter_args(p, default_older_help):
        ...
        p.add_argument(
            ...
            "(e.g. 'sonnet', 'gpt-5', 'nastech')",
        )
        p.add_argument(
            "--provider",
            help="Only match sessions billed through this provider "
            "(e.g. openrouter, anthropic, nastech)",
        )
        p.add_argument(
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <nastech home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    # cmd_sessions lives in nastech_cli/sessions_cmd.py (main.py decomposition).
    ...
    # =========================================================================
    # insights command  (parser built in nastech_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in nastech_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in nastech_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in nastech_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in nastech_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in nastech_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in nastech_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # for one release. The Nastech-Setup.exe success screen tells users to
    # run `nastech desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in nastech_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in nastech_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in nastech_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``nastech -c Pokemon Agent Dev`` → ``nastech -c 'Pokemon Agent Dev'``
    ...
    from nastech_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'nastech -c model' where 'model' is consumed as the
    ...
    # --yolo: set NASTECH_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (nastech hooks list, cron
    ...
    # ``nastech egress start`` refusing when credential_source=bitwarden

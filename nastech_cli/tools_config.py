...
Saves per-platform tool configuration to ~/.nastech/config.yaml under
...
# They're still in _NASTECH_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `nastech tools` → Video Generation, which walks
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``nastech tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from nastech_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``nastech tools`` checklist actually offers
    for ``platform``.
    ...
    so ``nastech tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "Nastech Subscription" — managed Firecrawl billed via Nastech
        #     subscription (requires_nastech_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Nastech Subscription" — managed FAL billed via the Nastech
        #     subscription (requires_nastech_auth + override_env_vars).
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Nastech Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Nastech Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_nastech_auth": True,
                "managed_nastech_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "Nastech Subscription" we set video_gen.provider = "fal"
    ...
    "browser": {
        ...
        # backend, never on the paid Nastech Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Nastech Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Nastech subscription (requires_nastech_auth +
        ...
        "providers": [
            ...
            },
            {
                ...
                "requires_nastech_auth": True,
                "managed_nastech_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set NASTECH_CUA_DRIVER_CMD to use a
...
# provider+model picker like `nastech model` — NOT this single-key prompt, so
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `nastech update` calls this
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        ...
        if verbose:
            ...
            # handle is ~/.nastech/logs/update.log — write straight to it so
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nastechresearch/nastech-agent:latest"
            )
            return
        ...
        # to ~/.nastech/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from nastech_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: nastech auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `nastech auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: nastech auth spotify")
        except Exception as exc:
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `nastech auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: nastech auth add xai-oauth"
                )
        elif idx == 1:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``nastech tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``nastech tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: nastech tools post-setup <key>")
        return 2
...
#:
#: Not gated on a Nastech sign-in here: the six ``bfl_flux3_*`` tools carry
#: ``check_fn=check_bfl_requirements``, so an enabled toolset still ships zero
#: schemas to a user with no Nastech credential — the same split Home Assistant
...
def _get_platform_tools(
    ...
    *,
    ...
    # "nastech-cli" (which include all _NASTECH_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[nastech-cli, spotify]`` after enabling Spotify via ``nastech
        ...
        # through ``nastech tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `nastech tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"nastech-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("nastech-")}
    ...
    # shipped as a bundled plugin but user must opt in via `nastech tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `nastech tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by nastech tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `nastech` instead of
    # `nastech-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `nastech update`/`nastech doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `nastech tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (nastech-cli, nastech-telegram, etc.)
    ...
    # Opening `nastech tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[NastechSubscriptionFeatures] = None,
) -> bool:
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        ...
        if feature and (feature.available or feature.managed_by_nastech):
            return True
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from nastech_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
# ("Nastech Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Nastech Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Nastech-managed Tool Gateway rows (``managed_nastech_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Nastech Portal
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # Nastech-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_nastech_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_nastech_auth")
            and not provider.get("managed_nastech_feature")
            and not features.nastech_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_nastech_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Nastech Subscription.
    ...
    # remaining hardcoded rows ("Nastech Subscription", "Firecrawl
    ...
    # the hardcoded "Nastech Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_nastech_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Nastech Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Nastech-managed rows were filtered out for unentitled
    ...
    (``ensure_nastech_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `nastech tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    from nastech_cli.nastech_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from nastech_cli.nastech_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Nastech Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Nastech Subscription rows).

    ``features`` (a ``NastechSubscriptionFeatures``) can be passed to avoid
    ...
    if provider.get("requires_nastech_auth") or managed_feature:
        if features is None:
            features = get_nastech_subscription_features(config)
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_nastech_message = _hidden_nastech_gateway_message(
        cat,
        config,
        f"the Nastech Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        # When the user is logged into Nastech, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Nastech.
        try:
            _nastech_logged_in = bool(
                get_nastech_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).nastech_auth_present
            )
        except Exception:
            _nastech_logged_in = False
        ...
        for p in providers:
            ...
            # Mark Nastech-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Nastech Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_nastech_feature"):
                if _nastech_logged_in:
                    sub_marker = "  ★ Included with your Nastech subscription"
                else:
                    sub_marker = "  ★ via Nastech Portal (login on select)"
    ...
    *,
    ...
    if managed_feature:
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_nastech
        ...
        if provider.get("web_backend"):
            ...
            return feature.managed_by_nastech and current == provider["web_backend"]
        return feature.managed_by_nastech
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on Nastech auth, or run
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Nastech Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # Nastech-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Nastech
    ...
    if managed_feature:
        from nastech_cli.nastech_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_nastech_portal_access,
        )

        if not ensure_nastech_portal_access(
            capability=f"{provider.get('name', 'the Nastech Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Nastech Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_nastech_auth without a managed gateway
    ...
    if provider.get("requires_nastech_auth") and not managed_feature:
        ...
        if not features.nastech_auth_present or not entitled:
            message = format_nastech_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Nastech Subscription')}",
            )
            _print_warning(
                f"  {message or 'Nastech Subscription is only available after logging into Nastech Portal.'}"
            )
            return
    ...
    # Nastech-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Nastech.
        ...
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_nastech_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_nastech_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.nastech_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Nastech Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``nastech model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `nastech model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Nastech Portal login + entitlement gate as _configure_provider:
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `nastech model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://nastech-agent.nastechresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Nastech subscription defaults", Colors.GREEN))
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, nastech mcp

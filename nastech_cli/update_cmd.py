"""Nastech update pipeline — extracted from ``nastech_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``nastech_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``nastech_cli.main`` reference — so existing call sites and test monkeypatches
that target ``nastech_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``nastech_cli.main``.
...
Imports are one-way: ``nastech_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``nastech_cli.main`` reference.

    Lets callers keep patching ``nastech_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``nastech_cli.main`` -> ``nastech_cli.update_cmd`` stays one-way
    at import time.
    """
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``nastech update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``nastech update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``nastech_cli.config_defaults``,
    ``nastech_cli.config``, and ``nastech_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.
    """
    import importlib

    importlib.invalidate_caches()
    for mod_name in ("nastech_cli.config_defaults", "nastech_cli.config", "nastech_cli.config_migrations"):
...
def _run_config_check_fresh() -> tuple:
    ...
    from nastech_cli.config import check_config_version
...
# Critical files that Nastech must be able to import immediately after an
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="nastech-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``nastech update`` can be driven by a
    ...
    """
    from nastech_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from nastech_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('nastech_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``nastech update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `nastech update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `nastech update`.
    ...
    try:
        ...
        return
    db_path = get_nastech_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `nastech sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    nastech sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  nastech sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``nastech update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``nastech update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: nastech curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    nastech dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``nastech_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.nastech-update-staging"
    backup = f"{dst}.nastech-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `nastech update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``nastech_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("NASTECH_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== nastech-update completed {action_id} ===")


def _update_via_zip(args):
    """Update Nastech Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `nastech update "
            f"--branch {branch}`, or update against main with `nastech update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/NastechResearch/nastech-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="nastech-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"nastech-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for nastech-agent itself
        ...
        # GitHub ZIPs extract to nastech-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"nastech-agent-{branch}")
    ...
    if not import_ok:
        ...
        print("  Re-run `nastech update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: nastech skills list-modified  "
    ...
    try:
        from nastech_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from nastech_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_nastech_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_nastech_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "nastech-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Nastech behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes nastech completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Nastech couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Nastech couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Nastech behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Nastech couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Nastech couldn't drop "
...
OFFICIAL_REPO_URLS = {
    ...
    "git@github.com:NastechResearch/nastech-agent",
}

OFFICIAL_REPO_URL = "https://github.com/NastechResearch/nastech-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_nastech_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_nastech_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NastechResearch/nastech-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NastechResearch/nastech-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NastechResearch/nastech-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``nastech update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from nastech_constants import get_default_nastech_root

    default_home = get_default_nastech_root()
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another nastech.exe is running:"]
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``nastech update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `nastech update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `nastech update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Nastech' editable-install extras or
    ...
    try:
        from nastech_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$NASTECH_HOME/bin/uv``, but on Termux the official
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `nastech update` is exactly the step
...
def _npm_lockfile_changed(nastech_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `nastech
    ...
    try:
        ...
        cache_file = nastech_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(nastech_root: Path) -> None:
...
def _log_only_write(text: str) -> None:
    ...
    During ``nastech update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``nastech update --check``: fetch and report without installing.
    ...
    """
    from nastech_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``nastech update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``nastech update`` without requiring a reinstall.  The
    ...
    ``nastech`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/nastech.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v nastech'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/nastech, code at /usr/local/lib/nastech-agent).
    fhs_link = Path("/usr/local/bin/nastech")
    ...
    path_comment = (
        "# Nastech Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``nastech-acp`` launcher next to the ``nastech`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``nastech update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``nastech-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Nastech as not installed even when it is.

    The shim simply delegates to the sibling ``nastech`` launcher with the
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        nastech_cmd = bin_dir / "nastech"
        acp_cmd = bin_dir / "nastech-acp"
        try:
            if not (nastech_cmd.is_file() or nastech_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of NASTECH_HOME under
      ``backups/`` (restorable via ``nastech import``; the #48200 wrong-path
    ...
    try:
        ...
        # NOTE: this function later does `from nastech_constants import
        # get_nastech_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from nastech_cli.config import get_nastech_home as _get_home
    ...
    try:
        ...
        home = get_nastech_home()
        try:
            display_path = f"{display_nastech_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  nastech import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``nastech update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``nastech update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run nastech from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.nastech-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".nastech-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The nastech.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m nastech_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``nastech
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m nastech_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "nastech_cli.main" in cmdline_low:
        ...
        # the `-m nastech_cli.main gateway run` argv, so autostarted gateways
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Nastech processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Nastech Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Nastech desktop app / other Nastech terminals, then re-run:"
    )
    lines.append("    nastech update")
    lines.append("  (or use `nastech update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``nastech update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from nastech_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning nastech-setup, and the update-in-progress marker
    ...
    the update with "Nastech is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Nastech backend (``nastech_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.nastech-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "nastech_cli.main" in argv_low and (
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.nastech-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    nastech.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Nastech...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m nastech_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: nastech gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``nastech gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``nastech-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("nastech-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``nastech update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``nastech-setup.exe``) executes
    ``$NASTECH_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_nastech_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``nastech update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``nastech update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_nastech_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `nastech serve` backend) will keep .pyd
    ...
    # the nastech.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .nastech-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Nastech is still
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Nastech gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in nastech_cli/config.py bricked
        # every user who ran ``nastech update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``nastech
        ...
        # the old bytecode (e.g. get_nastech_home added to nastech_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``nastech`` launch finishes the install
        ...
        # symbols in nastech_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `nastech` again "
        ...
        # build.  ``nastech desktop --build-only`` uses the content-hash stamp
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `nastech desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                # debuggable.
                #
                # Start the build subprocess with the Nastech-managed Node on PATH:
                # when `nastech update` runs inside the desktop updater chain
                # (Desktop → nastech-setup → nastech update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from nastech_constants import with_nastech_node_path

                _build_env = with_nastech_node_path()
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_nastech_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.nastech/cache/model_catalog.json
        ...
        try:
            from nastech_cli.profiles import backfill_profile_envs
        ...
        # ``nastech update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``nastech_cli.config`` and
        # ``nastech_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from nastech_cli.config import
        ...
        if version_bump_only:
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'nastech config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                print()
                print("Skipped. Run 'nastech config migrate' later to configure.")
        else:
        ...
        # check `nastech curator status`. Self-stamps after printing so it
        ...
        # Self-heal the nastech-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Nastech on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("nastech-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``nastech update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `nastech update`
                # must stay fast; `nastech computer-use install --upgrade`
        ...
        # When running as ``nastech update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_nastech_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... nastech-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... nastech-gateway*)
            ...
            try:
                from nastech_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all nastech-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: nastech gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `nastech update` restart it automatically, allow\n"
                        ...
                        # path in `nastech gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    if unmapped_count > 1:
                        print(
                            "    (or: nastech -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Nastech gateway unit files are still installed.
        # When both nastech.service (from a pre-rename install) and the
        # current nastech-gateway.service are enabled, they SIGTERM-fight
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_nastech_units():
                print()
                print("⚠ Legacy Nastech gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_nastech_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (nastech.service) fight the current")
                print("  nastech-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    nastech gateway migrate-legacy")
                print()
        ...
        print("  nastech model              # Select provider and model")

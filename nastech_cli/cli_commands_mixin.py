...
``cli.py``'s ``NastechCLI`` class. ``NastechCLI`` inherits ``CLICommandsMixin`` so
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: nastech --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Nastech changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Nastech hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Nastech config/state.
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from nastech_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `nastech profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: nastech -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `nastech journey`).
    ...
    def _handle_paste_command(self):
        ...
        from nastech_cli.clipboard import has_clipboard_image
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in nastech_cli/colors.py
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if target.isdigit():
            ...
            from nastech_cli.main import _resolve_session_by_name_or_id
        ...
        # same directory as a startup `nastech -c`/`--resume`. The startup resume
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        All resolution/persistence goes through nastech_cli.personality —
        ...
        if len(parts) > 1:
            ...
            if not name:
                ...
                try:
                    from nastech_cli.config import cfg_get, read_raw_config
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from nastech_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        from nastech_cli.pets import _set_active
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to nastech_cli.curator so the CLI and the `nastech curator`
        ...
        try:
            from nastech_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to nastech_cli.skills_hub."""
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``nastech bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: nastech bundles create "
        ...
        _cprint(
            ...
            f"Manage with `nastech bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Nastech-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `nastech cron`.
        """
        from cli import _DIM, _RST, _cprint
        from nastech_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from nastech_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Nastech process runs — use `nastech cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Nastech keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from nastech_cli.goals import draft_contract
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="nastech_prompt_")
        try:
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from nastech_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from nastech_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from nastech_cli.focus_view import format_hidden_line
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Nastech is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Nastech is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from nastech_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug nastech``   → upload to Nastech-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``nastech`` and ``local`` are mutually exclusive; if both are given,
        ...
        nastech = "nastech" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, nastech=nastech, yes=True
        )
        run_debug_share(args)

    def _handle_update_command(self) -> bool:
        """Handle /update — update Nastech Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``nastech update`` so the user sees update output directly and gets
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Nastech Agent"),
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Nastech' hotword listener.

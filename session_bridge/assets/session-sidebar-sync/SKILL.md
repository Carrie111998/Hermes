...
| Hydration and registration are both actionable | Claim one hydration job; do not call the registration pending tool. |
...
| Any Claude or Hermes registration delivery | Use only the saved `Session Inbox` project rooted at the resolved canonical local `.hermes` path. Keep the exact source cwd as authenticated readable metadata and the secondary runtime root, never as project selection. |
| Any in-place hydration delivery | Authenticate the exact linked task in that same `Session Inbox`; never reselect placement from the source cwd or git root. |
...
Call `session_status` exactly once before any native project call or lease. Require `health.running` and the watcher to be running. Require every reported provider entry to contain structurally valid `last_success`, `lag_seconds`, and `degraded_reason` fields. Preserve any non-null `degraded_reason` as provider-isolated health information; it must not globally block healthy queued delivery from another provider. Require the sanitized placement status to contain the exact resolved canonical local `Session Inbox` cwd and placement generation `1`, and read the registration and hydration pending/retry counts without broadening the request. If status is unavailable or malformed, the Session Inbox is unavailable, a global health requirement fails, or all four counts are zero, end immediately with no user-facing message and consume no job attempt.

Call the native tool `list_projects({})` exactly once before leasing either queue. Build the same canonical local project map required by the registration procedure and require exactly one local saved project whose canonical path equals the resolved Session Inbox cwd. The source cwd and git root are never project candidates. If project preflight or canonical inbox lookup fails, end without leasing and consume no job attempt.

Prefer one actionable hydration pending or retry job over registration. When either hydration count is nonzero, call `session_sidebar_hydration_pending(limit=1)` exactly once and follow the in-place hydration procedure. Otherwise call `session_sidebar_pending(limit=1)` exactly once and follow the registration procedure. Do not call both pending tools in one wake. Claim and process at most one total lease per wake.

## In-place Hydration Procedure

- **Select.** Use the existing project map and select only the saved `Session Inbox` project whose canonical path equals the resolved canonical local `.hermes` inbox cwd. Never select hydration placement from the exact source cwd or git root; those remain source identity only.
- **Read.** For the single hydration lease, require exact non-empty `lease_token`, `codex_thread_id`, `source_session_id`, `hydration_marker`, and `hydration_message`. The returned `codex_thread_id` is the only permitted target. Call `read_thread({"threadId":"<exact codex_thread_id>","turnLimit":10,"includeOutputs":false})` before any reservation or send. Pass no other fields.
- **Authenticate.** Authenticate the read response's nested task against the exact linked task ID, normalized local host, resolved Session Inbox project/cwd, and the legacy signed source registration marker for the returned source session. A missing or unreadable task maps to `native_task_not_indexed`; a source or legacy marker mismatch maps to `source_identity_mismatch` or `marker_conflict`; an explicit host, inbox project/cwd, environment, or task-kind contradiction maps to the hydration lane's fixed `codex_thread_conflict`. Settle the hydration lease once with `session_sidebar_hydration_fail`, always including the exact `codex_thread_id`, then end the wake. Never send hydration to a source-cwd, git-root, or projectless task.
- **Reconcile.** Search only the returned turns of that exact task for the exact hydration marker. If the marker is present in an authenticated completed turn and no active turn, approval request, user-input request, or system error remains, call `session_sidebar_hydration_commit(lease_token=<exact token>, codex_thread_id=<exact id>, hydration_marker=<exact marker>)` and end. Do not send again.
...
- **Reserve and send.** Otherwise call `session_sidebar_hydration_reserve(lease_token=<exact token>)` immediately before `send_message_to_thread({"threadId":"<exact id>","message":"<hydration_message verbatim>"})`. Send the returned `hydration_message` verbatim to the exact linked task ID. Do not add commentary, preview text, or instructions of your own.
...
4. Treat the Session Inbox as presentation placement only. Registration starts with the resolved Session Inbox as native cwd and never runs project commands. The exact source cwd remains authenticated readable metadata and the secondary runtime workspace root; it remains authoritative for every later command and file operation after continuation.
5. When `reconcile_required` is true, `recovered_thread_id` is returned, or `create_reserved` is true, reconcile before creating anything. Extract the exact authenticated signed marker from `registration_prompt`.
   ...
   - a project or cwd placement contradiction maps to `placement_mismatch`, while an explicit host, environment, or task-kind contradiction maps to `codex_thread_conflict`; never continue to creation after a candidate summary was returned;
...
- Never use source cwd or git root as native placement or project selection; preserve source cwd only as authenticated metadata and a runtime root.
...
Before ending, verify that status was called once, projects were listed at most once and only before pending, the resolved Session Inbox project was the only selected project, and pending was called at most once and only after both preflights passed. If a task was created, verify reserve definitely succeeded first, `registration_prompt` was used verbatim, native cwd was the inbox, ordered runtime roots were inbox then exact source cwd with filesystem-equivalent duplicates removed, and the returned exact ID was supplied to bind and to any later fail/release. If a task was reconciled, verify only an inbox-placed authenticated recovered ID was reused and no create was attempted. If a newly created task reached native-read polling, verify its exact ID was durably bound before that polling. If a reconciled candidate was read before binding, verify that read was used only to authenticate inbox placement and source identity and no rename or commit preceded a successful bind. If exact-ID polling reached an authenticated quiescent registration, verify rename was attempted only afterward. If exact-ID polling failed or timed out, verify no rename or commit was attempted and the exact ID was supplied to fail/release. If rename succeeded, verify the exact returned title was used before commit. If bind or rename failed, verify no commit was attempted and the exact ID was supplied to fail/release. If commit succeeded, verify that exact lease and thread ID became visible. If commit did not succeed, verify the noncommitted lease had exactly one fail/release attempt with a fixed code, whether successful, failed, or ambiguous.

...
Call `session_status` exactly once before any native project call or lease. Require `health.running` and the watcher to be running, require every reported provider `degraded_reason` to be null, and read the registration and hydration pending/retry counts without broadening the request. If status is unavailable or malformed, a health requirement fails, or all four counts are zero, end immediately with no user-facing message and consume no job attempt.

Call the native tool `list_projects({})` exactly once before leasing either queue. Build the same canonical local project map required by the registration procedure. If project preflight fails, end without leasing and consume no job attempt.
...
- **Select.** Use the existing project map and select the saved project in this exact order: exact source cwd, exact git root, then the saved `Session Inbox` project rooted at the canonical local `.hermes` path. Project grouping does not change the authoritative source cwd.
...
- **Authenticate.** Authenticate the read response's nested task against the exact linked task ID, normalized local host, chosen project/cwd, and the legacy signed source registration marker for the returned source session. A missing or unreadable task maps to `native_task_not_indexed`; a source or legacy marker mismatch maps to `source_identity_mismatch` or `marker_conflict`; an explicit host, project, cwd, environment, or task-kind contradiction maps to `codex_thread_conflict`. Settle the hydration lease once with `session_sidebar_hydration_fail`, always including the exact `codex_thread_id`, then end the wake.

...
Call `session_status` exactly once before any native project call or lease. Require `health.running` and the watcher to be running, require every reported provider `degraded_reason` to be null, require the sanitized placement status to contain the exact resolved canonical local `Session Inbox` cwd and placement generation `1`, and read the registration and hydration pending/retry counts without broadening the request. If status is unavailable or malformed, the Session Inbox is unavailable, a health requirement fails, or all four counts are zero, end immediately with no user-facing message and consume no job attempt.

...
- **Authenticate.** Authenticate the exact linked task ID, source marker, bridge ID, preview digest, no substantive work, and quiescence. A projectless legacy task is valid and remains valid. Exact authenticated projectless legacy hydration is exempt and valid. A missing or unreadable task maps to `native_task_not_indexed`; a source or marker mismatch maps to `source_identity_mismatch` or `marker_conflict`; an explicit host, environment, or task-kind contradiction maps to `codex_thread_conflict`. Settle once with `session_sidebar_hydration_fail`, always including the exact `codex_thread_id`.

...
- **Authenticate.** Authenticate the exact linked task ID, source marker, bridge ID, preview digest, no substantive work, and quiescence. A projectless legacy task is valid and remains valid. A missing or unreadable task maps to `native_task_not_indexed`; a source or marker mismatch maps to `source_identity_mismatch` or `marker_conflict`; an explicit host, environment, or task-kind contradiction maps to `codex_thread_conflict`. Settle once with `session_sidebar_hydration_fail`, always including the exact `codex_thread_id`.
- **Reconcile.** Search only the returned turns of that exact task for the exact hydration marker before send whenever `send_reserved` is true. If present in an authenticated completed turn and quiescent, call `session_sidebar_hydration_commit(lease_token=<exact token>, codex_thread_id=<exact id>, hydration_marker=<exact marker>)` and end. Do not send again.

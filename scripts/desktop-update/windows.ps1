...
# (%ORION_HOME%\orion-setup.exe). That binary has no self-update path --
# copy_self_to_orion_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `orion update` refreshes
...
#     -InstallRoot <path>   repo checkout (ORION_HOME\orion-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Orion.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .orion-update-in-progress in place (tests)
#
...
# .orion-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim ORION_HOME\.orion-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining ORION_UPDATE_STARTED_AT from the Desktop hand-off.
# orion_cli/update_lock.py's ancestry rule lets our
# `orion update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Orion.exe pid.
try {
    Add-Type -Namespace OrionHandoff -Name Win32 -MemberDefinition @'
...
$OrionHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $OrionHome ".orion-update-in-progress"
$LogDir = Join-Path $OrionHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $OrionHome ".orion-update-result.json"
$script:Ui = $null
$script:UiStage = "Orion will open once done."   # until the first gate; matches ui.html
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-OrionStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $edge) {
        ...
        if ($server) {
            try {
                ...
                $edgeProfile = Join-Path $TempDir ("orion-update-ui-{0}" -f $PID)
                $edgeArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [OrionHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `orion debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"orion debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [OrionHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Orion.exe before writing the replacement,
    ...
    # DO NOT spawn Orion.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [OrionHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [OrionHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [OrionHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Orion process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [OrionHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `orion update` deliberately runs its build steps with stdout
# inherited (orion_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:ORION_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:ORION_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
function Invoke-OrionStep([string]$Exe, [string[]]$OrionArgs, [string]$Tag) {
    ...
    # afterwards, where `orion debug share` picks it up.
    #
    ...
    # .orion-update-result.json, clearing .orion-update-in-progress,
    ...
    # Orion" until the user killed something by hand. Losing the tail of a
    ...
    $psi.Arguments = ($OrionArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    $psi.UseShellExecute = $false
    $psi.RedirectStandardOutput = $true
    $psi.RedirectStandardError = $true
    # orion update prints UTF-8 (checkmarks, arrows, box glyphs). PS 5.1
...
# the page can be QA'd in any browser; ORION_SELFTEST_FAIL=1 exercises the
# error state, ORION_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:ORION_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:ORION_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:ORION_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-OrionStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Orion install to
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:ORION_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:ORION_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:ORION_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:ORION_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "orion-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "orion-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "orion-pipe-drain-$stamp.pid"
...
'@
    ...
    # merely loud. `orion update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-OrionStep $powershell @(
    ...
    $flood = Invoke-OrionStep $powershell @(
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:ORION_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Orion to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Orion window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Orion fully and try again."
    ...
    Publish-UiProgress "Preparing Orion files"
    $shim = Join-Path $InstallRoot "venv\Scripts\orion.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Orion install open (venv\Scripts\orion.exe locked after 20s). Nothing was changed. Close other Orion windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the orion.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\orion.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_orion_exe must first rename the running orion.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that orion.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Orion window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Orion.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m orion_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Orion installer or `orion doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "orion_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Orion windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m orion_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed orion predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-OrionStep $pythonExe $updateArgs "update"
    Write-HandoffLog "orion update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all Orion windows") is not retryable.
    ...
    # `orion update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "orion update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-OrionStep $pythonExe @("-m", "orion_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `orion desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `orion debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Orion is back",
    ...
    if ($finalCode -ne 0) {
        ...
        Publish-UiProgress "Opening Orion"
        ...
        if (-not $cameBack -and $RelaunchExe) {
            ...
            $finalMsg = "Update complete. Reopen Orion to finish (it could not restart itself)."

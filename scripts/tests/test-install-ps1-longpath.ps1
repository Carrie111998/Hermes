...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[orion] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[orion`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[orion`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        OrionHome = $(if ($paths) { $paths.orion_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\orion\orion-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-orion'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-orion') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"

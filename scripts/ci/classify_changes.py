...
# tree generates llms.txt — the index every LLM (Orion included, via the
# orion-agent skill) reads to learn what Orion can do — and
...
_MCP_CATALOG_FILES = {"orion_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Orion-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``orion serve`` backend, the Docker image)

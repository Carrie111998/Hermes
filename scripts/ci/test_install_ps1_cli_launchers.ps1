# Behavioral test for install.ps1's orion launcher staging (PR #92092,
...
#
# The test lifts the real Install-OrionCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $OrionHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-OrionCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-OrionCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'orion-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'orion-agent'
    ...
    $missingThrew = $false
    try {
        Install-OrionCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing orion.exe fails the launcher stage'
    ...
    $orionV1 = [byte[]](77, 90, 1)
    $orionV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'orion.exe'), $orionV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-OrionCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $orionV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'orion.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'orion-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'orion.exe'), $orionV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'orion-acp.exe'), $acp)
    Install-OrionCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $orionV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'orion.exe'))) `
        'installer refreshes an existing Orion launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'orion-acp.exe'))) `
    ...
    Install-OrionCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'orion.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'orion.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'orion.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'orion.exe')) -and $cmdBody.Contains('%*')) `

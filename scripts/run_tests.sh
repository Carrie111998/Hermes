#!/usr/bin/env bash
# Canonical test runner for orion-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.orion/...)
#
# Usage:
...
# (ORION_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.orion/orion-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.orion/orion-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${ORION_PYTHON:-}" ] && [ -x "$ORION_PYTHON" ] \
    && "$ORION_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ORION_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `orion` binary rather
  # than the devShell hook.
  PYTHON="$ORION_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ORION_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ORION_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.orion/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.orion"
...
#
#   * ORION_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * ORION_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     ORION_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# ORION_RUN_SLOW_PET_TESTS / ORION_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no ORION_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in ORION_TEST_IMAGE ORION_TEST_WORKERS ORION_TEST_PATHS \
  ORION_TEST_FILE_TIMEOUT ORION_TEST_FILE_RETRIES ORION_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${ORION_RUN_SLOW_PET_TESTS:+ORION_RUN_SLOW_PET_TESTS="$ORION_RUN_SLOW_PET_TESTS"} \
  ${ORION_E2E_BROWSER:+ORION_E2E_BROWSER="$ORION_E2E_BROWSER"} \

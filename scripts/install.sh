...
# Python-driven tool session (e.g. Nastech terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NastechResearch/nastech-agent.git"
REPO_URL_HTTPS="https://github.com/NastechResearch/nastech-agent.git"
NASTECH_HOME="${NASTECH_HOME:-$HOME/.nastech}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${NASTECH_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$NASTECH_INSTALL_DIR"
...
#   code at /usr/local/lib/nastech-agent, command at /usr/local/bin/nastech,
#   data still at /root/.nastech (NASTECH_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$NASTECH_HOME/.no-bundled-skills so future"
            echo "                   'nastech update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Nastech.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.nastech/nastech-agent"
            echo "                   default (root, Linux): /usr/local/lib/nastech-agent"
            echo "  --nastech-home PATH  Data directory (default: ~/.nastech, or \$NASTECH_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Nastech installs the code under"
            echo "  /usr/local/lib/nastech-agent and links the command into"
            echo "  /usr/local/bin/nastech (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$NASTECH_HOME"
            echo "  (default /root/.nastech).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$NASTECH_HOME/nastech-agent are preserved in-place."
...
print_banner() {
    ...
    echo -e "${MAGENTA}${BOLD}"
    echo "┌─────────────────────────────────────────────────────────┐"
    echo "│             𓄃 Nastech Agent Installer                   │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Nastech Research.           │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `nastech update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Nastech-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Nastech Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install nastech command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `nastech` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $NASTECH_HOME/nastech-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $NASTECH_HOME/nastech-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/nastech-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $NASTECH_HOME/nastech-agent — then preserve it)
#
# Always no-op when the user set --dir or $NASTECH_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in NASTECH_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$NASTECH_HOME/nastech-agent/.git" ]; then
            INSTALL_DIR="$NASTECH_HOME/nastech-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/nastech-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/nastech-agent"
        ...
        # /usr/local/bin/nastech wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/nastech"
        log_info "  Data:    $NASTECH_HOME (unchanged)"
...
# Point a Nastech-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $NASTECH_HOME/node/bin — which is NOT on
...
# Nastech's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Nastech-managed npm, so calling it on
...
configure_managed_node_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$NASTECH_HOME/node/etc/npmrc"
}

get_nastech_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/nastech" ]; then
        echo "$link_dir/nastech"
    else
        echo "nastech"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://nastech-agent.nastechresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Nastech owns its own uv at $NASTECH_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (nastech_cli/managed_uv.py) looks in the same
    # place, so install.sh and `nastech update` stay in sync.
    local _managed_uv="$NASTECH_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $NASTECH_HOME/bin ..."
    mkdir -p "$NASTECH_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/nastech-uv-install.$$.log")"
    ...
    # directly into $NASTECH_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$NASTECH_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Nastech-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Nastech-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && node_satisfies_build "$(node --version)"; then
        ...
        log_warn "min-release-age-exclude) — installing Nastech-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Nastech-managed Node from a previous run over a too-old system one.
    if [ -x "$NASTECH_HOME/node/bin/node" ] && node_satisfies_build "$("$NASTECH_HOME/node/bin/node" --version)"; then
        export PATH="$NASTECH_HOME/node/bin:$PATH"
        log_success "Node.js $("$NASTECH_HOME/node/bin/node" --version) found (Nastech-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Nastech requires Node >=26) — installing Nastech-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.nastech/node/..."
    ...
    # Place into ~/.nastech/node/ and symlink binaries into the same bin dir
    # the nastech command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$NASTECH_HOME/node"
    mkdir -p "$NASTECH_HOME"
    mv "$extracted_dir" "$NASTECH_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$NASTECH_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$NASTECH_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$NASTECH_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$NASTECH_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.nastech/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Nastech install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Nastech Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `nastech update` path
                # (#4735).
                ...
                local stash_name
                stash_name="nastech-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Nastech behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"nastech-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up nastech command..."

    if [ "$USE_VENV" = true ]; then
        NASTECH_BIN="$INSTALL_DIR/venv/bin/python"
        NASTECH_ENTRYPOINT="$INSTALL_DIR/nastech"
    else
        NASTECH_BIN="$(which nastech 2>/dev/null || echo "")"
        if [ -z "$NASTECH_BIN" ]; then
            log_warn "nastech not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$NASTECH_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$NASTECH_ENTRYPOINT" ]; }; then
        log_warn "Nastech launcher prerequisites not found"
    ...
    # Create a user-facing shim for the nastech command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $NASTECH_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$NASTECH_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/nastech"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/nastech" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NASTECH_BIN" "$NASTECH_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/nastech" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/nastech"
    log_success "Installed nastech launcher → $command_link_display_dir/nastech"

    # Also expose `nastech-agent`. The `nastech-agent` console script declared in
    ...
    rm -f "$command_link_dir/nastech-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/nastech-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NASTECH_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/nastech-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NASTECH_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/nastech-agent"
    log_success "Installed nastech-agent launcher → $command_link_display_dir/nastech-agent"

    # Also expose `nastech-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `nastech-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Nastech as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/nastech-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/nastech-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NASTECH_BIN" "$NASTECH_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/nastech-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NASTECH_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/nastech-acp"
    log_success "Installed nastech-acp launcher → $command_link_display_dir/nastech-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "nastech command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v nastech' \
                >/dev/null 2>&1; then
        ...
        log_info "nastech not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Nastech Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Nastech Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so nastech works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.nastech directory structure (config at top level, code in subdir)
    mkdir -p "$NASTECH_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.nastech/.env (top level, easy to find)
    if [ ! -f "$NASTECH_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$NASTECH_HOME/.env"
            log_success "Created ~/.nastech/.env from template"
        else
            touch "$NASTECH_HOME/.env"
            log_success "Created ~/.nastech/.env"
        fi
    else
        log_info "~/.nastech/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$NASTECH_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.nastech/config.yaml (top level, easy to find)
    if [ ! -f "$NASTECH_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$NASTECH_HOME/config.yaml"
            log_success "Created ~/.nastech/config.yaml from template"
        fi
    else
        log_info "~/.nastech/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in nastech_cli/default_soul.py — the
    ...
    if [ ! -f "$NASTECH_HOME/SOUL.md" ]; then
        cat > "$NASTECH_HOME/SOUL.md" << 'SOUL_EOF'
You are Nastech Agent, an intelligent AI assistant created by Nastech Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.nastech/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.nastech/"

    # Seed bundled skills into ~/.nastech/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `nastech update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            > "$NASTECH_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $NASTECH_HOME/.no-bundled-skills"
        log_info "  Future 'nastech update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.nastech/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.nastech/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$NASTECH_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$NASTECH_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.nastech/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$NASTECH_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Nastech Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Nastech will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Nastech Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (nastech --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `nastech update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'nastech setup' after install."
        return 0
    fi
    ...
    # Run nastech setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m nastech_cli.main setup < /dev/tty
    else
        python -m nastech_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Nastech to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$NASTECH_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'nastech whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                NASTECH_CMD="$(get_nastech_command_path)"
                $NASTECH_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'nastech whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'nastech gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $NASTECH_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $NASTECH_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: nastech gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: nastech gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $NASTECH_CMD gateway > "$NASTECH_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.nastech/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: nastech gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: nastech gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.nastech-bootstrap-complete, which tells the Nastech
    ...
    local marker_path="$INSTALL_DIR/.nastech-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $NASTECH_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $NASTECH_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $NASTECH_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}nastech${NC}              Start chatting"
    echo -e "   ${GREEN}nastech setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}nastech config${NC}       View/edit configuration"
    echo -e "   ${GREEN}nastech config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}nastech gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}nastech update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'nastech' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'nastech' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'nastech' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$NASTECH_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$NASTECH_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$NASTECH_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$NASTECH_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$NASTECH_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
# extract a tree MISSING the electron binary, so the `electron`->`Nastech` rename
...
# _purge_electron_build_cache() used by `nastech desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Nastech-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `nastech desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Nastech" ]; then
            app="$desktop_dir/release/linux-unpacked/Nastech"
        elif [ -x "$desktop_dir/release/linux-unpacked/nastech" ]; then
            app="$desktop_dir/release/linux-unpacked/nastech"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Nastech.app" \
            "$desktop_dir/release/mac/Nastech.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `nastech desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading NASTECH_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if NASTECH_HOME="$NASTECH_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from nastech_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `nastech update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Nastech-managed Node
            # provisioned during prerequisites/node-deps (at $NASTECH_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $NASTECH_HOME.
    # $NASTECH_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'nastech update' on this host install.

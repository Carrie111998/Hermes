...
# Python-driven tool session (e.g. Orion terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Orion/orion-agent.git"
REPO_URL_HTTPS="https://github.com/zacharyjleach-stack/Aries.git"
ORION_HOME="${ORION_HOME:-$HOME/.orion}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ORION_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ORION_INSTALL_DIR"
...
#   code at /usr/local/lib/orion-agent, command at /usr/local/bin/orion,
#   data still at /root/.orion (ORION_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ORION_HOME/.no-bundled-skills so future"
            echo "                   'orion update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Orion.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.orion/orion-agent"
            echo "                   default (root, Linux): /usr/local/lib/orion-agent"
            echo "  --orion-home PATH  Data directory (default: ~/.orion, or \$ORION_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Orion installs the code under"
            echo "  /usr/local/lib/orion-agent and links the command into"
            echo "  /usr/local/bin/orion (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ORION_HOME"
            echo "  (default /root/.orion).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ORION_HOME/orion-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Orion Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Orion.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `orion update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Orion-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Orion Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install orion command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `orion` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ORION_HOME/orion-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ORION_HOME/orion-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/orion-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ORION_HOME/orion-agent — then preserve it)
#
# Always no-op when the user set --dir or $ORION_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ORION_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ORION_HOME/orion-agent/.git" ]; then
            INSTALL_DIR="$ORION_HOME/orion-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/orion-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/orion-agent"
        ...
        # /usr/local/bin/orion wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/orion"
        log_info "  Data:    $ORION_HOME (unchanged)"
...
# Point a Orion-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ORION_HOME/node/bin — which is NOT on
...
# Orion's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Orion-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ORION_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ORION_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ORION_HOME/node/etc/npmrc"
}

get_orion_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/orion" ]; then
        echo "$link_dir/orion"
    else
        echo "orion"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://your-orion-docs.example/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Orion owns its own uv at $ORION_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (orion_cli/managed_uv.py) looks in the same
    # place, so install.sh and `orion update` stay in sync.
    local _managed_uv="$ORION_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ORION_HOME/bin ..."
    mkdir -p "$ORION_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/orion-uv-install.$$.log")"
    ...
    # directly into $ORION_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ORION_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Orion-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Orion-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Orion-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Orion-managed Node from a previous run over a too-old system one.
    if [ -x "$ORION_HOME/node/bin/node" ] && [ -x "$ORION_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$ORION_HOME/node/bin/node" --version)"; then
        export PATH="$ORION_HOME/node/bin:$PATH"
        log_success "Node.js $("$ORION_HOME/node/bin/node" --version) found (Orion-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Orion-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Orion requires Node >=26) — installing Orion-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.orion/node/..."
    ...
    # Place into ~/.orion/node/ and symlink binaries into the same bin dir
    # the orion command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ORION_HOME/node"
    mkdir -p "$ORION_HOME"
    mv "$extracted_dir" "$ORION_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ORION_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ORION_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ORION_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ORION_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.orion/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Orion install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Orion Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `orion update` path
                # (#4735).
                ...
                local stash_name
                stash_name="orion-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``orion update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Orion behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up orion command..."

    if [ "$USE_VENV" = true ]; then
        ORION_BIN="$INSTALL_DIR/venv/bin/python"
        ORION_ENTRYPOINT="$INSTALL_DIR/orion"
    else
        ORION_BIN="$(which orion 2>/dev/null || echo "")"
        if [ -z "$ORION_BIN" ]; then
            log_warn "orion not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$ORION_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$ORION_ENTRYPOINT" ]; }; then
        log_warn "Orion launcher prerequisites not found"
    ...
    # Create a user-facing shim for the orion command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ORION_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ORION_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/orion"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/orion" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ORION_BIN" "$ORION_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/orion" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/orion"
    log_success "Installed orion launcher → $command_link_display_dir/orion"

    # Also expose `orion-agent`. The `orion-agent` console script declared in
    ...
    rm -f "$command_link_dir/orion-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/orion-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ORION_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/orion-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ORION_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/orion-agent"
    log_success "Installed orion-agent launcher → $command_link_display_dir/orion-agent"

    # Also expose `orion-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `orion-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Orion as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/orion-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/orion-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ORION_BIN" "$ORION_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/orion-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/orion-acp"
    log_success "Installed orion-acp launcher → $command_link_display_dir/orion-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "orion command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v orion' \
                >/dev/null 2>&1; then
        ...
        log_info "orion not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Orion Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Orion Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so orion works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.orion directory structure (config at top level, code in subdir)
    mkdir -p "$ORION_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.orion/.env (top level, easy to find)
    if [ ! -f "$ORION_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ORION_HOME/.env"
            log_success "Created ~/.orion/.env from template"
        else
            touch "$ORION_HOME/.env"
            log_success "Created ~/.orion/.env"
        fi
    else
        log_info "~/.orion/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.orion/config.yaml (top level, easy to find)
    if [ ! -f "$ORION_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ORION_HOME/config.yaml"
            log_success "Created ~/.orion/config.yaml from template"
        fi
    else
        log_info "~/.orion/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in orion_cli/default_soul.py — the
    ...
    if [ ! -f "$ORION_HOME/SOUL.md" ]; then
        cat > "$ORION_HOME/SOUL.md" << 'SOUL_EOF'
You are Orion Agent, an intelligent AI assistant created by Orion. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.orion/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.orion/"

    # Seed bundled skills into ~/.orion/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `orion update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            > "$ORION_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ORION_HOME/.no-bundled-skills"
        log_info "  Future 'orion update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.orion/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.orion/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ORION_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ORION_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.orion/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Orion Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Orion will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Orion Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `orion update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `orion tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$ORION_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'orion tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `orion tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `orion computer-use install` on their own).
    ...
    # Same upstream installer `orion computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Orion install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Orion'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'orion tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: orion computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'orion setup' after install."
        return 0
    fi
    ...
    # Run orion setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m orion_cli.main setup < /dev/tty
    else
        python -m orion_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Orion to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ORION_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'orion whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ORION_CMD="$(get_orion_command_path)"
                $ORION_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'orion whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'orion gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ORION_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ORION_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: orion gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: orion gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ORION_CMD gateway > "$ORION_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.orion/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: orion gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: orion gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.orion-bootstrap-complete, which tells the Orion
    ...
    local marker_path="$INSTALL_DIR/.orion-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ORION_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ORION_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ORION_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}orion${NC}              Start chatting"
    echo -e "   ${GREEN}orion setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}orion config${NC}       View/edit configuration"
    echo -e "   ${GREEN}orion config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}orion gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}orion update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'orion' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'orion' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'orion' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ORION_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ORION_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ORION_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `orion update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ORION_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Orion` rename
...
# _purge_electron_build_cache() used by `orion desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Orion-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `orion desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Orion" ]; then
            app="$desktop_dir/release/linux-unpacked/Orion"
        elif [ -x "$desktop_dir/release/linux-unpacked/orion" ]; then
            app="$desktop_dir/release/linux-unpacked/orion"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Orion.app" \
            "$desktop_dir/release/mac/Orion.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `orion desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading ORION_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if ORION_HOME="$ORION_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from orion_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `orion update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Orion-managed Node
            # provisioned during prerequisites/node-deps (at $ORION_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ORION_HOME.
    # $ORION_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'orion update' on this host install.

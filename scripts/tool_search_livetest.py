#!/usr/bin/env python3
"""Live test harness for Orion Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any orion imports.
ORIGINAL_HOME = os.environ.get("ORION_HOME")
ORIGINAL_AUTH = Path.home() / ".orion" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.orion/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.orion/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="orion_ts_live_"))
    orion_home = home_dir / ".orion"
    orion_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, orion_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".orion" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, orion_home / ".env")
        ...
        load_orion_dotenv(orion_home=str(Path.home() / ".orion"))
    ...
    (orion_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return orion_home
...
def reset_module_state():
    """Drop cached modules so the new ORION_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "orion_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ORION_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ORION_HOME"] = ORIGINAL_HOME
    else:

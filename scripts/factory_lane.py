...
MAX_BOOTSTRAP_POLICY_BYTES = 32 * 1024
_PROFILE_SLUG_RE = re.compile(r"^[a-z0-9](?:[a-z0-9-]{0,62})$")
_BRANCH_PREFIX_RE = re.compile(
    r"^[A-Za-z0-9](?:[A-Za-z0-9._-]*)(?:/[A-Za-z0-9](?:[A-Za-z0-9._-]*))*$"
)


def _load_bootstrap_policy(path, profile):
    """Load immutable local policy without following symlinks."""
    if not _PROFILE_SLUG_RE.fullmatch(profile or ""):
        raise RegistryError("bootstrap profile must be a lowercase safe slug")
    policy_path = Path(path)
    _reject_symlink_ancestors(policy_path)
    if policy_path.is_symlink():
        raise RegistryError("bootstrap policy must not be a symlink")
    try:
        st = policy_path.stat(follow_symlinks=False)
    except OSError as exc:
        raise RegistryError(f"bootstrap policy cannot be inspected: {exc}") from exc
    if not stat.S_ISREG(st.st_mode) or st.st_size > MAX_BOOTSTRAP_POLICY_BYTES:
        raise RegistryError("bootstrap policy must be a bounded regular file")
    if st.st_mode & 0o022:
        raise RegistryError("bootstrap policy must not be group/world writable")
    data = _bounded_json_file(policy_path, "bootstrap policy", MAX_BOOTSTRAP_POLICY_BYTES)
    if not isinstance(data, dict) or set(data) != {"profiles"}:
        raise RegistryError("bootstrap policy must contain only profiles")
    profiles = data["profiles"]
    if not isinstance(profiles, dict) or profile not in profiles:
        raise RegistryError(f"bootstrap profile {profile!r} is not configured")
    spec = profiles[profile]
    required = {"repo", "base_ref", "worktrees_parent", "branch_prefix"}
    if not isinstance(spec, dict) or set(spec) != required:
        raise RegistryError("bootstrap profile policy has unknown or missing fields")
    if any(not isinstance(spec[name], str) or not spec[name] for name in required):
        raise RegistryError("bootstrap policy fields must be non-empty strings")
    if not _BRANCH_PREFIX_RE.fullmatch(spec["branch_prefix"]) or ".." in spec["branch_prefix"]:
        raise RegistryError("bootstrap branch_prefix is invalid")

    repo = Path(spec["repo"])
    parent = Path(spec["worktrees_parent"])
    for candidate, label in ((repo, "repo"), (parent, "worktrees parent")):
        _reject_symlink_ancestors(candidate)
        if candidate.is_symlink() or not candidate.is_dir():
            raise RegistryError(f"bootstrap {label} must be an existing non-symlink directory")
    repo_real = _git_toplevel(repo)
    if repo_real != os.path.realpath(repo):
        raise RegistryError("bootstrap repo must be the canonical Git top-level")
    return {
        "repo": repo_real,
...
def _bootstrap_git(argv, timeout=30):
    """Run a fixed Git argv without a shell."""
    try:
        return subprocess.run(
            argv, capture_output=True, text=True, timeout=timeout, shell=False,
        )
    except (OSError, subprocess.TimeoutExpired) as exc:
        raise RegistryError(f"bootstrap git invocation failed: {exc}") from exc


def _bootstrap_ref_commit(repo, base_ref):
    result = _bootstrap_git(["git", "-C", repo, "rev-parse", "--verify", f"{base_ref}^{{commit}}"])
    sha = result.stdout.strip()
    if result.returncode != 0 or not _SHA40_RE.fullmatch(sha):
        raise RegistryError("bootstrap base_ref does not resolve to one commit")
    return sha


def _bootstrap_branch_exists(repo, branch):
    result = _bootstrap_git(
        ["git", "-C", repo, "show-ref", "--verify", "--quiet", f"refs/heads/{branch}"]
    )
    return result.returncode == 0


def _same_bootstrap_pending(owner, expected):
    fields = (
        "state", "bootstrap_nonce", "agent", "profile", "session_id", "pid",
        "process_start_time", "worktree", "branch", "base_sha",
    )
    return all(owner.get(field) == expected.get(field) for field in fields)


def _rollback_bootstrap(root, key, pending, repo, target, created):
    """Remove only this call's clean worktree/branch, then its exact pending owner."""
    with _registry_operation(root, "locks", "lanes") as root_anchor:
        with _dirfd_flock(root_anchor, (), ".worktree-admission.lock"), \
            ...
            current = _read_owner_via_chain(root_anchor, key)
            if current is None or not _same_bootstrap_pending(current, pending):
                return False
            branch_exists = _bootstrap_branch_exists(repo, pending["branch"])
            if (os.path.lexists(target) or branch_exists) and not created:
                # A non-zero add may have left partial artifacts, but it did not
                # prove which process created them. Preserve pending evidence.
                return False
            if os.path.lexists(target):
                if (
                    os.path.islink(target)
                    or _git_toplevel_or_none(target) != target
                    or _git_current_branch(target) != pending["branch"]
                ):
                    return False
                removed = _bootstrap_git(["git", "-C", repo, "worktree", "remove", target])
                if removed.returncode != 0 or os.path.lexists(target):
                    return False
            if branch_exists:
                try:
                    branch_sha = _bootstrap_ref_commit(
                        repo, f"refs/heads/{pending['branch']}",
                    )
                except RegistryError:
                    return False
                if branch_sha != pending["base_sha"]:
                    return False
                deleted = _bootstrap_git([
                    "git", "-C", repo, "branch", "-d", pending["branch"],
                ])
                if deleted.returncode != 0 or _bootstrap_branch_exists(repo, pending["branch"]):
                    return False
            _unlink_owner_via_chain(root_anchor, key)
            return True


def cmd_bootstrap(root, key, policy_path, profile, agent, session,
                  owner_pid, owner_start_time):
    """Atomically reserve, create, verify, and activate one deterministic worktree."""
    validate_key(key)
    spec = _load_bootstrap_policy(policy_path, profile)
    resolved_pid, resolved_start = _resolve_owner_identity(owner_pid, owner_start_time)
    if resolved_pid is None:
        raise RegistryError("bootstrap requires a transported live owner-pid")
    target = os.path.join(spec["parent"], f"{profile}-{key.lower()}")
    if os.path.dirname(target) != spec["parent"]:
        raise RegistryError("bootstrap target must be a direct child of the configured parent")
    branch = f"{spec['branch_prefix']}/{key.lower()}"
    base_sha = _bootstrap_ref_commit(spec["repo"], spec["base_ref"])
    nonce = secrets.token_hex(16)
    now = time.time()
    pending = _build_owner(
        agent, session, target, DEFAULT_TTL_HOURS, now, profile=profile,
        owner_pid=resolved_pid, owner_start_time=resolved_start,
    )
    pending.update({
        "state": "bootstrap_pending", "bootstrap_nonce": nonce,
    ...
    with _registry_operation(root, "locks", "lanes") as root_anchor:
        with _dirfd_flock(root_anchor, (), ".worktree-admission.lock"), \
                _dirfd_flock(root_anchor, ("locks", key), ".lock"):
            if _read_owner_via_chain(root_anchor, key) is not None:
                raise RegistryError(f"lane {key} already has an owner or pending bootstrap")
            if _find_claim_for_worktree(root_anchor, target) is not None:
                raise RegistryError("bootstrap target is already reserved")
            if os.path.lexists(target):
                raise RegistryError("bootstrap target already exists")
            if _bootstrap_branch_exists(spec["repo"], branch):
                raise RegistryError("bootstrap branch already exists")
            _write_owner_via_chain(root_anchor, key, pending)

    created = False
    try:
        added = _bootstrap_git([
            "git", "-C", spec["repo"], "worktree", "add", "-b", branch,
            target, base_sha,
        ])
        if added.returncode != 0:
            raise RegistryError(
                "git worktree add failed: " + (added.stderr.strip() or "unknown error")
            )
        created = True
        if _git_toplevel(target) != target or _git_current_branch(target) != branch:
            raise RegistryError("created worktree failed canonical path/branch verification")
        ...
        with _registry_operation(root, "locks", "lanes") as root_anchor:
            with _dirfd_flock(root_anchor, (), ".worktree-admission.lock"), \
                ...
                current = _read_owner_via_chain(root_anchor, key)
                if current is None or not _same_bootstrap_pending(current, pending):
                    raise RegistryError("bootstrap pending owner changed before activation")
                active = dict(current)
                active["state"] = "active"
                active.pop("bootstrap_nonce", None)
                _append_event_via_chain(root_anchor, key, "lane_claimed")
                # Activation is the final operation: no later failure may leave
                # an active owner while reporting bootstrap failure.
                _write_owner_via_chain(root_anchor, key, active)
    except Exception as exc:
        rolled_back = _rollback_bootstrap(
            root, key, pending, spec["repo"], target, created,
        )
        suffix = "rolled back" if rolled_back else "pending preserved for reconcile"
        if isinstance(exc, RegistryError):
            raise RegistryError(f"{exc} ({suffix})") from exc
        raise RegistryError(f"bootstrap failed: {exc} ({suffix})") from exc

    print(json.dumps({
        "key": key, "state": "active", "worktree": target,
        "branch": branch, "base_sha": base_sha,
    }, sort_keys=True))
    return 0
...
def evaluate_admission_guard(root, worktree_real, agent, session,
    ...
    if owner.get("state") == "bootstrap_pending":
        return False, f"worktree bootstrap for {key} is pending and cannot authorize mutation"
...
def release_owner_exact(root, key, *, agent, session, worktree, pid,
                        process_start_time):
    """Remove an owner only when every persisted identity field still matches."""
    validate_key(key)
    worktree_real = _canonical_worktree(worktree)
    expected = {
        "agent": agent,
    ...
    with _registry_operation(root, "lanes", "locks") as root_anchor:
        with _dirfd_flock(root_anchor, ("locks", key), ".lock"):
            ...
            if owner is None:
                return False
            if any(owner.get(field) != value for field, value in expected.items()):
                return False
            _unlink_owner_via_chain(root_anchor, key)
    return True
...
def _build_parser():
    ...
    p_bootstrap.add_argument("key")
    p_bootstrap.add_argument("--policy", required=True)
    p_bootstrap.add_argument("--profile", required=True)
    p_bootstrap.add_argument("--agent", required=True)
    p_bootstrap.add_argument("--session", required=True)
    p_bootstrap.add_argument("--owner-pid", type=int, required=True)
    p_bootstrap.add_argument("--owner-start-time")
    ...
    try:
        ...
        if args.command == "bootstrap":
            return cmd_bootstrap(
                root, args.key, args.policy, args.profile, args.agent, args.session,
                args.owner_pid, args.owner_start_time,
            )
        ...
        if args.command == "release-owner":
            released = release_owner_exact(
                root, args.key, agent=args.agent, session=args.session,
                worktree=args.worktree, pid=args.owner_pid,
                process_start_time=args.owner_start_time,
            )

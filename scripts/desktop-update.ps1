...
# (%NASTECH_HOME%\nastech-setup.exe). That binary has no self-update path --
# copy_self_to_nastech_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `nastech update` refreshes
...
#     -InstallRoot <path>   repo checkout (NASTECH_HOME\nastech-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Nastech.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .nastech-update-in-progress in place (tests)
#
...
# .nastech-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim NASTECH_HOME\.nastech-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately). nastech_cli/update_lock.py's ancestry rule lets our
# `nastech update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Nastech.exe pid.
try {
    Add-Type -Namespace NastechHandoff -Name Win32 -MemberDefinition @'
...
$NastechHome = Split-Path -Parent $InstallRoot
$MarkerPath = Join-Path $NastechHome ".nastech-update-in-progress"
$LogDir = Join-Path $NastechHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $NastechHome ".nastech-update-result.json"
$script:Ui = $null
...
function Show-ProgressWindow {
    ...
    try {
        ...
        $form.Text = "Nastech Update"
        $form.Size = New-Object System.Drawing.Size(720, 420)
        $form.StartPosition = "CenterScreen"
        $form.ControlBox = $false
        $form.TopMost = $true
        $label = New-Object System.Windows.Forms.Label
        $label.Text = "Updating Nastech -- do not close this window. Nastech restarts automatically when the update finishes."
        $label.Dock = "Top"
        $label.Height = 34
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [NastechHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Start-DesktopRelaunch {
    if ($RelaunchExe -and (Test-Path -LiteralPath $RelaunchExe)) {
        ...
        # DO NOT spawn Nastech.exe as our child: Electron/Chromium calls
        ...
        try {
            ...
            if ($r -and $r.ReturnValue -eq 0) {
                ...
                try {
                    if ($script:Win32) {
                        [NastechHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                        ...
                        while ((Get-Date) -lt $deadline) {
                            ...
                            if ($hwnd -ne [System.IntPtr]::Zero) {
                                [NastechHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                                [NastechHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
...
function Invoke-StreamedNastech([string]$Exe, [string[]]$NastechArgs, [string]$Tag) {
    ...
    $outFile = Join-Path $env:TEMP ("nastech-handoff-{0}-{1}.out" -f $Tag, $PID)
    $errFile = Join-Path $env:TEMP ("nastech-handoff-{0}-{1}.err" -f $Tag, $PID)
    ...
    $psi.Arguments = ($NastechArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    $psi.UseShellExecute = $false
    $psi.RedirectStandardOutput = $true
    $psi.RedirectStandardError = $true
    # nastech update prints UTF-8 (checkmarks, arrows, box glyphs). PS 5.1
    ...
    # drained at the end (nastech update is stdout-dominant).
...
try {
    ...
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Nastech window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Nastech fully and try again."
    ...
    $shim = Join-Path $InstallRoot "venv\Scripts\nastech.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Nastech install open (venv\Scripts\nastech.exe locked after 20s). Nothing was changed. Close other Nastech windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the nastech.exe shim guard, which step 2 just PROVED
    ...
    $nastechExe = Join-Path $InstallRoot "venv\Scripts\nastech.exe"
    if (-not (Test-Path -LiteralPath $nastechExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $nastechExe is missing. The install needs repair (run the Nastech installer or `nastech doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("update", "--yes", "--gateway", "--force", "--branch", $Branch)
    Write-HandoffLog ("running: nastech " + ($updateArgs -join " "))
    $res = Invoke-StreamedNastech $nastechExe $updateArgs "update"
    Write-HandoffLog "nastech update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all Nastech windows") is not retryable.
    ...
    # `nastech update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "nastech update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        $rebuild = Invoke-StreamedNastech $nastechExe @("desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `nastech desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "nastech update failed (exit $($res.Code)). See logs\desktop-update-handoff.log."
    }
    exit $finalCode
} finally {

...
measured in the production session DB) through `orion chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ORION_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Orion home with credentials for the
...
(tracker: Orion/orion-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "ORION_HOME": str(HOME),
                "ORION_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "orion_cli.main", "chat", "--query", q,

...
batch (tracker: [#77056](https://github.com/zacharyjleach-stack/Aries/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Orion home, same model, same tasks, same reps.
...
1. Create a dedicated Orion home with credentials for the models under test:
   ...
   integration at it; no Orion observability plugin needs to be enabled.

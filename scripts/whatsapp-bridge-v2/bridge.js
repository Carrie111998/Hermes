...
const INBOUND_CAPTURE_ALL_CHATS_RAW =
  process.env.WHATSAPP_CAPTURE_ALL_CHATS ??
  process.env.WHATSAPP_INBOUND_CAPTURE_CHATS;
const INBOUND_CAPTURE_ALL_CHATS = parseIdentifierList(INBOUND_CAPTURE_ALL_CHATS_RAW || '');
  ...
  async function enqueueMessages(messages, type, historyMeta = {}) {
    ...
    for (const msg of messages) {
      ...
      if (!msg.key.fromMe) {
        ...
        const captureAllInChat = isGroup && matchesAllowedChat(chatId, INBOUND_CAPTURE_ALL_CHATS, SESSION_DIR);
        if (!captureAllInChat && !matchesAllowedUser(senderId, ALLOWED_USERS, SESSION_DIR)) {
          try {
            console.log(JSON.stringify({
              event: 'ignored',
...
if (PAIR_ONLY) {
  ...
  app.listen(PORT, '127.0.0.1', () => {
    ...
    const inboundCaptureAll = Array.from(INBOUND_CAPTURE_ALL_CHATS);
    if (inboundCaptureAll.length > 0) {
      console.log(`📥 Inbound capture-all chats: ${inboundCaptureAll.join(', ')}`);
    }
    if (GROUP_CREATE_ENABLED) {

...
# files that finish in ~100s on a quiet box. The largest supported test file
# can exceed 300s at full local concurrency while finishing in 60-140s alone.
# The Docker build matrix jobs
...
def _run_one_file(
    ...
    *,
    ...
    file, rc, output, summary, subproc_wall = _run_one_file_once(
        file, pytest_args, repo_root, file_timeout, basetemp=basetemp
    )
    attempt = 0

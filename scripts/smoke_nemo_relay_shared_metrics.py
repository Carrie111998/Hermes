"""Run a real Orion CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_orion_executable(orion_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "orion",
        Path(".venv") / "Scripts" / "orion.exe",
    ):
        candidate = orion_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("orion")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Orion executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Orion source checkout containing .venv/bin/orion",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ORION_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["orion.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['orion.client.active']}"
        )
    [model] = by_name["orion.model_route.count"]
    expected_model = {
        "name": "orion.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['orion.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["orion.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['orion.task_run.started']}"
        )
    [terminal] = by_name["orion.task_run.finished"]
    ...
    [tool] = by_name["orion.tool_call.count"]
    ...
    lifecycle = by_name["orion.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["orion.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Orion development environment requires jsonschema"
        ) from exc
    ...
    if metrics["orion.client.active"] != [
        {
            "name": "orion.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['orion.client.active']}"
        )
    [model] = metrics["orion.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['orion.model_route.count']}"
        )
    [terminal] = metrics["orion.task_run.finished"]
    ...
    [tool] = metrics["orion.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["orion.skill.lifecycle.count"]
    ...
    loads = metrics["orion.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    orion_repo = args.orion_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    orion = _resolve_orion_executable(orion_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="orion-relay-shared-metrics-"))
    home = root / "orion-home"
    ...
    (root / "orion.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "orion.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Orion exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Orion model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Orion did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Orion did not print the mock model response")
    ...
    print("Orion -> NeMo Relay shared-metrics smoke test passed")

...
def main() -> int:
    ...
    if all_violations:
        print(f"ERROR: {len(all_violations)} subprocess calls missing stdin=:")
        for v in all_violations:
            snippet = v["snippet"].encode("ascii", errors="replace").decode("ascii")
            print(f"  {v['file']}:{v['line']}: {snippet}")
        if fix_mode:
            print("\nAdd stdin=subprocess.DEVNULL to each call above.")
        return 1
    else:
        print("OK: All TUI-context subprocess calls have explicit stdin=")
        return 0

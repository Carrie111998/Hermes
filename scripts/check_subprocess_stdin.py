...
When Orion runs in TUI mode, the gateway child process communicates with
...
# ``get_orion_home() / "plugins"`` (user) and ``./.orion/plugins/`` (project,
# gated behind ``ORION_ENABLE_PROJECT_PLUGINS``) — see
# ``orion_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``orion_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import orion_constants (this script
    ...
    # plugins in get_orion_home()/plugins/ and project plugins in
    # ./.orion/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_orion_home() / "plugins"]
    if os.environ.get("ORION_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".orion" / "plugins")

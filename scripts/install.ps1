...
# Repository JavaScript tooling requires $NodeVersion or newer. Returns $true
# when a `node --version` string clears that floor.
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old for the repository build (need Node.js $NodeVersion+)"
    }
...
function Install-Desktop {
    ...
    # enforces the repository Node floor and prepends the Hermes-managed

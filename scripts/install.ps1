# ============================================================================
# Orion Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$OrionHome = $(if ($env:ORION_HOME) { $env:ORION_HOME } else { "$env:LOCALAPPDATA\orion" }),
    [string]$InstallDir = $(if ($env:ORION_HOME) { "$env:ORION_HOME\orion-agent" } else { "$env:LOCALAPPDATA\orion\orion-agent" }),
    ...
    # builds apps/desktop into a launchable Orion.exe.
    #
    # Why opt-in:
    #   * Orion-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Orion.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Orion.exe
    ...
    #     `orion desktop` already builds on demand.
...
# ORION_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[orion] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'OrionInstall.LongPath').Type) {
            Add-Type -Namespace 'OrionInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [OrionInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -OrionHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('OrionHome')) {
    $OrionHome = ConvertTo-LongPath $OrionHome
} else {
    $OrionHome = ConvertTo-LongPath $(
        if ($env:ORION_HOME) { $env:ORION_HOME } else { "$env:LOCALAPPDATA\orion" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ORION_HOME) { "$env:ORION_HOME\orion-agent" } else { "$env:LOCALAPPDATA\orion\orion-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: OrionHome=$OrionHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:Orion/orion-agent.git"
$RepoUrlHttps = "https://github.com/zacharyjleach-stack/Aries.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Orion Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Orion.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $OrionHome)) {
        New-Item -ItemType Directory -Force -Path $OrionHome | Out-Null
    }
    $envFile = Join-Path $OrionHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $OrionHome "node"
...
function Install-Uv {
    # Orion owns its own uv at $OrionHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (orion_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `orion update` stay in sync.
    $managedUv = Join-Path $OrionHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $OrionHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $OrionHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $OrionHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $OrionHome "bin"
        ...
        # (orion_cli/managed_uv.py looks only at $OrionHome\bin\uv.exe).
...
# Put the Orion-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone orion-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Orion-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Orion-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("orion-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ORION_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Orion-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Orion setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Orion uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\orion\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Orion needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Orion copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\orion\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ORION_GIT_BASH_PATH`` (User scope) so Orion can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Orion-managed PortableGit install instead..."
    }

    # Download PortableGit into $OrionHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $OrionHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Orion features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Orion needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ORION_GIT_BASH_PATH`` (User env scope) so Orion can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $OrionHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $OrionHome\git\usr\bin\bash.exe
    $candidates += "$OrionHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$OrionHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ORION_GIT_BASH_PATH", $candidate, "User")
            $env:ORION_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ORION_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Orion may not find Git Bash."
    Write-Info "If needed, set ORION_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Orion requires Node >=26)"
    }

    # Prefer a Orion-managed Node from a previous run over a too-old system one.
    $managedNode = "$OrionHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$OrionHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$OrionHome\node"
        Write-Success "Node.js $version found (Orion-managed)"
        ...
        Update-ManagedNpm "$OrionHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Orion-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $OrionHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $OrionHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\orion-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$OrionHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$OrionHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$OrionHome\node") {
                    try {
                        Rename-Item "$OrionHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Orion-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$OrionHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $OrionHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\orion-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `orion
                    # update` path (#4735).
                    ...
                    $stashName = "orion-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. orion-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``orion update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Orion behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running orion processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/zacharyjleach-stack/Aries/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/zacharyjleach-stack/Aries/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/zacharyjleach-stack/Aries/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\orion-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\orion-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `orion update` (see the notes at the shared clone-path
    ...
    # next `orion update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Orion-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running orion process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running orion processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Orion_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (orion.exe) plus its child tree.
            & taskkill /F /T /IM orion.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM orion.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m orion_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not orion.exe.
            ...
            # Orion children re-exec through .orion-runtime, so killing only
            ...
            # user (see orion_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Orion " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `orion update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Orion (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `orion.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install orion-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and orion.exe broken with
    ...
    # here, not 30 seconds later when the user runs `orion`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Orion processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Orion processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Orion and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register orion.exe in dist-info/RECORD but fail to
        ...
        # `orion` command while orion-agent.exe / orion-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m orion_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `orion dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\orion_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `orion dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `orion dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: orion_cli/web_server.py"
        }
    }
...
function Install-OrionCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the orion launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring orion.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "orion.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the orion command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # orion_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("orion", "orion-acp")) {
    ...
    $requiredExe = Join-Path $Destination "orion.exe"
    $requiredCmd = Join-Path $Destination "orion.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the orion command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up orion command..."
    
    if ($NoVenv) {
        $orionBin = "$InstallDir"
    } else {
        # $OrionHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `orion update`'s autostash
        ...
        # installer staged under orion-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-OrionCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $orionBin = "$OrionHome\bin"
        Install-OrionCommandLaunchers -Root $InstallDir -Destination $orionBin | Out-Null
    }
    ...
    #   orion-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The orion-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept orion via $orionBin)"
        }
    }
    
    if ($currentPath -notlike "*$orionBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$orionBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $orionBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ORION_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\orion instead
    # of the Unix default ~/.orion
    $currentOrionHome = [Environment]::GetEnvironmentVariable("ORION_HOME", "User")
    if (-not $currentOrionHome -or $currentOrionHome -ne $OrionHome) {
        [Environment]::SetEnvironmentVariable("ORION_HOME", $OrionHome, "User")
        Write-Success "Set ORION_HOME=$OrionHome"
    }
    $env:ORION_HOME = $OrionHome
    
    # Update current session
    $env:Path = "$orionBin;$env:Path"
    
    Write-Success "orion command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.orion-bootstrap-complete which tells the Orion
    ...
    $markerPath = Join-Path $InstallDir ".orion-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ORION_HOME directory structure ($OrionHome, default %LOCALAPPDATA%\orion)
    New-Item -ItemType Directory -Force -Path "$OrionHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OrionHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OrionHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OrionHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OrionHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OrionHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OrionHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OrionHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OrionHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$OrionHome\.env"
    ...
    # Create config.yaml
    $configPath = "$OrionHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Orion's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$OrionHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in orion_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $OrionHome"
    
    # Seed bundled skills into $OrionHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $OrionHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $OrionHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$OrionHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $OrionHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Orion-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'orion setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\orion-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\orion-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\orion-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `orion tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $OrionHome "bin"
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Orion' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'orion tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `orion
...
# paths still lazy-install via install_cua_driver() (orion_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `orion computer-use install` runs. Bounded
        ...
        # Orion' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'orion tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: orion computer-use install"
            }
        } else {
...
# the final `electron` -> `Orion` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Orion.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_orion_desktop command
    # resolves apps/desktop/release/win-unpacked/Orion.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Orion-managed
    ...
    # The --dir mode produces an unpacked Orion.exe in
    ...
    # are NOT trying to work around). The Orion icon + product name are
    # stamped onto Orion.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\orion-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Orion.exe",
        "$desktopDir\release\win-arm64-unpacked\Orion.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Orion.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Orion icon + identity are stamped onto Orion.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, orion-agent#38216).
    ...
    #    Orion.exe. We deliberately do NOT point them at `orion desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Orion.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Orion.lnk')
        )
        ...
        # with the Orion icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.orion/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'orion setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m orion_cli.main setup
    } else {
...
function Start-GatewayIfConfigured {
    ...
    $orionCmd = "$InstallDir\venv\Scripts\orion.exe"
    if (-not (Test-Path $orionCmd)) {
        $orionCmd = "orion"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$OrionHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'orion whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: orion gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$OrionHome\logs\gateway.log"
            Start-Process -FilePath $orionCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$OrionHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: orion gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: orion gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$OrionHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$OrionHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$OrionHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$OrionHome\orion-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   orion              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   orion setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   orion config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   orion config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   orion gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   orion update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Orion repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Orion-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Orion to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://your-orion-docs.example/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
